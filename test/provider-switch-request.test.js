...
test("a coordinator-authorized switch clears terminal empty-turn processing state", function () {
  var h = makeHarness({
    ...
    history: [
      ...
      {
        type: "user_message",
        text: "Preserve this queued delivery",
        queueId: "q-after-empty",
        queuedPending: true,
        _ts: 201,
      },
    ],
  ...
  assert.strictEqual(h.switches.length, 1);
  assert.strictEqual(h.switches[0].preserveQueuedMessages, true);
  assert.strictEqual(h.session.pendingUserMessageQueue.length, 1);
});

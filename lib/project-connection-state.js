...
function findByRequestedId(sessions, requestedId) {
  ...
  var localId = parseInt(requestedId, 10);
...
function findRestoredActiveSession(options) {
  var active = findByRequestedId(options.sessions, options.requestedSessionId);
  ...
  return { active: active, storedPresence: options.storedPresence };
}

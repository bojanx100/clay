...
function initialModelForSession(session, sm, route, models) {
  ...
  if (model && route && !require("./coop-model-policy").appliesToSession(session) &&
      !modelInList(models, model)) return firstModel(models);
  return model;
}
...
function selectInitialModelState(options) {
  ...
  models = require("./coop-model-routing").modelsForSession(session, vendor, route && route.id, models);
...
function serializeSessionListEntry(session, options) {
  return Object.assign(
    { executionAuthority: require("./read-only-execution").forClient(session, options.sessionManager) },

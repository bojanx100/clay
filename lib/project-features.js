...
function attachCleanupForProject(ctx) {
  ...
  runtime.resumeLeadAfterPortfolioTerminal = function () {
    if (!leadWakeHandler) return false;
    return leadWakeHandler({ leadMode: leadModeEnabled() });
  };
...
function attachProjectFeatures(ctx) {
  ...
  var taskOrchestrator = attachTaskOrchestrator({
    cwd: cwd,
    getLeadMode: leadMode.getLeadMode,
    slug: slug,
  ...
  var userMessage = attachUserMessage({
    ...
    resolveCoordinatorUpdates: taskOrchestrator.resolveCoordinatorUpdates,

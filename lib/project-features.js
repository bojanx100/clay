...
var { attachCoopSelfCleanupRuntime } = require("./coop-self-cleanup-runtime");
...
function attachCleanupForProject(ctx) {
  ...
  function leadModeEnabled() {
    var ownerId = typeof ctx.getProjectOwnerId === "function" ? ctx.getProjectOwnerId() : null;
    if (!ownerId && typeof usersModule.getAllUsers === "function") {
      var users = usersModule.getAllUsers();
      if (users.length === 1 && users[0]) ownerId = users[0].id;
    }
    return !!ownerId && typeof usersModule.getLeadMode === "function" &&
      usersModule.getLeadMode(ownerId) === true;
  }
  ...
  var runtime = attachCoopSelfCleanupRuntime({
    ...
    intervalMs: ctx.coopSelfCleanupIntervalMs,
    now: ctx.coopSelfCleanupNow,
...
function attachProjectFeatures(ctx) {
  ...
  var userMessage = attachUserMessage({
    ...
    onUserMessageDispatched: function (session, text) {
      ...
      var reconciliationDirective = taskOrchestrator.resumeWaitingCoordinator(session, text);
      ...
      if (taskLauncher && typeof taskLauncher.handleTaskUserMessageDispatched === "function") {
        var taskDirective = taskLauncher.handleTaskUserMessageDispatched(session, text);

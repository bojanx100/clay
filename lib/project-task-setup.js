...
// Handles gh account/repo discovery and scaffolds recipes, prompts, config,
// TRIAGE.local.md, and the local dashboard prompt.
...
function attachTaskSetup(ctx) {
  ...
  // This wizard only produces issue recipes (kind: "issue"). A recipe already on
  // disk with a PR-review source must never be overwritten by it, or the PR-fix
  // flow silently turns into a duplicate issue launcher. Detect those by kind.
  function isPrReviewRecipeId(recipeId) {
  ...
  function handleState(ws, msg) {
    ...
    // Never default to a PR-review recipe — this is an issue wizard, and saving
    // would clobber it. An explicit request is still honored (read-only), but the
    // save handler refuses to overwrite it.
    var defaultId = (al.recipeId && !isPrReviewRecipeId(al.recipeId)) ? al.recipeId : "";
    if (!defaultId) {
      for (var ri = 0; ri < recipes.length; ri++) {
        if (!isPrReviewRecipeId(recipes[ri].id)) { defaultId = recipes[ri].id; break; }
      }
    }
    var recipeId = requested || defaultId || "assigned-to-me";
    var recipe = readJson(path.join(tasksDir, recipeId + ".json"));
  ...
  function fetchLabels(repo, env, cb) {
    ghText(["label", "list", "--repo", repo, "--limit", "200", "--json", "name", "--jq", ".[].name"], env, function (e, raw) {
      if (e) { cb([]); return; }
      cb(raw.split("\n").map(function (s) { return s.trim(); }).filter(function (s) { return !!s; }));
    });
  }
  ...
  function fetchCollaborators(repo, env, cb) {
    ghText(["api", "repos/" + repo + "/collaborators?per_page=100", "--jq", ".[].login"], env, function (e, raw) {
      if (e) { cb([]); return; }
      cb(raw.split("\n").map(function (s) { return s.trim(); }).filter(function (s) { return !!s; }));
    });
  }

  function discoverBoards(repo, env, cb) {
    var parts = String(repo || "").split("/");
    if (parts.length !== 2 || !parts[0] || !parts[1]) {
      cb({ ok: false, error: "Repo must be in owner/name form (e.g. trialview/v2)", boards: [] });
      return;
    }
    ghText([
      "api", "graphql",
      ...
      if (e) {
        ...
        cb({ ok: false, error: msg.trim().split("\n").slice(0, 3).join(" "), boards: [] });
        return;
      }
      try {
        out = JSON.parse(raw);
      } catch (parseErr) {
        cb({ ok: false, error: (parseErr && parseErr.message) || "Failed to parse GitHub response.", boards: [] });
        return;
      }
  ...
  function handleDiscover(ws, msg) {
    ...
    if (!account) { try { account = resolveGhAccount(cwd, null, {}) || ""; } catch (e) {} }
    var env = ghEnv(cwd, account);
    discoverBoards(repo, env, function (res) {
      if (!res.ok) {
        reply(ws, { type: "task_setup_boards", ok: false, error: res.error || null, repo: repo, account: account, boards: [], labels: [], collaborators: [] });
        return;
      }
      var labels = [];
      var collaborators = [];
      ...
      function done() {
        pending--;
        if (pending > 0) return;
        reply(ws, { type: "task_setup_boards", ok: true, error: null, repo: repo, account: account, boards: res.boards, labels: labels, collaborators: collaborators });
      }
      fetchLabels(repo, env, function (items) { labels = items; done(); });
      fetchCollaborators(repo, env, function (items) { collaborators = items; done(); });
    });
  }
  ...
  function doScaffold(ws, msg, opts) {
    ...
    // Refuse to overwrite a PR-review recipe with an issue recipe. This wizard
    // only builds kind:"issue" recipes; clobbering a PR-fix recipe turns it into
    // a second issue launcher that double-launches every assigned issue.
    if (isPrReviewRecipeId(cfg.recipeId)) {
      reply(ws, { type: "task_setup_result", ok: false, settingsOnly: settingsOnly,
        error: "'" + cfg.recipeId + "' is a PR-review recipe and can't be edited by the issue setup wizard. Choose a different recipe id (e.g. assigned-to-me)." });
      return;
    }
    var written = [];
    var warnings = [];

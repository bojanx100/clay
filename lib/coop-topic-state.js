...
function coopTopicState(topicRef, options) {
  ...
  if (!tasks.length) {
    // Foreground work on this exact lens is real linked work even before a task
    // exists -- the owner is watching Coop answer them right now.
    return foreground ? { state: STATE_WORKING, taskCount: 0 } : { state: "", taskCount: 0 };
  }

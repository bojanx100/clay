...
//   Done         -- the work is complete AND the owner accepted it, the owner
//                   explicitly accepted the topic, or the owner confirmed the
//                   topic closed.
//
...
// requires durable owner acceptance evidence, and everything unproven is
// Needs input with an inspectable provenance record (stateSource) saying
...
function coopTopicState(topicRef, options) {
  ...
  if (!tasks.length) {
    if (foreground) {
      ...
      return { state: STATE_WORKING, taskCount: 0, foreground: true, stateSource: "foreground" };
    }
    ...
    if (closedConfirmed) {
      return { state: STATE_DONE, taskCount: 0, foreground: false, stateSource: "topic_closed" };
    }
    if (disposition) {
      return {
        state: disposition.status === "done" ? STATE_DONE : STATE_NEEDS_INPUT,
        taskCount: 0, foreground: false,
        stateSource: "owner_disposition:" + (disposition.source || "recorded"),
      };
    }
    // No linked work, no disposition, not closed: an unresolved historical
    // topic. Never blank and never assumed Working -- the owner has to decide
    // what this is, which is exactly what Needs input means.
    return { state: STATE_NEEDS_INPUT, taskCount: 0, foreground: false, stateSource: "unlinked_default" };
  }
  ...
  if (completed > 0 && accepted >= completed) {
    return { state: STATE_DONE, taskCount: tasks.length, stateSource: "task_accepted" };
  }
  if (completed > 0) {
    return { state: STATE_NEEDS_INPUT, taskCount: tasks.length, awaitingAcceptance: true, stateSource: "task_awaiting_acceptance" };
  }
  // Linked tasks exist but none is running, blocked, or completed -- e.g. all
  // dismissed. Nothing here proves resolution, so the owner decides.

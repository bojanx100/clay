...
//   Done         -- a linked Coop execution completed, the owner accepted a
//                   regular task or topic, or the owner confirmed the topic
//                   closed.
//
...
// requires completed execution or durable owner resolution evidence, and
...
//
// A completed binding names a completed Coop-created project session, so its
// linked topic receives the same green Done indicator. This only derives the
// display state; closing the topic remains the owner's explicit action.
...
function linkedBindings(bindings, topicRef) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    linked.push({ coopTopicRef: binding.coopTopicRef, status: status,
      ownerAcceptance: null, completedExecution: status === "completed" });
  }
  return linked;
}
...
function coopTopicState(topicRef, options) {
  ...
  // A raw orchestration task remains an implementation milestone until the
  // owner accepts it. A completed Coop execution binding is different: it is
  // the durable projection of the linked project session and therefore shares
  // that session's green Done indicator.

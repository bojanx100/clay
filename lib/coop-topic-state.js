...
//
// `completed` maps to `completed` and bindings carry no ownerAcceptance, so a
// completed binding is awaiting acceptance by construction -- which is precisely
// the approved default that completed linked work never auto-closes a topic.
...
function linkedBindings(bindings, topicRef) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    if (!status) continue;
    linked.push({ coopTopicRef: binding.coopTopicRef, status: status, ownerAcceptance: null });
  }
  return linked;
}

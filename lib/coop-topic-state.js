...
function coopTopicState(topicRef, options) {
  ...
  if (!tasks.length) {
    // Every topic gets one of the three owner-facing states. Returning "" here
    // is why the sidebar rendered an identical dot on all 41 topics and no text
    // at all: no topic in the real index has linked canonical work, so the label
    // was never populated and "silence beats a false label" became "silence,
    // always". A topic with no linked task is an open conversation that is not
    // blocked on the owner and has not been accepted, which is exactly Working.
    // Foreground work on this exact lens is the same state for a stronger
    // reason -- the owner is watching Coop answer them right now.
    return { state: STATE_WORKING, taskCount: 0, foreground: foreground };
  }

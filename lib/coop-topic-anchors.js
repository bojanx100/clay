...
function proveAnchor(history, ref) {
  ...
  var segment = items ? { length: items.length } : lineage.segmentFor(history, sessionId);
  if (!segment) return { proven: false, reason: REASON_NO_HISTORY, fingerprint: null };
  var canonicalIndex = candidateIndex(ref, ANCHOR_OFFSETS[0], segment.length);
  if (canonicalIndex === null) {
    return { proven: false, reason: REASON_OUT_OF_RANGE, fingerprint: null };
  }
  for (var oi = 0; oi < ANCHOR_OFFSETS.length; oi++) {
    var index = candidateIndex(ref, ANCHOR_OFFSETS[oi], segment.length);
    if (index === null) continue;
    var resolved = lineage.recordAt(history, sessionId, index);
  ...
  return {
    ...
    fingerprint: anchorFingerprint((lineage.recordAt(history, sessionId, canonicalIndex) || {}).record),
  };
}

...
function attachProjectInteractions(ctx) {
  ...
  var debate = attachDebate({
    ...
    updateMemorySummary: memory.updateMemorySummary,
    initMemorySummary: memory.initMemorySummary,
    onCoopPlanningFinished: function (session) {
      if (ctx.planningGate && ctx.planningGate.planning) ctx.planningGate.planning.finished(session);
    },
  });
  checkForDmDebateBrief = debate.checkForDmDebateBrief;

  if (ctx.planningGate && slug === "lead") ctx.planningGate.planning = require("./coop-planning").attachCoopPlanning({
    slug: slug, sm: sm, debate: debate,
    canStart: function (session) { return !!ctx.planningGate.canPlan && ctx.planningGate.canPlan(session); },
    queueUpdate: function (session, text, options) {
      return ctx.planningGate.queueUpdate && ctx.planningGate.queueUpdate(session, text, options);
    },
    delegate: function (input) { return ctx.planningGate.delegate(input); },
  });
  ...
  return {
    ...
    handleDebatePauseToggle: debate.handleDebatePauseToggle,

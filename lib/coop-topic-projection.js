...
function publicTopic(topic, options, deps) {
  ...
  var planningSessions = relatedSessionLinks({ relatedExecutions: topic.relatedPlanning }, options, deps && deps.normalizeExecution);
  ...
  return {
    ...
    relatedSessions: relatedSessions.concat(planningSessions),
    executionProjectRefs: executionProjects,
  };
}

function hasTurnStart(topic, ref) {
  var refs = Array.isArray(topic && topic.turnRefs) ? topic.turnRefs : [];
  for (var i = 0; i < refs.length; i++) {
    if (refs[i].sessionStorageId === ref.sessionStorageId &&
        refs[i].startEventIndex === ref.startEventIndex) return true;
  }
  return false;
}

function hasConnectedTopic(index, ref) {
  var ids = Object.keys(index && index.topics || {});
  for (var i = 0; i < ids.length; i++) {
    if (ids[i] === "uncategorised-conversations") continue;
    var topic = index.topics[ids[i]];
    if (topic && topic.status !== "merged" && hasTurnStart(topic, ref)) return true;
  }
  return false;
}

// The catch-all is a temporary triage state, not a permanent Thread. Keep it
// visible only when a meaningful owner turn still lacks a non-catch-all
// connection. Noise-only membership stays in the durable index for audit, but
// cannot claim a sidebar row. Without history we retain the old reference-only
// projection so administrative inspection still sees the seed.
function hasUnresolvedCatchAllTriage(index, topic, history) {
  ...
  for (var i = 0; i < refs.length; i++) {
    ...
    if (!record || !topicRelevance.isOwnerRelevantRecord(record)) continue;
    if (!topicClassification.namesNewSubject(record.text || "")) continue;
    if (!hasConnectedTopic(index, ref)) return true;
  }
  return false;
}
...
function projectTopics(index, options, deps) {
  ...
  for (var i = 0; i < topics.length; i++) {
    ...
    if (topic.topicRef && topic.topicRef.topicId === "uncategorised-conversations" &&
        !hasUnresolvedCatchAllTriage(index, topic, options && options.history)) continue;

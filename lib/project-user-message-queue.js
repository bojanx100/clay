...
function coopIngressFromHistory(session, item, deps) {
  ...
  if (!item.coopIngressPreparedText) finalText = coopForegroundIngress.withTopicContext(finalText, item.coopTopicRef, item.coopProjectRef, item.coopTopicAnchor);
...
function hasCoopIngressDispatchBlocker(session) {
  return !session || !!(session.destroying || session.isProcessing || session.queryInstance ||

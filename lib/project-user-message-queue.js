...
function sendPreparedToSdk(session, args, deps) {
  ...
  session._messageDispatchFailureHandled = false;

  function failDispatch(error) {
    if (session._messageDispatchFailureHandled) return;
    session._messageDispatchFailureHandled = true;
    session.isProcessing = false;
    deps.onProcessingChanged();
    var text = "Failed to start query: " + ((error && error.message) || error || "unknown provider error");
    if (deps.sm && typeof deps.sm.sendAndRecord === "function") {
      deps.sm.sendAndRecord(session, { type: "error", text: text });
      deps.sm.sendAndRecord(session, { type: "done", code: 1 });
    } else {
      deps.sendToSession(session.localId, { type: "error", text: text });
      deps.sendToSession(session.localId, { type: "done", code: 1 });
    }
    if (deps.sm && typeof deps.sm.broadcastSessionList === "function") deps.sm.broadcastSessionList();
  }

  function startFreshQuery() {
    try {
      var started = deps.sdk.startQuery(session, finalText, args.images, deps.ensureProjectAccessForSession(session));
      if (started && typeof started.catch === "function") started.catch(failDispatch);
    } catch (error) {
      failDispatch(error);
    }
  }

  // Acceptance may precede a queued turn by minutes. Voice must attach its
  // reply to actual dispatch, never the preceding worker/Coop turn's output.
  if (!require("./voice-turn-replay").recordStart(session, args.clientMessageId, deps)) {
    deps.sendToSession(session.localId, { type: "message_failed", clientMessageId: args.clientMessageId,
      text: "Clay could not save this turn's dispatch. It was not started. Please retry." });
    return;
  }
  session._turnQueuedAt = args.queuedAt || Date.now();
  session._queryStartTs = session._turnQueuedAt;
  if (!session.isProcessing) {
    ...
    if (!session.queryInstance && !session._queryStarting &&
      ...
      startFreshQuery();
    } else if (deps.sdk.pushMessage(session, finalText, args.images) === false) {
      startFreshQuery();
    }
  } else if (deps.sdk.pushMessage(session, finalText, args.images) === false) {
    startFreshQuery();
...
function dispatchPreparedToSdk(session, args, deps) {
  ...
  if (require("./coop-settled-discussion").dispatch(session, args, deps)) return;
...
function attachProjectUserMessageQueue(ctx) {
  ...
  var deps = {
    sm: sm,
    crossProject: ctx.crossProject || ctx.opts && ctx.opts.crossProject,
    sdk: ctx.sdk,

...
function screenshotFromPayload(payload) {
  ...
  var data = typeof screenshot.data === "string" ? screenshot.data : "";
  var validBase64 = data.length > 0 &&
    data.length <= MAX_SCREENSHOT_BASE64 &&
    data.length % 4 === 0 &&
    /^[A-Za-z0-9+/]+={0,2}$/.test(data);
  var decoded = validBase64 ? Buffer.from(data, "base64") : null;
  if (screenshot.mediaType !== "image/png" || !decoded ||
      decoded.length < 8 ||
      decoded.subarray(0, 8).toString("hex") !== PNG_SIGNATURE) {
    var error = new Error("Live UI screenshots must be a bounded PNG capture");
    error.code = "LIVE_UI_SCREENSHOT_INVALID";
    throw error;
  }
  ...
  function handleMessage(ws, msg, pairing, selection) {
    ...
    try {
      handled = userMessage.handleUserMessage(ctx.pairControlWs(pairing), {
        ...
        images: screenshot ? [screenshot] : null,
      });
    } catch (error) {

...
// "get me Ward", "open the session working on X", and "go to that worker".
function isDirectHandoffAsk(text) {
  var ask = clean(text);
  return /\b(?:get me|open|go to|take me to|show me|switch to)\b/i.test(ask) &&
    /\b(?:session|worker|agent|ward)\b/i.test(ask);
}
...
function preflight(input, result) {
  ...
  if (!isDirectHandoffAsk(input.ask)) return finish(result, VERDICTS.UNMEASURABLE, ["NOT_DIRECT_HANDOFF_ASK"]);
  var state = resolutionState(input);
  if (state === "unmeasurable") return finish(result, VERDICTS.UNMEASURABLE, ["MISSING_RUNTIME_EVIDENCE"]);

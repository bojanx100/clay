...
// Working Topics do not necessarily have replayable canonical history, but the
// topic projection already carries its ACL-filtered, openable project sessions.
// Reuse that exact reference rather than sending the owner through an empty
// topic replay, which would otherwise restore the canonical Coop home.
function relatedSessionRef(topic) {
  var sessions = Array.isArray(topic && topic.relatedSessions) ? topic.relatedSessions : [];
  for (var i = 0; i < sessions.length; i++) {
    var ref = sessions[i] && sessions[i].sessionRef;
    if (ref && ref.projectId && ref.sessionStorageId) {
      return { projectId: ref.projectId, sessionStorageId: ref.sessionStorageId };
    }
  }
  return null;
...
function entryFor(topic, kind, reason, action) {
  ...
  var sessionRef = exactSessionRef(item) || relatedSessionRef(topic);
  var entry = {
  ...
  if (item) {
    ...
    if (item.controlRole === "council" || item.controlRole === "triage") {
      entry.controlRole = item.controlRole;
    }
  }
  if (sessionRef) entry.sessionRef = sessionRef;
  return entry;
}

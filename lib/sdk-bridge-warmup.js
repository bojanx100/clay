...
function attachBridgeWarmup(ctx) {
  ...
  function detectInstalledVendors(linuxUser) {
    ...
    var vendorNames = Object.keys(installedMap);
    for (var vi = 0; vi < vendorNames.length; vi++) {
      var vendor = vendorNames[vi];
      var info = yoke.getVendorInfo(vendor);
      if (installedMap[vendor] && result.indexOf(vendor) === -1 &&
          !(linuxUser && info && info.osUserIsolation === false)) {
        result.push(vendor);
      }
    }
    }
  ...
  async function warmup(linuxUser) {
    ...
    sm.providerRoutes = listProviderRoutes(sm.availableVendors, sm.installedVendors, sm);

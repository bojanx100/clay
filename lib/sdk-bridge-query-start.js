...
function attachBridgeQueryStart(ctx) {
  ...
  async function prepareVendor(vendor, linuxUser) {
    ...
    if (vendorAdapter) {
      ...
      if (!sm.modelsByVendor[vendor] && typeof vendorAdapter.supportedModels === "function") {
        ...
        var resolvedModels = modelCatalogCache.applyDiscovery(vendor, discoveredModels);
        sm.modelsByVendor[vendor] = vendor === "claude" ? withClaudeFallbackModels(resolvedModels) : resolvedModels;
      }
    }
  ...
  async function startQuery(session, text, images, linuxUser) {
    ...
    if (session.vendor && !adapters[session.vendor]) {
      // Force a real auth re-check (invalidate the module-global cache and
      // re-run `<vendor> login status`). Without force this returns the stale
      // cache captured at daemon start, so a vendor logged in AFTER startup
      // looks logged out. Earlier clients could then auto-run the login
      // command, which for Codex deletes the prior auth.json before a new
      // device flow completes. Forcing here prevents that false positive.
      var freshAuth = getFreshAuthState(true, linuxUser);
    ...
    if (session.vendor && !adapters[session.vendor]) {
      ...
      var authState = getFreshAuthState(false, linuxUser);

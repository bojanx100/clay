...
function pushMessage(ctx, session, text, images, options) {
  ...
  if (canPush) {
    ...
    var queueElapsed = session._performanceTurn.queueMs || 0;
    console.log("[PERF] provider_dispatch session=" + session.localId + " turn=" +
      turnSeq + " queue+" + Math.max(0, queueElapsed) + "ms mode=warm");
    return pushControlInput(ctx, session, session.queryInstance, text, images) !== false;
  }
  if (session._queryStarting && !(options && options.requireImmediate)) {
...
function failedControlledPreparation(ctx, session, error, fence) {
  ...
  if (error && error.code === "COOP_TOP_TIER_UNAVAILABLE") {
    return { ok: false, reason: coopModelPolicy.UNAVAILABLE_CODE, submission: "not_submitted" };
  }
  return { ok: false, reason: "provider_start_failed", submission: "not_submitted" };
}
...
function attachBridgeQueryStart(ctx) {
  ...
  function enforcePreparedCoopRoute(session, prepared) {
    if (!coopModelPolicy.appliesToSession(session)) return;
    var decision = coopModelPolicy.resolveTarget({
      vendor: session.vendor || null,
      providerRouteId: session.providerRouteId || null,
      model: prepared && prepared.model || null,
    });
    if (!decision.ok) throw coopModelPolicy.createUnavailableError(decision);
  }

  async function startQueryInner(session, text, images, linuxUser) {
    if (typeof ctx.applyEnvironmentRefresh === "function") {
      await ctx.applyEnvironmentRefresh();
    }
    try {
    ...
    try {
      ...
      enforcePreparedCoopRoute(session, prepared);
      executionFence.assertAction(session, "provider_start", controlledFence);
      var query = queryOptions.buildQueryOptions(startCtx, session, text, linuxUser,
        controlledFence, prepared);
      return launchQuery(startCtx, session, provider.adapter, query, query.text, images,
  ...
  return {
    ensureVendorReady: ensureVendorReady,
    pushMessage: function (session, text, images, options) {
      if (readOnlyExecution.isReadOnly(session, sm) && session.queryInstance &&
          session.queryInstance._clayReadOnlyExecution !== true) return false;
      return pushMessage(ctx, session, text, images, options);
    },

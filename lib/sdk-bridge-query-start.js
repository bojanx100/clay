...
function pushMessage(session, text, images, options) {
  ...
  if (canPush) {
    ...
    return session.queryInstance.pushMessage(text, images) !== false;
  }
...
function attachBridgeQueryStart(ctx) {
  ...
  return {
    ...
    pushMessage: function (session, text, images, options) {
      ...
      return pushMessage(session, prepareControlTurn(ctx, session, text).text, images, options);
    },

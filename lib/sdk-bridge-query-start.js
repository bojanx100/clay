...
function attachBridgeQueryStart(ctx) {
  ...
  async function startQuery(session, text, images, linuxUser) {
    ...
    if (session.vendor && !adapters[session.vendor]) {
      ...
      // looks logged out — and the client would then auto-run the login
      // command, which for codex DELETES the valid auth.json. Forcing here
      // recovers the adapter and prevents that destructive false positive.

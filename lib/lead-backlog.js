...
// Reduce any GitHub repo reference (slug, https URL, ssh URL, trailing .git)
// to a comparable lowercase "owner/name" slug. "" means unusable, which always
// fails closed — an unparseable origin must never be treated as ownership.
//
// The host, when present, must be github.com at a real boundary: a greedy
// `.*github\.com` would let "evilgithub.com/trialview/v2" compare EQUAL to the
// genuine repo, which is ownership evidence being forged. Anything that is not
// a bare slug or a github.com URL yields "" rather than a lucky partial parse.
var REPO_SLUG_RE = /^(?:(?:[^\s]*[/@])?github\.com[:/])?([\w.-]+)\/([\w.-]+)$/;

function normalizeRepoSlug(value) {
  if (typeof value !== "string") return "";
  var trimmed = value.trim().replace(/^\/+|\/+$/g, "").replace(/\.git$/i, "");
  if (!trimmed) return "";
  var match = trimmed.match(REPO_SLUG_RE);

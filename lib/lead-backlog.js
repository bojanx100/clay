...
function ghIssueArgs(sourceSpec) {
  ...
  // Assignee parity with the task launcher (project-task-sources.js:217): the
  // launcher treats "any" as NO assignee restriction, and issueAssignedTo()
  // returns true for it. Passing it through as a login produced
  //   gh issue list --assignee any
  // which fails with "Could not find an assignee with the login 'any'", so a
  // canonical all-issues recipe collected nothing at all. "me" maps to "@me";
  // every other non-empty value is a real login.
...
// Identity types supported for the (project, id) dedupe key, and their
// encoding. A work item is only deduplicable if BOTH parts are scalar
// identities we can compare exactly.
//
// The token is type-tagged so no two supported values can ever encode alike:
// number 1, string "1", boolean true and BigInt 1n stay four distinct items,
// and null and undefined stay distinct (plain JSON.stringify folds undefined
// into null, and throws outright on a BigInt — a single odd id would abort the
// whole portfolio build).
//
// For numbers the token follows Object.is exactly: 0 and -0 encode differently
// (they are different identities), Infinity and -Infinity differ, and all NaNs
// encode alike (Object.is(NaN, NaN) is true, so they are one identity).
//
// Objects, arrays, functions and symbols are NOT identities: two structurally
// equal objects are indistinguishable once encoded, so treating them as keys
// would silently merge unrelated work. They return null, and the caller fails
// closed by skipping the item and counting it, rather than guessing.
function identityToken(value) {
  if (value === null) return "null";
  var type = typeof value;
  if (type === "string") return "s:" + value;
  // Object.is, not String(): String(-0) is "0", so -0 and 0 would encode alike
  // and one item would be dropped. The whole point of this token is that no
  // two distinct supported values collide, so that has to hold here too.
...
// The encoded (project, id) pair, or null when either part is not an identity.
// Both tokens are strings, so JSON encoding the pair is unambiguous — unlike
// any delimiter join, which can be forged across the boundary.
...
function collectPortfolioItems(collections, now, health, byProject, seenIds, stats) {
  ...
  for (var ci = 0; ci < (collections || []).length; ci++) {
    ...
    for (var ii = 0; ii < (col.items || []).length; ii++) {
      ...
      //
      // The pair is type-tagged and JSON-encoded, not joined by a delimiter:
      // ANY delimiter can be forged across the boundary. Joining on NUL made
      // ("a<NUL>b", "c") and ("a", "b<NUL>c") collapse to one key, silently
      // dropping a real item. An item whose project or id is not a scalar
      // identity cannot be compared at all, so it is skipped and counted —
      // fail closed and observable, never a guessed key.
      var dedupeKey = dedupeKeyFor(item);
      if (dedupeKey === null) { stats.unidentifiable++; continue; }
...
function buildPortfolio(collections, opts) {
  ...
  return {
    ...
    summary: {
      ...
      // Items skipped because their project/id was not a scalar identity, so
      // they could not be compared for duplication. Surfaced, never silent.

...
function ledgerImplementationRoute(ctx, source, input) {
  ...
  for (var i = 0; i < entries.length; i++) {
    ...
    if (scopeCoversInput(entry.implementationScope, input)) matched.push(entry);
  }
  if (matched.length === 1) {
  ...
  if (candidates.length === 1 && !candidates[0].implementationScope) {
...
function scopeCoversInput(scope, input) {
  var projectRef = requestedProject(input);
  if (!scope || !projectRef) return false;
  var requestedScope = {
    projectRef: projectRef,
  ...
  return exact || ownerRequestsModule.carryForwardEligible(scope, requestedScope);
}
...
// this binding revision or an earlier one it can carry forward from. When the
...
// revision R also covers the same project's same task at a revision strictly
// greater than R. An earlier version of this comment claimed the whole function
...
//   approved revision ended terminal-unsuccessful, and that NO revision of the
//   task has ever completed. A route the carry-forward has not earned is still
//   refused there with `owner_implementation_scope_mismatch`.
//
...
//   it does not exercise the never-completed guard.
//
// The identity-and-monotonicity half of the rule is NOT reimplemented here. It is
// `ownerRequestsModule.carryForwardEligible` -- the same predicate

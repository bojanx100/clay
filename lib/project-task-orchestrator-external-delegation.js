...
function currentExecutionRoute(ctx, source, input) {
  ...
  // Consulted BEFORE the backward owner-turn scan below. Owner decision, ingress
  // 692, after this was measured live on 2026-08-23.
  //
  // The scan can select a turn that is unable to authorize anything and, by
  // returning it, hide the routes underneath. Live: a read_only_diagnosis
  // dispatch of clay-review-external-codex-recent-commits rev2 from session
  // 871a194b adopted ingress :595, whose text is "Do both".
  // isReadOnlyReviewIngress matches that turn -- and matches on the DISPATCH
  // being read-only, not on anything about the turn's relevance -- while
  // isImplementationIngress does not, which skips both the coverage guard below
  // and the Thread mint below. The route came back with an ingress and a null
  // coopTopicRef, implementationAdmission refused on its first line, and the
  // grant four routes further down was never reached. 595 authorizes nothing:
  // expectsExecution false, no decision, no scope. The grant covered the work
  // and needed no owner turn at all.
  //
  // This grants NOTHING new. standingGrantExecutionRoute proposes a container
  // only; implementationAdmission re-derives the grant independently from the
  // same policy file, so a route offered here is still refused there unless the
  // policy really covers the dispatch. Switch off, project not allowlisted, or a
  // shape that is not fully read-only all return {} and leave the scan to run
  // exactly as before -- so OFF stays byte-identical, which is property 1 of
  // coop-autonomy-grant.
  //
  // What it does change is ATTRIBUTION, which is the accepted cost: for the
  // narrow read_only_diagnosis class the grant now takes credit ahead of the
  // scan, and ahead of the queue and approval routes below it, so such work runs
  // under a deterministic grant:<taskId> Thread instead of an owner turn's. The
  // ledger route above still wins, because a task-scoped owner record is exact
  // evidence for this binding and is better attribution than policy.
  //
  // Only a grant that actually MINTED its Thread may preempt the scan. A mint
  // refusal keeps its original position at the end of this function: promoting a
  // refusal would let a failed policy read shadow a legitimate owner turn, which
  // is the same shadowing defect in the other direction.
  var granted = standingGrantExecutionRoute(ctx, input);
  if (granted && granted.coopTopicRef) return granted;
  ...
  // Reused, not recomputed: the successful case already returned above, so all
  // that can remain here is a mint refusal, kept at its original position so a
  // failed policy read cannot shadow the owner-turn routes above.
  if (granted && granted.coopThreadMintRefusal) return granted;

...
function hasProjectExecutionInput(input) {
  ...
  var fields = ["targetProject", "targetProjectId", "portfolioTaskId",
    "bindingRevision", "idempotencyKey", "implementationGrantRef", "mode", "controlRole"];
  for (var i = 0; i < fields.length; i++) {
    if (Object.prototype.hasOwnProperty.call(input, fields[i])) return true;
  }
  return false;
}
...
// A Governance Lifecycle grant is deliberately opt-in during the shadow-write
// rollout. Existing bindings keep their compatibility path, while any caller
// that names a grant is held to its exact task/project/revision/idempotency
// scope before a target coordinator can be created.
function governanceGrantProblem(ctx, input) {
  var grantId = String(input && input.implementationGrantRef || "").trim();
  if (!grantId) return null;
  var lifecycle = ctx && ctx.governanceLifecycle;
  if (!lifecycle || typeof lifecycle.executionAdmission !== "function") {
    return "governance lifecycle is unavailable";
  }
  var admitted = lifecycle.executionAdmission({ grantId: grantId,
    portfolioTaskId: input.portfolioTaskId, bindingRevision: input.bindingRevision,
    idempotencyKey: input.idempotencyKey, targetProject: requestedProject(input) });
  if (admitted && admitted.ok === true) return null;
  return "governance lifecycle grant refused: " + String(admitted && admitted.code || "invalid_grant");
}
...
function isImplementationIngress(item) {
  return item && item.type === "user_message" &&
    implementationIntent.implementationDecisionForEvent(item);
}

function isReadOnlyReviewIngress(item, input) {
  return item && item.type === "user_message" &&
    !implementationIntent.hasUnresolvedProject(item) &&
...
function approvalExecutionRoute(ctx, source, input) {
  // A named approval is the one admission route whose evidence may predate a
  // provider compaction. The helper follows only the successor's explicit,
  // bounded predecessor chain and otherwise returns current history unchanged.
  var history = itemApproval.approvalHistoryFor(source, ctx.sm && ctx.sm.sessions);
...
function ledgerImplementationRoute(ctx, source, input) {
  ...
  for (var i = 0; i < entries.length; i++) {
    ...
    if (!event || event.type !== "user_message" ||
        event.coopIngressId !== entry.ingressId ||
        implementationIntent.hasUnresolvedProject(event)) continue;
...
// The review counterpart, reading the separate reviewScopes store that
// coop-owner-requests.scopeReview writes. Never unioned with the implementation
// scopes above: a read-only acknowledgement must not become implementation
// authority just because both are "scopes".
function ownerRequestReviewScopes(ctx, ingressId) {
  var ledger = ctx.ownerRequests || ownerRequestsModule.getDefaultOwnerRequests();
  if (!ingressId || !ledger || typeof ledger.get !== "function") return null;
  var entry;
  try { entry = ledger.get(ingressId); }
  ...
  if (!entry) return [];
  return typeof ownerRequestsModule.reviewScopesFor === "function" ?
    ownerRequestsModule.reviewScopesFor(entry) : [];
}
...
function unscopedIngressCoverage(ctx, item, input, index, latestIndex) {
  ...
  // A read-only review turn cannot have an implementation scope -- writing one
  // would make "do them" implementation authority -- so it is covered by its own
  // reviewScopes store instead. Reachable only when the DISPATCH is already
  // provably read-only, because that is what isReadOnlyReviewIngress checks, and
  // review admission can only ever return reviewOnly work.
  //
  // This is what makes the coverage guard applicable to the review branch at all,
  // and therefore what closes the :595 class: a turn that never had a review
  // dispatched against this task has no scope naming it, so it can no longer
  // claim the route. A turn that genuinely authorized this review does, so
  // plural "do them" work keeps resolving after the owner has moved on.
  if (isReadOnlyReviewIngress(item, input)) {
    var reviewScope = scopeCoveringInput(
      ownerRequestReviewScopes(ctx, item && item.coopIngressId), input);
    if (scopeCoversInput(reviewScope, input)) {
      return { ok: true, topicRef: reviewScope.topicRef || null };
    }
  }
...
//
// Policy categories that need no owner turn route here. Revision bumps still
// resolve their prior scope by TopicRef, while read-only diagnosis and ordinary
// internal Clay/Coop work receive their deterministic policy Thread here.
function standingGrantExecutionRoute(ctx, source, input) {
  var projectRef = requestedProject(input);
  var taskId = String(input && input.portfolioTaskId || "").trim();
  if (!projectRef || !taskId) return {};
  var staged = source && Array.isArray(source.orchestrationTasks) ? source.orchestrationTasks : [];
  for (var i = 0; i < staged.length; i++) {
    if (staged[i] && staged[i].status === "waiting_user") return {};
    if (staged[i] && String(staged[i].clientRef || "") ===
        "portfolio:" + taskId + ":" + Number(input && input.bindingRevision)) return {};
  }
  var admitted;
  try {
    admitted = autonomyGrant.standingAdmission(Object.assign({}, input, {
  ...
  if (minted.topicRef) return { coopTopicRef: minted.topicRef, coopStandingAutonomy: true };
  return minted.code ? { coopThreadMintRefusal: minted.code, coopStandingAutonomy: true } : {};
}

// The container for an owner approval that was REFERENTIAL.
//
// When the owner answers a question Coop asked ("do 1 and 2", "both", "yes"),
// the answering turn is a real owner ingress -- so unlike the standing grant
// this path has an ingress to mint a deterministic Thread against, and no
// synthetic key is needed.
//
// Proposes only. `pendingQuestion.executionAdmission` re-derives the pending
// question and the assent independently in implementationAdmission, so a route
// this function offers is still refused there unless the owner really answered a
// question that named this exact binding.
function answeredQuestionExecutionRoute(ctx, source, input) {
  var projectRef = requestedProject(input);
  if (!projectRef || !String(input && input.portfolioTaskId || "").trim()) return {};
  var request = {
    targetProject: projectRef,
    portfolioTaskId: String(input.portfolioTaskId).trim(),
    bindingRevision: Number(input && input.bindingRevision),
  };
  var admitted;
  try {
    admitted = pendingQuestion.executionAdmission(input, request, source);
  } catch (error) {
    return {};
  }
  if (!admitted || admitted.ok !== true || !admitted.answeredQuestion) return {};
  var ingressId = String(admitted.answeredQuestion.ingressId || "");
  if (!ingressId) return {};
  var minted = ownerThreadRefFor(ctx, input, ingressId);
  if (minted.topicRef) {
    return { coopTopicRef: minted.topicRef, coopIngressId: ingressId };
  }
  return minted.code ? { coopIngressId: ingressId,
    coopThreadMintRefusal: minted.code } : {};
}
...
function currentExecutionRoute(ctx, source, input) {
  ...
  // The standing grant was briefly consulted HERE, ahead of the scan (owner
  // decision, ingress 692), because a turn that could authorize nothing was
  // claiming the route and hiding it. That was the narrow repair: it rescued the
  // read_only_diagnosis class only, and it cost attribution, because a grant
  // placed here also preempts the queue and approval routes below.
  //
  // It is back in its original position (owner decision, ingress 693). The cause
  // is now fixed at its source instead: the coverage guard below applies to review
  // ingresses too, so a turn like :595 that never had a review dispatched against
  // this task is skipped by the scan rather than returned from it. With nothing
  // shadowing them, queue, approval and the grant are each reached in their
  // original order and keep their original precedence.
  ...
  for (var i = items.length - 1; i >= 0; i--) {
    ...
    var coverage = null;
    // Applies to REVIEW ingresses too, which is the whole of the 2026-08-23
    // repair. This condition used to read `!requested && isImplementationIngress(item)`,
    // so a review-shaped turn entered the scan at the line above and then skipped
    // the only bound on it -- no recency requirement and no scope requirement --
    // which is the unbounded backward scan the 2026-08-19 ":482 / FIX!" fix was
    // meant to close, left half-wired. unscopedIngressCoverage now resolves review
    // turns against their own reviewScopes store, so this is passable for them
    // rather than vacuous.
    if (!requested) {
  ...
  var answered = answeredQuestionExecutionRoute(ctx, source, input);
  if (answered && (answered.coopTopicRef || answered.coopThreadMintRefusal)) return answered;
  var granted = standingGrantExecutionRoute(ctx, source, input);
  if (granted && (granted.coopTopicRef || granted.coopThreadMintRefusal)) return granted;
  return {};
}

function coordinateProjectExecution(ctx, input) {
  if (controlRole.isPeer(input.controlRole)) return { ok: false,
    error: "Council and Triage are planning debates in Coop. Use start_coop_planning, then " +
      "commission_coop_plan for a completed synthesis within authorized scope." };
  if (!ctx.createProjectExecution) return { ok: false, executionNotStarted: true, error: "Cross-project execution is unavailable" };
  ...
  if (!source || source.orchestrationParent || executionMetadata(source)) {
    return { ok: false, executionNotStarted: true, error: "Source Coop session is unavailable" };
  }
  ...
  if (problem) return { ok: false, executionNotStarted: true, error: problem };
  var grantProblem = governanceGrantProblem(ctx, input);
  if (grantProblem) return { ok: false, executionNotStarted: true, error: grantProblem };
  var classified = Object.assign({}, input);
  var role = controlRole.forExecution(input);
  // Legacy display-title inference must not turn an ordinary review into a
  // Council/Triage project execution. Those roles now belong to Coop debates.
  if (controlRole.isPeer(role)) classified.controlRole = "project_coordinator";
  if (readOnlyReviewAdmission.isReadOnlyPlanningReview(input)) classified.reviewOnly = true;
  var result = ctx.createProjectExecution(Object.assign({}, classified,
    currentExecutionRoute(ctx, source, classified), {
    source: projectIdentity.sessionRef({ projectId: ctx.projectId() }, source),
  }));
  function closeApprovedPlaceholder(value) {
    var taskId = value && String(value.approvalTaskId || "").trim();
    if (!taskId || typeof ctx.closeTask !== "function") return value;
    var parent = ctx.sessionForInput({ coordinatorSessionId: input.coordinatorSessionId });
    if (parent) ctx.closeTask(parent, taskId, "Owner approval accepted and execution dispatched");
    return value;
  }
  if (result && typeof result.then === "function") {
    return result.then(closeApprovedPlaceholder);
  }
  return closeApprovedPlaceholder(result);
}
...
function coordinateLocalExternalTask(ctx, input) {
  ...
  var problem = localDelegationProblem(coordinator);
...
module.exports = {
  ...
  governanceGrantProblem: governanceGrantProblem,

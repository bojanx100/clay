...
function warmupConnection(ctx, warmedUp) {
...
function sendUserPreferenceState(ctx, ws, wsUser) {
  ...
  if (usersModule && typeof usersModule.getLeadMode === "function") {
    var leadUserId = (wsUser && wsUser.id) || null;
    ctx.sendTo(ws, { type: "lead_mode_changed", leadMode: leadUserId ? usersModule.getLeadMode(leadUserId) : false });
  }
}
...
function sendSessionList(ctx, ws, snapshot) {
  ctx.sendTo(ws, {
    ...
    sessions: connectionState.serializeSessionList(snapshot.allSessions, {
      ...
      activeSessionId: ctx.sm.activeSessionId,

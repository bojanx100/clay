...
var ownerSidebarConnection = require("./coop-owner-sidebar-connection");
...
function ensureInitialVendorState(ctx) {
  ...
  sm.providerRoutes = require("./provider-routes").listProviderRoutes(
...
function sendInfoState(ctx, ws, snapshot) {
  ctx.sendTo(ws, {
    ...
    icon: typeof ctx.getIcon === "function" ? ctx.getIcon() : null,
...
function sendModelState(ctx, ws, snapshot) {
  ...
  ctx.sendTo(ws, {
    ...
    providerRoutes: sm.providerRoutes || require("./provider-routes").listProviderRoutes(sm.availableVendors || [], sm.installedVendors || [], sm),
  });
}
...
function sendInitialState(ctx, ws, wsUser, snapshot) {
  ...
  var getLastVendor = ctx.onGetProjectLastVendor || (ctx.opts && ctx.opts.onGetProjectLastVendor);
  var lastVendor = typeof getLastVendor === "function"
    ? getLastVendor(ctx.slug, snapshot.userId) : null;
  ctx.sendTo(ws, { type: "last_vendor", vendor: (lastVendor && lastVendor.vendor) || "" });
...
function sessionSwitchedProvider(ctx, active) {
  ...
  return {
    executionAuthority: require("./read-only-execution").forClient(active, sm),
    ...
    // hasHistory is a boolean, but `active.history` on a released lazy session
    // re-reads and parses the whole transcript to produce it -- and this runs
    // BEFORE replayRestoredSession, so it materialised the very history the
    // paged replay exists to avoid reading. The paging was being defeated one
    // line before it ran.
    hasHistory: historyStore.historyLength(active) > 0,
    capabilities: capabilities,
  };
}

function sessionSwitchedRuntime(ctx, active, ws) {
  var sm = ctx.sm;
  return {
    ...
    coopConversationState: coopConversationControl.clientStateFor(ctx, active, ws),
  };
}
...
function sendSessionSwitched(ctx, ws, active) {
  ctx.sendTo(ws, Object.assign(
    ...
    sessionSwitchedRuntime(ctx, active, ws),
...
function replayRestoredSession(ctx, ws, active) {
  ...
  var replay = coopTopicConnection.replayTopicSelection(ctx, ws, active) || coopMainReplay.forSession(ctx, active, ws);
  ctx.sm.replayHistory(replay && replay.replaySession || active, undefined, ws, replay && replay.transform || ctx.hydrateImageRefs, replay ? replay.options : null);
...
function handleSocketMessage(ctx, ws, raw, handleMessage) {
  ...
  if (!msg) return;
  try {
    if (coopTopicConnection.handleCoopMessage(ctx, ws, msg)) return;
    if (ownerSidebarConnection.handleOwnerSidebarMessage(ctx, ws, msg)) return;
...
function handleDisconnection(ctx, ws) {
  if (ctx.humanAttention && typeof ctx.humanAttention.disconnect === "function") ctx.humanAttention.disconnect(ws);

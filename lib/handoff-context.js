...
function buildHandoffContextFromHistory(history, options) {
  ...
  var header = "<clay_handoff_context>\n" +
    ...
    "You are joining an ongoing conversation mid-stream, not starting a new one. In your first response, do not introduce yourself, do not greet the user, do not announce or describe the handoff, and do not narrate that you have read or absorbed this context (no \"I can see you were working on...\"). Answer the current message directly, in the same working rhythm as the transcript, as if you had been in the conversation all along.\n" +
    "</handoff_instructions>";
  if (opts.cwd) header += "\nProject: " + opts.cwd;
...
function applyHandoffToOutgoingText(session, fullText) {
  ...
  var wrapped = session.handoffContext +
    ...
    fullText +
    "\n</current_user_message>\n\n" +
    "Answer only the <current_user_message> above. Use <clay_handoff_context> only as prior reference. Reply mid-conversation: no self-introduction, no handoff acknowledgement.";
  ...
  turns--;
  if (turns <= 0) {
    // Consumed at send time — but a turn can still FAIL (empty response,
    // stream error). Stash the context so restoreHandoffAfterFailedTurn can
    // re-arm it; a successful turn clears the stash in finalize. Matters most
    // for github-copilot, whose budget of 1 otherwise has zero retry headroom.
    session._handoffContextStash = session.handoffContext;
...
// Cap on post-consumption restores. Two failed-turn retries on top of the
// budgeted sends, then the handoff stays consumed for good.
var HANDOFF_MAX_FAILED_TURN_RESTORES = 2;

// Re-arm a handoff that was consumed by a send whose turn then FAILED (no
// real output). Without this, the last budgeted send — the ONLY send for
// github-copilot — burned the context on an error and the next turn started
// blank. Returns true when the context was restored.
function restoreHandoffAfterFailedTurn(session) {
  if (!session || !session._handoffContextStash) return false;
  var restores = session._handoffFailedTurnRestores || 0;
  if (restores >= HANDOFF_MAX_FAILED_TURN_RESTORES) {
    session._handoffContextStash = null;
    return false;
  }
  session._handoffFailedTurnRestores = restores + 1;
  session.handoffContext = session._handoffContextStash;
  session._handoffContextStash = null;
  session.handoffContextTurnsRemaining = 1;
  session.handoffContextConsumed = false;
  // Re-arm the copilot choreography: the retry's native session receives the
  // transcript wrapper again and must be dropped after its turn too.
  if (session.vendor === "github-copilot") session.copilotHandoffNativeReset = false;
  return true;
}
...
function finalizeHandoffAfterSuccessfulTurn(session) {
  ...
  // Success also settles any stashed context from the consuming send — the
  // failed-turn restore path must not fire after the vendor answered.
  session._handoffContextStash = null;
  session._handoffFailedTurnRestores = 0;
  if (!session.handoffContext) return false;
  session.handoffContext = null;
  session.handoffContextTurnsRemaining = 0;
  session.handoffContextConsumed = true;
  return true;
...
module.exports = {
  ...
  restoreHandoffAfterFailedTurn: restoreHandoffAfterFailedTurn,

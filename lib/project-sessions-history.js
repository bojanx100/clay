...
function replayableItems(items, hydrateImageRefs) {
  return sessionHistory.orderQueuedHistoryItems(
    items.filter(sessionHistory.isReplayableHistoryItem)).map(hydrateImageRefs);
}
...
function attachProjectSessionsHistory(ctx) {
  ...
  function handleHistoryMessage(ws, msg) {
    if (msg.type === "load_more_history") {
      ...
      if (canRangePage) {
        ...
        if (Array.isArray(window)) {
          ...
          sendTo(ws, {
            ...
            items: replayableItems(window.slice(relativeFrom, to - windowFrom), hydrateImageRefs),
      ...
      var to = before;
      var items = replayableItems(history.slice(from, to), hydrateImageRefs);

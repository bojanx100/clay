...
function normalizeClaim(input) {
  ...
  return {
    error: "", projectId: ref.projectId, key: key, holder: holder, ttlMs: value.ttlMs,
...
function persistedLease(value) {
  ...
  // Optional: identifies the OS process holding the claim, so a successor can
  // distinguish "a sibling is still working on this" from "the holder died".
  if (Number.isInteger(value.holderPid) && value.holderPid > 0) {
    record.holderPid = value.holderPid;
  }
  return record;
}

// Fail closed: no state at all is an empty store, but anything unreadable or
// unrecognizable poisons every mutation instead of being silently discarded.
function parseState(fsImpl, target, epoch) {
  try {
    var parsed = JSON.parse(fsImpl.readFileSync(target, "utf8"));
    ...
    return {
      ok: true,
      state: { schema: SCHEMA, version: SCHEMA_VERSION, epoch: epoch, leases: leases },
    };
  } catch (e) {
    // Only genuine parse/read failures mean corrupt state. A programming
    // error here must surface, not masquerade as a corrupt store and
    // silently disable every mutation.
...
function loadState(fsImpl, file) {
  var epochs = epochsOnDisk(fsImpl, file);
  if (epochs.length) return parseState(fsImpl, epochPath(file, epochs[0]), epochs[0]);
  // Legacy single-file layout from before the epoch protocol. Read it as
  // epoch 0 so the first commit publishes epoch 1 and takes over cleanly.
  if (fsImpl.existsSync(file)) return parseState(fsImpl, file, 0);
  return { ok: true, state: emptyState() };
}

// --- Serializable commit protocol ---------------------------------------------
//
// A check-then-rename can never be serializable: however carefully we verify
// ownership or an epoch first, a successor can commit in the window before our
// rename lands, and the rename then silently overwrites them. Holding a
// lockfile does not help either, because a lock that can be broken as stale is
// a lock a stalled holder can lose without noticing.
//
// So the commit itself is made atomic instead. State lives in per-epoch files
// (`claims.json.<N>`), and publishing epoch N+1 means CREATING that file with
// O_EXCL. Exactly one writer can win that create — it is a single atomic
// syscall — and every loser gets EEXIST and retries against fresh state. A
// stalled holder that wakes up late simply finds its target epoch already
// taken and aborts; it has no path by which it can overwrite or remove a
// successor's commit, because it never writes to a path a successor owns.
//
// Readers take the highest epoch present. Older epochs are pruned after a
// successful commit, and pruning is best-effort: a leftover file is harmless
// because it is strictly older than the winner.
var EPOCH_RE = /\.(\d+)$/;
var EPOCH_KEEP = 2;
...
// Every committed epoch currently on disk, newest first.
function epochsOnDisk(fsImpl, file) {
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    if (Number.isInteger(epoch) && epoch >= 0) found.push(epoch);
  }
  found.sort(function (a, b) { return b - a; });
  return found;
}

function epochPath(file, epoch) {
  return file + "." + epoch;
}

function pruneEpochs(fsImpl, file, keepAbove) {
  var epochs = epochsOnDisk(fsImpl, file);
  for (var i = EPOCH_KEEP; i < epochs.length; i++) {
    if (epochs[i] >= keepAbove) continue;
    try { fsImpl.unlinkSync(epochPath(file, epochs[i])); } catch (e) {}
  }
}

// Publish `state` as epoch `epoch`. Returns ok only when THIS writer created
// that epoch; a loser gets claim_store_conflict and must retry against the
// state the winner published.
function commitEpoch(fsImpl, file, state, epoch) {
  var directory = path.dirname(file);
  var target = epochPath(file, epoch);
  var descriptor = null;
  try {
    fsImpl.mkdirSync(directory, { recursive: true, mode: 0o700 });
    // The whole concurrency story is this one flag.
    descriptor = fsImpl.openSync(target, "wx", 0o600);
  } catch (e) {
  ...
  try {
    var payload = Object.assign({}, state, {
      schema: SCHEMA, version: SCHEMA_VERSION, epoch: epoch,
    });
    fsImpl.writeFileSync(descriptor, JSON.stringify(payload, null, 2) + "\n", "utf8");
    fsImpl.fsyncSync(descriptor);
    ...
    // Our own epoch file, so removing it cannot affect anyone else.
    try { fsImpl.unlinkSync(target); } catch (unlinkError) {}
    return { ok: false, reason: "persistence_failed" };
  }
  var synced = syncDirectory(fsImpl, directory);
  if (!synced.ok) return synced;
  pruneEpochs(fsImpl, file, epoch);
...
function createClaimLeases(options) {
  ...
  // Publish the current in-memory state as the next epoch. A loser of the
  // O_EXCL race gets claim_store_conflict; `mutate` turns that into a retry.
  function save() {
    if (loadError) return { ok: false, reason: loadError };
    var committed = commitEpoch(fsImpl, file, state, (state.epoch || 0) + 1);
    if (!committed.ok) return committed;
    state.epoch = committed.epoch;
    return { ok: true };
  }

  // Read-modify-write against freshly loaded state. On a lost commit race the
  // whole decision is redone against what the winner published — retrying the
  // WRITE alone would republish a decision made from stale state.
  function mutate(fn) {
  ...
  function acquireLocked(claim) {
    ...
    if (claim.holderPid) record.holderPid = claim.holderPid;
    if (index === -1) {
  ...
  // Restart-safe adoption WITHOUT sharing an identity between processes.
  //
  // Making the holder unique per process is what stops two concurrent
  // processes from releasing each other's work, but it also means a restarted
  // process no longer recognizes its predecessor's claims. Adoption closes
  // that gap on evidence rather than on a timer: a claim may be taken over
  // only when it has expired, or when `isHolderAlive(pid)` proves the holding
  // process is gone. A live sibling's claim is never touched.
  function adopt(input, isHolderAlive) {
    if (loadError) return { ok: false, reason: loadError };
    var claim = normalizeClaim(input);
    if (claim.error) return { ok: false, reason: claim.error };
    return mutate(function () { return adoptLocked(claim, isHolderAlive); });
  }

  function adoptLocked(claim, isHolderAlive) {
    var timestamp = now();
    var index = leaseIndex(state.leases, claim.projectId, claim.key);
    var existing = index === -1 ? null : state.leases[index];
    if (existing && existing.holder !== claim.holder && !isExpired(existing, timestamp)) {
      // A holder we cannot prove dead is presumed alive: adoption must never
      // be the mechanism that steals live work.
      var pid = existing.holderPid;
      var alive = typeof isHolderAlive === "function" && Number.isInteger(pid) && pid > 0
        ? isHolderAlive(pid) !== false
        : true;
      if (alive) return { ok: false, reason: "held", lease: clone(existing) };
    }
    var record = {
      projectId: claim.projectId,
      key: claim.key,
    ...
    return { ok: true, adopted: !!existing, lease: clone(record) };
  }

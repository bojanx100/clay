...
// First lines of the mention-session prompts. Exported so the CLI-session
// import list can recognize and hide debate worker sessions — without this,
// every debate leaks moderator/panelist rows titled by these strings into
// the import panel (owner report, 2026-08-02).
var DEBATE_MODERATOR_PROMPT_PREFIX = "You are moderating a structured debate among your AI teammates.";
var DEBATE_PANELIST_PROMPT_PREFIX = "You are participating in a structured debate as a panelist.";

function isDebateWorkerPrompt(text) {
  if (!text || typeof text !== "string") return false;
  var t = text.trim();
  // The prompt may arrive with injected instructions above it; check both
  // a clean prefix and containment of the exact first line.
  return t.indexOf(DEBATE_MODERATOR_PROMPT_PREFIX) !== -1
    || t.indexOf(DEBATE_PANELIST_PROMPT_PREFIX) !== -1;
}

function buildModeratorContext(ctx, debate) {
  var lines = [
    DEBATE_MODERATOR_PROMPT_PREFIX,
    "",
    "Topic: " + debate.topic,
...
module.exports = {
  ...
  isDebateWorkerPrompt: isDebateWorkerPrompt,
  DEBATE_MODERATOR_PROMPT_PREFIX: DEBATE_MODERATOR_PROMPT_PREFIX,
  DEBATE_PANELIST_PROMPT_PREFIX: DEBATE_PANELIST_PROMPT_PREFIX,

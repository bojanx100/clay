...
function attachLiveUiReports(ctx) {
  ...
  function checkReports() {
    ...
    pairReports.forEach(function (records, pairingId) {
      ...
      records.forEach(function (record) {
        ...
        var identityChanged = !!task &&
          (record.workerSessionId !== (task.workerSessionId || null) ||
           record.workerColor !== task.workerColor);
        if (task) {
          record.workerSessionId = task.workerSessionId || null;
          record.workerColor = task.workerColor || record.workerColor;
        }
        if (presentation.status !== record.status ||
            presentation.message !== record.message || identityChanged) {
  ...
  function continueReport(input) {
    ...
    var result = ctx.followUpTask({
      ...
      imageRefs: [{ mediaType: input.screenshot.mediaType, file: input.imageFile }],
  ...
  function createReport(input) {
    var result = ctx.coordinateExternalTask({
      ...
      clientRef: "live-ui:" + input.pairing.pairingId + ":" + input.clientMessageId,
      imageRefs: [{ mediaType: input.screenshot.mediaType, file: input.imageFile }],
    });
    if (!result || !result.ok) {
  ...
  function approve(ws, msg, pairing) {
    ctx.assertExtensionSender(ws, pairing);
    ...
    var accepted = ctx.registry.acceptMessage(
      ...
      function () {
        ...
        if (taskPresentation(task).status !== "completed") {
          var statusError = new Error("Only a completed Live UI change can be approved");
          statusError.code = "LIVE_UI_REPORT_NOT_COMPLETE";
          throw statusError;
        }
        if (!ctx.closeTask(pairing, session, record.taskId, "Approved in Live UI")) {
          var closeError = new Error("Clay could not archive the approved worker");
          closeError.code = "LIVE_UI_REPORT_APPROVE_FAILED";
          throw closeError;
        }
        recordsFor(pairing.pairingId).delete(reportId);

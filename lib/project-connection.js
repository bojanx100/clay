...
function orchestrationSessionFields(session) {
  return {
    ...
    orchestrationParent: session.orchestrationParent ? {
...
function attachConnection(ctx) {
  ...
  function handleConnection(ws, wsUser, handleMessage, handleDisconnection) {
    ...
    // Initial per-user Lead (CTO orchestrator) opt-in state. Off by default.
    if (usersModule && typeof usersModule.getLeadMode === "function") {
      var _lmUid = (wsUser && wsUser.id) || null;
      sendTo(ws, { type: "lead_mode_changed", leadMode: _lmUid ? usersModule.getLeadMode(_lmUid) : false });
    }
    ...
    if (active && !autoCreated) {
      ...
      sendTo(ws, Object.assign({ type: "session_switched", id: active.localId, title: active.title || null, cliSessionId: active.cliSessionId || null, loop: active.loop || null, vendor: active.vendor || null, providerRouteId: active.providerRouteId || null, requestedModel: active.requestedModel || active.model || null, verifiedModel: active.verifiedModel || null, modelVerificationSource: active.modelVerificationSource || null, automationMode: automationForSession(active, sm.currentPermissionMode || "default", getCodexConfig(sm, active)), permissionMode: active.permissionMode || null, codexApproval: active.codexApproval || null, codexSandbox: active.codexSandbox || null, codexWebSearch: active.codexWebSearch || null, hasHistory: (active.history && active.history.length > 0), capabilities: _vendorCaps, isProcessing: !!active.isProcessing, mode: active.mode || "gui", terminalId: typeof active.terminalId === "number" ? active.terminalId : null, runtimeMode: active.runtimeMode || null, runtimeTerminalId: typeof active.runtimeTerminalId === "number" ? active.runtimeTerminalId : null, tuiSuspended: !!active.tuiSuspended, queueingDisabled: !!active.queueingDisabled, queuedUserMessages: sm.queuedUserMessagesForClient ? sm.queuedUserMessagesForClient(active) : [], orchestrationTasks: orchestrationTasksForClient(active) }, orchestrationSessionFields(active)));
    ...
    // F-7: pending debate proposals live only in daemon memory and their
    // approval cards render only from the live tool event — after a refresh
    // the card sits outside the bounded history-replay window and the
    // proposal becomes effectively unclickable. Re-send every still-pending
    // proposal to each (re)connecting client so the card is always present
    // while the proposal is alive.
    if (pendingDebateProposals) {
      for (var dpKey in pendingDebateProposals) {
        var dpPending = pendingDebateProposals[dpKey];
        if (dpPending && dpPending.briefData) {
          sendTo(ws, { type: "debate_proposal_pending", briefData: dpPending.briefData });
        }
      }
    }
    ...
    ws.on("message", function (raw) {
      ...
      // A synchronous throw in any message handler must not escape to the
      // WebSocket 'message' event: from there it reaches uncaughtException
      // and tears down the whole daemon, killing every session over one bad
      // message. Observed 2026-07-31: an MCP-proposed debate approved from a
      // project session crashed the daemon on "Start debate". Log loudly,
      // tell the sender, keep the daemon alive.
      try {
        handleMessage(ws, msg);
      } catch (err) {
        var _hmLine = "[WS-HANDLER-ERROR] " + new Date().toISOString() +
          " type=" + (msg && msg.type) + " " + (err && err.stack || err);
        console.error(_hmLine);
        try { require("./config").diagLog(_hmLine); } catch (e2) {}
        try {
          sendTo(ws, {
            type: "toast",
            level: "error",

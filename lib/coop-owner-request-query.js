...
function sessionBelongsToTopic(entry, topicId) {
  var refs = Array.isArray(entry && entry.coopTopicRefs) ? entry.coopTopicRefs : [];
  if (!refs.length && entry && entry.coopTopicRef) refs = [entry.coopTopicRef];
  if (!refs.length) return true;
  return refs.some(function (ref) { return ref && ref.topicId === topicId; });
}

function attachClaimSessions(project, claim, sessions) {
  ...
  for (var i = 0; i < children.length; i++) {
    ...
    if (!sessionBelongsToTopic(child, claim.topicId)) continue;
    if (child.role !== "task_coordinator") {
      project.workers.push(sessionSummary(child));
      continue;
    }
    var taskCoordinator = sessionSummary(child);
    taskCoordinator.workers = [];
    var taskWorkers = sessions.childrenByParent[sessionKey(child.sessionRef)] || [];
    for (var w = 0; w < taskWorkers.length; w++) {
      taskCoordinator.workers.push(sessionSummary(taskWorkers[w]));
    }
    project.taskCoordinators.push(taskCoordinator);
  }
}

function projectMembers(project) {
  var members = (project.coordinator ? [project.coordinator] : [])
    .concat(project.taskCoordinators).concat(project.workers);
  for (var i = 0; i < project.taskCoordinators.length; i++) {
    members = members.concat(project.taskCoordinators[i].workers || []);
  }
  return members;
}
...
function buildOwnerRequestOverview(input) {
  ...
  // The unanswered list is the top-level payload and needs the same scope as
  // the tree: unscoped, a non-owner viewer of the Coop project received the
  // owner's outstanding questions and the topic titles derived from the
  // owner's own words. A request with no reachable ProjectRef -- including a
  // purely conversational one -- is not theirs to see.
  //
  // The owner is handed NO scope at all by the handler, so none of this can
  // narrow what the owner themselves is owed.
  function requestVisible(record) {
    if (!visible) return true;
    var refs = Array.isArray(record.projectRefs) ? record.projectRefs : [];
    for (var i = 0; i < refs.length; i++) {
      if (refs[i] && reachable(refs[i].projectId)) return true;
    }
    return false;
  }
  ...
  function projectNode(topic, projectRef) {
    ...
    if (!topic.projectIndex[ref.projectId]) {
      var node = { projectRef: ref, coordinator: null, taskCoordinators: [], workers: [] };
  ...
  for (var i = 0; i < requests.length; i++) {
    ...
    if (summary.outstanding && requestVisible(record)) unanswered.push(summary);
  ...
  for (var c = 0; c < claims.length; c++) {
    ...
    if (!project) continue;
    attachClaimSessions(project, claim, sessions);
  }
  ...
  var topicList = order.filter(function (topicId) {
    ...
    for (var pi = 0; pi < node.projects.length; pi++) {
      ...
      var members = projectMembers(projectEntry);

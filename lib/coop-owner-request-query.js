...
function buildOwnerRequestOverview(input) {
  ...
  // The owner always sees their own outstanding requests; scoping must never
  // hide the owner's backlog from the owner. Anyone else sees a request only
  // when it belongs to a project they can reach -- an unanswered request
  // carries a topic title derived from the owner's own words, so an unscoped
  // list leaks the owner's conversation to every viewer of the Coop project.
  var ownerView = source.ownerView !== false;
  function requestVisible(record) {
    if (!visible || ownerView) return true;
    var refs = Array.isArray(record.projectRefs) ? record.projectRefs : [];
    for (var r = 0; r < refs.length; r++) {
      if (reachable(refs[r].projectId)) return true;
    }
    return false;
  }
...
function overviewFrom(ownerRequests, sessionLedger, topicIndex, visibleProjects, ownerView) {
  var topics = {};

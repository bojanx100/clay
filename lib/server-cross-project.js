...
function matchingCommittedBinding(existing, request) {
  ...
  function holdsTopicClaim(binding, request) {
    ...
    if (!ownerRequests || !topicRef ||
        typeof ownerRequests.canonicalCoordinator !== "function") return true;
    var canonical;
    try { canonical = ownerRequests.canonicalCoordinator(topicRef, request.targetProject); }
    catch (e) { return true; }
    // No claim recorded at all: nothing to contradict, so this stays a replay.
    if (!canonical) return true;
    return canonical.sessionStorageId === binding.coordinator.sessionStorageId;
  }
  ...
  function createAndCommitExecution(input, request, target) {
    ...
    var claimed = claimTopicCoordinator(input, request, delivered.sessionRef);
    ...
    if (claimed && claimed.ok !== true) {
      var reason = claimed.reason === "coordinator_exists"
        ? "coordinator_exists" : "coordinator_claim_not_durable";
      // markUnavailable is the store's own transition for "the execution target
      // went away", which is exactly this: the pair is no longer ours to run.
      var cleanup = bindingStore.markUnavailable(request.portfolioTaskId,
        request.bindingRevision, reason);
      // If the cleanup itself cannot be persisted the binding is still active on
      // disk, so reporting a tidy refusal would hide a live losing coordinator.
      // Record durable attention instead and keep the failure retryable.
      if (cleanup && cleanup.ok !== true) {
        return bindingAttention(request, reason + ":cleanup_not_durable", { pending: true });
      }
      return { ok: false, retryable: true, reason: reason,
        coordinator: claimed.coordinator || null };
    }
    reconcileSessionLedger();
  ...
  function startMigratedExecution(input, request, target, reservation, superseded) {
    ...
    var migratedClaim = claimTopicCoordinator(input, request, delivered.sessionRef);
    if (migratedClaim && migratedClaim.ok !== true) {
      return bindingAttention(request, migratedClaim.reason === "coordinator_exists"
        ? "coordinator_exists" : "coordinator_claim_not_durable");
    }

...
function createCrossProjectRouter(opts) {
  ...
  function createAndCommitExecution(input, request, target) {
    ...
    if (claimed && claimed.ok === false && claimed.reason === "persistence_failed") {
      bindingStore.changeStatus(request.portfolioTaskId, request.bindingRevision,
        "unavailable", "coordinator_claim_not_durable");
      return { ok: false, retryable: true, reason: "coordinator_claim_not_durable" };
    }
    reconcileSessionLedger();

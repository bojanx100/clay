...
function createCrossProjectRouter(opts) {
  ...
  function canonicalOwnerEvent(entry, canonical, ingressId) {
    ...
    if (!sameTopic(event.coopTopicRef, entry.topicRef) &&
        !mainIngressRecovery.matchesRecoveredEntry(entry, canonical, ingressId, event)) return null;
    return event;
  }
  ...
  function replayImplementationDecision(entry, canonical, ingressId) {
    ...
    var recoveredEntry = mainIngressRecovery.matchesRecoveredEntry(
      entry, canonical, ingressId, event);
    var decision = recoveredEntry ? explicitImplementationDecision(event.text) :
      (event.coopImplementationDecision || explicitImplementationDecision(event.text));
    if (recoveredEntry && (!decision || decision.intent !== "implement" ||
        decision.projectName)) return entry;
    ...
    return ownerRequests.classify(ingressId, {
      ...
      projectRefs: recoveredEntry ?
        [{ projectId: mainIngressRecovery.CLAY_PROJECT_ID }] : entry.projectRefs,

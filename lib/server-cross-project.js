...
var settledTaskReconciliation = require("./coop-control-settled-task-reconciliation");
...
function createCrossProjectRouter(opts) {
  ...
  var coordinatorResolution = require("./coop-coordinator-resolution").createResolver({
    bindingStore: bindingStore, sessionForBinding: sessionForBinding,
    getLeadManager: function () {
  ...
  var unrestoredPreviewDrain = false;
  ...
  function prepareControlledRestart() {
    ...
    if (require("./preview-restart").canRestart({ restoreWorkOnStartup: options.restoreWorkOnStartup,
      ingress: controlledIngress, alreadyChecked: unrestoredPreviewDrain,
      recovery: coopStartupRecovery, forEachManager: forEachRegisteredManager })) {
      unrestoredPreviewDrain = true;
      controlledIngress = "draining";
      return { enabled: true, preparedHandoffs: 0, state: controlledIngress, unrestoredPreview: true };
    }
  ...
  function reconcileStrandedCompletions() {
    if (!bindingStore || typeof bindingStore.reconcileStrandedCompletions !== "function") return null;
    var bindings = bindingStore.reconcileStrandedCompletions({
      sessionForBinding: sessionForBinding,
      saveSession: saveBindingSession,
    });
    var tasks = reconcileSettledOwnerAcceptanceTasks();
    return { bindings: bindings, tasks: tasks, resolutions: coordinatorResolution.reconcile() };
  }

  function reconcileSettledOwnerAcceptanceTasks() {
    if (!bindingStore || typeof bindingStore.list !== "function") return { reconciled: [] };
    var lead = resolveProjectContextById(projectIdentity.LEAD_PROJECT_ID);
    var manager = sessionManagerForContext(lead);
    if (!manager) return { reconciled: [] };
    var reconciled = [];
    var bindings = bindingStore.list();
    for (var i = 0; i < bindings.length; i++) {
      var binding = bindings[i];
      var root = coopControlPlane.projectCoordinatorFor(manager, binding && binding.targetProject);
      var rootRef = projectIdentity.sessionRef({ projectId: projectIdentity.LEAD_PROJECT_ID }, root);
      var session = sessionForBinding(binding);
      var metadata = portfolioBindings.sessionExecutionBinding(session);
      if (!binding || !root || !sameSessionRef(rootRef, binding.projectCoordinator) ||
          !settledTaskReconciliation.canReconcile(binding, session, metadata)) continue;
      var repaired = coopControlPlane.reconcileSettledTask(manager, root, binding, {
        workerSessionRef: binding.coordinator,
        completionEventId: binding.completionEventId,
      });
      if (repaired.ok && !repaired.duplicate) reconciled.push(repaired.task.taskId);
    }
    if (reconciled.length && typeof manager.broadcastSessionList === "function") {
...
function matchingCommittedBinding(existing, request) {
  ...
  function primitiveSessionItemKey(session) {
    var taskLauncher = session && session.taskLauncher;
    return taskLauncher && (taskLauncher.automationClaimKey ||
      taskLauncher.itemKey || taskLauncher.prKey) || "";
  }

  function verifiedPrimitiveAdoptionProof(request, target, sessionRef) {
    var authorization = request && request.automationAuthorization;
    if (!authorization || authorization.kind !== automationAuthorization.PRIMITIVE_KIND) {
      return { ok: false, reason: "primitive_adoption_mismatch" };
    }
    var manager = sessionManagerForContext(target && target.context);
    var session = portfolioBindings.sessionByRef(manager, sessionRef,
      request.targetProject.projectId);
    var taskLauncher = session && session.taskLauncher;
    var itemKey = primitiveSessionItemKey(session);
    if (!session || session.hidden || !taskLauncher || taskLauncher.autoLaunch !== true ||
        taskLauncher.workflowCompleted || taskLauncher.executionCompletionReported ||
        taskLauncher.executionCompletionPending || itemKey !== authorization.itemKey) {
      return { ok: false, reason: "primitive_session_not_adoptable" };
    }
    var historical = authorization.qualificationReceipt && authorization.qualificationReceipt.historicalLaunch;
    if (historical && (!sameSessionRef(historical.sessionRef, sessionRef) ||
        historical.createdAt !== session.createdAt || historical.itemUrl !== taskLauncher.itemUrl ||
        authorization.qualificationReceipt.recipe.id !== taskLauncher.recipeId)) {
      return { ok: false, reason: "historical_launch_session_mismatch" };
    }
    return { ok: true, proof: {
      schema: "clay.project_auto_launch_primitive_adoption_proof",
      version: 1,
  ...
  function createAndCommitAdoptedExecution(input, request, target, sessionRef) {
    ...
    var session = portfolioBindings.sessionByRef(manager, sessionRef,
      request.targetProject.projectId);
    var rechecked = verifiedPrimitiveAdoptionProof(request, target, sessionRef);
    if (!rechecked.ok) {
  ...
  function createProjectExecution(input, intakeTicket) {
    input = Object.assign({}, input || {});
    delete input._verifiedPrimitiveAdoption;
    ...
    if (adoption.sessionRef) {
      var primitiveProof = verifiedPrimitiveAdoptionProof(request, target,
        adoption.sessionRef);
      if (!primitiveProof.ok) return primitiveProof;
      input = Object.assign({}, input, {
        _verifiedPrimitiveAdoption: primitiveProof.proof,
      });
    }
  ...
  function retryCoordinatorUpdates() {
    ...
    registeredProjectResolvers.forEach(function (context) {
      ...
      manager.sessions.forEach(function (session) {
        if (api.resumeAutomaticWork) api.resumeAutomaticWork(session);
        api.flushCoordinatorUpdates(session);
      });
    });
  }
  ...
  return {
    ...
    routeSettledDiscussion: require("./coop-settled-discussion").createRoute({
      bindingStore: bindingStore, delivery: durable,
      getLeadManager: function () {
        return sessionManagerForContext(resolveProjectContextById(projectIdentity.LEAD_PROJECT_ID));
      } }),
    ...
    reconcileCoordinatorResolutions: coordinatorResolution.reconcile,
    resolveProjectCoordinatorTask: function (input) {
      var result = coordinatorResolution.resolve(input);
      if (result.ok) reconcileSessionLedger();
      return result;
    },

...
function createCrossProjectRouter(opts) {
  ...
  function implementationAdmission(input, request) {
    ...
    for (var i = entries.length - 1; i >= 0; i--) {
      ...
      if (!entry || entry.ingressId !== ingressId || entry.expectsExecution !== true ||
          !entry.implementationDecision) continue;
      var projects = Array.isArray(entry.projectRefs) ? entry.projectRefs : [];
      if (projects.length === 0) return { ok: true, request: entry };
      for (var pi = 0; pi < projects.length; pi++) {
        if (projects[pi].projectId === targetId) return { ok: true, request: entry };
      }

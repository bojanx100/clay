...
function createCrossProjectRouter(opts) {
  ...
  function restartPreflight(store) {
    ...
    for (var activeIndex = 0; activeIndex < active.length; activeIndex++) {
      ...
      if (!entry || !ref || ref.projectId !== current.session_project_id ||
          ...
          entry.metadata.status !== "running" && entry.metadata.status !== "pending") {
        throw controlledError("COOP_CONTROL_RESTART_CHECKPOINT_REQUIRED",
  ...
  function prepareControlledRestart() {
    ...
    controlledIngress = "draining";
    try {
      var store = coopExecutionControl.getStore();
      var preflight = restartPreflight(store);
      for (var i = 0; i < preflight.specs.length; i++) {
        store.prepareRestartHandoff(preflight.specs[i]);
      }
      var result = { enabled: true, preparedHandoffs: preflight.activeCount,
        state: controlledIngress };
      return result;
    } catch (cause) {
      controlledIngress = "recovery_required";
      if (cause && cause.code === "COOP_CONTROL_RESTART_CHECKPOINT_REQUIRED") throw cause;
      throw controlledError("COOP_CONTROL_RESTART_CHECKPOINT_REQUIRED",
  ...
  function canonicalOwnerEvent(entry, canonical, ingressId) {
    ...
    if (!sameTopic(event.coopTopicRef, entry.topicRef) && !unscopedMain &&
        !classifiedMain &&
        !recoveredThreadAdmission.matchesRecoveredEntry(
          entry, canonical, ingressId, event)) return null;
    return event;
  }
  ...
  function replayImplementationDecision(entry, canonical, ingressId) {
    ...
    var recoveredEntry = recoveredThreadAdmission.decisionForRecoveredEntry(
      entry, canonical, ingressId, event);
    var decision = recoveredEntry ? recoveredEntry.decision :
      (event.coopImplementationDecision || explicitImplementationDecision(event.text));
    ...
    return ownerRequests.classify(ingressId, {
      ...
      projectRefs: recoveredEntry ? [recoveredEntry.projectRef] : entry.projectRefs,
  ...
  function implementationScopeMatches(entry, input, request) {
    var scope = entry && entry.implementationScope;
    if (!scope) return true;
    var expected = implementationScopeFor(input, request);
    return !!(scope.projectRef && scope.topicRef &&
  ...
  //
  // ALL FOUR conditions must hold. There is no partial credit, and each one is
  // load-bearing:
  //
  //   same project + same task  the approval names the work; a different target
  //                             is different work, however similar the text.
  //   newer revision            a carry-forward moves forward only. Reusing an
  //                             approval on the same or an earlier revision is
  //                             a replay, not a retry.
  //   approved revision ended
  //   terminal-unsuccessful     the owner's "yes" was not consumed. A revision
  //                             still pending or active has not finished, so
  //                             there is nothing to retry yet.
  //   no revision ever
  //   completed                 success CONSUMES the approval. Once any revision
  //                             of this task has completed, further work is new
  //                             work and needs its own "yes" -- which is exactly
  //                             why a completed rev1 must keep a later revision
  //                             blocked rather than inheriting authority.
  //
  // `superseded`, `unrouted` and `deleted` are deliberately NOT treated as
  // terminal-unsuccessful. They mean the attempt was withdrawn, replaced or
  // never routed -- bookkeeping, not a failed attempt the owner watched -- and
  // admitting them would let routine binding churn manufacture authorization.
  var CARRY_FORWARD_UNSUCCESSFUL = { failed: true, cancelled: true };
  ...
  function approvalCarriesForward(entry, input, request) {
    var scope = entry && entry.implementationScope;
    if (!scope || !scope.projectRef || !scope.topicRef) return false;
    var expected = implementationScopeFor(input, request);
    if (!expected.projectRef || !expected.portfolioTaskId) return false;
    if (scope.projectRef.projectId !== expected.projectRef.projectId ||
        scope.portfolioTaskId !== expected.portfolioTaskId) return false;
    ...
    // A store that cannot be read is not evidence that nothing completed.
    var revisions = bindingRevisionsFor(expected.portfolioTaskId);
    if (!revisions) return false;
    var approvedEndedUnsuccessful = false;
    for (var i = 0; i < revisions.length; i++) {
      var binding = revisions[i];
      var status = String(binding.status || "");
      if (status === "completed") return false;
      if (Number(binding.bindingRevision) === from &&
          CARRY_FORWARD_UNSUCCESSFUL[status] === true) approvedEndedUnsuccessful = true;
    }
    return approvedEndedUnsuccessful;
  }
    ...
    if (!ingressId) {
      return { ok: false, reason: "owner_implementation_decision_required",
        error: "owner_implementation_decision_required: no owner turn authorizes " +
          ...
          "A missing ThreadRef is not the blocker here." };
    }
...
function matchingCommittedBinding(existing, request) {
  ...
  function messageProjectExecution(input) {
    ...
    if (binding.mode === "project_coordinator" && controlPlaneBinding) {
      ...
      var leadManager = sessionManagerForContext(leadContext);
      var canonical = coopSessionRef();
      var resident = leadManager && canonical ? coopControlPlane.ensureProjectCoordinator(

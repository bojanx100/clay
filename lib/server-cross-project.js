...
function createCrossProjectRouter(opts) {
  ...
  function replayImplementationDecision(entry, canonical, ingressId) {
    ...
    if (!ref || ref.projectId !== projectIdentity.LEAD_PROJECT_ID ||
        ...
        !Number.isInteger(ref.eventIndex) || ref.eventIndex < 0) return entry;
    var event = history[ref.eventIndex];
    if (!event || event.type !== "user_message" || event.coopIngressId !== ingressId ||
        !sameTopic(event.coopTopicRef, entry.topicRef)) return entry;
  ...
  // Owner-authorized read-only planning/review admission.
  //
  // The implementation gate exists to stop Coop from staffing MUTATING work
  // the owner never approved. Council/Triage/design/review workers are a
  // different risk class: their ownership boundary is declared read-only
  // (workers with a "read-only" boundary are instructed never to modify files
  // or external state, and never receive a worktree), so the mutation the gate
  // guards against cannot happen. For those, an explicit owner turn on this
  // exact topic -- cited by ingress id -- is sufficient authorization even
  // when it carries no implementation decision ("do them"). BOTH signals are
  // required together: a read-only boundary without review framing, or review
  // framing without a read-only boundary, stays behind the implementation
  // gate, so real implementation work can never sneak through by wording.
  var REVIEW_FRAMING = /\b(council|triage|review|design|plan|planning|audit|research)\b/i;

  function readOnlyReviewRequest(input) {
    var ownedPaths = String(input && input.ownedPaths || "").trim().toLowerCase();
    if (ownedPaths.indexOf("read-only") !== 0) return false;
    return REVIEW_FRAMING.test(String(input && input.title || "") + " " +
      String(input && input.objective || ""));
  }
  ...
  function implementationAdmission(input, request) {
    ...
    var reviewOnly = readOnlyReviewRequest(input);
    for (var i = entries.length - 1; i >= 0; i--) {
      ...
      var implementationAdmitted = entry.expectsExecution === true &&
        !!entry.implementationDecision;
      // A withdrawn owner turn (the owner interrupted before any reply)
      // authorizes no review work. Implementation admission is deliberately
      // untouched by response state: its evidence is the durable decision.
      var withdrawn = !!(entry.response && entry.response.state === "superseded");
      if (!implementationAdmitted && !(reviewOnly && !withdrawn)) continue;
      ...
      return { ok: true, request: entry, reviewOnly: !implementationAdmitted };
    }

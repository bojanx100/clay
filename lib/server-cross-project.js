...
// createCrossProjectRouter({ getProjectContext, recordRecoveryEvent }) -> { deliver }
...
//   recordRecoveryEvent(event) -> void. Defaults to durable recovery-log
//   persistence; tests can inject a local collector.
function createCrossProjectRouter(opts) {
  var getProjectContext = (opts && opts.getProjectContext) || function () { return null; };
  var recordRecoveryEvent = (opts && typeof opts.recordRecoveryEvent === "function")
    ? opts.recordRecoveryEvent
  ...
  function deadLetter(targetSlug, sessionStorageId, reason) {
    try {
      ...
      // Recovery logging is best-effort and must not break routing.
    }

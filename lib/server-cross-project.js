...
function createCrossProjectRouter(opts) {
  ...
  function implementationAdmission(input, request, context) {
    ...
    // A typed ProjectRef-bound evidence diagnosis has no implementation scope to
    // grant. Admit it before looking up a Thread or owner decision, but only
    // when the narrow read-only classifier has rejected every mutation verb
    // and every ownership segment is explicitly read-only. Automation retains
    // its separate receipt and owner-decision gate because it can create work.
    if (!Object.prototype.hasOwnProperty.call(input || {}, "automationAuthorization") &&
        request.mode === "project_coordinator" &&
        readOnlyReviewAdmission.isReadOnlyPlanningReview(input)) {
      return { ok: true, reviewOnly: true };
    }
    if (Object.prototype.hasOwnProperty.call(input || {}, "automationAuthorization")) {
...
function matchingCommittedBinding(existing, request) {
  ...
  function createProjectExecution(input) {
    ...
    return approvalDispatchResult(linkThreadHandoff(request,
      createAndCommitExecution(input, request, target)), admitted);
  }
  ...
  function messageProjectExecution(input) {
    ...
    if (typeof bindingStore.markAvailable === "function") {
      var reactivate = binding.mode === "project_coordinator" && controlPlaneBinding &&

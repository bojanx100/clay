...
function createCrossProjectRouter(opts) {
  ...
  //   approved revision failed  the owner's "yes" was not consumed. A revision
  ...
  //                             consume a later, explicit approval.
  //
  ...
  function approvalTimestamp(entry) {
    var at = Number(entry && entry.implementationDecision && entry.implementationDecision.at);
  ...
  function approvalCarriesForward(entry, input, request) {
    ...
    if (!Number.isInteger(from) || !Number.isInteger(to) || to !== from + 1) return false;
    ...
    for (var i = 0; i < revisions.length; i++) {
      ...
      if (bindingMatchesCompletionScope(binding, scope) && status === "completed") {
        var completedAt = Number(binding.completedAt);
        if (!Number.isFinite(completedAt) || completedAt >= approvedAt) return false;
      }
      if (bindingMatchesApprovedScope(binding, scope) &&
          CARRY_FORWARD_UNSUCCESSFUL[status] === true) approvedFailures++;
    }
    return approvedFailures === 1;
  }

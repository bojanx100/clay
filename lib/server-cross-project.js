...
function createCrossProjectRouter(opts) {
  ...
  function implementationScopeMatches(entry, input, request) {
    ...
    return !!(expected.projectRef && expected.topicRef &&
      scope.projectRef && scope.topicRef &&
    ...
    for (var i = 0; i < revisions.length; i++) {
      ...
      if (status === "completed") {
        ...
        if (completedAt >= approvedAt) {
          // Legacy completion evidence without a typed ProjectRef and TopicRef
          // cannot prove it belongs to a different authorization scope.
          if (!bindingHasExplicitScope(binding) ||
              bindingMatchesCompletionScope(binding, scope)) return false;
        }
      }
      if (bindingMatchesApprovedScope(binding, scope) &&
        ...
        if (failedAt === null || failedAt < approvedAt) return false;
        approvedFailures++;
      }
    }

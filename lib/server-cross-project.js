...
function createCrossProjectRouter(opts) {
  ...
  function reconcileStrandedCompletions() {
    ...
    return { bindings: bindings, tasks: tasks };
  }
  ...
  // Projection builders reconcile once before walking every Topic. Reading the
  // already-reconciled ledger here keeps that walk side-effect free; callers
  // that have not established the snapshot must use topicSessionEvidence().
...
function matchingCommittedBinding(existing, request) {
  ...
  return {
    ...
    createExecution: guardControlledIngress(createProjectExecution),
    createProjectExecution: guardControlledIngress(createProjectExecution),

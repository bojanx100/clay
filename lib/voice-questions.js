...
function entries(session) {
  ...
  function add(kind, requestId, pending, raw, codex) {
    var questions = normalizeQuestions(raw, codex);
    ...
    var revision = crypto.createHash("sha256").update(JSON.stringify([
      identity.sessionStorageId(session), kind, requestId, versions.get(pending), questions,
    ])).digest("hex");
  ...
  blockedCount += Object.keys(session.pendingPermissions || {}).length + Object.keys(session.pendingUserDialogs || {}).length;
...
function attachVoiceQuestions(ctx, respond) {
  return function (ws, message) {
    ...
    if (!nextTurn) ctx.sendToSession(session.localId, { type: "user_turn_started", clientMessageId: message.clientMessageId });
    respond(ws, payload);

...
function ghStatusText() {
  try {
    return execSync('"' + ghBin() + '" auth status 2>&1', { encoding: "utf8", timeout: GH_STATUS_TIMEOUT });
  } catch (e) {
...
function cacheAccounts(accounts) {
  _accountsCache = { at: Date.now(), accounts: accounts };
...
function cachedAccounts() {
  if (_accountsCache.accounts && (Date.now() - _accountsCache.at) < ACCOUNTS_TTL) {
...
function listGitHubAccountsAsync() {
  ...
  return new Promise(function (resolve) {
    execFile(ghBin(), ["auth", "status"], { encoding: "utf8", timeout: GH_STATUS_TIMEOUT }, function (err, stdout, stderr) {

...
function attachProjectSessionsLifecycle(ctx) {
  ...
  function sendLastVendor(ws, vendor) {
    var userId = ws && ws._clayUser && ws._clayUser.id;
    if (!userId || !clients || typeof clients.forEach !== "function") {
      sendTo(ws, { type: "last_vendor", vendor: vendor });
      return;
    }
    var delivered = false;
    clients.forEach(function (client) {
      if (client && client._clayUser && client._clayUser.id === userId) {
        delivered = true;
        sendTo(client, { type: "last_vendor", vendor: vendor });
      }
    });
    if (!delivered) sendTo(ws, { type: "last_vendor", vendor: vendor });
  }
  ...
  function createSessionForMessage(ws, msg) {
      ...
      // Only an explicit picker choice changes this user's project default.
      // Internal session creation without msg.vendor must not overwrite it.
      if (msg.vendor && yoke.getVendorInfo(msg.vendor)) {
        var lastVendorUserId = ws && ws._clayUser ? ws._clayUser.id : null;
        if (typeof onSetProjectLastVendor === "function") {
          onSetProjectLastVendor(slug, msg.vendor, lastVendorUserId);
        }
        sendLastVendor(ws, msg.vendor);
      }
  ...
  function handleLifecycleMessage(ws, msg) {
    ...
    if (msg.type === "switch_session") {
      ...
      if (msg.id && sm.sessions.has(msg.id)) {
        ...
        if (typeof autoResumeRestartSession === "function") {
          autoResumeRestartSession(xmTarget, { userInitiated: true });
        }
    ...
    if (msg.type === "sync_external_session") {
      ...
      // historyLength, not syncTarget.history.length: this probe fires on every
      // visibilitychange/focus/pageshow, and on a released lazy session the
      // plain accessor re-reads and parses the whole transcript just to compare
      // a count. Tabbing back to Clay must not cost a multi-megabyte read.
      var behind = historyStore.historyLength(syncTarget) > deliveredLen;

...
function registerRecoveryTarget(value) {
  ...
  var targets = recoveryTargets[project.projectId] || [];
  var registered = false;
  for (var i = 0; i < targets.length; i++) {
    if (targets[i].sessionManager !== spec.sessionManager) continue;
    targets[i] = { handlers: spec.recoveryHandlers,
      projectRef: project, sessionManager: spec.sessionManager };
    registered = true;
    break;
  }
  if (!registered) targets.push({ handlers: spec.recoveryHandlers,
    projectRef: project, sessionManager: spec.sessionManager });
  recoveryTargets[project.projectId] = targets;
  return project;
}

function ownsSession(target, ref) {
  var manager = target && target.sessionManager;
  var sessions = manager && manager.sessions;
  if (!sessions || typeof sessions.forEach !== "function") return false;
  var found = false;
  sessions.forEach(function (session) {
    if (found) return;
    var current = projectIdentity.sessionRef(target.projectRef, session);
    if (current && current.projectId === ref.projectId &&
        current.sessionStorageId === ref.sessionStorageId) found = true;
  });
  return found;
}

function unregisterRecoveryTarget(sessionManager) {
  if (!sessionManager) return false;
  var removed = false;
  Object.keys(recoveryTargets).forEach(function (projectId) {
    var remaining = recoveryTargets[projectId].filter(function (target) {
      if (target.sessionManager !== sessionManager) return true;
      removed = true;
      return false;
    });
    if (remaining.length) recoveryTargets[projectId] = remaining;
    else delete recoveryTargets[projectId];
  });
  return removed;
}
...
function targetHandlers(ref, method) {
  ...
  var targets = project && recoveryTargets[project.projectId] || [];
  var sessionRef = projectIdentity.normalizeSessionRef(ref);
  var target = null;
  if (sessionRef) {
    for (var i = 0; i < targets.length; i++) {
      if (!ownsSession(targets[i], sessionRef)) continue;
      if (target) {
        throw recoveryError("Coop startup recovery target " + sessionRef.sessionStorageId +
          " is registered by multiple SessionManagers.");
      }
      target = targets[i];
    }
  }
  // Preserve the single-target seam used by isolated adapters whose test or
  // startup manager has not exposed its session map yet. Shared ProjectRefs
  // must always resolve the exact owning manager instead of registration order.
  ...
  return {
    ...
    cleanupReceived: function () {
      ...
      for (var i = 0; i < ids.length; i++) {
        var targets = recoveryTargets[ids[i]];
        for (var ti = 0; ti < targets.length; ti++) {
          var handler = targets[ti].handlers;
          if (typeof handler.cleanupReceived === "function") {
            cleaned += Number(handler.cleanupReceived()) || 0;
          }
        }
      }
      return cleaned;
...
function getExecutionControl(options) {
  ...
  if (!singleton) {
    ...
    if (singleton.enabled && !handoffModule.isHandoffControlEnabled(opts) &&
        require("./scheduled-execution-policy").restoresWork()) singleton.recoverIncomplete();
  }
  return singleton;
}
...
module.exports = {
  ...
  unregisterRecoveryTarget: unregisterRecoveryTarget,

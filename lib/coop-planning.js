  ...
  function finished(session) {
    ...
    var queued = ctx.queueUpdate && ctx.queueUpdate(source, "[Coop " + plan.kind + " planning result]\n" +
      ...
      { updateId: "planning:" + session.storageId + ":" + (plan.runVersion || 1),
        feedback: { kind: "planning", eventId: "planning:" + session.storageId + ":" + (plan.runVersion || 1),
          coopTopicRef: plan.topicRef, runVersion: plan.runVersion || 1,
          planningRef: identity.sessionRef({ projectId: identity.LEAD_PROJECT_ID }, session) } });
    if (queued) {

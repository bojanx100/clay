...
function attachTaskFollowup(ctx) {
  ...
  function dismissExternalTask(parentSession, task, reason) {
    ...
    if (task.projectAssignment && ctx.crossProject &&
        typeof ctx.crossProject.cancelPendingProjectAssignment === "function") {
      var cancelled = ctx.crossProject.cancelPendingProjectAssignment(parentSession, sm, task.taskId, reason);
      if (cancelled.ok) return true;
      if (cancelled.reason !== "not_pending_assignment") return false;
    }
    var refs = externalBinding(parentSession, task);
  ...
  function dispatchTaskMessage(parentSession, task, worker, text, imageRefs, ownerContinuation) {
    if (!ownerContinuation && ctx.canDispatch && !ctx.canDispatch(parentSession)) return false;
    var prompt = [
  ...
  function dispatchPendingTaskMessage(parentSession, task, worker) {
    var pending = worker.pendingCoordinatorMessages || [];
    var enabled = !ctx.canDispatch || ctx.canDispatch(parentSession);
    var index = pending.findIndex(function (item) { return enabled || item && item.origin === "owner_live_ui"; });
    if (index === -1) return false;
    var message = pending.splice(index, 1)[0];
    try {
      if (sm.saveSessionFile(worker, { durable: true }) === false) throw new Error("Pending worker message was not saved");
    } catch (error) {
      pending.splice(index, 0, message);
      return false;
    }
    return dispatchTaskMessage(parentSession, task, worker,
      typeof message === "string" ? message : message.text,
      typeof message === "string" ? null : message.imageRefs,
      message && message.origin === "owner_live_ui");
  }
  ...
  function message(input, liveUiFollowup) {
    ...
    var task = owned.task;
    if (!liveUiFollowup && ctx.canDispatch && !ctx.canDispatch(parentSession)) {
      return ctx.toolError("Coop orchestration is paused. Enable Lead mode to send coordinator updates.");
    }
    ...
    dispatchTaskMessage(parentSession, task, worker, text,
      Array.isArray(input.imageRefs) ? input.imageRefs : null, liveUiFollowup);
  ...
  function deliverCoordinatorUpdate(sessionStorageId, text) {
    ...
    return ctx.queueCoordinatorUpdate(session, text) === true;
  }
  ...
  function deliverCrossProjectEnvelope(envelope) {
    ...
    var feedback = require("./coop-task-feedback").resolve(ctx.crossProject, envelope);
    if (!ctx.queueCoordinatorUpdate(session, deliveryText(payload.text, eventId),
        { updateId: eventId, feedback: feedback })) {
      return { ok: false, reason: "coordinator_update_persistence_failed", retryable: true };
    }
    return { ok: true };
  }
  ...
  return {
    ...
    dispatchPendingTaskMessage: dispatchPendingTaskMessage,
    messageFromTool: function (input) { return message(input, false); },
    messageFromOwner: function (input) { return message(input, true); },

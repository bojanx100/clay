...
var { listProviderRoutes, routeForId, routeForVendor, hasLiveModelsForProvider, knownModelsForProvider, knownModelsForRoute } = require("./provider-routes");
...
function attachProviderSwitch(ctx) {
  ...
  function modelsForRoute(route, vendor) {
    ...
    if (route) {
      var known = knownModelsForRoute(route);

...
function attachProviderSwitch(ctx) {
  ...
  function resolveModelForVendor(vendor, model, provider) {
    ...
    var resolvesFableAlias = wanted === "fable" &&
      (vendor === "claude" || vendor === "github-copilot");
    var resolvesCopilotBestAlias = vendor === "github-copilot" && wanted === "best";
    if (resolvesFableAlias || resolvesCopilotBestAlias) {
  ...
  function verifiedRouteHasModel(route, model) {
    ...
    for (var i = 0; i < verified.length; i++) {
      if (canonicalModelId(verified[i]) === wanted ||
          coopModelRouting.modelMatches({ model: model }, verified[i])) return true;
    }
    return false;
  }

  function coopSwitchTarget(session, targetRoute, toVendor, requestedTargetModel) {
    return coopModelRouting.resolveSwitchTarget(sm, session, {
      vendor: toVendor,
      providerRouteId: targetRoute && targetRoute.id || null,
      model: requestedTargetModel || null,
    });
  }
  ...
  function executeProviderSwitch(params) {
    ...
    var coopTarget = coopSwitchTarget(session, targetRoute, toVendor,
      requestedTargetModel);
    if (coopTarget && !coopTarget.ok) {
      return { ok: false, reason: coopTarget.reason, code: coopTarget.code,
        message: coopTarget.message };
    }
    if (coopTarget) {
      toVendor = coopTarget.vendor;
      targetRoute = routeForId(coopTarget.providerRouteId);
      requestedTargetModel = coopTarget.model;
    }

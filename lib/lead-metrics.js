...
//   composeReport({ project, now, coverage, complexity,
//     trustObservations, trustPolicy }) -> metrics_report event
// Trust is informational unless a policy is explicitly injected. This keeps
// the existing structural done-gate stable while making any future promotion
...
function composeReport(input) {
  ...
  var trustReport = trust.composeTrustReport(input.trustObservations || [], trustPolicy);
  var structuralPass = coverage.pass && complexity.pass;
  return {
    type: "metrics_report",
    at: input.now || 0,
    ...
    mutation: null,
    trust: trustReport,
    pass: structuralPass && (!trustPolicy || trustReport.pass),
  };
}
...
function formatReportLine(report) {
  ...
  if (report.trust) parts.push(trust.formatTrustReport(report.trust));
  return (report.pass ? "metrics GREEN: " : "metrics RED: ") + parts.join(" · ");
}

// Behavioral handoff evidence is intentionally a separate typed trend. It
// must never turn a structural coverage/complexity done-gate green or red.
function composeGatekeepingEval(input) {
  return gatekeeping.composeGatekeepingEval(input);
}

function formatGatekeepingEvalLine(report) {
  return gatekeeping.formatGatekeepingEvalLine(report);
}
...
module.exports = {
  ...
  formatTrustReport: trust.formatTrustReport,
};

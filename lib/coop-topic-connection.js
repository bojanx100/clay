...
var coopMainReplay = require("./coop-main-replay");
...
function clientProjection(projection) {
  var sidebar = projection && projection.ownerSidebar;
  if (!sidebar || typeof sidebar !== "object") return projection;
  return Object.assign({}, projection, {
    ownerSidebar: {
      defaultOpen: sidebar.defaultOpen === true,
      revision: Number(sidebar.revision) || 0,
      working: Array.isArray(sidebar.working) ? sidebar.working : [],
      attentionGroups: Array.isArray(sidebar.attentionGroups) ? sidebar.attentionGroups : [],
      landed: Array.isArray(sidebar.landed) ? sidebar.landed : [],
      dismissed: Array.isArray(sidebar.dismissed) ? sidebar.dismissed : [],
      hidden: Array.isArray(sidebar.hidden) ? sidebar.hidden : [],
      counts: sidebar.counts && typeof sidebar.counts === "object" ? sidebar.counts : {},
    },
  });
}

function buildAndSendGlobalCoopProjection(ctx, ws, provider) {
  if (ws && typeof ws.readyState === "number" && ws.readyState !== 1) return;
  var projection = provider(ws);
  if (projection) ctx.sendTo(ws, clientProjection(projection));
}
...
function sendGlobalCoopProjection(ctx, ws) {
  ...
  // Let the already-built session identity and transcript reach the socket
  // before the expensive owner-work projection occupies the event loop.
  if (ws && ws.readyState === 1) {
    setImmediate(function () {
      setTimeout(function () { buildAndSendGlobalCoopProjection(ctx, ws, provider); }, 25);
    });
    return;
  }
  buildAndSendGlobalCoopProjection(ctx, ws, provider);
}
...
function boundedMembershipIndexes(topic, session, historyView) {
  ...
  var view = historyView || historyViewFor({ sm: null }, session);
  var ordinary = view && view.hasLineage
    ? coopSessionHistory.indexesForTopic(view, topic)
    : replyAnchor.topicMembershipIndexes(topic, session);
  var linked = ownerDecisionStaging.responseIndexesForTopic(view, session,
    topic && topic.topicRef).concat(require("./coop-owner-answer-membership").indexes(view,
      topic && topic.topicRef));
  var combined = [];
  var seen = {};
  var candidates = ordinary.concat(linked,
    require("./coop-owner-updates").indexesForTopic(view, topic && topic.topicRef));
  for (var ci = 0; ci < candidates.length; ci++) {
    var candidate = candidates[ci];
    if (!Number.isInteger(candidate) || seen[candidate]) continue;
    seen[candidate] = true;
    combined.push(candidate);
  }
  combined.sort(function (left, right) { return left - right; });
  var history = view && Array.isArray(view.history) ? view.history :
    (Array.isArray(session && session.history) ? session.history : []);
  var preserve = {};
  for (var li = 0; li < linked.length; li++) {
    var linkedItem = history[linked[li]];
    if (linkedItem && (linkedItem.type === "delta" || linkedItem.type === "delta_replace" ||
        linkedItem.type === "plan_content") && String(linkedItem.text || "").trim()) {
      preserve[linked[li]] = true;
    }
  }
  return topicRelevance.ownerRelevantIndexes(history, combined, { preserveIndexes: preserve });
}
...
function replayTransform(ctx, options) {
  var hydrate = typeof ctx.hydrateImageRefs === "function" ? ctx.hydrateImageRefs : function (item) { return item; };
  if (!options || (options.scope !== "main" && options.scope !== "topic" &&
      options.scope !== "project")) return hydrate;
  return coopMainReplay.transformFor(ctx);
}

function switchCanonicalReplay(ctx, ws, session, options, replaySession) {
  var source = replaySession || session;
  var transform = replayTransform(ctx, options);
  ...
  if (ctx.sm && typeof ctx.sm.switchSession === "function") {
    ctx.sm.switchSession(session.localId, ws, transform,
    ...
    ctx.sm.replayHistory(source, undefined, ws, transform, options);
  }
...
function prepareTopicReplay(ctx, ws, msg, deps) {
  ...
  if (msg.historyScope === "main") {
    ...
    var replaySession = replaySessionFor(ctx, session);
    var historyView = historyViewFor(ctx, session);
    var mainOptions = replayOptions(null, null, coopMainReplay.membershipIndexes(session), "main");
    if (historyView && historyView.hasLineage) {
      mainOptions.eventIndexes = coopMainReplay.membershipIndexes(replaySession, historyView);
      mainOptions.historyView = historyView;
    }
...
function handleSelection(ctx, ws, msg, deps) {
  ...
  var session = canonicalCoopSession(ctx);
  if (session && coopMainReplay.matchesSelection(ws, msg)) {
    ctx.sendTo(ws, { type: "coop_topic_selected", ok: true, topicRef: null, projectRef: null });
    return true;
  }
  ...
  try {
    ...
    else if (wantsMain) indexes = coopMainReplay.membershipIndexes(session);
    if (wantsMain && historyView && historyView.hasLineage) indexes = coopMainReplay.membershipIndexes(replaySession, historyView);
...
function handleTopicPagination(ctx, ws, msg, deps) {
  ...
  var replay = !session ? null : (mainLens
      ? { ok: true, options: replayOptions(null, null, coopMainReplay.membershipIndexes(session), "main") }
    : selectedTopicReplay(ctx, ws, session, deps));
  if (mainLens && replay && replay.ok) {
    replay.replaySession = replaySession;
    if (historyView && historyView.hasLineage) {
      replay.options.eventIndexes = coopMainReplay.membershipIndexes(replaySession, historyView);
      replay.options.historyView = historyView;
    }
  }
  if (!replay || !replay.ok) {
    ctx.sendTo(ws, {
  ...
  var page = sessionHistory.indexedHistoryPage(
    ...
    replayTransform(ctx, replay.options), replay.options
  );

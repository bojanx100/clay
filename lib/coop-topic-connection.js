...
function handleTopicClosureMessage(ctx, ws, msg) {
  ...
  try {
    if (type === "coop_topic_closure_propose") {
      // Authoritative evidence, not a project list. getProjectList returns
      // project statuses whose `sessions` field is a COUNT, so it could never
      // protect a matching session -- the sweep was deciding on nothing.
      var crossProject = ctx.crossProject || ctx.opts && ctx.opts.crossProject || null;
      ...
      result = index.proposeTopicClosures(session, {
        // Real sessions only. Passing the project list here meant a PROJECT
        // title matched sessionNames() -- which reads a `title` field -- so a
        // topic named after a project was shielded even when that project had
        // no sessions at all. Only a present, non-hidden session counts.
        ...
        tasks: typeof ctx.coopLinkedTasks === "function" ? ctx.coopLinkedTasks() : [],

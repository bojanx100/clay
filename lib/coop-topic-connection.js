...
function handleOwnerRequestOverview(ctx, ws, msg) {
  ...
  try {
    ...
    // Same per-viewer scope every other read on this socket applies. Slug
    // gating proves which PROJECT the socket is on, never who is looking.
    var isOwner = coopTopicOwnerCheck(ctx);
    overview = ownerRequestQuery.overviewFrom(ledger, sessionLedger, index,
      visibleProjects(ctx, ws, topicDeps(ctx)),
      typeof isOwner === "function" ? !!isOwner(ws) : false);
  } catch (e) {

...
function selectedTopicReplay(ctx, ws, session, deps) {
  ...
  return {
    ...
    options: replayOptions(result.topicRef, result.projectRef || null,
      boundedMembershipIndexes(resolved.topic, session, historyView), "topic", null, historyView),
  };
}
...
function prepareTopicReplay(ctx, ws, msg, deps) {
  ...
  if (msg.historyScope === "main") {
    ...
    var historyView = historyViewFor(ctx, session);
    var mainOptions = replayOptions(null, null, mainMembershipIndexes(session, historyView), "main", null, historyView);
...
function handleSelection(ctx, ws, msg, deps) {
  ...
  try {
    ...
    var indexes = null;
    if (resolved && resolved.ok) indexes = boundedMembershipIndexes(resolved.topic, session, historyView);
    else if (wantsMain) indexes = mainMembershipIndexes(session, historyView);
  ...
  if (result && result.ok && session && ctx.sm && typeof ctx.sm.replayHistory === "function") {
    ...
    ctx.sm.replayHistory(session, undefined, ws, ctx.hydrateImageRefs,
      replayOptions(result.topicRef, msg.projectRef || null,
        boundedMembershipIndexes(replayTopic, session, historyView), "drill_through",
...
function handleTopicPagination(ctx, ws, msg, deps) {
  ...
  var replay = !session ? null : (mainLens
    ? { ok: true, options: replayOptions(null, null, mainMembershipIndexes(session, historyView), "main", null, historyView) }
  ...
  var page = sessionHistory.indexedHistoryPage(
    historyView.history, replay.options.eventIndexes, msg.before, msg.target,

...
function prepareTopicReplay(ctx, ws, msg, deps) {
  ...
  if (msg.historyScope === "main") {
    ...
    if (historyView && historyView.hasLineage) {
      mainOptions.eventIndexes = mainMembershipIndexes(replaySession, historyView);
...
function handleSelection(ctx, ws, msg, deps) {
  ...
  try {
    ...
    if (wantsMain && historyView && historyView.hasLineage) indexes = mainMembershipIndexes(replaySession, historyView);
...
function handleTopicPagination(ctx, ws, msg, deps) {
  ...
  if (mainLens && replay && replay.ok) {
    ...
    if (historyView && historyView.hasLineage) {
      replay.options.eventIndexes = mainMembershipIndexes(replaySession, historyView);

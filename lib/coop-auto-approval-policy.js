...
function canonicalProjects(projects) {
  var byProjectId = {};
  var result = [];
  ...
  for (var i = 0; i < projects.length; i++) {
    ...
    if (!ref) continue;
    var label = text(item && item.label, 160) || null;
    var existing = byProjectId[ref.projectId];
    if (existing) {
      // A project can be observed through more than one workspace entry. Its
      // ProjectRef is canonical, so retain the first usable owner-facing name
      // instead of producing duplicate controls for the same policy target.
      if (!existing.label && label) existing.label = label;
      continue;
    }
    existing = { projectRef: { projectId: ref.projectId }, label: label };
    byProjectId[ref.projectId] = existing;
...
function createPolicyStore(options) {
  ...
  function stateFor(projectRef, projects) {
    ...
    var list = canonicalProjects(projects).map(function (item) {
      var ref = item.projectRef;

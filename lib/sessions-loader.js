...
function attachSessionLoader(ctx) {
  ...
  function resolveLastActivity(m, history, fileMtime) {
    ...
    if (typeof m.lastActivity === "number") {
      ...
      // A normal live append is already durable in the JSONL body. Its
      // accompanying metadata-only save may intentionally avoid rewriting a
      // large transcript, leaving line one with the prior activity time. The
      // appended event's own timestamp is the newer, direct evidence.
      if (recovered > m.lastActivity) return { value: recovered, derived: false };

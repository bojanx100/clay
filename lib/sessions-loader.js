...
function attachSessionLoader(ctx) {
  ...
  function hydrateSession(m, history, fileMtime, summary) {
    ...
    var hasPersistedRestartInterruption = !summary &&
      (m.interruptedByRestart || historyAnalysis.hasUncontinuedRestartInterruption(session.history));
  ...
  function loadSessions() {
    ...
    for (var li = 0; li < ordered.length; li++) {
      ...
      var summary = cachedMeta.hidden
        ? startupCache.lookup(cachedStorageId, ordered[li].stat)
        : null;
      if (summary) {

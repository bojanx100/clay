...
function attachSessionLoader(ctx) {
  ...
  function hydrateSession(m, history, fileMtime) {
    for (var hi = 0; hi < history.length; hi++) {
      var historyItem = history[hi];
      if (historyItem && historyItem.synthetic && historyItem.origin &&
          historyItem.origin.kind === "task-notification") {
        historyItem.internalOnly = true;
      }
    }
    ...
    var session = {
      ...
      // Prefer the persisted, authoritative lastActivity. Only legacy
      // session files saved before this field was persisted fall back to
      // the storage file's OS mtime (best-effort, not authoritative).
      lastActivity: typeof m.lastActivity === "number" ? m.lastActivity : (fileMtime || m.createdAt || Date.now()),
  ...
  function loadSessions() {
    ...
    for (var li = 0; li < loaded.length; li++) {
      ...
      if (loaded[li].migrated || session._archivedCompletedTaskOnLoad ||
          session._orchestrationProjectCompletionRecovered || session.interruptedByRestart ||

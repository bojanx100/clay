...
function createLoopRegistry(opts) {
  ...
  function tick() {
    if (scheduledExecution.isPaused()) return;
    var now = Date.now();
  ...
  function register(data) {
    var rec = {
      ...
      schedulerIdempotencyKey: data.schedulerIdempotencyKey || null,
  ...
  function updateRecord(id, data) {
    ...
    // Stamp the change time unless the caller supplied one explicitly, so the
    // record's updatedAt reflects real edits (it was previously frozen at
    // register time, since only update() bumped it).

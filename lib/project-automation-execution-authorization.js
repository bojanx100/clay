...
function normalizeAuthorization(value) {
  if (!exactKeys(value, ["schema", "version", "kind", "source", "projectRef",
    "candidateKey", "candidateDigest", "itemKey", "itemClass", "policyDigest",
    "eligibilityPass", "eligibility", "threadRef", "scope"])) return null;
  if (value.schema !== SCHEMA || value.version !== VERSION || value.kind !== KIND) return null;
...
function createAuthorization(candidate, request) {
  var value = candidate || {};
  var scope = request || {};
...
function createAuthorizationValidator(options) {
  ...
  function validate(input) {
    ...
    if (candidate.admission !== "auto" || candidate.status !== "pending") {

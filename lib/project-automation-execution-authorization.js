...
var PRIMITIVE_KIND = "project_auto_launch_primitive";
var SOURCE_KIND = "project_automation_candidate";
var MAX_TEXT = 240;
var PRIMITIVE_ADOPTION_PROOF_SCHEMA = "clay.project_auto_launch_primitive_adoption_proof";
var PRIMITIVE_ADOPTION_PROOF_VERSION = 1;
...
function normalizeAuthorization(value) {
  ...
  var isPrimitive = value && value.kind === PRIMITIVE_KIND;
  var hasReconsideration = Object.prototype.hasOwnProperty.call(value || {}, "reconsideration");
  if (!isProjectPolicy && !isScopedPolicy && !isAutoApproval && !isPrimitive) return null;
  ...
  if (hasReconsideration) commonKeys.push("reconsideration");
  ...
  var reconsidered = hasReconsideration ? reconsideration.normalize(value.reconsideration) : null;
  if (hasReconsideration && (!isCurrent || !reconsidered)) return null;
  ...
  if (reconsidered && reconsidered.primitiveLaunch && (!isPrimitive ||
      reconsidered.primitiveLaunch.itemKey !== itemKey ||
      reconsidered.primitiveLaunch.sessionRef.projectId !== projectRef.projectId)) return null;
  var normalized = {
    schema: SCHEMA,
  ...
  if (reconsidered) normalized.reconsideration = reconsidered;
  if (isCurrent) {
...
function createAuthorization(candidate, request, options) {
  ...
  var kind = opts.kind === SCOPED_KIND ? SCOPED_KIND :
    (opts.kind === AUTO_APPROVAL_KIND ? AUTO_APPROVAL_KIND :
      (opts.kind === PRIMITIVE_KIND ? PRIMITIVE_KIND : KIND));
  var workKind = value.intent && value.intent.autoKind === "pr-review" ? "pr_review" : "issue";
  var reconsidered = Object.prototype.hasOwnProperty.call(value, "reconsideration") ?
    reconsideration.fromCandidate(value.reconsideration) : null;
  if (Object.prototype.hasOwnProperty.call(value, "reconsideration") && !reconsidered) return null;
  var authorization = {
    schema: SCHEMA,
    version: VERSION,
    kind: kind,
  ...
  if (reconsidered) authorization.reconsideration = reconsidered;
...
function stableIdentity(value) {
  ...
  if (normalized.reconsideration) identityValue.reconsideration = normalized.reconsideration;
...
function hasInFlightReason(receipt) {
  return !!(receipt && receipt.coordinator && Array.isArray(receipt.coordinator.reasons) &&
    receipt.coordinator.reasons.indexOf("existing_primitive_in_flight") !== -1);
}

function normalizePrimitiveAdoptionProof(value) {
  if (!exactKeys(value, ["schema", "version", "sessionRef", "itemKey", "alreadyLaunched"]) ||
      value.schema !== PRIMITIVE_ADOPTION_PROOF_SCHEMA ||
      value.version !== PRIMITIVE_ADOPTION_PROOF_VERSION ||
      value.alreadyLaunched !== true) return null;
  var sessionRef = projectIdentity.normalizeSessionRef(value.sessionRef);
  var itemKey = text(value.itemKey);
  if (!sessionRef || !itemKey) return null;
  return {
    schema: PRIMITIVE_ADOPTION_PROOF_SCHEMA,
    version: PRIMITIVE_ADOPTION_PROOF_VERSION,
...
function primitiveAdoptionProofMatches(value, authorization) {
  var proof = normalizePrimitiveAdoptionProof(value);
  return !!(proof && authorization &&
    proof.sessionRef.projectId === authorization.projectRef.projectId &&
    proof.itemKey === authorization.itemKey);
}
...
function createAuthorizationValidator(options) {
  ...
  function validate(input) {
    ...
    var primitive = normalized.kind === PRIMITIVE_KIND;
    var expectedAdmission = (normalized.kind === SCOPED_KIND || normalized.kind === AUTO_APPROVAL_KIND) ?
      "owner_approval" : "auto";
    if ((!primitive && candidate.admission !== expectedAdmission) ||
        candidate.status !== "pending" && (!primitive || candidate.status !== "owner_approved")) {
    ...
    var reconsiderationError = reconsideration.candidateMismatch(candidate, normalized.reconsideration);
    if (reconsiderationError) return { ok: false, reason: reconsiderationError };
    if (primitive && candidate.intent.primitiveLaunch !== true) {
      return { ok: false, reason: "automation_primitive_launch_required" };
    }
    var loaded;
    ...
    if (normalized.source.workKind === "issue") {
      var inFlightReceipt = hasInFlightReason(normalized.qualificationReceipt);
      var allowInFlightPrimitive = primitive &&
        primitiveAdoptionProofMatches(input && input.primitiveAdoptionProof, normalized);
      if (normalized.reconsideration && normalized.reconsideration.primitiveLaunch &&
          (!allowInFlightPrimitive || JSON.stringify(normalized.reconsideration.primitiveLaunch) !==
          JSON.stringify(normalized.qualificationReceipt.historicalLaunch))) {
        return { ok: false, reason: "primitive_reconsideration_launch_mismatch" };
      }
      if (inFlightReceipt && !allowInFlightPrimitive) {
        return { ok: false, reason: "primitive_adoption_proof_required" };
      }
      ...
      if (qualified.receipt.historicalLaunch) {
        var expected = qualified.receipt.historicalLaunch;
        var actual = typeof opts.verifyHistoricalLaunch === "function" &&
          opts.verifyHistoricalLaunch(expected, qualified.receipt, loaded.policy);
        if (!allowInFlightPrimitive || !actual || !actual.ok ||
            JSON.stringify(actual.proof) !== JSON.stringify(expected) ||
            JSON.stringify(expected.sessionRef) !== JSON.stringify(input.primitiveAdoptionProof.sessionRef)) {
          return { ok: false, reason: "historical_launch_authorization_mismatch" };
        }
      }
    ...
    // A primitive authorization never grants a new launch. It only lets the
    // router adopt the exact session the legacy launcher has already started,
    // after all current qualification evidence above has been rechecked. The
    // router requires the matching adopted SessionRef for this kind, so this
    // cannot be replayed as generic implementation authority.
    if (primitive) {
      return { ok: true, authorization: normalized, candidate: candidate,
        policy: loaded.policy, primitiveLaunch: true };
    }

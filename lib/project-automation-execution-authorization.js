...
function createAuthorizationValidator(options) {
  ...
  function validate(input) {
    ...
    var candidateHasReconsideration = Object.prototype.hasOwnProperty.call(candidate,
      "reconsideration");
    var candidateReconsideration = candidateHasReconsideration ?
      reconsideration.fromCandidate(candidate.reconsideration) : null;
    if (candidateHasReconsideration && !candidateReconsideration) {
      return { ok: false, reason: "automation_candidate_malformed" };
    }
    if (JSON.stringify(candidateReconsideration || null) !==
        JSON.stringify(normalized.reconsideration || null)) {
      return { ok: false, reason: "automation_reconsideration_mismatch" };
    }

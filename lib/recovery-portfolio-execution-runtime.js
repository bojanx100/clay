...
function attachInfrastructureRecovery(ctx) {
  ...
  function afterStart(session, result) {
    if (result && typeof result.then === "function") {
      result.then(function (started) {
        if (started === false) recover(session);
      }, function () {});
  ...
  return { afterStart: afterStart, capture: capture, recover: recover, recoverAll: recoverAll };
}

...
function projectActivityState(session, projectId, sessions, bindings) {
  ...
  if (execution && (!binding || binding.status !== "active")) return "idle";
  ...
  if (task && !boundTaskStatus(task, bindingsForTask)) return "idle";
  var role = roleForProjectActivity(session, task, binding);
  return lifecycleState(session, binding, task, role, bindingsForTask);
}

function projectHasActiveWork(sessions, projectId, bindings) {
  var list = Array.isArray(sessions) ? sessions : [];
  for (var i = 0; i < list.length; i++) {
    if (workState(projectActivityState(list[i], projectId, list, bindings)) === "working") return true;
  }
  return false;
}
...
function attentionTaskStatus(tasks, bindingsForTask) {
  var active = "";
  var list = Array.isArray(tasks) ? tasks : [];
  for (var i = 0; i < list.length; i++) {
    var status = boundTaskStatus(list[i], bindingsForTask);
...
function lifecycleState(session, binding, task, role, bindingsForTask) {
  var bindingState = terminalBindingStatus(binding, role);
  if (bindingState) return bindingState;
  var state = liveLifecycleState(session, binding, task, role, bindingsForTask);
...
function liveLifecycleState(session, binding, task, role, bindingsForTask) {
  ...
  if (role === "project_coordinator") {
    ...
    var taskStatus = attentionTaskStatus(session && session.orchestrationTasks, bindingsForTask);

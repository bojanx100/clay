...
function normalizePolicy(value) {
  if (!exactKeys(value, ["version", "normalIssueIntake"]) ||
      (value.version !== VERSION && value.version !== CONFIGURED_BOARD_POLICY_VERSION)) return null;
  ...
  if (!issueStates || !boardStatuses || !classification || intake.requireAllBoardItems !== true ||
      intake.assignment !== "owner" || value.version === CONFIGURED_BOARD_POLICY_VERSION && !configuredBoard) return null;
...
function normalizeBoardItems(value, configuredBoard) {
  if (!Array.isArray(value) || !value.length) return null;
  var seen = {};
  var result = [];
  for (var i = 0; i < value.length; i++) {
...
function boardItemsFromIssue(issue, configuredBoard) {
...
function normalizeReceipt(value) {
  ...
  var boardItems = normalizeBoardItems(value.item.boardItems, receiptConfiguredBoard);
...
function boardEvidenceReason(profile) {
  return profile.normalIssueIntake.configuredBoard ?
...
function receiptMatchesPolicy(receipt, profile) {
  ...
  for (var i = 0; i < receipt.item.boardItems.length; i++) {
    ...
    if (profile.normalIssueIntake.boardStatuses.indexOf(boardItem.status) === -1) return false;
    var configuredBoard = profile.normalIssueIntake.configuredBoard;

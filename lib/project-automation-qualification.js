...
var primitiveEvidence = require("./project-primitive-launch-evidence");
...
function receiptSubject(value) {
  ...
  if (value.historicalLaunch) subject.historicalLaunch = value.historicalLaunch;
  return subject;
}
...
function stableReceiptIdentity(value) {
  ...
  if (receipt.historicalLaunch) subject.historicalLaunch = receipt.historicalLaunch;
  return subject;
}

function normalizeReceipt(value) {
  var keys = ["schema", "version", "projectRef", "policy", "recipe", "item", "evidenceAt",
    "assignment", "classification", "approval", "coordinator", "digest"];
  var historical = value && Object.prototype.hasOwnProperty.call(value, "historicalLaunch");
  if (historical) keys.push("historicalLaunch");
  ...
  if (historical) {
    var proof = primitiveEvidence.normalizeProof(value.historicalLaunch);
    if (!proof || proof.sessionRef.projectId !== projectRef.projectId ||
        proof.itemKey !== parsedKey.key || proof.policyDigest !== value.policy.digest ||
        proof.recipeDigest !== value.recipe.digest || proof.createdAt > value.evidenceAt ||
        proof.itemUrl !== "https://github.com/" + parsedKey.repo + "/issues/" + parsedKey.number) return null;
    normalized.historicalLaunch = proof;
  }
...
function previousReceiptSupportsInFlight(input, profile, projectRef, recipe, itemClass) {
  ...
  var current = verifyReceipt(previous, { policy: input.policy, now: input.now });
  if (!current.ok && previous) {
    current = primitiveEvidence.verifyProof(input.historicalPrimitiveLaunch, {
      policy: input.policy, projectRef: projectRef, itemKey: input.itemKey, now: input.now,
    });
    if (current.ok && current.receipt.digest !== previous.digest) return false;
  }
...
function receiptFor(input) {
  ...
  if (allowInFlightPrimitive && value.historicalPrimitiveLaunch) {
    var historical = primitiveEvidence.verifyProof(value.historicalPrimitiveLaunch, {
      policy: policy, projectRef: projectRef, itemKey: value.itemKey, now: evidenceAt,
    });
    if (!historical.ok) return historical;
    receipt.historicalLaunch = primitiveEvidence.normalizeProof(historical.proof);
  }
...
function receiptMatchesPolicy(receipt, profile, allowInFlightPrimitive) {
  ...
  if (!profile.normalIssueIntake.requireAllBoardItems) return receipt.item.boardItems.length === 0 && ruleFor(profile, receipt.classification.itemClass) === receipt.classification.rule;

...
function verifiedReviewCompletion(completion) {
  var evidenceFields = ["graphDigest", "summary", "verification", "integrationVerification"];
  return !!(completion && completion.status === "completed" &&
    ...
    evidenceFields.every(function (field) { return String(completion[field] || "").trim(); }) &&
...
function isCoordinatorVerifiedReadOnlyReview(session, completion) {
  ...
  return !!(metadata && metadata.mode === "project_coordinator" &&
    ...
    localPendingOwnerAcceptance(metadata) && verifiedReviewCompletion(completion) &&
...
function deliverTerminal(sm, crossProject, session, details) {
  ...
  var envelope = crossProject.createEnvelope({
    ...
    payload: {
      ...
      failureCode: input.terminalStatus !== "completed" ?
        String(metadata.reason || metadata.statusReason || "").slice(0, 128) || undefined : undefined,

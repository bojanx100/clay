...
// A Thread turn is addressed positionally -- start/end event index into the
// canonical transcript -- while an owner request stores its own absolute
// requestRef.eventIndex. Those indices drift: transcript delta coalescing
// rewrites the file on every reload without repointing the stored indices, so
// on live state essentially no requestRef.eventIndex still lands on its own
// ingress event. The positional range test therefore matched nothing and a
// turn reassignment moved zero owner requests while still reporting success.
//
// Given the canonical history, resolve each candidate by the immutable
// coopIngressId stamped on its ingress event and range-test the position it
// actually occupies today. Nothing is widened: an ingress that cannot be
// resolved -- absent, or duplicated, which fails closed in the resolver --
// falls back to the stored index, which is exactly what callers that pass no
// history keep getting.
function resolvedEventIndex(entry, ref, history) {
  if (!Array.isArray(history) || !history.length) return ref.eventIndex;
  var index = ownerEventResolution.resolveIndexByIngressId(history, entry.ingressId);
  return index >= 0 ? index : ref.eventIndex;
}

function retopicTurn(seam, fromTopicRef, toTopicRef, turnRef, history) {
  var state = seam.state;
  ...
  for (var i = 0; i < state.requests.length; i++) {
    ...
    if (!entry.topicRef || entry.topicRef.topicId !== from.topicId || !ref ||
        ref.sessionStorageId !== turn.sessionStorageId) continue;
    // Resolve only after the session guard, so an entry belonging to some other
    // session is never looked up in this session's transcript.
    var eventIndex = resolvedEventIndex(entry, ref, history);
    if (eventIndex < turn.startEventIndex || eventIndex > turn.endEventIndex) continue;

...
// Providers may split the exact contract across several delta records. Keep a
// trailing prefix of that exact contract out of Main until it can either become
// the full disclosure (and be removed) or ordinary text (and be restored).
// That prevents even a transient disclosure fragment without broadly matching
// assistant prose.
...
function createMainAuthorityDisclosureProjector() {
  ...
  function project(item) {
    if (!item) return item;
    if (item.type === "coop_owner_update") return Object.assign({}, item, {
      text: conversationText(removeLeadAuthorityDisclosures(item.text)),
    });
    if (item.type === "user_message" || item.type === "done" ||
        item.type === "coop_owner_response_started" || item.type === "coop_owner_decision_staged" ||
        item.type === "coop_internal_turn_started") {
      reset();
      return item;
    }
    ...
    projectedAssistantText = projected;
    if (item.type === type && item.text === text) return item;
    return Object.assign({}, item, { type: type, text: text });
  }
...
// opener. Explicit owner reports and proven response spans are conversation;
// unmarked tick commentary stays in All, including its first occurrence.
...
function historyOwnerRelevantIndexes(history, options) {
  ...
  var ownerResponseStart = Infinity;
  for (var i = 0; i < items.length; i++) {
    var item = items[i];
    if (i > 0 && items[i - 1] && items[i - 1].coopOwnerResponseStartsAfter === true) ownerResponseStart = i;
    if (item && item.type === "user_message" && item.compactedRetry !== true) {
      internalTurn = isInternalTurnOpener(item);
      ownerResponseStart = Infinity;
    }
    if (internalTurn) {
      ...
      if ((preserve[i] || i >= ownerResponseStart) && INTERNAL_PROGRESS_TYPES[item && item.type]) {
        kept.push(i);
        continue;
      }
      if (OWNER_ACTION_TYPES[item && item.type] || item && item.type === "coop_owner_update") {
        kept.push(i);
        continue;
      }
      continue;
    ...
    if (isOwnerRelevantRecord(item) &&
        !(opts.suppressLeadAuthorityDisclosures !== false && isOnlyLeadAuthorityDisclosure(item))) kept.push(i);
  }
  return kept;
}
...
function mainLensEventIndexes(history, options) {
  return historyOwnerRelevantIndexes(history, Object.assign({}, options, { suppressLeadAuthorityDisclosures: true }));
}

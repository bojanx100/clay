...
function historyOwnerRelevantIndexes(history) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    if (isOwnerRelevantRecord(item)) kept.push(i);
  }
  return kept;
}
...
function ownerRelevantIndexes(history, indexes) {
  var items = Array.isArray(history) ? history : [];
  var source = Array.isArray(indexes) ? indexes : [];
  var relevant = historyOwnerRelevantIndexes(items);
...
function mainLensEventIndexes(history) {
  return historyOwnerRelevantIndexes(history);
}

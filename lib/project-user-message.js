...
function attachUserMessage(ctx) {
  ...
  function dispatchPreparedToSdk(session, finalText, images, steer, queueId, displayText, imageCount, clientMessageId, pastes, fromQueue) {
    ...
    if (!steer && !fromQueue && shouldQueueDuringProcessing(session)) {
  ...
  function handleUserMessage(ws, msg) {
    ...
    var shouldQueue = wasProcessing || (!steer && shouldQueueMessage(session));

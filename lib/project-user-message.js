...
function attachUserMessage(ctx) {
  ...
  var activateCoordinator = ctx.activateCoordinator || null;
  ...
  function flushQueuedUserMessage(session) {
    ...
    // A steer is one explicit selection. Leave the other messages queued after
    // the selected turn completes instead of draining the whole queue.
    if (next.hidden) session._holdQueuedMessagesAfterSteer = true;
  ...
  function scheduleQueuedUserMessageFlush(session) {
    ...
    session._queuedFlushTimer = setTimeout(function () {
      session._queuedFlushTimer = null;
      if (session._holdQueuedMessagesAfterSteer) {
        delete session._holdQueuedMessagesAfterSteer;
        sendQueuedUserMessagesState(session);
        return;
      }
      if (!session.pendingUserMessageQueue || session.pendingUserMessageQueue.length === 0) {
  ...
  function handleUserMessage(ws, msg) {
    ...
    if (msg.type === "coordinate_queued_message") {
      ...
      if (!taskParentSession || !taskQueueId || !activateCoordinator ||
      ...
      var coordinatorText = activateCoordinator(taskParentSession, taskQueueItem.text);
      ...
      queuePreparedMessage(
        taskParentSession,
        coordinatorText,
        taskQueueItem.images,
        taskQueueItem.queueId,
        taskQueueItem.displayText,
        taskQueueItem.imageCount,
        taskQueueItem.clientMessageId,
        taskQueueItem.pastes,
        { front: true, silent: true, hidden: true }
      );
      sm.saveSessionFile(taskParentSession);
      if (taskParentSession.isProcessing) {
        taskParentSession.steerInterruptRequested = true;
        taskParentSession.taskStopRequested = true;
        if (taskParentSession.abortController) taskParentSession.abortController.abort();
      } else {
        flushQueuedUserMessage(taskParentSession);
      }
      return true;
    }

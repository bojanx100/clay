...
function attachUserMessage(ctx) {
  ...
  var startOrchestrationTask = ctx.startOrchestrationTask || null;
  ...
  function markQueuedHistoryAsOrchestrationTask(session, queueId, taskId) {
    var item = findQueuedHistoryMessage(session, queueId);
    if (!item) return null;
    delete item.queuedDuringProcessing;
    delete item.queuedPending;
    delete item.steerPending;
    item.orchestrationTaskId = taskId;
    item._ts = Date.now();
    sm.saveSessionFile(session);
    return item;
  }
  ...
  function handleUserMessage(ws, msg) {
    ...
    if (msg.type === "start_queued_message_as_task") {
      var taskParentSession = getSessionForMessage(ws, msg);
      var taskQueueId = msg.queueId || "";
      if (!taskParentSession || !taskQueueId || !startOrchestrationTask ||
      ...
      var taskResult = startOrchestrationTask(taskParentSession, taskQueueItem, ws._clayUser || null);
      if (!taskResult) {
        taskParentSession.pendingUserMessageQueue.splice(taskQueueIndex, 0, taskQueueItem);
        return true;
      }
      markQueuedHistoryAsOrchestrationTask(taskParentSession, taskQueueId, taskResult.taskId);

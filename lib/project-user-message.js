...
function validateCoopTopicIngress(ctx, session, msg, ws) {
  ...
  // Main stays route-free except for a narrow, unambiguous owner command that
  // explicitly creates a project implementation Thread. The project is named
  // in the command itself, so this never revives stale client-selected refs.
  if (mainScope) {

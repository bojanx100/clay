...
function attachUserMessage(ctx) {
  ...
  var resolveOrchestrationTask = ctx.resolveOrchestrationTask || null;
  ...
  function handleUserMessage(ws, msg) {
    ...
    if (msg.type === "resolve_orchestration_task") {
      var resolveParentSession = getSessionForMessage(ws, msg);
      var resolveTaskId = String(msg.taskId || "");
      if (!resolveParentSession || !resolveTaskId || !resolveOrchestrationTask) return true;
      var resolution = resolveOrchestrationTask(
        resolveParentSession,
        resolveTaskId,
        String(msg.summary || ""),
        String(msg.verification || "")
      );
      if (resolution && resolution.isError) {
        sendTo(ws, {
          type: "error",
          text: resolution.content && resolution.content[0] && resolution.content[0].text ||

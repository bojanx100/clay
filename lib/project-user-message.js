...
function attachUserMessage(ctx) {
  ...
  function validateCoopTopicIngress(session, msg, ws) {
    ...
    // Main and All deliberately reach the canonical transcript without a
    // Thread route. They must not be reclassified from stale refs that the
    // ingress normalizer intentionally discarded.
    if (canonicalCoop && msg &&

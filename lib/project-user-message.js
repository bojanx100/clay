...
function canAccessCoopChannel(session, ws, options, usersModule, projectSlug) {
  ...
  if (multiUser) {
    if (!userId || !usersModule.canAccessSession(
      userId, session, { visibility: "public" })) return false;
  } else if (session.ownerId) {
    return false;
  }
  var getProjectList = options && options.getProjectList;
  var channel = coopChannels.normalizeChannelMetadata(session.coopChannel);
  if (!channel || typeof getProjectList !== "function") return false;
  return !!coopChannels.findProject(
    getProjectList(multiUser ? userId : null) || [], channel.projectSlug);
}
...
function attachUserMessage(ctx) {
  ...
  var closeOrchestrationTask = ctx.closeOrchestrationTask || null;
  var retryOrchestrationReconciliation = ctx.retryOrchestrationReconciliation || null;
  var listAdoptionCoordinators = ctx.listAdoptionCoordinators || null;
  var proposeSessionAdoption = ctx.proposeSessionAdoption || null;
  ...
  function shouldQueueDuringProcessing(session) {
    return shouldQueueMessage(session);
  }
  ...
  function getSessionForMessage(ws, msg) {
    ...
    if (typeof requestedId === "number" && isFinite(requestedId)) {
      ...
      if (requestedSession) {
        ...
        if (!canAccessCoopChannel(
          requestedSession, ws, opts, usersModule, slug)) return null;
        if (!msg.preserveActiveSession) {
    ...
    return canAccessCoopChannel(
      activeSession, ws, opts, usersModule, slug) ? activeSession : null;
  }
  ...
  function dispatchPreparedToSdk(session, finalText, images, steer, queueId, displayText, imageCount, clientMessageId, pastes, fromQueue, intent) {
    ...
    if (intent === "task" && coordinateQueuedMessage) {
      coordinateQueuedMessage(session, {
        text: finalText,
    ...
    if (!steer && !fromQueue && (intent === "queue" || shouldQueueDuringProcessing(session))) {
  ...
  function flushQueuedUserMessage(session) {
    ...
    dispatchPreparedToSdk(session, next.text, next.images, false, next.queueId, next.displayText, next.imageCount, next.clientMessageId, next.pastes, true);
  }
  ...
  function handleUserMessage(ws, msg) {
    if (msg.type === "promote_session_to_coordinator") {
      var promotedSession = getSessionForMessage(ws, msg);
      if (!promotedSession || promotedSession.orchestrationParent) return true;
      promotedSession.coordinationMode = true;
      sm.saveSessionFile(promotedSession);
      sm.broadcastSessionList();
      sendToSession(promotedSession.localId, {
    ...
    if (msg.type === "term_create") {
      ...
      var terminalOpts = {
        sessionId: ws._clayActiveSession || null,
      };
      if (msg.initialCommand) terminalOpts.initialInput = String(msg.initialCommand);
      var t = tm.create(msg.cols || 80, msg.rows || 24, getOsUserInfoForWs(ws), ws, terminalOpts);
      if (!t) {
    ...
    if (msg.type === "term_close") {
      if (msg.id) {
        ...
        if (filtered.length !== saved.length) {
          ...
          sendToSession(_termSessionId, { type: "context_sources_state", active: filtered });
        }
      }
      return true;
    }
    ...
    if (msg.type === "list_orchestration_coordinators") {
      var adoptionSource = getSessionForMessageWithoutSwitch(ws, msg.sourceSessionId);
      var coordinatorCandidates = adoptionSource && listAdoptionCoordinators
        ? listAdoptionCoordinators(adoptionSource)
        : [];
      coordinatorCandidates = coordinatorCandidates.filter(function (candidate) {
        return !!getSessionForMessageWithoutSwitch(ws, candidate.id);
      });
      sendTo(ws, {
        type: "orchestration_coordinator_candidates",
        sourceSessionId: adoptionSource ? adoptionSource.localId : null,
        candidates: coordinatorCandidates,
      });
      return true;
    }

    if (msg.type === "propose_session_adoption") {
      var proposalSource = getSessionForMessageWithoutSwitch(ws, msg.sourceSessionId);
      var proposalCoordinator = getSessionForMessageWithoutSwitch(ws, msg.coordinatorSessionId);
      var proposed = !!(proposalSource && proposalCoordinator && proposeSessionAdoption &&
        proposeSessionAdoption(proposalSource, proposalCoordinator));
      sendTo(ws, {
        type: "session_adoption_proposed",
        ok: proposed,
        sourceSessionId: proposalSource ? proposalSource.localId : null,
        coordinatorSessionId: proposalCoordinator ? proposalCoordinator.localId : null,
      });
      return true;
    }

    if (msg.type === "close_orchestration_task") {
      var closeParentSession = getSessionForMessage(ws, msg);
      var closeTaskId = String(msg.taskId || "");
      if (!closeParentSession || !closeTaskId || !closeOrchestrationTask) return true;
      if (ws._clayUser) {
        var closePerms = usersModule.getEffectivePermissions(ws._clayUser, osUsers);
        if (!closePerms.sessionDelete) {
          sendTo(ws, { type: "error", text: "You do not have permission to close worker conversations" });
          return true;
        }
      }
      closeOrchestrationTask(closeParentSession, closeTaskId, ws);
      return true;
    }

    if (msg.type === "retry_orchestration_reconciliation") {
      var retryParentSession = getSessionForMessage(ws, msg);
      if (retryParentSession && retryOrchestrationReconciliation) {
        retryOrchestrationReconciliation(retryParentSession);
      }
      return true;
    }
    ...
    // The evaluator needs the fact that an owner asked for a direct handoff,
    // but never the ask itself.  Capture only a typed intent from canonical
    // Coop conversations; direct owner-opened sessions stay outside this
    // artifact. The opaque id can be returned by a future clickable reference
    // or supplied by a typed navigation action.
    if (canCaptureCoopHandoff(session, ws, usersModule, slug, msg.text)) {
      var capturedHandoff = coopHandoffTraceStore.recordIntent({
        id: msg.handoffTraceId,
        ownerId: handoffTraceOwnerId(ws, usersModule),
        channel: msg.channel,
        expectedTarget: msg.handoffTarget,
        requiresAssistantObservation: true,
        observeAssistantTurns: observeAssistantTurns(session),
      });
      if (capturedHandoff && capturedHandoff.ok) {
        sendTo(ws, { type: "coop_handoff_intent", handoffTraceId: capturedHandoff.id });
      }
    }

    if (hasStaleProcessingState(session)) {
    ...
    var intent = msg.intent === "task" || msg.intent === "queue" || msg.intent === "steer"
      ? msg.intent
      : "chat";
    var steer = msg.steer === true || intent === "steer";
    var wasProcessing = !!session.isProcessing;
    var shouldQueue = intent !== "task" &&
      (intent === "queue" || wasProcessing || (!steer && shouldQueueMessage(session)));
    var queueId = shouldQueue ? makeQueueId() : null;
    ...
    if (intent === "task") {
      userMsg2.coordinationRequest = true;
      userMsg2.messageIntent = "task";
    } else if (intent === "queue") {
      userMsg2.messageIntent = "queue";
    }
    ...
    fullText = applyCoopChannelScope(session, fullText);
    ...
    if (tabSources.length > 0) {
      ...
      Promise.all(tabPromises).then(function(results) {
        ...
        dispatchPreparedToSdk(session, fullText, msg.images, steer, queueId, msg.text || "", displayImageCount, clientMessageId, msg.pastes || null, false, intent);
      });
    } else {
      dispatchPreparedToSdk(session, fullText, msg.images, steer, queueId, msg.text || "", displayImageCount, clientMessageId, msg.pastes || null, false, intent);
    }

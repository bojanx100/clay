...
function validateCoopTopicIngress(ctx, session, msg, ws) {
  ...
  // or a clear owner implementation request that names one project. The
  // contextual marker was minted after canonical evidence resolved one open
  // Thread; normalizeComposerScope deletes any client-supplied copy. An
  // implementation request is resolved against the current project inventory,
  // so neither path revives stale refs or guesses across duplicate names.
  var start = null;
  var implementation = null;
  var routedProjectName = "";
  if (mainScope && !contextualThreadTarget) {
    start = threadLifecycle.implementationThreadStartDecision(msg.text);
    implementation = threadLifecycle.explicitImplementationDecision(msg.text);
    routedProjectName = start ? start.projectName : implementation && implementation.projectName;
    if (!routedProjectName) {
      return { ok: true, topicRef: null, projectRef: null, classification: "conversational" };
    }
    var projectRef = projectRefForImplementationThreadStart(projects, routedProjectName);
    // Main is already the canonical conversation. An uncertain name must not
    // discard its owner's message or create execution authority for a guess.
    // Explicitly selected Thread/project refs still use their strict route.
    if (!projectRef) return { ok: true, topicRef: null, projectRef: null,
  ...
  if (hasTopic || canonicalCoop) {
    ...
    if (canonicalCoop && typeof index.reconcileTopicAnchors === "function") {
      index.reconcileTopicAnchors(replaySession);
      ...
      if (typeof index.retrofitTopicTitles === "function") {
        index.retrofitTopicTitles(replaySession);
      }
    }
  }
  if (canonicalCoop && !hasTopic) {
    ...
    return index.classifyCanonicalIngress(replaySession, classificationMessage, {
      projects: projects,
      recordExplicitRoute: mainScope && !!routedProjectName,
      isProjectAvailable: function (ref) { return projectRefAvailable(ctx.getProjectList, ref, userId); },
    });
  }
  return replyAnchorModule.replyAnchorForRoute(index, replaySession, index.validateIngress(replaySession, msg, {
...
function attachUserMessage(ctx) {
  ...
  function bindCoopTopicMessage(session, msg, eventIndex) {
    var replaySession = topicLineage.buildReplaySession(session, sm && sm.sessions) || session;
    return replyAnchorModule.bindTopicMembership(topicIndexFor(session), replaySession, msg, eventIndex);
  }
  ...
  var handlers = handlersModule.attachProjectUserMessageHandlers({
    ...
    resolveCoordinatorUpdates: ctx.resolveCoordinatorUpdates,
  ...
  var messageContext = contextModule.attachProjectUserMessageContext({
    ...
    resolveCoopThreadIntentTarget: function (session, evidence) {
      var historyView = sm && typeof sm.getHistoryView === "function" ? sm.getHistoryView(session) : null;
      return threadIntent.resolveDominantTarget(topicIndexFor(session), session,
        Object.assign({}, evidence || {}, { historyView: historyView }));
    },

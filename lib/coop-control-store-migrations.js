...
var recoverySchema = require("./coop-control-recovery-schema");
var staleR6Schema = require("./coop-control-stale-r6-reconciliation-schema");
...
var TABLE_SHAPES = Object.freeze({
  ...
  coop_control_handoffs: recoverySchema.TABLE_SHAPES.coop_control_handoffs,
  coop_control_checkpoints: recoverySchema.TABLE_SHAPES.coop_control_checkpoints,
  coop_control_outbox: recoverySchema.TABLE_SHAPES.coop_control_outbox,
  coop_control_inbox: recoverySchema.TABLE_SHAPES.coop_control_inbox,
  coop_control_effects: recoverySchema.TABLE_SHAPES.coop_control_effects,
  coop_control_delivery_payloads: recoverySchema.TABLE_SHAPES.coop_control_delivery_payloads,
  coop_control_successor_receipts: recoverySchema.TABLE_SHAPES.coop_control_successor_receipts,
  coop_control_stale_r6_reconciliation_receipts:
    staleR6Schema.TABLE_SHAPES.coop_control_stale_r6_reconciliation_receipts,
});
...
function applyRecoveryControlFoundation(db) {
  recoverySchema.apply(db);
}

function applyRecoveryPayloadReceipts(db) {
  recoverySchema.applyPayloadReceipts(db);
}

function applyStaleR6ReconciliationReceipt(db) {
  staleR6Schema.apply(db);
}
...
var MIGRATIONS = Object.freeze([
  ...
  Object.freeze({ version: 4, name: "recovery-control-foundation", apply: applyRecoveryControlFoundation }),
  Object.freeze({ version: 5, name: "recovery-payload-receipts", apply: applyRecoveryPayloadReceipts }),
  Object.freeze({ version: 6, name: "stale-r6-reconciliation-receipt", apply: applyStaleR6ReconciliationReceipt }),
]);
...
function expectedSchema(version) {
  ...
  if (version >= 4) {
    objects.push({ type: "table", name: "coop_control_handoffs",
      table: "coop_control_handoffs", sql: recoverySchema.HANDOFFS_SQL });
    objects.push({ type: "table", name: "coop_control_checkpoints",
      table: "coop_control_checkpoints", sql: recoverySchema.CHECKPOINTS_SQL });
    objects.push({ type: "table", name: "coop_control_outbox",
      table: "coop_control_outbox", sql: recoverySchema.OUTBOX_SQL });
    objects.push({ type: "table", name: "coop_control_inbox",
      table: "coop_control_inbox", sql: recoverySchema.INBOX_SQL });
    objects.push({ type: "table", name: "coop_control_effects",
      table: "coop_control_effects", sql: recoverySchema.EFFECTS_SQL });
  }
  if (version >= 5) {
    objects.push({ type: "table", name: "coop_control_delivery_payloads",
      table: "coop_control_delivery_payloads", sql: recoverySchema.DELIVERY_PAYLOADS_SQL });
    objects.push({ type: "table", name: "coop_control_successor_receipts",
      table: "coop_control_successor_receipts", sql: recoverySchema.SUCCESSOR_RECEIPTS_SQL });
  }
  if (version >= 6) {
    objects.push({ type: "table", name: "coop_control_stale_r6_reconciliation_receipts",
      table: "coop_control_stale_r6_reconciliation_receipts", sql: staleR6Schema.RECEIPTS_SQL });
  }

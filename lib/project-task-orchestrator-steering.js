...
function createProjectCoordinatorSteering(deps) {
  return function steerProjectCoordinator(input) {
    ...
    if (!targetProject || !targetCoordinator ||
        targetCoordinator.projectId !== projectIdentity.LEAD_PROJECT_ID &&
        targetCoordinator.projectId !== targetProject.projectId ||
    ...
    if (!result || result.ok !== true) {
      // A wrong coordinator ref is the one refusal the caller can fix on its
      // own, but only if it is told which identities the binding accepts. The
      // binding itself stays withheld; just the legal SessionRefs are named.
      var expected = result && result.reason === "coordinator_ref_mismatch" &&
        Array.isArray(result.expectedCoordinators) && result.expectedCoordinators.length ?
        "; this binding accepts targetCoordinator " + result.expectedCoordinators.map(function (ref) {
          return ref.projectId + "/" + ref.sessionStorageId;
        }).join(" or ") : "";
      var error = deps.toolError("project coordinator steering requires attention: " +
        (result && result.reason || "delivery_error") + expected);
      // A control-plane coordinator may be valid before its project child has
      // been created. Keep this one explicit router result structured so the
      // caller can retry the same command with the exact binding identity.
      if (result && result.reason === "binding_pending" && result.retryable === true) {
    ...
    return deps.toolSuccess("Steered Coop project coordinator " + targetCoordinator.sessionStorageId +
      "; its ProjectRef-bound control plane relayed the update.");
  };
}

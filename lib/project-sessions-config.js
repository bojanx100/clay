function attachProjectSessionsConfig(ctx) {
  ...
  function sendRestartResult(ws, result) {
    ...
    var pending = result && result.pending === true;
    sendTo(ws, {
      type: "restart_server_result",
      ok: true,
      pending: pending,
      activeProviderTools: result && result.activeProviderTools || 0,
    });
    send({
      type: "toast",
      level: "info",
      message: pending ? "Restart queued until active provider tools finish." : "Server is restarting...",
    });
  }

  function sendRestartError(ws, err) {
    sendTo(ws, {
      type: "restart_server_result",
      ok: false,
  ...
  function handleConfigMessage(ws, msg) {
    ...
    if (msg.type === "set_keep_awake") {
      if (typeof opts.onSetKeepAwake === "function") {
        Promise.resolve(opts.onSetKeepAwake(msg.value, msg.headlessClamshell === true)).then(function (kaResult) {
          sendTo(ws, {
            type: "set_keep_awake_result",
            ok: kaResult.ok,
            keepAwake: kaResult.keepAwake,
            offerHeadlessClamshell: kaResult.offerHeadlessClamshell === true,
            headlessClamshellRequested: kaResult.headlessClamshellRequested === true,
            error: kaResult.error || null,
          });
          ...
          sendTo(ws, {
            ...
            ok: false,
            keepAwake: !!msg.value,
            error: err && err.message ? err.message : "Failed to change Keep Awake",
          });
        });
      }
      return true;
    ...
    if (msg.type === "restart_server") {
      if (typeof opts.onRestart === "function") {
        try {
          Promise.resolve(opts.onRestart()).then(function (restartResult) {
            sendRestartResult(ws, restartResult);
          }).catch(function (err) {
            sendRestartError(ws, err);
          });
        } catch (err) {
          sendRestartError(ws, err);
        }

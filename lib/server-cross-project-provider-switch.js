...
function canonicalSuccessorProof(leadSm, binding, source, sourceSession) {
  ...
  while (currentId && currentId !== bindingSource.sessionStorageId && !seen[currentId]) {
    ...
    // Compaction overwrites an intermediate session's timestamp when it becomes
    // a predecessor. Only the newest edge still has two copies of the same time;
    // older edges retain reciprocal identities and chronological compaction times.
    if (typeof current.compactedAt === "number" && typeof previous.compactedAt === "number" &&
        (previous.compactedAt > current.compactedAt ||
          current === sourceSession && current.compactedAt !== previous.compactedAt)) return null;
    current = previous;

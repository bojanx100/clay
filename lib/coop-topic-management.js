...
// The ledger range-tests each owner request against the turn's start/end event
// index, and the stored requestRef.eventIndex has drifted away from the turn it
// denotes (see coop-owner-event-resolution). Handing it the canonical
// transcript lets it resolve each request by its immutable ingress id first, so
// a reassignment moves the requests that actually sit in the turn instead of
// silently moving none.
function canonicalCoopHistory(ctx) {
  var session = ownerRequestMigrations.canonicalCoopSession(ctx && ctx.sm, null);
  return session && Array.isArray(session.history) ? session.history : null;
}
...
function reassignThreadTurn(ctx, index, sourceRef, targetRef, turnRef) {
  ...
  if (ledger && typeof ledger.retopicTurn === "function") {
    try { moved = ledger.retopicTurn(sourceRef, targetRef, turnRef, canonicalCoopHistory(ctx)); }

  ...
  if (operation === "state") {
    ...
    if (next === threadLifecycle.THREAD_STATES.HANDED_OFF) {
      ...
      if (result.ok && next === threadLifecycle.THREAD_STATES.CLOSED) {
        archiveCompletedTopicSessions(ctx, ws, result.thread || index.resolve(sourceRef, true).topic);
        reconcileClosedTopicRequests(ctx, index.resolve(sourceRef, true).topic);
      }
    }

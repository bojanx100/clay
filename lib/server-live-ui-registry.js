...
function createLiveUiRegistry(options) {
  ...
  var projectHandlers = new Map();
  var extensionConnections = new Map();
  ...
  function create(input) {
    ...
    if (!isLoopbackOrigin(input.allowedOrigin) &&
        (!input.allowRemoteOrigin || !isExactHttpOrigin(input.allowedOrigin))) {
      fail("LIVE_UI_ORIGIN_DENIED",
        "Live UI requires an exact server-authorized origin");
    }
  ...
  function registerProjectHandler(projectSlug, handler) {
    var key = String(required(projectSlug, "projectSlug"));
    if (typeof handler !== "function") {
      fail("LIVE_UI_INVALID", "Live UI project handler must be a function");
    }
    projectHandlers.set(key, handler);
    return function () {
      if (projectHandlers.get(key) === handler) projectHandlers.delete(key);
    };
  }

  function routeProjectMessage(projectSlug, ws, message) {
    var handler = projectHandlers.get(String(projectSlug || ""));
    if (!handler) return false;
    return handler(ws, message) !== false;
  }

  function rememberExtensionConnection(input) {
    input = input || {};
    var extensionInstanceId = String(required(
      input.extensionInstanceId, "extensionInstanceId"));
    extensionConnections.set(extensionInstanceId, {
      ws: required(input.ws, "ws"),
      userId: String(required(input.userId, "userId")),
    });
  }

  function extensionConnection(extensionInstanceId, userId) {
    var key = String(extensionInstanceId || "");
    var connection = extensionConnections.get(key);
    if (!connection) return null;
    if (!connection.ws || connection.ws.readyState !== 1) {
      extensionConnections.delete(key);
      return null;
    }
    if (userId && connection.userId !== String(userId)) return null;
    return connection.ws;
  }

  function forgetExtensionConnection(ws) {
    extensionConnections.forEach(function (connection, key) {
      if (connection.ws === ws) extensionConnections.delete(key);
    });
  }
  ...
  return {
    ...
    registerProjectHandler: registerProjectHandler,
    routeProjectMessage: routeProjectMessage,
    rememberExtensionConnection: rememberExtensionConnection,
    extensionConnection: extensionConnection,
    forgetExtensionConnection: forgetExtensionConnection,
    create: create,
    prove: prove,
    reconnect: reconnect,
    dedupe: dedupe,

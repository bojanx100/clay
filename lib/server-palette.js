...
function isLiveUiTopLevelSession(session, orchestrationGroups) {
  return !session.orchestrationParent &&
    !session.orchestrationGroupParent &&
    !orchestrationTaskState.orchestrationParentForClient(session) &&
    !orchestrationTaskState.orchestrationGroupParentForClient(
      session, orchestrationGroups) &&
...
function buildLiveUiProject(projects, users, user, onGetProjectAccess, projectSlug) {
  ...
  var orchestrationGroups =
    orchestrationTaskState.buildOrchestrationSessionGroups(accessibleSessions);
  var sessions = [];
  for (var i = 0; i < accessibleSessions.length &&
      sessions.length < LIVE_UI_MAX_SESSIONS; i++) {
    var session = accessibleSessions[i];
    if (!isLiveUiTopLevelSession(session, orchestrationGroups)) continue;
    sessions.push({
      id: session.localId,

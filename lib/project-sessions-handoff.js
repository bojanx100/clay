...
function attachProjectSessionsHandoff(ctx) {
  ...
  function handleHandoffMessage(ws, msg) {
    ...
    if (msg.type === "handoff_session") {
      ...
      // Collect situational state (git, tasks, plan docs, original goal) once
      // and share it with both the on-disk package and the inline brief — a
      // single git shell-out and dir scan instead of two.

...
function validPayload(payload) {
  ...
  if (typeof payload.portfolioTaskId !== "string" || !payload.portfolioTaskId ||
      ...
      (payload.terminalStatus !== "completed" && payload.terminalStatus !== "failed" &&
       ...
       !(payload.executionMode === "project_coordinator" && payload.terminalStatus === "needs_input" &&
         payload.ownerNotification === true) &&
...
function boundedPayload(payload) {
  ...
  if (payload.type === "coordinator_update") {
    var update = { type: payload.type, text: payload.text };
    if (typeof payload.portfolioTaskId === "string" && payload.portfolioTaskId &&
        payload.portfolioTaskId.length <= 256 && isPositiveInteger(payload.bindingRevision)) {
      update.portfolioTaskId = payload.portfolioTaskId;
      update.bindingRevision = payload.bindingRevision;
    }
    return update;
  }
  var bounded = {

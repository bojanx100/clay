...
var CONTROL_ROLES = { council: true, triage: true };
...
function controlRoleFor(task) {
  var role = cleanText(task && (task.controlRole || task.role), "").toLowerCase();
  return CONTROL_ROLES[role] ? role : "";
}
  ...
  for (var s = 0; s < sessions.length; s++) {
    ...
    for (var t = 0; t < tasks.length; t++) {
      ...
      items.push({
        ...
        // Peer control sessions are navigation aids, not task-title surfaces.
        // Preserve the role so the narrow sidebar can use a stable concise
        // label while the exact session still exposes the full objective.
        controlRole: controlRoleFor(task),

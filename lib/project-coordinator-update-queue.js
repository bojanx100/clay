...
var taskFeedback = require("./coop-task-feedback");
var MAX_ATTEMPTS = 3;
var RETRY_MS = 60000;
...
function attachCoordinatorUpdateQueue(ctx) {
  ...
  function queue(session, text, options) {
    if (!current(session) || typeof text !== "string" || !text.trim()) return false;
    var updateId = options && options.updateId || crypto.randomUUID();
    if ((session.pendingCoordinatorUpdates || []).some(function (entry) { return entry.updateId === updateId; }) ||
        (session.history || []).some(function (entry) {
          return Array.isArray(entry.coordinatorUpdateIds) && entry.coordinatorUpdateIds.indexOf(updateId) !== -1;
        })) return true;
    var entries = (session.pendingCoordinatorUpdates || []).concat({
      updateId: updateId, text: text, queuedAt: now(), state: "pending", attempts: 0,
      feedback: taskFeedback.normalized(options && options.feedback),
    });
  ...
  function flush(session) {
    ...
    var text = selected.map(function (entry) {
      return entry.text + (entry.feedback ? "\n<coop_feedback_ref>" +
        JSON.stringify(entry.feedback) + "</coop_feedback_ref>" : "");
    }).join("\n\n---\n\n");
    ...
    if (!existing) history.push({ type: "user_message", text: text, synthetic: true,
      ...
      coordinatorUpdateIds: selected.map(function (entry) { return entry.updateId; }),
      coordinatorFeedback: selected.map(function (entry) { return taskFeedback.normalized(entry.feedback); })
    ...
    ctx.onProcessingChanged();
    changed(session);
    if (session.coopHome) ctx.sendToSession(session.localId, { type: "coop_internal_turn_started" });

...
function clearOwnedCompactionState(input, ownsTurn) {
  var session = input.session;
  if (!ownsTurn || !session.compacting) return false;
  session.compacting = false;
  if (!session.destroying) {
    input.sendAndRecord(session, { type: "compacting", active: false });
  }
  return true;
}
...
function finalizeStream(input) {
  ...
  var recovery = require("./sdk-context-recovery");
  var recoveryBlocker = recovery.pending(session) ? recovery.blockedReason(input) : "";
  if (session.abortController === input.abortController) session.abortController = null;
  clearPendingState(session, input.clearInteractiveToolWaits, ownsTurn);
  clearOwnedCompactionState(input, ownsTurn);
  resetCopilotAfterHandoff(session, input.sm);
  restoreFailedHandoff(session, input.sm, ownsTurn);
  session.isProcessing = false;
  if (ownsTurn && recovery.finish(input, recoveryBlocker)) return;

...
// Revision 1 is first-time staffing: the caller has just read the backlog, so
// the board facts behind the item are fresh. Every revision above 1 is a
// RESTAFF of work admitted earlier, and admission-time eligibility proves only
// that the issue was ownable when it was admitted -- not that it still is.
// trialview/v2#2777 went out at r2 through exactly this path, long after the
// issue had been delivered, reassigned, and moved to Dev Complete.
//
// recovery-portfolio-execution gates its own restaffs, but that covers only
// the ":infrastructure-recovery-rN" path. This is the "staff-" path, which the
// #2777 incident actually came through, and it was ungated.
//
// Fail closed: a restaff with no live verdict is refused, not allowed. The
// failure this guards is doing unwanted work on someone else's issue, so
// absence of evidence must never read as permission.
function restaffGate(target, opts) {
  if (target.bindingRevision <= 1) return "";
  var revalidate = opts && opts.revalidateRestaff;
  if (typeof revalidate !== "function") return "restaff_revalidation_required";
  var verdict;
  try { verdict = revalidate(target); }
...
function composeStaffingDecision(item, route, opts) {
  ...
  var restaff = restaffGate(target, opts);
  if (restaff) return staffingAttention(item, restaff);

...
function createProjectLocalMcpServers(ctx) {
  ...
  function getLocalMcpServers(forSession) {
    ...
    if (forSession && sessionMcpGate && typeof sessionMcpGate.createServers === "function") {
      try {
        Object.assign(availableServers, sessionMcpGate.createServers(forSession) || {});
      } catch (e) {
        console.error("[project] Failed to bind session MCP servers:", e.message);
      }
    }

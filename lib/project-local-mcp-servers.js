  ...
  function getLocalMcpServers(forSession) {
    ...
    if (!isMate && !isHostAgent && slug === "lead") {
      ...
      availableServers[assignmentMcp.SERVER_NAME] = assignmentMcp.createAssignmentServer(
        adapter, sm, forSession, taskOrchestrationGate);
      ...
      availableServers[conversationMcp.SERVER_NAME] = conversationMcp.createConversationServer(adapter, sm, forSession);
      var memoryMcp = require("./coop-owner-model-mcp");
      availableServers[memoryMcp.SERVER_NAME] = memoryMcp.createOwnerMemoryServer(adapter, sm, forSession);
    }

...
function attachMemory(ctx) {
  ...
  function loadMateDigests(mateCtx, mateId, query) {
    // Same guard as loadMateClaudeMd: getMateDir throws on a non-string id.
    if (!mateId || typeof mateId !== "string") return "";
    ...
    if (hasGlobalSearch) {
      ...
      try {
        ...
        // Own sessions
        // This timeline needs a title and a timestamp -- never the transcript.
        // `s.history.length` on a released lazy session re-reads and parses the
        // whole file, so building a list of titles paged EVERY session in the
        // project into memory. That is the exact heap blowup the lazy history

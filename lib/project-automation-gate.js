...
function createAutomationGate(options) {
  ...
  function evaluateLaunch(request) {
    ...
    // Preserve a prior base-policy denial (malformed policy, invalid project,
    // unavailable authority) instead of replacing its concrete reason while
    // attempting to construct issue evidence from inputs that authority has
    // already rejected.
    if (decided.decision === authority.DENY) return record(decided);

    // Issue implementation is only discoverable when the current scan can
    // construct the project's typed receipt. A nonce says only "this scan";
    // the receipt names the issue, every board item, policy, recipe,
    // assignment, classification, and approval requirement that qualified it.
    var recipeKind = String(req.recipeKind || "issue").toLowerCase();
    var issueRecipe = recipeKind === "issue" || recipeKind === "issues";
    var receipt = null;
    if (issueRecipe) {
      var recipe = req.recipe && typeof req.recipe === "object" ? {
        id: req.recipe.id,
        digest: policyModule.recipeDigest(req.recipe),
        kind: "issue",
      } : {
        id: req.intent && req.intent.recipeId,
        digest: req.recipeDigest,
        kind: "issue",
      };
      var qualified = qualification.receiptFor({
        policy: input.policy,
        ...
        itemKey: req.itemKey,
        itemClass: itemClass,
        assignedToOwner: input.assignedToOwner,
        recipeAllowsUnassigned: input.recipeAllowsUnassigned,
        allowInFlightPrimitive: req.existingPrimitiveSession === true,
        previousQualificationReceipt: req.previousQualificationReceipt || null,
        historicalPrimitiveLaunch: req.historicalPrimitiveLaunch || null,
        now: input.now,
      });
      if (!qualified.ok) {
        decided.decision = authority.DENY;
        decided.reason = qualified.reason;
        decided.requiresApproval = true;
        decided.audit = Object.assign({}, decided.audit, {
          decision: authority.DENY,
          reason: qualified.reason,
          qualification: { verdict: "denied", reason: qualified.reason },
        });
        return record(decided);
      }
      receipt = qualified.receipt;
      var expectedAdmission = decided.reason === "policy_autonomous" ? "auto" : "owner_approval";
      if (receipt.classification.admission !== expectedAdmission) {
        decided.decision = authority.DENY;
        decided.reason = "qualification_admission_mismatch";
        decided.requiresApproval = true;
        decided.audit = Object.assign({}, decided.audit, {
          decision: authority.DENY,
          reason: "qualification_admission_mismatch",
          qualification: { verdict: "denied", reason: "qualification_admission_mismatch" },
        });
        return record(decided);
      }
    }
    ...
    var candidate = {
      ...
      safety: scopedAutonomy.assessCandidateSafety({
        ...
        spendBudget: req.spendBudget === true,
  ...
  function verifyAuthorization(authorization, projectRef, verificationOptions) {
    ...
    // responsibility for this work yet. A completed binding normally grants
    // no new authority either. The one narrow exception is the owner's Done
    // workflow after this exact adopted primitive already reported its internal
    // implementation complete: completion releases worker capacity, but must
    // not make the owner's separately authenticated close step impossible.
    var completedOwnerDone = verificationOptions &&
      verificationOptions.allowCompletedOwnerDone === true &&
      binding.status === "completed" && binding.automationAuthorization &&
      binding.automationAuthorization.kind === automationAuthorization.PRIMITIVE_KIND;
    if (binding.status !== "active" && !completedOwnerDone) {
  ...
  function evaluateExternal(request) {
    ...
    var leadMode = readLeadMode() === true;
    var projectRef = resolveProjectRef();
    var allowCompletedOwnerDone = req.externalKind === "done_workflow" &&
      req.ownerTriggered === true && req.approval && req.approval.granted === true &&
      typeof req.approval.by === "string" && req.approval.by.trim().length > 0;
    var verdict = leadMode ?
      verifyAuthorization(req.coopAuthorization, projectRef, {
        allowCompletedOwnerDone: allowCompletedOwnerDone,
      }) : { ok: true };

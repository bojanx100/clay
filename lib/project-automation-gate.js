...
function createAutomationGate(options) {
  ...
  function holdsClaim(itemKey) {
    ...
    return !!lease && lease.holder === holder && lease.expiresAt > now();
  }
  ...
  function reconcileClaims(activeItemKeys) {
    ...
    for (var j = 0; j < held.length; j++) {
      ...
      if (leases.release({ projectRef: projectRef, key: lease.key, holder: holder }).ok) released++;
    }
    ...
    return {
      ok: true, adopted: adopted, released: released,
      reclaimed: reclaimed, skippedRecent: skippedRecent,
    };
  }

...
function createAutomationGate(options) {
  ...
  function evaluateLaunch(request) {
    ...
    var acquired = leases.acquire({
      ...
      policyDigest: (probeInput.policy && probeInput.policy.digest) || null,
      ttlMs: claimTtlMs,
    });
    if (!acquired.ok) {
      // A held claim is the no-duplicate-launch guarantee doing its job. Name
      // WHO holds it, because "our own earlier tick is still on this" and
      // "another process owns this" are very different operationally.
      if (acquired.reason === "held") {
        var mineAlready = acquired.lease && acquired.lease.holder === holder;
        return record(withReason(probe, "deny",
          mineAlready ? "claim_already_active" : "claim_held_elsewhere"));
      }
      return record(withReason(probe, "deny", acquired.reason));
    }
    if (acquired.created !== true) {
      // Our own runtime already claimed this item on an earlier, overlapping
      // tick and has not launched it yet.
  ...
  function reconcileClaims(activeItemKeys) {
    ...
    for (var j = 0; j < held.length; j++) {
      ...
      if (!active[lease.key]) {
        // Our own in-flight states are owned by a live turn of this process;
        // touching them would race the very launch reconciliation protects.
        // Only a RUNNING claim with no live work is genuinely an orphan.
        if (mine && lease.state !== "RUNNING") { settled[lease.key] = true; continue; }
        // Not our work and not ours to judge: another process may be running
        // it right now. Only our own orphans are released outright.
        if (mine) {
          var freed = leases.release({
            projectRef: projectRef, key: lease.key, holder: holder, token: lease.token,
          });
          if (freed.ok) { released++; settled[lease.key] = true; }
          else failures.push({ op: "release", key: lease.key, reason: freed.reason });
          continue;
        }
        // A foreign claim for work we are not running. Clear it only when its
        // holder is provably dead AND no session exists for it — never on a
        // timer, and never while the holder might still be working.
        var orphan = leases.resolveOrphan({
          projectRef: projectRef, key: lease.key, holder: holder,
          holderPid: holderPid, ttlMs: claimTtlMs,
        }, { isHolderAlive: isHolderAlive, sessionExists: false });
        if (orphan.ok && orphan.released) { released++; settled[lease.key] = true; }
        else if (orphan.reason === "held" || orphan.reason === "ambiguous_intent") foreign++;
        else if (!orphan.ok) failures.push({ op: "orphan", key: lease.key, reason: orphan.reason });
        continue;
      }

      if (mine) {
        // Only RUNNING is renewable. CLAIMED and LAUNCHING are in-flight
        // states owned by a live turn of this process; leaving them alone is
        // what stops reconciliation from racing an active launch.
        if (lease.state !== "RUNNING") { settled[lease.key] = true; continue; }
        var renewed = leases.renew({
          projectRef: projectRef, key: lease.key, holder: holder,
          token: lease.token, ttlMs: claimTtlMs,
        });
      ...
      var takenOver = leases.resolveOrphan({
        projectRef: projectRef, key: lease.key, holder: holder,
        holderPid: holderPid, ttlMs: claimTtlMs,
      }, { isHolderAlive: isHolderAlive, sessionExists: true });
      if (takenOver.ok) { adopted++; settled[lease.key] = true; }
      else if (takenOver.reason === "held" || takenOver.reason === "ambiguous_intent") {
        foreign++;
        settled[lease.key] = true;
      } else {
        failures.push({ op: "adopt", key: lease.key, reason: takenOver.reason });
      }
    }
    ...
    for (i = 0; i < activeKeys.length; i++) {
      ...
      // Running work with no live claim: re-acquire and drive it straight to
      // RUNNING, since the session demonstrably exists.
      var retaken = leases.acquire({
        projectRef: projectRef, key: activeKeys[i], holder: holder,
        holderPid: holderPid, ttlMs: claimTtlMs,
      });
      if (retaken.ok) {
        var promoted = leases.beginLaunch({
          projectRef: projectRef, key: activeKeys[i], holder: holder,
          token: retaken.lease.token, intent: { recovered: true },
        });
        if (promoted.ok) {
          leases.confirmRunning({
            projectRef: projectRef, key: activeKeys[i], holder: holder,
            token: retaken.lease.token, ttlMs: claimTtlMs,
          });
        }
        reclaimed++;
      }
  ...
  // THE LAUNCH FENCE. Renewing a lease before starting could never be a fence:
  // the process can be suspended past the TTL between the renew and the
  // launch, another holder can take the item, and the first launch still
  // happens. So the commitment is made DURABLE before any side effect —
  // CLAIMED -> LAUNCHING, carrying the fencing token, the canonical claim key,
  // the actor and the policy digest, and the launch intent.
  //
  // Two properties follow. A crash after this point leaves a durable record
  // that recovery can reason about instead of an invisible half-start; and
  // LAUNCHING does not expire, so no timer can ever license a second launch
  // while the first is unresolved.
  //
  // Policy is re-read and re-checked HERE rather than trusted from
  // evaluateLaunch, because an owner can tighten it in between.
  function beginLaunch(request) {
    var req = request || {};
    var projectRef = resolveProjectRef();
    if (!projectRef || !req.itemKey || !req.token) {
      return { ok: false, reason: "invalid_claim" };
    }
    // Revalidate authority against CURRENT policy, not the decision's copy.
    refresh();
    var revalidated = authority.decideAutomation(baseInput("launch", {
      itemClass: req.itemClass || null,
      itemKey: req.itemKey,
      approval: req.approval || null,
      claim: { held: true, holder: holder, expiresAt: now() + claimTtlMs },
    }));
    if (revalidated.decision !== authority.EXECUTE) {
      record(revalidated);
      return { ok: false, reason: "policy_revoked:" + revalidated.reason };
    }
    var loaded = currentPolicy();
    return leases.beginLaunch({
      projectRef: projectRef,
      key: claimKeyFor(req.itemKey),
      holder: holder,
      token: req.token,
      actor: req.actor || null,
      policyDigest: (loaded && loaded.ok && loaded.policy.digest) || null,
      intent: req.intent || null,
    });
  }

  // LAUNCHING -> RUNNING, once canonical session metadata is persisted. Only
  // now is the claim renewable, and only now is the work "started" as far as
  // recovery is concerned.
  function confirmRunning(request) {
    var req = request || {};
    var projectRef = resolveProjectRef();
    if (!projectRef || !req.itemKey || !req.token) {
      return { ok: false, reason: "invalid_claim" };
    }
    return leases.confirmRunning({
      projectRef: projectRef,
      key: claimKeyFor(req.itemKey),
      holder: holder,
      token: req.token,
  ...
  // Read-only: does a live claim for this item exist at all, in any state?
  function holdsClaim(itemKey) {
    var projectRef = resolveProjectRef();
    if (!projectRef || !itemKey) return false;
    var lease = leases.get(projectRef, claimKeyFor(itemKey));
  ...
  function renewClaim(itemKey) {
    ...
    var lease = leases.get(projectRef, claimKeyFor(itemKey));
    if (!lease) return { ok: false, reason: "not_held" };
    return leases.renew({
      projectRef: projectRef, key: claimKeyFor(itemKey), holder: holder,
      token: lease.token, ttlMs: claimTtlMs,
    });
  }
  ...
  function releaseClaim(itemKey) {
    var projectRef = resolveProjectRef();
    if (!projectRef || !itemKey) return { ok: false, reason: "invalid_claim" };
    var current = leases.get(projectRef, claimKeyFor(itemKey));

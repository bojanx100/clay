...
// A claim renewed this recently is presumed to belong to a live holder. During
// a daemon replacement the old process can still be running and claiming while
// the new one reconciles, and both share a variant identity.
var RECLAIM_GRACE_MS = 120000;
...
// --- Holder identity ----------------------------------------------------------
//
// A holder must satisfy two opposing requirements:
//   * STABLE across a restart of the same daemon, or the restart reconcile
//     could not re-adopt its own in-flight claims and would double-start work;
//   * DISTINCT between daemons running concurrently, or one daemon's reconcile
//     would treat another's live lease as its own and renew or release it.
//
// `"project-automation:" + slug` alone satisfies only the first. A dev and a
// prod daemon share CLAY_HOME (config.js only suffixes the daemon FILES), so
// they would collide exactly. The instance id below is therefore persisted
// per daemon variant: created once, reused across restarts of that variant,
// and different for the other variant.
var INSTANCE_ID_RE = /^[0-9a-f-]{36}$/;

function instanceIdFile() {
  return path.join(config.CONFIG_DIR,
    config.isDevMode ? "automation-holder-dev.id" : "automation-holder.id");
}

function runtimeInstanceId(fsImpl, file) {
  var target = file || instanceIdFile();
  try {
    var existing = String(fsImpl.readFileSync(target, "utf8")).trim();
  ...
  try {
    ...
    fsImpl.writeFileSync(target, created + "\n", { encoding: "utf8", mode: 0o600 });
  } catch (e) {
    // If it cannot be persisted the id is per-process rather than per-daemon.
    // That is the SAFE direction: a fresh id can never be mistaken for another
    // holder's, it only costs one TTL of adoption after a restart.
    return created;
  }
  return created;
}
...
function createAutomationGate(options) {
  ...
  // Stable across this daemon's restarts, distinct from any other daemon
  // sharing CLAY_HOME. See runtimeInstanceId above for why both matter.
  var holder = opts.holder ||
    ("project-automation:" + (slug || "unknown") + ":" +
      runtimeInstanceId(opts.fs || fs, opts.instanceIdFile));
  ...
  function reconcileClaims(activeItemKeys) {
    ...
    if (readLeadMode() !== true) return { ok: true, adopted: 0, released: 0, skipped: "lead_mode_off" };
    var projectRef = resolveProjectRef();
    ...
    var reclaimed = 0;
    var skippedRecent = 0;
    // Individual claim operations can fail (a busy store, a lost commit race).
    // Reporting ok:true regardless would tell the caller reconciliation had
    // succeeded when live work may have been left unclaimed.
    var failures = [];
    var timestamp = now();
    ...
    for (var j = 0; j < held.length; j++) {
      ...
      if (active[lease.key]) {
        ...
        if (renewed.ok) adopted++;
        else failures.push({ op: "renew", key: lease.key, reason: renewed.reason });
        continue;
      }
      // Not active HERE — but during a daemon replacement the outgoing process
      // shares this variant's identity and may have claimed the item moments
      // ago, after we loaded sessions. Releasing that would hand the item to a
      // duplicate launch, so a recently refreshed claim is left alone and
      // simply allowed to lapse if it really is dead.
      var lastRefreshed = lease.expiresAt - claimTtlMs;
      if (timestamp - lastRefreshed < RECLAIM_GRACE_MS) {
        skippedRecent++;
        continue;
      }
      var freedLease = leases.release({ projectRef: projectRef, key: lease.key, holder: holder });
      if (freedLease.ok) released++;
      else failures.push({ op: "release", key: lease.key, reason: freedLease.reason });
    }

    // The mirror case, and the one that actually causes duplicate launches:
    // work that is still running but whose claim is missing or already lapsed
    // (a long restart, or a holder id that could not be persisted). Re-acquire
    // it, or the next tick — or another daemon — will start it again.
    var activeKeys = Object.keys(active);
    for (i = 0; i < activeKeys.length; i++) {
      if (seen[activeKeys[i]]) continue;
      var retaken = leases.acquire({
        projectRef: projectRef, key: activeKeys[i], holder: holder, ttlMs: claimTtlMs,
      });
    ...
    if (failures.length) {
      console.error("[automation-gate] reconciliation left " + failures.length +
        " claim operation(s) unresolved for project " + (slug || "?") + ": " +
    ...
    return {
      ...
      skippedRecent: skippedRecent, failures: failures,
    };
  }

...
var INITIAL_TITLE_MAX_WORDS = 12;
var INITIAL_TITLE_MAX_CHARS = 96;
...
var NON_SUBJECT_WORDS = {
  ...
  // Inflected request verbs are phrasing, not evidence that two owner turns
  // share a subject. Treating "needs" as a weak topical match made an
  // unrelated request inherit whichever recent Thread also happened to need
  // something.
  needs: true, wants: true,
};
...
// New automatic Threads get a coherent complete clause when one already fits
// the sidebar bound. Historical retrofit keeps readableTitle's tighter legacy
// contract, while progressive refinement can replace this initial purpose once
// several related owner turns support something clearer.
function completeInitialTitle(text) {
  var parts = splitClauses(text);
  for (var i = 0; i < parts.length; i++) {
    if (clauseInformative(parts[i]) < 2) continue;
    var candidate = String(parts[i] || "").trim()
      .replace(/^(?:what do you mean by|i think|i need|i want|we need|we want|can you|could you|would you|please)\s+/i, "");
    var words = candidate.split(/\s+/);
    var start = 0;
    while (start < words.length) {
      var lead = words[start].toLowerCase().replace(/[^a-z0-9']/g, "");
      if (lead && BOILERPLATE_LEAD[lead]) start++;
      else break;
    }
    words = words.slice(start);
    candidate = words.join(" ");
    if (words.length > TITLE_MAX_WORDS) {
      var boundary = candidate.match(/^(.+?)\s+(?:(?:initially|currently)\s+)?(?:needs?|must|should|requires?)\b/i);
      var subjectWords = boundary ? boundary[1].trim().split(/\s+/) : [];
      if (subjectWords.length >= 3 && subjectWords.length <= TITLE_MAX_WORDS) {
        candidate = boundary[1].trim();
        words = subjectWords;
      }
    }
    var title = candidate.replace(/[\s,;:.!?…]+$/, "");
    if (!title || words.length > INITIAL_TITLE_MAX_WORDS ||
        title.length > INITIAL_TITLE_MAX_CHARS) continue;
    return title.charAt(0).toUpperCase() + title.slice(1);
  }
  return "";
}
...
function derivedMetadata(text) {
  ...
  var title = completeInitialTitle(text) || diagnosticTitle(text) || readableTitle(text);
...
module.exports = {
  ...
  completeInitialTitle: completeInitialTitle,

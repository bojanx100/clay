...
// Unsolicited process signals cannot authorize taking the whole service down.
// Ctrl+C remains the explicit operator stop; daemon restart uses exit code 120.
function shouldStopWatcher(signal) {
...
function takeOverExistingDev(config, options) {
  ...
  function waitForWatcherExit(attempt) {
    if (!watcherStillAlive(priorWatcher)) return Promise.resolve();
    if (attempt >= waitAttempts) {
      throw new Error("Existing Clay supervisor is still running; takeover refused to preserve service.");
    }
...
module.exports = {
  shouldStopWatcher: shouldStopWatcher,

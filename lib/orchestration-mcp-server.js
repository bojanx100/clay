...
function getToolDefs(onDelegate, onMessage, onPlan, onReport, onRetry, onAdopt, onResolve,
  ...
  return [{
    ...
    description: "Pause owned tasks for one user decision, or stage a non-scheduling approval question for exact portfolio task/revision/ProjectRef scopes before asking the owner. Use only when human judgment is genuinely unavoidable.",
    inputSchema: {
      coordinatorSessionId: coordinatorIdField("Stable coordinator id stated in the coordinator instructions"),
      taskIds: z ? z.array(z.string()).min(1).optional().describe("Affected existing owned task ids; mutually exclusive with approvalScopes") : {},
      ...
      })).min(1).max(16).optional().describe("Exact not-yet-delegated portfolio revisions to stage without scheduling; mutually exclusive with taskIds") : {},
      question: stringField("One precise question for existing taskIds; omitted for approvalScopes because Clay generates the exact staged question", true),

...
function applyMigration(ledger, session, migration) {
  ...
  if (reason) return { migrationId: migration.migrationId || "", ok: false, reason: reason };
  return Object.assign({ migrationId: migration.migrationId || "" },
    reconcileOwnerRequestEvidence(ledger, session, migration.evidence || {}));
}

// Runs synchronously after the Lead session manager has restored its canonical
// Coop transcript. Generic replay repairs facts provable from ordinary turn
// structure (including explicit Implement approval); the finite migration then
// settles only separately audited response events. Both paths are idempotent.
function migrateOwnerRequestHistory(ledger, sm, options) {
  ...
  for (var i = 0; i < selected.migrations.length; i++) {
    var result = applyMigration(ledger, session, selected.migrations[i]);

...
function prepareExactMigrationRequests(ledger, session, migration) {
  ...
  for (var i = 0; i < requests.length; i++) {
    ...
    if (!event || !event.coopIngressId || event.coopIngressSequence !== expected.sequence ||
        (expected.ingressId && event.coopIngressId !== expected.ingressId)) {
      return { ok: false, reason: "request_evidence_changed", recorded: recorded };
    }
    ...
    if (existing && (!existing.requestRef ||
        ...
        existing.requestRef.eventIndex !== expected.eventIndex ||
        existing.ingressSequence !== expected.sequence)) {
      return { ok: false, reason: "request_ref_mismatch", recorded: recorded };
    }
    prepared.push({ event: event, expected: expected, existing: existing });
  }
    ...
    if (!item.existing) {
      var created = ledger.record({
        ...
        requestRef: {
          ...
          eventIndex: item.expected.eventIndex,
        },

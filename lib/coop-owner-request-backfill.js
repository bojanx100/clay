...
//   superseded  -- the owner repeated the same request before this copy ended;
...
function auditOwnerRequests(history) {
  ...
  for (var k = 0; k < ingresses.length; k++) {
    ...
    var implementationDecision = event.coopImplementationDecision ||
      explicitImplementationDecision(event.text);
    if (implementationDecision) {
      implementationDecision = Object.assign({}, implementationDecision, {
        source: implementationDecision.source || "explicit_owner_turn",
        at: implementationDecision.at || event._ts || null,
      });
    }
    audited.push({

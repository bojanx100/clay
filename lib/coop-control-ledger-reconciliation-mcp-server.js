...
function reconcile(deps, input) {
  ...
  if (!session) return result({ ok: false, code: "not_authorized" }, true);
  ...
  var projectId = deps.sm && typeof deps.sm.getProjectId === "function"
    ? deps.sm.getProjectId() : null;
  var canonicalOwner = isCanonicalCoopSession(session) &&
    projectId === projectIdentity.LEAD_PROJECT_ID;
  for (var ti = 0; ti < topicChanges.length; ti++) {
    if (topicChanges[ti] &&
        topicChanges[ti].closeOutcome === threadLifecycle.CLOSE_OUTCOMES.NOT_PURSUING &&
        !canonicalOwner) {
      return result({ ok: false, code: "owner_authorization_required" }, true);
    }
  }
  var ledgers = stores(deps);
  var completed = [];
...
function linkOwnerResponse(deps, input) {
  ...
  var linked = ownerResponseLinkage.stageOwnerResponse({
    ...
    saveSession: deps.sm && typeof deps.sm.saveSessionFile === "function" ?
      function (current) { return deps.sm.saveSessionFile(current, { durable: true }); } : null,
  });
  if (linked.ok && deps.sm.sendToSession) deps.sm.sendToSession(session, {
    type: "coop_owner_response_started",
    coopTopicRefs: (input.requests || []).map(function (request) {
      var record = stores(deps).ownerRequests.get(request.ingressId);
      return record && record.topicRef;
    ...
    inputSchema: Object.assign({}, auth, {
      ...
      topics: z ? z.array(z.object({
        ...
        closeOutcome: z.enum([
          ...
          threadLifecycle.CLOSE_OUTCOMES.NOT_PURSUING,
        ]).optional(),

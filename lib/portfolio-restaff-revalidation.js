...
function normalizeObservation(value, now) {
  ...
  return {
    ...
    hasBoardStatus: Object.prototype.hasOwnProperty.call(value, "boardStatus"),
    state: state || null,
...
function restaffEligibility(input) {
  ...
  var status = normalizedStatus(observation.boardStatus);
  ...
  for (var i = 0; i < policy.boardExclusions.length; i++) {
    if (policy.boardExclusions[i] === status) {
      return decision(false, "board_status_excluded", { boardStatus: observation.boardStatus });
    }
  }

...
function readOnlyReviewAwaitingLocalAcceptance(record, role) {
...
function coordinatorVerifiedReadOnlyReviewCompletesDirectly(record, input) {
  ...
  return !!(readOnlyReviewAwaitingLocalAcceptance(record, role) && input &&
    input.terminalStatus === "completed" && input.ownerNotification !== true &&
...
function ownerAcceptanceTransitionMatches(record, input, terminalStatus, accepted) {
  return record.status === "needs_input" && record.ownerAcceptanceRequired === true &&
    terminalStatus === "completed" && accepted &&
    typeof input.implementationCompletedAt === "number" &&
    input.implementationCompletedAt === record.implementationCompletedAt &&
    Number(input.implementationCompletionRevision) === Number(record.implementationCompletionRevision) &&
    String(input.implementationGraphDigest || "") === String(record.implementationGraphDigest || "");
}
...
function createBindingCompletionApi(ctx) {
  ...
  function complete(taskId, revision, details) {
    ...
    if (record.status === "completed" || record.status === "failed" ||
      ...
      var acceptanceTransition = ownerAcceptanceTransitionMatches(record, input, terminalStatus, accepted);

...
function compactPriorHistory(session, latestIndex, includeInterruptedResponse) {
  var history = session && Array.isArray(session.history) ? session.history : [];
  if (includeInterruptedResponse && latestIndex >= 0) {
    return history.slice(0, latestIndex).concat(history.slice(latestIndex + 1));
  }
...
function buildCompactContinuationPrompt(session, options) {
  ...
  var priorHistory = compactPriorHistory(session, latestIndex, opts.includeInterruptedResponse);
...
function attachSessionCompaction(ctx) {
  ...
  function compactAndContinue(sourceSession, options) {
    ...
    var inPlace = !!controlled || opts.inPlace === true;
    function authorizedRenewal(session) {
      if (session.destroying || session.taskStopRequested || session._deleted || session.hidden) return false;
      if (opts.automatic && (controlled || session.coopHome) &&
          (ctx.getLeadMode || require("./lead-mode").getLeadMode)() !== true) return false;
      if (!controlled && !session.coopIncarnation) return true;
      var execution = session.orchestrationPolicy && session.orchestrationPolicy.portfolioExecution;
      return !!((!controlled || execution && execution.status === "running") && session._coopExecutionFence &&
        require("./coop-control-fence").isCurrent(session, "provider_start"));
    }
    // Retracted: refusing every controlled compaction left exhausted workers
    // stuck. Renew only the provider conversation, keeping the exact Clay
    // SessionRef, capability, task graph and pending coordinator messages.
    if (controlled && !authorizedRenewal(sourceSession)) {
      sm.sendAndRecord(sourceSession, { type: "error",
        text: "Cannot renew controlled execution " + controlled.executionId +
          ": its current runtime capability is unavailable or the execution ended." });
      return null;
    }
    if (!resident && !inPlace && hasUnresolvedOrchestrationTasks(sourceSession)) {
      sendToSession(sourceSession.localId, {
        type: "error", text: "Cannot compact a coordinator while its worker tasks still need attention.",
      });
      return null;
    }
    ...
    var prompt = buildCompactContinuationPrompt(sourceSession, {
      ...
      includeInterruptedResponse: inPlace || resident,
    });
    if (resident || inPlace) return residentCompaction.compactResident(
      Object.assign({}, ctx, { validateRenewal: authorizedRenewal }), sourceSession, prompt, latestImages);

var residentCompaction = require("./coop-coordinator-compaction");
var crypto = require("crypto");
var { buildHandoffContextFromHistory } = require("./handoff-context");
var coopChannels = require("./project-coop-channels");
var ownerDecisionStaging = require("./coop-owner-decision-staging");
...
function findLatestUserMessage(session) {
  ...
  for (var i = history.length - 1; i >= 0; i--) {
    var item = history[i];
    if (item && item.type === "user_message" && !item.queuedPending && !item.coopIngressPending) {
...
function activeCoopUserMessage(session) {
  var state = session && session.coopConversationIngress;
  var ingressId = state && state.activeIngressId;
  var history = session && Array.isArray(session.history) ? session.history : [];
  if (!ingressId) return null;
  for (var i = history.length - 1; i >= 0; i--) {
    var item = history[i];
    if (!item || item.type !== "user_message") continue;
    if (item.coopIngressId === ingressId || item.coopContinuationIngressId === ingressId) {
      return { item: item, index: i };
    }
  }
  return null;
...
function compactionUserMessage(session) {
  return activeCoopUserMessage(session) || findLatestUserMessage(session);
}
...
function imageRefsForLatestMessage(latest) {
  var item = latest && latest.item ? latest.item : null;
  if (!item || !Array.isArray(item.imageRefs)) return [];
  var refs = [];
  for (var i = 0; i < item.imageRefs.length; i++) {
    var ref = item.imageRefs[i];
    if (!ref || !ref.file) continue;
    refs.push({ mediaType: ref.mediaType || "image/png", file: ref.file });
  }
  return refs;
}

function latestImageReferenceText(latestItem, imagesDir) {
  if (!latestItem || !Array.isArray(latestItem.imageRefs) || latestItem.imageRefs.length === 0) return "";
  var lines = [];
  for (var i = 0; i < latestItem.imageRefs.length; i++) {
    var ref = latestItem.imageRefs[i];
    if (!ref || !ref.file) continue;
    var label = imagesDir ? require("path").join(imagesDir, ref.file) : ref.file;
...
function buildCompactContinuationPrompt(session, options) {
  ...
  var currentText = compactCurrentText(opts, latestItem) + latestImageReferenceText(latestItem, opts.imagesDir);
...
function compactionReasonText(session, options) {
  var opts = options || {};
  if (opts.reason === "claude_image_history") {
    return "Claude's native history contained an image that the API could no longer process.";
  }
...
// The durable control-plane binding a Coop-controlled execution runs under.
// This is the exact shape `coop-control-fence.controlMetadata` reads for a
// non-Coop-home session, and the exact key `server-cross-project`'s
// `controlledSessionIndex` uses to resolve an execution back to its session.
// Read locally rather than through the fence module so compaction keeps no
// dependency on the SQLite control store, and so the canonical Coop home
// session -- whose `coopIncarnation` the fence also reports, and whose
// compaction/rotation is a supported operation -- is deliberately NOT matched.
function controlPlaneBinding(session) {
  var policy = session && session.orchestrationPolicy;
  var execution = policy && policy.portfolioExecution;
  return execution && execution.control && execution.control.executionId ?
...
// A typed plan decision has no worker/session to orphan. Keeping it on the
// source solely to avoid compaction would contradict its restart-safe owner
// lifecycle, so move this exact validated record with the continuation. Other
// unresolved work remains fenced and continues to refuse compaction.
function isTransferableOwnerDecision(task) {
  var decision = ownerDecisionStaging.validDecision(task && task.ownerDecision);
  return !!decision && decision.state === "unanswered";
}
...
function hasUnresolvedOrchestrationTasks(session) {
  ...
  for (var i = 0; i < tasks.length; i++) {
    if (!isSettledTask(tasks[i]) && !isTransferableOwnerDecision(tasks[i])) return true;
  }
  return false;
}
...
function transferCoopIngressState(sourceSession, newSession) {
  if (!sourceSession.coopHome && !sourceSession.coopChannel) return;
  // The predecessor keeps its canonical transcript for owner-ledger references,
  // while the live ingress lane moves exactly once to the only dispatchable
  // continuation. Queue dispatch reconstructs each visible turn there.
  if (sourceSession.coopConversationIngress) {
    newSession.coopConversationIngress = sourceSession.coopConversationIngress;
    delete sourceSession.coopConversationIngress;
  }
  if (Array.isArray(sourceSession.pendingCoopIngress) && sourceSession.pendingCoopIngress.length > 0) {
    newSession.pendingCoopIngress = sourceSession.pendingCoopIngress;
    delete sourceSession.pendingCoopIngress;
  }
}

function copyContinuationProvenance(message, latestItem, activeIngressId) {
  if (activeIngressId) message.coopContinuationIngressId = activeIngressId;
  ...
  for (var i = 0; i < fields.length; i++) {
    if (Object.prototype.hasOwnProperty.call(latestItem, fields[i])) {
      message[fields[i]] = latestItem[fields[i]];
    }
  }
}

function rebaseCoopResponseState(session, continuedUserMessageIndex, activeIngressId) {
  var state = session && session.coopConversationIngress;
  if (!state) return;
  var responseStart = Array.isArray(session.history) ? session.history.length : 0;
  if (activeIngressId) state.activeResponseStartIndex = responseStart;
  var pending = state.pendingOwnerResponse;
  if (!pending || pending.version !== 1 || !pending.turnRef) return;
  pending.turnRef = Object.assign({}, pending.turnRef, {
    sessionStorageId: session.storageId || session.cliSessionId,
    eventIndex: continuedUserMessageIndex,
  });
  pending.responseStartEventIndex = responseStart;
}
...
function attachSessionCompaction(ctx) {
  ...
  var loadImagesForSdk = ctx.loadImagesForSdk || function () { return []; };
  ...
  function compactAndContinue(sourceSession, options) {
    ...
    var resident = residentCompaction.isResident(sourceSession, sm);
    if (!resident && hasUnresolvedOrchestrationTasks(sourceSession)) {
    ...
    // A Coop-controlled execution is FENCED to one exact session identity. The
    // control plane pins `coop_control_incarnations.session_storage_id` to it,
    // `controlledSessionIndex` resolves the execution back through it, and
    // graceful restart refuses to checkpoint an execution whose pinned session
    // is not the live one.
    //
    // Compaction mints a brand-new storageId and MOVES `orchestrationPolicy`
    // (control metadata included) onto it, while nothing repoints the control
    // plane -- it has no concept of compaction at all. The successor is not
    // even a usable controlled session: `_coopExecutionFence` is a
    // non-enumerable own property, so the runtime capability does not travel
    // and `coop-control-fence.fenceFor()` throws COOP_CONTROL_FENCE_MISSING on
    // it. The observed result was a silent orphan: the control plane kept
    // reporting the execution running while zero turns ran, and the next
    // graceful restart failed preflight and closed controlled ingress
    // process-wide.
    //
    // Re-homing the binding here instead would mean minting a fresh capability,
    // repointing the incarnation row, the binding store, the session ledger and
    // the topic claim -- the whole `migrate_control_plane_binding` surface --
    // implicitly, from a provider-failure path, in a project-local module with
    // no cross-project authority. That is the authority widening the control
    // plane exists to prevent. Refuse instead: the execution keeps its exact
    // pinned session, so restart still checkpoints it, and the owner can steer,
    // complete or dismiss it through the ingress that stays open.
    var controlled = controlPlaneBinding(sourceSession);
    if (controlled) {
      sm.sendAndRecord(sourceSession, {
        type: "error",
        text: "Clay did not compact this session: it runs a Coop-controlled execution (" +
          controlled.executionId + ") that is bound to this exact session identity. " +
          "Compacting would re-home the conversation and orphan the control plane. " +
          "Steer, complete or dismiss the execution from Coop instead.",
      });
      return null;
    }
    sourceSession._compactionInProgress = true;

    var latest = opts.latestUserMessage || compactionUserMessage(sourceSession);
    var ingressState = sourceSession.coopConversationIngress;
    var activeIngressId = ingressState && ingressState.activeIngressId || null;
    var latestImageRefs = imageRefsForLatestMessage(latest);
    var latestImages = null;
    if (latestImageRefs.length > 0) {
      try {
        var reloaded = loadImagesForSdk(latestImageRefs);
        if (Array.isArray(reloaded) && reloaded.length > 0) latestImages = reloaded;
      } catch (e) {
        console.error("[compaction] Failed to reload latest image attachments:", e.message || e);
      }
    }
    ...
    if (resident) return residentCompaction.compactResident(ctx, sourceSession, prompt, latestImages);
    ...
    transferCoopIngressState(sourceSession, newSession);
    ...
    if (latestImageRefs.length > 0) {
      continuedUserMessage.imageRefs = latestImageRefs;
      continuedUserMessage.imageCount = latestImageRefs.length;
    }
    copyContinuationProvenance(continuedUserMessage, latest && latest.item, activeIngressId);
    sm.sendAndRecord(newSession, continuedUserMessage);
    rebaseCoopResponseState(newSession, newSession.history.length - 1, activeIngressId);
    ...
    sdk.startQuery(newSession, prompt, latestImages, ensureProjectAccessForSession(newSession));
    return newSession;
  }
...
module.exports = {
  ...
  buildCompactContinuationPrompt: buildCompactContinuationPrompt,
  controlPlaneBinding: controlPlaneBinding,

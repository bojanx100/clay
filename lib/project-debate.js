...
function attachDebate(ctx) {
  ...
  function handleDebateStart(ws, msg) {
    ...
    // Validate that the moderator and panelists resolve to REAL Mates in
    // this user's registry before going live (F-10). Without this, a debate
    // with unknown ids starts anyway, builds an empty mention name-map, and
    // dead-ends after the moderator's first turn (no mention ever matches →
    // instant conclude-confirm). The MCP approval path got this validation
    // with F-5; the native path was still trusting its input.
    var resolvedModerator = resolveMateId(mateCtx, msg.moderatorId);
    if (!resolvedModerator) {
      ctx.sendTo(ws, { type: "debate_error", error: "Moderator does not match an existing Mate. Debate not started." });
      return;
    }
    msg.moderatorId = resolvedModerator;
    var resolvedPanelists = [];
    for (var rpi = 0; rpi < msg.panelists.length; rpi++) {
      var rp = msg.panelists[rpi];
      var rpid = resolveMateId(mateCtx, rp && rp.mateId);
      if (!rpid) {
        console.warn("[debate] Dropping unknown panelist mateId:", rp && rp.mateId);
        continue;
      }
      resolvedPanelists.push({ mateId: rpid, role: (rp && rp.role) || "", brief: (rp && rp.brief) || "" });
    }
    if (!resolvedPanelists.length) {
      ctx.sendTo(ws, { type: "debate_error", error: "None of the panelists match existing Mates. Debate not started." });
      return;
    }
    msg.panelists = resolvedPanelists;
  ...
  function handleDebatePauseToggle(ws, msg) {
    var session = ctx.getSessionForWs(ws);
    // Diag breadcrumb: pause clicks have failed silently twice (F-9 ghost
    // state, then an unexplained no-op on 2026-08-01). Record exactly what
    // this handler saw so the next failure names itself in diag-dev.log.
    try {
      require("./config").diagLog("[DEBATE-PAUSE] " + new Date().toISOString() +
    ...
    if (!wantPaused && debate._pendingAdvance) {
    ...
    ctx.sendToSession(session.localId, { type: "debate_pause_state", paused: wantPaused, holding: !!(wantPaused && debate._pendingAdvance) });
  }
  ...
  function handleMcpDebateApproval(session, briefData, mateId, ws) {
    if (!session) {
      ...
      if (ws) ctx.sendTo(ws, { type: "debate_error", error: "Could not match this approval to an active session. Open the session that proposed the debate and approve again." });
    ...
    if (session._debate && (session._debate.phase === "preparing" || session._debate.phase === "reviewing" || session._debate.phase === "live")) {
      ...
      if (ws) ctx.sendTo(ws, { type: "debate_error", error: "Another debate is already active on this session. Stop it before starting a new one." });
    ...
    // Resolve panelists and moderator to real Mates BEFORE building state.
    // A null moderator used to flow into startDebateLive and crash the whole
    // daemon on getMateDir(path.join(root, null)) — 2026-07-31.
    var panelists = (briefData.panelists || []).map(function (p) {
      var rid = resolveMateId(mateCtx, p && p.mateId);
      if (!rid) {
        console.warn("[debate] Dropping unknown panelist mateId:", p && p.mateId);
        return null;
      }
      return { mateId: rid, role: (p && p.role) || "", brief: (p && p.brief) || "" };
    }).filter(function (p) { return !!p; });

    if (!panelists.length) {
      console.warn("[debate] Cannot start MCP debate: no proposed panelist matches an existing Mate");
      if (ws) ctx.sendTo(ws, { type: "debate_error", error: "None of the proposed panelists match existing Mates. Debate not started." });
      return { ok: false, reason: "no proposed panelist matches an existing Mate" };
    }
    ...
    if (!moderatorId) {
      ...
      if (moderatorId) {
        console.log("[debate] No moderator mate on proposal; falling back to", moderatorId);
      }
    }
    if (!moderatorId) {
      console.warn("[debate] Cannot start MCP debate: no available Mate can moderate");
      if (ws) ctx.sendTo(ws, { type: "debate_error", error: "No available Mate can moderate this debate. Debate not started." });
      return { ok: false, reason: "no available Mate can moderate" };
    }

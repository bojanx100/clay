...
function approvalEventForTask(history, request, events) {
  ...
  for (var i = items.length - 1; i >= 0; i--) {
    ...
    var approval = explicitItemApproval(event.text);
    if (!approval) continue;
    var snapshot = pendingApprovalSnapshotAt(events, event._ts);
    var resolved = resolveApprovalTask(snapshot, approval.subject, request);
    if (resolved.ok) return { event: event, approval: approval, resolved: resolved };
  }
  return null;
}
...
function executionAdmission(input, request, canonical, deps) {
  ...
  var resolved = resolveApprovalTask(snapshot, approval.subject,

...
// Compaction has one explicit predecessor edge per provider continuation. An
// approval can cross that edge only while the whole, short chain remains
// unambiguous. This is intentionally separate from replay/display lineage:
// admission must prefer refusing old evidence to scanning an unbounded history.
var MAX_COMPACTED_APPROVAL_PREDECESSORS = 16;
...
function sessionsByStorage(sessions) {
  ...
  sessions.forEach(function (session) {
    ...
    if (!storageId) return;
    // Two live objects claiming one storage id are not a continuation proof.
    // Withhold both rather than choosing Map insertion order as an authority
    // rule. Keep the marker separate so a third duplicate cannot restore it.
    if (seen[storageId]) {
      indexed[storageId] = null;
      return;
    }
    seen[storageId] = true;
    indexed[storageId] = session;
  });
  return indexed;
}

// Returns only a fully verified, finite predecessor chain, oldest first. Any
// malformed edge, missing predecessor, duplicate storage id, cycle, or bound
// breach withholds every predecessor and leaves the caller with current-session
// evidence alone. It never writes either session or the owner-request record.
function compactedApprovalSessions(session, sessions) {
  var current = session || null;
  var currentStorageId = storageIdOf(current);
  if (!current || !currentStorageId) return current ? [current] : [];
  var indexed = sessionsByStorage(sessions);
  var chain = [];
  var visited = Object.create(null);
  var predecessorCount = 0;
  while (current) {
    currentStorageId = storageIdOf(current);
    if (!currentStorageId || visited[currentStorageId]) return [session];
    visited[currentStorageId] = true;
    chain.push(current);
    var predecessorId = String(current.compactedFromStorageId || "").trim();
    if (!predecessorId) break;
    if (!projectIdentity.isSessionStorageId(predecessorId) ||
        predecessorCount >= MAX_COMPACTED_APPROVAL_PREDECESSORS ||
        !indexed[predecessorId]) return [session];
    current = indexed[predecessorId];
    predecessorCount++;
  }
  return chain.reverse();
}

function compactedApprovalSessionFor(session, sessions, wantedStorageId) {
  var wanted = String(wantedStorageId || "").trim();
  if (!projectIdentity.isSessionStorageId(wanted)) return null;
  var chain = compactedApprovalSessions(session, sessions);
  for (var i = 0; i < chain.length; i++) {
    if (storageIdOf(chain[i]) === wanted) return chain[i];
  }
  return null;
}

function approvalHistoryFor(session, sessions) {
  var chain = compactedApprovalSessions(session, sessions);
  var history = [];
  for (var i = 0; i < chain.length; i++) {
    var items = Array.isArray(chain[i] && chain[i].history) ? chain[i].history : [];
    for (var j = 0; j < items.length; j++) history.push(items[j]);
  }
  return history;
}

function snapshotIncludesRequest(snapshot, request) {
  var tasks = snapshot && Array.isArray(snapshot.tasks) ? snapshot.tasks : [];
  var wanted = taskKey(request);
  if (!wanted) return false;
  for (var i = 0; i < tasks.length; i++) {
    if (taskKey(tasks[i]) === wanted) return true;
  }
  return false;
}
...
function executionAdmission(input, request, canonical, deps) {
  ...
  // A current-session fully named approval remains self-contained as before.
  // A predecessor approval is different: its only right to reach the current
  // dispatch is the explicit compaction edge, so it must also prove this exact
  // request was already staged when the owner spoke. That keeps a post-hoc
  // approval in old transcript context from being adopted after compaction.
  var requestRef = entry && entry.requestRef;
  var compactedApproval = !!(requestRef &&
    requestRef.sessionStorageId !== storageIdOf(canonical));
  if (compactedApproval && !snapshotIncludesRequest(snapshot, request)) {
    return { ok: false, reason: "owner_approval_no_pending_item" };
  }
...
module.exports = {
  MAX_COMPACTED_APPROVAL_PREDECESSORS: MAX_COMPACTED_APPROVAL_PREDECESSORS,
  MAX_PENDING_ITEMS: MAX_PENDING_ITEMS,
  approvalEventForTask: approvalEventForTask,
  approvalHistoryFor: approvalHistoryFor,
  approvalProjectMatches: approvalProjectMatches,
  compactedApprovalSessionFor: compactedApprovalSessionFor,

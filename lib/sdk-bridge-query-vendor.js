...
async function ensureInitialAdapter(ctx, session, linuxUser) {
  ...
  var ready = await ctx.ensureVendorReady(session.vendor, linuxUser, session);
...
async function recoverAuthenticatedAdapter(ctx, session, linuxUser) {
  ...
  var recovered = await ctx.ensureVendorReady(session.vendor, linuxUser, session);
...
function abandonUnavailable(fence) {
  ...
  return { ok: false, reason: "provider_unavailable", submission: "not_submitted" };
}

function rejectUnsupportedIsolation(ctx, session, linuxUser, fence) {
  if (!linuxUser || !session.vendor) return null;
  var info = require("./yoke").getVendorInfo(session.vendor);
  if (!info || info.osUserIsolation !== false) return null;
  var name = info.displayName || session.vendor;
  console.error("[sdk-bridge] Refusing " + name + " for OS-isolated user " + linuxUser + ": per-user spawning is not implemented");
  ctx.sendAndRecord(session, { type: "error", text: name + " is not available for OS-isolated users yet." });
  ctx.sendAndRecord(session, { type: "done", code: 1 });
  return { result: abandonUnavailable(fence) };
}
...
async function prepareSessionAdapter(ctx, session, linuxUser, fence) {
  var isolationResult = rejectUnsupportedIsolation(ctx, session, linuxUser, fence);

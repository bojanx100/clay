...
var CODEX_FALLBACK_MODELS = [
  {
    ...
    description: "Most capable OpenAI model for complex reasoning, coding, computer use, research, and document creation.",
...
function preferredCodexBest(models) {
  ...
  if (Array.isArray(models)) {
    for (var i = 0; i < models.length; i++) {
      ...
      if (/^gpt-6-astra(?:-|$)/i.test(value)) astra = value;
...
// Resolve the catalog to present for a discovery attempt, together with honest
// provenance. `liveModels` is whatever `model/list` returned; an empty or
// missing list means discovery failed or has not run yet, so the static seed
// stands in -- and the caller MUST be told, because persisting a seed as
// last-known-good drops every real model the seed does not name (see
// lib/model-catalog-cache.js). Keeping the substitution and its label in one
// place is what stops the two from drifting apart.
function resolveCodexCatalog(liveModels) {
  var live = Array.isArray(liveModels) && liveModels.length > 0;
  var models = live ? liveModels : fallbackCodexModels();
  return {
    models: models,
    defaultModel: preferredCodexDefault(models),
    provenance: live ? "live-discovery" : "fallback-seed",
  };
}
...
module.exports = {
  ...
  resolveCodexCatalog: resolveCodexCatalog,

...
function sessionReference(session) {
  return session.storageId || session.cliSessionId || session.localId;
}
...
function attachSessionHandoff(ctx) {
  ...
  function handoff(ws, msg) {
    ...
    target.handoff = {
      sourceSessionId: sessionReference(source),
    ...
    if (typeof sm.saveSessionFile === "function") {
      sm.saveSessionFile(target, { durable: true });
    }

    var sourceEntry = {
      type: "handoff_created",
      targetSessionId: sessionReference(target),
    ...
    var prompt = contextBuilder.buildHandoffContext({
      cwd: source.activeWorktree || ctx.cwd,
      source: source,
      targetVendor: targetVendor,
  ...
  function resolveSourceChain(boundSession) {
    ...
    for (var depth = 0; depth < MAX_HANDOFF_CHAIN_DEPTH; depth++) {
      ...
      sm.sessions.forEach(function (candidate) {
        if (!source && String(sessionReference(candidate)) === String(sourceSessionId)) source = candidate;
      });
  ...
  function findSource(chain, requestedId) {
    ...
    for (var i = 0; i < chain.length; i++) {
      if (String(chain[i].localId) === String(requestedId) ||
          String(sessionReference(chain[i])) === String(requestedId)) return chain[i];
    }
    return null;
  }

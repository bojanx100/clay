...
function attachImage(ctx) {
  ...
  function hydrateImageRefs(entry) {
    ...
    if (entry.type !== "user_message" && entry.type !== "mention_user" &&
        entry.type !== "user_mention" && entry.type !== "tool_result") return entry;
    var images = [];
    ...
    if (!Array.isArray(hydrated.images) || hydrated.images.length === 0) hydrated.images = images;
    return hydrated;
  }
  ...
  // Reload saved image files (imageRefs: [{mediaType, file}]) back into the
  // SDK multimodal form [{mediaType, data}]. Used to replay a scheduled
  // message's images at dispatch time (they were persisted to disk when queued).
  function loadImagesForSdk(imageRefs) {
    ...
    for (var i = 0; i < imageRefs.length; i++) {
      ...
      try {
        ...
        out.push({ mediaType: ref.mediaType || "image/png", data: data, savedPath: path.join(imagesDir, ref.file) });
      } catch (e) {
        console.error("[images] Failed to reload scheduled image " + ref.file + ":", e.message);
      }
    }
    return out;
  }

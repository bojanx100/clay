...
function attachCoopFanIn(ctx) {
  ...
  function localAttempt(event) {
    ...
    var feedback = require("./coop-task-feedback").resolve(crossProject, {
      eventId: event.eventId, source: { projectId: getProjectId(), sessionStorageId: event.sessionStorageId },
      destination: { projectId: LEAD_PROJECT_ID, sessionStorageId: event.coopSessionStorageId },
    });
    if (queueCoordinatorUpdate(localCoopSession, eventMessage(event),
        { updateId: event.eventId, feedback: feedback }) === false) {
      return { ok: false, delivered: false, reason: "coordinator_update_persistence_failed" };
    }

...
function attachCoopFanIn(ctx) {
  ...
  function attemptDelivery(event) {
    ...
    if (envelope && typeof crossProject.deliverEnvelope === "function") {
      var typedResult = crossProject.deliverEnvelope(envelope);
      if (typedResult && typedResult.ok) return { ok: true, delivered: true };
      return {
        ok: false,
        delivered: false,
        reason: typedResult && typedResult.reason || "delivery_error",
        terminal: !!(typedResult && typedResult.deadLettered),
      };
    }
    var result;
    try {

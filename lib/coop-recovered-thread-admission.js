...
//
// `_event_missing` belongs here now that it means one thing. Each recovery module pins an
// absolute eventIndex, and transcript delta coalescing (cf7f197ee1) renumbered the
// canonical transcript from ~218k items to ~38k, putting all four pinned coordinates past
// the end. A pinned coordinate that no longer exists cannot come back: the transcript only
// shrinks, and the next renumber moves it again. So all four migrations were retried on
// every single boot, forever, reporting the same code.
//
// It could not be marked terminal before because it was overloaded: the same string was
// returned for an unusable session manager and for a canonical session not yet loaded,
// both of which genuinely do succeed on a later run. Those now report
// `*_session_unavailable` and stay retryable, which is also what made this family so
// expensive to diagnose - one code for three very different causes.
var TERMINAL_CODE_SUFFIXES = ["_event_digest_mismatch", "_event_identity_mismatch",
  "_event_route_mismatch", "_event_topic_mismatch", "_event_ambiguous", "_event_missing"];

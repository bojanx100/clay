function attachProjectSessionsUserState(ctx) {
  ...
  function handleSetLeadMode(ws, msg) {
    // This is one shared Clay setting. The designated Clay owner may write
    // it; every result includes stored state so clients cannot drift.
    ...
    var lmCanChange = leadMode.isAuthority(ws._clayUser || null, multiUser, lmOwnerId);
    ...
    if (lmResult && lmResult.ok) leadMode.broadcast(Object.assign({ type: "lead_mode_changed" }, lmState));
    return true;
  }
  ...
  function handleUserStateMessage(ws, msg) {
    ...
    if (msg.type === "set_lead_mode") return handleSetLeadMode(ws, msg);

function attachProjectSessionsUserState(ctx) {
  ...
  function handleUserStateMessage(ws, msg) {
    ...
    if (msg.type === "set_lead_mode") {
      // Per-user opt-in for the Lead (CTO orchestrator). Off by default;
      // turning it off is the kill switch — no core path may depend on it.
      var lmUserId = ws._clayUser ? ws._clayUser.id : null;
      if (!lmUserId) {
        sendTo(ws, { type: "set_lead_mode_result", ok: false, error: "no_user" });
        return true;
      }
      var lmResult = usersModule.setLeadMode(lmUserId, msg.enabled === true);
      if (lmResult && lmResult.ok) {
        sendTo(ws, { type: "set_lead_mode_result", ok: true, leadMode: lmResult.leadMode });
        sendTo(ws, { type: "lead_mode_changed", leadMode: lmResult.leadMode });
      } else {
        sendTo(ws, { type: "set_lead_mode_result", ok: false, error: (lmResult && lmResult.error) || "unknown" });
      }
      return true;
    }

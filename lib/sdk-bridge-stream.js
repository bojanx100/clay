...
// Watchdog budgets (module scope so midstreamTimeoutFor is unit-testable).
// Reasoning models emit NO stream events while the model reasons between
// items — silent gaps of 30-60s+ are NORMAL on complex tasks. The recovery log
// first showed the 30s mid-stream watchdog repeatedly killing healthy Codex
// turns at 30-35s of silence (case=mid-generation), then auto-resume started a
// fresh conversation that went silent reasoning again → killed again — a loop
// that burned tokens and spammed "Connection dropped… Resuming…" bubbles
// without making progress. Codex got a 120s budget (fix d108f9b8e1). Claude was
// assumed to stream continuously, but Opus 4.8 with extended thinking reasons
// silently between events too — the recovery log caught the identical loop on
// vendor=claude (session 577, repeated mid-generation @ 30-35s). So the base
// mid-generation budget is 120s for every vendor; only first-event and
// tool-active retain their own budgets.
var STREAM_MIDSTREAM_TIMEOUT_MS = 120 * 1000;
var STREAM_MIDSTREAM_TIMEOUT_CODEX_MS = 120 * 1000;

function midstreamTimeoutFor(vendor) {
  return vendor === "codex" ? STREAM_MIDSTREAM_TIMEOUT_CODEX_MS : STREAM_MIDSTREAM_TIMEOUT_MS;
}

// A context-window overflow ("prompt is too long") can surface EITHER as a
// thrown error out of the stream loop OR as an in-stream error event — both
// paths must classify it so the client shows the recoverable
// "context_overflow" card (with the compact/clear affordance) instead of a
// bare, dead-end error bubble. Kept module-scope so it's unit-testable and
...
// Whether a stream event counts as real progress for the mid-generation
// watchdog. Everything counts EXCEPT the catch-all "system" yokeType with no
// payload — unrecognized SDK meta the processor silently drops. A flood of
// those (observed: 570 in one wedged turn) would otherwise reset the watchdog
// on every tick and keep a dead turn alive forever behind spinning dots.
...
function attachBridgeStream(ctx) {
  ...
  var slug = ctx.slug || "";
  var streamInactivityTimeoutMs = 10 * 60 * 1000;
  var streamFirstEventTimeoutMs = 45 * 1000;
  ...
  async function processQueryStream(session) {
    ...
    var watchdogTimer = setInterval(function () {
      ...
      var timeoutMs = activeToolCount > 0
        ? streamInactivityTimeoutMs
        : (sawAnyEvent ? midstreamTimeoutFor(session.vendor || (adapter && adapter.vendor)) : streamFirstEventTimeoutMs);
      var since = sawAnyEvent ? (Date.now() - lastEventAt) : (Date.now() - turnStartedAt);
      if (since >= timeoutMs) {
        clearInterval(watchdogTimer);
        var watchdogCase = activeToolCount > 0 ? "tool-active" : (sawAnyEvent ? "mid-generation" : "first-event");
        ...
        providerHealth.recordFailure(session.vendor || (adapter && adapter.vendor) || "claude", "watchdog:" + watchdogCase);
        ...
        if (!session.destroying) {
          ...
          if (session.taskStopRequested) {
            ...
            if (canRetryStreamEnd) {
              ...
              providerHealth.recordFailure(session.vendor || (adapter && adapter.vendor) || "claude", "resume-gave-up:stream-ended", { strong: true });
      ...
      if (session.isProcessing) {
        ...
        if (err.name === "AbortError" || (myAbortController && myAbortController.signal.aborted) || session.taskStopRequested) {
          ...
          if (conflicts.length > 0) {
            ...
            providerHealth.recordFailure(session.vendor || (adapter && adapter.vendor) || "claude", "transient:" + String(errDetail).slice(0, 80));
            session._transientRetryUsed = true;
            sendAndRecord(session, { type: "thinking_stop" });
            ...
            if (isContextOverflow) {
              ...
              providerHealth.recordFailure(session.vendor || (adapter && adapter.vendor) || "claude", "provider-error:" + String(err.message || "").slice(0, 80), { strong: true });

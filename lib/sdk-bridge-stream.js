...
function attachBridgeStream(ctx) {
  ...
  async function processQueryStream(session) {
    ...
    var rateLimitUsageRequested = false;
    var activeTools = {};
    ...
    try {
      for await (var msg of myQueryInstance) {
        // Claude's live stream only emits rate-limit events near a warning or
        // rejection. Once the query is initialized, ask the adapter for the
        // structured `/usage` snapshot so normal in-process sessions also get
        // current plan percentages. Worker-backed sessions already inject the
        // same events themselves and do not expose this optional method.
        if (!rateLimitUsageRequested && typeof myQueryInstance.getRateLimitUsageEvents === "function") {
          rateLimitUsageRequested = true;
          myQueryInstance.getRateLimitUsageEvents().then(function(usageEvents) {
            if (!Array.isArray(usageEvents) || session.destroying) return;
            for (var ui = 0; ui < usageEvents.length; ui++) {
              processSDKMessage(session, usageEvents[ui]);
            }
          }).catch(function(e) {
            console.error("[sdk-bridge] current Claude usage fetch failed (non-fatal):", e.message || e);
          });
        }
        ...
        if (msg && msg.yokeType === "error") {
          ...
          } else if (isTransientStreamError(adapterErrText) && !session._transientRetryUsed
              && !session.taskStopRequested && autoResumeAllowed(session)
              && typeof opts.scheduleMessage === "function") {
            // A connectivity drop delivered as an adapter error event rather
            // than a thrown error — same one-shot, budget-capped retry the
            // thrown path uses. Deliberately records NO provider-health
            // failure: a blip is not an outage, and letting it set
            // providerFailoverPending here would race this resume against a
            // failover scheduled from the finally block. Escalation still
            // happens — once the auto-resume budget is spent, autoResumeAllowed
            // goes false and the branch below records the strong failure.
            console.warn("[sdk-bridge] Transient adapter stream error for session " + session.localId
              + "; auto-retrying once: " + adapterErrText);
            recordRecoveryEvent({
              kind: "transient",
              sessionId: session.localId,
              vendor: session.vendor || (adapter && adapter.vendor) || "claude",
              source: "adapter-error",
              error: String(adapterErrText).slice(0, 300),
            });
        ...
        if (err.name === "AbortError" || (myAbortController && myAbortController.signal.aborted) || session.taskStopRequested) {
          ...
          if (conflicts.length > 0) {
            ...
            if (isContextOverflow) {
              ...
              // Force a real re-check before trusting an auth-like error:
              // the vendor may have been logged in after daemon start, and a
              // false auth_required used to auto-run the login command,
              // which for Codex deletes the prior auth.json.
              var freshAuth = getFreshAuthState(true, getLinuxUserForSession(session));
      ...
      if (!session.destroying && typeof opts.reconcileQueuedUserMessages === "function") {
        try { opts.reconcileQueuedUserMessages(session); } catch (e) {}
      }
    }
  }

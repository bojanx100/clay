...
function createServer(opts) {
  ...
  function ledgerTopicBindings(topicRef, metadata, ws, snapshot) {
    if (!crossProject || typeof crossProject.topicSessionEvidence !== "function") return [];
    var current = snapshot && snapshot.reconciled &&
      typeof crossProject.currentTopicSessionEvidence === "function";
    var entries = current
      ? crossProject.currentTopicSessionEvidence(topicRef, metadata)
      : crossProject.topicSessionEvidence(topicRef, metadata, {
        topicLinks: snapshot && snapshot.topicLinks || currentCoopTopicLinks(),
      });
    var linked = [];
  ...
  function globalCoopProjectionFor(ws) {
    ...
    var topicLinks = currentCoopTopicLinks();
    var ledgerSnapshot = { topicLinks: topicLinks, reconciled: false };
    // Reconcile before reading the owner work ledger. This makes replay,
    // reconnect, compaction continuations and provider switches converge on the
    // same session/binding evidence instead of leaving a prior projection's
    // activity label in place.
    if (crossProject && typeof crossProject.reconcileSessionLedger === "function") {
      var reconciliation = crossProject.reconcileSessionLedger({ topicLinks: topicLinks });
      ledgerSnapshot.reconciled = !!(reconciliation && reconciliation.ok);
    }
    ...
    return globalCoopProjection.buildGlobalCoopProjection({
      ...
      computeCoopTopicState: function (topicRef, metadata) {
        ...
        return coopTopicState.projectedTopicState(topicRef, {
          ...
          bindings: ledgerTopicBindings(topicRef, metadata, ws, ledgerSnapshot),

...
var coopMainIngressRecovery = require("./coop-main-ingress-recovery");
...
function migrateLeadOwnerRequestHistory(extra, ctx) {
  ...
  var recovery = coopMainIngressRecovery.migrateProductionFromSessionManager(
    coopTopicIndex.getDefaultTopicIndex(), ledger, ctx.sm);
  if (!recovery.ok) {
    console.error("[coop-main-ingress-recovery] startup migration failed closed:",
    ...
    if (ctx.sm && typeof ctx.sm.registerCoopControlRecoveryTarget === "function") {
      ...
      if (controlRecovery && typeof controlRecovery.then === "function") {
        controlRecovery.then(function () {
          if (typeof ctx.sm.reconcileCoopControlSessions === "function") {

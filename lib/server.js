...
function coopTopicLinkedBindings(bindings, topicRef, metadata) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    if (topicId && topicId === wanted) linked.push(binding);
  }
  return linked;
}
...
function createServer(opts) {
  ...
  // only evidence is a binding has to read Working / awaiting acceptance rather
  // than the unlinked default. Read through the router's existing binding-store
  // accessor; there is no second store.

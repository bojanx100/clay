...
function executionSessionRef(binding) {
  var record = binding || {};
...
function createServer(opts) {
  ...
  // Durable cross-project execution bindings can carry a canonical Coop
  // TopicRef, which makes them linked work in their own right: a topic whose
  // only evidence is a binding has to read the linked project session's live
  // lifecycle rather than the unlinked default. Read through the router's
  // existing binding-store accessor; there is no second store.
  function currentExecutionBindings() {
    if (!crossProject || typeof crossProject.getExecutionBindings !== "function") return [];
    try {
      var list = crossProject.getExecutionBindings();
  ...
  function resolvedExecutionSession(binding, ws) {
    var ref = executionSessionRef(binding);
    if (!ref) return null;
    var resolved = projectReferenceResolver.resolveSessionRef(ref, ws);
    return resolved && resolved.ok ? resolved.session : null;
  }
  ...
  function archiveCompletedCoopTopicSessions(topicRef, topic, ws) {
    var refs = [];
    var seen = {};
    var bindings = coopTopicLinkedBindings(currentExecutionBindings(), topicRef,
      { status: topic && topic.status || "open" });
    for (var i = 0; i < bindings.length; i++) {
      var bindingRef = executionSessionRef(bindings[i]);
      if (bindingRef) refs.push(bindingRef);
    }
    var executions = Array.isArray(topic && topic.relatedExecutions) ? topic.relatedExecutions : [];
    for (var j = 0; j < executions.length; j++) {
      var executionRef = projectIdentity.normalizeSessionRef(executions[j] && executions[j].sessionRef);
      if (executionRef) refs.push(executionRef);
    }
  ...
  function globalCoopProjectionFor(ws) {
    ...
    // Read once per projection: the accessor deep-clones, and the seam below
    // runs for every topic.
    var executionBindings = currentExecutionBindings();
    return globalCoopProjection.buildGlobalCoopProjection({
      ...
      computeCoopTopicState: function (topicRef, metadata) {
        ...
        return coopTopicState.projectedTopicState(topicRef, {
          ...
          bindings: coopTopicLinkedBindings(executionBindings, topicRef, metadata,
            function (binding) { return resolvedExecutionSession(binding, ws); }),
  ...
  function refreshCanonicalCoopTopics(session) {
    if (!session || !session.coopHome) return;
    var lead = projects.get("lead");
    if (!lead || typeof lead.forEachClient !== "function") return;
    coopTopicLiveIndex.refreshCanonicalCoopTopics({
      session: session,
  ...
  function refreshCoopActionQueues() {
    ...
    if (home) refreshCanonicalCoopTopics(home);
  }
    ...
    var ctx = createProjectContext({
      ...
      computeCoopTopicWorkState: function (topicRef, metadata) {
        ...
        return coopTopicState.projectedTopicState(topicRef, {
          ...
          bindings: coopTopicLinkedBindings(currentExecutionBindings(), topicRef, metadata,
            function (binding) { return resolvedExecutionSession(binding, ws); }),
          metadata: metadata || null,
          foreground: {
            isProcessing: !!coopSession.isProcessing,

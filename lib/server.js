...
var createBrowserExtensionState =
  require("./project-browser-extension").createBrowserExtensionState;
var createHumanAttention = require("./human-attention").createHumanAttention;
...
function createServer(opts) {
  ...
  // The browser extension belongs to this Clay daemon, not to whichever
  // project page happened to reconnect first. Every project's browser MCP
  // server therefore resolves through the same in-memory bridge state.
  var browserExtensionState = createBrowserExtensionState();
  // One daemon-wide ledger merges simultaneous phone, laptop, and tab leases.
  // A project-local timer would double-count every cross-device overlap.
  var humanAttention = createHumanAttention();
  ...
  var crossProject = serverCrossProject.createCrossProjectRouter({
    deliveryRetryIntervalMs: 1000,
    coopExecutionControl: coopExecutionControl,
    coopStartupRecovery: coopStartupRecovery,
    restoreWorkOnStartup: require("./scheduled-execution-policy").restoresWork(),
    ...
    onThreadHandedOff: function (input) {
      return coopTopicIndex.getDefaultTopicIndex().linkExecution(input.topicRef, {
        projectRef: input.projectRef, sessionRef: input.sessionRef, taskRef: input.taskRef,
      });
    },
    onPortfolioExecutionTerminal: function () {
      var lead = projects.get("lead");
      return lead && typeof lead.resumeLeadAfterPortfolioTerminal === "function" ?
        lead.resumeLeadAfterPortfolioTerminal() : false;
    },
  });

  var coopMaintenance = require("./coop-control-maintenance").createControlMaintenance({
    projects: function () { return projects; },
    crossProject: crossProject,
    ownerRequests: coopOwnerRequests.getDefaultOwnerRequests(),
    topicIndex: coopTopicIndex.getDefaultTopicIndex(),
    isReady: function () { return crossProject.controlledIngressState() === "open"; },
    isLeadModeEnabled: leadMode.getLeadMode,
    onUpdated: refreshGlobalCoopViewers,
    recordFailure: function (reason) { recordStartupFailure("coop_control_maintenance", reason); },
  });
  var unsubscribeCoopMaintenance = leadMode.subscribe(function () { coopMaintenance.request("lead_mode"); });
  coopMaintenance.start();
  ...
  function ledgerTopicBindings(topicRef, metadata, ws) {
    if (!crossProject || typeof crossProject.currentTopicSessionEvidence !== "function") return [];
    var entries = crossProject.currentTopicSessionEvidence(topicRef, metadata);
    var linked = [];
  ...
  function globalCoopProjectionFor(ws) {
    if (crossProject && typeof crossProject.withExecutionBindingSnapshot === "function" &&
        typeof crossProject.hasExecutionBindingSnapshot === "function" &&
        !crossProject.hasExecutionBindingSnapshot()) {
      return crossProject.withExecutionBindingSnapshot(function () {
        return globalCoopProjectionFor(ws);
      });
    }
    var coopSession = canonicalCoopSessionForState();
    var isCoopOwner = connectedUserIsCoopOwner(ws);
    var ownerRequestLedger = coopOwnerRequests.getDefaultOwnerRequests();
    var ownerLedgerSessions = crossProject && crossProject.sessionLedger &&
      typeof crossProject.sessionLedger.list === "function"
      ? crossProject.sessionLedger.list({ topLevelOnly: false, includeHidden: true,
        includeMissing: true }) : [];
    return globalCoopProjection.buildGlobalCoopProjection({
      ...
      // Owner-only request metadata. Non-owner viewers retain their generic
      // Coop navigation projection but never receive the owner's request ledger.
      ownerRequests: isCoopOwner && ownerRequestLedger && typeof ownerRequestLedger.list === "function"
        ? ownerRequestLedger.list() : [],
      ownerLedgerSessions: isCoopOwner ? ownerLedgerSessions : [],
  ...
  function refreshCanonicalCoopTopics(session) {
    ...
    return coopTopicLiveIndex.refreshCanonicalCoopTopics({
      ...
      advance: function (canonicalSession) {
        return coopMaintenance.run("canonical_turn");
      },
      forEachClient: lead.forEachClient,
      projectionFor: globalCoopProjectionFor,
      sendTo: function (ws, projection) {
        lead.sendTo(ws, coopTopicConnection.clientProjection(projection));
      },
    });
  }
  ...
  function broadcastProcessingChange() {
    coopMaintenance.request("processing");
  ...
  function addProject(cwd, slug, title, icon, projectOwnerId, worktreeMeta, extraOpts) {
    ...
    var extra = extraOpts || {};
    var projectId = projectIdentity.projectIdForRuntime(extra, cwd, slug, worktreeMeta);
    var ctx = createProjectContext({
      ...
      onCoopActionQueueChanged: refreshCoopActionQueues,
      ...
      browserExtensionState: browserExtensionState,
      ...
      getHubSchedules: function () {
        var allSchedules = [];
        var schedulesPaused = require("./scheduled-execution-policy").isPaused();
        projects.forEach(function (ctx, s) {
          var status = ctx.getStatus();
      ...
      listAutoApprovalProjects: function () {
        var items = [];
        var seenProjectIds = {};
        projects.forEach(function (project, projectSlug) {
          if (!project || projectSlug === "lead") return;
          var status = project.getStatus && project.getStatus();
          if (status && status.isMate) return;
          var manager = project.getSessionManager && project.getSessionManager();
          var projectId = manager && typeof manager.getProjectId === "function" ? manager.getProjectId() : null;
          if (!projectId || seenProjectIds[projectId]) return;
          seenProjectIds[projectId] = true;
          items.push({
            projectRef: { projectId: projectId },
            label: status && (status.title || status.project) || projectSlug,
          });
        });
        return items;
      },
      ...
      applyCoopActionDecision: function (request, ws) {
        return coopActionDecision.applyDecision({
          request: request,
          // V1 is explicitly bound by the server owner at startup. Omitting
          // this option keeps the historical fail-closed response; it never
          // guesses a ProjectRef or turns any unavailable orchestrator into a
          // break-glass admission.
          actorId: ws && ws._clayUser && ws._clayUser.id || "",
          emergencyRepairPolicy: opts.emergencyRepairPolicy || null,
    ...
    var restoreWork = require("./scheduled-execution-policy").restoresWork();
    if (restoreWork && ctx.sm && typeof ctx.sm.registerCoopControlRecoveryTarget === "function") {
    ...
    } else if (restoreWork && ctx.sm && typeof ctx.sm.recoverCoopControlStartup === "function") {
    ...
    if (restoreWork && crossProject && typeof crossProject.reconcileStrandedCompletions === "function") {
      crossProject.reconcileStrandedCompletions();
    }
    projects.set(slug, ctx);
    coopMaintenance.request("project_registered");
  ...
  function getProjectContextsById(projectId) {
    var found = [];
    projects.forEach(function (ctx) {
      if (ctx.projectId === projectId) found.push(ctx);
    });
    found.sort(function (left, right) {
      var leftStatus = left.getStatus ? left.getStatus() : {};
      var rightStatus = right.getStatus ? right.getStatus() : {};
  ...
  function getProjectContextById(projectId) {
    return getProjectContextsById(projectId)[0] || null;
  }
  ...
  var projectReferenceResolver = projectIdentity.createReferenceResolver({
    getProjectById: getProjectContextById,
    getProjectsById: getProjectContextsById,
    canAccessProject: canAccessProjectRef,
    canAccessSession: canAccessSessionRef,
  });
  ...
  function removeProject(slug) {
    ...
    coopMaintenance.request("project_removed");
  ...
  function broadcastPresenceChange() {
    ...
    presenceTimer = setTimeout(function flushPresenceChange() {
      presenceTimer = null;
      if (crossProject && typeof crossProject.withExecutionBindingSnapshot === "function" &&
          typeof crossProject.hasExecutionBindingSnapshot === "function" &&
          !crossProject.hasExecutionBindingSnapshot()) {
        return crossProject.withExecutionBindingSnapshot(flushPresenceChange);
      }
      if (!users.isMultiUser()) {
        broadcastAll({
          type: "projects_updated",
  ...
  function runCoopExecutionReaper(options) {
    if (!crossProject || typeof crossProject.runExecutionReaper !== "function") {
      return { ok: false, reason: "execution_reaper_unavailable", findings: [] };
    }
    return crossProject.runExecutionReaper(options || {});
  }
  ...
  function destroyAll() {
    ...
    coopMaintenance.stop();
    unsubscribeCoopMaintenance();
    crossProject.stopDeliveryRetry();
    var shutdowns = [];
    projects.forEach(function (ctx, slug) {
  ...
  return {
    ...
    runCoopExecutionReaper: runCoopExecutionReaper,

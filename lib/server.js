...
function createServer(opts) {
  ...
  function canonicalCoopSessionForState() {
    ...
    var sessions = lead && typeof lead.getSessions === "function" ? lead.getSessions() : null;
    if (!Array.isArray(sessions)) return null;
    for (var i = 0; i < sessions.length; i++) {
      if (sessions[i] && sessions[i].coopHome) return sessions[i];
    }
    return null;
  }
  ...
  function globalCoopProjectionFor(ws) {
    ...
    return globalCoopProjection.buildGlobalCoopProjection({
      ...
      computeCoopTopicState: function (topicRef) {

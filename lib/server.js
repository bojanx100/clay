...
var coopRecoveredThreadAdmission = require("./coop-recovered-thread-admission");
...
function migrateLeadOwnerRequestHistory(extra, ctx) {
  ...
  var result = coopOwnerRequestBackfill.migrateOwnerRequestHistory(
    ledger, ctx.sm);
  if (!result.ok) {
    // migrateOwnerRequestHistory returns {ok, backfill, migrations} with no
    // top-level reason. In steady state (no pending one-time migrations)
    // `migrations` is an empty array — which is truthy — so the naive
    // `result.reason || result.migrations` logged `[]` and dropped the real
    // cause sitting in result.backfill.reason. Assemble the detail explicitly.
    var failedMigrations = (Array.isArray(result.migrations) ? result.migrations : [])
      .filter(function (item) { return !item || item.ok !== true; });
    var backfillFailed = result.backfill && result.backfill.ok !== true;
    var migrationDetail = result.reason ||
      (backfillFailed && result.backfill.reason) ||
      (failedMigrations.length ? failedMigrations : null) ||
      result;
  ...
  var recovery = coopRecoveredThreadAdmission.migrateProductionFromSessionManager(
    coopTopicIndex.getDefaultTopicIndex(), ledger, ctx.sm);
  if (recovery.ok) {
    // Record the applied/no-op status once per boot so retirement is
    // answerable: a one-time repair that reports ok + noop on a clean boot,
    // with no failure line, has fully applied and is safe to delete. Success
    // was previously invisible, so "has this finished?" could not be answered.
    var okEntries = Array.isArray(recovery.migrations) ? recovery.migrations : [];
    recoveryLog.recordRecoveryEvent({
      kind: "coop_startup_migration",
      migration: "coop-recovered-thread-admission",
      ok: true,
      allNoop: recovery.noop === true,
      detail: okEntries.map(function (entry) {
        return { key: entry.key, ok: entry.ok === true, noop: entry.noop === true };
      }),
    });
  } else {
    // `failures` carries one entry per failed migration. Log it explicitly
    // rather than the whole result: console.error's default inspect depth
    // renders the nested per-migration result as "[Object]". Carry each
    // migration's `terminal` flag so a wedged evidence-mismatch (can never
    // self-heal — operator must act or the migration must be retired) is
    // distinguishable in the canary from a transient retryable failure.
    var failures = Array.isArray(recovery.failures) ? recovery.failures : [];
    console.error("[coop-recovered-thread-admission] startup migration failed closed:",
      JSON.stringify(failures.length ? failures : recovery));
    for (var i = 0; i < failures.length; i++) {
      recordStartupMigrationFailure(
        "coop-recovered-thread-admission:" + (failures[i].migration || "unknown"),
        { code: failures[i].code || null, terminal: failures[i].terminal === true,
          result: failures[i].result || null });
    }
    if (!failures.length) {
      recordStartupMigrationFailure("coop-recovered-thread-admission", recovery);
    }
  }
}
...
function createServer(opts) {
  ...
  function archiveCompletedCoopTopicSessions(topicRef, topic, ws) {
    ...
    for (var i = 0; i < candidates.length; i++) {
      ...
      archiveCompletedCoopSession(manager, resolved.session);
    }

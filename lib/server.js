...
function createServer(opts) {
  ...
  var appHandler = function (req, res) {
    ...
    if (req.method === "OPTIONS") {
      if (/^\/p\/[^/]+\/api\/task-(launch|dashboard-state)$/.test(req.url.split("?")[0])) {
        ...
        if (taskOrigin === "http://127.0.0.1:8765" || taskOrigin === "http://localhost:8765") {
  ...
  // --- Live socket tracking (for fast shutdown) ---
  // Node's server.close() stops accepting new connections but WAITS for open
  // ones to end. A held-open WebSocket (an idle browser tab) or a keep-alive
  // client (the codex mcp-bridge) can therefore block close indefinitely, so
  // the port is never released and the next daemon can't bind. We track every
  // raw socket and destroy them all on shutdown so close() returns at once.
  ...
  // Nothing keeps an idle WebSocket warm otherwise: the client's app-level ping
  // timer is throttled (and eventually suspended) by the browser when its tab is
  // backgrounded, and OS-default TCP keepalive is on the order of hours. An idle
  // socket then gets torn down by NAT/proxy/OS within ~30-60s and becomes a
  // "zombie" (readyState OPEN but dead), so returning to the tab flashes the
  // "Reconnecting…" overlay. A protocol-level ping is answered by the browser
  // automatically, BELOW JS, so it works even while the tab is throttled. This
  // keeps the connection alive in the background and reaps genuinely dead clients.
  var WS_PING_INTERVAL_MS = 30000;
  function setupWsKeepalive(ws) {
    ws.isAlive = true;
    ws.on("pong", function () { ws.isAlive = true; });
  }
  var wsKeepaliveTimer = setInterval(function () {
  ...
  // Never allow the keepalive timer to block process shutdown.
  if (wsKeepaliveTimer.unref) wsKeepaliveTimer.unref();
  server.on("close", function () { clearInterval(wsKeepaliveTimer); });

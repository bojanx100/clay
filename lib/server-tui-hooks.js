...
function installTuiHooks(opts) {
  opts = opts || {};
  // Comparison daemons share native Claude homes with the active instance.
  // They must not redirect its notifications or rewrite its permission list.
  var daemonConfig = require("./config").loadConfig();
  if (daemonConfig && daemonConfig.manageClaudeSettings === false) return;

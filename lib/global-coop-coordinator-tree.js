...
function attachGlobalCoopCoordinatorTree(ctx) {
  ...
  function effectiveTaskStatus(task, child) {
  ...
  function buildControlPlane(options, leadProject, project, projectId) {
    ...
    for (var ti = 0; ti < tasks.length; ti++) {
      ...
      var status = effectiveTaskStatus(task, child);
      if (status === "dismissed" && typeof options.reconcileDismissedSession === "function") {
    ...
    return {
      ...
      taskCoordinatorCount: taskEntries.length,

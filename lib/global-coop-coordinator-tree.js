...
var executionState = require("./coop-sidebar-execution-state");
var coordinatorTransparency = require("./coop-coordinator-transparency").coordinatorTransparency;
...
var pendingAssignmentNode = require("./global-coop-assignment-node").pendingAssignmentNode;

var ACTIVE_STATUSES = { waiting: true, queued: true, ready: true, running: true, reviewing: true };
...
function attachGlobalCoopCoordinatorTree(ctx) {
  ...
  function workerNodes(options, project, projectId, sessions, child, maps) {
    ...
    for (var i = 0; i < sessions.length; i++) {
      ...
      var status = executionState.taskState(binding.task, candidate, projectId, options.portfolioBindings);
      ...
      nodes.push({
        ...
        role: "worker",
        status: status,
        activity: cleanText(executionState.activity(status, candidate, binding.task), ""),
  ...
  function threadStatus(thread, children) {
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      if (children[i] && children[i].role === "project_assignment") pending = true;
      if (status === "running") active = true;
      if (status === "queued" || status === "ready") pending = true;
      if (status === "waiting") waiting = true;
      if (status === "reviewing") reviewing = true;
    }
  ...
  function buildControlPlane(options, leadProject, project, projectId) {
    ...
    var durableBindings = executionState.bindingBuckets(options && options.portfolioBindings);
    ...
    for (var ti = 0; ti < tasks.length; ti++) {
      ...
      var pendingNode = pendingAssignmentNode(root, task, projectId);
      if (pendingNode) { taskEntries.push({ task: task, node: pendingNode }); continue; }
      ...
      var status = executionState.effectiveTaskStatus(task, child, projectId, durableBindings, workers);
      if (status === "dismissed" && task.archivedAt) continue;
      ...
      taskEntries.push({ task: task, node: {
        ...
        status: status,
        activity: cleanText(executionState.activity(status, child, task), ""),
    ...
    for (var hi = 0; hi < threads.length; hi++) {
      ...
      threadChildren.sort(function (a, b) {
        var aRank = isLiveStatus(a.status) ? 0 : 1;
        var bRank = isLiveStatus(b.status) ? 0 : 1;
    ...
    return {
      coordinators: [Object.assign({
        ...
        children: children,
      }, coordinatorTransparency(root, children, options.includeActionQueue === true))],
  ...
  function build(options, project, projectId) {
    ...
    function nodeFor(session, depth, visited) {
      ...
      return {
        ...
        status: binding ? executionState.taskState(binding.task, session, projectId,
          options.portfolioBindings, childNodes) : "queued",
        activity: cleanText(executionState.activity(binding ? executionState.taskState(binding.task,
          session, projectId, options.portfolioBindings, childNodes) : "queued", session, binding && binding.task), ""),

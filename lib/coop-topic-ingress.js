...
function clearDerivedRouteFields(msg) {
  ...
  delete msg.coopRouteAttention;
  delete msg.coopIncludeClosedThread;
  delete msg.coopContextualThreadTarget;
}

function retryDraft(msg) {
  var draft = { text: typeof msg.text === "string" ? msg.text : "" };
  if (Array.isArray(msg.images) && msg.images.length) draft.images = msg.images.slice();
  if (Array.isArray(msg.pastes) && msg.pastes.length) draft.pastes = msg.pastes.slice();
  return draft;
}

function rejectedRouteFailure(session, msg, route) {
  return {
    type: "message_failed",
    sessionId: session && session.localId,
    clientMessageId: msg.clientMessageId || null,
    reason: route.code || "topic_target_unavailable",
    silent: true,
    recoverCoopMain: true,
    coopThreadRef: msg.coopThreadRef || msg.threadRef || null,
    coopTopicRef: msg.coopTopicRef || msg.topicRef || null,
    coopProjectRef: msg.coopProjectRef || msg.projectRef || null,
    retryDraft: retryDraft(msg),
...
function prepareIngress(ctx, ws, msg, session) {
  ...
  var suppressUnprovenImplementation = !explicitTarget && scope.unscoped &&
    threadIntent.isImplementationIntent(msg && msg.text) &&
    typeof ctx.resolveCoopThreadIntentTarget === "function" &&
    contextualTarget && contextualTarget.code !== "thread_target_ambiguous";
  var parsedIntent = suppressUnprovenImplementation ? null :
    threadIntent.parse(msg && msg.text, { explicitTarget: explicitTarget });
  ...
  if (!route.ok) {
    ...
    // The message remains fail-closed: this signal only removes the optimistic
    // bubble, restores the draft, and lets the matching client lens return to
    // Main. The rejected text is never resubmitted or rerouted automatically.
    ctx.sendTo(ws, rejectedRouteFailure(session, msg, route));
  ...
  if (route.routingAttention) msg.coopRouteAttention = route.routingAttention;
  ...
  msg.coopImplementationDecision = !route.routingAttention && (explicitTarget || scope.unscoped)

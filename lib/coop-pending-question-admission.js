...
function pendingQuestionFor(session, request) {
  ...
  for (var i = 0; i < tasks.length; i++) {
    ...
    var askedAt = Number(task.updatedAt);
    if (!Number.isFinite(askedAt)) continue;
    return { taskId: task.taskId, askedAt: askedAt,
      question: String(task.userQuestion) };
  }
  return null;
}
...
function executionAdmission(input, request, canonicalSession) {
  ...
  var turn = answeringTurn(canonicalSession, pending.askedAt);
  if (!turn) return null;

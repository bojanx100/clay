...
// A real owner answer in the staged flow was "Yes I approve". The old grammar
// accepted both "yes" and "approve" separately but rejected their ordinary
// first-person combination. Keep this closed and anchored: a named object or
// any extra instruction still fails the whole-sentence match.
var PERSONAL_CONFIRMATION = "(?:i\\s+(?:approve|confirm)(?:\\s+it)?)";
...
var ASSENT = new RegExp("^(?:" +
  ...
  AFFIRMATIVE + "(?:\\s+" + PERSONAL_CONFIRMATION + ")?" +
    "(?:\\s+(?:to\\s+)?" + SELECTION + ")?" +
...
function askedQuestionTurn(session, pending) {
  ...
  function comparable(value) {
    // Inline-code formatting is presentation, not content. Preserve every
    // identifier character and the terminating question mark, while allowing
    // the generated question to appear inside the coordinator's visible
    // explanation instead of requiring it to be the entire assistant turn.
    return String(value || "").replace(/`/g, "").replace(/\s+/g, " ").trim();
  }
  function exactVisibleQuestion() {
    var rendered = comparable(visible);
    var expected = comparable(question);
    return rendered && expected && rendered.indexOf(expected) !== -1 ? { _ts: lastAt } : null;
  }
  for (var i = 0; i < items.length; i++) {
    ...
    if (item.type === "delta_replace") {
      visible = String(item.text || "");
    } else if (item.type === "assistant_message" || item.type === "delta") {
      visible += String(item.text || "");
    } else continue;
    lastAt = at;
  }

...
function getToolDefs(sendCommand, getTabList, contextOps) {
  ...
  if (contextOps && contextOps.reconnectExtension) {
    tools.push(def(
      "browser_reconnect",
      "Reconnect the Clay browser extension through an open Clay page",
      buildShape({}, []),
      function () {
        return Promise.resolve(contextOps.reconnectExtension()).then(function (result) {
          return { content: [{
            type: "text",
            text: result.requestedClients ? "Browser extension connected through " +
              result.requestedClients + " Clay page(s)" : "Browser extension is already connected",
          }] };
        });
      }
    ));
  ...
  tools.push(def(
    "browser_open",
    "Open a grouped browser tab in the background and return its tab ID",
    buildShape({
      url: { type: "string", description: "URL to open" },
    }, ["url"]),
    function (args) {
      return sendCommand("tab_open", { url: args.url, active: false }).then(function (result) {
        if (result && (result.error || result.groupError)) throw new Error(result.error || result.groupError);
        return { content: [{ type: "text", text: "Opened tab " + (result.tabId || result.id || "unknown") + ": " + args.url }] };
      });
    }
  ));
  ...
  tools.push(def(
    ...
    function (args) {
      ...
      return sendCommand("tab_screenshot", extArgs, 10000).then(function (result) {
        if (!result || result.error || !result.image) throw new Error(result && result.error || "Screenshot failed");
        return {
          content: [

...
var reconsideration = require("./project-automation-reconsideration");
...
var DISQUALIFICATION_SCHEMA = "clay.portfolio_binding_disqualification";
var DISQUALIFICATION_VERSION = 1;
...
function normalizeRequest(value, options) {
  ...
  if (controlRole.isPeer(classifiedRole) || controlRole.normalize(input.controlRole) === "project_coordinator") {
    request.controlRole = classifiedRole;
  }
...
function conflictingWorkIdentity(bindings, request) {
  ...
  for (var i = 0; i < bindings.length; i++) {
    ...
    if (WORK_IDENTITY_BLOCKING_STATUSES[candidate.status] || verifiedWorkCompletion(candidate)) {
      // An owner may explicitly ask to reconsider one verified completed
      // binding. That receipt names this exact terminal record, so it cannot
      // authorize a different completed alias, an active duplicate, or a
      // merely pending acceptance.
      if (candidate.status === "completed" && reconsideration.matchesCompletedBinding(
          request.automationAuthorization && request.automationAuthorization.reconsideration,
          candidate)) continue;
      return candidate;
    }
  }
...
function normalizeDisqualification(value) {
  if (!value || typeof value !== "object" || Array.isArray(value) ||
      value.schema !== DISQUALIFICATION_SCHEMA || value.version !== DISQUALIFICATION_VERSION ||
      !normalizedString(value.reason) || typeof value.at !== "number" ||
      !Number.isFinite(value.at)) return null;
  var result = {
    schema: DISQUALIFICATION_SCHEMA,
    version: DISQUALIFICATION_VERSION,
    reason: normalizedString(value.reason).slice(0, 128),
    at: value.at,
  };
  var detail = normalizeFailureDetails(value.detail);
...
function copyOptionalStatus(record, value) {
  ...
  var resolution = require("./portfolio-coordinator-resolution").normalize(value.coordinatorResolution);
  if (resolution) record.coordinatorResolution = resolution;
  var disqualification = normalizeDisqualification(value.disqualification);
  if (disqualification) record.disqualification = disqualification;
...
function primitiveRetryAuthorization(value, request) {
  var authorization = automationAuthorization.normalizeAuthorization(value);
  if (!authorization || authorization.kind !== automationAuthorization.PRIMITIVE_KIND ||
      authorization.projectRef.projectId !== request.targetProject.projectId ||
      authorization.scope.portfolioTaskId !== request.portfolioTaskId ||
      authorization.scope.bindingRevision !== request.bindingRevision ||
      authorization.scope.idempotencyKey !== request.idempotencyKey ||
      authorization.scope.mode !== request.mode) return null;
  return authorization;
}

// A reservation that never routed has no execution to preserve. It may refresh
// only its current primitive qualification/authority after the router has
// already validated it, while every durable work, target, topic, and scope
// identity remains exact. Active and terminal records always conflict.
function refreshesRearmablePrimitiveAuthority(binding, request) {
  if (!binding || (binding.status !== "unrouted" && binding.status !== "unavailable")) return false;
  if (JSON.stringify(binding.workIdentity || null) !==
      JSON.stringify(request.workIdentity || null)) return false;
  var replay = Object.assign({}, request, {
    automationAuthorization: binding.automationAuthorization,
  });
  if (!sameLegacyRequest(binding, replay)) return false;
  var previous = primitiveRetryAuthorization(binding.automationAuthorization, request);
  var current = primitiveRetryAuthorization(request.automationAuthorization, request);
  if (!previous || !current) return false;
  return previous.source.recipeId === current.source.recipeId &&
    previous.source.workKind === current.source.workKind &&
    previous.candidateKey === current.candidateKey && previous.itemKey === current.itemKey &&
    previous.threadRef.threadId === current.threadRef.threadId;
}

function requestEquivalence(binding, request) {
  if (!sameLegacyRequest(binding, request)) {
    return refreshesRearmablePrimitiveAuthority(binding, request) ?
      "rearmable_authority_refresh" : "conflict";
  }
...
function createPortfolioExecutionBindings(options) {
  ...
  var resolveByCoordinator = require("./portfolio-coordinator-resolution").createApi({
    bindingIndex: bindingIndex, cleanId: cleanId, clone: clone, state: state, save: save,
  ...
  function reserve(input) {
    ...
    if (index !== -1) {
      var equivalence = requestEquivalence(state.bindings[index], request);
      if (equivalence === "conflict") {
        return { ok: false, reason: "idempotency_conflict" };
      }
      if (state.bindings[index].status === "cancelled" &&
          state.bindings[index].disqualification) {
        return { ok: false, reason: "binding_disqualified",
          binding: clone(state.bindings[index]) };
      }
      ...
      if (stale.status === "unrouted" || stale.status === "unavailable") {
        ...
        if (equivalence === "rearmable_authority_refresh") {
          stale.automationAuthorization = clone(request.automationAuthorization);
          stale.controlPlaneProvenance = clone(request.controlPlaneProvenance);
          stale.taskPayloadDigest = request.taskPayloadDigest;
          stale.provider = request.provider;
          stale.model = request.model;
        }
  ...
  function changeStatus(portfolioTaskId, bindingRevision, status, reason) {
    ...
    if (index === -1) return { ok: false, reason: "binding_not_found" };
    var record = state.bindings[index];
    if (record.disqualification) return { ok: false, reason: "binding_disqualified" };
  ...
  function markProjectCoordinatorAvailable(portfolioTaskId, bindingRevision, coordinator) {
    if (loadError) return { ok: false, reason: loadError };
    var index = bindingIndex(state.bindings, cleanId(portfolioTaskId), bindingRevision);
    if (index === -1) return { ok: false, reason: "binding_not_found" };
    var record = state.bindings[index];
    var requestedCoordinator = projectIdentity.normalizeSessionRef(coordinator);
    var residentControlPlane = record.projectCoordinator &&
      record.projectCoordinator.projectId === projectIdentity.LEAD_PROJECT_ID &&
      record.controlPlaneProvenance;
    if (record.mode !== "project_coordinator" || !record.coordinator ||
        !residentControlPlane && !sameSessionRef(requestedCoordinator, record.coordinator)) {
  ...
  // Auto-retirement is narrower than generic status mutation: it is the
  // terminal disposition for a binding that was rechecked against the live
  // issue and failed the restaff policy. It accepts re-armable and failed
  // records, but never rewrites completed work or an active execution.
  function retireForDisqualification(portfolioTaskId, bindingRevision, verdict) {
    if (loadError) return { ok: false, reason: loadError };
    var index = bindingIndex(state.bindings, cleanId(portfolioTaskId), bindingRevision);
    if (index === -1) return { ok: false, reason: "binding_not_found" };
    var record = state.bindings[index];
    var value = verdict && typeof verdict === "object" ? verdict : {};
    var reason = normalizedString(value.reason).slice(0, 128);
    if (!reason) return { ok: false, reason: "disqualification_reason_required" };
    if (record.status === "cancelled" && record.disqualification &&
        record.disqualification.reason === reason) {
      return { ok: true, duplicate: true, binding: clone(record) };
    }
    if (record.status === "completed" || record.status === "needs_input" ||
        record.status === "superseded" || record.status === "deleted") {
      return { ok: false, reason: "binding_terminal" };
    }
    if (record.status === "active") return { ok: false, reason: "binding_active" };
    var previous = clone(record);
    var at = typeof value.at === "number" && Number.isFinite(value.at) ? value.at : now();
    record.status = "cancelled";
    record.updatedAt = at;
    record.statusReason = ("restaff_disqualified:" + reason).slice(0, 240);
    record.failureCode = reason;
    var detail = normalizeFailureDetails(value.detail);
    if (detail) record.failureDetails = detail;
    ...
    if (detail) record.disqualification.detail = detail;
    delete record.attentionAt;
    var written = save();
    if (!written.ok) {
      state.bindings[index] = previous;
  ...
  return {
    ...
    resolveByCoordinator: resolveByCoordinator,
    file: file,
    ...
    retireForDisqualification: retireForDisqualification,
...
module.exports = {
  ...
  DISQUALIFICATION_SCHEMA: DISQUALIFICATION_SCHEMA,
  DISQUALIFICATION_VERSION: DISQUALIFICATION_VERSION,

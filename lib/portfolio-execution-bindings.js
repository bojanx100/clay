...
function conflictingWorkIdentity(bindings, request) {
  ...
  for (var i = 0; i < bindings.length; i++) {
    ...
    if (WORK_IDENTITY_BLOCKING_STATUSES[candidate.status] || verifiedWorkCompletion(candidate)) {
      // Reconsideration names the exact settled record. A primitive-only
      // receipt also binds the original launch and every prior record digest;
      // it can adopt existing work while keeping its prior acceptance intact.
      if (reconsideration.matchesCompletedBinding(

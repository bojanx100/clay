...
function normalizeRequest(value) {
  ...
  var coopTopicRef = normalizeBindingTopicRef(input.coopTopicRef);
...
function createPortfolioExecutionBindings(options) {
  ...
  function ensureCompletionMarkers(session, metadata, saveSessionFile) {
    ...
    if (changed && typeof saveSessionFile === "function") saveSessionFile(session);
    return changed;
  }
  ...
  function completionEvidence(record, session, saveSessionFile) {
    ...
    ensureCompletionMarkers(session, metadata, saveSessionFile);
    if (record.mode === "direct_leaf") {
      if (metadata.status !== "completed" && metadata.status !== "failed") return null;
    ...
    if (metadata.status !== "completed" || !metadata.projectCompletionDeliveryEventId) return null;
    return {
      eventId: metadata.projectCompletionDeliveryEventId,
      completedAt: metadata.completedAt || now(),
      ownerNotification: false,

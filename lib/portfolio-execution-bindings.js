...
// The stable identity of the WORK, as opposed to portfolioTaskId which is only
// the identity of one attempt at it. Callers already carry this (the Lead
// backlog calls it candidateKey, e.g. "launch:trialview/v2#2522"); it was simply
// never persisted onto the binding, so the store had no way to tell that two
// differently-named bindings described the same job. Free-form by design: it
// spans GitHub issues, owner requests, and automation candidates, so it is
// trimmed and length-capped rather than forced into the task-id grammar.
function normalizeWorkIdentity(value) {
  if (typeof value !== "string") return "";
  return value.trim().slice(0, MAX_WORK_IDENTITY);
}

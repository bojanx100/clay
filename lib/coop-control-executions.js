...
var MODES = Object.freeze({ project_coordinator: "coordinator", direct_leaf: "worker" });
...
function normalizeRequest(value) {
  ...
  if (!projectIdentity.isTaskId(source.portfolioTaskId) ||
      ...
      !projectIdentity.isTaskId(source.idempotencyKey) || !MODES[source.mode] ||
...
function createExecutionControl(options) {
  ...
  function assertCapability(token, action) {
    if (!ACTIONS[action]) throw error("COOP_CONTROL_AUTHORITY_DENIED", "The authority does not permit that action.");
  ...
  return {
    ...
    bindStart: bindStart,
    close: function () { closed = true; live = Object.create(null); if (ownsStore) store.close(); },
    complete: complete,
    createFence: createFence,
    inspect: inspect,
    markProviderStarted: markProviderStarted,
    openStartBarrier: openStartBarrier,
    recoverIncomplete: function () { live = Object.create(null); return store.recoverIncompleteExecutions(); },

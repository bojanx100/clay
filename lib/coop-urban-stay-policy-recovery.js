...
function validateRecord(ledger, event) {
  ...
  if (record.implementationDecision) {
    if (topicId(record.topicRef) !== THREAD_ID || !matchingDecision(record, event) ||
        !exactProjectScope(record, true) || !exactImplementationScope(record)) {
...
function migrateProduction(index, ledger, session) {
  ...
  if (current.applied) {
    return { ok: true, migrationId: RECOVERY_ID, threadCreated: target.created,
      membershipAdded: membership.added, decisionBackfilled: false,
...
function migrateProductionFromSessionManager(index, ledger, sm) {
  ...
  if (!sm || !sm.sessions || typeof sm.sessions.forEach !== "function") {
    return { ok: false, code: "urban_stay_policy_recovery_event_missing" };
  }
  ...
  if (!matches) return { ok: false, code: "urban_stay_policy_recovery_event_missing" };
  if (matches !== 1) {

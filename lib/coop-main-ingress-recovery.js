...
function recover(index, ledger, session, reassign) {
  ...
  // Identity is unconditional, but the handoff only conflicts once this run actually has
  // to populate the target, which is not known until planning below has finished.
  if (existingTarget.ok && !usableTarget(existingTarget.topic, true)) {
    return { ok: false, code: "recovery_target_conflict" };
  }
  var planned = [];
  ...
  // Nothing planned means every turn already lives in the target, so a handed off
  // target is the expected replay state rather than a conflict.
  var target = ensureTarget(index, !planned.length);

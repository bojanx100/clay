// Startup repair for the three known Main-origin ingress misroutes.
//
// The owner-gated `coop_main_ingress_recovery` WebSocket lever was retired: live
// ingress 360 now carries an admitted implementation decision, so its pre-admission
// gate could only ever answer execution_already_admitted. The repair is applied
// automatically by the startup migration below, which alone proves the canonical
// event digests and therefore alone may remove duplicate membership.
...
// A handed off target only conflicts while the repair still has to create or first
// populate it. Once the turns already live in the target and the sole remaining work
// is dropping stale source membership, the handoff is expected, not a conflict.
function usableTarget(topic, allowHandedOff) {
  return !!topic && topic.title === "Voice" && topic.status === "open" &&
    (allowHandedOff || topic.threadState !== "handed_off");
}

function ensureTarget(index, allowHandedOff) {
  var existing = index.resolve(targetRef(), true);
  if (existing.ok) {
    if (!usableTarget(existing.topic, allowHandedOff)) {
...
function migrateProduction(index, ledger, session) {
  ...
  if (targetPresent && !records[0].applied) {
    ...
    if (!pendingTarget.ok || !usableTarget(pendingTarget.topic, false)) {
      return { ok: false, code: "recovery_target_conflict" };
    }
  }
  // Nothing is left in Main alone, so every repair below is duplicate cleanup: the
  // target already holds each turn and is never created or first populated here.
  var cleanupOnly = !locations.some(function (location) {
    return location === SOURCE_THREAD_ID;
  });
  var moved = 0;
  var created = false;
  if (!allTarget) {
    var target = ensureTarget(index, cleanupOnly && records[0].applied);
  ...
  if (records[0].applied) {
    return { ok: true, migrationId: PRODUCTION_RECOVERY_ID,
      noop: moved === 0 && created === false, moved: moved,
  ...
  return { ok: true, migrationId: PRODUCTION_RECOVERY_ID, noop: false, moved: moved,
...
function migrateProductionFromSessionManager(index, ledger, sm) {
  ...
  if (!sm || !sm.sessions || typeof sm.sessions.forEach !== "function") {
    return { ok: false, code: "recovery_canonical_session_unavailable" };
  }
  ...
  if (!matches) return { ok: false, code: "recovery_canonical_session_unavailable" };

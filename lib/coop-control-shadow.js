...
// The canonical projection of a responseRef is the event ref and nothing else.
// A responseRef may also carry a messageUuid anchor (coop-owner-response-
// resolution.js), and it is deliberately dropped here: the anchor is a
// redundant identity for a position the projection already carries, it adds no
// authority, and admitting it would both move the digest of every answered
// record already stored and require the durable store's exact-field validator
// to learn a field it has no use for.
function referenceOnlyResponseRef(ref) {
  if (!ref) return ref === undefined ? undefined : null;
  return {
...
function referenceOnlyResponse(response) {
  ...
  return {
    ...
    responseRef: referenceOnlyResponseRef(source.responseRef),

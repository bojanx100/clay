...
function markInteractiveToolWait(session, requestId) {
  if (!session.interactiveToolWaits) session.interactiveToolWaits = {};
  session.interactiveToolWaits[requestId] = true;
}

function clearInteractiveToolWait(session, requestId) {
  if (!session.interactiveToolWaits) return;
  delete session.interactiveToolWaits[requestId];
}
...
function attachBridgeDialogs(ctx) {
  ...
  function handleElicitation(session, request, opts) {
    ...
    return new Promise(function(resolve) {
      ...
      markInteractiveToolWait(session, requestId);
      ...
      if (opts.signal) {
        opts.signal.addEventListener("abort", function() {
          if (!session.pendingElicitations[requestId]) return;
          delete session.pendingElicitations[requestId];
          clearInteractiveToolWait(session, requestId);
          sendAndRecord(session, { type: "elicitation_resolved", requestId: requestId, action: "reject" });

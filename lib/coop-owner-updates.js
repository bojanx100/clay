...
function evidence(view) {
  ...
  (view.history || []).forEach(function (item) {
    if (item && item.type === "user_message" && item.synthetic && item.autoAction && item.coopLeadWake) {
      var review = require("./coop-proactive-review").feedbackRef(item.coopProactiveReview);
      if (review) byId[review.eventId] = review;
    }

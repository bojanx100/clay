...
function createConcurrencyLimiter(options) {
  ...
  // A pre-receipt record with no binding is a durable migration remnant, not an
  // ambiguous current execution. The original launcher has no way to put an
  // old direct session in the binding store; any such session is independently
  // included by collectLiveSessionKeys(). Counting the remnant as unknown
  // would permanently exhaust capacity after the cutover.
  ...
  function occupiesSlot(candidate) {
    if (isLegacyUnboundCandidate(candidate)) return false;

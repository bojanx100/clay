...
function createConcurrencyLimiter(options) {
  ...
  function collectLiveSessionKeys() {
    ...
    if (!sm || !sm.sessions || typeof sm.sessions.forEach !== "function") {
      return { ok: true, keys: keys, byKey: byKey };
    }
    var index = 0;
    try {
      sm.sessions.forEach(function (session) {
        ...
        if (tl.workflowCompleted || tl.executionCompletionReported) return;
  ...
  function completionEventMatches(session, binding, completedAt) {
    var events = Array.isArray(session && session.orchestrationEvents) ?
      session.orchestrationEvents : [];
    for (var i = 0; i < events.length; i++) {
      var event = events[i] || {};
      var data = event.data || {};
      if (event.type !== "project_completed") continue;
      if (data.portfolioTaskId !== binding.portfolioTaskId ||
          data.bindingRevision !== binding.bindingRevision) continue;
      if (positiveFiniteNumber(event.at) !== completedAt) continue;
      if (data.integrationVerification !== "yes" || data.escalationRequired !== "no") continue;
      return true;
    }
    return false;
  }

  function projectRefMatchesSession(session, projectRef) {
    var sessionProject = projectIdentity.normalizeProjectRef(
      session && (session.projectRef || (session.projectId ? { projectId: session.projectId } : null)));
    return !sessionProject || sessionProject.projectId === projectRef.projectId;
  }

  function hiddenCompletedSessionEvidence(candidate, binding, session) {
    var projectRef = projectIdentity.normalizeProjectRef(candidate && candidate.projectRef);
    var sessionRef = projectIdentity.sessionRef(projectRef, session);
    if (!projectRef || !sessionRef || !projectRefMatchesSession(session, projectRef)) return null;
    if (!session || session.hidden !== true) return null;
    ...
    var completedAt = positiveFiniteNumber(policy && policy.completedAt);
    if (!policy || !completion || !completedAt || completion.revokedAt != null ||
        session.isProcessing === true) return null;
    if (policy.targetProject && (!projectIdentity.normalizeProjectRef(policy.targetProject) ||
        policy.targetProject.projectId !== projectRef.projectId)) return null;
    if (policy.portfolioTaskId !== binding.portfolioTaskId ||
        policy.bindingRevision !== binding.bindingRevision ||
        policy.mode !== "project_coordinator" || policy.status !== "completed" ||
        policy.idempotencyKey !== automationIdentity.idempotencyKeyFor(
          binding.portfolioTaskId, binding.bindingRevision) ||
        !projectIdentity.normalizeSessionRef(policy.source) ||
        typeof policy.projectCompletionResultEventId !== "string" ||
        !policy.projectCompletionResultEventId.trim() ||
        typeof policy.projectCompletionDeliveryEventId !== "string" ||
        !policy.projectCompletionDeliveryEventId.trim()) {
      return null;
    }
    if (completion.status !== "completed" ||
        completion.portfolioTaskId !== binding.portfolioTaskId ||
        completion.bindingRevision !== binding.bindingRevision ||
        positiveFiniteNumber(completion.completedAt) !== completedAt ||
        completion.integrationVerification !== "yes" ||
        completion.escalationRequired !== "no") {
      return null;
    }
    if (!completionEventMatches(session, binding, completedAt)) return null;
    return {
      sessionRef: sessionRef,
      completedAt: completedAt,
      resultEventId: policy.projectCompletionResultEventId,
      deliveryEventId: policy.projectCompletionDeliveryEventId,
    };
  }

  function missingBindingReleasedByCompletedSession(candidate, binding) {
    var snapshot;
    try { snapshot = getBindings ? getBindings() : null; } catch (error) { return false; }
    if (!repairEvidence.bindingConflicts(candidate, snapshot, opts.projectSlug, binding).ok) return false;
    if (!sm || !sm.sessions || typeof sm.sessions.forEach !== "function") return false;
    var sessions = [];
    var found = null;
    var ambiguous = false;
    try {
      sm.sessions.forEach(function (session) {
        ...
        var evidence = hiddenCompletedSessionEvidence(candidate, binding, session);
        if (!evidence) return;
        if (found) {
          ambiguous = true;
          return;
    ...
    return !!found && !ambiguous && repairEvidence.noLiveSession(candidate, {
      sessionSnapshot: { projectRef: candidate.projectRef, sessions: sessions },
    }, opts.projectSlug).ok;
  }
  ...
  // above — releases one.
  function occupiesSlot(candidate, liveKeys) {
    ...
    if (!record || typeof record !== "object") {
      var itemKey = normalizeItemKey(candidate && (candidate.itemKey || candidate.candidateKey));
      if (itemKey && liveKeys && liveKeys[itemKey] === true) return true;
      return !missingBindingReleasedByCompletedSession(candidate, binding);
    }
  ...
  function collectAdmittedKeys(liveKeys) {
    ...
    for (var i = 0; i < result.candidates.length; i++) {
      ...
      if (!occupiesSlot(candidate, liveKeys)) continue;
  ...
  function inFlight() {
    ...
    var admitted = collectAdmittedKeys(live.byKey);
    if (!admitted.ok) {

...
function isFableEntry(m) { return valueOf(m).toLowerCase().indexOf("fable") !== -1; }
function isMetaEntry(m) { var v = valueOf(m); return v === "default" || v === "best"; }

// Merge probe extras into the picker list at the right spot: a new flagship
// Opus (Opus 5) belongs right after Fable — just below the most-capable model —
// not appended at the bottom past Haiku. If there's no Fable entry, insert ahead
// of the first concrete model (after the leading meta selectors). Skips extras
// already present.
function mergeExtras(list, extras) {
  var base = Array.isArray(list) ? list.slice() : [];
  if (!extras || !extras.length) return base;
  var present = {};
  base.forEach(function (m) { var v = valueOf(m); if (v) present[v] = 1; });
  var toAdd = extras.filter(function (e) { return !present[valueOf(e)]; });
  if (!toAdd.length) return base;

  var idx = -1;
  for (var i = 0; i < base.length; i++) { if (isFableEntry(base[i])) { idx = i + 1; break; } }
  if (idx === -1) { // no Fable — insert after the leading meta selectors
    idx = 0;
    for (var j = 0; j < base.length; j++) { if (isMetaEntry(base[j])) idx = j + 1; else break; }
  }
  Array.prototype.splice.apply(base, [idx, 0].concat(toAdd));
  return base;
}

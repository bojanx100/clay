...
function advanceCanonicalCoopTopics(options, canonicalSession) {
  ...
  // Daemon maintenance runs these indexed migrations after completed turns.
  if (retro.ok && typeof index.ensureTitleRetrofit === "function") {
    index.ensureTitleRetrofit(session);
  }
  // Consolidate settled titles before assigning dispositions to historical fragments.

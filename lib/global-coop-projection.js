...
function coordinatorTree(options, project, projectId) {
  ...
  function nodeFor(session, depth, visited) {
    ...
    var childSessions = children[storageId] || [];
    var childNodes = [];
    for (var ci = 0; ci < childSessions.length; ci++) {
      var child = nodeFor(childSessions[ci], depth + 1, nextVisited);
      if (child) childNodes.push(child);
    }
    childNodes.sort(function (a, b) { return (b.updatedAt || 0) - (a.updatedAt || 0); });
    return {
      ...
      role: session.coordinationMode && !parentStorageId(session) ? "coordinator" : "worker",
      status: taskStatus(session, byStorageId),
  ...
  for (var si = 0; si < sessions.length; si++) {
    if (parentStorageId(sessions[si])) workerCount++;
    if (!sessions[si].coordinationMode || parentStorageId(sessions[si])) continue;
...
function advanceCanonicalCoopTopics(options, canonicalSession) {
  ...
  return index.ensureRetro(session, {
    projects: contexts,
...
function clientTopic(topic, group) {
  ...
  return {
    ...
    title: topic.title,
    status: topic.status,
    // Derived from canonical linked work: "working" | "needs_input" | "done",
    // or "" when the topic has no linked work and therefore no honest state.
    // This replaces `active`, which was `status === "open"` -- true of every
    // topic that reaches the client, so it could never be false.
...
function buildGlobalCoopProjection(options) {
  ...
  if (!leadProject || !canAccessProject(opts, leadProject)) {
    return { type: "global_coop_projection", coop: null, projects: [], topics: [], topicProjection: null };
  }
  var projects = [];
  ...
  return {
    ...
    actionQueue: coopActionQueue.buildActionQueue(queueProjects, {}),
    coop: coopHome(opts, leadProject, projectIdentity.LEAD_PROJECT_ID),
    projects: projects,
    topics: clientTopics(indexedTopics),

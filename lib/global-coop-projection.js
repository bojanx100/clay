...
function controlPlaneSessionProjection(options, leadProject) {
  var result = [];
  var sessions = sessionList(leadProject);
  for (var i = 0; i < sessions.length; i++) {
    var policy = coopControlPlane.controlPolicy(sessions[i]);
    if (!policy || policy.role !== "council" && policy.role !== "triage" ||
        !canAccessSession(options, leadProject, sessions[i])) continue;
    var ref = projectIdentity.sessionRef({ projectId: projectIdentity.LEAD_PROJECT_ID }, sessions[i]);
    if (!ref) continue;
    result.push({
      role: policy.role,
      title: cleanText(sessions[i].title, policy.role === "council" ? "Council" : "Triage"),
      sessionRef: ref,
    });
  }
...
function buildGlobalCoopProjection(options) {
  ...
  return {
    ...
    controlPlaneSessions: controlPlaneSessionProjection(opts, leadProject),
    projects: projects,
    topics: topics,
    threads: topics,

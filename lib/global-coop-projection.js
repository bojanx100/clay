...
function topicProjection(options, contexts, leadProject) {
  ...
  return index.project({
    ...
    canAccessProject: function (actor, ref) {
      for (var i = 0; i < contexts.length; i++) {
        var status = projectStatus(contexts[i]);
        if (projectIdFor(contexts[i], status) !== ref.projectId) continue;
        return canAccessProject(options, contexts[i]);
      }
      return false;
    },
  });
...
function clientTopic(topic, group) {
  ...
  return {
    ...
    relatedExecution: topic.relatedExecutions || [],

...
var primitiveReconsideration = require("./project-automation-primitive-reconsideration");
...
function normalize(value) {
  if (value && value.schema === primitiveReconsideration.SCHEMA) return primitiveReconsideration.normalize(value);
...
function fromCandidate(value) {
  if (!plainObject(value)) return null;
  if (value.schema === primitiveReconsideration.SCHEMA) return primitiveReconsideration.normalize(value);
  return normalize({
    schema: value.schema,
...
function candidateMismatch(candidate, expected) {
  var present = Object.prototype.hasOwnProperty.call(candidate, "reconsideration");
  var receipt = present ? fromCandidate(candidate.reconsideration) : null;
  if (present && !receipt) return "automation_candidate_malformed";
  return JSON.stringify(receipt || null) === JSON.stringify(expected || null) ? null :
...
function matchesCompletedBinding(value, binding) {
  if (value && value.schema === primitiveReconsideration.SCHEMA) return primitiveReconsideration.matchesBinding(value, binding);

...
function migrateProduction(index, ledger, session) {
  ...
  if (current.applied) {
    return { ok: true, migrationId: RECOVERY_ID, decisionBackfilled: false,
  ...
  return { ok: true, migrationId: RECOVERY_ID, decisionBackfilled: true,
    threadRef: { threadId: THREAD_ID } };
}
...
function migrateProductionFromSessionManager(index, ledger, sm) {
  ...
  if (!sm || !sm.sessions || typeof sm.sessions.forEach !== "function") {
    return { ok: false, code: "threads_recovery_event_missing" };
  }
  ...
  if (!matches) return { ok: false, code: "threads_recovery_event_missing" };

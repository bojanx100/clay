...
var processing = require("./coop-sidebar-execution-state").processing;
...
function coordinatorTransparency(root, children, owner) {
  ...
  function visit(node) {
    ...
    if (node.status === "running") working = true;
    if (["waiting", "reviewing", "ready", "queued"].indexOf(node.status) !== -1) waiting = true;
  ...
  var status = contextBlocked || reportsBlocked || attention ? "needs_input" :
    processing(root) ? "running" : working ? "monitoring" : waiting ? "waiting" : "standby";
  var activity = processing(root) ? text(root.currentActivity) || "Reviewing project work" :
    contextBlocked ? "Project context needs attention" : reportsBlocked ? "Report delivery needs review" :
    attention ? "Project work needs attention" : working ? "Assigned work is in progress" :
    waiting ? "Assigned work is waiting; no execution is running" : "No active assignments";

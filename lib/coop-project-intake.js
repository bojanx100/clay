...
var currentContext = require("./coop-coordinator-context-delivery").currentContext;

var RETRY_MS = 60000;
var MAX_NOTIFICATIONS = 3;
var REPORT_BLOCKED_ACTIVITY = "Review the project coordinator's pending reports before accepting this assignment";
...
function createProjectIntake(options) {
  ...
  function notify(manager, root, task) {
    var record = task.projectAssignment;
    if (modeOn() && !assignment.closed(task) && assignment.valid(root, task) &&
        record.phase === "attention" && record.reason === "coordinator_report_delivery_attention" &&
        !(root.pendingCoordinatorUpdates || []).length && task.status === "needs_input" &&
        task.currentActivity === REPORT_BLOCKED_ACTIVITY && !task.userQuestion) {
      var recovered = assignment.update(manager, root, task, { phase: "pending", reason: "",
        nextNotificationAt: 0, attentionEnvelope: null, attentionReported: false, nextAttentionAt: 0,
        attentionGeneration: Number(record.attentionGeneration || 0) + 1 },
      { status: "queued", currentActivity: "Awaiting project coordinator acceptance" });
      if (!recovered.ok) return false;
      record = task.projectAssignment;
    }
    ...
    var reportAttentionNeeded = (root.pendingCoordinatorUpdates || []).some(function (entry) {
      return entry.state === "uncertain" || entry.state === "attention";
    });
    if (reportAttentionNeeded) {
      var paused = assignment.update(manager, root, task, { phase: "attention",
        reason: "coordinator_report_delivery_attention" }, { status: "needs_input",
        currentActivity: REPORT_BLOCKED_ACTIVITY });
      if (paused.ok) reportAttention(manager, root, task);
      return false;
    }
  ...
  function accept(caller, manager, input) {
    ...
    if (!currentContext(caller, context)) return { ok: false, reason: "coordinator_context_refresh_required",
      message: "Project rules were not supplied to this provider, or changed during this turn. Refresh coordinator context before accepting work." };

    ...
    handler: function (args) {
      ...
      return onPropose(briefData).then(function (result) {
        ...
        if (result && result.action === "error") {
          // The user approved, but the debate could not actually start
          // (F-6): report the truth instead of a false success.
          return { content: [{ type: "text", text: "Debate was approved but could NOT start: " + (result.reason || "unknown reason") + ". No debate is running." }] };
        }

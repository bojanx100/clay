...
var deliverProjectAttention =
  require("./project-task-orchestrator-project-completion-transport").deliverProjectAttention;
...
// Terminalization is durable execution state, not an archive instruction. A
// visible worker stays navigable after it completes; only a separately proven
// owner archive may hide it. Keeping this compatibility entry point lets the
// explicit topic-close path retain its scoped archive behavior.
function archiveCompletedCoopSession(sm, session, options) {
  if (!sm || !completedCoopExecution(session)) return false;
  if (!options || options.explicit !== true || session.hidden) return false;
...
function attachCompletionGate(ctx) {
  ...
  function recordImplementationAwaitingAcceptance(session, state, redeliver) {
    var execution = session.orchestrationPolicy.portfolioExecution;
    if (!acceptancePending(execution)) return false;
  ...
  // A refused envelope used to vanish: recordProjectCompletion returned false,
  // nothing was written, and nothing was said. The coordinator believed it had
  // reported done while its binding kept projecting active for days. Record
  // the refusal durably and hand the coordinator the exact missing field, once
  // per distinct refusal so a repeated identical envelope cannot loop.
  ...
  // Returns "completed", "refused", "changed" or "".
  function recordProjectCompletion(session, redeliver) {
    ...
    if (existing.status === "completed") {
      if (acceptancePending(execution)) {
        return recordImplementationAwaitingAcceptance(session, existing, redeliver) ?
          "needs_input" : "";
      }
      ...
      return finalized || delivered ? "completed" : "";
    }
    var result = taskState.workerResultText(session);
    // The envelope is authored in a chat turn, so it arrives formatted. Read
    // the fields through the presentation normalizer: markdown emphasis around
    // the labels must not void a real completion, and the marker itself still
    // has to own its whole line so declining prose can never complete a graph.
    ...
    if (acceptancePending(execution)) {
      recordImplementationAwaitingAcceptance(session, completed.state, true);
      return "needs_input";
    }
  ...
  function recordProjectNeedsInput(session) {
    ...
    var result = taskState.workerResultText(session);
    if (!/(?:^|\n)WORKER_STATUS:\s*needs_input\b/i.test(result)) return false;
    var reason = taskState.workerReasonFromResult(result);
    ...
    if (changed) {
      // Review-only and peer coordinators terminate their source binding when
      // they return owner attention. Release the matching control lease before
      // publishing that terminal projection so a restart cannot observe
      // `needs_input` in the session while the durable execution remains live.
      if (reviewAttention && execution.status !== "needs_input") {
        finishControlledExecution(session, "needs_input");
      }
      execution.status = "needs_input";
      execution.updatedAt = Date.now();
      if (reason) execution.reason = reason;
      else delete execution.reason;
      coordinatorHierarchy.rollUpTaskCoordinator(sm, session, "needs_input", reason);
    }
    if (reviewAttention && !execution.attentionDeliveredAt) {
      var report = taskState.projectCompletionFromResult(result);
      var delivered = deliverProjectAttention(sm, crossProject, session,
  ...
  function restore(session) {
    ...
    var revoked = taskGraph.reconcileProjectCompletion(session);
    var graphState = taskGraph.graphResolutionState(session);
    var outcome = graphState.phase === "complete" ? recordProjectCompletion(session) : "";
    var completed = outcome === "completed";
    var needsInput = !completed && recordProjectNeedsInput(session);
    var completionHydrated = completionWasMissing && !!session.orchestrationProjectCompletion;
    if (revoked || outcome || needsInput || completionHydrated) persistState(session);

...
function hasVisibleDescendant(sm, session) {
...
// One shared auto-archive gate for Coop-created portfolio sessions. Both normal
// execution completion and an explicit close of the linked topic come here, so
// owner-created direct sessions can never be hidden by either lifecycle path.
function archiveCompletedCoopSession(sm, session) {
  if (!sm || !completedCoopExecution(session)) return false;
  var evidenceChanged = isPortfolioProjectCoordinator(session) && archiveTaskEvidence(session);
  var needsHide = !session.hidden || hasVisibleDescendant(sm, session);
  if (!evidenceChanged && !needsHide) return false;

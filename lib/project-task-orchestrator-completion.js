...
function attachCompletionGate(ctx) {
  ...
  function recordProjectNeedsInput(session) {
    ...
    var canaryAttention = unavailableVisualCanary(reason, result), terminalAttention = reviewAttention || canaryAttention;
    var changed = execution.status !== "needs_input" ||
      String(execution.reason || "") !== reason;
    if (changed) {
      // Terminal attention releases its lease before publishing so a restart
      // cannot see `needs_input` while the durable execution remains live.
      if (terminalAttention && execution.status !== "needs_input") {
    ...
    if (terminalAttention && !execution.attentionDeliveredAt) {

...
function attachCompletionGate(ctx) {
  ...
  function recordProjectCompletion(session, redeliver, options) {
    ...
    if (existing.status === "completed") {
      ...
      var finalized = finalizeProjectCompletion(session, existing, reviewDelivery);
      var delivered = (redeliver || reviewDelivery) &&
        deliverProjectCompletion(sm, crossProject, session, existing);
      if (reviewDelivery && delivered && delivered.ok === true) {
        clearLocalReviewAcceptance(execution);
      }
    ...
    finalizeProjectCompletion(session, completed.state, reviewDelivery);
    var delivered = deliverProjectCompletion(sm, crossProject, session, completed.state);
    if (reviewDelivery && delivered && delivered.ok === true) {
      clearLocalReviewAcceptance(execution);
    }

...
var budget = require("./lead-budget");
...
//   input.budget: lead-budget.buildDailyBudget output, or null
//   input.now: injected clock (ms)
//   input.upNextCount: how many queued items to preview (default 3)

function emptyDigest() {
  return {
    shipped: [], blocked: [], failed: [], staffedBy: {}, terminal: {}, lastMetrics: null,
    projectCompleted: [], projectRevoked: [], workerCompleted: [], portfolioCompleted: [],
  };
}

function recordCompletionEvent(digest, event) {
  if (!event) return false;
  if (event.type === "project_completed") digest.projectCompleted.push(event);
  else if (event.type === "project_completion_revoked") digest.projectRevoked.push(event);
  else if (event.type === "worker_completed") digest.workerCompleted.push(event);
  else if (event.type === "portfolio_completed") digest.portfolioCompleted.push(event);
  else return false;
  return true;
}

function recordTaskEvent(digest, event) {
  if (!event || !event.item) return;
  if (event.type === "staffed") digest.staffedBy[event.item.id] = event;
  else if (event.type === "completed") {
    digest.shipped.push(event);
    digest.terminal[event.item.id] = true;
  } else if (event.type === "blocked") {
    digest.blocked.push(event);
    digest.terminal[event.item.id] = true;
  } else if (event.type === "failed") {
    digest.failed.push(event);
    digest.terminal[event.item.id] = true;
  }
}
...
function digestEvents(events) {
  ...
  for (var i = 0; i < events.length; i++) {
    var ev = events[i];
    if (ev && ev.type === "metrics_report") d.lastMetrics = ev;
    else if (!recordCompletionEvent(d, ev)) recordTaskEvent(d, ev);
  }
  d.inFlight = [];
...
function completionLabel(event) {
  return String(event.portfolioTaskId || "portfolio") + " rev " +
    String(event.bindingRevision || event.completionRevision || "?");
}

function completionGateSection(digest) {
  var lines = ["## Completion gates"];
  if (!digest.projectCompleted.length && !digest.projectRevoked.length &&
      !digest.workerCompleted.length && !digest.portfolioCompleted.length) {
    lines.push("- no portfolio completion transitions");
  }
  for (var i = 0; i < digest.projectCompleted.length; i++) {
    lines.push("- PROJECT VERIFIED: " + completionLabel(digest.projectCompleted[i]));
  }
  for (var j = 0; j < digest.workerCompleted.length; j++) {
    lines.push("- WORKER EVIDENCE: " + completionLabel(digest.workerCompleted[j]) +
      " (does not close project or portfolio)");
  }
  for (var k = 0; k < digest.projectRevoked.length; k++) {
    lines.push("- PROJECT REVOKED: " + completionLabel(digest.projectRevoked[k]) +
      " — " + (digest.projectRevoked[k].reason || "new or unresolved work"));
  }
  for (var m = 0; m < digest.portfolioCompleted.length; m++) {
    lines.push("- PORTFOLIO CLOSED BY COOP: " +
      String(digest.portfolioCompleted[m].portfolioTaskId || "portfolio"));
  }
  lines.push("");
  return lines;
}
...
function healthSection(canaries, lastMetrics, dailyBudget) {
  var lines = ["## Health"];
  if (canaries) {
  ...
  lines.push("- " + budget.formatBurnRate(dailyBudget));
  return lines;
}
...
function composeStandup(input) {
  ...
  var dailyBudget = (input && input.budget) || null;
  ...
  var lines = ["# Standup", ""]
    ...
    .concat(completionGateSection(digest))
    .concat(upNextSection(portfolio, digest, upNextCount))
    .concat(healthSection(canaries, digest.lastMetrics, dailyBudget));

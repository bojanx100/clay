...
var { attachSessionHandoff: attachNewSessionHandoff } = require("./project-session-handoff");
...
function attachSessions(ctx) {
  ...
  var sessionMcpGate = ctx.sessionMcpGate || null;
  ...
  var newSessionHandoffHandlers = attachNewSessionHandoff({
    cwd: cwd,
    sm: sm,
    isMate: isMate,
    splitStore: ctx.splitStore || null,
  ...
  if (sessionMcpGate) {
    sessionMcpGate.createServers = function (forSession) {
      var info = require("./yoke").getVendorInfo(forSession && forSession.vendor);
      if (info && info.sessionBoundTools === false) return undefined;
      var sessionAdapter = adapters[(forSession && forSession.vendor) || ""] || adapter;
      var server = newSessionHandoffHandlers.createMcpServer(sessionAdapter, forSession);
      if (!server) return undefined;
      var result = {};
      result[server.name || "clay-handoff"] = server;
      return result;
    };
  }
  ...
  function handleSessionsMessage(ws, msg) {
    if (msg.type === "handoff_session_options" ||
        (msg.type === "handoff_session" && msg.handoffMode === "new-session")) {
      return newSessionHandoffHandlers.handleMessage(ws, msg);
    }
    var handlers = [

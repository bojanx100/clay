  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "set_session_full_access") {
      ...
      if (!fullAccessSession || fullAccessMode !== "gui") {
        sendTo(ws, { type: "session_full_access_result", ok: false, id: msg.id || null, error: "YOLO is only available for GUI sessions." });
        return true;
      }
      if (usersModule.isMultiUser() && ws._clayUser &&
      ...
      (async function () {
        try {
          ...
          sendTo(ws, { type: "session_full_access_result", ok: false, id: fullAccessSession.localId, error: "Failed to change YOLO mode: " + (e.message || e) });
        }
      })();
      return true;
    }

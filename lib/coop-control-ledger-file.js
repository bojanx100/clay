...
function acquireLock(file, options) {
  ...
  var timeoutMs = Number.isFinite(opts.timeoutMs) ? opts.timeoutMs : DEFAULT_LOCK_TIMEOUT_MS;
  ...
  while (true) {
    try {
      ...
      if (now() >= deadline) {
        var timeout = new Error("Timed out acquiring Coop ledger lock: " + lockFile);
        timeout.code = "COOP_LEDGER_LOCK_TIMEOUT";
        throw timeout;
      }
      sleep(10);
    }

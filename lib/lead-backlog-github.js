...
function ghIssueFields(sourceSpec) {
  ...
  var qualificationProfile = qualification.policyProfile(sourceSpec && sourceSpec.policy);
  if (splitList(filters.skipProjectStatuses).length > 0 ||
      Array.isArray(policyStatuses) && policyStatuses.length > 0 ||
      qualificationProfile && qualificationProfile.normalIssueIntake.requireAllBoardItems) fields += ",projectItems";
...
function policyReason(sourceSpec) {
  ...
  if (typeof policy.digest !== "string" || sourceSpec.policyDigest !== policy.digest) return "policy_stale";
  if (policy.digest !== automationPolicy.policyDigest(policy) || !Array.isArray(policy.recipes)) return "policy_stale";
  return qualification.policyProfile(policy) ? null : "qualification_policy_missing";
}

function policyContainsRecipe(policy, recipe) {
  var digest = automationPolicy.recipeDigest(recipe);
  for (var i = 0; i < policy.recipes.length; i++) {
    if (policy.recipes[i] && policy.recipes[i].id === recipe.id &&
        policy.recipes[i].digest === digest) return true;
  }
  return false;
}
...
function canonicalSourceReason(sourceSpec) {
  ...
  var reason = policyReason(sourceSpec);
  if (reason) return reason;
  if (!policyContainsRecipe(sourceSpec.policy, sourceSpec.recipe) ||
      normalizeRepoSlug(sourceSpec.recipe.source && sourceSpec.recipe.source.repo) !== sourceSpec.repo) {
...
function collectGithubIssues(execFn, sourceSpec, project, cb) {
  ...
  var qualificationProfile = qualification.policyProfile(sourceSpec.policy);
  var qualificationRequiresBoard = !!(qualificationProfile &&
    qualificationProfile.normalIssueIntake.requireAllBoardItems === true);
  function collect(currentLogin) {
    execFn("gh", ghIssueArgs(sourceSpec), function (err, stdout) {
      ...
      function next() {
        ...
        if (!qualificationRequiresBoard) return consider(raw);
        ...
        evidenceLookups++;
        boardEvidence.collectBoardItemEvidence(execFn, sourceSpec.repo, raw.number,
          qualificationProfile.normalIssueIntake.configuredBoard || null, function (evidence) {
          if (!evidence.ok) {
            exclusions.push({ number: raw.number, reason: evidence.reason });
            return next();
          }
          consider(Object.assign({}, raw, { projectItems: evidence.projectItems }));
        });
      }
      next();
    });

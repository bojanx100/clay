    ...
    execFn("gh", ghIssueArgs(sourceSpec), function (err, stdout) {
      ...
      for (var i = 0; i < parsed.length; i++) {
        ...
        if (!sourceSpec.recipe) {
          if (projectStatusAccepts(parsed[i], sourceSpec.filters) && filterAccepts(item, sourceSpec.filters)) items.push(item);
          continue;
        }
        if (!taskSources.issueMatches(sourceSpec.recipe, {}, parsed[i], currentLogin)) {
          exclusions.push({ number: parsed[i].number, reason: "launcher_recipe_ineligible" });
          continue;
        }
        var status = policyStatusEligibility(parsed[i], sourceSpec);
        if (!status.ok || !status.eligible) {
          exclusions.push({ number: parsed[i].number, reason: status.reason });
          continue;
        }
        var assigned = taskSources.issueAssignedToOwner(sourceSpec.recipe, {}, parsed[i], currentLogin);
        var qualified = qualificationEligibility(parsed[i], sourceSpec, currentLogin);
        if (!qualified.ok || !qualified.eligible) {
          exclusions.push({ number: parsed[i].number, reason: qualified.reason ||
            "qualification_unresolvable" });
          continue;
        }
        var decision;
        try {
          decision = sourceSpec.candidateEligibility(sourceSpec.repo + "#" + parsed[i].number,
        ...
        if (!decision || decision.ok !== true || decision.eligible !== true) {
          exclusions.push({ number: parsed[i].number, reason: decision && decision.reason ||
      ...
      cb(null, items, { exclusions: exclusions });
    });
  }

...
function completedMessage(event, threadId) {
  ...
  var images = role === "user" ? item.content.filter(require("./codex-rollout-images").isImage) : [];
  return text || images.length ? { role: role, text: text, id: item.id || "", imageContent: images } : null;
}
...
function completedUserPrefix(line, decode, maxChars) {
  ...
  if (line.length <= 64 * 1024) {
    try {
      ...
      return message && message.role === "user" ? (message.text || "Image attachment").slice(0, maxChars) : null;
    } catch (e) {}
  }
  ...
  if (foundText) return decode(line, foundText.index + foundText[0].length - 1, maxChars);
  var imagePart = /(?<!\\)"type"\s*:\s*"(?:image|local_image)"/g;
  imagePart.lastIndex = foundContent.index + foundContent[0].length;
  return imagePart.exec(line) ? "Image attachment" : null;
}

...
function attachCoopSessionLedger(options) {
  ...
  function topicEvidence(topicRef, metadata) {
    ...
    for (var i = 0; i < state.entries.length; i++) {
      ...
      // A durable binding can be the only surviving evidence after an adapter
      // restart or a historical session cleanup. Do not call that owner work
      // idle just because its old session is gone: a binding-backed attention
      // state remains actionable and must reach the top-level Coop projection.
      // This deliberately does not revive unbound missing rows or any hidden
      // session, which remain non-live history.
      var durableAttention = !entry.hidden && !entry.sessionPresent && matchedBinding &&
        projectedState === "needs_input";
      if (!visibleEvidence && !durableAttention && projectedState !== "done") continue;

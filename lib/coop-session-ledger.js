...
function buildEntry(input) {
  ...
  return {
    ...
    workState: lifecycle.projectedWorkState(status, session, binding, execution),
...
function attachCoopSessionLedger(options) {
  ...
  // Narrow repair surface: delete exact rows by SessionRef. reconcile() cannot
  // do this and must not -- it carries every prior row forward on purpose, so a
  // row whose project no longer exists at all can never be dropped by normal
  // operation. Refuses outright if any named row still has a live session, so
  // this can never be turned into a way to erase real work, and it inherits the
  // same fail-closed write path as reconcile.
  function removeEntries(refs) {
    if (unreadable) {
      recordCanary("write_refused", unreadable.code,
        "Refusing to modify an unreadable Coop session ledger.");
      return { ok: false, code: "persistence_unreadable", removed: 0 };
    }
    var refList = Array.isArray(refs) ? refs : [];
    if (!refList.length) return { ok: true, removed: 0 };
    var wanted = {};
    for (var ri = 0; ri < refList.length; ri++) {
      var ref = projectIdentity.normalizeSessionRef(refList[ri]);
      if (!ref) return { ok: false, code: "invalid_session_ref", removed: 0 };
      wanted[keyFor(ref.projectId, ref.sessionStorageId)] = true;
    }
    var kept = [];
    var removed = 0;
    for (var ei = 0; ei < state.entries.length; ei++) {
      var entry = state.entries[ei];
      if (!wanted[keyFor(entry.projectRef.projectId, entry.sessionStorageId)]) {
        kept.push(entry);
        continue;
      }
      if (entry.sessionPresent === true) {
        return { ok: false, code: "session_present", removed: 0 };
      }
      removed++;
    }
    if (!removed) return { ok: true, removed: 0 };
    var nextState = { schema: SCHEMA, version: VERSION, entries: kept };
    try { writeState(fsImpl, file, nextState); }
    catch (e) {
      recordCanary("write", e && e.code || "write_failed",
        e && e.message || "Coop session ledger write failed.");
      return { ok: false, code: e && e.code || "write_failed", removed: 0 };
    }
    state = nextState;

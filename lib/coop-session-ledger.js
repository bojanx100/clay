// Reconciled manifest of every project session Coop created or touched. It
// joins binding and live-session authority by exact ProjectRef + storage id so
// callers never have to infer session truth from append-only Lead notes.
...
function terminalOutcome(status, role, sources) {
  ...
  var summary = sources.completion && sources.completion.summary ||
  ...
  var at = finite(sources.completion && sources.completion.completedAt) ||
...
function buildEntry(input) {
  ...
  var bindings = (input.bindings || []).slice().sort(compareBindings);
  ...
  var status = lifecycleState(session, binding, task, role);
...
function attachCoopSessionLedger(options) {
    ...
    for (var pr = 0; pr < projects.length; pr++) {
      ...
      for (var ei = 0; ei < eligibleIds.length; ei++) {
        ...
        next[currentKey] = buildEntry({
          ...
          parent: parent,
          task: task,
          bindings: bindingMap[currentKey] || [],
          parentBindings: parentKey && bindingMap[parentKey] || [],
          topicRefs: linkMap[currentKey] || [],
        });
      }
    }
  ...
  function topicEvidence(topicRef, metadata) {
    ...
    for (var i = 0; i < state.entries.length; i++) {
      ...
      // One session may have served multiple portfolio revisions/topics. The
      // query is for one exact TopicRef, so its evidence must carry that ref;
      // entry.coopTopicRef is only the stable first item in the full link set
      // and may name an unrelated earlier/later topic.

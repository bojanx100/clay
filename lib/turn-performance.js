...
function configure(session, options) {
...
function begin(session, nowValue) {
  ...
  session._performanceTurn = {
    ...
    toolCalls: 0,
    verificationCalls: 0,
    route: Object.assign({ model: session.model || null, effort: session.effort || null }, session._performanceRoute),
    resultSeen: false,
    failed: false,
    finished: false,
  };
...
function observe(session, event, nowValue) {
  ...
  if ((type === "tool_start" || type === "tool_executing") && id) {
...
function finish(session, outcome, nowValue, writer) {
  ...
  var record = {
    schema: "clay.turn_performance.v1", at: now,
    ...
    vendor: session.vendor || null, model: state.route.model, effort: state.route.effort,
...
module.exports = { begin: begin, configure: configure, observe: observe, finish: finish, performanceFile: performanceFile };

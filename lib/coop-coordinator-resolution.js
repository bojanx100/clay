...
function createResolver(ctx) {
  function resolve(input) {
    ...
    var matches = ctx.bindingStore.list().filter(function (binding) {
      return binding.mode === "project_coordinator" && binding.targetProject.projectId === policy.projectRef.projectId &&
        task.clientRef === "portfolio:" + binding.portfolioTaskId + ":" + binding.bindingRevision &&
        sameRef(binding.projectCoordinator, source) && sameRef(binding.coordinator, task.workerSessionRef);
    });
    if (matches.length !== 1) return { ok: false, reason: "resolution_binding_mismatch" };

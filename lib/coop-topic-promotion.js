...
// Automatic topics carry an id derived from their own title hash at creation
// (see coop-topic-classification.automaticTopicId). That is what separates them
// from the curated seeds, which share source "automatic" but keep stable,
// readable ids, and from manual/split topics, which are deliberate owner acts.
...
function ownerRelevantTurnCount(topic, history) {
  ...
  for (var i = 0; i < turnRefs.length; i++) {
    var start = turnRefs[i] && turnRefs[i].startEventIndex;
    if (typeof start !== "number" || start < 0 || start >= items.length) continue;
    if (!topicRelevance.isInternalHistoryItem(items[start])) count += 1;
  }
  return count;
}

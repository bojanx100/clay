...
var HAZARD_PATTERNS = {
  ...
  spendBudget: /\b(?:spend|budget(?:\s+(?:exception|increase|overrun|override))?|billing|purchase|invoice|cost\s+override)\b/i,
...
function assessCandidateSafety(input) {
  ...
  var safety = {
    ...
    spendBudget: value.spendBudget === true,
...
function ownerGrantFrom(input) {
  ...
  var scopes = Array.isArray(entry.implementationScopes) && entry.implementationScopes.length ?
    entry.implementationScopes : (entry.implementationScope ? [entry.implementationScope] : []);
  var scope = null;
  for (var i = 0; i < scopes.length; i++) {
    if (scopes[i].portfolioTaskId === authorizationTaskId) {
      if (scope) return { ok: false, reason: "scoped_policy_owner_provenance_required" };
      scope = scopes[i];
    }
  }

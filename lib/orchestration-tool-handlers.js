...
function createToolHandlers(deps) {
  ...
  function projectExecutionResult(result, input) {
    if (!result || result.ok !== true) {
      ...
      var failure = deps.error("project execution failed: " + reason);
      if (result && result.executionNotStarted === true && failure && typeof failure === "object") {
        failure.structuredContent = { executionNotStarted: true };
      }
      return failure;
    }
    if (result.phase === "assignment_queued") {
      return deps.success("Queued project assignment " + JSON.stringify(result.taskRef) +
        " for the resident project coordinator. No execution session has started yet. " +
        "The coordinator will inspect the scope and current project rules before accepting it.");
    }
  ...
  function delegate(input) {
    if (require("./coop-control-role").isPeer(input.controlRole)) return deps.error(
      "Council and Triage require start_coop_planning in Coop, then commission_coop_plan for authorized work.");
    ...
    var task = taskGraph.createTask(parent, {
      ...
      effort: ["low", "medium", "high"].indexOf(input.effort) >= 0 ? input.effort : null,
  ...
  function plan(input) {
    ...
    for (var i = 0; i < specs.length; i++) {
      ...
      var task = taskGraph.createTask(parent, {
        ...
        effort: ["low", "medium", "high"].indexOf(spec.effort) >= 0 ? spec.effort : null,
  ...
  function resolve(input) {
    ...
    if (task.status === "completed" && task.resolvedByCoordinator) {
      var replay = deps.afterResolve && deps.afterResolve(parent, task);
      return replay && replay.ok === false ? deps.error("Coordinator resolution is saved; portfolio reconciliation needs attention: " + replay.reason) :
        deps.success("Task " + task.taskId + " is already verified and completed.");
    }
    ...
    deps.schedule(parent);
    var reconciled = deps.afterResolve && deps.afterResolve(parent, task);
    if (reconciled && reconciled.ok === false) return deps.error("Coordinator resolution is saved; portfolio reconciliation needs attention: " + reconciled.reason);

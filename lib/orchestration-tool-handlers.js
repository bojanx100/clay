...
function createToolHandlers(deps) {
  ...
  function requestInput(input) {
    ...
    var reason = String(input.reason || "").trim();
    if (!reason) return deps.error("reason is required");
    if (ids.length && suppliedApprovalScopes.length) {
      return deps.error("taskIds and approvalScopes are mutually exclusive");
    }
    ...
    if (ids.length === 0) return deps.error("taskIds or approvalScopes is required");

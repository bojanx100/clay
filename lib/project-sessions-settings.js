...
function attachProjectSessionsSettings(ctx) {
  ...
  function handleSettingsMessage(ws, msg) {
    ...
    if (msg.type === "set_server_default_model" && msg.model) {
      ...
      var session = getSessionForWs(ws);
      if (session && !coopModelPolicy.appliesToSession(session) &&
          (session.vendor || "claude") === serverModelVendor) {
        session.model = msg.model;
        try { sm.saveSessionFile(session); } catch (e) {}
        sdk.setModel(session, msg.model);
      }
    ...
    if (msg.type === "set_project_default_model" && msg.model) {
      ...
      var session = getSessionForWs(ws);
      if (session && !coopModelPolicy.appliesToSession(session) &&
          (session.vendor || "claude") === modelVendor) {
        session.model = msg.model;
        try { sm.saveSessionFile(session); } catch (e) {}
        sdk.setModel(session, msg.model);
      }
    ...
    if (msg.type === "get_project_provider_routing_profile") {
      var routingState = { profile: sm.providerRoutingProfile || "balanced" };
      if (typeof opts.onGetProjectProviderRoutingProfile === "function") {
        routingState = opts.onGetProjectProviderRoutingProfile(slug) || routingState;
      }
      sendTo(ws, {
        type: "project_provider_routing_profile",
        slug: slug,
        profile: normalizeRoutingProfile(routingState.profile),
      });
      return true;
    }

    if (msg.type === "set_project_provider_routing_profile") {
      var requestedProfile = normalizeRoutingProfile(msg.profile);
      var routingResult = { ok: false, profile: requestedProfile, error: "Not supported" };
      if (typeof opts.onSetProjectProviderRoutingProfile === "function") {
        routingResult = opts.onSetProjectProviderRoutingProfile(slug, requestedProfile) || routingResult;
      }
      if (routingResult.ok) {
        sm.providerRoutingProfile = normalizeRoutingProfile(routingResult.profile);
      ...
      sendTo(ws, {
        ...
        slug: slug,
        ok: !!routingResult.ok,
        profile: normalizeRoutingProfile(routingResult.profile),

...
function attachAutoLaunch(ctx) {
  ...
  async function launchScheduled(recipeId, extraArgs) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      var itemIssueKey = issueKey(recipe, item.number);
      var existingIssueSession = !prKind ? tl.findExistingSessionForItem(recipe, item, false) : null;
    ...
    return { ok: true, started: started, skipped: skipped, deferred: deferred, vendorDeferred: vendorDeferred };
  }
  ...
  function notifyCompleted(session, summary) {
    ...
    if (tl.autoKind === "pr-review" && tl.prKey) {
    ...
    } else if (tl.autoKind !== "pr-review" && tl.itemNumber != null) {
      ...
      try {
        ...
        if (recipeI) {
          ...
          issueLaunchState.recordCompletion(issueKey(recipeI, tl.itemNumber), statusNow, progressed);
        }
      } catch (e) {

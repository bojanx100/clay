...
function attachAutoLaunch(ctx) {
  ...
  async function launchScheduled(recipeId, extraArgs) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (authorized.decision !== "execute") {
        skipped.push(item);
        activity.record({
          type: authorized.decision === "propose" ? "proposed" : "blocked",
          recipeId: recipe.id,
          autoKind: autoKind,
          number: item.number,
          url: item.url,
      ...
      // Last check before anything irreversible. The verdict above and this
      // launch are not atomic — an await or a slow tick can span the whole
      // lease — so confirm the claim is still ours rather than starting on a
      // stale authorization.
      if (typeof automationGate.holdsClaim === "function" &&
          authorized.reason !== "lead_mode_off_legacy" &&
          !automationGate.holdsClaim(itemIssueKey)) {
        skipped.push(item);
        activity.record({
          type: "blocked", recipeId: recipe.id, autoKind: autoKind,
          number: item.number, url: item.url, title: item.title,
          reason: "claim_lost_before_launch",
        });
        continue;
      }
      // Consuming launch state and starting the session are one transaction:
      // if either half fails, undo the claim so the item is not pinned for a
      // session that never ran. The launch-state write itself is intentionally
      // inside the try — a throw there used to escape cleanup entirely.
      var sess;
      // Snapshot the launch state BEFORE consuming it, so a failed start can
      // put it back. Releasing the claim alone was not enough: the PR pass
      // stayed spent and the issue stayed marked launched, so an item whose
      // session never started could lose a pass or be deduped forever.
      ...
      try {
        ...
        // Renewing before the call proves the claim was ours at that instant,
        // but session creation is itself a suspendable gap. Re-verify AFTER
        // it: if the claim was lost while the session was being built, the
        // session is torn down rather than left running unclaimed, which is
        // what a second daemon would otherwise duplicate.
        if (typeof automationGate.holdsClaim === "function" &&
            authorized.reason !== "lead_mode_off_legacy" &&
            !automationGate.holdsClaim(itemIssueKey)) {
          throw new Error("automation claim lost during session creation for " + itemIssueKey);
        }
      } catch (startError) {
        if (sess) {
          try {
            if (sm && typeof sm.hideSession === "function") sm.hideSession(sess.localId, null);
          } catch (teardownError) {
            console.error("[auto-launch] could not tear down an unclaimed session:",
        ...
        // The claim was taken for work that never began. Hand it straight back
        // rather than leaving the item pinned until the lease expires, and say
        // so when even that fails.
        var freed = automationGate.releaseClaim(itemIssueKey);
        if (freed && freed.ok !== true) {
          console.error("[auto-launch] could not release claim for " + itemIssueKey +
            " after a failed start (" + freed.reason + "); it stays held until the lease expires");
        }
        throw startError;
      }
      // Stamp the CANONICAL claim key on the session. It cannot be re-derived
      // later from taskLauncher.itemKey: only PR items carry `item.key`
      // (project-task-sources.js), so for issues that field is empty while the
      // claim was taken under the computed "repo#number" key. Without this the
      // completion hook would never release an issue's claim, and the Done
      // workflow would be refused for work this automation legitimately owns.
      if (sess && sess.taskLauncher) {
        sess.taskLauncher.automationClaimKey = itemIssueKey;
        if (sm && typeof sm.saveSessionFile === "function") sm.saveSessionFile(sess);
      }
      started.push(sess);
      activity.record({
        type: "started",
  ...
  async function runScheduled(record) {
    ...
    // Renew the claims of work that is still running BEFORE deciding anything
    // this tick. An auto-launched coding session routinely outlives the lease
    // TTL; without a heartbeat its claim would lapse mid-flight and a later
    // tick could start the same item a second time. Ticks are far more
    // frequent than the TTL, so this keeps a claim alive exactly as long as
    // its work — and releases it the moment the work is gone.
    try {
      reconcileAutomationClaims();
    } catch (e) {
      console.error("[auto-launch] claim reconciliation failed:", e && e.message);
    }
    var recipes = recipeListFrom(cfg);
  ...
  function notifyCompleted(session, summary) {
    ...
    // The work is finished, so the claim is released: the item becomes
    // available again for a future bounce, and no lease outlives its work.
    var claimKey = tl.automationClaimKey || tl.itemKey || tl.prKey;
    if (claimKey) {
      var releasedClaim = automationGate.releaseClaim(claimKey);
      if (releasedClaim && releasedClaim.ok !== true) {
        console.error("[auto-launch] could not release claim for " + claimKey +
          " on completion (" + releasedClaim.reason + ")");
      }
    }
    activity.record({
      type: "completed",
  ...
  // Restart migration. Auto-launched sessions that are still alive keep (and
  // renew) their claim so a restart cannot double-start them; claims whose
  // session is gone are released so the item can be re-proposed instead of
  // staying pinned until the lease lapses.
  function reconcileAutomationClaims() {
    var active = [];
    if (sm && sm.sessions && typeof sm.sessions.forEach === "function") {
      sm.sessions.forEach(function (session) {
        ...
        var key = tl.automationClaimKey || tl.itemKey || tl.prKey;
        if (key) active.push(key);
      });
    }
    return automationGate.reconcileClaims(active);
  }

  return {
    automationGate: automationGate,
    reconcileAutomationClaims: reconcileAutomationClaims,

...
function attachAutoLaunch(ctx) {
  ...
  async function launchScheduled(recipeId, extraArgs) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      // OWNERSHIP. Board work must be assigned to the owner to be picked up
      // automatically, and this check sits OUTSIDE the Coop gate on purpose:
      // the gate short-circuits to legacy behavior when Lead mode is off, and
      // "an unassigned item must never auto-launch" has to hold in both modes.
      //
      // trialview/v2#2539 was unassigned, was picked up anyway, and produced
      // PR #2591. The stamp is set by project-task-sources against a RESOLVED
      // login; a missing stamp means ownership could not be proven, which is

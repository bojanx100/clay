...
function attachAutoLaunch(ctx) {
  ...
  var automationGate = ctx.automationGate || createAutomationGate({
    ...
    emitCandidate: function (candidate) {
      ...
      var stored = candidates.upsert(candidate, refreshOptions);
      if (!stored.ok) {
  ...
  async function launchScheduled(recipeId, extraArgs) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      // gating, because those constraints are about THIS launcher's ability to
      // start a session — and under Lead mode ON it is not going to start one.
      //

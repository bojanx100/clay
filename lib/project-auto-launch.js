...
function attachAutoLaunch(ctx) {
  ...
  // the same item again and duplicate it). So each live legacy auto-launched
  // session is DRAINED: recorded as adopted evidence against its item, so
  // discovery treats the item as already in flight and proposes nothing, while
  // the session finishes on its own.
  //
  ...
  function drainLegacyAutomation() {
    ...
    return {
      ...
      drained: drained, ambiguous: ambiguous, failures: failures,
      completionReconciliation: completionReconciliation,
    };
  }

...
function attachAutoLaunch(ctx) {
  ...
  async function launchScheduled(recipeId, extraArgs) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      // THE FENCE. Before any side effect, commit the launch durably:
      // CLAIMED -> LAUNCHING, carrying the fencing token, the canonical claim
      // key, the actor and the policy digest, and the launch intent. This also
      // re-reads policy, so an owner who tightened it since evaluateLaunch is
      // honored here rather than at a stale decision.
      //
      // A renew-before-start could never be a fence: the process can be
      // suspended past the TTL between the two and the launch still happens.
      // LAUNCHING does not expire, so nothing can license a second launch
      // while this one is unresolved.
      var claimToken = authorized.lease && authorized.lease.token;
      var launchIntent = {
        recipeId: recipe.id,
      ...
      if (authorized.reason !== "lead_mode_off_legacy") {
        var fenced = automationGate.beginLaunch({
          itemKey: itemIssueKey,
          token: claimToken,
          itemClass: authorized.audit && authorized.audit.itemClass,
          intent: launchIntent,
        });
        if (!fenced.ok) {
          // Nothing has happened yet, so give the claim straight back rather
          // than pinning the item for a launch that never began.
          var unfenced = automationGate.releaseClaim(itemIssueKey);
          if (unfenced && unfenced.ok !== true) {
            console.error("[auto-launch] could not release claim for " + itemIssueKey +
              " after a refused launch fence (" + unfenced.reason + ")");
          }
          skipped.push(item);
          activity.record({
      ...
      // if either half fails, the launch state is put back and the claim is
      // released, so nothing is left half-started.
      var sess;
      ...
      try {
        ...
        sess = tl.startSessionForItem(null, recipe, item, itemArgs, null, {
          auto: true,
          // Stamped onto the session BEFORE it is persisted, so a crash can
          // never leave a running session that reconciliation cannot tie back
          // to its claim.
          automationClaimKey: itemIssueKey,
          automationClaimToken: claimToken,
        });
      } catch (startError) {
        try {
        ...
        if (freed && freed.ok !== true) {
          console.error("[auto-launch] could not release claim for " + itemIssueKey +
            " after a failed start (" + freed.reason + "); it stays held until recovery");
        }
        throw startError;
      }
      // LAUNCHING -> RUNNING only once canonical session metadata exists. Until
      // this lands the claim stays in the non-expiring ambiguous state, which
      // recovery resolves by evidence rather than by duplicating the work.
      if (authorized.reason !== "lead_mode_off_legacy") {
        var running = automationGate.confirmRunning({
          itemKey: itemIssueKey,
          token: claimToken,
          session: sess && (sess.storageId || sess.cliSessionId) || null,
        });
        if (!running.ok) {
          console.error("[auto-launch] could not confirm RUNNING for " + itemIssueKey +
            " (" + running.reason + "); recovery will resolve it by evidence");
        }
      }
      started.push(sess);
      activity.record({

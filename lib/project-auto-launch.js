...
function attachAutoLaunch(ctx) {
  ...
  async function launchScheduled(recipeId, extraArgs) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      // Coop gate. Checked AFTER the cheap local filters but BEFORE any state
      // is consumed, so a proposal never burns a PR pass or marks an issue
      // launched. A non-execute verdict means this automation is not allowed
      // to start the work itself: "propose" hands it to Coop, "deny" stops.
      ...
      try {
        ...
        // rather than leaving the item pinned until the lease expires.
        automationGate.releaseClaim(itemIssueKey);
        throw startError;
      }
  ...
  async function runScheduled(record) {
    ...
    try { reconcileAutomationClaims(); } catch (e) {}
    var recipes = recipeListFrom(cfg);
  ...
  function notifyCompleted(session, summary) {
    ...
    if (claimKey) automationGate.releaseClaim(claimKey);
    activity.record({
      type: "completed",

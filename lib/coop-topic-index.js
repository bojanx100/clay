...
// valid reference is.
var projectionDeps = { normalizeExecution: normalizeExecution };
...
function createTopicIndex(options) {
  ...
  function ensureRetro(session, retroOptions) {
    ...
    for (var ti = 0; ti < extracted.turns.length; ti++) {
      ...
      for (var mi = 0; mi < classification.topics.length; mi++) {
        if (mergeRefs(classification.topics[mi], eventRefs)) changed = true;
        if (mergeTurnRefs(classification.topics[mi], [turnRef])) changed = true;
      }
    }
  ...
  function project(options) {
    var index = load(); var groups = {}; var topics = Object.keys(index.topics);
    for (var i = 0; i < topics.length; i++) {
      var topic = index.topics[topics[i]]; var group = normalizeGroup(topic.group);
      ...
      if (options && options.history &&
          !topicRelevance.topicHasRelevantTurn(topic, options.history)) continue;
      ...
      if (options && options.history &&
          !topicAnchors.isProjectable(topic, options.history)) continue;
      if (!group || group.kind === "project" && !topicProjection.canProject(options, group.projectRef)) continue;
      var key = group.kind === "project" ? "project:" + group.projectRef.projectId : group.kind;
      if (!groups[key]) groups[key] = { kind: group.kind, projectRef: group.projectRef || null, topics: [] };
      groups[key].topics.push(topicProjection.publicTopic(topic, options, projectionDeps));
    }
    ...
    return { type: "coop_topic_projection", canonicalSessionRef: index.canonicalSessionStorageId ? { projectId: projectIdentity.LEAD_PROJECT_ID, sessionStorageId: index.canonicalSessionStorageId } : null, groups: list };
  }
  ...
  function classifyCanonicalIngress(session, message, options) {
    ...
    return { ok: true, topicRef: copyRef(classified.topic.topicRef), projectRef: copyRef(inferredProject), created: classified.created };
  }
  ...
  function reconcileTopicAnchors(session) {
    ...
    if (!session || !session.coopHome || !storageId) return { ok: false, code: "canonical_coop_required" };
    var index = load();
    if (index.canonicalSessionStorageId && index.canonicalSessionStorageId !== storageId) {
      return { ok: false, code: "canonical_session_mismatch" };
    }
    var history = Array.isArray(session.history) ? session.history : [];
    var summary = topicAnchors.reconcileAnchors(index, { historyFor: function () { return history; }, now: now });
  ...
  function retrofitTopicTitles(session) {
    ...
    var history = Array.isArray(session.history) ? session.history : [];
    var report = topicRetrofit.retrofitTitles(index, { historyFor: function () { return history; }, now: now });
  ...
  // retrofitTopicTitles above are callable-on-demand and per-topic idempotent,
  // but nothing in the daemon reliably invoked them: the message-ingress hook
  // sits behind routing that genuine owner Coop traffic provably bypassed
  // (owner evidence: a real canonical owner message left zero audits). This
  // runs on the projection path instead -- the one path that demonstrably
  // executes with the daemon's authoritative cached canonical session -- and
  // stamps the index (state.titleRetrofit) so the migration body runs exactly
  // once per schema version, across reloads and restarts, without owner test
  // messages or external file mutation. Fails closed without stamping when
  // the canonical history is unavailable, so an early empty-history call can
  // never suppress every topic and burn the once-only stamp doing it.
  function ensureTitleRetrofit(session) {
    ...
    var history = Array.isArray(session.history) ? session.history : [];
    if (history.length === 0) return { ok: false, code: "canonical_history_unavailable" };
    var reconciled = reconcileTopicAnchors(session);
  ...
  // zero of 23 orchestration tasks carried a coopTopicRef), so their
  // resolution cannot be proven. This records that durably per topic instead
  // of leaving the state blank or guessing Working: linked topics derive live
  // state from their tasks, unlinked open topics get an inspectable
  // "unlinked_historical" needs-input record, and existing dispositions --
  // including explicit owner decisions -- are never overwritten. Fails closed
  // without stamping when the canonical history is unavailable, exactly like
  // the title retrofit, so a partially-loaded session cannot burn the stamp.
  ...
  // One explicit owner decision on one topic. Authority and the stale-state
  // check belong to the connection layer; this resolves, applies and persists.
  function applyTopicDisposition(ref, decision) {
    var result = resolve(ref, true);
    if (!result.ok) return result;
    var applied = topicDisposition.applyOwnerDecision(result.topic, decision, { now: now });
    if (!applied.ok) return applied;
    save();
    return { ok: true, topicRef: result.ref, disposition: applied.disposition };
  }

  return { load: load, save: save, ensureRetro: ensureRetro, reconcileTopicAnchors: reconcileTopicAnchors, retrofitTopicTitles: retrofitTopicTitles, ensureTitleRetrofit: ensureTitleRetrofit, ensureDispositionBackfill: ensureDispositionBackfill, applyTopicDisposition: applyTopicDisposition, rename: rename, move: move, merge: merge, split: split, close: function (ref) { return changeStatus(ref, "closed"); }, reopen: function (ref) { return changeStatus(ref, "open"); }, addEventMembership: addEventMembership, linkExecution: linkExecution, resolveCanonicalEvent: resolveCanonicalEvent, project: project, validateIngress: validateIngress, classifyCanonicalIngress: classifyCanonicalIngress, resolve: resolve, file: file };
}

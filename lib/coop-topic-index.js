...
function canProject(options, ref) {
  if (!ref) return true;
  return !options || typeof options.canAccessProject !== "function" || options.canAccessProject(options.actor, ref) === true;
}

  function visibleExecution(value, options) {
    var execution = normalizeExecution(value, 0);
    if (!execution) return null;
    var ref = execution.projectRef || execution.sessionRef && { projectId: execution.sessionRef.projectId } ||
      execution.taskRef && { projectId: execution.taskRef.projectId };
    if (ref && !canProject(options, ref)) return null;
    if (Array.isArray(execution.children)) {
      execution.children = execution.children.map(function (child) { return visibleExecution(child, options); }).filter(Boolean);
    }
    return execution;
  }
...
function topicProjectionMetadata(topic) {
  var refs = Array.isArray(topic.eventRefs) ? topic.eventRefs : []; var turns = Array.isArray(topic.turnRefs) ? topic.turnRefs : [];
  return {
    topicRef: copyRef(topic.topicRef), title: topic.title, group: copyRef(topic.group), source: topic.source, status: topic.status,
    createdAt: topic.createdAt, updatedAt: topic.updatedAt, eventCount: refs.length, turnCount: turns.length,
    firstEventRef: refs.length ? copyRef(refs[0]) : null, lastEventRef: refs.length ? copyRef(refs[refs.length - 1]) : null,
  };
}

function computedTopicState(topic, options) {
  var computed = options && typeof options.computeTopicState === "function" ? options.computeTopicState(copyRef(topic.topicRef), topicProjectionMetadata(topic)) || {} : {};
    var turnCount = Array.isArray(topic.turnRefs) ? topic.turnRefs.length : 0;
    var activity = cleanProjectionText(computed.currentActivity || computed.activity, 240);
    var summary = cleanProjectionText(computed.rollingSummary || computed.summary, 240) ||
    ...
    var decisions = Array.isArray(computed.decisions) ? computed.decisions.map(function (item) {
      return cleanProjectionText(typeof item === "string" ? item : item && (item.summary || item.title || item.text), 240);
...
function publicTopic(topic, options) {
  var computed = computedTopicState(topic, options); var summaryOnly = !!(options && options.summaryOnly); var metadata = topicProjectionMetadata(topic);
  return {
    ...
    turnRefs: summaryOnly ? [] : copyRef(topic.turnRefs),
      ...
      unreadCount: computed.unreadCount, attention: computed.attention, currentActivity: computed.currentActivity,
      relatedExecutions: topic.relatedExecutions.map(function (item) { return visibleExecution(item, options); }).filter(Boolean),
    };
  }
  ...
  function project(options) {
    ...
    for (var i = 0; i < topics.length; i++) {
      ...
      if (topic.status === "merged") continue;
      ...
      groups[key].topics.push(publicTopic(topic, options));
    }
  ...
  function validateIngress(session, message, options) {
    ...
    var resolved = resolve(ref, false);
  ...
  return { load: load, save: save, ensureRetro: ensureRetro, rename: rename, move: move, merge: merge, split: split, close: function (ref) { return changeStatus(ref, "closed"); }, reopen: function (ref) { return changeStatus(ref, "open"); }, addEventMembership: addEventMembership, linkExecution: linkExecution, resolveCanonicalEvent: resolveCanonicalEvent, project: project, validateIngress: validateIngress, classifyCanonicalIngress: classifyCanonicalIngress, resolve: resolve, file: file };
}

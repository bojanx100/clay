...
function createTopicIndex(options) {
  ...
  function resolveCanonicalEvent(ref, event) {
    ...
    if (!result.ok) return result;
    var allowed = {};
    var eventRefs = Array.isArray(result.topic.eventRefs) ? result.topic.eventRefs : [];
    var turnRefs = Array.isArray(result.topic.turnRefs) ? result.topic.turnRefs : [];
    allowed[storageId] = true;
    for (var ei = 0; ei < eventRefs.length; ei++) if (eventRefs[ei] && eventRefs[ei].sessionStorageId) allowed[eventRefs[ei].sessionStorageId] = true;
    for (var ti = 0; ti < turnRefs.length; ti++) if (turnRefs[ti] && turnRefs[ti].sessionStorageId) allowed[turnRefs[ti].sessionStorageId] = true;

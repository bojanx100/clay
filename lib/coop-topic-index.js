...
var threadClosureRepair = require("./coop-thread-closure-repair");
...
function createTopicIndex(options) {
  ...
  // Repairs records left with status "closed" and a live threadState by a close
  // that wrote status alone. Read-only when nothing is damaged, so it is safe to
  // re-run; see coop-thread-closure-repair.healClosedThreadStates for the rules.
  function healClosedThreadStates(options) {
    var state = load();
    var report = threadClosureRepair.healClosedThreadStates(state,
  ...
  function linkPlanning(ref, value) {
    var result = resolve(ref, false);
    var normalized = normalizeExecution(value, 0);
    if (!result.ok) return result;
    if (!normalized || !normalized.sessionRef || normalized.sessionRef.projectId !== "system-lead" ||
        normalized.taskRef || normalized.children) return { ok: false, code: "invalid_planning_ref" };
    if (!Array.isArray(result.topic.relatedPlanning)) result.topic.relatedPlanning = [];
    var key = JSON.stringify(normalized);
    if (result.topic.relatedPlanning.some(function (item) { return JSON.stringify(item) === key; })) {
      return { ok: true, unchanged: true };
    }
    result.topic.relatedPlanning.push(normalized);
    result.topic.updatedAt = now(); save(); return { ok: true };
  }
  ...
  return {
    load: load,
    linkPlanning: function (ref, value) { return mutate({ ok: false, code: "persistence_failed" }, function () {
      return linkPlanning(ref, value);
    }); },
    save: save,
    ...
    healClosedThreadStates: function (options) { return mutate({ ok: false, code: "persistence_failed" }, function () { return healClosedThreadStates(options); }); },

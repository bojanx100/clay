...
function initialState() {
  return { schemaVersion: SCHEMA_VERSION, canonicalSessionStorageId: null, topics: {},
    threadLifecycleVersion: 1, threadCorrections: [], retro: { version: RETRO_VERSION, completedEventCount: 0 } };
}
...
function createTopicIndex(options) {
  ...
  function ensureRetro(session, retroOptions) {
    ...
    var fromEvent = !retroChanged && index.retro.version === RETRO_VERSION && Number.isInteger(index.retro.completedEventCount) &&
  ...
  function classifyCanonicalIngress(session, message, options) {
    ...
    if (classified.created || lifecycleChanged) save();

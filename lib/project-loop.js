...
var schedulerServiceModule = require("./project-scheduler-service");
...
function attachLoop(ctx) {
  ...
  function triggerFromQueue(record, triggerKind) {
    ...
    if (record.source === "schedule") {
      if (!record.linkedTaskId) {
        ...
        return { ok: false, code: "missing_linked_task", message: "Schedule has no linked task." };
      }
    ...
    try {
      ...
      return { ok: false, code: "missing_prompt", message: "PROMPT.md is missing for " + loopFilesId + "." };
    }
    ...
    if (triggerKind !== "manual") send({ type: "schedule_run_started", recordId: record.id });
    startLoop({ maxIterations: record.maxIterations, name: record.name });
    return { ok: true, started: true, recordId: record.id, loopFilesId: loopFilesId };
  }

  function runRecordNow(id) {
    if (loopState.active || loopState.phase === "executing" || loopState.phase === "crafting") {
      return { ok: false, code: "loop_active", message: "Another loop is already active in this project." };
    }
    var record = loopRegistry.getById(id);
    if (!record) return { ok: false, code: "not_found", message: "Schedule not found in this project." };
    var result = triggerFromQueue(record, "manual");
    if (result.ok) send({ type: "loop_rerun_started", recordId: record.id });
    return result;
  }
  ...
  var loopRegistry = createLoopRegistry({
    ...
    onTrigger: function (record) {
      ...
      triggerFromQueue(record, "scheduled");
    },
    onChange: function () {
  ...
  function finishLoop(reason) {
    ...
    if (pendingTriggers.length > 0) {
      ...
      setTimeout(function () {
        triggerFromQueue(next, "scheduled");
      }, 1000);
    }
  }
  ...
  var schedulerService = schedulerServiceModule.attachProjectScheduler({
    cwd: cwd,
    fs: fs,
    path: path,
    loopRegistry: loopRegistry,
    runRecordNow: runRecordNow,
  });

...
// Parallel instances can keep their recent-project inventory separate too.
var CLAYRC_PATH = process.env.CLAY_RC_PATH
  ? path.resolve(process.env.CLAY_RC_PATH) : path.join(REAL_HOME, ".clayrc");
...
// Dedicated, structured record of auto-recovery events (watchdog stalls,
// transient socket-close retries). Kept separate from the noisy daemon log so
// the recovery history is cheap to read and tail on demand.
function recoveryLogPath() {
  return path.join(CONFIG_DIR, _devMode ? "recovery-events-dev.log" : "recovery-events.log");
}

// Dedicated diagnostics log for the reconnect/"crash" investigation
// (event-loop stalls, slow full-history saves). Separate, tailable file so the
// signal isn't buried in the noisy inherited-stdout dev log.
function diagLogPath() {
  return path.join(CONFIG_DIR, _devMode ? "diag-dev.log" : "diag.log");
}
...
function saveConfig(config) {
  // Lead mode is server-authoritative and can change through a live WebSocket
  // while daemon-owned settings still hold an older in-memory config object.
  // Preserve the durable Coop section on those unrelated writes.
  if (!config.coop) {
    var current = loadConfig();
    if (current && current.coop) config.coop = current.coop;
  }
  ensureConfigDir();
...
// Coop control-kernel activation is durable daemon configuration. When this
// section exists it deliberately overrides inherited shell state, including
// explicit false values used for rollback.
//
// store/executions/recovery activate together; partial activation of those
// three is unsupported. handoffTrigger is a narrower fourth switch layered on
// top of them: the kernel being live must never by itself arm automatic Class B
// handoffs, so an activated kernel with no handoffTrigger key projects "0".
// Every name here is stripped from the hermetic test pass and projected from
// daemon config, so a switch omitted from this map is one the owner cannot turn
// on durably and one a developer shell can leak into `npm test`.
var COOP_CONTROL_ENVIRONMENT = Object.freeze({
  store: "CLAY_COOP_CONTROL_STORE",
  executions: "CLAY_COOP_CONTROL_EXECUTIONS",
  recovery: "CLAY_COOP_CONTROL_RECOVERY",
  handoffTrigger: "CLAY_COOP_HANDOFF_TRIGGER",
});

function controlKernelConfig(config) {
  var coop = config && config.coop;
  var kernel = coop && coop.controlKernel;
  if (!kernel || typeof kernel !== "object" || Array.isArray(kernel)) return null;
  return kernel;
}

function coopControlEnvironment(config, baseEnvironment) {
  var environment = Object.assign({}, baseEnvironment || process.env);
  var kernel = controlKernelConfig(config);
  if (!kernel) return environment;
  var names = Object.keys(COOP_CONTROL_ENVIRONMENT);
  for (var i = 0; i < names.length; i++) {
    var name = names[i];
    environment[COOP_CONTROL_ENVIRONMENT[name]] = kernel[name] === true ? "1" : "0";
  }
  return environment;
}

function applyCoopControlEnvironment(config, environment) {
  var target = environment || process.env;
  var kernel = controlKernelConfig(config);
  if (!kernel) return target;
  var names = Object.keys(COOP_CONTROL_ENVIRONMENT);
  for (var i = 0; i < names.length; i++) {
    var name = names[i];
    target[COOP_CONTROL_ENVIRONMENT[name]] = kernel[name] === true ? "1" : "0";
  }
  return target;
}

// Does a process with this pid exist? Existence only, never ownership.
// `process.kill(pid, 0)` reports ESRCH when no process exists and EPERM when
// the process exists but belongs to another user. Unknown errors fail
// conservatively toward "alive" because every caller gates cleanup or a
// duplicate daemon launch on this answer.
function isPidAlive(pid) {
  // process.kill accepts zero and negative values as process-group selectors,
  // so reject everything except a positive integer before asking the kernel.
...
function saveClayrc(rc) {
  fs.mkdirSync(path.dirname(CLAYRC_PATH), { recursive: true });
...
module.exports = {
  ...
  COOP_CONTROL_ENVIRONMENT: COOP_CONTROL_ENVIRONMENT,
  coopControlEnvironment: coopControlEnvironment,
  applyCoopControlEnvironment: applyCoopControlEnvironment,

...
var { isContextOverflowError, isConversationImageError } = require("./sdk-bridge-stream");
...
var controlPlaneBinding = require("./project-session-compaction").controlPlaneBinding;

function hasPositiveTokenUsage(usage) {
  ...
  for (var i = 0; i < fields.length; i++) {
    if (typeof usage[fields[i]] === "number" && usage[fields[i]] > 0) return true;
  }
  return false;
}
...
function attachMessageProcessor(ctx) {
  ...
  function recoverClaudeImageHistory(session, text) {
    if (session.vendor !== "claude" || !isConversationImageError(text)) return false;
    console.warn("[sdk-bridge] unusable Claude image history for session " + session.localId);
    var canCompact = typeof opts.compactAndContinue === "function" &&
      !session._claudeImageCompactionQueued && !session.compactedFromLocalId &&
      !session.compactedIntoLocalId && (session.compactionDepth || 0) < 1;
    var continuation = null;
    if (canCompact) {
      session._claudeImageCompactionQueued = true;
      try {
        continuation = opts.compactAndContinue(session, { reason: "claude_image_history" });
      } catch (error) {
        console.error("[sdk-bridge] Claude image-history compaction failed for session " +
          session.localId + ": " + (error.message || error));
        sendAndRecord(session, { type: "error",
          text: "Clay could not move this conversation past Claude's invalid image history. Start a fresh session and paste the relevant context." });
      }
    } else {
      sendAndRecord(session, { type: "error",
        text: "Claude's native conversation history contains an image the API can no longer process. Start a fresh or compacted continuation." });
    }
    sendAndRecord(session, { type: "done", code: continuation ? 0 : 1 });
  ...
  // A single tool_result can be enormous — a grep whose matches include session
  // .jsonl files produced a 130MB result here. Recording it verbatim bloats the
  // history file, and every daemon start re-parses it into the heap, which is
  // what OOM-crashed the daemon in a restart loop. The provider already has its
  // own copy for model context; this entry is only replayed to the UI, so a
  // bounded prefix is all it ever needs to carry.
  var MAX_TOOL_RESULT_CHARS = 256 * 1024;

  function capToolResultContent(content) {
    var text = typeof content === "string" ? content : "";
    if (text.length <= MAX_TOOL_RESULT_CHARS) return text;
    return text.slice(0, MAX_TOOL_RESULT_CHARS) + "\n\n[Clay truncated " +
      (text.length - MAX_TOOL_RESULT_CHARS) + " characters of tool output]";
  }
  ...
  function processSDKMessage(session, parsed) {
    ...
    // Per-turn timing. Warm query handles reset these fields before every push.
    if (session._queryStartTs) {
      var _elapsed = Date.now() - session._queryStartTs;
      var _perfPrefix = "[PERF] session=" + session.localId + " turn=" +
        (session._turnPerfId || "unknown") + " ";
      if (parsed.yokeType === "init") {
        console.log(_perfPrefix + "provider_init +" + _elapsed + "ms");
      }
      if (parsed.yokeType === "turn_start") {
        console.log(_perfPrefix + "provider_turn_start +" + _elapsed + "ms");
      }
      if ((parsed.yokeType === "text_delta" || parsed.yokeType === "tool_input_delta" ||
          parsed.yokeType === "thinking_delta") && !session._firstActivityLogged) {
        session._firstActivityLogged = true;
        console.log(_perfPrefix + "first_model_activity +" + _elapsed + "ms type=" + parsed.yokeType);
      }
      if (parsed.yokeType === "text_delta" && !session._firstTextLogged) {
        session._firstTextLogged = true;
        console.log(_perfPrefix + "first_visible_text +" + _elapsed + "ms");
      }
      if (parsed.yokeType === "result") {
        console.log(_perfPrefix + "result +" + _elapsed + "ms");
      }
    }
    ...
    if (parsed.yokeType === "init") {
      ...
      if (parsed.model) {
        ...
        sendToSession(session, {
          ...
          providerRoutes: sm.providerRoutes || listProviderRoutes(sm.availableVendors || [], sm.installedVendors || [], sm),
        });
      }
    ...
    if (parsed.yokeType === "turn_start") {
      ...
      // A late rate_limit event can arrive after the result event. Remember
      // whether this turn actually finished so that event cannot wake an idle,
      // already-completed session with a synthetic "continue" turn.
      session._lastTurnCompletedProductively = false;
      ...
      var toolResultEntry = {
        type: "tool_result",
        id: parsed.toolId,
        content: capToolResultContent(parsed.content),
        ...
        for (var i = 0; i < content.length; i++) {
          ...
          if (block.type === "tool_result" && !session.sentToolResults[block.tool_use_id]) {
            ...
            var toolResultMsg = {
              ...
              content: capToolResultContent(resultText),
      ...
      if (isContextOverflowError(Array.isArray(parsed.errors) ? parsed.errors.join("; ") : String(parsed.errors || ""))) {
        require("./sdk-context-recovery").mark({ sm: sm, sendAndRecord: sendAndRecord }, session);
      ...
      if (isZeroCost && recoverClaudeImageHistory(session, previewTrimmed)) return;
      ...
      // Codex reports normalized token usage but no dollar cost. Treat either
      // signal as billable usage so a real Codex response can recover route
      // health and reset the session-local retry budget. Activity is still
      // required, so an empty turn with input-token accounting is not success.
      var turnCompletedProductively = !session.providerFailoverPending &&
        session._turnSawActivity && (!isZeroCost || hasPositiveTokenUsage(parsed.usage));
      session._lastTurnCompletedProductively = turnCompletedProductively;
      if (turnCompletedProductively) require("./sdk-context-recovery").productive(session, sm);
      var productiveTurnStateChanged = false;
      if (turnCompletedProductively && session.rateLimitUseCreditsPending) {
        // The provider reported an overage-capable rejection during a turn that
        // nevertheless completed with real output and billable usage. There is
        // no unfinished work to continue. Leaving these flags set makes stream
        // finalization append another synthetic turn; short replies such as
        // "Holding." then reset the retry budget and repeat indefinitely.
        session.rateLimitAutoContinuePending = false;
        session.rateLimitUseCreditsPending = false;
        session.rateLimitResetsAt = null;
        productiveTurnStateChanged = true;
        console.log("[sdk-bridge] Usage-credit continue cancelled after productive turn for session " + session.localId);
      }
      ...
      if (!session._turnSawActivity && isZeroCost && !isLoginPrompt) {
        ...
        // A Coop-controlled execution is fenced to one exact session identity,
        // so compaction refuses it (see project-session-compaction.js). Ask the
        // same question here rather than announcing a compaction that cannot
        // happen: the else-branch below already reports the wedged provider
        // truthfully, and a "Clay is compacting..." notice followed by a
        // refusal is exactly the misleading-report shape to avoid.
      ...
      if (turnCompletedProductively && session._consecutiveAutoResumes) {
        session._consecutiveAutoResumes = 0;
        session._resumeGaveUpNotified = false;
        productiveTurnStateChanged = true;
      }
      if (productiveTurnStateChanged) sm.saveSessionFile(session);
      ...
      if (turnCompletedProductively && (session.handoffContext || session._handoffContextStash)) {
        if (handoffContextModule.finalizeHandoffAfterSuccessfulTurn(session)) {
          sm.saveSessionFile(session);
        }
      }
      // Per-provider health: a turn that produced real activity with positive
      // cost or normalized token usage is the canonical clean completion. An
      // empty result still cannot recover a provider route.
      if (turnCompletedProductively) {
          ...
          if (!session.scheduledMessage && !session.destroying) {
            ...
            if (acEnabled) {
              if (info.isUsingOverage) {
                session.rateLimitResetsAt = null;
                if (!session.isProcessing && session._lastTurnCompletedProductively) {
                  // The result reached us first and already completed the work.
                  // A trailing account-status event must not manufacture a new
                  // prompt for an otherwise idle session.
                  session.rateLimitAutoContinuePending = false;
                  session.rateLimitUseCreditsPending = false;
                  console.log("[sdk-bridge] Late usage-credit continue suppressed after productive turn for session " + session.localId);
                } else {
                  // Extra usage available: continue after this rejected turn
                  // ends, without creating a scheduled-message bubble.

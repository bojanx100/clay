...
var { queuePendingProviderFailover } = require("./sdk-provider-failover-signals");
...
function attachMessageProcessor(ctx) {
  ...
  function isMonthlySpendLimitText(text) {
    ...
    return lower.indexOf("monthly spend limit") !== -1
      && (lower.indexOf("usage-credits") !== -1 || lower.indexOf("admin-settings/usage") !== -1);
  }
  ...
  function handleMonthlySpendLimit(session, text) {
    ...
    providerHealth.recordFailure(exhaustedVendor, "usage-credits-exhausted", { immediate: true });
  ...
  function processSDKMessage(session, parsed) {
    ...
    if (parsed.yokeType === "turn_start") {
      ...
      if (!session.providerFailoverPending && session._turnSawActivity && !isZeroCost && session.handoffContext) {

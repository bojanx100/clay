...
function attachMessageProcessor(ctx) {
  ...
  function handleMonthlySpendLimit(session, text) {
    if (!isMonthlySpendLimitText(text)) return false;
    var exhaustedVendor = session.vendor || (adapter && adapter.vendor) || "claude";
    var resetsAt = session.rateLimitResetsAt || session.rateLimitLastResetsAt ||
    ...
    session.providerFailoverPending = {
      vendor: exhaustedVendor,
      reason: "usage-credits-exhausted",
      resetsAt: resetsAt,
    };
    providerHealth.recordFailure(exhaustedVendor, "usage-credits-exhausted", {
      immediate: true,
    ...
    if (!session.monthlySpendLimitNotified) {
      session.monthlySpendLimitNotified = true;
      sendAndRecord(session, {
        type: "info",
        text: "Claude usage credits are exhausted for this organization. Run /usage-credits in Claude or ask an admin to raise the monthly spend limit before continuing with Claude.",
        variant: "warning",
      });
    }
    return true;
  ...
  function emitAuthRequired(session) {
    ...
    // Guard against false positives before triggering the client auto-login.
    // The codex app-server can surface a transient/stale "unauthorized" while
    // auth.json is actually valid. Auto-running `codex login --device-auth`
    // DELETES auth.json at startup, so a false auth_required logs the user out
    // for real. Force a fresh `<vendor> login status` check and suppress if
    // the vendor is in fact logged in.
    try {
      var yoke = require("./yoke");
      yoke.invalidateAuthCache();
      var authNow = yoke.checkAuth();
  ...
  function processSDKMessage(session, parsed) {
    ...
    if (parsed.yokeType === "turn_start") {
      ...
      if (parsed.yokeType === "text_delta" && typeof parsed.text === "string") {
        if (handleMonthlySpendLimit(session, parsed.text)) return;
      ...
      if (handleMonthlySpendLimit(session, parsed.content || "")) return;
      var toolResultEntry = {
        type: "tool_result",
        id: parsed.toolId,
      ...
      if (parsed.messageRole === "assistant" && !session.streamedText && Array.isArray(content)) {
        var assistantText = content
          ...
          .join("");
        if (assistantText) {
          if (handleMonthlySpendLimit(session, assistantText)) return;
          session._turnSawActivity = true;
          if (session.responsePreview.length < 200) {
      ...
      if (parsed.subtype === "error_during_execution") {
        ...
        if (handleMonthlySpendLimit(session, execError)) {
          sendAndRecord(session, { type: "done", code: 1 });

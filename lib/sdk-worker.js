...
async function handleSetEffort(msg) {
  if (!queryInstance || !msg.effort) return;
  try {
    // The SDK query has no setEffort method; live effort changes go through
    // the flag-settings layer. Keep this mapping in sync with
    // effortFlagSettings() in yoke/adapters/claude.js: "ultracode" is a
    // pseudo level (xhigh effort + standing dynamic-workflow orchestration),
    // and the settings schema has no "max" level so xhigh is pinned until
    // the next query picks up the real max via the effort option.
    if (typeof queryInstance.applyFlagSettings === "function") {
      var isUltracode = msg.effort === "ultracode";
      var effortLevel = (isUltracode || msg.effort === "max") ? "xhigh" : msg.effort;
      await queryInstance.applyFlagSettings({ ultracode: isUltracode ? true : null, effortLevel: effortLevel });
    } else if (typeof queryInstance.setEffort === "function") {
...
async function handleWarmup(msg) {
  ...
  // abort. Without this every warmup leaves a one-line ("hi") session file
  // in ~/.claude/projects/<encodedCwd>/ that gets adopted into the project
  // sidebar on restart.

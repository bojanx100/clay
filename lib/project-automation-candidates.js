...
// The launcher state is the canonical answer to "has this issue already been
// handled?". Both the project launcher and Lead need the same answer: a state
// entry blocks re-staffing unless the launcher explicitly armed one bounce back
// to a ready status. Keep this small decision here, next to the durable
// candidate handoff, so a Lead scan cannot turn a completed candidate into a
// fresh staffing decision by reimplementing the rule differently.
function completionEligibility(issueLaunchState, itemKey) {
  if (!issueLaunchState || typeof issueLaunchState.hasEntry !== "function" ||
      typeof issueLaunchState.shouldRelaunch !== "function") {
    return { ok: false, eligible: false, reason: "completion_state_unresolvable" };
  }
  try {
    if (!issueLaunchState.hasEntry(itemKey)) {
      return { ok: true, eligible: true, reason: "not_previously_launched" };
    }
    if (issueLaunchState.shouldRelaunch(itemKey)) {
...
function createCandidateStore(options) {
  ...
  // candidate admissible exactly once after a fresh eligible scan (admission
  // becomes auto, status owner_approved, attention and old scan evidence are
  // cleared). A NO is terminal: attention is cleared and the item is never
  // re-prompted, which is the difference between "declined" and "still waiting".

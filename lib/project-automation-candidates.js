...
function createCandidateStore(options) {
  ...
  function upsert(candidate, options) {
    ...
    if (options && Object.prototype.hasOwnProperty.call(options, "bindingSnapshot")) {
      var reconciled = candidateReconciliation.reconcile(existing, options.bindingSnapshot, {
        nextPrimitiveAttempt: options.allowCompletedPrimitive === true && record.intent &&
          record.intent.primitiveLaunch === true ? record.eligibilityPass : null,
      });
  ...
  function adoptRunningLegacy(projectRef, itemKey, evidence) {
    ...
    var existing = state.candidates[index];
    // Startup migration cannot erase an explicit, exact-session adoption retry.
    var primitiveRepair = require("./project-automation-primitive-reconsideration");
    var retry = primitiveRepair.normalize(existing.reconsideration);
    var preparedPrimitive = existing.status === "pending" && existing.intent &&
      existing.intent.primitiveLaunch === true && retry &&
      retry.primitiveLaunch.itemKey === existing.itemKey &&
      retry.primitiveLaunch.sessionRef.projectId === ref.projectId && evidence &&
      retry.primitiveLaunch.sessionRef.sessionStorageId === evidence.sessionStorageId;
    // Already admitted through Coop: that binding wins, do not rewrite it.
    if (existing.status === "admitted" || preparedPrimitive) {

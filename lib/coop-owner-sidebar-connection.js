...
function handleOwnerSidebarMessage(ctx, ws, msg) {
  if (!msg || msg.type !== "coop_owner_sidebar_prioritize") return false;
  function reply(payload) {
    ctx.sendTo(ws, Object.assign({ type: "coop_owner_sidebar_priority_result" }, payload));
  }
  ...
  var current = projection && projection.ownerSidebar && Number(projection.ownerSidebar.priorityRevision) || 0;
  ...
  var result = priority.applyPriority(msg.topicRef, msg.direction, priorityCandidates(projection), priorityOptions(ctx));
  if (!result.ok) {
    reply({ ok: false, code: result.code });
    return true;
  }
  reply({ ok: true, changed: !!result.changed, priorityRevision: result.priority.revision });

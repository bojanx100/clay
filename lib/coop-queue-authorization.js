...
function absoluteHistoryIndex(history, sessionStorageId, eventIndex) {
  var resolved = lineage.recordAt(history, sessionStorageId || "", eventIndex);
  return resolved && Number.isInteger(resolved.absoluteIndex) ? resolved.absoluteIndex : null;
}
...
function authorizationEventForTopic(topic, history) {
  ...
  for (var i = 0; i < refs.length; i++) {
    var ref = refs[i] || {};
    add(absoluteHistoryIndex(history, ref.sessionStorageId || "", ref.eventIndex));
    ...
    add(absoluteHistoryIndex(history, ref.sessionStorageId || "", ref.eventIndex + 1));
  }
  for (var j = 0; j < turns.length; j++) {
    var turn = turns[j] || {};
    add(absoluteHistoryIndex(history, turn.sessionStorageId || "", turn.startEventIndex));
    add(absoluteHistoryIndex(history, turn.sessionStorageId || "", turn.startEventIndex + 1));
  }

  ...
  function handleRecordsMessage(ws, msg) {
    ...
    if (msg.type === "bulk_delete_sessions") {
      ...
      for (var di = 0; di < msg.sessionIds.length; di++) {
        ...
        if (bulkTarget.coopHome) continue;
    ...
    if (msg.type === "delete_session") {
      ...
      if (msg.id && sm.sessions.has(msg.id)) {
        ...
        if (dsTarget && dsTarget.coopHome) {
          sendTo(ws, { type: "error", text: "The Coop home is permanent and cannot be deleted" });
          return true;
        }
    ...
    if (msg.type === "hide_session") {
      if (msg.id && sm.sessions.has(msg.id)) {
        ...
        if (hideTarget.coopHome) {
          sendTo(ws, { type: "error", text: "The Coop home is permanent and cannot be hidden" });
          return true;
        }

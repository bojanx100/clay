function attachProjectSessionsRecords(ctx) {
  ...
  function closeCoordinatorWorkers(session) {
    var tasks = activeOrchestrationTasks(session);
  ...
  function handleRecordsMessage(ws, msg) {
    ...
    if (msg.type === "hide_session") {
      if (msg.id && sm.sessions.has(msg.id)) {
        ...
        var activeCoordinatorTasks = activeOrchestrationTasks(hideTarget);
        if (activeCoordinatorTasks.length > 0 && msg.closeWorkers !== true) {
          sendTo(ws, {
            type: "coordinator_close_required",
            id: hideTarget.localId,
            title: hideTarget.title || "Coordinator",
            activeWorkerCount: activeCoordinatorTasks.length,
          });
          return true;
        }
        if (activeCoordinatorTasks.length > 0) closeCoordinatorWorkers(hideTarget);

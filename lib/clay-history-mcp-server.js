    ...
    handler: function (args) {
      try {
        ...
        for (var i = 0; i < slice.length; i++) {
          var line = formatHistoryEntry(slice[i]);
...
// Render one persisted history entry as a single transcript line, or null for
// entry types read_session does not surface.
function formatHistoryEntry(entry) {
  ...
  if (entry.type === "user_message") {
    ...
    // A recorded tool_result carries { id, content, is_error } and never
    // `name`/`input`. Rendering it through the tool_executing branch produced
    // an empty "[TOOL] " line for every result in the transcript.
    label = entry.is_error ? "TOOL ERROR" : "TOOL RESULT";
...
module.exports = { getToolDefs: getToolDefs, formatHistoryEntry: formatHistoryEntry };

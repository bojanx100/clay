function attachProjectSessionsSearch(ctx) {
  ...
  function handleSearchMessage(ws, msg) {
    ...
    if (msg.type === "list_cli_sessions") {
      ...
      var includeCoopManaged = msg.includeCoopManaged === true;
      var adoptable = sm.listAdoptableCliSessions(importVendor, {
        includeCoopManaged: includeCoopManaged,
      });
      sendTo(ws, {
    ...
    if (msg.type === "import_cli_session") {
      if (msg.cliSessionId) {
        // Importing a Coop-owned session hands it back to whoever asked, so the
        // requester is the owner to stamp when the session has none yet.

...
function pickVendor(preference, health, tier) {
  for (var vi = 0; vi < preference.length; vi++) {
    var vendor = preference[vi];
    if ((health[vendor] || "healthy") === "unhealthy") continue;
    var model = MODEL_TABLE[vendor] && MODEL_TABLE[vendor][tier];
    if (!model) continue;
    return { vendor: vendor, model: model, fallback: vi > 0 };
  }
  return null;
}
...
function routeWorkItem(classification, opts) {
  ...
  var pick = pickVendor(preference, health, tier);
  if (!pick) return null;
  return {
    vendor: pick.vendor,
    model: pick.model,
    ...
    rationale: taskClass + "/" + risk +
      ...
      " -> tier " + tier + " on " + pick.vendor +
      (pick.fallback ? " (preferred vendor unavailable)" : ""),
  };
}

...
// availability are INJECTED by the caller (the Lead reads them from
// provider-health.js at decision time).
...
var HIGH_RISK_RE = /\b(migration|schema|database|auth|security|permission|daemon|restart|payment|billing|public api|breaking|irreversible|delete (data|user)|drop table|production)\b/i;
...
function classifyWorkItem(item) {
  ...
  for (var li = 0; li < labels.length; li++) {
    var label = String(labels[li] || "").toLowerCase();
    if (label === "bug") taskClass = "debugging";
    else if (label === "security") taskClass = "security";
    else if (label === "design" || label === "architecture") taskClass = "design";
    else if (label === "chore" || label === "mechanical") taskClass = "mechanical";
    else if (label === "research" || label === "spike") taskClass = "research";
    if (taskClass) break;
  }
  if (!taskClass) {
    for (var ci = 0; ci < CLASS_RULES.length; ci++) {
      if (CLASS_RULES[ci].re.test(text)) { taskClass = CLASS_RULES[ci].taskClass; break; }
    }
  }
  if (!taskClass) taskClass = "implementation";

  var risk = HIGH_RISK_RE.test(text) ? "high"
    : (MEDIUM_RISK_RE.test(text) ? "medium" : "low");
...
function routeWorkItem(classification, opts) {
  ...
  var escalated = (opts && opts.escalated) || 0;
  ...
  for (var vi = 0; vi < preference.length; vi++) {
    var vendor = preference[vi];
    var state = health[vendor] || "healthy";
    if (state === "unhealthy") continue;
    ...
    return {
      ...
      rationale: taskClass + "/" + risk +
        (escalated ? " (escalated x" + escalated + ")" : "") +
        " -> tier " + tier + " on " + vendor +
        (vi > 0 ? " (preferred vendor unavailable)" : ""),
    };
  }
  return null;
}

...
function completedOwnerAcceptanceWait(session, metadata) {
  ...
  if (metadata.ownerAcceptanceRequired !== true || !acceptance ||
      acceptance.status !== "pending") return false;
  if (completion.status !== "completed" ||
      !/^yes\b/i.test(String(completion.integrationVerification || "").trim()) ||
      !/^no\b/i.test(String(completion.escalationRequired || "").trim())) return false;
  if (!Number.isFinite(completion.completedAt) || completion.completedAt <= 0) return false;
  return completion.portfolioTaskId === metadata.portfolioTaskId &&
    completion.bindingRevision === metadata.bindingRevision &&
    metadata.implementationCompletedAt === completion.completedAt &&
    metadata.implementationCompletionRevision === completion.completionRevision &&
    metadata.implementationGraphDigest === completion.graphDigest;
}

...
function pendingOwnerAcceptance(metadata) {
  var acceptance = metadata && metadata.ownerAcceptance;
  if (!metadata || metadata.ownerAcceptanceRequired !== true || !acceptance) return false;
  return acceptance.status === "pending";
}

function completedProjectEvidence(session) {
  var completion = session && session.orchestrationProjectCompletion;
  if (!completion || completion.status !== "completed") return null;
  if (!/^yes\b/i.test(String(completion.integrationVerification || "").trim())) return null;
  if (!/^no\b/i.test(String(completion.escalationRequired || "").trim())) return null;
  if (!Number.isFinite(completion.completedAt) || completion.completedAt <= 0) return null;
  return completion;
}

function matchesImplementationCompletion(metadata, completion) {
  if (completion.portfolioTaskId !== metadata.portfolioTaskId ||
      completion.bindingRevision !== metadata.bindingRevision) return false;
  if (metadata.implementationCompletedAt !== completion.completedAt) return false;
  if (metadata.implementationCompletionRevision !== completion.completionRevision) return false;
  return metadata.implementationGraphDigest === completion.graphDigest;
}

function completedOwnerAcceptanceWait(session, metadata) {
  var completion = completedProjectEvidence(session);
  if (!completion || !metadata || metadata.mode !== "project_coordinator") return false;
  if (metadata.status !== "failed" || metadata.reason !== "restart_recovery") return false;
  return pendingOwnerAcceptance(metadata) && matchesImplementationCompletion(metadata, completion);
}

function restoreOwnerAcceptanceWait(session, metadata, durable, sameIncarnation) {
  if (!sameIncarnation || !durable || !durable.current ||
      durable.current.failureCode !== "restart_recovery") return false;
  if (!completedOwnerAcceptanceWait(session, metadata)) return false;
  ...
  delete metadata.failureDetails;
  session.isProcessing = false;
  return true;
}
...
function createTargetExecutionControl(ctx) {
  ...
  function reconcileSession(session, metadata) {
    ...
    if (settledReview.receiptMatchesDurable(session, metadata, durable)) return false;
    var settledReceipt = settledReview.createReceipt(session, metadata, durable, Date.now);
    if (settledReceipt) {
      applyDurableTerminal(session, metadata, durable, sameIncarnation);
      metadata.settledReadOnlyReviewReconciliation = settledReceipt;
      return true;
    }
    // Older owner-acceptance waits left a live lease behind. Startup recovery
    // correctly quarantined that lease, but it also made the already-verified
    // coordinator impossible to message. Restore only an exact, same-revision
    // completion record; all partial or mismatched failures remain terminal.
    if (restoreOwnerAcceptanceWait(session, metadata, durable, sameIncarnation)) return true;

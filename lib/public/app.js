...
import { initTheme, getThemeColor, getComputedVar, onThemeChange, getCurrentTheme, getChatLayout, setChatLayout } from './modules/theme.js';
...
import { initTaskWizard } from './modules/project-task-wizard.js';
...
import { initVoiceConversation } from './modules/voice-conversation.js';
...
import { coopHeaderTitle, isCoopProjectSlug } from './modules/coop-identity.js';
...
import { initLiveUiExtensionPicker } from './modules/live-ui-extension-picker.js';
...
import { initConnection, connect as _connConnect, setStatus as _connSetStatus, scheduleReconnect as _connScheduleReconnect, cancelReconnect as _connCancelReconnect, sendUserAction as _connSendUserAction } from './modules/app-connection.js';
...
import { initMisc, flushPendingExtMessages, showImageModal as _miscShowImageModal, closeImageModal as _miscCloseImageModal, showPasteModal as _miscShowPasteModal, closePasteModal as _miscClosePasteModal, showForceChangePinOverlay as _miscShowForceChangePinOverlay, sendExtensionCommand as _miscSendExtensionCommand, handleExtensionResult as _miscHandleExtensionResult } from './modules/app-misc.js';
import { showConfirm as _miscShowConfirm, hideConfirm as _miscHideConfirm } from './modules/confirm-modal.js';
...
import { initCoopIncarnationControls } from './modules/coop-incarnation-controls.js';
import { initHumanAttention } from './modules/human-attention.js';
...
// --- DOM refs ---
  ...
  // "Open in new tab" navigates natively (target=_blank); also dismiss the modal.
  $("image-modal").querySelector(".image-modal-open").addEventListener("click", closeImageModal);
  ...
  try {
    ...
    // This paints before the socket connects, so a cache written by an older
    // build -- when the Coop project was still named "Lead" -- would show a Lead
    // identity on every cold start and every restart until `info` arrived.
    // Resolve through the Coop rule rather than trusting the cached string.
    if (_cachedProjectName || isCoopProjectSlug(currentSlug)) {
      projectName = coopHeaderTitle(currentSlug, _cachedProjectName, _cachedProjectName);
      if (headerTitleEl) headerTitleEl.textContent = projectName;
      var _tbp = $("title-bar-project-name");
      if (_tbp) _tbp.textContent = projectName;
    }
  ...
  createStore({
    ...
    wsPath: wsPath,
    dmMode: false,
    // heartbeat / liveness (see app-connection.js)
    lastPongAt: 0,
    ...
    liveUiState: "idle",
    liveUiPairingId: null,
    liveUiReconnectCredential: null,
    liveUiSelection: null,
    liveUiError: null,
  activeCoordinationMode: false,
  activeCoordinatorDemotionPending: false,
    activeOrchestrationParent: null,
    orchestrationTaskPreviewExpanded: false,
    expandedCoordinatorWorkerGroups: {},
    expandedWorktreeFamilies: {},
    workspaceGroupStates: {},
    cliSessionId: null,
    ...
    currentModels: [],
    currentModelsLoading: false,
    modelsByVendor: {},
    // This user's last-used vendor for the project; seeds "New session".
    lastVendor: "",
    // Static adapter metadata sent before any vendor is initialized.
    vendorInfo: {},
    providerHubStatus: { providers: [], routingProfile: "balanced" },
  ...
  // Apply the server-stored chat layout preference on load. Settings live
  // server-side (no localStorage), so without this the layout defaults to
  // "channel" until the user opens settings. Fetched early to minimize flash.
  fetch('/api/profile').then(function (r) { return r.ok ? r.json() : null; }).then(function (d) {
    if (d && d.chatLayout) setChatLayout(d.chatLayout);
  }).catch(function () {});
  ...
  initCommandPalette({
    ...
    selectSession: function (id) {
      ...
      _connSendUserAction({ type: "switch_session", id: id });
    },
    openDm: function (userId) { openDm(userId); },
  ...
  messagesEl.addEventListener("click", function(e) {
    ...
    showConfirm("Fork session from this message?", function() {
      _connSendUserAction({ type: "fork_session", uuid: forkUuid });
    }, "Fork", false);
  });
  ...
  function addUserMessage(text, images, pastes, fromUserId, fromUserName, opts) { _renAddUserMessage(text, images, pastes, fromUserId, fromUserName, opts); }
  ...
  function switchProject(slug, options) { _projSwitchProject(slug, options); }

  function syncBrowserProjectRoute(slug) {
    basePath = "/p/" + slug + "/";
    wsPath = "/p/" + slug + "/ws";
    store.set({ basePath: basePath, wsPath: wsPath });
  }

  function updateHistoryGlobalSessionRef(slug, urlRef) {
    var nextRef = slug !== "lead" && urlRef ? urlRef : null;
    if (!sameSessionRef(store.get('activeGlobalSessionRef'), nextRef)) {
      store.set({ activeGlobalSessionRef: nextRef });
    }
  }

  function sessionSwitchActionFromHistory(slug, urlRef) {
    if (urlRef) return { type: "switch_session", storageId: urlRef.sessionStorageId };
    var savedId = readTabSession(slug);
    if (!savedId) return null;
    var numericId = parseInt(savedId, 10);
    if (numericId > 0 && String(numericId) === String(savedId)) {
      return { type: "switch_session", id: numericId };
    }
    return { type: "switch_session", storageId: savedId };
  }

  function clearHistoryActiveSession(slug) {
    store.set({
      activeSessionId: null,
      activeSessionProjectSlug: slug,
      activeSessionTitle: "",
      cliSessionId: null,
      activeCoordinationMode: false,
      activeCoordinatorDemotionPending: false,
  ...
  window.addEventListener("popstate", function () {
    ...
    if (!newSlug) return;
    var urlRef = readUrlSessionRef(newSlug);
    updateHistoryGlobalSessionRef(newSlug, urlRef);
    if (newSlug !== store.get('currentSlug')) {
      resetFileBrowser();
      closeArchive();
      hideMemory();
      ...
      window.dispatchEvent(new CustomEvent("clay:project-switching", { detail: { slug: newSlug } }));
      syncBrowserProjectRoute(newSlug);
      clearHistoryActiveSession(newSlug);
      connect();
      return;
    }
    if (newSlug === "lead" && !urlRef) {
      syncCoopLensFromUrl(_connSendUserAction);
      renderSessionList(null);
      _projRestoreCanonicalLeadHome();
      return;
    }
    var switchAction = sessionSwitchActionFromHistory(newSlug, urlRef);
    if (switchAction) {
      clearHistoryActiveSession(newSlug);
      _connSendUserAction(switchAction);
    }
  });
  ...
  initServerSettings({
    ...
    get modelsByVendor() { return store.get('modelsByVendor'); },
    get currentModel() { return store.get('currentModel'); },
    get currentVendor() { return store.get('currentVendor'); },
    ...
    get codexApproval() { return store.get('codexApproval'); },
    set codexApproval(v) { store.set({ codexApproval: v }); },
    get codexSandbox() { return store.get('codexSandbox'); },
    set codexSandbox(v) { store.set({ codexSandbox: v }); },
    get codexWebSearch() { return store.get('codexWebSearch'); },
    set codexWebSearch(v) { store.set({ codexWebSearch: v }); },
    get installedVendors() { return store.get('installedVendors') || []; },
    get availableVendors() { return store.get('availableVendors') || []; },
    get providerRoutes() { return store.get('providerRoutes') || []; },
    get fullAutoMode() { return store.get('fullAutoMode'); },
    get skipPermsEnabled() { return store.get('skipPermsEnabled'); },
  ...
  // --- Human work budget (one daemon clock across every device) ---
  initHumanAttention();

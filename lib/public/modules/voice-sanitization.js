...
export function sanitizeVoiceText(value, limit) {
  ...
  var max = limit === 0 ? Infinity : Number.isInteger(limit) && limit > 0 ? limit : 640;
  if (text.length > max) text = text.slice(0, max - 1).trim() + "…";
  return text;
}

// Keep browser utterances short without silently dropping the rest of a reply.
export function voiceSpeechChunks(text) {
  var remaining = String(text || "");
  var chunks = [];
  while (remaining.length > 480) {
    var end = remaining.lastIndexOf(" ", 480);
    if (end < 1) end = 480;

...
export function visibleCoopAssignment(node) {
  return !!(node && node.role === "project_assignment" && node.taskRef && node.coordinatorRef &&
    (ACTIVE_STATUSES[node.status] || ATTENTION_STATUSES[node.status]));
}
...
function visibleNode(value, depth, parentRole) {
  ...
  if (value.role === "project_assignment") {
    return (depth === 1 && parentRole === "project_coordinator" ||
      depth === 2 && parentRole === "thread") && visibleCoopAssignment(value);
  }
...
function cloneCoordinatorNode(node, depth, parentRole) {
  ...
  return {
    ...
    coordinatorRef: value.coordinatorRef || null,
    assignmentPhase: safeText(value.assignmentPhase, ""),
    ...
    transparency: depth === 0 && value.transparency ? JSON.parse(JSON.stringify(value.transparency)) : null,
    dependencyState: safeText(value.dependencyState, ""),
    dependencies: safeArray(value.dependencies).map(cloneTaskRef).filter(Boolean),
    controlResults: safeArray(value.controlResults).map(cloneControlResult).filter(Boolean),
...
function pruneTerminalCoordinatorNode(node) {
  if (!node) return null;
  var children = safeArray(node.children).map(pruneTerminalCoordinatorNode).filter(Boolean);
  var value = Object.assign({}, node, { children: children });
  if (value.role === "project_coordinator") return value.transparency || children.length > 0 ? value : null;
  return hasVisibleCoopProjectWork(value) ? value : null;
}
...
export function cloneCoopProjectHierarchy(value) {
  return safeArray(value).map(function (node) {
    ...
    return pruneTerminalCoordinatorNode(node);

...
function cloneCoordinatorNode(node, depth) {
  var value = node || {};
  if (depth === 1 && !visibleCoopTaskCoordinator(value)) return null;
  if (depth === 2 && !visibleCoopWorker(value)) return null;
  ...
  return {
    ...
    children: (depth < 2 ? safeArray(value.children) : []).map(function (child) {

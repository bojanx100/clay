...
export function renderLoopGroup(loopId, children, groupKey, deps) {
  ...
  // A chevron promises hidden content. A single-session group (e.g. an
  // MCP-started debate: one live session, no setup session) has nothing to
  // reveal — expanding would only duplicate the header's own target row —
  // so render an alignment spacer instead of a dead control.
  var expandable = visibleChildren.length > 1 || runCount > 1;
  if (expandable) {
    var chevron = document.createElement("button");
    chevron.className = "session-loop-chevron";
    chevron.innerHTML = iconHtml("chevron-right");
    ...
    chevSpacer.className = "session-loop-chevron spacer";
    el.appendChild(chevSpacer);
  }

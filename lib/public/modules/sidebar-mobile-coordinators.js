...
function mobileItemIsLeadOwned(item) {
  if (item && item.type === "loop") {
    return (item.children || []).some(function (session) { return !!session.leadOwned; });
  }
  return !!(item && item.data && item.data.leadOwned);
}

export function buildMobileOwnershipSections(items) {
  var meItems = [];
  var leadItems = [];
  for (var i = 0; i < items.length; i++) {
    if (mobileItemIsLeadOwned(items[i])) leadItems.push(items[i]);
    else meItems.push(items[i]);
  }
  var sections = [];
  if (meItems.length > 0) sections.push({ key: "me", label: "ME", items: meItems });
  if (leadItems.length > 0) sections.push({ key: "lead", label: COOP_IDENTITY, items: leadItems });
  return sections;
}

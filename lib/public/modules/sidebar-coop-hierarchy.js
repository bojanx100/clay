...
import { visibleCoopTaskCoordinator } from './sidebar-coop-hierarchy-model.js';
...
function statusLabel(node, depth) {
  ...
  if (ATTENTION_STATUSES[status]) return "Needs input";
...
function createHierarchyRow(node, depth, prefix, options) {
  ...
  title.textContent = safeText(node.title, depth === 0 ? "Project coordinator" : "Task coordinator");
...
function createHierarchyNode(node, depth, prefix, options) {
  ...
  wrapper.className = prefix + "coop-project-coordinator-node" +
    (depth === 0 ? " root" : " child");
  wrapper.appendChild(createHierarchyRow(node, depth, prefix, options));
  var children = depth === 0 ? safeArray(node.children).filter(visibleCoopTaskCoordinator) : [];

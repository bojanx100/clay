...
function createHierarchyNode(node, depth, prefix, options) {
  ...
  var results = node && node.role === "thread" ? safeArray(node.controlResults) : [];
  for (var ri = 0; ri < results.length; ri++) {
    var result = document.createElement("div");
    result.className = prefix + "coop-control-result";
    var title = document.createElement("span");
    title.className = prefix + "coop-control-result-title";
    title.textContent = safeText(results[ri].title, "Control result");
    result.appendChild(title);
    var summary = document.createElement("span");
    summary.className = prefix + "coop-control-result-summary";
    summary.textContent = safeText(results[ri].summary, "Control review completed.");

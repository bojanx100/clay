...
export function showDoneNotification() {
  ...
  if (activeItem) {
    ...
    // container -- that also holds role/vendor/auto badges as adjacent
    // siblings with no whitespace in the markup (e.g. "Coordinator" would
    // otherwise glue onto the title as "CoordinatorREDESIGN").
    var textEl = activeItem.querySelector(".session-item-title") || activeItem.querySelector(".session-item-text");
...
export function initNotifications(_ctx) {
  ...
  var notificationSettings = $("us-notifications");

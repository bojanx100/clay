...
function saveAutoLaunch() {
  ...
  var vendorEl = document.getElementById("ps-auto-launch-vendor");
  ...
  if (vendorEl) {
    var claudePct = parseInt(vendorEl.value, 10);
    if (!Number.isFinite(claudePct)) claudePct = 60;
...
function switchSection(name) {
  ...
  if (name === "automation") {
    if (ctx.ws && ctx.ws.readyState === 1) {
      ctx.ws.send(JSON.stringify({ type: "get_auto_launch" }));
    }
  }
...
export function handleSetProjectGitAccountResult(msg) {
  ...
  if (msg && msg.ok) {
    ...
    if (ctx.ws && ctx.connected && currentSlug) {
      ctx.ws.send(JSON.stringify({ type: "get_project_git_account", slug: currentSlug }));
    }
  } else if (status) {

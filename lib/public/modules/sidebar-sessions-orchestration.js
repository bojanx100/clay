...
function modalShell(source) {
  ...
  intro.innerHTML = "Offer <strong>" + escapeHtml(source.title || "New Session") +
    "</strong> to an owning AI. It will read a compact handoff and decide whether this is a task, dependency, context, or unrelated.";
...
export function handleSessionAdoptionProposed(msg) {
  ...
  if (msg.ok) {
    showToast("Offered to the coordinator. It will classify the session when ready.", "success");
    closePicker();
  } else {

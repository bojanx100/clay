...
export function workspaceSummaryHtml(state) {
  ...
  return '<section class="ws-section ws-context-card">' +
    '<div class="ws-section-heading"><span>Workspace</span>' +
    '<i data-lucide="folder-git-2"></i></div>' +
...
export function devSectionHtml(state) {
  var dev = state.dev;
  if (!dev) {
    return '<section class="ws-section ws-environment" id="ws-dev-section">' +
      '<div class="ws-section-heading"><span>Local environment</span>' +
      '<i data-lucide="server-off"></i></div>' +
      '<div class="ws-empty-callout">No development script was found in package.json.</div>' +
      '</section>';
  }
  ...
  var branch = dev.branch
    ...
    : "";
  return '<section class="ws-section ws-environment ws-environment-' +
    escapeHtml(status) + '" id="ws-dev-section">' +
    ...
    localLink + '</div>' + control + '</div>' +
    (isUp ? liveUiControlsHtml(dev) : '') + '</section>';
}
...
export function linkedItemsHtml(state) {
  ...
  return '<section class="ws-section"><div class="ws-section-heading">' +
    '<span>Linked work</span><i data-lucide="list-tree"></i></div>' +

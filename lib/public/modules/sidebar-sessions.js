...
function renderSessionTopActions() {
  ...
  // Kiro: always GUI (no TUI adapter for Kiro). Only shown when kiro-cli is
  // installed, since it is not present on every deployment.
  var installedVendors = store.get('installedVendors') || [];
  if (installedVendors.indexOf("kiro") !== -1) {
    var kiroBtn = document.createElement("button");
    kiroBtn.className = "session-top-action";
    kiroBtn.type = "button";
    kiroBtn.title = "New Kiro CLI session";
    kiroBtn.innerHTML = '<img src="/kiro-avatar.svg" class="session-top-action-icon" alt=""><span>Kiro</span>';
    kiroBtn.addEventListener("click", function () {
      if (getWs() && store.get('connected')) {
        getWs().send(JSON.stringify({ type: "new_session", vendor: "kiro" }));
      }
    });
    wrap.appendChild(kiroBtn);
  }

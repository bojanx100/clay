...
function sessionVendorMarkup(s, parent) {
  ...
  var attemptCount = Number(parent && parent.attemptCount) || 0;
  var attempt = Number(parent && parent.attempt) || 0;
  var attemptLabel = attemptCount > 1 ? "Worker attempt " + attempt + " of " + attemptCount : "";
  ...
  if (attemptLabel) vendorDotTitle = attemptLabel + " · " + vendorDotTitle;
...
function appendSummarySection(container, label, items, formatter) {
  items = items || [];
  var section = document.createElement("div");
  section.className = "global-coop-summary-section";
  var heading = document.createElement("span");
  heading.className = "global-coop-summary-label";
  heading.textContent = label;
  section.appendChild(heading);
  var text = document.createElement("span");
  text.className = "global-coop-summary-text";
  text.textContent = items.map(formatter).filter(Boolean).join(" · ") || "None recorded.";
...
function appendProjectedProject(container, project) {
  ...
  group.className = "global-coop-project" +
    (project.summary.attention.length > 0 ? " global-coop-project-attention" : "") +
  ...
  lens.type = "button";
  lens.className = "global-coop-project-header";
  ...
  metrics.className = "global-coop-project-metrics";
  ...
  appendSummarySection(group, "Goals", project.summary.goals, function (item) { return item; });
  appendSummarySection(group, "Decisions", project.summary.decisions, function (item) { return item; });
  appendSummarySection(group, "Active work", project.summary.activeWork, summaryItemText);
  appendSummarySection(group, "Attention", project.summary.attention, summaryItemText);
  appendSummarySection(group, "Verified outcomes", project.summary.outcomes, summaryItemText);
  appendProjectedSessionTree(group, project.summary.coordinatorTree || []);
  renderCoopProjectTopics(group, project.topics || [], { mobile: false, projectRef: project.projectRef });
  var actions = document.createElement("div");
  actions.className = "global-coop-project-actions";
  var handoff = document.createElement("button");
  handoff.type = "button";
  handoff.className = "global-coop-handoff-action";
...
function appendProjectedSessionTree(container, nodes) {
  var tree = document.createElement("div");
  tree.className = "global-coop-session-tree";
  for (var i = 0; i < nodes.length; i++) appendProjectedSessionNode(tree, nodes[i], 0);
  if (tree.childNodes.length > 0) container.appendChild(tree);
}

function appendProjectedSessionNode(container, node, depth) {
  if (!node || !node.sessionRef) return;
  var row = document.createElement("button");
  row.type = "button";
  row.className = "global-coop-session-node global-coop-session-" + node.role;
  ...
  for (var i = 0; i < children.length; i++) appendProjectedSessionNode(container, children[i], depth + 1);
}
...
function createGlobalCoopConversationSection() {
  ...
  renderCoopTopicOverview(section, model, { mobile: false, includeGroups: false });
  for (var i = 0; i < model.projects.length; i++) appendProjectedProject(section, model.projects[i]);
  renderCoopTopicGroup(section, "Cross-project", model.crossProjectTopics, { mobile: false });
  renderCoopTopicGroup(section, "Uncategorised", model.uncategorisedTopics, { mobile: false });
  return section;
}

...
// Store updates can arrive while modules are loading, before sidebar.js has
// installed its DOM context. Keep a cached list ready for the first real
// render, but never attempt a projection-driven repaint until that context is
// available.
var sidebarSessionsInitialized = false;
...
store.subscribe(function (state, prev) {
  if (state.lastVendor !== prev.lastVendor || state.installedVendors !== prev.installedVendors) {
    _listSignature = null;
    if (sidebarSessionsInitialized && !isSessionListLoading()) renderSessionList(null);
  }
  ...
  if ((state.activeSessionId !== prev.activeSessionId ||
       ...
       state.coopProjectionVersion !== prev.coopProjectionVersion ||
       state.coopActionQueue !== prev.coopActionQueue ||
       state.coopNowIndex !== prev.coopNowIndex ||
      ...
      cachedSessions.length > 0 && sidebarSessionsInitialized) {
...
export function initSidebarSessions() {
  sidebarSessionsInitialized = true;

...
function renderSessionItem(s) {
  ...
  if (s.coordinationMode) {
    ...
    textHtml += '<span class="session-role-badge worker" title="Worker session">Worker</span>';
  }
  var vendorOverrides = store.get('sessionVendorOverrides') || {};
  var rememberedVendor = s.vendor ? "" : (vendorOverrides[sessionVendorOverrideKey(s.id, s.cliSessionId)] || "");
  var sessionVendor = s.vendor || rememberedVendor || "claude";
...
function coordinatorGroupExpanded(item) {
  ...
  for (var i = 0; i < item.children.length; i++) {
    ...
    if (searchMatchIds !== null && searchMatchIds.has(item.children[i].id)) return true;
  }
  return false;
}
...
function renderCoordinatorGroup(item) {
  ...
  toggle.className = "session-coordinator-toggle";
  toggle.type = "button";
  toggle.title = expanded ? "Collapse workers" : "Expand workers";
  toggle.setAttribute("aria-label", toggle.title);
  toggle.setAttribute("aria-expanded", expanded ? "true" : "false");
  toggle.innerHTML = iconHtml(expanded ? "chevron-down" : "chevron-right");
  toggle.addEventListener("click", function (e) {
    e.preventDefault();
    e.stopPropagation();
    if (expandedCoordinatorGroups.has(item.data.id)) expandedCoordinatorGroups.delete(item.data.id);
    else expandedCoordinatorGroups.add(item.data.id);
    _listSignature = null;
    renderSessionList(null);
  });
...
function sessionListSignature(sessions) {
  ...
  for (var i = 0; i < sessions.length; i++) {
    ...
    parts.push([
      ...
      s.visibility || "",
      s.vendor || "",
      s.coordinationMode ? "coordinator" : (s.orchestrationParent ? "worker" : ""),
      s.orchestrationActiveCount || 0,
      s.orchestrationParent ? (s.orchestrationParent.sessionId || "") : "",
      s.orchestrationParent ? (s.orchestrationParent.workerColor || "") : "",
  ...
  for (var a = 0; a < sessions.length; a++) {
    ...
    if (sessions[a].active && sessions[a].orchestrationParent) {
      activeCoordinator = sessions[a].orchestrationParent.sessionId || "";
    }
  }
...
export function renderSessionList(sessions) {
  ...
  for (var j = 0; j < normalSessions.length; j++) {
    ...
    if (normalSession.orchestrationParent && coordinatorChildren[normalSession.orchestrationParent.sessionId]) continue;

...
import { actionQueueSignature } from './coop-action-queue-ui.js';
...
import { topicReviewSignature } from './sidebar-coop-topic-review.js';
import {
...
store.subscribe(function (state, prev) {
  ...
  if ((state.activeSessionId !== prev.activeSessionId ||
       ...
       state.openCoopActionItemId !== prev.openCoopActionItemId ||
       ...
       // Topic review panel + Done section: same repaint contract as the
       // action queue above.
       state.openCoopTopicReviewId !== prev.openCoopTopicReviewId ||
       state.coopTopicReviewPending !== prev.coopTopicReviewPending ||
       state.coopTopicReviewErrors !== prev.coopTopicReviewErrors ||
...
function currentSessionListSignature() {
  ...
  return store.get('currentSlug') === "lead"
    ? signature + "|" + globalCoopProjectionSignature() + "|" +
      ...
      "|" + topicReviewSignature() +
      "|" + actionQueueSignature()
    : signature;
}

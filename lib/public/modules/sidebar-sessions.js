...
export function openImportSessionPicker(vendorFilter) {
  ...
  sendUserAction({ type: "list_cli_sessions", vendor: importPickerVendor || "" });
}
...
export function openAutoLaunchActivity(anchor) {
  if (!anchor) return;
  showAutoLaunchActivityPopover(anchor);
}
...
function updateCountdowns() {
  ...
  for (var i = 0; i < upcoming.length; i++) {
    ...
    if (u.id === AUTOLAUNCH_REC_ID) {
      ...
      // Badge reflects FINISHED work (completed today). In-progress sessions
      // already appear in the session list, so we deliberately don't surface a
      // "started" count here — the signal is what completed, especially work
      // that finished autonomously without the user watching.
      var doneToday = 0;
      var completedEver = 0;
...
function showAutoLaunchActivityPopover(anchor) {
  ...
  // The activity feed is a log of what FINISHED. In-progress ("started") items
  // are intentionally excluded: running sessions already appear in the session
  // list, so the value here is seeing completions — especially auto-launch work
  // that finished autonomously without the user watching.
  var events = (autoLaunchActivity.events || []).filter(function (e) { return e.type === "completed"; });
  ...
  for (var k = 0; k < items.length; k++) {
    items[k].addEventListener("click", function () {
      ...
      if (sid || stid) {
        // Send storageId (persistent) when available so the server resolves the
        // correct live session; fall back to the legacy localId for old events.

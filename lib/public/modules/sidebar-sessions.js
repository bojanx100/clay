...
function compareSessionListItems(a, b) {
  var aData = a && (a.type === "session" || a.type === "coordinator") ? a.data : a;
  var bData = b && (b.type === "session" || b.type === "coordinator") ? b.data : b;
...
function collectItemSessionIds(item) {
  ...
  if (item.type === "coordinator" && item.data && typeof item.data.id === "number") {
    var coordinatorIds = isSessionVisibleBySearch(item.data.id) ? [item.data.id] : [];
    var coordinatorChildren = item.children || [];
    for (var ci = 0; ci < coordinatorChildren.length; ci++) {
      if (isSessionVisibleBySearch(coordinatorChildren[ci].id)) coordinatorIds.push(coordinatorChildren[ci].id);
    }
...
function renderSessionItem(s) {
  ...
  if (s.coordinationMode) {
    textHtml += '<span class="session-role-badge coordinator" title="Coordinator session">Coordinator</span>';
    ...
    textHtml += '<span class="session-role-badge worker" title="' + workerTitle + '">' + workerLabel + '</span>';
  }
  var vendorOverrides = store.get('sessionVendorOverrides') || {};
  var rememberedVendor = s.vendor ? "" : (vendorOverrides[sessionVendorOverrideKey(s.id, s.cliSessionId)] || "");
  var sessionVendor = s.vendor || rememberedVendor || "claude";
  ...
  if (orchestrationParent && orchestrationParent.taskStatus) {
    vendorDotClass += " worker-status-" + orchestrationParent.taskStatus;
    var taskStatusLabel = String(orchestrationParent.taskStatus).replace(/_/g, " ");
    vendorDotTitle = taskStatusLabel.charAt(0).toUpperCase() + taskStatusLabel.slice(1) + " · " + vendorDotTitle;
  }
  textHtml += '<span class="' + vendorDotClass + '" title="' + vendorDotTitle + '"></span>';
...
function sessionListSignature(sessions) {
  ...
  for (var i = 0; i < sessions.length; i++) {
    ...
    parts.push([
      ...
      s.visibility || "",
      s.vendor || "",
      s.coordinationMode ? "coordinator" : (orchestrationParent ? "worker" : ""),
      s.orchestrationActiveCount || 0,
      s.orchestrationPhase || "",
      s.orchestrationUnresolvedCount || 0,
      orchestrationParent ? (orchestrationParent.sessionId || "") : "",
      orchestrationParent ? (orchestrationParent.workerColor || "") : "",
      orchestrationParent ? (orchestrationParent.taskStatus || "") : "",
      orchestrationParent ? (orchestrationParent.attempt || "") : "",
      orchestrationParent ? (orchestrationParent.attemptCount || "") : "",
      s.demotionPending ? 1 : 0,
  ...
  for (var a = 0; a < sessions.length; a++) {
    ...
    var activeParent = sessions[a].orchestrationGroupParent || sessions[a].orchestrationParent;
    if (sessions[a].active && activeParent) {
      activeCoordinator = activeParent.sessionId || "";
    }
  }
  var searchSig = (searchQuery || "") + "|" + (searchMatchIds ? Array.from(searchMatchIds).sort().join(",") : "");
  var expandedWorkerGroups = store.get("expandedCoordinatorWorkerGroups") || {};
  var expandedWorkerSig = Object.keys(expandedWorkerGroups).filter(function (key) {
    return expandedWorkerGroups[key];
  }).sort().join(",");
  return parts.join("") + "||g:" + activeGroup + "||c:" + activeCoordinator + "||s:" + searchSig +
    "||w:" + expandedWorkerSig;
}
...
export function renderSessionList(sessions) {
  ...
  // Build virtual items: normal sessions + coordinator groups + one entry per loop group.
  var normalById = {};
  var coordinatorChildren = {};
  for (var ni = 0; ni < normalSessions.length; ni++) normalById[normalSessions[ni].id] = normalSessions[ni];
  for (var wi = 0; wi < normalSessions.length; wi++) {
    var workerSession = normalSessions[wi];
  ...
  for (var j = 0; j < normalSessions.length; j++) {
    var normalSession = normalSessions[j];
    var normalParent = normalSession.orchestrationGroupParent || normalSession.orchestrationParent;
    if (normalParent && coordinatorChildren[normalParent.sessionId]) continue;
    if (normalSession.coordinationMode && coordinatorChildren[normalSession.id]) {
      coordinatorChildren[normalSession.id].sort(function (a, b) {
        return (b.lastActivity || 0) - (a.lastActivity || 0);
      });
      items.push({
        type: "coordinator",
        data: normalSession,
        children: coordinatorChildren[normalSession.id],
        lastActivity: normalSession.lastActivity || 0,
      });
    } else {
      items.push({ type: "session", data: normalSession, lastActivity: normalSession.lastActivity || 0 });
    }
  }
  ...
  for (var n = 0; n < items.length; n++) {
    ...
    if (item.type === "coordinator" && item.data && !isSessionVisibleBySearch(item.data.id)) {
      var visibleWorker = item.children.some(function (child) { return isSessionVisibleBySearch(child.id); });
      if (!visibleWorker) continue;
    }
    if ((item.type === "session" || item.type === "coordinator") && item.data && item.data.bookmarked) {
  ...
  for (var bi = 0; bi < bookmarkedItems.length; bi++) {
    favoritesContainer.appendChild(bookmarkedItems[bi].type === "coordinator"
      ? renderCoordinatorGroup(bookmarkedItems[bi])
      : renderSessionItem(bookmarkedItems[bi].data));
  }
  ...
  for (var ri = 0; ri < regularItems.length; ri++) {
    ...
    if (item.type === "loop") {
      ...
      regularContainer.appendChild(renderCoordinatorGroup(item));
    } else {

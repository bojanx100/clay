...
function sessionLeadingMarkup(s) {
  var textHtml = "";
...
function sessionRoleMarkup(s, parent) {
  if (s.coordinationMode) {
    return '<span class="session-role-badge coordinator" title="Coordinator session">Coordinator</span>';
  }
  if (!parent) return "";
  var attemptCount = Number(parent.attemptCount) || 0;
  var attempt = Number(parent.attempt) || 0;
  var workerLabel = attemptCount > 1 ? "Worker " + attempt + "/" + attemptCount : "Worker";
  var workerTitle = attemptCount > 1 ? "Worker attempt " + attempt + " of " + attemptCount : "Worker session";
  return '<span class="session-role-badge worker" title="' + workerTitle + '">' + workerLabel + "</span>";
}
...
function sessionVendorMarkup(s, parent) {
  ...
  if (parent && parent.taskStatus) {
    vendorDotClass += " worker-status-" + parent.taskStatus;
    var taskStatusLabel = String(parent.taskStatus).replace(/_/g, " ");
    vendorDotTitle = taskStatusLabel.charAt(0).toUpperCase() + taskStatusLabel.slice(1) + " · " + vendorDotTitle;
  }
  return '<span class="' + vendorDotClass + '" title="' + vendorDotTitle + '"></span>';
}
...
function createSessionItemText(s, parent) {
  var textHtml = sessionLeadingMarkup(s);
  textHtml += sessionRoleMarkup(s, parent);
...
function projectedStatusLabel(status) {
  return String(status || "available").replace(/_/g, " ");
}

function projectedRowTitle(row) {
  return row.title || (row.role === "worker" ? "Worker" : "Conversation");
}

function appendProjectedStatus(el, row, availability) {
  var status = document.createElement("span");
  var value = availability === "available" ? (row.status || "available") : availability;
  status.className = "global-coop-row-status global-coop-row-status-" + value;
  status.title = globalAvailabilityLabel(availability) || projectedStatusLabel(value);
  status.setAttribute("aria-hidden", "true");
  el.appendChild(status);
}

function appendProjectedTitle(el, row, availability) {
  var text = document.createElement("span");
  text.className = "session-item-text";
  var role = document.createElement("span");
  role.className = "session-role-badge global-coop-role " + (row.role || "conversation");
  role.textContent = row.role === "direct_leaf" ? "Direct" :
    (row.role === "coop" ? "Coop" : (row.role === "worker" ? "Worker" : "Coordinator"));
  var title = document.createElement("span");
  title.className = "session-item-title";
  title.textContent = projectedRowTitle(row);
  text.appendChild(role);
  text.appendChild(title);
  if (row.activity) {
    var activity = document.createElement("span");
    activity.className = "global-coop-row-activity";
    activity.textContent = row.activity;
    text.appendChild(activity);
  }
  if (availability !== "available") {
    var unavailable = document.createElement("span");
    unavailable.className = "global-coop-row-unavailable";
...
function bindProjectedSessionNavigation(el, row, availability) {
  ...
  function open() {
    requestGlobalSessionRef(row.sessionRef, sendUserAction);
  }
  el.setAttribute("role", "button");
  el.setAttribute("tabindex", "0");
  el.setAttribute("aria-label", "Open " + projectedRowTitle(row) + " in its project");
...
function createProjectedSessionRow(row, extraClass) {
  var availability = globalAttemptAvailability(row);
  var isActive = globalSessionRefKey(row.sessionRef) &&
    globalSessionRefKey(row.sessionRef) === globalSessionRefKey(store.get("activeGlobalSessionRef"));
  var el = document.createElement("div");
  el.className = "session-item global-coop-row " + (extraClass || "") +
    (availability === "available" ? "" : " global-coop-row-unavailable-state") +
    (row.attention ? " global-coop-row-attention" : "") +
    (row.legacy ? " global-coop-row-legacy" : "") +
    (isActive ? " active" : "");
  if (row.sessionRef) el.dataset.sessionRef = row.sessionRef.projectId + ":" + row.sessionRef.sessionStorageId;
  appendProjectedStatus(el, row, availability);
  appendProjectedTitle(el, row, availability);
  bindProjectedSessionNavigation(el, row, availability);
  return el;
}

function createProjectedSectionHeader(label, extraClass) {
  var header = document.createElement("div");
  header.className = "session-group-header global-coop-section-header " + (extraClass || "");
...
function appendProjectedAttempts(container, task, attempts) {
  for (var i = 0; i < attempts.length; i++) {
    ...
    container.appendChild(createProjectedSessionRow(attempt, "global-coop-worker-row"));
  }
}

function createProjectedTaskToggle(task, count, onToggle) {
  var expanded = isGlobalTaskExpanded(task.taskRef);
  var button = document.createElement("button");
  button.type = "button";
  button.className = "global-coop-history-toggle";
  button.setAttribute("aria-expanded", expanded ? "true" : "false");
  button.textContent = expanded ? "Hide attempt history" :
    count + (count === 1 ? " earlier attempt" : " earlier attempts");
...
function appendProjectedTask(container, task) {
  var taskEl = document.createElement("div");
  taskEl.className = "global-coop-task global-coop-task-" + task.status;
  var label = document.createElement("div");
  label.className = "global-coop-task-label";
  label.textContent = projectedStatusLabel(task.status) + (task.activity ? " · " + task.activity : "");
  ...
  if (task.terminal) {
    if (expanded) appendProjectedAttempts(taskEl, task, history);
  } else {
    appendProjectedAttempts(taskEl, task, task.currentAttempts);
  }
  if (history.length > 0) {
    taskEl.appendChild(createProjectedTaskToggle(task, history.length, function () { renderSessionList(null); }));
    if (expanded && !task.terminal) appendProjectedAttempts(taskEl, task, task.historicalAttempts);
  }
  container.appendChild(taskEl);
}

function appendProjectedCoordinator(container, coordinator) {
  var group = document.createElement("div");
  group.className = "global-coop-coordinator";
  group.appendChild(createProjectedSessionRow(coordinator, "global-coop-coordinator-row"));
  var tasks = coordinator.visibleTasks.concat(coordinator.terminalTasks);
  for (var i = 0; i < tasks.length; i++) appendProjectedTask(group, tasks[i]);
  container.appendChild(group);
}

function appendProjectedProject(container, project) {
  var group = document.createElement("section");
  group.className = "global-coop-project" + (project.worktree ? " global-coop-worktree" : "") +
    (project.legacyLead ? " global-coop-legacy-lead" : "") +
    (project.unavailableProject ? " global-coop-project-unavailable" : "") +
    (project.attention ? " global-coop-project-attention" : "");
  group.dataset.projectId = project.projectRef && project.projectRef.projectId || "";
  var label = (project.icon ? project.icon + " " : "") + project.title;
  group.appendChild(createProjectedSectionHeader(label, "global-coop-project-header"));
  for (var ci = 0; ci < project.coordinators.length; ci++) {
    appendProjectedCoordinator(group, project.coordinators[ci]);
  }
  for (var li = 0; li < project.directLeaves.length; li++) {
    var leaf = Object.assign({}, project.directLeaves[li], {
      role: project.legacyLead ? "worker" : "direct_leaf",
    });
    group.appendChild(createProjectedSessionRow(leaf, "global-coop-direct-row"));
  }
  for (var wi = 0; wi < project.worktrees.length; wi++) appendProjectedProject(group, project.worktrees[wi]);
  container.appendChild(group);
}

function createCanonicalCoopRow() {
  var home = cachedSessions.find(function (session) { return session.coopHome; });
  var coopReference = getGlobalCoopReference();
  if (!home && coopReference) {
    var projected = createProjectedSessionRow(Object.assign({}, coopReference, {
      title: "Coop",
      role: "coop",
    }), "global-coop-permanent-row session-coop-home");
    projected.setAttribute("aria-label", "Open Coop");
    return projected;
  }
  if (!home) return null;
  var display = Object.assign({}, home, { sidebarTitle: "Coop" });
  var row = renderSessionItem(display);
  row.classList.add("global-coop-permanent-row", "session-coop-home");
...
function createGlobalCoopConversationSection() {
  ...
  var model = buildGlobalCoopDisplayModel(searchQuery);
  section.appendChild(createProjectedSectionHeader("Coop", "global-coop-heading"));
  var coopRow = createCanonicalCoopRow();
  if (coopRow) section.appendChild(coopRow);
  if (!model.hasProjection) {
    var loading = document.createElement("div");
    loading.className = "global-coop-empty";
    loading.textContent = "Loading project conversations…";
    section.appendChild(loading);
    return section;
  }
  for (var i = 0; i < model.projects.length; i++) appendProjectedProject(section, model.projects[i]);
  return section;
}

function isLeadAutomation(session) {
  return !!(session && !session.coopHome && !session.coopChannel &&
    (session.taskLauncher || session.loop));
}

function createLeadAutomationSection() {
  var automations = cachedSessions.filter(isLeadAutomation);
  if (automations.length === 0) return null;
  var section = document.createElement("div");
  section.className = "global-coop-automations";
  section.appendChild(createProjectedSectionHeader("Automations & schedules", "global-coop-automation-heading"));
  for (var i = 0; i < automations.length; i++) section.appendChild(renderSessionItem(automations[i]));
  return section;
}
...
function currentSessionListSignature() {
  ...
  return store.get('currentSlug') === "lead" ? signature + "|" + globalCoopProjectionSignature() : signature;
}
...
function createRegularContainer(dateGroups) {
  ...
  for (var i = 0; i < dateGroups.length; i++) appendDateGroup(regularContainer, dateGroups[i]);
...
export function renderSessionList(sessions) {
  ...
  if (store.get('currentSlug') === "lead") {
    ...
    var automations = createLeadAutomationSection();
    if (automations) listEl.appendChild(automations);
  ...
  listEl.appendChild(createRegularContainer(model.dateGroups));
  finishSessionListRender();
}

...
function createSessionItemText(s, parent) {
  ...
  textHtml += '<span class="session-item-title">' + highlightMatch(s.title || "New Session", searchQuery) + '</span>';
  return textHtml;
}
...
function appendDateGroup(container, group) {
  ...
  for (var si = 0; si < cachedSessions.length; si++) {
    if (cachedSessions[si].coopHome) protectedIds[cachedSessions[si].id] = true;
  }

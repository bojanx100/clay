...
// A peer review's objective can be a full implementation brief. In the
// sidebar that is navigation context, not a heading: use a stable role label
// and retain the raw title as hover and accessible detail on the exact session.
export function compactActionTitle(entry) {
  var role = controlRoleLabel(entry && entry.controlRole);
  return role ? role + " review" : text(entry && entry.title, "Untitled work");
}
...
export function normalizeNowIndex(message) {
  ...
  for (var i = 0; i < entries.length; i++) {
    ...
    out.push({
      ...
      controlRole: entry.controlRole === "council" || entry.controlRole === "triage"
        ? entry.controlRole : "",
...
function createNowRow(entry, options) {
  ...
  if (entry.controlRole) row.dataset.controlRole = entry.controlRole;
  ...
  title.textContent = compactActionTitle(entry);
  if (entry.controlRole) title.setAttribute("title", entry.title);
  ...
  if (entry.sessionRef) {
    ...
    if (entry.controlRole) meta.classList.add(prefix + "coop-action-item-meta-control");

// coop-action-queue-ui.js - The owner's "Now" index, rendered at the top of
// Coop on desktop and mobile, plus the decision transport shared with the
...
export function normalizeNowIndex(message) {
  ...
  for (var i = 0; i < entries.length; i++) {
    ...
    out.push({
      ...
      title: text(entry.title, "Untitled topic"),
...
function createNowRow(entry, options) {
  ...
  row.setAttribute("aria-label", entry.title + ", " + entry.reason + ", " +
    (canOpenTopic ? "opens the topic" : "no destination available"));
...
// nothing is genuinely current, and a heading saying so is noise.

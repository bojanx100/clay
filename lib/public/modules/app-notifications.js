...
function showLoginReminderBanner() {
  ...
  banner.innerHTML =
    ...
    '<div class="notif-banner-body">' +
      ...
      '<div class="notif-banner-text">Finish signing in inside the popup. Clay refreshes its auth automatically when the login completes.</div>' +
      '<div class="notif-banner-actions">' +
...
export function handleNotificationCreated(msg) {
  ...
  // Never start a login command from a notification. A stale auth signal can
  // otherwise cause Codex device auth to clear a still-valid credential. The
  // banner remains visible on every connected device and starts the command
  // only after the user explicitly chooses it.
  if (notif.type === "auth_required") {
    showAuthRequiredBanner(notif);
    return;
  }

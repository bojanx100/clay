...
import {
  createDesktopLeadProjectItem,
  filterLeadProjects,
  findVisibleLeadProject,
  isLeadProject
} from './sidebar-lead.js';
import { aggregateFamily, groupRailProjects } from './worktree-family.js';
import { createWorktreeRailGroup } from './sidebar-worktree-rail.js';

// --- Project state ---
var cachedProjectList = [];
var cachedProjectListSignature = null;

// Only the fields the projects sheet actually renders. Presence/processing
// churn must not count as a change, or the sheet would rebuild constantly.
function projectListSignature(projects, currentSlug) {
  var list = projects || [];
  var parts = [String(currentSlug || ""), store.get('leadModeEnabled') === true ? "lead:on" : "lead:off"];
  for (var i = 0; i < list.length; i++) {
    var p = list[i] || {};
    parts.push([p.slug, p.name, p.icon, p.isWorktree ? 1 : 0, p.parentSlug || "",
      p.worktreeAccessible === false ? 0 : 1].join("\u0001"));
  }
  return parts.join("\u0002");
}
...
store.subscribe(function (state, previous) {
  if (state.leadModeEnabled !== previous.leadModeEnabled) {
    renderIconStrip(cachedProjectList, cachedCurrentSlug);
  }
});
...
function showIconCtxMenu(anchorEl, slug, name) {
  ...
  if (isWorktree) {
    ...
    removeWtItem.addEventListener("click", function (e) {
      e.stopPropagation();
      closeProjectCtxMenu();
      sendUserAction({ type: "remove_project_check", slug: slug, name: name || slug });
    });
...
function showProjectCtxMenu(anchorEl, slug, name, icon, position) {
  ...
  if (!store.get('permissions') || store.get('permissions').deleteProject !== false) {
    ...
    deleteItem.addEventListener("click", function (e) {
      e.stopPropagation();
      closeProjectCtxMenu();
      sendUserAction({ type: "remove_project_check", slug: slug, name: name });
    });
...
function showEmojiPicker(slug, anchorEl) {
  ...
  removeBtn.addEventListener("click", function (e) {
    e.stopPropagation();
    closeEmojiPicker();
    sendUserAction({ type: "set_project_icon", slug: slug, icon: null });
  });
    ...
    for (var i = 0; i < emojis.length; i++) {
      (function (emoji) {
        ...
        btn.addEventListener("click", function (e) {
          e.stopPropagation();
          closeEmojiPicker();
          sendUserAction({ type: "set_project_icon", slug: slug, icon: emoji });
        });
...
function showProjectRename(slug, currentName) {
  ...
  function commitRename() {
    ...
    if (newName && newName !== currentName) {
      sendUserAction({ type: "set_project_title", slug: slug, title: newName });
...
function showTrashZone() {
  ...
  trash.addEventListener("drop", function (e) {
    ...
    if (slug) sendUserAction({ type: "remove_project_check", slug: slug });
  });
}
...
function setupDragHandlers(el, slug) {
  ...
  el.addEventListener("drop", function (e) {
    ...
    sendUserAction({ type: "reorder_projects", slugs: slugs });
  });
...
function groupProjects(projects) {
  ...
  for (var i = 0; i < projects.length; i++) {
    var p = projects[i];
    if (p.isMate || isLeadProject(p)) continue;
    visible.push(p);
  }
  var grouped = groupRailProjects(visible);
  return { parents: grouped.parents, wtByParent: grouped.worktreesByParent };
}
...
function appendLeadIconItem(container, projects, currentSlug) {
  var leadProject = findVisibleLeadProject(projects);
  if (!leadProject) return;
  if (!leadProject.icon) leadProject.icon = "🧭";
  var leadEl = createIconItem(leadProject, currentSlug);
  leadEl.classList.add("icon-strip-lead-item");
  container.appendChild(leadEl);
}

export function renderIconStrip(projects, currentSlug) {
  // The mobile projects sheet reads this cache but is not store-driven, so an
  // open sheet would stay on "Loading projects…" forever if the list first
  // arrives while it is showing. Announced as an event rather than a direct
  // call because sidebar-mobile imports this module.
  //
  // Fired only on an actual change: renderIconStrip also runs on DM
  // transitions, home-hub changes, and attention refreshes, and rebuilding an
  // open sheet on those would drop focus mid-interaction for no new data.
  var listSignature = projectListSignature(projects, currentSlug);
  var listChanged = listSignature !== cachedProjectListSignature;
  cachedProjectListSignature = listSignature;
  cachedProjectList = projects;
  cachedCurrentSlug = currentSlug;
  if (listChanged && typeof window !== "undefined" && window.dispatchEvent) {
    window.dispatchEvent(new CustomEvent("clay:project-list-updated"));
  }
  ...
  appendLeadIconItem(container, projects, currentSlug);
  ...
  for (var i = 0; i < grouped.parents.length; i++) {
    ...
    if (worktrees.length > 0) {
      container.appendChild(createWorktreeRailGroup(el, p, worktrees));
    } else {
...
function renderProjectList(projects, currentSlug) {
  ...
  var leadProject = findVisibleLeadProject(projects);
  if (leadProject) {
    list.appendChild(createDesktopLeadProjectItem(leadProject, currentSlug, function (slug) {
      if (switchProject) switchProject(slug);
    }));
  }

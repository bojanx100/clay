...
function updatePaneFullAccessButton(button, session) {
  ...
  button.title = enabled ? "YOLO is on — click to restore permission prompts" : "YOLO is off — permission prompts are on";
}
...
function createPane(pane, index) {
  ...
  fullAccess.innerHTML = '<span class="session-full-access-label">YOLO</span><span class="session-full-access-track" aria-hidden="true"><span></span></span>';
  ...
  fullAccess.addEventListener("click", function () {
    ...
    showConfirm("Enable YOLO for this session? Tool requests will be approved automatically without permission prompts.", function () {

...
export function initiateRewind(uuid) {
  if (ctx.processing) {
...
export function resolveRewindMode(chatOnly, selectedMode) {
  if (chatOnly) return "chat";
  return selectedMode || "both";
}

function getSelectedMode() {
  var checked = rewindModeOptions
    ? rewindModeOptions.querySelector('input[name="rewind-mode"]:checked')
    : null;
  return resolveRewindMode(cachedPreview && cachedPreview.chatOnly,
    checked ? checked.value : "both");
}
...
export function showRewindModal(data) {
  ...
  var chatOnly = data.chatOnly === true;
  cachedPreview = {
    fileCount: fileCount,
  ...
  if (rewindModeOptions) {
    rewindModeOptions.style.display = chatOnly ? "none" : "";
    var defaultRadio = rewindModeOptions.querySelector(
      'input[value="' + (chatOnly ? "chat" : "both") + '"]');

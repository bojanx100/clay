...
export function openTerminal() {
  ...
  // Hide file viewer / workspace panel if open (also unwatches)
  closeFileViewer();
  closeWorkspacePanel();

  container.classList.remove("hidden");
  isOpen = true;
  var openBtn = document.getElementById("terminal-toggle-btn");
  if (openBtn) openBtn.classList.add("active");

  ensureTerminalAssets().then(function () {
    if (isOpen && generation === openGeneration) finishOpeningTerminal();
  }).catch(function (err) {
    console.error("[terminal] Failed to load terminal assets:", err);
  });
}

function finishOpeningTerminal() {
...
export function closeTerminal() {
  ...
  isOpen = false;
  var closeBtn = document.getElementById("terminal-toggle-btn");
...
export function handleTermList(msg) {
  ...
  var serverTerminals = (msg.terminals || []).filter(function (t) {
    return t && t.kind !== "tui-session" && t.sessionId === store.get("activeSessionId");
  });

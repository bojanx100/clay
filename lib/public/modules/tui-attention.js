...
// When the modal hosts a vendor login command (e.g. "codex login
// --device-auth"), modalLoginVendor holds the vendor and modalLoginBuf
// accumulates terminal output so we can detect a successful login on exit
// and automatically refresh Clay's server-side auth cache. Non-login modals
// (live TUI sessions) leave these null/empty.
var modalLoginVendor = null;
var modalLoginBuf = "";
...
function handleLoginExit() {
  var vendorName = vendorDisplayName(modalLoginVendor);
  var success = /successfully logged in|logged in using|login successful/i.test(modalLoginBuf || "");
  if (modalXterm) {
    try {
      if (success) {
        modalXterm.write("\r\n\x1b[92m[" + vendorName + " login complete \u2014 refreshing Clay\u2026]\x1b[0m\r\n");
      } else {
        modalXterm.write("\r\n\x1b[90m[" + vendorName + " login exited]\x1b[0m\r\n");
      }
    } catch (e) {}
  }
  // Auth cache in yoke is module-global, so refreshing via this project's
  // WS re-checks every vendor for all projects. Harmless if login failed:
  // it just re-confirms the current (still logged-out) state.
  if (modalWs && modalWs.readyState === 1) {
    try { modalWs.send(JSON.stringify({ type: "refresh_vendors" })); } catch (e) {}
  }
  if (success) {
...
function openTuiModalReady(terminalId, sourceSlug, info) {
  ...
  var infoObj = info || {};
  modalLoginVendor = infoObj.loginVendor || null;
  modalLoginBuf = "";
  setModalBreadcrumb({
  ...
  modalWs.onmessage = function (e) {
    ...
    if (msg.type === "term_output" && modalXterm && msg.data) {
      if (modalLoginVendor) {
        modalLoginBuf += msg.data;
        if (modalLoginBuf.length > 8000) modalLoginBuf = modalLoginBuf.slice(-8000);
      }

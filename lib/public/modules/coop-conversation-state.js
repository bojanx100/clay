...
function accessibleSummary(model) {
  ...
  if (model.pending > 0) parts.push(queuedLabel(model));
  if (model.listening) parts.push("Listening");
  return parts.join(", ");
}

function queuedLabel(model) {
  if (!model || model.pending <= 0) return "";
  var label = model.pending + " message" + (model.pending === 1 ? "" : "s") + " queued";
  return model.replying ? label + " — finishing current answer" : label;
}
...
function render() {
  ...
  if (model.pending > 0) appendSpan(status, "coop-conversation-pending", queuedLabel(model));
...
export function setCoopConversationState(message) {
  var state = message && message.active ? {
    ...
    activeThreadRefs: Array.isArray(message.activeThreadRefs) ? message.activeThreadRefs.slice() : [],
    queuedThreadRefs: Array.isArray(message.queuedThreadRefs) ? message.queuedThreadRefs.slice() : [],

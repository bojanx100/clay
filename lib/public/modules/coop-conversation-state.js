...
function workLabel(state, target) {
  if (state !== "working") return WORK_LABELS[state] || WORK_LABELS.idle;
  return target ? "Working on " + target : "Working";
}
...
export function coopConversationDisplayModel(state, options) {
  ...
  return {
    ...
    workState: workState,
    workLabel: workLabel(workState, target),
    workTarget: target,
...
function render() {
  ...
  if (model.projectTitle) appendSpan(status, "coop-conversation-lens", "Project: " + model.projectTitle);

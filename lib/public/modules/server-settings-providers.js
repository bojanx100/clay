...
import { applyProviderReadiness, sortProvidersByReadiness } from './vendor-ui.js';
...
export function handleProviderStatus(message) {
  var normalized = message ? Object.assign({}, message) : { providers: [], routingProfile: "balanced" };
  normalized.providers = sortProvidersByReadiness(normalized.providers || []);
  applyProviderReadiness(normalized.providers);
  store.set({ providerHubStatus: normalized });
  var refresh = document.getElementById("settings-providers-refresh");
  var providers = normalized.providers;
  var verifying = false;
  for (var i = 0; i < providers.length; i++) {

...
import { ensureMermaidAsset } from './external-assets.js';
import { MARKDOWN_SANITIZE_OPTIONS, renderMarkdownLink } from './markdown-link-policy.js';

// Initialize markdown parser
marked.use({ gfm: true, breaks: false });

// Apply one policy to web URLs and workspace file references before the
// sanitized HTML reaches any of Clay's markdown surfaces.
marked.use({
  renderer: {
...
var currentMermaidThemeVars = getMermaidThemeVars();
var mermaidInitialized = false;

export function updateMermaidTheme(vars) {
  currentMermaidThemeVars = vars;
...
function ensureMermaid() {
  return ensureMermaidAsset().then(function () {
...
export function renderMarkdown(text) {
  ...
  var normalized = text
    .replace(/[\u201C\u201D]/g, '"')
    .replace(/[\u2018\u2019]/g, "'");
  return DOMPurify.sanitize(marked.parse(normalized), MARKDOWN_SANITIZE_OPTIONS);
}
...
export function highlightCodeBlocks(el) {
  ...
  // 4-8s. After history_done, app-messages.js calls the CHUNKED variant on
  // the full messages container, so unhighlighted blocks (selector
  // :not(.hljs)) are picked up progressively without freezing the main thread.
  if (store.get('replayingHistory')) return;
  el.querySelectorAll("pre code:not(.hljs):not(.language-mermaid)").forEach(highlightOneBlock);
  injectCopyButtons(el);
}

// Post-replay pass: highlighting hundreds of blocks in one synchronous sweep
// blocked the main thread for multiple seconds — starving the WS pong timers
// and the 3s connect handshake guard, which then read as "random reconnects"
// (see the [clay-perf] instrumentation and app-connection.js). Highlighting
// does not change a block's height (same text, same font — colors only), so a
// progressive pass does NOT re-trigger the sticky-bottom reflow chase that
// per-message highlighting during replay caused. Each batch yields back to the
// event loop so pongs, streams, and input stay responsive.
export function highlightCodeBlocksChunked(el, opts) {
  var batchSize = (opts && opts.batchSize) || 12;
  var onDone = opts && opts.onDone;
  var blocks = Array.prototype.slice.call(
    el.querySelectorAll("pre code:not(.hljs):not(.language-mermaid)")
  );
  var idx = 0;
  function step() {
    var end = Math.min(idx + batchSize, blocks.length);
    for (; idx < end; idx++) {
      var block = blocks[idx];
      // A live-stream message may have highlighted this block since the
      // snapshot was taken — skip it rather than re-highlighting.
      if (block.classList.contains("hljs")) continue;
      try { highlightOneBlock(block); } catch (e) {}
    }
    if (idx < blocks.length) {
      setTimeout(step, 0);
      return;
    }
    // Copy buttons are cheap next to hljs; one pass at the end keeps the
    // per-batch work purely about highlighting.
    injectCopyButtons(el);
    if (onDone) onDone(blocks.length);
  }
  step();
}
...
export function renderMermaidBlocks(el, opts) {
  opts = opts || {};
  ...
  var blocks = Array.prototype.slice.call(el.querySelectorAll("pre code.language-mermaid"));
  var onDomChange = opts.onDomChange;
  ...
  function applyDomChange(fn) {
    if (onDomChange) onDomChange(fn);
    else fn();
  }
  ...
  function next() {
    ...
    var codeEl = blocks[idx++];
    var pre = codeEl.parentElement;
    if (!pre || pre.dataset.mermaidProcessed) { next(); return; }
    pre.dataset.mermaidProcessed = "true";

    var source = codeEl.textContent;
    if (!source || !source.trim()) { next(); return; }
  ...
  ensureMermaid().then(next).catch(function () {

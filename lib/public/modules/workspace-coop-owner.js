...
export function setWorkspaceOwnerTasks(tasks) {
  ownerTasks = Array.isArray(tasks) ? tasks : [];
}
...
export function renderWorkspaceCoopOwner(container, options) {
  ...
  var input = options || {};
  var opts = Object.assign({}, input, {
    details: store.get("coopOwnerLedgerDetails") || {},
    onDetailsChange: function (details) { store.set({ coopOwnerLedgerDetails: details }); },
  });
  container.innerHTML = "";
  var selectedTab = input.selectedTab === "tasks" ? "tasks" : "overview";
  var tabs = document.createElement("div");
  tabs.innerHTML = workspaceTabsHtml(selectedTab);
  container.appendChild(tabs.firstChild);
  var tabPanel = document.createElement("div");
  tabPanel.className = "ws-tab-panel";
  tabPanel.setAttribute("role", "tabpanel");
  container.appendChild(tabPanel);
  wireWorkspaceTabs(container, selectedTab, function (next) {
    if (typeof input.onTabSelect === "function") input.onTabSelect(next);
  });
  if (selectedTab === "tasks") {
    tabPanel.innerHTML = workspaceTasksHtml(input.tasks || ownerTasks);
    wireWorkspaceTaskLinks(tabPanel, input.send);
    refreshIcons();
    return true;
  }

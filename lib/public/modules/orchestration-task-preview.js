...
var stagedApprovalPending = Object.create(null);
...
function canOpenTaskSession(task) {
  return !!(task && (task.workerSessionRef ||
    (Number.isInteger(task.workerSessionId) && task.workerSessionId > 0)));
}

function openTaskSession(task) {
  if (!task) return false;
  if (task.workerSessionRef) {
    return requestCanonicalSession(
      task.workerSessionRef,
      sendWsJson,
      "owner_request_hierarchy"
    );
  }
  if (!Number.isInteger(task.workerSessionId) || task.workerSessionId <= 0) return false;
  return sendWsJson({ type: "switch_session", id: task.workerSessionId });
}

function stagedApprovalScope(task) {
  var set = task && task.approvalSet;
  var scopes = set && Array.isArray(set.scopes) ? set.scopes : [];
  if (!task || task.status !== "waiting_user" || !String(set && set.setId || "").trim() ||
      !Number.isFinite(Number(set && set.stagedAt)) || !String(task.userQuestion || "").trim()) {
    return null;
  }
  for (var i = 0; i < scopes.length; i++) {
    var scope = scopes[i] || {};
    var taskId = String(scope.portfolioTaskId || "").trim();
    var revision = Number(scope.bindingRevision);
    var projectId = String(scope.targetProject && scope.targetProject.projectId || "").trim();
    if (!/^[A-Za-z0-9][A-Za-z0-9_-]{0,159}$/.test(taskId) ||
        !Number.isInteger(revision) || revision < 1 || !projectId) continue;
    if (String(task.clientRef || "") === "portfolio:" + taskId + ":" + revision) {
      return { portfolioTaskId: taskId, bindingRevision: revision, projectId: projectId };
    }
  }
  return null;
}

// One staged set can contain several independently approvable revisions. The
// set id binds the ProjectRef and every staged member, while clientRef binds
// this row's exact task/revision within that set. Keep the identity stable so a
// re-render or reconnect retries the same durable ingress rather than minting
// a second owner approval.
function stagedApprovalIdentity(task) {
  var scope = stagedApprovalScope(task);
  var setId = String(task && task.approvalSet && task.approvalSet.setId || "").trim();
  var clientRef = String(task && task.clientRef || "").trim();
  if (!scope || !setId || !clientRef) return "";
  return "coop-approval:" + setId + ":" + clientRef;
}

function approvalPending(task) {
  var identity = stagedApprovalIdentity(task);
  var pending = identity && stagedApprovalPending[identity];
  var ws = getWs();
  if (!pending || pending.ws !== ws || !ws || ws.readyState !== 1) return null;
  return pending;
}

function approvalButtonText(pending) {
  if (!pending) return "Approve";
  return pending.state === "accepted" ? "Approval received" : "Sending…";
}

function updateApprovalControl(pending) {
  var control = pending && pending.control;
  if (!control) return;
  control.button.disabled = true;
  control.button.textContent = approvalButtonText(pending);
  control.error.textContent = "";
}

function markApprovalPending(task, button, error) {
  var identity = stagedApprovalIdentity(task);
  if (!identity) return;
  stagedApprovalPending[identity] = {
    ws: getWs(),
    state: "sending",
    control: { button: button, error: error },
  };
}

function registerApprovalControl(task, button, error) {
  var pending = approvalPending(task);
  if (!pending) return null;
  pending.control = { button: button, error: error };
  return pending;
}

export function handleStagedApprovalMessageAccepted(clientMessageId) {
  var identity = String(clientMessageId || "").trim();
  var pending = identity && stagedApprovalPending[identity];
  if (!pending) return false;
  pending.state = "accepted";
  updateApprovalControl(pending);
  return true;
}

export function handleStagedApprovalMessageFailed(clientMessageId, message) {
  var identity = String(clientMessageId || "").trim();
  var pending = identity && stagedApprovalPending[identity];
  if (!pending) return false;
  var control = pending.control;
  delete stagedApprovalPending[identity];
  if (control) {
    control.button.disabled = false;
    control.button.textContent = "Approve";
    control.error.textContent = String(message || "Approval was not received. Try again.");
  }
  return true;
}

function clearInactiveApprovalPending(tasks) {
  var active = Object.create(null);
  var list = Array.isArray(tasks) ? tasks : [];
  for (var i = 0; i < list.length; i++) {
    var identity = stagedApprovalIdentity(list[i]);
    if (identity) active[identity] = true;
  }
  Object.keys(stagedApprovalPending).forEach(function (identity) {
    if (!active[identity]) delete stagedApprovalPending[identity];
  });
}

function approvalClickMessage(task) {
  var scope = stagedApprovalScope(task);
  var approvalIdentity = stagedApprovalIdentity(task);
  var sessionId = store.get("activeSessionId");
  if (!scope || !approvalIdentity || !store.get("activeCoopHome") || sessionId == null) return null;
  return {
    type: "message",
    text: "Approve " + scope.portfolioTaskId + " revision " + scope.bindingRevision +
      " implementation for ProjectRef " + scope.projectId,
    clientMessageId: approvalIdentity,
    intent: "chat",
    sessionId: sessionId,
    // Approval placeholders are canonical Coop control-plane state. A click
    // must not inherit a topic/project lens and accidentally attribute the
    // exact approval to whichever filtered view happens to be open.
...
function createTaskRow(task) {
  ...
  var approval = stagedApprovalScope(task);
  var open = null;
  if (approval) {
    var approvalError = document.createElement("span");
    approvalError.className = "orchestration-task-approval-error";
    approvalError.setAttribute("role", "alert");
    body.appendChild(approvalError);
    open = document.createElement("button");
    open.type = "button";
    open.className = "orchestration-reconciliation-retry orchestration-task-approve";
    var pendingApproval = registerApprovalControl(task, open, approvalError);
    open.disabled = !!pendingApproval;
    open.textContent = approvalButtonText(pendingApproval);
    open.setAttribute("aria-label", "Approve " + approval.portfolioTaskId + " revision " +
      approval.bindingRevision);
    open.addEventListener("click", function (event) {
      event.preventDefault();
      event.stopPropagation();
      if (open.disabled) return;
      var message = approvalClickMessage(task);
      if (!message || !sendWsJson(message)) {
        approvalError.textContent = "Not connected. Try again.";
        return;
      }
      approvalError.textContent = "";
      markApprovalPending(task, open, approvalError);
      open.disabled = true;
      open.textContent = "Sending…";
    });
  } else {
...
export function renderOrchestrationTaskPreview(host, tasks, coordinatorState) {
  host.innerHTML = "";
  clearInactiveApprovalPending(tasks);

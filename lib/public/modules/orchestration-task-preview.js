...
function approvalIsPending(task) {
  var identity = stagedApprovalIdentity(task);
  var pending = identity && stagedApprovalPending[identity];
  var ws = getWs();
  return !!(pending && pending.ws === ws && ws && ws.readyState === 1);
}

function markApprovalPending(task) {
  var identity = stagedApprovalIdentity(task);
  if (identity) stagedApprovalPending[identity] = { ws: getWs() };
}
...
function createTaskRow(task) {
  ...
  if (approval) {
    ...
    open.disabled = approvalIsPending(task);
    open.textContent = open.disabled ? "Approval sent" : "Approve";
    ...
    open.addEventListener("click", function (event) {
      ...
      markApprovalPending(task);
      open.disabled = true;
      open.textContent = "Approval sent";
    });
  } else {

...
function renderSlashCommandResult(msg) {
  ...
  var text = msg.text || "";
  var pending = msg.pending || /^Scanning .+ tasks…$/.test(text);
  if (pending) {
    var chip = document.createElement("div");
    chip.style.cssText = "display:inline-flex;align-items:center;gap:8px;background:var(--code-bg);border:1px solid var(--border-subtle);border-radius:999px;padding:8px 12px;color:var(--text-secondary);font-size:13px;line-height:1.3";
    chip.innerHTML = iconHtml("loader", "icon-spin");
    var label = document.createElement("span");
    label.textContent = text;
    chip.appendChild(label);
    cmdBlock.appendChild(chip);
    addToMessages(cmdBlock);
    refreshIcons(cmdBlock);
    scrollToBottom();
    return;
  }

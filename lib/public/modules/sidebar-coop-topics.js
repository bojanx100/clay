...
function controlSessionTitle(section) {
  var role = text(section && section.role, "").toLowerCase();
  if (role === "council") return "Council review";
  if (role === "triage") return "Triage review";
  return text(section && section.title, section && section.label);
}
...
function appendControlResult(container, result, options) {
  ...
  var resultParts = [text(result.projectTitle, ""), text(result.question, "")].filter(function (value) {
    return !!value;
  });
  if (resultParts.length) {
    var context = document.createElement("span");
    context.className = prefix + "coop-control-result-context";
    context.textContent = resultParts.join(" · ");
    row.appendChild(context);
  }
  row.addEventListener("click", function () {
    var send = typeof opts.send === "function" ? opts.send : sendUserAction;
    if (result.topicRef && requestCoopTopic({ topicRef: result.topicRef, projectRef: null }, send)) {
      finishNavigation(opts);
      return;
    }
    if (result.executionRef && requestCanonicalSession(result.executionRef, send)) finishNavigation(opts);
  });
...
function appendControlPlaneSession(container, section, options) {
  ...
  title.textContent = controlSessionTitle(section);
  title.setAttribute("title", text(section.title, section.label));
  row.appendChild(title);
  var state = document.createElement("span");
  state.className = prefix + "coop-control-plane-state";
  state.textContent = label;
  row.appendChild(state);
  var contextParts = [text(section.projectTitle, ""), text(section.activity, "")].filter(function (value, index, values) {
    return !!value && values.indexOf(value) === index;
  });
  if (contextParts.length) {
    var context = document.createElement("span");
    context.className = prefix + "coop-control-plane-context";
...
export function renderCoopTopicSections(container, model, options) {
  ...
  var prefix = opts.mobile ? "mobile-" : "";
  // Current task cards remain narrow orientation controls. Their session ref
  // comes from the server-built action item and resolves through the existing
  // ACL-scoped session resolver before any project switch, so activation never
  // creates a replacement session or falls back to a transcript-less shell.
  renderCoopNowIndex(container, Object.assign({}, opts, {
    ...
    openSession: function (sessionRef) {
      return requestCanonicalSession(sessionRef, sendUserAction, "owner_request_hierarchy");
    },
  }));

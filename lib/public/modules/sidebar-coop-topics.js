...
function topicActivity(topic) {
  ...
  return WORK_STATE_LABELS[state] || WORK_STATE_LABELS.working;
}
...
function createTopicRow(topic, options) {
  ...
  activityEl.className = prefix + "coop-topic-activity " + prefix +
    "coop-topic-activity-" + topicStatusClass(topic.workState || "working");
  activityEl.textContent = activity;
  // Not aria-hidden: the row's aria-label already includes the state, but the
  // visible text must also be a real, selectable, translatable string rather
  // than a colour a screen reader and a colour-blind owner both miss.

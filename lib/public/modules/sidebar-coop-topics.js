...
export function renderCoopTopicGroup(container, label, topics, options) {
  ...
  for (var i = 0; i < items.length; i++) group.appendChild(createTopicRow(items[i], opts));
...
function appendProjectSection(container, section, options) {
  ...
  container.appendChild(group);
  return section.hierarchy.length;
}
...
function appendControlPlaneSession(container, section, options) {
  ...
  row.className = prefix + "coop-control-plane-row " + prefix +
    "coop-control-plane-" + text(section.role, "session");
  row.textContent = text(section.label, "Control plane");
  row.setAttribute("aria-label", "Open " + row.textContent);
  row.addEventListener("click", function () {
...
export function renderCoopTopicSections(container, model, options) {
  ...
  for (var i = 0; i < sections.length; i++) {
    if (sections[i].kind === "project_coordinator") appendProjectSection(container, sections[i], opts);
    else if (sections[i].kind === "control_plane") appendControlPlaneSession(container, sections[i], opts);
    else renderCoopTopicGroup(container, sections[i].label, sections[i].topics,
      Object.assign({}, opts, { allowEmpty: sections[i].kind === "threads" }));
  }
  return container;
}

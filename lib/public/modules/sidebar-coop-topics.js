...
import { renderOwnerRequestPanel } from './coop-owner-requests-panel.js';
import { ensureOwnerRequestOverview } from './coop-owner-requests-refresh.js';
...
export function renderCoopTopicSections(container, model, options) {
  ...
  var prefix = opts.mobile ? "mobile-" : "";
  // Reaching the Coop sections IS the view activation: this is the only place
  // either surface draws them. The refresh module coalesces the stream of
  // renders into at most one server query per window, so this is a caller, not
  // a poll.
  ensureOwnerRequestOverview();
  // What the owner asked for and never got outranks everything else on this
  // surface, including what is happening right now, so it is drawn first.
  renderOwnerRequestPanel(container, Object.assign({}, opts, { send: sendUserAction }));

...
// and a collapsed expander listing related top-level project sessions. The
// Review decision flow renders on the quiet meta line only when actionable.
// Nothing else is offered inline.
...
function createTopicRow(topic, options) {
  ...
  // The quiet secondary line: state words (with the reinforcing dot) and the
  // single Review action when one is actionable. A stateless, non-reviewable
  // row renders no meta line at all -- no reserved blank second line.
  if (activity || review) {
    ...
    if (review) meta.appendChild(review);

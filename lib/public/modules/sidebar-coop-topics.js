...
function appendControlResult(container, result, options) {
  ...
  row.addEventListener("click", function () {
    ...
    if (result.executionRef && requestCanonicalSession(result.executionRef, send)) {
      finishNavigation(opts);
      return;
    }
    if (!result.topicRef || !requestCoopTopic({ topicRef: result.topicRef, projectRef: null }, send)) return;
    finishNavigation(opts);
  });

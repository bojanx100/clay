...
function createTopicRow(topic, options) {
  ...
  // Attention lives on the wrapper: the state marker sits on the meta line
  // below the row button, outside the button's subtree.
  ...
  // The title is the primary content and owns the row. The state moved to a
  // quiet secondary line so 31 identical uppercase pills stop competing with
  // the titles -- but it stays in the accessible name, so a screen reader
  // still hears "title, state" on the row itself.
  ...
  // The quiet secondary line: state words with the reinforcing dot. A
  // stateless row renders no meta line at all -- no reserved blank second
  // line. Decisions are made in the topic decision surface, never here.
  if (activity) {
    var meta = document.createElement("div");
    meta.className = prefix + "coop-topic-meta";
    // The dot reinforces the text label; it is never the only carrier of the
    // state.
...
export function renderCoopTopicSections(container, model, options) {
  ...
  var prefix = opts.mobile ? "mobile-" : "";
  // What needs the boss comes first, above the lenses and the topic list: a
  // link-only index whose rows open the canonical topic the decision lives in.
  renderCoopActionQueue(container, Object.assign({}, opts, {
    openTopic: function (item) {
      var found = findGlobalCoopTopic(item.topicRef, item.projectRef);

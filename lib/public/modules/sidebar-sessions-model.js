...
var TERMINAL_COOP_STATUSES = {
  completed: true, dismissed: true, superseded: true,
};
...
function provenTerminalCoopProjection(session) {
  if (session && session.coopTerminal === true) return true;
...
export function sessionsForOrdinaryProjectSidebar(sessions) {
  ...
  return list.filter(function (session) {
    // Persistent project coordinators are Coop control-plane sessions. Legacy
    // target-local roots may remain on disk during migration, but they are not
    // project navigation. Their task-coordinator children naturally become
    // top-level when this parent is absent from the visible list.
    // The wire format deliberately strips the private Coop SessionRef and
    // exposes only the validated leadOwned boolean. Requiring the private
    // coopControlledBy object here let every real broadcast root leak through
    // even though hand-built tests carrying that object appeared correct.
    if (session && session.coordinationRole === "project_coordinator" &&
        session.leadOwned) return false;

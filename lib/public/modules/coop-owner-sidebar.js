...
function appendWorkerDetail(panel, detail, prefix) {
  ...
  provenance.textContent = "Project " + text(projectRef.projectId, "unavailable") +
    " · Worker session " + text(sessionRef.sessionStorageId, "unavailable");
  panel.appendChild(provenance);
  if (detail.sessionRef || detail.sourceSessionRef) {
    appendButton(panel, prefix + "coop-owner-link", "Open worker session", function () {
      openSession(detail.sessionRef || detail.sourceSessionRef, options);
    }, "Open the canonical worker session");
  }
}
...
function appendDestinations(row, entry, options, prefix) {
  ...
  if (entry.sourceSessionRef) {
    var sourceLabel = isDynamicAction(entry) ? "Worker session" : "Original request";
    appendButton(destinations, prefix + "coop-owner-link", sourceLabel, function () {
      openSession(entry.sourceSessionRef, options);
    }, isDynamicAction(entry) ? "Open the canonical worker session" :

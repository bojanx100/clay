...
function appendWorkerDetail(panel, detail, prefix) {
  var isResult = detail.type === "worker_result";
  var sessionLabel = detail.sourceKind === "source" ? "Source session" : "Worker session";
  var openLabel = detail.sourceKind === "source" ? "Open source session" : "Open worker session";
  ...
  body.textContent = isResult
    ? text(detail.resolution, "Worker reported completion")
    : text(detail.question, "Needs your decision");
  ...
  provenance.textContent = "Project " + text(projectRef.projectId, "unavailable") +
    " · " + sessionLabel + " " + text(sessionRef.sessionStorageId, "unavailable");
  panel.appendChild(provenance);
  if (detail.sessionRef || detail.sourceSessionRef) {
    appendButton(panel, prefix + "coop-owner-link", openLabel, function () {
      openSession(detail.sessionRef || detail.sourceSessionRef, options);
    }, "Open the canonical " + sessionLabel.toLowerCase());
  }
}
...
function appendDestinations(row, entry, options, prefix) {
  ...
  if (entry.sourceSessionRef) {
    var dynamicSource = entry.action && entry.action.workerDetail && entry.action.workerDetail.sourceKind === "source";
    var sourceLabel = isDynamicAction(entry) ? (dynamicSource ? "Source session" : "Worker session") : "Original request";
    appendButton(destinations, prefix + "coop-owner-link", sourceLabel, function () {
      openSession(entry.sourceSessionRef, options);
    }, isDynamicAction(entry) ? (dynamicSource ? "Open the canonical source session" : "Open the canonical worker session") :
...
function createRow(entry, sidebar, options) {
  ...
  title.addEventListener("click", function (event) {
    ...
    else if (!openThread(entry, options)) toggleDetail(entry, options);
  });
...
function appendAttentionSection(container, sidebar, options) {
  ...
  var heading = document.createElement("h2");
  heading.className = prefix + "coop-owner-heading";
  heading.textContent = "Needs attention (" + count + ")";
  section.appendChild(heading);
  for (var gi = 0; gi < groups.length; gi++) {
    ...
    projectHeading.className = prefix + "coop-owner-project-heading";
    projectHeading.textContent = text(group.title, "Unassigned") + " (" + entries.length + ")";
    project.appendChild(projectHeading);
    for (var ei = 0; ei < entries.length; ei++) project.appendChild(createRow(entries[ei], sidebar, options));

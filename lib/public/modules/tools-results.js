...
function expandToolGroupForImages(tool, images) {
  if (!tool || !tool.groupId || !images || images.length === 0) return;
  var group = findToolGroup(tool.groupId);
  if (!group || !group.el) return;
  group.autoExpanded = true;
...
export function updateToolResult(id, content, isError, images) {
  ...
  expandToolGroupForImages(tool, images);
...
function renderImageToolResult(resultBlock, tool, images) {
  ...
  } else if (tool.input && tool.input.file_path) {
  ...
  resultBlock.className = "tool-result-block";
  tool.el.classList.add("expanded");
  // A result that carried only unusable image entries (no url, no data) must
  // not render as a blank, error-less block — show an explicit placeholder.
  if (appended === 0) {
    renderPlainToolResult(resultBlock, "(no image to display)", true);
  }
}

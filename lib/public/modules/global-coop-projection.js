...
function cloneProjection(message) {
  ...
  return {
    ...
    controlPlaneSessions: safeArray(message.controlPlaneSessions).map(function (item) {
      return {
        role: safeText(item && item.role, ""),
        title: safeText(item && item.title, "Control plane"),
        sessionRef: cloneReference(item && item.sessionRef),
        status: safeText(item && item.status, "idle"),
        activity: safeText(item && item.activity, ""),
        processing: item && item.processing === true && item.status === "running",
        topicRef: cloneReference(item && item.topicRef),
      };
    }).filter(function (item) {
      return !!(item.sessionRef && (item.role === "council" || item.role === "triage"));
    }),

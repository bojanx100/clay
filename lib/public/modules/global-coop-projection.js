...
import { normalizeActionQueue, setActionQueue, normalizeNowIndex, setNowIndex } from './coop-action-queue-ui.js';
import { cloneGlobalCoopProject } from './global-coop-project-model.js';
import { normalizeControlPlaneSessions } from './global-coop-control-normalization.js';
var projection = null;
var LENS_QUERY_KEY = "coopProject";
var TOPIC_QUERY_KEY = "coopTopic";
// A dedicated key, because absence of both other keys already MEANS All. A new
// named lens cannot be expressed by omission without colliding with it.
var SCOPE_QUERY_KEY = "coopLens";
var MAIN_SCOPE = "main";
...
function cloneProjection(message) {
  ...
  var projects = safeArray(message.projects).map(cloneGlobalCoopProject);
  ...
  return {
    ...
    controlPlaneSessions: normalizeControlPlaneSessions(message.controlPlaneSessions),
    controlPlaneResults: safeArray(message.controlPlaneResults)
      .map(cloneControlResult).filter(Boolean),
    // This is a bounded, reference-only server view. Rows still resolve their
    // destinations against the normalized topic projection before navigating.
    ownerSidebar: message.ownerSidebar && typeof message.ownerSidebar === "object"
      ? message.ownerSidebar : null,
  };
}

function projectMatches(project, query) {
  if (!query) return true;
  function coordinatorText(node) {
    return [node && node.title, node && node.status, node && node.activity]
      .concat(safeArray(node && node.children).map(coordinatorText)).join(" ");
  }
  var text = [project.title, project.slug]
    .concat((project.topics || []).map(topicText))
    .concat((project.summary && project.summary.coordinatorTree || []).map(coordinatorText))
    .join(" ").toLowerCase();
  return text.indexOf(query) !== -1;
}

export function setGlobalCoopProjection(message, send) {
  projection = cloneProjection(message);
  // The owner queue rides the same projection push, so it stays in step with
  // everything else the sidebar shows.
  setActionQueue(normalizeActionQueue(message));
  setNowIndex(normalizeNowIndex(message));
  // A null projection is a reconnect gap. Only a complete server projection
  // may invalidate references, keeping the selected title stable offline.
  if (projection) {
    ...
    var activeTopicRef = store.get("activeCoopTopicRef") || (store.get("activeCoopLens") || {}).topicRef;
    var activeProjectRef = store.get("activeCoopProjectRef") || (store.get("activeCoopLens") || {}).projectRef || null;
    if (activeTopicRef && !findGlobalCoopTopic(activeTopicRef, activeProjectRef)) {
      clearActiveTopicLens(false);
      store.set({ activeCoopLensScope: MAIN_SCOPE });
      beginCoopSelection(null, null, MAIN_SCOPE, "replaceState", send);
    }
  }
...
export function projectLensPath(pathname, search, projectRef, topicRef, scope) {
  var params = new URLSearchParams(search || "");
  if (scope === MAIN_SCOPE) params.set(SCOPE_QUERY_KEY, MAIN_SCOPE);
...
function syncProjectLensUrl(projectRef, method, scope) {
  if (typeof location === "undefined" || typeof history === "undefined") return;
  if (location.pathname !== "/p/lead/") return;
  var next = projectLensPath(location.pathname, location.search, projectRef, null, scope);
...
function syncTopicLensUrl(projectRef, topicRef, method) {
  if (typeof location === "undefined" || typeof history === "undefined") return;
  if (location.pathname !== "/p/lead/") return;
  var next = projectLensPath(location.pathname, location.search, projectRef, topicRef, null);
  if (next === location.pathname + location.search) return;
  history[method || "pushState"](null, "", next);
}
...
export function syncCoopLensFromUrl(send) {
  ...
  var scope = "";
  try { scope = new URLSearchParams(location.search).get(SCOPE_QUERY_KEY) || ""; } catch (e) {}
  ...
  if (!projectId) {
    if (typeof send !== "function") return false;
    // No topic and no project: Main unless the URL explicitly asks for All.
    // Main is the default the owner lands on when entering Coop; All stays
    // reachable and is what a shared or bookmarked All URL restores.
    var wanted = scope === MAIN_SCOPE || scope === "" ? MAIN_SCOPE : "canonical";
    var settled = !store.get("activeCoopTopicRef") && !store.get("activeCoopProjectRef") &&
      !store.get(PENDING_SELECTION_KEY) && activeCoopLensScope() === wanted;
    if (settled) return true;
    return beginCoopSelection(null, null, wanted, "replaceState", send);
  }
  for (var i = 0; i < projects.length; i++) {
...
export function buildGlobalCoopDisplayModel(searchQuery) {
  ...
  return {
    ...
    controlPlaneSessions: projection && projection.controlPlaneSessions || [],
    controlPlaneResults: projection && projection.controlPlaneResults || [],
    ownerSidebar: projection && projection.ownerSidebar || null,
  };
}
...
// Main is the owner-facing default: the whole conversation with execution
// narration removed. It is a server replay scope like the others, so what the
// owner sees and what the server thinks the lens contains cannot drift.
export function requestMainCoopLens(send) {
  return beginCoopSelection(null, null, MAIN_SCOPE, "pushState", send);
}
export function requestRecoveredMainCoopLens(send) { return beginCoopSelection(null, null, MAIN_SCOPE, "replaceState", send); }

export function activeCoopLensScope() {
  var explicitScope = store.get("activeCoopLensScope");
  if (explicitScope === "main" || explicitScope === "canonical" ||
      explicitScope === "topic" || explicitScope === "project") return explicitScope;
  if (store.get("activeCoopTopicRef")) return "topic";
  if (store.get("activeCoopProjectRef")) return "project";
  return "canonical";
}
...
function commitPendingSelection(pending) {
  ...
  if (pending.topicRef) {
    ...
    store.set({ activeCoopLens: lens, activeCoopTopicRef: null, activeCoopProjectRef: lens.projectRef,
      activeCoopLensScope: "project", activeCoopTopicStale: false });
  } else if (pending.historyScope === MAIN_SCOPE) {
    syncProjectLensUrl(null, pending.historyMethod, MAIN_SCOPE);
    clearActiveTopicLens();
    store.set({ activeCoopLensScope: MAIN_SCOPE });
  } else {
    syncProjectLensUrl(null, pending.historyMethod);
    clearActiveTopicLens();
    store.set({ activeCoopLensScope: "canonical" });
  }
...
export function handleCoopTopicSelected(message) {
  ...
  if (!selectionMatches(message, pending.topicRef, pending.projectRef)) return true;
  var coop = getGlobalCoopReference();
...
export function handleCoopTopicResult(message) {
  if (!message || message.type !== "coop_topic_result" && message.type !== "coop_thread_result") return false;
...
export function requestCanonicalEvent(eventRef, topicRef, projectRef, send) {
  if (!eventRef || typeof send !== "function") return false;
  ...
  // The client projection intentionally contains only bounded first/last
  // previews. An owner-ledger row can still carry an exact middle-event ref,
  // so the server remains the authority for resolving it instead of making
  // navigation depend on whether that event happened to be previewed.
  if (!exactEvent && eventRef.sessionStorageId &&
      Number.isInteger(eventRef.eventIndex) && eventRef.eventIndex >= 0) {
    exactEvent = eventRef;
  }
  if (!exactEvent || !topicRef) return false;
  return send({ type: "resolve_canonical_event", eventRef: cloneReference(exactEvent),
    topicRef: cloneReference(selected ? selected.topicRef : topicRef),
    projectRef: cloneReference(selected ? selected.projectRef : projectRef) }) !== false;
}

export function requestCanonicalSession(sessionRef, send, scope) {
  if (!sessionRef || typeof send !== "function") return false;
  var message = { type: "resolve_session_ref", sessionRef: sessionRef };

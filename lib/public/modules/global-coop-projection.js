...
function isCoopGroup(group) {
  return !!(group && !group.legacyLead &&
    (stableProjectId(group) === "system-lead" || group.isCoop === true));
}

function cloneProjection(message) {
  if (!message || message.type !== "global_coop_projection") return null;
  return { type: message.type, coop: message.coop || null, projects: safeArray(message.projects) };
}
...
function projectDisplay(group, depth) {
  ...
  return {
    ...
    legacyLead: !!(group && group.legacyLead),
    unavailableProject: !!(group && group.unavailableProject),
    attention: !!(group && group.attention),

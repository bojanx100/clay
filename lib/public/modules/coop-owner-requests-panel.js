...
import { requestCanonicalEvent, requestCanonicalSession } from './global-coop-projection.js';
...
function createTreeElement(row) {
  var node = document.createElement(row.sessionRef ? "button" : "div");
  if (row.sessionRef) node.type = "button";
  return node;
}
...
function createTreeNode(row, options) {
  var opts = options || {};
  var prefix = opts.mobile ? "mobile-" : "";
  var node = createTreeElement(row);
  ...
  var canOpen = !!(row.sessionRef && !row.hidden && (row.live || row.present));
  if (row.sessionRef && !canOpen) {
    node.disabled = true;
    node.setAttribute("title", "Session unavailable");
  }
  ...
  if (canOpen && typeof opts.send === "function") {
    node.addEventListener("click", function () {
      if (!requestCanonicalSession(row.sessionRef, opts.send, "owner_request_hierarchy")) return;
      if (typeof opts.onNavigate === "function") opts.onNavigate();
    });
  }
  return node;

...
// Transient per-modal state, deliberately not persisted: Coop-managed rows are
// mostly auto-archived orchestration work, so the picker always opens on the
// clean list and the owner opts in when they are hunting for a closed
// Coop-owned session.
var importPickerIncludeCoopManaged = false;
...
export function openImportSessionPicker(vendorFilter) {
  ...
  importPickerIncludeCoopManaged = false;

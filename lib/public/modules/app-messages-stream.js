...
import { autoStartLoginIfNeeded } from './app-notifications.js';
...
import { addUserMessage, addSystemMessage, removeMatePreThinking, appendDelta, replaceAssistantText, finalizeAssistantBlock, addConflictMessage, addContextOverflowMessage, scrollToBottom } from './app-rendering.js';
...
function handleAuthRequired(msg) {
  ...
  setStatus("connected");
  if (!store.get('loopActive')) enableMainInput();
  // Never auto-launch the login flow for replayed history: old auth_required
  // messages persist in the session transcript, and re-running the login
  // command on session open is destructive (codex login wipes auth.json
  // before the new flow completes, logging the user out for real).

...
import { resetClientState, openResolvedGlobalSession } from './app-projects.js';
...
import { setGlobalCoopProjection, consumeGlobalSessionRefResolution } from './global-coop-projection.js';
...
var sessionMessageHandlers = {
  ...
  session_ref_resolved: handleGlobalSessionRefResolved,
...
function handleGlobalSessionRefResolved(msg) {
  var result = consumeGlobalSessionRefResolution(msg);
  if (!result) return;
  if (result.ok) {
    openResolvedGlobalSession(result);
    return;
  }
  // Resolution errors stay on the referenced global row.  Do not show a
  // toast, switch scope, or manufacture a replacement conversation.
  renderSessionList(null);
}

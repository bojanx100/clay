...
import { getInputDraft, restoreInputDraft, saveInputDraftForSession, restoreInputDraftForSession } from './input.js';
...
import { syncListedExecutionAuthority } from './execution-authority-ui.js';
...
var sessionMessageHandlers = {
  ...
  coop_thread_result: handleTopicResult,
...
function handleSessionList(msg) {
  ...
  syncListedExecutionAuthority(msg);
  var coopHome = null;
...
function handleGlobalCoopProjection(msg) {
  setGlobalCoopProjection(msg, sendCoopTopicMessage);
...
function handleTopicSelected(msg) {
  ...
  else showToast("That Coop Thread is unavailable or you no longer have access.", "error");
  renderSessionList(null);
}

function handleTopicResult(msg) {
  handleCoopTopicResult(msg);
  if (!msg.ok) showToast("That Thread change could not be applied.", "error");
}
...
function initialDraftForSessionSwitch(previousSessionId, draft) {
...
function handleSessionSwitched(msg) {
  ...
  var initialDraft = initialDraftForSessionSwitch(prevSid, getInputDraft());
  if (prevSid) {
  ...
  if (initialDraft) {
    restoreInputDraft(initialDraft);

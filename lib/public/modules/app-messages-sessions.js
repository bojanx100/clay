...
function handleSessionSwitched(msg) {
  ...
  // Keyed on the project, not on coopHome. A project channel or a topic lens is
  // still Coop, but it arrives with the target project's title as msg.title --
  // which is how selecting a project used to repaint the header away from Coop.
  var headerTitle = coopHeaderTitle(currentSlug, msg.coopHome ? "Coop" : msg.title, "Clay");
  if (headerTitleEl) headerTitleEl.textContent = headerTitle;
  if (isCoopProjectSlug(currentSlug)) {
    var projectTitleEl = document.getElementById("title-bar-project-name");

...
import { handleOwnerRequestOverview } from './coop-owner-requests.js';
import { invalidateOwnerRequestOverview } from './coop-owner-requests-refresh.js';
...
import { handleCoopIncarnationResult } from './coop-incarnation-controls.js';
...
var sessionMessageHandlers = {
  ...
  coop_owner_requests: handleOwnerRequests,
  coop_incarnation_result: handleCoopIncarnationResult,
...
function handleGlobalCoopProjection(msg) {
  ...
  // A new projection is the one signal that the owner-facing backlog may have
  // moved. Mark it stale rather than querying here: the render below is the
  // caller, and the refresh module coalesces a burst of projections into one
  // request.
  invalidateOwnerRequestOverview();
  syncCoopLensFromUrl(sendCoopTopicMessage);
  renderSessionList(null);
}

// The overview handler only lands the answer in the store. The Coop sidebar is
// what draws the backlog, so without this repaint the panel stays invisible
// until some unrelated render happens to come along.
function handleOwnerRequests(msg) {
  var handled = handleOwnerRequestOverview(msg);
  if (handled && store.get('currentSlug') === "lead") renderSessionList(null);
  return handled;
}

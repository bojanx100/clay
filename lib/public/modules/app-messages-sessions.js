...
function updateCoordinatorBadge(coordinationMode, orchestrationParent) {
  ...
  if (orchestrationParent) {
    ...
    badge.disabled = true;
    badge.title = "This session coordinates parallel work";
    if (label) label.textContent = "Coordinator";
  } else {
...
export function handleSessionMessage(msg) {
  switch (msg.type) {
    ...
    case "coordinator_status":
      store.set({ activeCoordinationMode: !!msg.coordinationMode });
      updateCoordinatorBadge(!!msg.coordinationMode, store.get("activeOrchestrationParent"));
      return true;
...
function handleSessionSwitched(msg) {
  ...
  var sessionSwitchUpdate = { activeSessionId: msg.id, activeSessionTitle: msg.title || "", cliSessionId: msg.cliSessionId || null, vendorCapabilities: msg.capabilities || {}, sessionIsProcessing: !!msg.isProcessing, activeSessionMode: effectiveMode, activeTerminalId: effectiveTerminalId, sessionHasHistory: !!msg.hasHistory, currentProviderRouteId: msg.providerRouteId || null, requestedModel: msg.requestedModel || "", verifiedModel: msg.verifiedModel || "", modelVerificationSource: msg.modelVerificationSource || "", activeCoordinationMode: !!msg.coordinationMode, activeOrchestrationParent: msg.orchestrationParent || null };

...
import { setGlobalCoopProjection, syncCoopLensFromUrl } from './global-coop-projection.js';
...
function handleSessionSwitched(msg) {
  ...
  if (headerTitleEl) headerTitleEl.textContent = msg.coopHome ? "Coop" : (msg.title || "Clay");
  if (msg.coopHome) {
    var projectTitleEl = document.getElementById("title-bar-project-name");
    if (projectTitleEl) projectTitleEl.textContent = "Coop";
  }

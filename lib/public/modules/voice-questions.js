...
export function createVoiceQuestions(options) {
  ...
  function review() {
    return "Your answers: " + selected.questions.map(function (q, i) {
      return "Question " + (i + 1) + ": " + (answers[q.id] || []).join(", ") + ".";
    }).join(" ") + " Say submit answers, change answers, or repeat question.";
  }
  ...
  return {
    ...
    consume: function (text) {
      ...
      if (!selected) {
        if (blocked) { say("A permission or private form needs attention on screen. This voice flow can answer ordinary questions."); return { handled: true }; }
        return null;
      }
      ...
      if (command !== "submit answers" && command !== "send answers" && command !== "confirm answers") {
        say("Say submit answers to send them, or change answers to revise them.");
        return { handled: true };
      }
      var submission = id();
      ...
      if (!send(payload)) { say("Your answers were not sent. Say submit answers after reconnecting."); return { handled: true }; }
    ...
    receive: function (message, replaying) {
      ...
      if (message.type === "voice_question_state" && message.clientRequestId === requestId) {
        ...
        if (submitting && reconcileSubmission) {
          ...
          if (unconsumed) {
            if (opts.onRejected) opts.onRejected(uncertain, "Your answers are still waiting. Say submit answers to send them.");
            else say("Your answers are still waiting. Say submit answers to send them.");
          }
        ...
        if (!selected || !next || selected.revision !== next.revision) {
          ...
          else if (blocked) say("A permission or private form needs attention on screen.");
        }
        publish();

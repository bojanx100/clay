...
// Build the human summary parts for one vendor (e.g. ["5h 100% resets 2h 10m",
// "7d 35% resets 40h 50m"]). Prunes expired entries as a side effect.
function buildVendorUsageParts(vendor) {
  ...
  for (var i = 0; i < RATE_LIMIT_USAGE_TYPES.length; i++) {
    ...
    var pctStr = formatUtilization(entry.utilization);
    parts.push(rateLimitTypeShortLabel(type) + (pctStr ? " " + pctStr : "") + " resets " + timeStr);
  }
  return parts;
}

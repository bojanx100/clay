...
export function handleRateLimitEvent(msg) {
  ...
  if (isRejected && msg.resetsAt) {
    ...
    if (!msg.isUsingOverage && delayUntilReset > 0 && isShortBlockingWindow &&
        vendorSupportsScheduledMessages(activeVendor)) {
...
export function updateRateLimitUsage(msg) {
  ...
  if (!vendorTracksRateLimits(vendor)) {
    if (vendor === (store.get('currentVendor') || "claude") && rateLimitUsageEl) {
      rateLimitUsageEl.style.display = "none";
    }
    return;
  }

...
// Show only providers with real usage data. Empty provider shells imply Clay
// knows a value when the upstream account API returned no supported windows.
function renderRateLimitUsageLink() {
  ...
  if (mobileRateLimitUsageEl) {
    mobileRateLimitUsageEl.innerHTML = usageHtml;
    mobileRateLimitUsageEl.hidden = segments.length === 0;
  }
  refreshIcons();
}

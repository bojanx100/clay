...
// Keep the two most useful account-level numbers in the same position for every
// vendor: the short-window reset and weekly utilization. Providers do not
// always emit both windows together, so show an em dash until the other update
// arrives instead of making the two vendor chips look like different metrics.
...
function anyLiveRateLimitReset() {
  for (var vendor in rateLimitResetState) {
    ...
    if (buildVendorUsageParts(vendor).length > 0) return true;
  }
  return false;
}
...
// Show usage for EVERY vendor with live limits (not just the active session's),
// so the user's real bottleneck stays visible regardless of which chat is open.
// Each vendor links to its own usage page. Stable order: claude, codex, rest.
function renderRateLimitUsageLink() {
  ...
  for (var k in rateLimitResetState) {
    if (rateLimitResetState.hasOwnProperty(k) && vendorOrder.indexOf(k) === -1) vendorOrder.push(k);
  }
  var segments = [];
  ...
  if (segments.length === 0) {
    // No live data for any vendor — fall back to a single "Check usage" link
    // for the session currently in view.
    var activeVendor = store.get('currentVendor') || "claude";
    rateLimitUsageEl.innerHTML = rateLimitUsageSegment(activeVendor, ["Check usage"]);
  } else {
    rateLimitUsageEl.innerHTML = segments.join('<span class="usage-check-sep">·</span>');
  }
  refreshIcons();
}

...
var mobileRateLimitUsageEl = null;
...
function tickRateLimitUsage() {
  if (!rateLimitUsageEl && !mobileRateLimitUsageEl) return;
...
function ensureRateLimitUsageLinks() {
  ...
  if (!mobileRateLimitUsageEl) {
    mobileRateLimitUsageEl = document.getElementById("mobile-rate-limit-usage");
  }
  return rateLimitUsageEl || mobileRateLimitUsageEl;
}
...
// Keep the same usage and reset fields visible for every vendor. Providers do
// not always emit both windows together, so retain the slot with an em dash
// until its account-level update arrives.
function buildVendorUsageParts(vendor) {
  ...
  return [
    fiveHourUsageBar(fiveHourEntry) + " resets " + (fiveHourReset || "—"),
    "7d " + (weeklyPercent || "—"),
  ];
}
...
// Claude and Codex always stay visible so their metrics can be compared at a
// glance. Additional providers appear while they have live usage data.
function renderRateLimitUsageLink() {
  if (!ensureRateLimitUsageLinks()) return;
  ...
  if (rateLimitUsageEl) rateLimitUsageEl.innerHTML = usageHtml;
  if (mobileRateLimitUsageEl) mobileRateLimitUsageEl.innerHTML = usageHtml;
  refreshIcons();
}

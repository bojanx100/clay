...
// --- Structured conclusion card (Debate Workflow v2) ---
// The single canonical place to find what a debate decided: rendered live
// when the moderator's final synthesis turn completes, and re-rendered from
// history replay (debate_conclusion entries are persisted).
export function renderDebateConclusion(entry) {
  if (!ctx || !ctx.messagesEl) return;
  flushTurnStream();

  var el = document.createElement("div");
  el.className = "debate-brief-card debate-conclusion-card";

  var header = document.createElement("div");
  header.className = "debate-brief-card-header";
  header.innerHTML =
    '<span class="debate-brief-card-icon">' + iconHtml("flag") + '</span>' +
    '<span class="debate-brief-card-title">Debate Conclusion</span>' +
    '<span class="debate-brief-card-chevron">' + iconHtml("chevron-down") + '</span>';

  var body = document.createElement("div");
  body.className = "debate-brief-card-body";
  var metaHtml = '<div class="debate-brief-topic">' + escapeHtml(entry.topic || "") + '</div>';
  if (entry.moderatorName || entry.rounds) {
    metaHtml += '<div class="debate-brief-context">Synthesis by ' +
      escapeHtml(entry.moderatorName || "the moderator") +
      (entry.rounds ? " after " + entry.rounds + " round" + (entry.rounds === 1 ? "" : "s") : "") + '</div>';
  }
  metaHtml += '<div class="debate-conclusion-text md-content">' + renderMarkdown(entry.text || "") + '</div>';
  body.innerHTML = metaHtml;

  var actions = document.createElement("div");
  actions.className = "debate-brief-actions";
  var copyBtn = document.createElement("button");
  copyBtn.className = "debate-brief-start-btn";
  copyBtn.innerHTML = iconHtml("copy") + " Copy synthesis";
  copyBtn.addEventListener("click", function () {
    try {
      navigator.clipboard.writeText(entry.text || "");
      copyBtn.innerHTML = iconHtml("check") + " Copied";
      refreshIcons();
    } catch (e) {}
  });
...
export function renderMcpDebateProposal(toolId, input) {
  // F-7: the server re-sends pending proposals on every (re)connect; a
  // client that already shows this proposal's card must not stack another.
  if (input && input.proposalId && ctx.messagesEl) {
    var existingCard = ctx.messagesEl.querySelector('.debate-brief-card[data-proposal-id="' + input.proposalId + '"]');
  ...
  if (input && input.proposalId) el.dataset.proposalId = input.proposalId;

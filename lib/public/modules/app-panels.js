...
import { providerLabel } from './app-rendering.js';
import { VENDOR_NAMES, sortProviderRoutesByReadiness } from './vendor-ui.js';
...
var autoApprovalWrap = null;
var autoApprovalChip = null;
var autoApprovalChipLabel = null;
var autoApprovalPopover = null;
var autoApprovalSummary = null;
var autoApprovalDisable = null;
var autoApprovalGlobalSection = null;
var autoApprovalGlobalToggle = null;
var autoApprovalProjectSection = null;
var autoApprovalProjectToggle = null;
var autoApprovalInherit = null;
var autoApprovalOverridesSection = null;
var autoApprovalOverrides = null;
var autoApprovalData = null;
var autoApprovalRequested = false;
...
function rebuildModelList() {
  ...
  if (s.currentModelsLoading && s.currentModels.length === 0) {
    var loading = document.createElement("div");
    loading.className = "config-model-loading";
    loading.textContent = "Loading models\u2026";
    configModelList.appendChild(loading);
    return;
  }
...
function requestAutoApprovalState() {
  if (autoApprovalRequested) return;
  autoApprovalRequested = true;
  sendUserAction({ type: "get_auto_approval_state" });
}

function setToggleState(button, checked, label, disabled) {
  if (!button) return;
...
function closeAutoApprovalPopover() {
  if (autoApprovalPopover) autoApprovalPopover.classList.add("hidden");
  if (autoApprovalChip) autoApprovalChip.setAttribute("aria-expanded", "false");
}

function renderAutoApproval() {
  if (!autoApprovalWrap || !autoApprovalChip || !autoApprovalData) return;
  autoApprovalWrap.classList.remove("hidden");
  var result = autoApprovalData;
  var state = result.state;
  var effective = state && state.effective;
  var enabled = !!(effective && effective.enabled);
  var coop = result.scope === "coop";
  autoApprovalChip.classList.toggle("is-enabled", enabled || !!(state && state.defaultControl && state.defaultControl.enabled));
  autoApprovalChipLabel.textContent = coop ?
    (state && state.defaultControl && state.defaultControl.enabled ? "Approvals: On" : "Approvals: Off") :
    (enabled ? "Approval: On" : "Approval: Off");
  if (autoApprovalSummary) {
    if (!state) autoApprovalSummary.textContent = result.error || "Auto-approval state is unavailable.";
    else if (coop) autoApprovalSummary.textContent = state.defaultControl.enabled ?
      "Eligible work is approved across projects unless a project override changes it." :
      "All projects require individual approval unless a project override enables eligible work.";
    else if (effective && effective.expired) autoApprovalSummary.textContent = "This authorization expired and is no longer active.";
    else autoApprovalSummary.textContent = enabled ?
      "Eligible low-risk work can proceed without another approve command." :
      "Eligible work still waits for an explicit owner approval.";
  }
  var canChange = result.canChange === true;
  if (autoApprovalGlobalSection) autoApprovalGlobalSection.classList.toggle("hidden", !coop);
  if (autoApprovalProjectSection) autoApprovalProjectSection.classList.toggle("hidden", coop);
  if (autoApprovalOverridesSection) autoApprovalOverridesSection.classList.toggle("hidden", !coop);
  if (autoApprovalDisable) {
    autoApprovalDisable.disabled = !canChange;
    autoApprovalDisable.classList.toggle("hidden", !enabled && !(coop && state && state.defaultControl && state.defaultControl.enabled));
  }
  if (coop && state) {
    var globalEnabled = state.defaultControl && state.defaultControl.enabled === true;
    setToggleState(autoApprovalGlobalToggle, globalEnabled,
      globalEnabled ? "All projects: enabled — disable" : "All projects: disabled — enable", !canChange);
  }
  if (!coop && state) {
    setToggleState(autoApprovalProjectToggle, enabled,
      enabled ? "This project: enabled — disable" : "This project: disabled — enable", !canChange);
    if (autoApprovalInherit) {
      var hasOverride = !!state.projectOverride;
      autoApprovalInherit.classList.toggle("hidden", !hasOverride);
      autoApprovalInherit.disabled = !canChange;
    }
  }
  if (coop && autoApprovalOverrides && state) {
    autoApprovalOverrides.replaceChildren();
    var projects = state.projects || [];
    for (var i = 0; i < projects.length; i++) {
      var project = projects[i];
      var row = document.createElement("div");
      row.className = "auto-approval-override";
      var label = document.createElement("span");
      label.textContent = project.label || project.projectRef.projectId;
      var button = document.createElement("button");
      var projectEnabled = !!(project.effective && project.effective.enabled);
      button.textContent = project.hasOverride ? (projectEnabled ? "Override on" : "Override off") :
        (projectEnabled ? "Inherited on" : "Inherited off");
      button.disabled = !canChange;
      button.addEventListener("click", (function (ref, isEnabled) {
        return function () {
          sendUserAction({ type: "set_auto_approval_project", projectRef: ref, enabled: !isEnabled });
        };
      })(project.projectRef, projectEnabled));
      row.appendChild(label);
      row.appendChild(button);
      autoApprovalOverrides.appendChild(row);
    }
  }
}

export function handleAutoApprovalState(msg) {
  autoApprovalData = msg || null;
  autoApprovalRequested = false;
  if (msg && msg.ok === false) showToast(msg.error || "Could not update auto-approval", "error");
  renderAutoApproval();
}
...
export function initPanels() {
  ...
  autoApprovalWrap = $("auto-approval-wrap");
  autoApprovalChip = $("auto-approval-chip");
  autoApprovalChipLabel = $("auto-approval-chip-label");
  autoApprovalPopover = $("auto-approval-popover");
  autoApprovalSummary = $("auto-approval-summary");
  autoApprovalDisable = $("auto-approval-disable");
  autoApprovalGlobalSection = $("auto-approval-global-section");
  autoApprovalGlobalToggle = $("auto-approval-global-toggle");
  autoApprovalProjectSection = $("auto-approval-project-section");
  autoApprovalProjectToggle = $("auto-approval-project-toggle");
  autoApprovalInherit = $("auto-approval-inherit");
  autoApprovalOverridesSection = $("auto-approval-overrides-section");
  autoApprovalOverrides = $("auto-approval-overrides");
  ...
  function renderProviderRouteMenu() {
    ...
    var resolvedRouteId = inferCurrentRouteId(current, currentRouteId);
    var providerStatus = store.get('providerHubStatus') || {};
    var routes = sortProviderRoutesByReadiness(getProviderRoutesForMenu(), providerStatus.providers || []);
    var html = "";
  ...
  if (providerRouteMenu) {
    providerRouteMenu.addEventListener("click", function (ev) {
      ...
      showConfirm(
        ...
        function () {
          sendUserAction({ type: "handoff_session", targetVendor: route.vendor, targetRouteId: route.id || null, source: "config-popup" });
        },
        "Switch provider",
        false
      );
  ...
  store.subscribe(function (state, prev) {
    if (state.currentSlug !== prev.currentSlug || state.activeSessionId !== prev.activeSessionId) {
      autoApprovalData = null;
      autoApprovalRequested = false;
      closeAutoApprovalPopover();
      requestAutoApprovalState();
    }
    // Vendor toggle state
    ...
    if (state.currentModel !== prev.currentModel ||
        ...
        state.currentModelsLoading !== prev.currentModelsLoading ||
  ...
  if (autoApprovalChip) {
    autoApprovalChip.addEventListener("click", function (e) {
      e.stopPropagation();
      requestAutoApprovalState();
      var opening = autoApprovalPopover.classList.contains("hidden");
      autoApprovalPopover.classList.toggle("hidden");
      autoApprovalChip.setAttribute("aria-expanded", opening ? "true" : "false");
      if (configPopover) configPopover.classList.add("hidden");
      if (configChip) configChip.classList.remove("active");
    });
  }
  if (autoApprovalGlobalToggle) {
    autoApprovalGlobalToggle.addEventListener("click", function () {
      var enabled = this.getAttribute("aria-checked") === "true";
      sendUserAction({ type: "set_auto_approval_global", enabled: !enabled });
    });
  }
  if (autoApprovalProjectToggle) {
    autoApprovalProjectToggle.addEventListener("click", function () {
      var enabled = this.getAttribute("aria-checked") === "true";
      sendUserAction({ type: "set_auto_approval_project", enabled: !enabled });
    });
  }
  if (autoApprovalInherit) {
    autoApprovalInherit.addEventListener("click", function () {
      sendUserAction({ type: "clear_auto_approval_project_override" });
    });
  }
  if (autoApprovalDisable) {
    autoApprovalDisable.addEventListener("click", function () {
      if (autoApprovalData && autoApprovalData.scope === "coop") {
        sendUserAction({ type: "set_auto_approval_global", enabled: false });
      } else {
        sendUserAction({ type: "set_auto_approval_project", enabled: false });
      }
    });
  }
  ...
  document.addEventListener("click", function (e) {
    ...
    if (autoApprovalPopover && autoApprovalChip && !autoApprovalPopover.contains(e.target) && e.target !== autoApprovalChip) {
      closeAutoApprovalPopover();
    }
  });
...
export function updateConfigChip() {
  ...
  if (autoApprovalWrap) autoApprovalWrap.classList.remove("hidden");
  requestAutoApprovalState();
  var s = store.snap();
  var vendor = s.currentVendor || "claude";

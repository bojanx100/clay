...
function modelDisplayName(value, models) {
  ...
  if (models) {
    for (var i = 0; i < models.length; i++) {
      ...
      else if ((m.value === value || m.id === value || m.name === value) && (m.displayName || m.display_name || m.label || m.name || m.id)) return m.displayName || m.display_name || m.label || m.name || m.id;
    }
  }
  return value;
}
...
function modelEntryLabel(model) {
  if (!model) return "";
  if (typeof model === "string") return model;
  return model.displayName || model.display_name || model.label || model.name || model.id || model.value || "";
}
...
function rebuildModelList() {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    var desc = typeof item === "string" ? "" : (item.description || item.desc || "");
...
export function initPanels() {
  ...
  function routeStatusText(route, current, currentRouteId) {
    ...
    if ((route.id && route.id === resolvedRouteId) || (!route.id && route.vendor === current)) return "Current route";
  ...
  if (providerRouteMenu) {
    providerRouteMenu.addEventListener("click", function (ev) {
      ...
      showConfirm(
        ...
        function () {
          if (ws) ws.send(JSON.stringify({ type: "handoff_session", targetVendor: route.vendor, targetRouteId: route.id || null, targetModel: targetModel, source: "config-popup" }));
        },
        "Switch provider",
        false
      );

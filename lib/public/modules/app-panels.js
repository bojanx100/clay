...
import { coopApprovalPresentation, projectApprovalChange } from './auto-approval-panel.js';
import { applyExecutionAuthority } from './execution-authority-ui.js';
...
function requestAutoApprovalState() {
  if (store.get("currentSlug") !== "lead") return;
...
function hideAutoApprovalControl() {
  closeAutoApprovalPopover();
  if (autoApprovalWrap) autoApprovalWrap.classList.add("hidden");
}

function renderAutoApproval() {
  if (!autoApprovalWrap || !autoApprovalChip) return;
  var panel = coopApprovalPresentation(autoApprovalData);
  if (store.get("currentSlug") !== "lead" || !panel) {
    hideAutoApprovalControl();
    return;
  }
  autoApprovalWrap.classList.remove("hidden");
  if (autoApprovalChipLabel) autoApprovalChipLabel.textContent = "Approvals";
  if (autoApprovalSummary) autoApprovalSummary.textContent = panel.summary;
  var canChange = autoApprovalData.canChange === true;
  if (autoApprovalProjects) autoApprovalProjects.replaceChildren();
  if (autoApprovalProjects) {
    var projects = panel.projects;
    if (!projects.length) {
    ...
    for (var i = 0; i < projects.length; i++) {
      ...
      button.className = "auto-approval-project-toggle";
      button.type = "button";
      button.setAttribute("role", "switch");
      button.setAttribute("aria-checked", project.enabled ? "true" : "false");
      button.setAttribute("aria-label", project.toggleName);
      button.textContent = project.toggleLabel;
      button.disabled = !canChange;
      button.addEventListener("click", (function (row) {
        return function () {
          var change = projectApprovalChange(row);
          if (change) sendUserAction(change);
        };
      })(project));
...
export function initPanels() {
  ...
  store.subscribe(function (state, prev) {
    ...
    if (state.currentModel !== prev.currentModel ||
        ...
        state.currentExecutionAuthority !== prev.currentExecutionAuthority ||

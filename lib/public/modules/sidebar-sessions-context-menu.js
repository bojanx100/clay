...
import { openCoordinatorPicker } from './sidebar-sessions-orchestration.js';
...
export function showSessionCtxMenu(anchorBtn, sessionId, title, cliSid, sessionData) {
  ...
  var isPermanentCoopConversation = !!(sessionData && (sessionData.coopHome || sessionData.coopChannel));

  var menu = document.createElement("div");
  menu.className = "session-ctx-menu";

  if (!isPermanentCoopConversation) {
  ...
  if (sessionData && !sessionData.orchestrationParent && !sessionData.coordinationMode) {
    var promoteItem = document.createElement("button");
    promoteItem.className = "session-ctx-item";
    promoteItem.innerHTML = iconHtml("git-branch") + " <span>Promote to coordinator</span>";
    ...
    var coordinateItem = document.createElement("button");
    coordinateItem.className = "session-ctx-item";
    coordinateItem.innerHTML = iconHtml("git-branch") + " <span>Add to coordinator\u2026</span>";
    coordinateItem.addEventListener("click", function (e) {
      e.stopPropagation();
      closeSessionCtxMenu();
      openCoordinatorPicker(sessionData);
    });
    menu.appendChild(coordinateItem);
  } else if (sessionData && sessionData.coordinationMode && !sessionData.orchestrationParent) {
    var demoteItem = document.createElement("button");
    demoteItem.className = "session-ctx-item";
    demoteItem.innerHTML = iconHtml("git-branch") + " <span>" +
      (sessionData.demotionPending ? "Cancel pending demotion" : "Demote to normal chat") +
      "</span>";
    demoteItem.addEventListener("click", function (e) {
      e.stopPropagation();
      closeSessionCtxMenu();
      sendUserAction({
        type: "demote_session_from_coordinator",
        sessionId: sessionId,
        action: sessionData.demotionPending ? "cancel" : undefined,
      });
    });
    menu.appendChild(demoteItem);
  }

  if (!isPermanentCoopConversation && store.get('isMultiUserMode') &&
      sessionData && sessionData.ownerId && sessionData.ownerId === store.get('myUserId')) {
  ...
  if ((!store.get('permissions') || store.get('permissions').sessionDelete !== false) && !isPermanentCoopConversation) {
    ...
    deleteItem.addEventListener("click", function (e) {
      e.stopPropagation();
      closeSessionCtxMenu();
      confirmDeleteSession({ id: sessionId, title: title, coopHome: isPermanentCoopConversation });
    });
    ...
    handoffItem.addEventListener("click", (function(routeForClick) {
      return function(e) {
        ...
        showConfirm(
          ...
          function () {
            sendUserAction({ type: "handoff_session", sessionId: sessionId, targetVendor: routeForClick.vendor, targetRouteId: routeForClick.id || null, source: "sidebar-menu" });
          },
          "Switch provider",
          false
        );

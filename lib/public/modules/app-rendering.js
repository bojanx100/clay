import { appendCoordinatorDiscussionLink } from "./coordinator-discussion-link.js";
...
import { VENDOR_AVATARS, VENDOR_NAMES } from './vendor-ui.js';
import { takeStreamChunk } from './stream-batcher.js';
import { openFile } from './filebrowser.js';
import { decodeMarkdownFilePath } from './markdown-link-policy.js';
...
var markdownLinkNavigationInitialized = false;

function initMarkdownLinkNavigation() {
  if (markdownLinkNavigationInitialized) return;
  markdownLinkNavigationInitialized = true;
  document.addEventListener("click", function (event) {
    var target = event.target;
    var anchor = target && typeof target.closest === "function"
      ? target.closest("a[data-clay-file-path]") : null;
    if (!anchor) return;
    event.preventDefault();
    var filePath = decodeMarkdownFilePath(anchor.getAttribute("data-clay-file-path"));
...
function drainStreamTick() {
  ...
  var next = takeStreamChunk(streamBuffer);
  var chunk = next.chunk;
  streamBuffer = next.remaining;
  ...
  if (contentEl) {
    // Markdown parsing walks the complete growing reply. Doing that once per
    // delta made a long stream quadratic and starved input, paints, and socket
    // events. Keep the live stream in one text node and render Markdown once
    // when the turn is flushed/finalized.
    if (!streamTextNode || streamTextNode.parentNode !== contentEl) {
      contentEl.textContent = "";
      streamTextNode = document.createTextNode(_s.currentFullText);
      contentEl.appendChild(streamTextNode);
    }
    if (typeof streamTextNode.appendData === "function") streamTextNode.appendData(chunk);
    else streamTextNode.nodeValue += chunk;
  }
...
export function flushStreamBuffer() {
  if (streamDrainTimer) { cancelAnimationFrame(streamDrainTimer); streamDrainTimer = null; }
  if (streamBuffer.length > 0) {
    var _sBeforeFlush = store.snap();
    var _contentBeforeFlush = _sBeforeFlush.currentMsgEl && _sBeforeFlush.currentMsgEl.querySelector(".md-content");
    if (_contentBeforeFlush) {
      if (!streamTextNode || streamTextNode.parentNode !== _contentBeforeFlush) {
        _contentBeforeFlush.textContent = "";
        streamTextNode = document.createTextNode(_sBeforeFlush.currentFullText);
        _contentBeforeFlush.appendChild(streamTextNode);
      }
      if (typeof streamTextNode.appendData === "function") streamTextNode.appendData(streamBuffer);
      else streamTextNode.nodeValue += streamBuffer;
    }
    store.set({ currentFullText: _sBeforeFlush.currentFullText + streamBuffer });
    streamBuffer = "";
  }
  var _s = store.snap();
  if (_s.currentMsgEl) {
...
export function markUserMessageAccepted(clientMessageId) {
  var message = findUserMessageByClientId(clientMessageId, true);
  if (message) message.dataset.messageAccepted = "1";
}
...
export function addSystemMessage(text, isError, variant, conversationRef) {
  ...
  appendCoordinatorDiscussionLink(div, conversationRef);
  addToMessages(div);
  scrollToBottom();
}

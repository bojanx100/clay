...
import { showConfirm, showImageModal, showPasteModal } from './app-misc.js';
...
export function addCopyHandler(msgEl, rawText) {
  ...
  actions.querySelector(".msg-assistant-reply-btn").addEventListener("click", function (e) {
    ...
    if (selection && selection.toString() && msgEl.contains(selection.anchorNode) && msgEl.contains(selection.focusNode)) {
      selectedText = selection.toString().trim();
    }
    var replyText = selectedText || rawText.trim();
    var maxReplyLength = 600;
    if (replyText.length > maxReplyLength) {
      replyText = replyText.slice(0, maxReplyLength).trimEnd() + "\u2026";
    }
    var quote = replyText.split(/\r?\n/).map(function (line) {
      return "> " + line;
    }).join("\n");

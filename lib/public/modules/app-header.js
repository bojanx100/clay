...
import { isCoopProjectSlug } from './coop-identity.js';
import { getMessagesEl } from './dom-refs.js';
import { getActivityEl, setActivityEl, getTurnCounter, setTurnCounter, getPrependAnchor, setPrependAnchor, finalizeAssistantBlock, providerLabel } from './app-rendering.js';
...
import { highlightCodeBlocksChunked, renderMermaidBlocks } from './markdown.js';
...
function modelDisplayName(value, models) {
  ...
  if (models) {
    for (var i = 0; i < models.length; i++) {
      var model = models[i];
      var modelValue = typeof model === "string" ? model : (model && (model.value || model.model || model.id || model.name)) || "";
      if (modelValue === value) {
        return typeof model === "string" ? model : (model.displayName || model.display_name || model.label || model.name || model.id || model.model || model.value || value);
      }
    }
  }
  return value;
...
export function initHeader() {
  ...
  if (headerRenameBtn) {
    headerRenameBtn.addEventListener("click", function () {
      ...
      // The Coop identity is not a session name the owner can edit. Renaming
      // here would both overwrite the header with arbitrary text and rename the
      // canonical Coop session underneath it.
      if (isCoopProjectSlug(store.get('currentSlug'))) return;
  ...
  if (headerInfoBtn) {
    headerInfoBtn.addEventListener("click", function (e) {
      ...
      var s = store.snap();
      addRow("Provider", providerLabel(s.currentVendor || "claude", s.currentProviderRouteId || null, s.currentModel || ""));
      if (s.currentVendor === "github-copilot" && s.verifiedModel) {
        addRow("Requested model", modelDisplayName(s.requestedModel || s.currentModel || s.verifiedModel || "", s.currentModels || []));
        addRow(s.modelVerificationSource === "config" ? "Configured model" : "Verified model", modelDisplayName(s.verifiedModel, s.currentModels || []));
      } else if (s.currentModel) {
        addRow("Model", modelDisplayName(s.currentModel, s.currentModels || []));
      }
      ...
      if (window.innerWidth > 768 && popRect.right > window.innerWidth - 8) {
...
export function prependOlderHistory(items, meta) {
  var messagesEl = getMessagesEl();
  var wasReplayingHistory = store.get('replayingHistory');
  ...
  // Process each item through the rendering pipeline. Guarded by try/finally:
  // a thrown error from a single malformed history item must not leave
  // prependAnchor set. A stuck anchor silently redirects every later append
  // (including a queued message flushed into a live turn) to this stale
  // position instead of the bottom of the transcript, making new messages
  // appear to "land" far above the latest content.
  ...
  try {
    ...
    store.set({ replayingHistory: wasReplayingHistory });
  ...
  function preserveAnchorDuringDomChange(fn) {
    if (!anchorEl) { fn(); return; }
    var beforeTop = anchorEl.getBoundingClientRect().top;
    fn();
    var afterTop = anchorEl.getBoundingClientRect().top;
    messagesEl.scrollTop += (afterTop - beforeTop);
  }

  function finishPrependedHistoryPostPass() {
    // Notify in-session search that history was prepended (for pending scroll targets)
    onSessionSearchHistoryPrepended();
  }

  // Mermaid rendering changes block heights above the viewport. Render after
  // the initial anchor restore and compensate each DOM replacement so loading
  // earlier history does not yank the current viewport.
  renderMermaidBlocks(messagesEl, {
    onDomChange: preserveAnchorDuringDomChange,
    onDone: finishPrependedHistoryPostPass,
  });

  // Highlight the prepended code blocks. Per-message highlighting was skipped
  // during the replay loop above (replayingHistory guard), and unlike the
  // initial-load path there was no post-pass here — so "load more" content
  // rendered permanently unhighlighted. Chunked so a big page of history can't
  // freeze the main thread; color-only, so restored scroll position holds.
  if (!wasReplayingHistory) {

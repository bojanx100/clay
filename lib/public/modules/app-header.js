...
function modelDisplayName(value, models) {
  ...
  if (models) {
    for (var i = 0; i < models.length; i++) {
      var model = models[i];
      var modelValue = typeof model === "string" ? model : (model && (model.value || model.id || model.name)) || "";
      if (modelValue === value) {
        return typeof model === "string" ? model : (model.displayName || model.display_name || model.label || model.name || model.id || model.value || value);
      }
    }
  }
  return value;

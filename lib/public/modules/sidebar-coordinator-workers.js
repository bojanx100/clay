...
function workerPriority(worker) {
  ...
  if (isAttentionStatus(status)) return 0;
  if (!isResolvedStatus(status)) return 1;
  return 2;
}
...
export function coordinatorWorkerDisplay(children, coordinatorId, forceExpanded) {
  ...
  var current = ranked.filter(function (worker) {
    return !isResolvedStatus(workerStatus(worker));
  });
  var resolved = ranked.filter(function (worker) {
    return isResolvedStatus(workerStatus(worker));
  });
  var collapsed = current.length > 0 ? current : resolved;
  var workers = expanded ? ranked : collapsed.slice(0, MAX_COLLAPSED_COORDINATOR_WORKERS);
  var visibleIds = {};
  var hiddenActive = 0;

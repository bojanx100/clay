...
export function createVoiceConversationController(options) {
  ...
  var assistantText = "";
  var speechQueue = [];
  var interruptionTimer = null;
  var recoveryRequest = null, recoveryTimer = null, pendingPlayback = "";
  ...
  var state = {
    ...
    wantsListening: false, interruptionListening: false,
  };
  var interruption = createVoiceInterruption({
    createRecognition: opts.createInterruptionRecognition, language: opts.language,
    echoText: function () { return utterance ? utterance.text : ""; },
    onState: function (listening) {
      if (state.interruptionListening === listening) return;
      state.interruptionListening = listening; publish();
    },
    onInterrupt: function () {
      silence();
      // Let speaker output drain before reopening the ordinary recognizer.
      state.phase = "listening"; publish();
      interruptionTimer = schedule(function () { interruptionTimer = null; resume(); }, 350);
    },
  });
  ...
  function snapshot() {
    return {
      ...
      interruptionListening: state.interruptionListening,
    };
  }
  ...
  function silence() {
    interruption.stop();
    if (interruptionTimer) unschedule(interruptionTimer);
  ...
  function flushPending() {
    if (pendingPlayback && !state.reconnecting && !activeTurn && !state.speaking &&
        !(opts.questions && opts.questions.isWaiting())) {
      var playback = pendingPlayback; pendingPlayback = ""; speak(playback); return;
    }
  ...
  function beginRecognition() {
    if (!state.wantsListening || state.reconnecting || state.speaking || recognition || interruptionTimer) return false;
    var owned;
    try {
  ...
  function speakNext() {
    ...
    try {
      ...
      if (state.wantsListening) interruption.start();
    } catch (error) {
      interruption.stop();
      utterance = null;
      speechQueue = [];
  ...
  function end(reason) {
    ...
    state.pending = [];
    pendingPlayback = "";
    if (recoveryTimer) unschedule(recoveryTimer);
    recoveryTimer = recoveryRequest = null;
    activeTurn = null;
    assistantText = "";
    repliesToActiveTurn = false;
    state.working = false;
  ...
  return {
    ...
    cancelTurn: function () {
      if (recoveryTimer) unschedule(recoveryTimer);
      recoveryTimer = recoveryRequest = null; pendingPlayback = "";
      state.pending = [];
      activeTurn = null;
      assistantText = "";
      state.working = false;
    ...
    receive: function (message, replayingHistory) {
      ...
      if (message.type === "voice_turn_state") {
        if (!activeTurn || message.clientMessageId !== activeTurn.id || message.clientRequestId !== recoveryRequest) return;
        if (recoveryTimer) unschedule(recoveryTimer);
        recoveryTimer = recoveryRequest = null; state.error = "";
        if (message.state === "queued") return;
        if (message.state === "running") {
          assistantText = message.text || ""; repliesToActiveTurn = true; state.working = true; publish(); return;
        }
        activeTurn = null; repliesToActiveTurn = false; state.working = false; assistantText = "";
        if (message.state === "completed" || message.state === "failed") {
          pendingPlayback = message.text || (message.state === "failed" ? "The turn failed." : "The turn finished without a spoken reply.");
        } else if (message.state === "interrupted") {
          pendingPlayback = "Your earlier turn was interrupted. " + (message.text || "No final reply was recorded.");
        } else pendingPlayback = "I could not recover an exact reply for that voice message. Check the conversation before retrying; I have not resent it.";
        flushPending(); publish(); return;
      }
      if (message.type === "user_turn_started") {
        if (activeTurn && message.clientMessageId === activeTurn.id) {
          if (recoveryTimer) unschedule(recoveryTimer);
          recoveryTimer = recoveryRequest = null;
        }
    ...
    setConnected: function (connected) {
      ...
      if (!connected) {
        if (state.speaking) pendingPlayback = [utterance && utterance.text].concat(speechQueue).filter(Boolean).join(" ");
        if (activeTurn && opts.requestTurnState) repliesToActiveTurn = false;
        if (recoveryTimer) unschedule(recoveryTimer);
        recoveryTimer = recoveryRequest = null;
        stopRecognition();
        silence();
        ...
        if (activeTurn && opts.requestTurnState) {
          if (recoveryTimer) unschedule(recoveryTimer);
          recoveryRequest = newId();
          opts.requestTurnState(clone(activeTurn.routing), activeTurn.id, recoveryRequest);
          recoveryTimer = schedule(function () {
            recoveryTimer = null;
            state.error = "Reply recovery did not respond. Reconnect to retry; your request has not been resent."; publish();
          }, 5000);
        }
        resume();
      }

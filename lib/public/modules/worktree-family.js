...
export function projectFamilySlugs(projects, slug) {
  var family = familyOf(projects, slug);
  if (!family.parent || family.parent.slug !== slug) return [slug];
  var slugs = [family.parent.slug];
  for (var i = 0; i < family.worktrees.length; i++) {
    if (family.worktrees[i].worktreeAccessible !== false) slugs.push(family.worktrees[i].slug);
  }
  return slugs;
}

export function familyNavigationTarget(projects, slug, rememberedSlug) {
  var slugs = projectFamilySlugs(projects, slug);
  if (slugs.length < 2 || !rememberedSlug) return slug;
  for (var i = 0; i < slugs.length; i++) {
    if (slugs[i] === rememberedSlug) return rememberedSlug;
  }
  return slug;
}

export function worktreeFamilyExpanded(parentSlug, worktrees, currentSlug, expandedBySlug, forceOpen) {
  if (forceOpen) return true;
  var states = expandedBySlug || {};
  if (Object.prototype.hasOwnProperty.call(states, parentSlug)) return states[parentSlug] === true;
  var children = worktrees || [];
  for (var i = 0; i < children.length; i++) {
    if (children[i].slug === currentSlug) return true;
  }
  return false;
}

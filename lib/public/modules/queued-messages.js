...
function sendAsTask(queueId) {
  var ws = getWs();
  if (!ws || ws.readyState !== 1 || !queueId) return;
  ws.send(JSON.stringify({
    type: "start_queued_message_as_task",
    queueId: queueId,
...
function renderQueuedMessages() {
  ...
  for (var i = 0; i < queuedItems.length; i++) {
    (function (item) {
      ...
      taskBtn.title = "Start this as a separate worker session";
      taskBtn.innerHTML = iconHtml("git-branch") + "<span>Run separately</span>";
      taskBtn.addEventListener("click", function () {
        sendAsTask(item.queueId);
      });

...
import { collapseOrchestrationTaskPreview, renderOrchestrationTaskPreview } from './orchestration-task-preview.js';
...
export function coopActionLabels(state) {
  ...
  if (isCoop) {
    return {
      composerLabel: "Delegate",
      composerAriaLabel: "Delegate to a worker",
      composerTitle: "Delegate to a worker (Option/Alt+Enter)",
...
function renderQueuedMessages() {
  ...
  for (var i = 0; i < queuedItems.length; i++) {
    (function (item) {
      ...
      taskBtn.innerHTML = iconHtml("git-branch") + "<span>" + actionLabels.queuedCoordinateLabel + "</span>";
  ...
  if (orchestrationTasks.length > 0) {
    ...
    taskList = renderOrchestrationTaskPreview(taskPreview, orchestrationTasks, orchestrationState);
  }
  refreshIcons();

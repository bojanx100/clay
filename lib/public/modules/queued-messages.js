...
function closeTask(task) {
  ...
  showConfirm(
    'Close "' + (task.title || "Parallel task") + '"? Its worker conversation and history will be permanently removed.',
    function () {
      ws.send(JSON.stringify({
...
function renderQueuedMessages() {
  ...
  for (var ti = 0; ti < orchestrationTasks.length; ti++) {
    (function (task) {
      ...
      detail.textContent = (task.status || "queued") + (task.provider ? " · " + task.provider : "");
      body.appendChild(title);
      ...
      close.title = "Close task and worker conversation";
      close.setAttribute("aria-label", "Close task and worker conversation");

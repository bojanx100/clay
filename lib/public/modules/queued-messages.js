...
import { showTaskResolutionDialog } from './orchestration-task-resolution-dialog.js';
...
function resolveTask(task) {
  var ws = getWs();
  if (!ws || ws.readyState !== 1 || !task || !task.taskId) return;
  showTaskResolutionDialog(task, function (summary, verification) {
    ws.send(JSON.stringify({
      type: "resolve_orchestration_task",
      taskId: task.taskId,
      sessionId: store.get("activeSessionId") || null,
      summary: summary,
...
function renderQueuedMessages() {
  ...
  for (var ti = 0; ti < orchestrationTasks.length; ti++) {
    (function (task) {
      ...
      var resolve = null;
      if (task.status === "needs_input" || task.status === "failed") {
        resolve = document.createElement("button");
        resolve.type = "button";
        resolve.className = "orchestration-task-resolve";
        resolve.title = "Resolve with coordinator verification";
        resolve.setAttribute("aria-label", "Resolve with coordinator verification");
        resolve.innerHTML = iconHtml("check");
      ...
      if (resolve) row.appendChild(resolve);

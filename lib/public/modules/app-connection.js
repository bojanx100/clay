...
import { readTabSession } from './session-tab-state.js';
import { attachPendingHandoffTrace, clearSentHandoffTrace } from './coop-handoff-client.js';
...
export function sendUserAction(obj) {
  var ws = getWs();
  var action = attachPendingHandoffTrace(obj);
...
export function connect() {
  ...
  var activeSessionRef = readTabSession(currentSlug);
  if (!activeSessionRef && store.get('activeSessionProjectSlug') === currentSlug) {

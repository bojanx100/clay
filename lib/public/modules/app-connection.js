...
import { getWs, setWs, setWsSendFailureHandler } from './ws-ref.js';
import { decideSocketAction, initialSessionReference, projectSessionIdForSync, shouldProbeLiveness, shouldProcessSocketMessage } from './connection-policy.js';
...
// A plain project click should resolve through server-side user presence and
// lastViewedAt. Keep that intent across connection retries until the target
// session is acknowledged, so a failed first handshake cannot resurrect the
// stale tab session that caused the project-navigation bug.
var pendingDefaultRestoreSlug = null;
...
function requestExternalSessionSync(reason) {
  ...
  var sessionId = projectSessionIdForSync({
    currentSlug: store.get('currentSlug'),
    activeSessionProjectSlug: store.get('activeSessionProjectSlug'),
...
export function initConnection() {
  ...
  setWsSendFailureHandler(function () {
...
export function connect(options) {
  ...
  if (options && options.preferProjectDefault) pendingDefaultRestoreSlug = currentSlug;
  var urlSessionRef = readUrlSessionRef(currentSlug);
  var activeSessionRef = initialSessionReference({
    currentSlug: currentSlug,
    urlSessionRef: urlSessionRef,
    tabSessionId: readTabSession(currentSlug),
    activeSessionProjectSlug: store.get('activeSessionProjectSlug'),
    activeSessionId: store.get('activeSessionId'),
    cliSessionId: store.get('cliSessionId'),
    preferProjectDefault: pendingDefaultRestoreSlug === currentSlug,
  });
  if (activeSessionRef) {
  ...
  setWs(newWs);
  var socketActivated = false;

  function activateSocket() {
    if (socketActivated || getWs() !== newWs) return;
  ...
  connectTimeoutId = setTimeout(function () {
    // A main-thread pause can delay the native open event beyond this timeout
    // even after the WebSocket handshake has completed. Activating this socket
    // preserves the cached extension registration instead of replacing it.
    if (!store.get('connected') && newWs.readyState === 1) {
      activateSocket();
      return;
    }
  ...
  newWs.onmessage = function (event) {
    ...
    noteInboundLiveness();
    // Backup: if we're receiving messages, complete the same connection
    // activation so cached extension state is replayed too.
    if (!store.get('connected')) {
      activateSocket();
    }
    ...
    if (msg.type === "session_switched" && pendingDefaultRestoreSlug === store.get('currentSlug')) {
      pendingDefaultRestoreSlug = null;
    }
    processMessage(msg);
  };
}

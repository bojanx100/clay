...
export function liveUiControlsHtml(dev) {
  ...
  if (error) html += '<div class="live-ui-error">' + escapeHtml(error) + '</div>';
  return html + '</div>';
}
...
export function handleLiveUiState(msg) {
  var currentPairingId = store.get('liveUiPairingId');
  var transientPairingError = msg.state === "error" && msg.pairingId &&
    msg.pairingId === currentPairingId;
  var error = msg.error || null;
  if (msg.state === "revoked" && msg.reason === "server_restart") {
    error = "Live UI ended after Clay restarted. Open it again.";
  } else if (msg.state === "revoked" && msg.reason === "reconnect_rejected") {
    error = "Live UI could not reconnect. Open it again.";
  }
  var next = {
    liveUiState: transientPairingError ? "paired" : (msg.state || "idle"),
    liveUiError: error,
  };
  ...
  if (msg.state === "revoked" || (msg.state === "error" && !transientPairingError)) {
    if (msg.state === "revoked") {
      next.liveUiPairingId = null;
      next.liveUiReconnectCredential = null;
    }

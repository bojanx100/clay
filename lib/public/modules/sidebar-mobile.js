...
function createMobileSessionItem(s) {
  ...
  if (s.orchestrationParent && s.orchestrationParent.workerColor) {
    el.style.setProperty("--worker-color", s.orchestrationParent.workerColor);
  }
    ...
    for (var bi = 0; bi < bookmarkedItems.length; bi++) {
      if (bookmarkedItems[bi].type === "coordinator") {
        container.appendChild(createMobileCoordinatorGroup(bookmarkedItems[bi], {
          ...
          iconHtml: iconHtml,
          refresh: refreshMobileChatSheet
        }));
      } else {

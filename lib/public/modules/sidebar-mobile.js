...
import {
  ...
  decorateMobileLeadChatChip,
  filterLeadProjects,
  findLeadProject
} from './sidebar-lead.js';
import {
  buildGlobalCoopDisplayModel,
  globalAttemptAvailability,
  globalAvailabilityLabel,
  globalSessionRefKey,
  isGlobalTaskExpanded,
  requestGlobalSessionRef,
  toggleGlobalTaskExpanded,
...
function appendMobileSessionProcessing(parent, s) {
  if (!s || !s.isProcessing) return null;
...
function renderSheetProjects(listEl) {
  // Group worktrees under their parent project (same grouping + collapse state
  // as the desktop icon strip, persisted under "clay-wt-collapsed"). Parents
  // with worktrees become a collapsible folder; the chevron toggles without
  // switching projects.
  var projects = getCachedProjectList();
  var leadProject = findLeadProject(projects);
  if (leadProject) {
  ...
  var grouped = groupProjects(filterLeadProjects(projects));
  for (var i = 0; i < grouped.parents.length; i++) {
    (function (p) {
      var worktrees = grouped.wtByParent[p.slug] || [];
      if (worktrees.length === 0) {
        listEl.appendChild(buildMobileProjectRow(p, false));
        return;
      }

      var folder = document.createElement("div");
      folder.className = "mobile-project-folder" + (isWtCollapsed(p.slug) ? " collapsed" : "");

      var parentRow = buildMobileProjectRow(p, false);
      var chevron = document.createElement("span");
      chevron.className = "mobile-folder-chevron";
      chevron.innerHTML = iconHtml("git-branch");
      chevron.setAttribute("aria-label", "Toggle worktrees");
      chevron.addEventListener("click", function (e) {
        e.preventDefault();
        e.stopPropagation();
        var nowCollapsed = folder.classList.toggle("collapsed");
        setWtCollapsed(p.slug, nowCollapsed);
      });
      parentRow.appendChild(chevron);
      folder.appendChild(parentRow);

      var items = document.createElement("div");
      items.className = "mobile-folder-items";
      for (var w = 0; w < worktrees.length; w++) {
        items.appendChild(buildMobileProjectRow(worktrees[w], true));
      }
...
function renderSheetSessions(listEl) {
  ...
  function buildProjectChip(p, isWorktree, parentSlug) {
    ...
    if (isWorktree) chip.dataset.wtParent = parentSlug;
    if (!isAccessible) chip.disabled = true;
    // Worktree chips start hidden when their parent folder is collapsed (shared
    // collapse state with the desktop icon strip). Hide via inline style so it
    // can't be defeated by stylesheet specificity/caching.
    if (isWorktree && isWtCollapsed(parentSlug)) chip.style.display = "none";
    ...
    decorateMobileLeadChatChip(chip, p);
  ...
  // Group worktrees under their parent. Each parent with worktrees gets an
  // inline toggle (git-branch icon) that collapses/expands its worktree chips —
  // same persisted state ("clay-wt-collapsed") as the desktop icon strip.
  var leadProject = findLeadProject(getCachedProjectList());
  if (leadProject) chips.push(buildProjectChip(leadProject, false, null));

  var grouped = groupProjects(getCachedProjectList());
  for (var ci = 0; ci < grouped.parents.length; ci++) {
    (function (p) {
      var worktrees = grouped.wtByParent[p.slug] || [];
      var chip = buildProjectChip(p, false, null);

      if (worktrees.length > 0) {
        var toggle = document.createElement("span");
        toggle.className = "mobile-chat-chip-wt-toggle" + (isWtCollapsed(p.slug) ? " collapsed" : "");
        toggle.innerHTML = iconHtml("git-branch");
        toggle.setAttribute("aria-label", "Toggle worktrees");
        toggle.addEventListener("click", function (e) {
          e.preventDefault();
          e.stopPropagation();
          var nowCollapsed = !isWtCollapsed(p.slug);
          setWtCollapsed(p.slug, nowCollapsed);
          toggle.classList.toggle("collapsed", nowCollapsed);
          var wtChips = filterBar.querySelectorAll('[data-wt-parent="' + p.slug + '"]');
          for (var wc = 0; wc < wtChips.length; wc++) {
            wtChips[wc].style.display = nowCollapsed ? "none" : "";
          }
        });
      ...
      chips.push(chip);
      for (var w = 0; w < worktrees.length; w++) {
        chips.push(buildProjectChip(worktrees[w], true, p.slug));
      }
    })(grouped.parents[ci]);
  }

  var favoriteChipMates = (store.get('isMultiUserMode') ? getCachedMates() : []).filter(function (m) {
...
function createMobileSessionTitle(s, orchestrationParent) {
  ...
  appendMobileSessionRoleBadge(titleSpan, s, orchestrationParent);
  appendMobileAutoBadge(titleSpan, s);
  titleSpan.appendChild(document.createTextNode(s.title || "New Session"));
  return titleSpan;
}

function appendMobileSessionRoleBadge(titleSpan, s, orchestrationParent) {
  if (s.coordinationMode) {
    var coordinatorBadge = document.createElement("span");
    coordinatorBadge.className = "mobile-session-role-badge coordinator";
    coordinatorBadge.textContent = "Coordinator";
    titleSpan.appendChild(coordinatorBadge);
  } else if (orchestrationParent) {
    var workerBadge = document.createElement("span");
    workerBadge.className = "mobile-session-role-badge worker";
    var attemptCount = Number(orchestrationParent.attemptCount) || 0;
    var attempt = Number(orchestrationParent.attempt) || 0;
    workerBadge.textContent = attemptCount > 1 ? "Worker " + attempt + "/" + attemptCount : "Worker";
    workerBadge.title = attemptCount > 1 ? "Worker attempt " + attempt + " of " + attemptCount : "Worker session";
    titleSpan.appendChild(workerBadge);
  }
}
...
function createMobileNewSessionRow() {
  ...
  claudeBtn.addEventListener("click", function () {
    ...
    if (sendUserAction({ type: "new_session", vendor: "claude" })) closeMobileSheet();
  });
  ...
  codexBtn.addEventListener("click", function () {
    if (sendUserAction({ type: "new_session", vendor: "codex" })) closeMobileSheet();
  });
...
function mobileProjectedLabel(row) {
  return row.title || (row.role === "worker" ? "Worker" : "Conversation");
}

function mobileProjectedStatus(row, availability) {
  var status = document.createElement("span");
  var value = availability === "available" ? (row.status || "available") : availability;
  status.className = "mobile-global-coop-status mobile-global-coop-status-" + value;
  status.setAttribute("aria-hidden", "true");
  return status;
}

function mobileProjectedRole(row) {
  var role = document.createElement("span");
  role.className = "mobile-session-role-badge mobile-global-coop-role " + (row.role || "conversation");
  role.textContent = row.role === "coop" ? "Coop" :
    (row.role === "worker" ? "Worker" : (row.role === "direct_leaf" ? "Direct" : "Coordinator"));
  return role;
}

function bindMobileProjectedNavigation(el, row, availability) {
  if (availability !== "available" || !row.sessionRef) {
    el.setAttribute("role", "status");
    el.setAttribute("aria-disabled", "true");
    return;
  }
  function open() { requestGlobalSessionRef(row.sessionRef, sendUserAction); }
  el.setAttribute("role", "button");
  el.setAttribute("tabindex", "0");
  el.setAttribute("aria-label", "Open " + mobileProjectedLabel(row) + " in its project");
...
function createMobileProjectedRow(row, extraClass) {
  var availability = globalAttemptAvailability(row);
  var active = globalSessionRefKey(row.sessionRef) &&
    globalSessionRefKey(row.sessionRef) === globalSessionRefKey(store.get("activeGlobalSessionRef"));
  var el = document.createElement("div");
  el.className = "mobile-session-item mobile-global-coop-row " + (extraClass || "") +
    (active ? " active" : "") + (availability === "available" ? "" : " unavailable");
  el.appendChild(mobileProjectedStatus(row, availability));
  var title = document.createElement("span");
  title.className = "mobile-session-title";
  title.appendChild(mobileProjectedRole(row));
  title.appendChild(document.createTextNode(mobileProjectedLabel(row)));
  if (row.activity) {
    var activity = document.createElement("span");
    activity.className = "mobile-global-coop-activity";
    activity.textContent = row.activity;
    title.appendChild(activity);
  }
  if (availability !== "available") {
    var unavailable = document.createElement("span");
    unavailable.className = "mobile-global-coop-unavailable";
    unavailable.textContent = globalAvailabilityLabel(availability);
    title.appendChild(unavailable);
  }
  el.appendChild(title);
  bindMobileProjectedNavigation(el, row, availability);
  return el;
}

function createMobileProjectedHeader(label, className) {
  var header = document.createElement("div");
  header.className = "mobile-global-coop-header " + (className || "");
...
function appendMobileProjectedAttempts(container, task, attempts) {
  for (var i = 0; i < attempts.length; i++) {
    var attempt = Object.assign({}, attempts[i], {
      role: "worker",
      status: task.status,
      activity: task.activity || attempts[i].activity || "",
    });
    container.appendChild(createMobileProjectedRow(attempt, "mobile-global-coop-worker"));
  }
}

function appendMobileProjectedTask(container, task) {
  var taskEl = document.createElement("div");
  taskEl.className = "mobile-global-coop-task mobile-global-coop-task-" + task.status;
  var label = document.createElement("div");
  label.className = "mobile-global-coop-task-label";
  label.textContent = String(task.status || "unknown").replace(/_/g, " ") +
    (task.activity ? " · " + task.activity : "");
  taskEl.appendChild(label);
  var expanded = isGlobalTaskExpanded(task.taskRef);
  var history = task.terminal ? task.currentAttempts.concat(task.historicalAttempts) : task.historicalAttempts;
  if (task.terminal) {
    if (expanded) appendMobileProjectedAttempts(taskEl, task, history);
  } else {
    appendMobileProjectedAttempts(taskEl, task, task.currentAttempts);
  }
  if (history.length > 0) {
    var toggle = document.createElement("button");
    toggle.type = "button";
    toggle.className = "mobile-global-coop-history-toggle";
    toggle.setAttribute("aria-expanded", expanded ? "true" : "false");
    toggle.textContent = expanded ? "Hide attempt history" :
      history.length + (history.length === 1 ? " earlier attempt" : " earlier attempts");
    toggle.addEventListener("click", function () {
      toggleGlobalTaskExpanded(task.taskRef);
      refreshMobileChatSheet();
    });
    taskEl.appendChild(toggle);
    if (expanded && !task.terminal) appendMobileProjectedAttempts(taskEl, task, task.historicalAttempts);
  }
  container.appendChild(taskEl);
}

function appendMobileProjectedProject(container, project) {
  var group = document.createElement("section");
  group.className = "mobile-global-coop-project" + (project.worktree ? " worktree" : "");
  group.appendChild(createMobileProjectedHeader((project.icon ? project.icon + " " : "") + project.title));
  for (var ci = 0; ci < project.coordinators.length; ci++) {
    var coordinator = project.coordinators[ci];
    group.appendChild(createMobileProjectedRow(coordinator, "mobile-global-coop-coordinator"));
    var tasks = coordinator.visibleTasks.concat(coordinator.terminalTasks);
    for (var ti = 0; ti < tasks.length; ti++) appendMobileProjectedTask(group, tasks[ti]);
  }
  for (var li = 0; li < project.directLeaves.length; li++) {
    group.appendChild(createMobileProjectedRow(
      Object.assign({}, project.directLeaves[li], { role: "direct_leaf" }),
      "mobile-global-coop-direct"
    ));
  }
  for (var wi = 0; wi < project.worktrees.length; wi++) appendMobileProjectedProject(group, project.worktrees[wi]);
  container.appendChild(group);
}

function isMobileLeadAutomation(session) {
  return !!(session && !session.coopHome && !session.coopChannel &&
    (session.taskLauncher || session.loop));
}

// Mirrors the server's coopHomeCandidate() in lib/sessions.js: eligible to
// stand in for the canonical Coop conversation if the coopHome flag itself
// hasn't (yet) synced to this client.
function isMobileCoopHomeCandidate(session) {
...
function findMobileCoopHomeFallback(sessions) {
  var best = null;
  for (var i = 0; i < sessions.length; i++) {
    var session = sessions[i];
    if (!isMobileCoopHomeCandidate(session)) continue;
    var score = session.lastViewedAt || session.lastActivity || session.createdAt || 0;
    var bestScore = best ? (best.lastViewedAt || best.lastActivity || best.createdAt || 0) : -1;
    if (!best || score > bestScore) best = session;
  }
  return best;
}

function createMobileCanonicalCoopRow() {
  var sessions = getCachedSessions();
  // The client cache can momentarily miss the coopHome flag (reconnect race,
  // stale broadcast, etc.), which used to leave the Coop tab with nothing
  // clickable at all. Fall back to the best-matching session so there's
  // always a way in to Lead.
  var home = sessions.find(function (session) { return session.coopHome; }) ||
    findMobileCoopHomeFallback(sessions);
  if (!home) return null;
  var row = createMobileSessionItem(Object.assign({}, home, { title: "Coop" }));
  row.classList.add("mobile-global-coop-row", "mobile-global-coop-permanent");
  row.setAttribute("aria-label", "Open Coop");
  row.addEventListener("keydown", function (event) {
    if (event.key !== "Enter" && event.key !== " ") return;
    event.preventDefault();
    sendUserAction({ type: "switch_session", id: home.id });
  });
  return row;
}

function renderMobileGlobalCoopSessions(container) {
  var model = buildGlobalCoopDisplayModel("");
  container.appendChild(createMobileProjectedHeader("Coop", "mobile-global-coop-heading"));
  var coopRow = createMobileCanonicalCoopRow();
  if (coopRow) container.appendChild(coopRow);
  if (!model.hasProjection) {
    var loading = document.createElement("div");
    loading.className = "mobile-global-coop-empty";
    loading.textContent = "Loading project conversations…";
    container.appendChild(loading);
    return;
  }
  for (var i = 0; i < model.projects.length; i++) appendMobileProjectedProject(container, model.projects[i]);
  var automations = getCachedSessions().filter(isMobileLeadAutomation);
  if (automations.length > 0) {
    container.appendChild(createMobileProjectedHeader("Automations & schedules", "mobile-global-coop-automation-heading"));
    for (var ai = 0; ai < automations.length; ai++) container.appendChild(createMobileSessionItem(automations[ai]));
  }
}
...
function renderMobileSessionsInto(container) {
  ...
  container.appendChild(createMobileNewSessionRow());

  var autoLaunchRow = createMobileAutoLaunchRow();
  if (autoLaunchRow) container.appendChild(autoLaunchRow);

  var partitioned = partitionMobileSessions(getCachedSessions());
  ...
  appendMobileRegularItems(container, split.regularItems);
}

// Refresh mobile chat sheet when session data updates (called from renderSessionList)
function updateMobileChatChipActive(chip, currentDmUserId, currentSlug) {
  chip.classList.remove("active");
  if (chip.dataset.type === "project" && chip.dataset.slug === currentSlug && !currentDmUserId) {
    chip.classList.add("active");
    return;
  }
...
function refreshMobileChatChips(sheet) {
  ...
  for (var i = 0; i < chips.length; i++) {
    updateMobileChatChipActive(chips[i], currentDmUserId, currentSlug);

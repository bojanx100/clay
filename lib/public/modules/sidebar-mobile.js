...
function renderMobileSessionsInto(container) {
  ...
  // Kiro: only shown when kiro-cli is installed.
  var mobileInstalledVendors = store.get('installedVendors') || [];
  if (mobileInstalledVendors.indexOf("kiro") !== -1) {
    var kiroBtn = document.createElement("button");
    kiroBtn.className = "mobile-session-new mobile-session-new-vendor";
    kiroBtn.innerHTML = '<img src="/kiro-avatar.svg" class="mobile-session-new-icon" alt=""><span>Kiro</span>';

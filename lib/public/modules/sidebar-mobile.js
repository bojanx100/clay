...
function createMobileLoopGroup(loopId, children, groupKey) {
  ...
  for (var ci = 0; ci < children.length; ci++) {
    ...
    if (children[ci].isProcessing) {
      if (!processingSession) processingSession = children[ci];
    }
  ...
  appendMobileSessionProcessing(header, processingSession);
...
function renderMobileSessionsInto(container) {
  ...
  // Build virtual items, nesting owned workers under their coordinator.
  var items = buildMobileCoordinatorItems(normalSessions);
  ...
  for (var n = 0; n < items.length; n++) {
    var item = items[n];
    if (item.type !== "loop" && item.data && item.data.bookmarked) {
  ...
  if (bookmarkedItems.length > 0) {
    ...
    for (var bi = 0; bi < bookmarkedItems.length; bi++) {
      if (bookmarkedItems[bi].type === "coordinator") {
        container.appendChild(createMobileCoordinatorGroup(bookmarkedItems[bi], {
  ...
  for (var ri = 0; ri < regularItems.length; ri++) {
    ...
    if (item.type === "loop") {
      ...
      container.appendChild(createMobileCoordinatorGroup(item, {
        createSessionItem: createMobileSessionItem,
        rerender: refreshMobileChatSheet
      }));
    } else {

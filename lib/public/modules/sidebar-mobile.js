...
function appendMobileProjectedProject(container, project) {
  ...
  lens.addEventListener("click", function () { requestProjectChannel(project, sendUserAction); });
  ...
  handoff.addEventListener("click", function () { if (project.slug) switchProject(project.slug); });
...
function appendMobileProjectedSessionNode(container, node, depth) {
  ...
  row.addEventListener("click", function () { requestCanonicalSession(node.sessionRef, sendUserAction); });
...
function createMobileCanonicalCoopRow() {
  ...
  row.addEventListener("click", function () { requestCoopHome(sendUserAction); });
  row.addEventListener("keydown", function (event) {
    if (event.key !== "Enter" && event.key !== " ") return;
    event.preventDefault();
    requestCoopHome(sendUserAction);
  });
  return row;
}

...
import {
  ...
  requestCoopHome,
  requestProjectChannel,
} from './global-coop-projection.js';
import { renderCoopProjectTopics, renderCoopTopicGroup, renderCoopTopicOverview } from './sidebar-coop-topics.js';
...
function renderSheetSessions(listEl) {
  ...
  function buildProjectChip(p, isWorktree, parentSlug) {
    ...
    var isCoopRoot = !!p.coopHome || p.slug === "lead";
    ...
    // Coop is the permanent root rather than another Lead-labelled project.
    // Its fixed position and the nested lens tree carry that relationship.
    if (!isCoopRoot) decorateMobileLeadChatChip(chip, p);
...
function appendMobileProjectedProject(container, project) {
  ...
  group.className = "mobile-global-coop-project" +
    (project.summary.attention.length > 0 ? " attention" : "") +
    (store.get("activeCoopLens") && store.get("activeCoopLens").projectRef &&
      store.get("activeCoopLens").projectRef.projectId === project.projectRef.projectId ? " active" : "");
  var lens = document.createElement("button");
  lens.type = "button";
  lens.className = "mobile-global-coop-project-header";
  lens.textContent = (project.icon ? project.icon + " " : "") + project.title;
  lens.setAttribute("aria-label", "Open " + project.title + " project lens");
  lens.addEventListener("click", function () {
    if (requestProjectChannel(project, sendUserAction)) finishMobileCoopNavigation();
  });
  group.appendChild(lens);
  var metrics = document.createElement("div");
  metrics.className = "mobile-global-coop-metrics";
  metrics.textContent = project.summary.metrics.activeCoordinators + " coordinators · " +
    project.summary.metrics.activeWorkers + " workers · " + project.summary.metrics.health;
  group.appendChild(metrics);
  appendMobileSummary(group, "Goals", project.summary.goals, function (item) { return item; });
  appendMobileSummary(group, "Decisions", project.summary.decisions, function (item) { return item; });
  appendMobileSummary(group, "Active work", project.summary.activeWork, mobileSummaryItem);
  appendMobileSummary(group, "Attention", project.summary.attention, mobileSummaryItem);
  appendMobileSummary(group, "Verified outcomes", project.summary.outcomes, mobileSummaryItem);
  appendMobileProjectedSessionTree(group, project.summary.coordinatorTree || []);
  renderCoopProjectTopics(group, project.topics || [], {
    mobile: true,
    projectRef: project.projectRef,
    onNavigate: finishMobileCoopNavigation,
  });
  var actions = document.createElement("div");
  actions.className = "mobile-global-coop-actions";
  var handoff = document.createElement("button");
  handoff.type = "button";
  handoff.className = "mobile-global-coop-handoff-action";
  handoff.textContent = "Open canonical project";
  handoff.addEventListener("click", function () {
    if (!project.slug) return;
    switchProject(project.slug);
    finishMobileCoopNavigation();
  });
...
function appendMobileProjectedSessionTree(container, nodes) {
  var tree = document.createElement("div");
  tree.className = "mobile-global-coop-session-tree";
  for (var i = 0; i < nodes.length; i++) appendMobileProjectedSessionNode(tree, nodes[i], 0);
  if (tree.childNodes.length > 0) container.appendChild(tree);
}

function appendMobileProjectedSessionNode(container, node, depth) {
  if (!node || !node.sessionRef) return;
  var row = document.createElement("button");
  row.type = "button";
  row.className = "mobile-global-coop-session-node mobile-global-coop-session-" + node.role;
  row.style.setProperty("--coop-lineage-depth", String(depth));
  row.setAttribute("aria-label", "Open canonical session " + node.title);
  row.textContent = node.title + (node.activity ? " · " + node.activity : "");
  row.addEventListener("click", function () {
    if (requestCanonicalSession(node.sessionRef, sendUserAction)) finishMobileCoopNavigation();
  });
  container.appendChild(row);
  var children = node.children || [];
  for (var i = 0; i < children.length; i++) appendMobileProjectedSessionNode(container, children[i], depth + 1);
}

function mobileSummaryItem(item) {
  var details = item && (item.activity || item.summary);
  return (item && item.title || "") + (details ? " — " + details : "");
}

function appendMobileSummary(container, label, items, formatter) {
  items = items || [];
  var section = document.createElement("div");
  section.className = "mobile-global-coop-summary";
  section.textContent = label + ": " + (items.map(formatter).filter(Boolean).join(" · ") || "None recorded.");
...
function createMobileCanonicalCoopRow() {
  ...
  row.addEventListener("click", function () {
    if (requestCoopHome(sendUserAction)) finishMobileCoopNavigation();
  });
...
function renderMobileGlobalCoopSessions(container) {
  ...
  renderCoopTopicOverview(container, model, {
    mobile: true,
    includeGroups: false,
    onNavigate: finishMobileCoopNavigation,
  });
  for (var i = 0; i < model.projects.length; i++) appendMobileProjectedProject(container, model.projects[i]);
  renderCoopTopicGroup(container, "Cross-project", model.crossProjectTopics, {
    mobile: true,
    onNavigate: finishMobileCoopNavigation,
  });
  renderCoopTopicGroup(container, "Uncategorised", model.uncategorisedTopics, {
    mobile: true,
    onNavigate: finishMobileCoopNavigation,
  });
}
...
store.subscribe(function (state, previous) {
  if (state.activeCoopLens !== previous.activeCoopLens) refreshMobileChatSheet();
});

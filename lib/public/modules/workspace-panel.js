...
import { mergeWorkspaceState, acceptsWorkspaceResponse } from './workspace-panel-state.js';
...
import { initWorkspaceGroupPreferences, wireWorkspaceGroupControls } from './workspace-group-collapse.js';
import { workspaceTabsHtml, workspaceTasksHtml, wireWorkspaceTabs, wireWorkspaceTaskLinks } from './workspace-tasks-view.js';
import { devSectionHtml, linkedItemsHtml, sessionMediaHtml, workspaceSummaryHtml, workspaceGroupStart, workspaceGroupEnd } from './workspace-panel-sections.js';
import { hasCoopOwnerContext, shouldDefaultOpenCoopOwnerLedger, renderWorkspaceCoopOwner, syncWorkspaceOwnerRefreshControl, setWorkspaceOwnerTasks } from './workspace-coop-owner.js';
var isOpen = false;
...
var requestedFor = null, nextWorkspaceRequestId = 0, latestRequestIdBySession = {}; // in-flight session and newest id by session
...
function renderCoopOwnerControl() {
  var ownerMode = syncWorkspaceOwnerRefreshControl(document.getElementById("workspace-refresh-btn"), getCachedSessions());
  if (!ownerMode) return false;
  var rendered = renderWorkspaceCoopOwner(bodyEl(), {
    send: sendWs,
    onNavigate: closeWorkspacePanel,
    selectedTab: selectedTab,
    onTabSelect: function (next) { selectedTab = next; renderCoopOwnerControl(); },
  });
...
function ownerActionStateChanged(state, previous) {
  return state.coopActionPending !== previous.coopActionPending ||
    state.coopActionError !== previous.coopActionError ||
    state.coopActionDone !== previous.coopActionDone;
}
// Fire a workspace_get for the active session; the caller owns the placeholder.
function requestState() {
  if (!store.get('connected')) return;
  requestedFor = curSession();
  var requestId = ++nextWorkspaceRequestId;
  latestRequestIdBySession[String(requestedFor)] = requestId;
  sendWs({ type: "workspace_get", sessionId: requestedFor, requestId: requestId });
}
...
function pollDevStatus() {
  ...
  sendWs({ type: "workspace_tasks_get", sessionId: curSession() });
}
...
export function initWorkspacePanel() {
  ...
  if (refreshBtn) refreshBtn.addEventListener("click", function () {
    if (hasCoopOwnerContext(getCachedSessions())) return;
  ...
  if (!subscribed) {
    ...
    store.subscribe(function (state, prev) {
      ...
      if (state.workspaceGroupStates !== prev.workspaceGroupStates) {
        if (renderCoopOwnerControl()) return;
        var current = stateBySession[curSession()];
        if (current) render(current.state);
        return;
      }
      if (ownerActionStateChanged(state, prev) && renderCoopOwnerControl()) return;
...
export function handleWorkspaceState(msg) {
  ...
  if (!acceptsWorkspaceResponse(latestRequestIdBySession[String(id)], msg)) return;
  if (msg.error) {
    setRefreshing(false);
...
export function handleWorkspaceTasksState(msg) {
  var id = msg.sessionId != null ? msg.sessionId : curSession();
  if (hasCoopOwnerContext(getCachedSessions()) && String(id) === String(curSession())) {
    setWorkspaceOwnerTasks(msg.tasks);
    if (isOpen) renderCoopOwnerControl();
    return;
  }
  var cached = stateBySession[id];
  if (!cached || !cached.state || !Array.isArray(msg.tasks)) return;
  cached.state.tasks = msg.tasks;
  if (isOpen && String(id) === String(curSession()) && selectedTab === "tasks") render(cached.state);
}
export function handleWorkspaceDevStatus(msg) {
  ...
  if (isOpen) {
    if (renderCoopOwnerControl()) { sendWs({ type: "workspace_tasks_get", sessionId: curSession() }); return; }
...
function updateDevSection(state) {
  ...
  wireWorkspaceGroupControls(next);
  refreshIcons();
  return true;
}
...
function render(state) {
  ...
  var html = workspaceTabsHtml(selectedTab);

  if (selectedTab === "tasks") {
    b.innerHTML = html + '<div class="ws-tab-panel" role="tabpanel">' + workspaceTasksHtml(state.tasks) + '</div>';
    wireBody(b);
    return;
  }
  ...
  if (state.sessionMedia && state.sessionMedia.length) {
    html += workspaceGroupStart("workspace-session-screenshots", "Session screenshots", "images") +
      sessionMediaHtml(state.sessionMedia) + workspaceGroupEnd();
  }
...
function wireBody(b) {
  wireWorkspaceTabs(b, selectedTab, function (next) {
    selectedTab = next; var cached = stateBySession[curSession()]; if (cached) render(cached.state);
  });
  wireWorkspaceTaskLinks(b, sendWs);
  wireWorkspaceGroupControls(b);

...
import { liveUiControlsHtml, wireLiveUiControls } from './live-ui.js';
...
function itemCardHtml(item) {
  ...
  var unresolved = item.unresolved ? '<div class="ws-item-note">Couldn\'t load details — open on GitHub.</div>' : "";
  return '' +
    '<div class="ws-item">' +
...
function devSectionHtml(state) {
  ...
  if (external) {
    ...
    // Surface the port the server will bind (the next free one) so it's clear
    // up front where this worktree will be served — no guessing, no collision.
    var portHint = dev.port ? ' on :' + escapeHtml(String(dev.port)) : '';
    control = '<button class="ws-devbtn ws-dev-start" data-dev="start"><i data-lucide="play"></i>Start dev server' + portHint + '</button>';
  } else {
    // running / starting — Clay owns this server: offer Stop and Restart.
    // Restart reclaims the same port so a stale server can be cycled in place.
    control = '<div class="ws-devbtns">' +
      '<button class="ws-devbtn ws-dev-stop" data-dev="stop"><i data-lucide="square"></i>Stop dev server</button>' +
      '<button class="ws-devbtn ws-dev-restart" data-dev="restart"><i data-lucide="refresh-cw"></i>Restart</button>' +
      '</div>';
  }
...
function render(state) {
  ...
  if (state.items && state.items.length) {
    ...
    // Skeleton phase: the GitHub round-trips are still in flight.
    html += '<div class="ws-empty-sm">Loading issues…</div>';
  } else {

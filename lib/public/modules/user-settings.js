...
// Read-only mirror of the shared server state. The sole writable control is
// Clay Settings → Coop, where the server authorizes the designated owner.
export function handleSharedLeadModeState(msg) {
  if (!msg || typeof msg.leadMode !== 'boolean') return;
  var value = document.getElementById('us-coop-lead-mode-value');
  if (value) value.textContent = msg.leadMode ? 'On' : 'Off';
}

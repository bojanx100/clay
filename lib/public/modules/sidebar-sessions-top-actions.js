...
function showCodexStartMenu(anchorBtn, deps) {
  ...
  var coordinatorItem = document.createElement("button");
  coordinatorItem.className = "session-ctx-item";
  coordinatorItem.innerHTML = iconHtml("git-branch") + " <span>New coordinator</span>";
  coordinatorItem.addEventListener("click", function (e) {
    e.stopPropagation();
    deps.closeMenu();
    sendUserAction({ type: "new_session", vendor: "codex", coordinator: true });
  });
  menu.appendChild(coordinatorItem);
...
function showClaudeStartMenu(anchorBtn, deps) {
  ...
  coordinatorItem.addEventListener("click", function (e) {
    e.stopPropagation();
    deps.closeMenu();
    sendUserAction({ type: "new_session", vendor: "claude", coordinator: true });
  });
  menu.appendChild(coordinatorItem);

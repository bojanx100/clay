...
import { getActivityEl, setActivityEl, addToMessages, scrollToBottom, currentProviderAvatar } from './app-rendering.js';
...
// Self-heal: the "thinking" activity dots have no other guaranteed clear path
// if the turn-ending message (done/result/status) is lost while the socket is
// a zombie (post-sleep / tunnel drop) and never replayed. Without this, the
// dots stick forever until a manual refresh. Re-armed on every setActivity.
var activitySelfHealTimer = null;
var ACTIVITY_SELF_HEAL_MS = 180000;
...
export function setSendBtnMode(mode) {
  ...
  var isTouchComposer = "ontouchstart" in window;
  if (mode === "stop" && !isTouchComposer) {
    ...
    sendBtn.title = "Send. Option-click to steer while a response is active.";
...
export function blinkIO() {
  ...
  var mobileSessionDot = document.querySelector('.mobile-session-item.active .mobile-session-processing') ||
                         document.querySelector('.mobile-loop-child.active .mobile-session-processing');
  ...
  ioTimer = setTimeout(function () {
    ...
    var sd = document.querySelector(".session-item.active .session-vendor-dot.io") ||
             document.querySelector(".session-item.active .session-processing.io") ||
...
export function blinkSessionDot(sessionId) {
  var el = document.querySelector('.session-item[data-session-id="' + sessionId + '"] .session-vendor-dot') ||
           document.querySelector('.session-item[data-session-id="' + sessionId + '"] .session-processing') ||
           document.querySelector('.mobile-session-item[data-session-id="' + sessionId + '"] .mobile-session-processing') ||
           document.querySelector('.mobile-loop-child[data-session-id="' + sessionId + '"] .mobile-session-processing');
  if (!el) return;
  el.classList.add("io");
...
export function setActivity(text) {
  if (activitySelfHealTimer) { clearTimeout(activitySelfHealTimer); activitySelfHealTimer = null; }
  if (text) {
    // Arm/re-arm the self-heal timer so the dots can never stick forever if the
    // turn-ending message is lost (zombie socket, no history replay on soft
    // reconnect). A live turn re-arms this on each activity event.
    activitySelfHealTimer = setTimeout(function () {
      activitySelfHealTimer = null;
      if (getActivityEl()) { getActivityEl().remove(); setActivityEl(null); }
    }, ACTIVITY_SELF_HEAL_MS);
    if (!getActivityEl()) {
      ...
      if (!_isDm) {
        var _avatarUrl = currentProviderAvatar();
        _actEl.innerHTML =

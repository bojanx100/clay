...
export function sendMessage(options) {
  ...
  if (isActiveCoopTopicStale()) {
    ctx.addSystemMessage("This Coop topic is no longer available. Select an available topic before sending.", true);
    return;
  }
  ...
  if (ingressType) {
    ...
    if (voiceCoopRouting && voiceCoopRouting.stale) {
      ctx.addSystemMessage("The Coop topic used when voice recording started is no longer available. The message was not sent.", true);
      return;
    }

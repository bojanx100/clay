...
import { sendVoiceText } from './voice-conversation-send.js';
...
// Voice sends directly so a spoken turn cannot overwrite a draft or send
// unrelated attachments. The server still applies the ordinary queue rules.
export function sendVoiceConversationMessage(text, routing, clientMessageId) {
  if (!sendVoiceText(text, routing, clientMessageId)) return false;
  if (ctx && !ctx.processing) ctx.addUserMessage(text, null, null, null, null, { optimisticId: clientMessageId });
  return true;
}

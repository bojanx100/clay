...
export function sendMessage(options) {
  ...
  if (isActiveCoopTopicStale()) {
    ctx.addSystemMessage("This Coop Thread is no longer available. Select an available Thread before sending.", true);
    return;
  }
  var activeCoopRouting = getActiveCoopIngressRefs();
  ...
  if (ingressType) {
    ...
    if (voiceCoopRouting) activeCoopRouting = voiceCoopRouting;
  }
...
function sendInputSync() {
  ...
  if (isActiveCoopTopicStale()) return;

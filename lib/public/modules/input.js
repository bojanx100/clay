...
import { isQueueingDisabledForCurrentSession } from './queued-messages.js';
...
function setComposerIntent(intent) {
  composerIntent = intent === "task" || intent === "queue" || intent === "steer"
    ? intent
    : "chat";
  var selector = document.getElementById("composer-intent");
  if (selector && selector.value !== composerIntent) selector.value = composerIntent;
  var wrapper = document.getElementById("input-wrapper");
  if (wrapper) wrapper.dataset.messageIntent = composerIntent;
  if (ctx && ctx.inputEl) {
    var placeholders = {
      chat: "Message the coordinator...",
      task: "Assign a task to a worker...",
      queue: "Queue for the coordinator's next turn...",
      steer: "Steer the current turn...",
    };
    ctx.inputEl.placeholder = placeholders[composerIntent];
  }
}

// --- Send ---
export function sendMessage(options) {
  var intent = options && options.intent ? options.intent : composerIntent;
  ...
  ctx.inputEl.value = "";
  if (intent !== "chat") setComposerIntent("chat");
  sendInputSync();
  clearPendingImages();
  autoResize();
  ctx.inputEl.focus();
...
export function initInput(_ctx) {
  ctx = _ctx;
  var intentSelector = document.getElementById("composer-intent");
  if (intentSelector) {
    intentSelector.addEventListener("change", function () {
      setComposerIntent(intentSelector.value);
  ...
  ctx.inputEl.addEventListener("keydown", function (e) {
    ...
    if (e.key === "Enter" && !e.shiftKey && !isComposing) {
      ...
      sendMessage({ intent: (e.metaKey || e.ctrlKey) ? "task" : (e.altKey ? "steer" : composerIntent) });
    }
  });
  ...
  ctx.sendBtn.addEventListener("click", function (e) {
    ...
    if (!hasSendableContent() && ghost) {
      ctx.inputEl.value = ghost;
      if (ctx.hideSuggestionChips) ctx.hideSuggestionChips();
      sendMessage({ intent: e.altKey ? "steer" : composerIntent });
      return;
    }

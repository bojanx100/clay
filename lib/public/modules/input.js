...
import { takeVoiceIngress } from './stt.js';
...
export function sendMessage(options) {
  ...
  var coopLens = store.get("activeCoopLens");
  if (store.get("activeCoopHome") && coopLens && coopLens.projectRef) {
    payload.coopProjectRef = coopLens.projectRef;
  }
...
function sendInputSync() {
  ...
  ctx.ws.send(JSON.stringify({ type: "input_sync", text: ctx.inputEl.value, sessionId: store.get("activeSessionId") || null }));
}

...
export function restoreInputDraftForSession(slug, sessionId) {
  ...
  restoreInputDraft(key ? (drafts[key] || null) : null);
}

// A session switch is asynchronous: the owner keeps typing while it is in
// flight, and the restore lands afterwards. Assigning over the composer at
// that moment silently destroys text the owner had already typed -- reported
// on both first load and project change, and reliably reproducible whenever
// the switch is slow enough to type into.
//
// Typed text is always the freshest intent, so it wins over any stored draft.
// The draft's attachments are still merged in, since those cannot be retyped.
export function restoreInputDraft(draft) {
  ...
  var wouldClobber = !!live && live !== incoming && incoming.indexOf(live) !== 0;

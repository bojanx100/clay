...
import { canUseTaskIntent, isQueueingDisabledForCurrentSession } from './queued-messages.js';
import { meaningfulTextTitle } from './text-title.js';
import { getSTTCoopRouting, takeVoiceIngress } from './stt.js';
import { captureCoopComposerScope } from './coop-composer-scope.js';
import { stageVoiceConversationIngress, takeVoiceConversationIngress } from './voice-conversation-routing.js';
import { sendWsJson } from './ws-ref.js';
import { imageResizePlan } from './input-image-normalize.js';
...
var restoreInputDraftListenerBound = false;
var draftUnloadListenerBound = false;
// Composer drafts are mirrored to sessionStorage so an unexpected page reload
// (e.g. a server restart that triggers a reconnect/reload) doesn't lose
// in-flight typing. This is transient per-tab crash-recovery state, NOT a user
// setting/preference (those are forbidden from web storage and persisted
// server-side). sessionStorage is cleared when the tab closes — exactly the
// lifetime a draft should have.
var DRAFT_STORAGE_KEY = "clay-session-drafts";
...
export var builtinCommands = [
  ...
  { name: "hide", desc: "Hide this session" },
  { name: "close", desc: "Hide this session (alias for /hide)" },
  { name: "archive", desc: "Hide this session (alias for /hide)" },
  ...
  { name: "provider", desc: "Switch provider without downgrading model" },
  { name: "switch", desc: "Switch provider and accept its selected model" },
  { name: "launch", desc: "Preview or start project task sessions" },
  { name: "task", desc: "Run this message in a coordinated worker" },
];

// --- Send ---
export function sendMessage(options) {
  var intent = options && options.intent === "task" && canUseTaskIntent(store.snap()) ? "task" : "chat";
  var steer = intent === "steer" || !!(options && options.steer);
  if (!steer && ctx.processing && isQueueingDisabledForCurrentSession()) {
    steer = true;
  }
  var willQueue = intent === "queue" || !!(ctx.processing && !steer && intent !== "task");
  ...
  if (text.indexOf("/task ") === 0) {
    var taskIntentAvailable = canUseTaskIntent(store.snap());
    intent = taskIntentAvailable ? "task" : "chat";
    steer = !taskIntentAvailable && !!(ctx.processing && isQueueingDisabledForCurrentSession());
    willQueue = !taskIntentAvailable && !!(ctx.processing && !steer);
  ...
  if (text === "/close" || text === "/hide" || text === "/archive") {
    var activeSessionId = store.get("activeSessionId");
    ctx.inputEl.value = "";
    clearPendingImages();
    autoResize();
    if (ctx.ws && ctx.connected && activeSessionId) {
      ctx.ws.send(JSON.stringify({ type: "hide_session", id: activeSessionId }));
    } else if (!ctx.connected) {
      ctx.addSystemMessage("Not connected — command not sent.", true);
    } else {
      ctx.addSystemMessage("No active session to hide.", true);
    }
    return;
  }
  ...
  if (text === "/compact" || text.indexOf("/compact ") === 0) {
    var compactText = text.substring("/compact".length).trim();
    ctx.inputEl.value = "";
    clearPendingImages();
    autoResize();
    if (ctx.ws && ctx.connected) {
      ctx.ws.send(JSON.stringify({
        ...
        text: compactText || "Continue from the compacted context.",
      }));
    } else {
      ctx.addSystemMessage("Not connected — command not sent.", true);
    }
  ...
  if (text === "/launch" || text.indexOf("/launch ") === 0) {
    ctx.inputEl.value = "";
    clearPendingImages();
    autoResize();
    if (ctx.ws && ctx.connected) {
      ctx.ws.send(JSON.stringify({ type: "task_launch", command: text }));
    } else {
      ctx.addSystemMessage("Not connected — command not sent.", true);
    }
    return;
  ...
  var activeCoopRouting = captureCoopComposerScope();
  var ingressType = takeVoiceIngress();
  var voiceCoopRouting = null;
  if (!ingressType) {
    voiceCoopRouting = takeVoiceConversationIngress();
    if (voiceCoopRouting) ingressType = "voice";
  }
  if (ingressType) {
    if (!voiceCoopRouting) voiceCoopRouting = getSTTCoopRouting();
    if (voiceCoopRouting && voiceCoopRouting.stale) {
      ctx.addSystemMessage("The Coop Thread used when voice recording started is no longer available. The message was not sent.", true);
      return;
    }
    if (voiceCoopRouting && voiceCoopRouting.scope) activeCoopRouting = voiceCoopRouting;
  }
  // A Voice utterance has already captured its exact ThreadRef. It remains
  // valid even if the UI lens changed while the microphone was open; text
  // messages still fail closed on a stale current composer destination.
  if (activeCoopRouting && activeCoopRouting.stale) {
    ctx.addSystemMessage("The Coop composer destination is no longer available. Select a destination before sending.", true);
    return;
  }
  ...
  if (scheduleDelayMs > 0) {
    ...
    var schedPayload = { type: "schedule_message", text: text || "", resetsAt: resetsAt, sessionId: store.get("activeSessionId") || null };
    if (ingressType) schedPayload.ingressType = ingressType;
    if (activeCoopRouting) {
      schedPayload.coopComposerScope = activeCoopRouting.scope;
      if (activeCoopRouting.topicRef) schedPayload.coopTopicRef = activeCoopRouting.topicRef;
      if (activeCoopRouting.projectRef) schedPayload.coopProjectRef = activeCoopRouting.projectRef;
    }
    // Carry images so a scheduled message keeps its attachment instead of
    // dropping it and queueing text only.
    if (images.length > 0) schedPayload.images = images;
    if (!sendWsJson(schedPayload)) {
      ctx.addSystemMessage("Connection lost — scheduled message not sent. Reconnecting now.", true);
      return;
    }
    clearScheduleDelay();
    ctx.inputEl.value = "";
  ...
  var payload = { type: "message", text: text || "", clientMessageId: clientMessageId };
  if (ingressType) payload.ingressType = ingressType;
  if (activeCoopRouting) {
    payload.coopComposerScope = activeCoopRouting.scope;
    if (activeCoopRouting.topicRef) payload.coopTopicRef = activeCoopRouting.topicRef;
    if (activeCoopRouting.projectRef) payload.coopProjectRef = activeCoopRouting.projectRef;
  }
  ...
  if (ctx.processing) payload.steer = steer ? true : false;
  if (!sendWsJson(payload)) {
    ctx.addSystemMessage("Connection lost — message not sent. Reconnecting now.", true);
    return;
  }

  // Render immediately for messages that start or steer a turn. Queued
  // messages stay in the queue bar and enter the transcript when the server
  // dispatches them as the next turn. Rendering happens only after the socket
  // accepted the frame, so a closing/zombie socket cannot leave a false bubble
  // while the composer is cleared.
  if (!willQueue) {
    ctx.addUserMessage(text, images.length > 0 ? images : null, pastes.length > 0 ? pastes : null, null, null, { optimisticId: clientMessageId });
  }
  ...
  if (_committedVendor) {
    if (_vtw2) {
      ...
      if (_avi && _avIcon) {
        var _committedRouteId = store.get('currentProviderRouteId') || null;
      var _committedModel = store.get('currentModel') || "";
      var _committedName = providerShortName(_committedVendor, _committedRouteId, _committedModel);
      _avIcon.src = providerAvatar(_committedVendor, _committedRouteId, _committedModel);
        _avIcon.alt = _committedName;
        _avi.title = _committedName + " session";
  ...
  // Show pre-thinking dots before server responds. Queued prompts stay in
  // the queue banner until they actually run.
  if (!willQueue) {
...
// Dedicated conversational Voice mode stages its immutable routing snapshot
// here and reuses the normal message/queue path. That keeps Voice messages in
// the canonical transcript with trusted Coop ingress semantics instead of
// creating a parallel Webapp-only send path.
export function sendVoiceConversationMessage(text, routing) {
  var utterance = typeof text === "string" ? text.trim() : "";
  if (!ctx || !ctx.inputEl || !utterance || !stageVoiceConversationIngress(routing)) return false;
  ctx.inputEl.value = utterance;
  autoResize();
  sendMessage({ intent: "chat" });
  return true;
}
...
function imagePayloadFromDataUrl(dataUrl) {
  ...
  if (!typeMatch || !data) return null;
  return { mediaType: typeMatch[1], data: data };
}

function addPendingImage(dataUrl, providerDataUrl) {
  var image = imagePayloadFromDataUrl(dataUrl);
  if (!image) return;
  var providerImage = providerDataUrl ? imagePayloadFromDataUrl(providerDataUrl) : null;
  if (providerImage) {
    image.providerMediaType = providerImage.mediaType;
    image.providerData = providerImage.data;
  }
...
// Re-add an existing image (by URL, e.g. a session screenshot) into the
// composer as a pending attachment, reusing the normal paste pipeline.
export function addPendingImageFromUrl(url) {
  return fetch(url).then(function (r) {
    ...
    if (ctx && ctx.inputEl) { ctx.inputEl.focus(); autoResize(); }
  });
}
...
function draftKey(slug, sessionId) {
  if (!slug || !sessionId) return "";
  return String(slug) + ":session:" + String(sessionId);
}
...
function persistDraftsToStorage() {
  try {
    sessionStorage.setItem(DRAFT_STORAGE_KEY, JSON.stringify(store.get('sessionDrafts') || {}));
  } catch (e) {}
}

// Called once at bootstrap (initInput) before the initial session_switched so
// restoreInputDraftForSession can pick up a draft that survived a reload.
export function hydrateDraftsFromStorage() {
  try {
    var raw = sessionStorage.getItem(DRAFT_STORAGE_KEY);
    if (!raw) return;
    var saved = JSON.parse(raw);
    if (saved && typeof saved === "object") {
      // In-memory drafts (if any) win — they're fresher than the persisted copy.
      store.set({ sessionDrafts: Object.assign({}, saved, store.get('sessionDrafts') || {}) });
    }
  } catch (e) {}
}

// Coalesced so a fast typist does not write sessionStorage on every keystroke.
var draftSnapshotTimer = null;
function scheduleDraftSnapshot() {
  if (draftSnapshotTimer) return;
  draftSnapshotTimer = setTimeout(function () {
    draftSnapshotTimer = null;
    try {
      saveInputDraftForSession(store.get('currentSlug'), store.get('activeSessionId'));
    } catch (e) {}
  }, 250);
}

export function saveInputDraftForSession(slug, sessionId) {
  var key = draftKey(slug, sessionId);
  if (!key) return;
  var drafts = Object.assign({}, store.get('sessionDrafts') || {});
  var draft = getInputDraft();
  if (hasDraftContent(draft)) {
    drafts[key] = draft;
  } else {
    delete drafts[key];
  }
  store.set({ sessionDrafts: drafts });
  persistDraftsToStorage();
}

export function restoreInputDraftForSession(slug, sessionId) {
  var key = draftKey(slug, sessionId);
  var drafts = store.get('sessionDrafts') || {};
  // A confirmed session switch is authoritative: the destination chat owns
  // its composer, including an intentionally empty draft.
  restoreInputDraft(key ? (drafts[key] || null) : null, { replace: true });
}

// Recovery restores can land asynchronously after the owner has started
// typing. Keep that newer live text unless the caller is applying an
// authoritative chat/reset transition with { replace: true }.
//
// The draft's attachments are still merged in, since those cannot be retyped.
export function restoreInputDraft(draft, options) {
  if (!ctx || !ctx.inputEl) return;
  var live = ctx.inputEl.value || "";
  var incoming = typeof draft === "string" ? draft : ((draft && draft.text) || "");
  // Only a genuine divergence is a clobber. Restoring the same text (or a
  // prefix the owner has since extended) is the normal idempotent repaint.
  var preserveLive = !(options && options.replace);
  var wouldClobber = preserveLive && !!live && live !== incoming && incoming.indexOf(live) !== 0;
  if (typeof draft === "string") {
    if (!wouldClobber) ctx.inputEl.value = draft;
    pendingImages = [];
    pendingPastes = [];
    pendingFiles = [];
  } else {
    draft = draft || {};
    if (!wouldClobber) ctx.inputEl.value = draft.text || "";
    pendingImages = Array.isArray(draft.images) ? draft.images.slice() : [];
    pendingPastes = normalizeDraftPastes(draft.pastes);
    pendingFiles = Array.isArray(draft.files) ? draft.files.slice() : [];
  }
  renderInputPreviews();
  autoResize();
  // Dismiss any autocomplete left open in the previous input context (e.g. a
  // slash-command menu still showing after switching sessions/projects).
  hideSlashMenu();
  hideMentionMenu();
}

function normalizeDraftPastes(pastes) {
  var out = [];
  if (!Array.isArray(pastes)) return out;
  for (var i = 0; i < pastes.length; i++) {
    var paste = pastes[i];
    if (typeof paste === "string") {
      out.push({ text: paste, preview: meaningfulTextTitle(paste, 80) || paste.slice(0, 80) });
    } else if (paste && typeof paste.text === "string") {
      out.push({
        text: paste.text,
        preview: paste.preview || meaningfulTextTitle(paste.text, 80) || paste.text.slice(0, 80),
      });
    }
  }
  return out;
...
function readImageBlob(blob) {
  ...
  reader.onload = function (ev) {
    ...
    var img = new Image();
    img.onload = function () {
      var plan = imageResizePlan(img.naturalWidth, img.naturalHeight, estimatedBytes);
      if (!plan.resize) {
        addPendingImage(dataUrl);
        return;
      }
      ...
      cx.drawImage(img, 0, 0, plan.width, plan.height);
      var typeMatch = dataUrl.match(/^data:(image\/(?:png|jpeg|gif|webp));base64,/i);
      var sourceType = typeMatch ? typeMatch[1].toLowerCase() : "image/png";
      var outputType = plan.lossy ? "image/jpeg" :
        (sourceType === "image/gif" ? "image/png" : sourceType);
      var resized = canvas.toDataURL(outputType, RESIZE_QUALITY);
      if (plan.lossy) {
        addPendingImage(resized);
      } else {
        addPendingImage(dataUrl, resized);
      }
    };
    img.onerror = function () {
      if (ctx.addSystemMessage) ctx.addSystemMessage("Image could not be decoded", true);
    };
    img.src = dataUrl;
  };
...
function getAllCommands() {
  var commands = builtinCommands.concat(ctx.slashCommands());
  if (canUseTaskIntent(store.snap())) return commands;
  return commands.filter(function (command) { return command.name !== "task"; });
}
...
function sendInputSync() {
  ...
  var syncPayload = { type: "input_sync", text: ctx.inputEl.value, sessionId: store.get("activeSessionId") || null };
  var activeCoopRouting = captureCoopComposerScope();
  if (activeCoopRouting) {
    syncPayload.coopComposerScope = activeCoopRouting.scope;
    if (activeCoopRouting.topicRef) syncPayload.coopTopicRef = activeCoopRouting.topicRef;
    if (activeCoopRouting.projectRef) syncPayload.coopProjectRef = activeCoopRouting.projectRef;
  }
  ctx.ws.send(JSON.stringify(syncPayload));
}
...
export function initInput(_ctx) {
  ctx = _ctx;
  // Restore any drafts that survived a reload before the initial session_switched
  // triggers restoreInputDraftForSession.
  hydrateDraftsFromStorage();
  if (!draftUnloadListenerBound) {
    draftUnloadListenerBound = true;
    // Capture the live composer state right before the tab unloads/reloads so an
    // unexpected refresh keeps the in-flight draft. pagehide covers reloads and
    // navigations; visibilitychange(hidden) is the reliable mobile equivalent.
    var saveCurrentDraft = function () {
      var s = store.snap();
      saveInputDraftForSession(s.currentSlug, s.activeSessionId);
    };
  ...
  if (!restoreInputDraftListenerBound) {
    restoreInputDraftListenerBound = true;
    window.addEventListener("clay:restore-input-draft", function (e) {
      restoreInputDraft(e.detail || null);
      if (ctx.inputEl) {
  ...
  document.addEventListener("paste", function (e) {
    ...
    // A substantial text/plain payload is almost always the user's intent, even
    // when the source app ALSO placed an image/RTF snapshot on the clipboard (a
    // rich-copy artifact from editors, terminals, spreadsheets, etc). Capture it
    // as a paste chip BEFORE the image/file branches below — otherwise "copy a
    // big log, paste it" gets hijacked into an image attachment and the text
    // never reaches the model (it arrives "empty").
    var earlyText = cd.getData ? cd.getData("text/plain") : "";
    if (earlyText && earlyText.length >= 500) {
      e.preventDefault();
      var earlyFallback = earlyText.substring(0, 50).replace(/\n/g, " ");
      var earlyPreview = meaningfulTextTitle(earlyText, 70) || earlyFallback;
      if (earlyPreview === earlyFallback && earlyText.length > 50) earlyPreview += "...";
      pendingPastes.push({ text: earlyText, preview: earlyPreview });
      renderInputPreviews();
      return;
    }
    ...
    // Long text paste (>=500 chars) is handled at the top of this handler,
    // before the image/file branches, so it is never reached here.
  ...
  ctx.inputEl.addEventListener("input", function () {
    autoResize();
    sendInputSync();
    // Mirror keystrokes into the persisted draft. Saving only on switch and
    // unload lost anything typed before the FIRST session_switched, because
    // no snapshot of it had been taken yet.
  ...
  ctx.inputEl.addEventListener("keydown", function (e) {
    ...
    if (e.key === "Enter" && !e.shiftKey && !isComposing) {
      ...
      // Cmd/Ctrl+Enter remains a normal textarea newline shortcut.
      ...
      var enterIntent = e.altKey && canUseTaskIntent(store.snap()) ? "task" : "chat";
      sendMessage({ intent: enterIntent });
    }
  });
  ...
  // must not adopt a ghost suggestion. On touch devices setSendBtnMode keeps
  // the primary composer action in send mode, so an empty click is a no-op.
  // Otherwise it's a send.
  ...
  if (taskBtn) {
    taskBtn.addEventListener("click", function () {
      if (hasSendableContent() && canUseTaskIntent(store.snap())) {
        sendMessage({ intent: "task" });
      }
    });
  }
}

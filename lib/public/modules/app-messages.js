...
import { restoreCoopLiveConversation, handleCoopOwnerDecisionStaged, projectMainCoopStreamMessage, shouldSuppressCoopTopicStream } from './app-messages-coop-topics.js';
...
import { handleHumanAttentionMessage } from './human-attention.js';
...
export function processMessage(msg) {
    ...
    if (handleHumanAttentionMessage(msg)) return;
    if (isStaleSessionMessage(msg)) return;
    ...
    if (msg.type === "history_done") restoreCoopLiveConversation(msg);
    if (handleHistoryMessage(msg)) return;
    if (handleSessionMessage(msg)) return;
    if (handleToolMessage(msg)) return;
    if (msg.type === "coop_internal_turn_started") {
      finalizeAssistantBlock();
      store.set({ coopTopicLiveTurnVisible: false });
      return;
    }
    if (msg.type === "coop_owner_response_started") {
      finalizeAssistantBlock();
      var selectedTopic = store.get("activeCoopTopicRef");
      if (selectedTopic && (msg.coopTopicRefs || []).some(function (ref) {
        return ref.topicId === selectedTopic.topicId;
      })) store.set({ coopTopicLiveTurnVisible: true });
      return;
    }
    if (handleCoopOwnerDecisionStaged(msg)) { finalizeAssistantBlock(); return; }
    ...
    switch (msg.type) {
      ...
      case "info":
        if (msg.text && !msg.project && !msg.cwd) {
          addSystemMessage(msg.text, false, msg.variant, msg.conversationRef);
          break;
        }
        ...
        var _providerWs = getWs();
        if (_providerWs && _providerWs.readyState === 1) {
          _providerWs.send(JSON.stringify({ type: "get_provider_status" }));
        }
        store.set({
        ...
        if (store.get('dmMode') && store.get('dmTargetUser') && store.get('dmTargetUser').isMate) {
          ...
          updateTitleBarProjectIdentity(msg.title || store.get('projectName'), msg.icon || null);
        }
        updatePageTitle();
        if (msg.version) {

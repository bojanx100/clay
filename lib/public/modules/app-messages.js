...
// --- App module imports ---
import { addToMessages, addSystemMessage, finalizeAssistantBlock, getPrependAnchor, providerLabel } from './app-rendering.js';
...
import { shouldSuppressCoopTopicStream } from './app-messages-coop-topics.js';
    ...
    switch (msg.type) {
      ...
      case "info":
        ...
        if (store.get('dmMode') && store.get('dmTargetUser') && store.get('dmTargetUser').isMate) {
          ...
          // Runs on every connect and reconnect. Route both through the Coop
          // identity rule so a reconnect cannot repaint a stale Lead name.
          var _infoTitle = coopHeaderTitle(store.get('currentSlug'), store.get('projectName'), "Clay");
          headerTitleEl.textContent = _infoTitle;
          var tbProjectName = document.getElementById("title-bar-project-name");
          if (tbProjectName) {
            tbProjectName.textContent = isCoopProjectSlug(store.get('currentSlug'))
              ? _infoTitle
              : (msg.title || store.get('projectName'));
          }
        }
        updatePageTitle();
        if (msg.version) {

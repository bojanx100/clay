...
export function processMessage(msg) {
    ...
    switch (msg.type) {
      ...
      case "history_done":
        ...
        if (messagesEl) {
          // --- Freeze instrumentation: time the synchronous highlight pass ---
          // This loop calls hljs.highlightElement per code block over the whole
          // transcript in one go and is the prime suspect for the multi-second
          // main-thread freeze on session/project switch. Log block count +
          // per-stage timings so we can confirm before optimizing. Filter the
          // console with: [clay-perf]
          var _now = (window.performance || Date).now.bind(window.performance || Date);
          var _blockCount = messagesEl.querySelectorAll("pre code:not(.hljs):not(.language-mermaid)").length;
          ...
          var _tMm1 = _now();
          var _hlMs = _tHl1 - _tHl0;
          var _mmMs = _tMm1 - _tHl1;
          var _totalMs = _replayPerf ? (_tMm1 - _replayPerf.startedAt) : null;
          try {
            console.log(
              "[clay-perf] history replay: " +
              (_replayPerf ? _replayPerf.total : "?") + " items, " +
              _blockCount + " code blocks, " + _mermaidCount + " mermaid | " +
              "highlight=" + _hlMs.toFixed(0) + "ms" +
              " mermaid=" + _mmMs.toFixed(0) + "ms" +
              (_totalMs != null ? " total-since-meta=" + _totalMs.toFixed(0) + "ms" : "") +
              (_hlMs > 100 ? "  <-- MAIN-THREAD FREEZE" : "")
            );
          } catch (e) {}
          _replayPerf = null;
        }

...
export function createMainAuthorityDisclosureProjector() {
  ...
  function project(message) {
    ...
    if (projected === rawAssistantText) return message;
    return Object.assign({}, message, { type: "delta_replace", text: projected });
  }
...
function progressTextKey(message) {
  if (!message || !INTERNAL_PROGRESS_TYPES[message.type] || typeof message.text !== "string") return "";
  var text = message.text.trim().replace(/\s+/g, " ");
  return text ? "progress:" + text : "";
}

// Live streaming has no explicit turn id on every event. The internal
// user_message opener is therefore the boundary that carries its provenance.
// Keep one distinct progress milestone visible, then hide unchanged copies from
// later automation turns. Owner-opened turns continue through the normal
// denylist path, so typed messages and their answers are never suppressed.
export function createTurnRelevanceTracker() {
  ...
  function reset() {
    internalTurn = false;
    seenProgress = Object.create(null);
  }
  ...
  function relevance(message) {
    ...
    if (!internalTurn) return messageRelevance(message);
    if (OWNER_ACTION_TYPES[message && message.type]) return RELEVANCE_OWNER;
    var key = progressTextKey(message);

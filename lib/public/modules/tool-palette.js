...
var SESSION_TOOLS = [
  ...
  { id: "dashboard-btn",           icon: "layout-dashboard", label: "Dashboard",     url: "/p/webapp/dashboard/", projects: ["webapp"] },
];
...
// Resolve relative tool routes against Clay's current origin. Legacy tools
// with a loopback host still follow the current hostname for remote access.
...
export function initToolPalettes() {
  ...
  // Apply project filter on init and whenever the active project changes.
  applyProjectFilter(store.get('currentSlug'));
  store.subscribe(function (state, prev) {
    if (state.currentSlug !== prev.currentSlug) {
      applyProjectFilter(state.currentSlug);
    }
  });
...
// Show or hide tools that declare a `projects` filter based on the active slug.
// Uses a CSS class so it's independent of the user-editable hide/show state.
function applyProjectFilter(slug) {
  for (var name in PALETTES) {
    var tools = PALETTES[name].tools;
    for (var i = 0; i < tools.length; i++) {
      var tool = tools[i];
      if (!tool.projects) continue;
      var btn = document.getElementById(tool.id);
      if (!btn) continue;
      var matches = slug && tool.projects.indexOf(slug) !== -1;
      btn.classList.toggle('palette-tile--proj-hidden', !matches);
    }
  }
}
...
function buildToolButton(tool, paletteName) {
  ...
  btn.addEventListener('click', function (e) {
    ...
    if (tool.url) {
      e.preventDefault();
      e.stopImmediatePropagation();
      window.open(resolveToolUrl(tool.url), '_blank', 'noopener');
    }
  });
...
function savePreferences(name) {
  ...
  for (var i = 0; i < activeTiles.length; i++) {
    if (!activeTiles[i].classList.contains('palette-tile--proj-hidden')) {
      order.push(activeTiles[i].dataset.toolId);
    }
  }
  ...
  if (hiddenGrid) {
    ...
    for (var j = 0; j < hiddenTiles.length; j++) {
      if (!hiddenTiles[j].classList.contains('palette-tile--proj-hidden')) {
        hidden.push(hiddenTiles[j].dataset.toolId);
      }
    }
  }

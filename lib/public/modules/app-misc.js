...
import { cancelPendingExtensionRecovery, recoverDisconnectedExtensionBridge } from './extension-bridge-recovery.js';
...
export function flushPendingExtMessages() {
  ...
  // Re-register the extension before replaying Live UI lifecycle events. A
  // daemon restart forgets the extension socket and every in-memory pairing.
  ...
  if (_pendingExtMessages.length > 0) {
    ...
    for (var i = 0; i < queued.length; i++) {
      if (_extensionConnected && queued[i].type === "browser_tab_list") continue;
...
export function handleLiveUiBridgeMessage(msg) {
  if (!msg) return false;
  if (msg.type === "clay_live_ui_relay" && msg.envelope) {
    sendOrQueue(msg.envelope);
    return true;
  }
...
export function handleExtensionDisconnect(diagnostic) {
  ...
  sendOrQueue({
    type: "browser_tab_list",
    tabs: [],
    connected: false, disconnectReason: String(diagnostic && diagnostic.reason || "port_disconnected").slice(0, 160)
  });
}
...
export function initMisc() {
  ...
  window.addEventListener("message", function(event) {
    ...
    if (msg.type === "clay_ext_disconnected") {
      handleExtensionDisconnect(msg);
    }
    handleLiveUiBridgeMessage(msg);

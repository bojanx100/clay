  ...
  switch (msg.type) {
    case "history_meta":
      store.set({
        ...
        historyScope: msg.scope || "all",
        historyFocusEventIndex: Number.isInteger(msg.focusEventIndex) ? msg.focusEventIndex : null,
      });
...
function handleHistoryDone(msg) {
  ...
  focusCanonicalHistoryEvent();
}

function focusCanonicalHistoryEvent() {
  var eventIndex = store.get('historyFocusEventIndex');
  if (!Number.isInteger(eventIndex) || !messagesEl) return;
  store.set({ historyFocusEventIndex: null });
  requestAnimationFrame(function () {
    var target = messagesEl.querySelector('[data-history-index="' + eventIndex + '"]');
    if (!target) return;

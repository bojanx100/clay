...
import { initSchedulerConfig, setupCreateModal, openCreateModal, openCreateModalWithRecord, closeCreateModal, openDeleteDialog, removePreview, getPreviewEl, showPreviewOnCell, showPreviewOnSlot, showPreviewForCreate, applyDraggedTask } from './scheduler-config.js';
...
import { DAY_NAMES, DAY_SHORT, MONTH_NAMES, getISOWeekNumber, getWeekStart, pad, esc, formatDateTime, cronToHuman, detectInterval, parseCronSimple } from './scheduler-utils.js';
    ...
    (function (btn) {
      btn.addEventListener("click", function (e) {
        ...
        if (action === "run") {
          ...
          openDeleteDialog(selectedTaskId, null, true);
  ...
  for (var i = 0; i < btns.length; i++) {
    (function (btn) {
      btn.addEventListener("click", function (e) {
        ...
        else if (action === "delete") openDeleteDialog(id, null, !rec.cron);
      });
    })(btns[i]);
  }
}
...
export function getUpcomingSchedules(windowMs) {
  ...
  for (var i = 0; i < filtered.length; i++) {
    ...
    // Recurring schedules (those with a cron, e.g. auto-launch) stay visible even
    // when nextRunAt has just passed: right after a run fires the registry briefly
    // leaves nextRunAt in the past before computing the next occurrence, which
    // otherwise made the countdown chip flicker out every cycle. An enabled
    // recurring schedule should stay visible the whole time. One-off schedules
    // still drop off once they are due (diff > 0 required).
    var include = r.cron ? (diff <= windowMs) : (diff > 0 && diff <= windowMs);
    if (include) {
      result.push({ id: r.id, name: r.name, nextRunAt: r.nextRunAt, color: r.color || "", paused: !!r.paused });
    }
  }
  return result;
}

...
import { buildCreateCronFromOptions } from './scheduler-cron-builders.js';
import { DAY_NAMES, MONTH_NAMES } from './scheduler-utils.js';
...
export function openDeleteDialog(recId, eventDate, isOneOff) {
...
function buildCreateCron() {
  ...
  for (var i = 0; i < btns.length; i++) {
    customDays.push(btns[i].dataset.dow);
  }
  var customIntervalEl = document.getElementById("sched-custom-interval");
  var customUnitEl = document.getElementById("sched-custom-unit");

  return buildCreateCronFromOptions({
    selectedDate: createSelectedDate,
    hour: h,
    minute: m,
    recurrence: createRecurrence,
    customConfirmed: createCustomConfirmed,
    interval: createInterval,
    intervalCustom: createIntervalCustom,
    customInterval: customIntervalEl ? parseInt(customIntervalEl.value, 10) || 1 : 1,
    customUnit: customUnitEl ? customUnitEl.value : "week",

...
// selected Coop Thread, rendered above the conversation.
//
...
import { createThreadControls } from './coop-thread-controls.js';
...
export function buildTopicDecisionSurface(topic, options) {
  ...
  surface.setAttribute("aria-label", "Controls for " + canonicalTopicTitle(topic, "this Thread"));
  var heading = document.createElement("div");
  heading.className = "coop-topic-decision-heading";
  heading.setAttribute("role", "heading");
  heading.setAttribute("aria-level", "2");
  heading.textContent = "Thread — " + canonicalTopicTitle(topic, "this Thread");
  surface.appendChild(heading);
  surface.appendChild(createThreadControls(topic, opts));

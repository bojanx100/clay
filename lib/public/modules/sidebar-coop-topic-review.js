...
export function provenanceText(topic) {
  ...
  if (source === "unlinked_default") {
    return "No linked task records prove how this historical Thread was resolved. It waits for your decision.";
  }
  ...
  if (source === "owner_disposition:unlinked_historical") {
    return "Recorded as unresolved: no linked task records prove how this historical Thread ended.";
  }
...
var ERROR_TEXT = {
  ...
  stale_state: "This Thread changed. Review the updated state.",
...
// Repaint signature for the sidebar list: only the Closed section toggle affects
...
function buildPanel(topic, verbs, prefix, id, panelId) {
  ...
  consequence.textContent = topic.workState === "done"
    ? "Reopen returns this Thread to Needs input."
    : "Accept records this Thread as resolved. Request changes records what should change. Keep waiting leaves it as is.";
...
export function createTopicDecisionPanel(topic, options) {
  ...
  wrapper.setAttribute("aria-label", "Decide Thread " + canonicalTopicTitle(topic, "Thread"));

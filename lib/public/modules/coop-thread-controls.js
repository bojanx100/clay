...
export function openThreadCloseDialog(topic, send, restore) {
  ...
  var built = dialog("Close Thread", "Choose the outcome for “" + title +
    "”. Its canonical transcript provenance remains available.", restore);
  ...
  actions.appendChild(dialogButton("Implemented / resolved", "coop-thread-dialog-button", function () {
    choose("implemented_resolved");
  }));
  actions.appendChild(dialogButton("Not pursuing", "coop-thread-dialog-button destructive", function () {
    choose("not_pursuing");
  }));

...
export function handleRateLimitMessage(msg) {
  switch (msg.type) {
    ...
    case "scheduled_message_sent":
      ...
      // Persisted schedule events describe the old transcript. The current
      // session switch/status messages own the live processing controls.
      if (!store.get('replayingHistory')) setStatus("processing");
      return true;

...
import { addSystemMessage, scrollToBottom } from './app-rendering.js';
...
// iOS WebKit (Safari and every iOS browser, including iOS Chrome) mishandles
// continuous recognition: with `continuous = true` the session never returns a
// transcript and never ends, so the microphone stays hot while the composer sits
// on "Listening..." forever. Recognition must therefore be single-utterance on
// iOS, and a session that ends must be surfaced instead of silently restarted:
// iOS also refuses a `start()` that is not inside a user gesture, so the old
// restart-from-onend loop relocked the composer onto a dead microphone.
var NO_RESULT_TIMEOUT_MS = 8000;
...
var sawResultThisSession = false;
var noResultTimer = null;
var singleUtteranceMode = false;
...
// True for every iOS/iPadOS browser: they all run on WebKit and inherit its
// speech-recognition behavior regardless of the wrapper (Safari, CriOS, FxiOS).
function isAppleMobileWebKit() {
  if (typeof navigator === 'undefined') return false;
  var ua = navigator.userAgent || '';
  if (/iPad|iPhone|iPod/.test(ua)) return true;
  // iPadOS 13+ reports a desktop Macintosh UA; touch support separates it.
...
// Single-utterance recognition that produces nothing at all is the iOS hang.
// Nothing will ever fire, so a timer is the only way to stop lying to the user.
function armNoResultWatchdog() {
  if (!singleUtteranceMode) return;
  clearNoResultWatchdog();
  noResultTimer = setTimeout(function () {
    noResultTimer = null;
    if (recording && !sawResultThisSession) {
      finishWithNotice('Voice input heard nothing and has stopped. Tap the mic to try again.');
    }
  }, NO_RESULT_TIMEOUT_MS);
}

// Ends recording and tells the user why, so the composer never keeps a
// "Listening..." placeholder over a recognition session that is already dead.
function finishWithNotice(message) {
  stopRecording();
  if (message) addSystemMessage(message, true);
}
...
function startRecording() {
  ...
  // iOS returns no transcript at all when continuous is set; keep it off there.
  singleUtteranceMode = isAppleMobileWebKit();
  recognition.continuous = !singleUtteranceMode;
  ...
  recognition.onend = function() {
    ...
    // On iOS a session ends after a single utterance and cannot be restarted
    // outside a user gesture, so restarting here produced an invisible dead
    // microphone under a "Listening..." placeholder. End the turn visibly.
    if (singleUtteranceMode) {
      finishWithNotice(sawResultThisSession
        ? null
        : 'Voice input heard nothing and has stopped. Tap the mic to try again.');
      return;
    }
    try {
      recognition.start();
      ...
      finishWithNotice(sawResultThisSession
        ? null
        : 'Voice input stopped before it captured anything. Tap the mic to try again.');
    }
  };
  ...
  try {
    ...
    store.set({ voiceDictationActive: true });
    ...
    store.set({ voiceDictationActive: false });

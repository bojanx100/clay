...
import { coopHeaderTitle, isCoopProjectSlug, sessionHeaderCandidate } from './coop-identity.js';
...
export function updatePageTitle() {
  // Read only the dedicated title span, not the whole .session-item-text
  // container -- that container also holds role/vendor/auto badges as
  // adjacent siblings with no whitespace in the markup, so textContent on
  // the whole thing would glue e.g. "Coordinator" onto the title.
  var activeItem = ctx.sessionListEl.querySelector(".session-item.active .session-item-title");
  var sidebarSessionTitle = activeItem ? activeItem.textContent : "";
  // session_switched is the acknowledgement for the chat being displayed, so
  // its store title outranks a sidebar row rebuilt from an older session_list.
  // The row remains the fallback during initial connection, when session_list
  // intentionally arrives before session_switched, and for incomplete payloads.
  var sessionTitle = sessionHeaderCandidate(
    store.get('activeSessionId'), store.get('activeSessionTitle'), sidebarSessionTitle);
  // This runs on every session-list render, so before the Coop rule was applied
  // here it was the main way the header flipped Coop -> Lead: selecting a topic
  // or opening the sidebar re-rendered the list, and the header was recomputed
  // from the sidebar DOM / activeSessionTitle / projectName instead of from the
  // Coop identity.
  var slug = store.get('currentSlug');
  // A mate DM owns both title elements while it is open: app-messages-dm.js
  // paints the mate's name, and the info handler skips its own header write for
  // the same reason. This function runs on every session-list render, so it has
  // to leave them alone entirely -- writing the underlying session or project
  // title here would erase the mate's name a moment after it was set.
  if (!!store.get('dmMode') || document.body.classList.contains("mate-dm-active")) return;
  var isCoop = isCoopProjectSlug(slug);
  // Routed through the one applier so a sidebar render cannot paint over a
  // selected topic's title. Returns what it wrote, for the tab title below.
  var headerTitle = applyCoopChatHeader(sessionTitle, ctx.projectName)
    || coopHeaderTitle(slug, sessionTitle, ctx.projectName);
  var tbProjectName = ctx.$("title-bar-project-name");
  if (tbProjectName && isCoop) {
    // The title bar names the project, so it stays "Coop" even when the chat
    // header is showing a selected topic.
    tbProjectName.textContent = coopHeaderTitle(slug, null, null);
  } else if (tbProjectName && ctx.projectName) {
  ...
  // The tab title follows the same identity rule, so a backgrounded tab cannot
  // advertise a Lead persona the header no longer shows.
  var docProjectName = isCoop ? headerTitle : ctx.projectName;
  if (docProjectName && sessionTitle && !isCoop) {
    document.title = sessionTitle + " - " + docProjectName;
  } else if (docProjectName) {
    document.title = docProjectName + " - Clay";
  } else {
    document.title = "Clay";
  }
}

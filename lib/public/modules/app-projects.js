...
import { rememberTabSessionRef, projectNavigationHistoryUpdate, syncTabSessionRefUrl } from './session-tab-state.js';
  ...
  if (!st.mateProjectSlug) {
    for (var pi = 0; pi < cachedProjects.length; pi++) {
      if (cachedProjects[pi].slug === st.currentSlug) {
        // Runs right after the info handler writes the header, so it has to
        // apply the same Coop rule -- otherwise the server's project title
        // repaints a Lead identity moments after connect or reconnect.
        var updatedName = coopHeaderTitle(st.currentSlug,
          cachedProjects[pi].title || cachedProjects[pi].project, null);
...
export function switchProject(slug, options) {
  ...
  if (isHomeHubVisible()) {
    hideHomeHub();
    if (slug === store.get('currentSlug') && slug !== "lead" && !(options && options.sessionRef)) return;
  }

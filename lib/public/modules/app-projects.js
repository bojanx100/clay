...
import { rememberTabSessionRef, projectNavigationHistoryUpdate, syncTabSessionRefUrl, forgetTabSessionRefForProject } from './session-tab-state.js';
import { familyOf as deriveFamily, displayProjectIdentity } from './worktree-family.js';
...
export function switchProject(slug, options) {
  ...
  if (slug === store.get('currentSlug')) {
    ...
    store.set({ activeSessionId: null, activeSessionProjectSlug: null, cliSessionId: null }); resetClientState();
    connect({ preferProjectDefault: true });
    return;
  }
  resetFileBrowser();
  closeArchive();
  ...
  resetClientState();
  connect({ preferProjectDefault: !(options && options.sessionRef) });
}

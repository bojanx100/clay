...
import { coopHeaderTitle } from './coop-identity.js';
...
import { rememberTabSessionRef, projectNavigationHistoryUpdate, syncTabSessionRefUrl, forgetTabSessionRefForProject, readMostRecentTabProject } from './session-tab-state.js';
import { familyOf as deriveFamily, displayProjectIdentity, familyNavigationTarget, projectFamilySlugs } from './worktree-family.js';
import { clearGlobalCoopProjection } from './global-coop-projection.js';
...
export function updateTitleBarProjectIdentity(fallbackName, fallbackIcon) {
  ...
  var identity = displayProjectIdentity(
    cachedProjects, store.get('currentSlug'), fallbackName, fallbackIcon);
  var name = identity.name;
  name = coopHeaderTitle(store.get('currentSlug'), name, null);
  var icon = identity.icon;
...
export function resetClientState() {
  ...
  resetToolState();
  restoreInputDraft(null, { replace: true });
...
export function restoreCanonicalLeadHome() {
  if (store.get("currentSlug") !== "lead") return false;
  store.set({ activeGlobalSessionRef: null });
  syncTabSessionRefUrl("lead", null);
  var homeId = store.get("coopHomeSessionId");
  if (homeId && homeId === store.get("activeSessionId")) return true;
  if (homeId) return sendUserAction({ type: "switch_session", id: homeId });
  store.set({ activeSessionId: null, activeSessionProjectSlug: null, cliSessionId: null }); resetClientState();
  connect();
  return true;
}

function resolveProjectFamilyTarget(slug, options) {
  if (options && (options.sessionRef || options.exactProject)) return slug;
  var familySlugs = projectFamilySlugs(cachedProjects, slug);
  var rememberedSlug = readMostRecentTabProject(familySlugs);
  return familyNavigationTarget(cachedProjects, slug, rememberedSlug);
}

export function switchProject(slug, options) {
  if (!slug) return;
  slug = resolveProjectFamilyTarget(slug, options);
  var st = store.snap();
  ...
  if (slug === store.get('currentSlug')) {
    if (options && options.sessionRef && options.sessionRef.sessionStorageId) {
      syncTabSessionRefUrl(slug, options.sessionRef);
      return sendUserAction({ type: "switch_session", storageId: options.sessionRef.sessionStorageId });
    }
    if (slug === "lead") {
      restoreCanonicalLeadHome();
      return;
    }
    if (wasDm) {
  ...
  resetScheduler(slug);
  if (st.currentSlug === "lead" && slug !== "lead") {
    clearGlobalCoopProjection();
    forgetTabSessionRefForProject(slug, "system-lead");
  }
  store.set({ currentSlug: slug, activeSessionId: null, activeSessionProjectSlug: null, cliSessionId: null });
  window.dispatchEvent(new CustomEvent("clay:project-switching", { detail: { slug: slug } }));
  store.set({ basePath: "/p/" + slug + "/" });
  store.set({ wsPath: "/p/" + slug + "/ws" });
  var historyUpdate = projectNavigationHistoryUpdate(
    slug,
    options,
    document.documentElement.classList.contains("pwa-standalone")
  );
  history[historyUpdate.method](null, "", historyUpdate.url);
  resetClientState();
  connect();
}

...
import { autoResize, saveInputDraftForSession } from './input.js';
...
export function enterDmMode(key, targetUser, messages) {
  ...
  var s = store.snap();
  // Leaving the project context — close project settings if it's open.
...
export function connectMateProject(slug) {
  ...
  store.set({ currentSlug: slug, wsPath: "/p/" + slug + "/ws",
    activeSessionId: null, activeSessionProjectSlug: null, cliSessionId: null });
  resetClientState();
  connect();
}
...
export function disconnectMateProject() {
  ...
  saveInputDraftForSession(current.currentSlug, current.activeSessionId);

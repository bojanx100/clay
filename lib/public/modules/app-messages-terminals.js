...
export function handleTerminalMessage(msg) {
  switch (msg.type) {
    ...
    case "term_created":
      if (store.get('pendingProviderSetup')) {
        var _ps = store.get('pendingProviderSetup');
        store.set({ pendingProviderSetup: null });
        openTuiModal(msg.id, _ps.slug, {
          sessionTitle: (_ps.displayName || _ps.vendor || "Provider") + (_ps.action === "install" ? " installer" : " account"),
          projectName: _ps.slug,
          compact: true,
          setupVendor: _ps.vendor || null,
          setupAction: _ps.action || "login",
          setupDisplayName: _ps.displayName || null,
        });
        return true;
      }

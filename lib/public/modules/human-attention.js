...
function renderProjectRows(rows) {
  ...
  for (var r = 0; rows && r < rows.length; r++) {
    if (Number(rows[r].durationMs || 0) > 0) recorded.push(rows[r]);
  }
...
function buildUi() {
  ...
  wrap.innerHTML =
    ...
    '<section id="human-attention-popover" class="human-attention-popover hidden" role="dialog" aria-modal="false" aria-labelledby="human-attention-title">' +
      '<div class="human-attention-heading"><div><strong id="human-attention-title">Human work</strong><span>5am–5am · all Clay devices</span></div>' +
      ...
      '<div class="human-attention-body">' +
        ...
        '<p>Focused use plus a 3-minute thinking window. Hidden tabs and unattended AI work are excluded.</p>' +
      '</div>' +
    '</section>';

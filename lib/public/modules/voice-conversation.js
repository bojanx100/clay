...
// composer dictation: it is only started from the canonical Voice Thread and
// retains the ThreadRef copied before microphone permission is requested.

import { iconHtml, refreshIcons } from './icons.js';
import { store } from './store.js';
import { sendVoiceConversationMessage } from './input.js';
import { captureVoiceConversationRouting } from './voice-conversation-routing.js';
import { createVoiceConversationController } from './voice-conversation-controller.js';

export var VOICE_THREAD_ID = "recovery-voice-ingresses-360-362";
...
function topicId(ref) {
  return String(ref && (ref.threadId || ref.topicId || ref.topicKey || ref.id) || "").trim();
}

function selectedVoiceThread() {
  return !!(store.get("activeCoopHome") &&
    topicId(store.get("activeCoopTopicRef")) === VOICE_THREAD_ID);
}
...
function renderVisibility() {
  ...
  var shouldShow = selectedVoiceThread() || (active && (active.listening || active.working || active.speaking || active.reconnecting));
  ...
  if (refs.panel && shouldShow && !selectedVoiceThread()) {
    refs.threadHint.textContent = "Continuing the Voice Thread captured when recording began.";
  } else if (refs.threadHint) {
    refs.threadHint.textContent = "Messages stay in this Voice Thread.";
  }
  renderState(active);
}
...
function startOrStopListening() {
  ...
  if (!selectedVoiceThread()) {
    refs.status.textContent = "Open the Voice Thread before listening.";
    refs.status.classList.add("voice-conversation-error");
    return;
  }
  controller.start(captureVoiceConversationRouting());
}
...
function createPanel() {
  ...
  if (!inputArea || !inputWrapper || !attachWrap) return false;
  ...
  panel.innerHTML =
    ...
    '</div>';
  inputArea.insertBefore(panel, inputWrapper);
  refs = {
    button: button,
    panel: panel,

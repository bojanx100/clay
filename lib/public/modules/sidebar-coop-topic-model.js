...
var RETAINED_TERMINAL_STATUSES = {
  completed: true, dismissed: true, cancelled: true,
};
...
// active or needs the owner's attention. Terminal Thread containers may still
// carry active children, so recurse instead of treating handed-off history as
...
export function hasVisibleCoopProjectWork(node) {
  ...
  if ((role === "thread" || role === "task_coordinator" || role === "worker") &&
      (ACTIVE_COORDINATOR_STATUSES[status] || ATTENTION_COORDINATOR_STATUSES[status] ||
        ((role === "task_coordinator" || role === "worker") && RETAINED_TERMINAL_STATUSES[status]))) {
    return true;
  }

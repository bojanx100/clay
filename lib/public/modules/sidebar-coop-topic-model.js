...
//
// Order: Uncategorised, projects, Cross-project, Parked, then collapsed Closed.
// Threads are only the intake/discussion layer: once a
// matter is admitted to canonical execution its persistent project coordinator
// is the navigation surface, so the topic row leaves every section. A project
// hierarchy remains visible even when it has no intake topics; a category with
// neither Threads nor hierarchy yields no descriptor at all.
export function coopTopicSections(model) {
  ...
  for (var pi = 0; pi < projects.length; pi++) {
    var roots = projects[pi] && projects[pi].summary &&
      Array.isArray(projects[pi].summary.coordinatorTree)
      ? projects[pi].summary.coordinatorTree : [];
    for (var ri = 0; ri < roots.length; ri++) {
      var rootRef = roots[ri] && roots[ri].sessionRef;
      if (!rootRef || !rootRef.projectId || !rootRef.sessionStorageId) continue;
      coordinatorRefs[rootRef.projectId + ":" + rootRef.sessionStorageId] = true;
    }
  }
  function topicIsAdmitted(topic) {
    var source = safeText(topic && topic.stateSource, "");
    if (source.indexOf("task_") === 0 || source.indexOf("execution_") === 0) return true;
    var links = topic && Array.isArray(topic.relatedSessions) ? topic.relatedSessions : [];
    for (var li = 0; li < links.length; li++) {
      var sessionRef = links[li] && links[li].sessionRef;
      if (sessionRef && coordinatorRefs[sessionRef.projectId + ":" + sessionRef.sessionStorageId]) {
        return true;
      }
    }
    return false;
  }
  function splitThreads(topics) {
    var open = [];
    var list = Array.isArray(topics) ? topics : [];
    for (var i = 0; i < list.length; i++) {
      var state = list[i] && list[i].threadState || "exploring";
      if (state === "handed_off" || topicIsAdmitted(list[i])) continue;
      if (state === "closed") closedThreads.push(list[i]);
      else if (state === "parked") parkedThreads.push(list[i]);
      else open.push(list[i]);
    }
    return open;
  }
  var uncategorised = splitThreads(value.uncategorisedTopics);
  if (uncategorised.length > 0) {
    sections.push({ kind: "uncategorised", label: "Uncategorised", projectRef: null, icon: "", topics: uncategorised });
  }
  for (var i = 0; i < projects.length; i++) {
    var project = projects[i] || {};
    var projectTopics = splitThreads(project.topics);
  ...
  var cross = splitThreads(value.crossProjectTopics);
  if (cross.length > 0) {
    sections.push({ kind: "cross_project", label: "Cross-project", projectRef: null, icon: "", topics: cross });
  }
  if (parkedThreads.length > 0) {
    sections.push({ kind: "parked", label: "Parked", projectRef: null, icon: "", topics: parkedThreads });
  }
  if (closedThreads.length > 0) {
    sections.push({ kind: "closed", label: "Closed", projectRef: null, icon: "", topics: closedThreads });
  }
  return sections;
}

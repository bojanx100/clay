...
export function topicRefKey(ref) {
  ...
  return String(ref.threadId || ref.topicId || ref.topicKey || ref.id || ref.key || "");
}
...
function coordinatorProjectTitle(value) {
  var title = safeText(value, "Project");
...
var ACTIVE_COORDINATOR_STATUSES = {
  waiting: true,
  queued: true, ready: true, running: true, reviewing: true,
};
var ATTENTION_COORDINATOR_STATUSES = {
  blocked: true, failed: true, needs_input: true, waiting_user: true,
};

function actionableControlStatus(value) {
  var status = safeText(value, "").toLowerCase().replace(/\s+/g, "_");
  return !!(ACTIVE_COORDINATOR_STATUSES[status] || ATTENTION_COORDINATOR_STATUSES[status]);
}

// A project coordinator is a durable container, not work by itself. Keep it
// in navigation only while one of its task, worker, or Thread descendants is
// active or needs the owner's attention. Terminal task and worker history
// stays in the owner Work tracker instead of lingering in the sidebar.
// Terminal Thread containers may still carry active children, so recurse
// instead of treating handed-off history as visible work on its own.
export function hasVisibleCoopProjectWork(node) {
  var value = node || {};
  var role = safeText(value.role, "");
  var status = safeText(value.status, "");
  if ((role === "thread" || role === "project_assignment" || role === "task_coordinator" || role === "worker") &&
      (ACTIVE_COORDINATOR_STATUSES[status] || ATTENTION_COORDINATOR_STATUSES[status])) {
    return true;
  }
  var children = Array.isArray(value.children) ? value.children : [];
  for (var i = 0; i < children.length; i++) {
    if (hasVisibleCoopProjectWork(children[i])) return true;
  }
  return false;
}
...
export function cloneControlResult(value) {
  var item = value && typeof value === "object" ? value : {};
  var role = safeText(item.role, "");
  if (role !== "council" && role !== "triage") return null;
  var summary = safeText(item.summary, "");
  if (!summary) return null;
  return {
    role: role,
    title: safeText(item.title, role === "council" ? "Council result" : "Triage result"),
    status: safeText(item.status, "completed"),
    summary: summary,
    verification: safeText(item.verification, ""),
    completedAt: typeof item.completedAt === "number" ? item.completedAt : null,
    topicRef: cloneReference(item.topicRef),
    projectRef: cloneReference(item.projectRef),
    projectTitle: safeText(item.projectTitle, ""),
    topicTitle: safeText(item.topicTitle, ""),
    question: safeText(item.question, ""),
    taskRef: cloneReference(item.taskRef),
    canonicalKey: safeText(item.canonicalKey, ""),
    executionRef: cloneReference(item.executionRef),
    containerSessionRef: cloneReference(item.containerSessionRef),
  };
}
...
export function cloneTopic(topic, fallbackProjectRef, fallbackGroup) {
  ...
  var threadRef = cloneReference(value.threadRef);
  if (!topicRef && threadRef && threadRef.threadId) topicRef = { topicId: threadRef.threadId };
  ...
  if (!threadRef) threadRef = { threadId: topicRefKey(topicRef) };
  ...
  var allowedThreadStates = { exploring: true, parked: true, handed_off: true, closed: true };
  var threadState = safeText(value.threadState, "");
  if (!allowedThreadStates[threadState]) threadState = status === "closed" ? "closed" : "exploring";
  return {
    topicRef: topicRef,
    threadRef: threadRef,
    threadState: threadState,
    closeOutcome: value.closeOutcome === "implemented_resolved" || value.closeOutcome === "not_pursuing"
      ? value.closeOutcome : null,
    hidden: value.hidden === true,
    lastTurnRef: cloneReference(value.lastTurnRef),
    projectRef: projectRef,
    group: safeText(groupName || value.category || fallbackGroup, fallbackGroup || "uncategorised").toLowerCase(),
    title: safeText(value.title || value.name || value.label, "Untitled Thread"),
    status: status,
    ...
    controlResults: safeTopicList(value.controlResults).map(cloneControlResult).filter(Boolean),
...
//
// Order: Threads, Project coordinators, Council, then Triage.
// A Thread leaves the list only after its typed handoff succeeds. Parked and
// needs-decision work remain in Threads; discarded and handed-off records do
// not grow a second archival category in the navigation.
export function coopTopicSections(model) {
  var value = model || {};
  var sections = [];
  var projects = Array.isArray(value.projects) ? value.projects : [];
  var sourceTopics = Array.isArray(value.allTopics) ? value.allTopics : [];
  var threads = sourceTopics.filter(function (topic) {
    var state = topic && topic.threadState || "exploring";
    return state !== "handed_off" && state !== "closed";
  });
  if (threads.length > 0) {
    sections.push({ kind: "threads", label: "Threads", projectRef: null, icon: "", topics: threads });
  }
  var coordinators = [];
  for (var i = 0; i < projects.length; i++) {
    var project = projects[i] || {};
    ...
    var visibleHierarchy = hierarchy.filter(function (node) {
      return !!(node && node.role === "project_coordinator" && node.sessionRef &&
        (node.transparency || hasVisibleCoopProjectWork(node)));
    }).slice(0, 1);
    if (visibleHierarchy.length === 0) continue;
    var projectTitle = coordinatorProjectTitle(project.title);
    var projectedHierarchy = visibleHierarchy.map(function (node, index) {
      if (index !== 0) return node;
      return Object.assign({}, node, { title: projectTitle + " coordinator" });
    });
    coordinators.push({
      label: projectTitle,
  ...
  if (coordinators.length > 0) {
    sections.push({
      kind: "project_coordinators",
      label: "Project coordinators",
      coordinators: coordinators,
    });
  }
  var controlSessions = Array.isArray(value.controlPlaneSessions) ? value.controlPlaneSessions : [];
  var roles = [{ role: "council", label: "Council" }, { role: "triage", label: "Triage" }];
  for (var ri = 0; ri < roles.length; ri++) {
    var sessions = controlSessions.filter(function (session) {
      return session && session.role === roles[ri].role && session.sessionRef &&
        session.sessionRef.projectId && session.sessionRef.sessionStorageId &&
        actionableControlStatus(session.status);
    }).map(function (session) {
      return {
        title: safeText(session.title, roles[ri].label),
        role: roles[ri].role,
        sessionRef: cloneReference(session.sessionRef),
        status: safeText(session.status, "idle"),
        activity: safeText(session.activity, ""),
        processing: session.processing === true && session.status === "running",
        topicRef: cloneReference(session.topicRef),
        projectRef: cloneReference(session.projectRef),
        projectTitle: safeText(session.projectTitle, ""),
        topicTitle: safeText(session.topicTitle, ""),
        question: safeText(session.question, ""),
        taskRef: cloneReference(session.taskRef),
        containerSessionRef: cloneReference(session.containerSessionRef),
        canonicalKey: safeText(session.canonicalKey, ""),
      };
    });
    // Terminal Council/Triage evidence is deliberately kept in the durable
    // ledger and controlPlaneResults projection, but it is not current work.
    // Rendering it here made every completed, dismissed, cancelled, and retry
    // result accumulate in the sidebar beside the actionable control session.
    var results = [];
    if (sessions.length === 0 && results.length === 0) continue;
    sections.push({
      kind: roles[ri].role,
      label: roles[ri].label,
      sessions: sessions,
      results: results,
    });
  }

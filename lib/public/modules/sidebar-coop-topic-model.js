...
// collapsed Done section. Topics are only the intake/discussion layer: once a
...
// neither topics nor hierarchy yields no descriptor at all.
export function coopTopicSections(model) {
  ...
  function splitDone(topics) {
    ...
    for (var i = 0; i < list.length; i++) {
      if (topicIsAdmitted(list[i])) continue;

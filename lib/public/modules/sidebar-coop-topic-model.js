...
export function cloneTopic(topic, fallbackProjectRef, fallbackGroup) {
  ...
  return {
    ...
    status: status,
    // Derived from canonical linked work; "" when the topic has none, which is
    // the honest answer rather than a label true of every open topic.
...
//
// Order: Uncategorised, then projects, then Cross-project. A category with no
// topics yields no descriptor at all, so the renderer cannot emit an empty
// wrapper or a heading for something the owner cannot open.
export function coopTopicSections(model) {
  var value = model || {};
  var sections = [];
  var uncategorised = Array.isArray(value.uncategorisedTopics) ? value.uncategorisedTopics : [];
  ...
  for (var i = 0; i < projects.length; i++) {
    var project = projects[i] || {};
    var projectTopics = Array.isArray(project.topics) ? project.topics : [];
    if (projectTopics.length === 0) continue;
    sections.push({
      kind: "project",
  ...
  var cross = Array.isArray(value.crossProjectTopics) ? value.crossProjectTopics : [];
  if (cross.length > 0) {

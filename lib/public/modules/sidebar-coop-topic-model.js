...
//
// Order: Threads, project-named coordinators, Council, then Triage.
...
export function coopTopicSections(model) {
  ...
  for (var ci = 0; ci < controlSessions.length; ci++) {
    sections.push({
      kind: "control_plane",
      label: safeText(controlSessions[ci].title, "Control plane"),
      role: controlSessions[ci].role,
      sessionRef: cloneReference(controlSessions[ci].sessionRef),
      topics: [],
    });
  }
  return sections;

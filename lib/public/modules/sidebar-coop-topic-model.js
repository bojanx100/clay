...
function cloneRelatedExecution(value) {
  var list = Array.isArray(value) ? value : (value ? [value] : []);
  return list.map(function (item) {
    var execution = item || {};
    var nested = execution.relatedExecution || execution.children || execution.executions;
    return {
      sessionRef: cloneReference(execution.sessionRef || execution.canonicalSessionRef),
      taskRef: cloneReference(execution.taskRef),
      title: safeText(execution.title || execution.label || execution.name, "Related execution"),
      role: safeText(execution.role, "worker"),
      status: safeText(execution.status, ""),
      activity: safeText(execution.activity || execution.currentActivity, ""),
      updatedAt: typeof execution.updatedAt === "number" ? execution.updatedAt : null,
      children: cloneRelatedExecution(nested),
    };
  });
}
...
export function cloneTopic(topic, fallbackProjectRef, fallbackGroup) {
  ...
  return {
    ...
    status: status,
    active: value.active === undefined ? (status === "active" || status === "running") : !!value.active,
    ...
    decisions: decisions,
    relatedExecution: cloneRelatedExecution(value.relatedExecution || value.relatedExecutions || value.execution || value.executions),

...
function linkTaskCoordinator(sm, root, child, input) {
  ...
  if (!task) {
    task = taskGraph.createTask(root, {
      ...
      controlRole: input.brief.controlRole || request.controlRole,
      reviewOnly: input.brief.reviewOnly === true || request.reviewOnly === true,
...
function bindControlPlaneTaskCoordinator(sm, child, input) {
  var request = input && input.request;
  var rootRef = projectIdentity.normalizeSessionRef(input && input.projectCoordinatorRef);
  var taskId = input && input.taskId;
  if (!child || !request || !rootRef || rootRef.projectId !== projectIdentity.LEAD_PROJECT_ID ||
      !projectIdentity.isTaskId(taskId)) return false;
  child.coordinationMode = true;
  child.coordinationRole = TASK_COORDINATOR;
  child.projectCoordinatorRef = rootRef;
  child.projectCoordinatorProjectRef = request.targetProject;
  child.controlPlaneParent = {
    taskId: taskId,
...
module.exports = {
  ...
  bindControlPlaneTaskCoordinator: bindControlPlaneTaskCoordinator,

...
var HISTORICAL_ACTIVE_STATUSES = { pending: true, active: true, unavailable: true, queued: true,
  ready: true, running: true, reviewing: true };
var HISTORICAL_TERMINAL_STATUSES = { completed: true, dismissed: true, cancelled: true, deleted: true,
  failed: true, superseded: true, unrouted: true };
var HISTORICAL_RESOLUTION_ACTIONS = { execution_completed: true, execution_failed: true,
  execution_superseded: true, execution_deleted: true, task_completed: true,
  task_dismissed: true, session_missing: true };
function historicalBinding(record) {
...
function historicalKey(record) {
  var binding = historicalBinding(record);
...
function historicalResolution(record) {
  ...
  return !!((record && record.terminalOutcome && record.terminalOutcome.status) ||
    HISTORICAL_RESOLUTION_ACTIONS[action] || HISTORICAL_TERMINAL_STATUSES[bindingStatus]);
}
function historicalApprovalBlocker(record) {
  var binding = historicalBinding(record) || {};
...
// Classify the persisted Coop session ledger without mutating live state.
function classifyHistoricalLedger(entries) {
  ...
  for (var j = 0; j < list.length; j++) {
    ...
    var key = historicalKey(record); var duplicate = keyCounts[key] > 1;
    var projectId = record.projectRef && record.projectRef.projectId; var classification = "idle";
    var terminal = false;
    if (projectId === projectIdentity.LEAD_PROJECT_ID && !binding.portfolioTaskId) {
      ...
      terminal = true;
    } else if (lifecycle === "missing" && record.sessionPresent === false &&
        action === "session_missing") {
      classification = "failed";
      terminal = true;
    } else if (HISTORICAL_TERMINAL_STATUSES[lifecycle] || HISTORICAL_TERMINAL_STATUSES[status]) {
      classification = "terminal";
      terminal = true;
    } else if (lifecycle === "needs_input" || work === "needs_input" || status === "needs_input") {
      classification = historicalApprovalBlocker(record) ? "approval_gated" : "needs_input";
    } else if (HISTORICAL_ACTIVE_STATUSES[lifecycle] || HISTORICAL_ACTIVE_STATUSES[work] ||
        HISTORICAL_ACTIVE_STATUSES[status]) {
      classification = "active";
    }
    var reconciled = terminal && historicalResolution(record);
    var row = {
      key: key,
      ...
      sessionStorageId: record.sessionStorageId || null,
      parentSessionRef: record.parentSessionRef || null, parentTaskId: record.parentTaskId || null,
      title: record.title || "",
      portfolioTaskId: binding.portfolioTaskId || null,

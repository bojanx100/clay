...
// Types used by the loop: staffed | implementation_verified | completed |
//   blocked | failed |
//   project_completed | project_completion_revoked | worker_completed |
//   portfolio_completed | standup_composed | lead_tick_wake | lead_note
//
...
var classifyHistoricalLedger = require("./lead-history-reconciliation").classifyHistoricalLedger;
...
function prepareEvent(ev, opts) {
  ...
  if (ev.type === "completed" && ev.item &&
      String(ev.item.project || "").toLowerCase() === "webapp") {
    var acceptance = ev.ownerAcceptance;
    if (!acceptance || acceptance.status !== "accepted" || acceptance.withdrawnAt != null) {
      var implementation = {};
      for (var eventKey in ev) implementation[eventKey] = ev[eventKey];
      implementation.type = "implementation_verified";
      implementation.acceptanceStatus = "pending";
      implementation.guardReason = "explicit_owner_acceptance_required";
...
// The next seq comes from the HIGHEST seq in the ledger, never from the last
// row's. Deriving it from the last row makes the sequence collapse the moment
// anything appends a row without one:
//
//   `undefined + 1` is NaN, JSON.stringify writes NaN as `null`, and the append
//   after that computes `null + 1` === 1 -- so numbering silently restarts from
//   1 over rows that already own those numbers.
//
// Observed live 2026-09-04: ~/.clay/lead/ledger.jsonl carried max seq 1440
// while its tail read 1,2,3,null,null,1,2, because the lead_finding rows were
// written by direct fs.appendFileSync instead of through this module. Anything
// ordering or de-duplicating the ledger by seq was reading nonsense, and the
// dismissal appended that day was itself stamped seq 1.
//
// Scanning is free: readEvents() already materialises every row for this call.
// This is deliberately tolerant of foreign writers rather than trying to stop
// them -- the ledger is an append-only file that several agents write to, so
// the numbering has to be self-healing.
function highestSeq(events) {
  var max = 0;
  var list = Array.isArray(events) ? events : [];
  for (var i = 0; i < list.length; i++) {
    var seq = Number(list[i] && list[i].seq);
    if (Number.isFinite(seq) && seq > max) max = seq;
  }
  return max;
}

function recordFor(input, highest, opts) {
  var record = {};
  for (var key in input) record[key] = input[key];
  var base = Number(highest);
  record.seq = (Number.isFinite(base) && base > 0 ? base : 0) + 1;
  // For an append-only ledger the write time IS the timestamp, so a caller that
  // supplies no `at` must not land one of 0. Zero is not "very early", it is no
  // ordering evidence at all -- and consumers that snapshot "what was already
  // pending when the owner spoke" read 0 as earlier than every approval ever
  // made. appendAttention supplies no `at`, so every attention event written
  // without an injected clock was stamped 0; live state carries one (seq 541).
  var supplied = (opts && opts.now !== undefined) ? opts.now : record.at;
  record.at = typeof supplied === "number" && Number.isFinite(supplied) && supplied > 0
    ? supplied : Date.now();
...
function appendEvent(ev, opts) {
  ...
  // seq = highest seq + 1, so a foreign row written without a seq cannot
  // restart the numbering. See highestSeq() for what that cost on 2026-09-04.
  var events = readEvents({ dir: dir });
  var record = recordFor(input, highestSeq(events), opts);
...
// Items currently consuming execution capacity: staffed with no execution
// terminal event after it. `implementation_verified` retains the owner's
// acceptance gate and all verification evidence, but execution has ended and
// must release its slot immediately.
...
function inFlight(opts) {
  ...
  for (var i = 0; i < events.length; i++) {
    ...
    else if (ev.type === "implementation_verified") delete open[ev.item.id];
...
function bindingBlockReason(binding) {
  ...
  if (binding.attentionAt) return "cutover_attention";
...
function attentionKey(event) {
  var explicit = String(completionField(event, "attentionKey") || "").trim();
  if (explicit) return explicit.slice(0, 240);
  var portfolioTaskId = String(completionField(event, "portfolioTaskId") || "").trim();
  var revision = Number(completionField(event, "bindingRevision"));
  if (portfolioTaskId) return portfolioTaskId + ":" + (revision || 0);
  var itemId = String(completionField(event, "itemId") || "").trim();
  return itemId ? "item:" + itemId : "";
}

function unresolvedAttention(events) {
  var open = {};
  var list = Array.isArray(events) ? events : [];
  for (var i = 0; i < list.length; i++) {
    var key = attentionKey(list[i]);
    if (!key) continue;
    if (list[i].type === "staffing_attention" || list[i].type === "cutover_attention") {
      open[key] = list[i];
    } else if (list[i].type === "attention_resolved") delete open[key];
  }
  return Object.keys(open).map(function (key) { return open[key]; });
}

function appendAttention(input, opts) {
  var value = input || {};
  var key = attentionKey(value);
  var reason = String(value.reason || "").trim();
  if (!key || !reason) return null;
  return appendEvent({
    type: value.type === "staffing_attention" ? "staffing_attention" : "cutover_attention",
    attentionKey: key,
    itemId: value.itemId || null,
    portfolioTaskId: value.portfolioTaskId || null,
    bindingRevision: value.bindingRevision || null,
    reason: reason.slice(0, 240),
    fallbackAllowed: false,
  }, opts);
}

function resolveAttention(input, opts) {
  var key = attentionKey(input || {});
  if (!key) return null;
  return appendEvent({ type: "attention_resolved", attentionKey: key }, opts);
}

function attentionMatchesBindings(event, bindings, portfolioTaskId) {
  var eventPortfolioTaskId = String(completionField(event, "portfolioTaskId") || "");
  if (eventPortfolioTaskId && eventPortfolioTaskId === String(portfolioTaskId || "")) return true;
  for (var i = 0; i < bindings.length; i++) {
    if (eventPortfolioTaskId === bindings[i].portfolioTaskId) return true;
  }
  return false;
}

function hasRelevantAttention(value, bindings) {
  var open = unresolvedAttention(value.events);
  for (var i = 0; i < open.length; i++) {
    if (attentionMatchesBindings(open[i], bindings, value.portfolioTaskId)) return true;
  }
  return Array.isArray(value.attentionItems) && value.attentionItems.length > 0;
}
...
function portfolioPreflight(value, bindings) {
  ...
  if (hasRelevantAttention(value, bindings)) return "cutover_attention";
...
module.exports = {
  ...
  inFlight: inFlight,
  classifyHistoricalLedger: classifyHistoricalLedger,
  failureCount: failureCount,
  appendAttention: appendAttention,
  resolveAttention: resolveAttention,
  unresolvedAttention: unresolvedAttention,

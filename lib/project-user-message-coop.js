...
function withThreadIntentContext(fullText, intent, result, threadRef) {
  if (!intent) return fullText;
  var payload = {
    kind: intent.kind,
...
function attachCoopForegroundIngress(ctx) {
  ...
  function applyText(metadata, msg, fullText) {
    ...
    if (msg.coopRouteAttention === "project_target_unavailable") {
      fullText = [
        "<coop_routing_attention>",
        "The named project could not be resolved uniquely from the accessible project inventory.",
        "Clarify the intended project with the owner. Continue discussing the request and its constraints; this turn grants no project implementation authority.",
        "</coop_routing_attention>",
        "", fullText,
      ].join("\n");
    }
    if (msg.coopThreadIntent) {
      fullText = withThreadIntentContext(fullText, msg.coopThreadIntent,
        msg.coopThreadControlResult,
        msg.coopThreadIntent.kind === "ambiguous" ? null : msg.coopThreadRef);
    }
  ...
  function recordOwnerRequest(session, metadata, msg, eventIndex) {
    ...
    try {
      ...
      return !!ledger.classify(ingress.ingressId, {
  ...
  function recordPrepared(session, metadata, msg, fullText) {
    if (!metadata.coopIngress || !Array.isArray(session.history)) return true;
    for (var i = session.history.length - 1; i >= 0; i--) {
      ...
      if (msg.coopRouteAttention) item.coopRouteAttention = msg.coopRouteAttention;
      item.coopTopicAnchor = msg.coopTopicAnchor || null;
      item.coopThreadIntent = msg.coopThreadIntent || null;
      item.coopThreadControlResult = msg.coopThreadControlResult || null;
      if (typeof msg.coopComposerScope === "string") item.coopComposerScope = msg.coopComposerScope;
      // This item was appended before routing finished, so its enriched prompt
      // and classification exist only in memory until a full rewrite occurs.
      // Mark the in-place mutation explicitly and require that rewrite before
      // the provider sees a turn that restart recovery could not reconstruct.
      session._historyNeedsRewrite = true;
      try {
        if (ctx.sm.saveSessionFile(session, { durable: true }) !== true) return false;
      } catch (error) { return false; }
      return recordOwnerRequest(session, metadata, msg, i);
    }
    return false;
  }
  ...
  function dispatch(prepared) {
    ...
    ctx.queue.dispatchPreparedToSdk(prepared.session, {
      ...
      coopThreadRef: prepared.msg.coopThreadRef || null,
      coopTopicRef: prepared.msg.coopTopicRef || null,
      coopProjectRef: prepared.msg.coopProjectRef || null,
    });
    return true;
  }

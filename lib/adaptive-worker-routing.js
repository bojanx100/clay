...
function buildCandidates(sm, parentSession, task, classification, policy, diagnostics) {
  ...
  for (var i = 0; i < routes.length; i++) {
    ...
    if (task.modelPinned) {
      ...
      diagnostics.modelPinAdvertised = true;
      entries = [pinnedEntry];
    }
    for (var j = 0; j < entries.length; j++) {
      ...
      if (health.state === "unhealthy") {
        if (task.modelPinned) {
          diagnostics.modelPinUnhealthy = true;
          if (!diagnostics.selectedToken) diagnostics.selectedToken = model;
        }
        continue;
      }
...
function unavailableDecision(sm, parentSession, task, classification, diagnostics) {
  ...
  if (task.modelPinned && !diagnostics.modelPinAdvertised) {
  ...
  } else if (task.modelPinned && diagnostics.modelPinUnhealthy) {
    detail = "Pinned model " + task.model + " resolves to advertised selectable token " +

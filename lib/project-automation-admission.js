...
function createCandidateAdmission(options) {
  ...
  // rather than trusted.
  var getBinding = opts.getBinding ||
    (crossProject && typeof crossProject.getBinding === "function" ? crossProject.getBinding : null);
  ...
  function admitOne(candidate) {
    ...
    if (candidate.admission !== "auto") {
      candidates.recordAttention(projectRef, candidate.candidateKey,
    ...
    var result = crossProject.createProjectExecution({
      source: coopSource,
  ...
  // admitPending -> { ok, admitted, deferred, failed, attention }
  ...
  function admitPending() {
    ...
    var queue;
    try {
      queue = candidates.pending({ status: "pending" });
    } catch (e) {
    ...
    for (var i = 0; i < pending.length; i++) {
      ...
      if (outcome.state === "admitted") admitted++;

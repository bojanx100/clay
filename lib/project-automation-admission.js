...
function createCandidateAdmission(options) {
  ...
  function admitOne(candidate, options) {
    ...
    if (candidate.admission === "owner_approval" && !ownerApproved && !controlSupersedesScopedPolicy &&
    ...
    if (candidate.admission === "owner_approval" && !ownerApproved && !scopedApproved &&
    ...
    } else if (candidate.admission !== "auto" && !ownerApproved && !scopedApproved && !autoApproved) {
      return fail(projectRef, candidate, "binding_history_unavailable");
    }
    if (candidate.admission !== "auto" && !ownerApproved && !scopedApproved && !autoApproved) {
      var approvalScope = {
    ...
    note({
      ...
      authorization: autoApproved ? "project_auto_approval" :

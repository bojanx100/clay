...
function createCandidateAdmission(options) {
  ...
  function currentQualification(candidate, projectRef, options) {
  ...
  function admitOne(candidate, options) {
    ...
    var qualified = currentQualification(candidate, projectRef, options);
    ...
    if (getBindings) {
      ...
      var selection = selectBindingRevision(portfolioTaskId, bindingHistory,
        primitiveLaunch ? { candidate: candidate, source: coopSessionRef() } : null);
      if (!selection.ok) {
        releaseAutoApproval();

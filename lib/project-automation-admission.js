...
// Immutable histories are never reused; later automation admissions get a new revision.
...
function selectBindingRevision(portfolioTaskId, bindings) {
  ...
  for (var i = 0; i < bindings.length; i++) {
    ...
    if (!binding || binding.portfolioTaskId !== portfolioTaskId || !Number.isSafeInteger(binding.bindingRevision) || binding.bindingRevision < 1) continue;
    if (!latest || binding.bindingRevision > latest.bindingRevision) latest = binding;
  }
  if (!latest) return { ok: true, bindingRevision: 1 };
  if (!IMMUTABLE_BINDING_STATUSES[latest.status]) return { ok: true, bindingRevision: latest.bindingRevision };
  if (latest.bindingRevision >= Number.MAX_SAFE_INTEGER) return { ok: false, reason: "binding_revision_exhausted" };
...
function createCandidateAdmission(options) {
  ...
  // Only the complete-history reader can distinguish terminal revision 1 from a safe retry.
  ...
  function admitOne(candidate, options) {
    ...
    // A scoped policy may replace one repeated owner prompt only for a current,
    // low-risk candidate. The policy returns an exact durable grant reference;
    // it never turns the original owner-gated candidate into an autonomous one.
    var scopedDecision = null;
    if (candidate.admission === "owner_approval" && !ownerApproved &&
    ...
    if (candidate.admission !== "auto" && !ownerApproved && !scopedApproved) {

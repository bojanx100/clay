...
function resolveTailscaleIp(networkInterfaces) {
  var ifaces = networkInterfaces || os.networkInterfaces();
  var ifaceNames = Object.keys(ifaces);

  for (var i = 0; i < ifaceNames.length; i++) {
    var addrs = ifaces[ifaceNames[i]] || [];
    for (var j = 0; j < addrs.length; j++) {
      var addr = addrs[j];
      if (!addr || addr.internal || addr.family !== "IPv4") continue;
      var parts = String(addr.address || "").split(".");
      if (parts.length !== 4 || parts[0] !== "100") continue;
...
function tailscaleUrlForPort(port, networkInterfaces) {
  var ip = resolveTailscaleIp(networkInterfaces);
  return port && ip ? "http://" + ip + ":" + port : null;
}
...
module.exports = {
  ...
  resolveTailscaleIp: resolveTailscaleIp,

...
function attachProjectLiveUiPairing(ctx) {
  function targetTab(msg) {
    ...
    var tabs = Array.isArray(msg.tabs) ? msg.tabs : [];
    for (var i = 0; i < tabs.length; i++) {
      if (Number(tabs[i].id) === Number(msg.targetTabId) && tabs[i].url) {
        return tabs[i];
      }
    }
    var error = new Error(
  ...
  function sendPairCommand(extension, args) {
    if (ctx.browserState._extensionWs === extension.ws) {
      return ctx.sendExtensionCommandAny("live_ui_pair", args, 5000);
    }
    ctx.sendTo(extension.ws, {
      type: "extension_command",
      command: "live_ui_pair",
      args: args,
      requestId: null,
    });
  ...
  function finishPair(ws, msg, session, tab, target, targetError) {
    try {
      ...
      var extension = ctx.extensionIdentity(null, ws, msg.extensionId);

...
function projectIdForRuntime(extra, cwd, slug) {
...
function createReferenceResolver(ctx) {
  ...
  function resolveProjectRef(ref, actor) {
    ...
    var project = getProjectById(normalized.projectId);
    if (!project) return { ok: false, code: "project_not_found" };
    if (!canAccessProject(actor, project)) return { ok: false, code: "access_denied" };
    return { ok: true, ref: normalized, project: project };
  }
  ...
  function resolveSessionRef(ref, actor) {
    ...
    var projectResult = resolveProjectRef({ projectId: normalized.projectId }, actor);
    if (!projectResult.ok) return projectResult;
    var manager = getSessionManager(projectResult.project);
    var session = manager && typeof manager.resolveSessionRef === "function"
      ? manager.resolveSessionRef(normalized) : null;
    if (!session) return { ok: false, code: "session_not_found" };
    if (!canAccessSession(actor, projectResult.project, session)) {
      return { ok: false, code: "access_denied" };
    }
    return { ok: true, ref: normalized, project: projectResult.project, session: session };
  }

...
function attachProjectLiveUi(ctx) {
  ...
  function routedExtensionIdentity(pairing) {
    if (!pairing) return null;
    if (typeof registry.extensionConnection !== "function") return null;
    var routedWs = registry.extensionConnection(
      pairing.extensionInstanceId, pairing.userId);
    if (!routedWs) return null;
    return {
      ws: routedWs,
      ownerKey: clientIdFor(routedWs),
      instanceId: pairing.extensionInstanceId,
    };
  }

  function extensionIdentity(pairing, candidateWs) {
    var local = localExtensionIdentity(pairing);
    var routed = routedExtensionIdentity(pairing);
    if (candidateWs) {
      if (routed) {

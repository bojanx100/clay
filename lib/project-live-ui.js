...
var attachLiveUiChat =
  require("./project-live-ui-chat").attachLiveUiChat;
...
function attachProjectLiveUi(ctx) {
  ...
  var chat = attachLiveUiChat({
    registry: registry,
    sm: ctx.sm,
    userMessage: ctx.userMessage,
    assertExtensionSender: assertExtensionSender,
    pairControlWs: pairControlWs,
  ...
  function pair(ws, msg) {
    ...
    if (!session || session.hidden || !sessionMatches(session, msg.sessionId)) {
      var sessionError = new Error("The active session cannot be paired");
    ...
    workspace.getLiveUiTarget(session, function (target) {
      try {
        ...
        var allowedOrigin = exactLoopbackOrigin(target.localUrl);
        var tabOrigin = exactLoopbackOrigin(tab.url);
        if (!allowedOrigin || tabOrigin !== allowedOrigin) {
          var originError = new Error("The target tab must use the session's Clay-managed loopback origin");
        ...
        chat.registerPair(created.pairing.pairingId, session.localId);
  ...
  function reconnectTarget(ws, pairing) {
    ...
    var tabOrigin = tab && tab.url ? exactLoopbackOrigin(tab.url) : null;
  ...
  function closeTarget(ws, msg, pairing) {
    ...
    chat.clearPair(pairing.pairingId);
    var controlWs = pairControlWs(pairing);
    if (controlWs) {
  ...
  function relay(ws, msg) {
    ...
    if (msg.event === "chat.message") {
      return chat.handleMessage(ws, msg, pairing, selections.get(pairing.pairingId));
    }

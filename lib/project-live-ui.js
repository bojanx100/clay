...
function attachProjectLiveUi(ctx) {
  ...
  var reports = attachLiveUiReports({
    ...
    followUpTask: ctx.taskOrchestrator.messageFromOwner,
    persistSession: ctx.sm.saveSessionFile,
  ...
  function rememberExtension(ws, instanceId) {
    if (typeof registry.rememberExtensionConnection === "function") {
      registry.rememberExtensionConnection({
  ...
  function localExtensionIdentity(pairing) {
    var extensionWs = browserState._extensionWs;
    if (!extensionWs) return null;
    if (extensionWs.readyState !== 1) return null;
    var instanceId = browserState._extensionId;
    if (!instanceId) instanceId = clientIdFor(extensionWs);
    if (pairing) {
      if (pairing.extensionInstanceId !== instanceId) return null;
      if (pairing.userId !== userIdFor(extensionWs)) return null;
    }
    return {
      ws: extensionWs,
  ...
  function routedExtensionIdentity(pairing, instanceId, userId) {
    if (typeof registry.extensionConnection !== "function") return null;
    var routedId = pairing ? pairing.extensionInstanceId : instanceId;
    var routedUserId = pairing ? pairing.userId : userId;
    if (!routedId) return null;
    var routedWs = registry.extensionConnection(
      routedId, routedUserId);
    if (!routedWs) return null;
    return {
      ws: routedWs,
      ownerKey: clientIdFor(routedWs),
      instanceId: routedId,
    };
  }

  function extensionIdentity(pairing, candidateWs, instanceId) {
    var local = localExtensionIdentity(pairing);
    var routed = routedExtensionIdentity(
      pairing, instanceId, candidateWs ? userIdFor(candidateWs) : null);
    if (candidateWs) {
      if (routed) {
        if (routed.ws === candidateWs) return routed;
      }
      if (local) {
        if (local.ws === candidateWs) {
          rememberExtension(local.ws, local.instanceId);
          return local;
        }
      }
    }
  ...
  function assertExtensionSender(ws, pairing) {
    var extension = extensionIdentity(pairing, ws);
    if (ws !== extension.ws ||
  ...
  function reconnectTarget(ws, msg, pairing) {
    var extension = assertExtensionSender(ws, pairing);
    var tab = browserState._browserTabList[pairing.targetTabId];
    var targetUrl = msg.payload && msg.payload.targetUrl || tab && tab.url;
    var tabOrigin = targetUrl ? liveUiTarget.exactHttpOrigin(targetUrl) : null;
  ...
  function sendTarget(pairing, event, payload) {
    var extension = extensionIdentity(pairing);
    sendTo(extension.ws, {
      type: "live_ui_relay",
      protocolVersion: PROTOCOL_VERSION,
      pairingId: pairing.pairingId,
  ...
  function routeToPairingProject(ws, msg, pairing) {
    if (pairing.projectSlug === slug) return false;
    var extension = extensionIdentity(pairing, ws);
    if (ws !== extension.ws) {
      var extensionError = new Error(
        "Live UI cross-project relays must come from the paired extension");
      extensionError.code = "LIVE_UI_EXTENSION_MISMATCH";
      throw extensionError;
    }
    if (typeof registry.routeProjectMessage === "function" &&
        registry.routeProjectMessage(pairing.projectSlug, ws, msg)) return true;
  ...
  function relay(ws, msg) {
    ...
    if (routeToPairingProject(ws, msg, pairing)) return;
    ...
    if (msg.event === "target.reconnect") return reconnectTarget(ws, msg, pairing);
  ...
  function dispatchLiveUiMessage(ws, msg) {
    var initial = msg.type === "live_ui_request_pair" ||
      msg.type === "live_ui_create_bound_session";
    if (initial && msg.projectSlug && msg.projectSlug !== slug) {
      var extension = extensionIdentity(null, ws);
      var routed = Object.assign({}, msg, { extensionId: extension.instanceId });
      if (registry.routeProjectMessage(msg.projectSlug, ws, routed)) return true;
      var routeError = new Error("The selected Live UI project is unavailable");
      routeError.code = "LIVE_UI_PROJECT_UNAVAILABLE";
      throw routeError;
    }
  ...
  function handleDisconnect(ws) {
    if (!ws) return;
    if (typeof registry.forgetExtensionConnection === "function") {
      registry.forgetExtensionConnection(ws);
    }
    if (!ws._clayLiveUiClientId) return;
  ...
  var unregisterProjectHandler =
    typeof registry.registerProjectHandler === "function" ?
      registry.registerProjectHandler(slug, handleLiveUiMessage) : null;

  function dispose() {
    if (unregisterProjectHandler) {
      unregisterProjectHandler();
      unregisterProjectHandler = null;
    }
  }

...
function attachProjectLiveUi(ctx) {
  ...
  function pair(ws, msg) {
    ...
    function finishPair(target, targetError) {
      try {
    ...
    if (msg.reconnectServer === true) {
      if (!workspace.reconnectLiveUiTarget) {
        var reconnectUnavailable = new Error("This Clay version cannot reconnect the selected chat to a development server");
        reconnectUnavailable.code = "LIVE_UI_RECONNECT_UNAVAILABLE";
        finishPair(null, reconnectUnavailable);
        return;
      }
      workspace.reconnectLiveUiTarget(session, tab.url, function (result) {
        if (result && result.ok) {
          finishPair(result.target, null);
          return;
        }
        var reconnectError = new Error(result && result.error ?
          result.error : "Clay could not reconnect this chat to the development server");
        reconnectError.code = result && result.code ?
          result.code : "LIVE_UI_RECONNECT_FAILED";
        finishPair(null, reconnectError);
      });
      return;
    }
    workspace.getLiveUiTarget(session, function (target) {
      finishPair(target, null);
    });
  }

...
function ownerTexts(topic, history) {
  ...
  var refs = topicAnchors.provenTurnRefs(topic, sessionHistory);
  var texts = [];
  for (var i = 0; i < refs.length; i++) {
    var verdict = topicAnchors.proveAnchor(sessionHistory, refs[i]);
    if (!verdict.proven) continue;
    var sessionId = refs[i] && refs[i].sessionStorageId || "";
    var segment = Array.isArray(sessionHistory)
      ? { length: sessionHistory.length }
      : lineage.segmentFor(sessionHistory, sessionId);
    var index = segment ? topicAnchors.candidateIndex(refs[i], verdict.offset, segment.length) : null;
    var record = index === null ? null : (lineage.recordAt(sessionHistory, sessionId, index) || {}).record;

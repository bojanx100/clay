...
function selectClosureCandidates(index, options) {
  ...
  for (var i = 0; i < ids.length; i++) {
    ...
    if (ownerBlocked(topic)) continue;
    if (hasLiveTask(ids[i], opts.tasks)) continue;
    if (hasLiveBinding(ids[i], opts.bindings)) continue;
    if (hasPresentSession(ids[i], opts.sessionEvidence)) continue;
    // The owner is still owed an answer on this topic. Nothing outranks that:
    // closing it buries the very question the ledger exists to keep visible.
    if (opts.outstandingTopicIds && opts.outstandingTopicIds[ids[i]]) continue;

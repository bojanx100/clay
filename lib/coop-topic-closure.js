...
function hasMatchingPresentSession(topic, options, names) {
  var opts = options || {};
  if (typeof opts.hasMatchingSession === "function") {
    return opts.hasMatchingSession(topic);
  }
  return hasMatchingSession(topic, names || sessionNames(opts.sessions));
}
...
function dismissedSession(sessionRef, evidence) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    // Typed comparison. Matching on storage id alone let a hidden session in
    // ANOTHER project mark a live reference dead and expose its topic.
    if (!ref || ref.sessionStorageId !== wanted.sessionStorageId ||
        ref.projectId !== wanted.projectId) continue;
...
function blockedBy(topicId, topic, options) {
  var opts = options || {};
  if (hasMatchingPresentSession(topic, opts)) return "matching_session";
...
//
// The status === "open" requirement here (and in selectClosureCandidates and
// applyClosureProposal) is DELIBERATELY not relaxed. While closes wrote status
// alone it made its own victims unrepairable -- a half-closed record was stuck
// in the rail and ineligible for the sweep that would have fixed it -- but that
// was the status-only write's fault, not this guard's. With the write fixed the
// damage is no longer produced, and the records damaged before the fix are
// repaired by scripts/heal-closed-thread-states.js. Relaxing the guard instead
// would let the sweep act on records the owner did not rule on as open, which is
// the one thing this two-touch flow exists to prevent.
...
function selectClosureCandidates(index, options) {
  ...
  for (var i = 0; i < ids.length; i++) {
    ...
    if (hasMatchingPresentSession(topic, opts, names)) continue;
...
function applyClosureProposal(index, decision, options) {
  ...
  if (proposal.resolvedAt) {
    return { ok: true, duplicate: true, closed: 0, confirmed: proposal.confirmed === true,
      closedTopicIds: Array.isArray(proposal.closedTopicIds) ? proposal.closedTopicIds.slice() : [] };
  }
  ...
  var blocked = [];
  var topics = (index && index.topics) || {};
  // The sweep closes rows that track nothing, so it has no per-row outcome to
  // record. It defaults to the same outcome the single-topic owner close uses
  // (coop-topic-index.close), which keeps a swept row visible under Done rather
  // than hiding it: not_pursuing sets hidden:true and would make the owner's
  // confirmed sweep look like the rows were deleted. Overridable per call.
  var closeOutcome = opts.closeOutcome || threadLifecycle.CLOSE_OUTCOMES.IMPLEMENTED_RESOLVED;
  for (var i = 0; i < proposal.candidates.length; i++) {
    ...
    // Retire the THREAD, not just the status field. Assigning topic.status
    // directly left threadState at its live value, and the owner's Threads rail
    // filters on threadState alone -- so a swept topic stayed a Threads row
    // forever, AND could never be swept again, because selection here requires
    // status === "open". The lifecycle layer owns the whole tuple.
    var applied = threadLifecycle.applyRecordStatus(topic, "closed", {
      closeOutcome: closeOutcome, now: now,
    });
    if (!applied.ok) {
      blocked.push({ topicId: topicId, title: topic.title, reason: applied.code });
      continue;
    }
    topic.updatedAt = now();
    topic.closureAudit = { proposalId: proposal.proposalId, reason: CLOSURE_REASON, closedAt: now() };
    closed += 1;
    closedTopicIds.push(topicId);
  }
  proposal.resolvedAt = now();
  proposal.confirmed = true;
  proposal.closed = closed;
  proposal.closedTopicIds = closedTopicIds.slice();
  return { ok: true, closed: closed, skipped: skipped, blocked: blocked,
    closedTopicIds: closedTopicIds, confirmed: true };
}

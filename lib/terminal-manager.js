...
function createTerminalManager(opts) {
  ...
  function create(cols, rows, osUserInfo, ownerWs, opts) {
    ...
    // opts.cwd lets a caller spawn in a subdirectory of the project (e.g. the
    // dev server in a git worktree) instead of the project root.
    var spawnCwd = (opts && opts.cwd) || cwd;
    var pty = createTerminal(spawnCwd, cols, rows, osUserInfo, opts);
    if (!pty) return null;
    ...
    pty.onExit(function (e) {
      ...
      var msg = JSON.stringify({ type: "term_exited", id: id, exitCode: session.exitCode });
      for (var ws of session.subscribers) {
        if (ws.readyState === 1) ws.send(msg);
      }
  ...
  function attach(id, ws) {
    ...
    if (session.exited) {
      sendTo(ws, { type: "term_exited", id: id, exitCode: session.exitCode });
    }

...
var DEFAULT_THINKING_GRACE_MS = 3 * 60 * 1000;
...
function createHumanAttention(options) {
  ...
  function load() {
    ...
    try {
      ...
      for (var i = 0; i < ids.length; i++) users[ids[i]] = normalizeUser(sourceUsers[ids[i]]);
    } catch (e) {}
  }
  ...
  function signal(clientKey, input) {
    ...
    var active = value.visible === true && value.focused === true && value.engaged === true;
    ...
    if (active && state.explicitAt > 0 && at < state.explicitAt + thinkingGraceMs) {

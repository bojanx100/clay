...
function createHumanAttention(options) {
  ...
  function leaseUntil(state) {
    ...
    if (state.continuityGraceUntil) return state.continuityGraceUntil;
  ...
  function reconcile(targetAt) {
    ...
    clients.forEach(function (state, key) {
      if (state.disconnected && leaseUntil(state) <= now) clients.delete(key);
    });
  }
  ...
  function signal(clientKey, input) {
    ...
    var state = previous && previous.userId === userId ? previous : {
      ...
      continuityGraceUntil: 0,
      disconnected: false,
    };
    var wasMobileForeground = state.mobileForeground === true;
    ...
    if (active && state.explicitAt > 0 &&
      ...
      state.continuityGraceUntil = 0;
      state.disconnected = false;
    } else if (wasMobileForeground && state.activeSince && previousLease >= at) {
      state.mobileForeground = false;
      state.lastEvidenceAt = at;
      state.continuityGraceUntil = at + thinkingGraceMs;
      state.disconnected = false;
    } else if (state.activeSince && previousLease >= at &&
        state.explicitAt + thinkingGraceMs > at) {
      state.mobileForeground = false;
      state.lastEvidenceAt = at;
      state.continuityGraceUntil = state.explicitAt + thinkingGraceMs;
      state.disconnected = false;
    } else if (state.continuityGraceUntil > at) {
      state.mobileForeground = false;
    } else {
      state.activeSince = 0;
  ...
  function disconnect(clientKey) {
    var at = nowFn();
    reconcile(at);
    var state = clients.get(clientKey);
    if (!state) return;
    if (state.mobileForeground && leaseUntil(state) >= at) {
      state.mobileForeground = false;
      state.lastEvidenceAt = at;
      state.continuityGraceUntil = at + thinkingGraceMs;
    ...
    if (state.continuityGraceUntil > at) {
      state.disconnected = true;
      return;
    }
    if (state.activeSince && leaseUntil(state) >= at &&
        state.explicitAt + thinkingGraceMs > at) {
      state.mobileForeground = false;
      state.lastEvidenceAt = at;
      state.continuityGraceUntil = state.explicitAt + thinkingGraceMs;

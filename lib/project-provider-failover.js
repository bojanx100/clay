...
var coopModelRouting = require("./coop-model-routing");
...
function currentRouteId(routes, session) {
  if (!session) return null;
  if (session.providerRouteId) return session.providerRouteId;
  // Older native sessions predate route metadata. Infer only when the vendor
  // owns one route; Copilot has two real routes and must stay explicit.
  var inferred = null;
  for (var i = 0; i < routes.length; i++) {
    if (!routes[i] || routes[i].vendor !== session.vendor) continue;
    if (inferred) return null;
    inferred = routes[i].id;
  }
  return inferred;
}
function failoverPolicy(sm) {
  var policy = sm && (sm.workerRoutingPolicy || sm.adaptiveRoutingPolicy) || {};
  if (!policy.profile) {
    policy = Object.assign({}, policy, {
      profile: normalizeRoutingProfile(sm && sm.providerRoutingProfile),
    });
  }
  return policy;
}
function fallbackContext(sm, session) {
  ...
  var sourceRoute = session && session.providerRouteId ? routeForId(session.providerRouteId) : null;
  if (sourceTier === null && sourceRoute &&
      typeof sourceRoute.defaultCapabilityTier === "number") {
    sourceTier = sourceRoute.defaultCapabilityTier;
  }
...
function candidateForModel(route, targetModel, routeIndex, modelIndex, context, catalogSource) {
  ...
  if (targetTier === null && typeof route.defaultCapabilityTier === "number") {
    targetTier = route.defaultCapabilityTier;
  }
...
function compareFallbackCandidates(context, a, b) {
  ...
  if (context.policy.profile === "free-endurance" &&
      a.freeAllowancePotential !== b.freeAllowancePotential) {
    return a.freeAllowancePotential ? -1 : 1;
  }
  if (context.policy.profile === "best-available" && a.targetTier !== b.targetTier) {
    return b.targetTier - a.targetTier;
  }
...
function fallbackCandidates(sm, session, switcher) {
  if (coopModelPolicy.appliesToSession(session)) {
    return coopModelRouting.fallbackCandidates(sm, session, activeModel);
  }
  var context = fallbackContext(sm, session);
  if (!context) return [];
  var routes = Array.isArray(sm.providerRoutes) && sm.providerRoutes.length
    ? sm.providerRoutes
    : listProviderRoutes(sm.availableVendors || [], sm.installedVendors || [], sm);
  context.currentRouteId = currentRouteId(routes, session);
  var candidates = [];
  for (var i = 0; i < routes.length; i++) {
...
function attachProjectProviderFailover(ctx) {
  ...
  var coopRouting = coopModelRouting.attachRuntime({
    sm: sm, switcher: switcher, activeModel: activeModel,
  });
  var ensureCoopTopTierRoute = coopRouting.ensureRoute;
  sm.ensureCoopTopTierRoute = ensureCoopTopTierRoute;
  ...
  function failoverAndContinue(session, failure) {
    ...
    if (!coopModelPolicy.appliesToSession(session) &&
        !getComparableFailoverSetting(session)) {
    ...
    if (candidates.length === 0) {
      if (coopModelPolicy.appliesToSession(session)) {
        coopRouting.recordUnavailable(session, coopRouting.currentSelection(session));
      }
      return scheduleAfterProviderReset(session, failure);
    }
  ...
  return {
    ...
    ensureCoopTopTierRoute: ensureCoopTopTierRoute,
    switcher: switcher,
  };
}

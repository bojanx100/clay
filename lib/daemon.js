...
var relay = createServer({
  ...
  onGetServerDefaultModel: function (vendor) {
    if (vendor === "claude") return { model: config.defaultClaudeModel || config.defaultModel || null };
    if (vendor === "codex") return { model: config.defaultCodexModel || null };
    if (vendor === "github-copilot") return { model: config.defaultCopilotModel || null };
    return { model: config.defaultModel || null };
  },
  onSetServerDefaultModel: function (model, vendor) {
    if (vendor === "claude") {
      if (model) config.defaultClaudeModel = model;
      else delete config.defaultClaudeModel;
    } else if (vendor === "codex") {
      if (model) config.defaultCodexModel = model;
      else delete config.defaultCodexModel;
    } else if (vendor === "github-copilot") {
      if (model) config.defaultCopilotModel = model;
      else delete config.defaultCopilotModel;
    } else if (model) {
  ...
  onGetProjectDefaultModel: function (slug, vendor) {
    for (var i = 0; i < config.projects.length; i++) {
      if (config.projects[i].slug === slug) {
        if (vendor === "claude") return { model: config.projects[i].defaultClaudeModel || config.projects[i].defaultModel || null };
        if (vendor === "codex") return { model: config.projects[i].defaultCodexModel || null };
  ...
  onSetProjectDefaultModel: function (slug, model, vendor) {
    for (var i = 0; i < config.projects.length; i++) {
      if (config.projects[i].slug === slug) {
        if (vendor === "claude") {
          if (model) config.projects[i].defaultClaudeModel = model;
          else delete config.projects[i].defaultClaudeModel;
        } else if (vendor === "codex") {
          if (model) config.projects[i].defaultCodexModel = model;
          else delete config.projects[i].defaultCodexModel;
        } else if (model) {

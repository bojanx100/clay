...
function countActiveProviderTools() {
  var count = 0;
  if (!relay || typeof relay.forEachProject !== "function") return count;
  relay.forEachProject(function (ctx) {
    if (!ctx || !ctx.sm || !ctx.sm.sessions) return;
    ctx.sm.sessions.forEach(function (session) {
      if (!session || !session.isProcessing) return;
      count += Math.max(0, Number(session._activeProviderToolCount) || 0);
    });
  });
  return count;
}
...
function spawnAndRestart(options) {
  ...
  var activeToolCount = countActiveProviderTools();
  ...
  restartDrainPending = true;
  if (activeToolCount > 0) {
    console.log("[daemon] Restart queued until " + activeToolCount + " active provider tool call(s) finish");
    relay.broadcastAll({
      type: "toast",
      level: "info",
  ...
  streamWatchdog.waitForActiveTools(countActiveProviderTools).then(function (result) {
    if (!result.drained) {
      var timeoutMessage = "Restart cancelled after waiting for " + result.count +
        " active provider tool call(s) to finish.";
      console.error("[daemon] " + timeoutMessage);
      relay.broadcastAll({ type: "toast", level: "error", message: timeoutMessage });
      relay.broadcastAll({ type: "restart_server_result", ok: false, error: timeoutMessage });
      restartDrainPending = false;
      return;
    }

...
function createProjectContext(opts) {
  ...
  var _foundation = attachProjectFoundation({
    ...
    browserExtensionState: opts.browserExtensionState,
  });
  ...
  var getOsUserInfoForLinuxUser = _foundation.getOsUserInfoForLinuxUser;
  ...
  var _schedulerGate = _foundation.schedulerGate;
  ...
  if (_schedulerGate) {
    _schedulerGate.service = _loop.schedulerService;
  }
  ...
  var _interactions = attachProjectInteractions({
    planningGate: _taskOrchestrationGate,
    cwd: cwd,
    slug: slug,
    sm: sm,
    sdk: sdk,
  ...
  function handleDisconnection(ws) {
    ...
    if (disconnectBrowserExtension(
      browserState, ws, "clay_websocket", "socket_closed")) {
  ...
  var _features = attachProjectFeatures({
    ...
    isCoopTopicOwner: opts.isCoopTopicOwner,
  ...
  if (_taskOrchestrationGate && _taskOrchestrator) {
    _taskOrchestrationGate.queueUpdate = _taskOrchestrator.queueCoordinatorUpdate;
    if (_taskOrchestrationGate.planning) _taskOrchestrator.flushPlanningReports = _taskOrchestrationGate.planning.flushReports;
    _taskOrchestrationGate.canPlan = function (session) {
      return !!opts.crossProject && opts.crossProject.canRunCoordinatorUpdate(slug, session);
    };
    _taskOrchestrationGate.delegate = _taskOrchestrator.delegateFromTool;
    _taskOrchestrationGate.acceptAssignment = _taskOrchestrator.acceptProjectAssignmentFromTool;
  ...
  var _connection = attachConnection({
    ...
    humanAttention: opts.humanAttention || null,
  });
  ...
  var unregisterCrossProject = opts.crossProject &&
    ...
    opts.crossProject.registerProjectResolver({
      ...
      validateAutomationAuthorization: function (input) {
        if (!_autoLaunch || typeof _autoLaunch.validateAutomationAuthorization !== "function") {
          return { ok: false, reason: "automation_authorization_unavailable" };
        }
        return _autoLaunch.validateAutomationAuthorization(input);
      },
      slug: slug,
  ...
  return {
    ...
    getStatus: _status.getStatus,
    getSessionManager: function () { return sm; },
    resumeLeadAfterPortfolioTerminal: function () {
      return _features.coopSelfCleanup &&
        _features.coopSelfCleanup.resumeLeadAfterPortfolioTerminal ?
        _features.coopSelfCleanup.resumeLeadAfterPortfolioTerminal() : false;
    },
    getTaskOrchestrator: function () { return _taskOrchestrator; },

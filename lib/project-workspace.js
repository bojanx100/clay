...
function attachWorkspace(ctx) {
  ...
  function reconnectLiveUiTarget(session, tabUrl, cb) {
    var writableRoot = liveUiBinding.editableRoot(cwd, activeWtFor(session));
    devStatusPayload(session, function (status) {
      var result = liveUiBinding.reconnectTarget(writableRoot, tabUrl, status);
      if (result.ok && session) session.devCwdAbs = writableRoot;
      cb(result);
    }, writableRoot);
  }
  ...
  return {
    ...
    reconnectLiveUiTarget: reconnectLiveUiTarget,
  };
}

...
// script + computed port. Also starts/stops restart-safe dev servers: the main
// checkout and every git worktree get their own detached process group on the
// first free port at/above the project's base port, so any number of
// sessions/worktrees can run concurrently without colliding. The panel always
// reports the port a server actually bound to.
//
...
var createDevServerSupervisor =
  require("./project-workspace-dev-supervisor").createDevServerSupervisor;
var liveUiBinding = require("./project-workspace-live-ui-binding");
var attachProjectLiveUiWorkspace =
  require("./project-live-ui-workspace").attachProjectLiveUiWorkspace;
...
var workspaceTaskProjection = require("./workspace-task-projection");
var fs = require("fs");
...
function attachWorkspace(ctx) {
  ...
  var sm = ctx.sm;
  var getProjectId = ctx.getProjectId || function () { return null; };
  var getExecutionBindings = ctx.getExecutionBindings || function () { return []; };
  var liveUiWorkspace = attachProjectLiveUiWorkspace({
    getProjectList: ctx.getProjectList,
    listenerWorkingDirs: workspaceDevDiscovery.listenerWorkingDirs,
  ...
  //   resolvedCwd -> { pid, port, script, command, branch, startedAt }
  function checkLocalPort(port, cb) {
    gitw.probePort(port, function (ipv4Live) {
      if (ipv4Live) return cb(true);
      workspaceDevDiscovery.probeIpv6Port(port, cb);
    });
  }

  var devServerSupervisor = createDevServerSupervisor({
    projectDir: cwd,
    slug: slug,
    checkPort: checkLocalPort,
    portBelongsToDir: portBelongsToDir,
    requireOsUserInfo: !!osUsers,
  });
  var devServers = devServerSupervisor.managedEntries();
  ...
  function taskRows() {
    ...
    if (sm && sm.sessions && typeof sm.sessions.forEach === "function") {
      sm.sessions.forEach(function (session) { if (session && !session.hidden) sessions.push(session); });
    }
    return workspaceTaskProjection.projectTasks({
  ...
  function liveServerFor(dirAbs) {
    ...
    if (s && s.owner === "managed" && devServerSupervisor.isProcessAlive(s)) return s;
  ...
  function boundDirFor(session) {
    ...
    return liveUiBinding.editableRoot(cwd, activeWtFor(session));
  }
  ...
  function ownLivePorts(exceptAbs) {
    ...
    Object.keys(devServers).forEach(function (k) {
      var s = liveServerFor(k);
      if (s && k !== exceptAbs && s.port) ports.push(s.port);
    });
    return ports;
  }
  ...
  function devStatusPayload(session, cb, rootOverride, osUserInfo) {
    var boundAbs = rootOverride ? path.resolve(rootOverride) : boundDirFor(session);
    ...
    function finishPayload(result) {
      if (result.external && result.portLive && det) {
        devServerSupervisor.observeExternal({
          cwd: boundAbs,
          script: det.script,
          command: det.command,
          port: result.port,
          branch: branch,
          osUserInfo: osUserInfo || null,
        });
      } else if (!result.running) {
        devServerSupervisor.forgetStoppedExternal(boundAbs);
      }
    ...
    if (server) {
      ...
      checkLocalPort(server.port, function (live) {
  ...
  function sendDevStatusTo(ws) {
    devStatusPayload(getSessionForWs(ws), function (p) { sendTo(ws, p); }, null,
      getOsUserInfoForWs(ws));
  }

  function getLiveUiTarget(session, cb) {
    var writableRoot = boundDirFor(session);
    devStatusPayload(session, function (status) {
      cb(liveUiBinding.targetFromStatus(writableRoot, status));
    });
  }

  function inspectLiveUiTarget(tabUrl, userId, cb) {
    liveUiWorkspace.inspect(tabUrl, userId, cb);
  }

  function bindLiveUiTarget(session, target) {
    if (!session || !target || target.projectSlug !== slug ||
        !target.writableRoot) return false;
    session.devCwdAbs = target.writableRoot;
    if (typeof persistSession === "function") {
      try { persistSession(session); } catch (error) {}
    }
    return true;
  }

  function attachLiveUiTarget(session, tabUrl, userId, cb) {
    inspectLiveUiTarget(tabUrl, userId, function (result) {
      if (!result.ok) return cb(result);
      if (result.target.projectSlug !== slug) {
        cb({
          ok: false,
      ...
      if (session) bindLiveUiTarget(session, result.target);
      cb(result);
    });
  }
  ...
  function buildState(session, requestId, cb) {
    ...
    devStatusPayload(session, function (st) {
      ...
      Promise.all([
        ...
        cb({
          ...
          sessionId: session ? session.localId : null,
          requestId: requestId,
          error: "Failed to load workspace context: " + (e && e.message || e),
        });
      });
    });
  }
  ...
  function startDevServer(ws, msg, opts) {
    ...
    if (opts.takeover && existing) {
      ...
      if (existing.owner === "managed") {
        devServerSupervisor.stop(runAbs);
      } else {
    ...
    function spawnDev(port) {
      var result = devServerSupervisor.start({
        cwd: runCwd,
        script: det.script,
        command: det.command,
        port: port,
        branch: runBranch,
        osUserInfo: getOsUserInfoForWs(ws),
        onPort: function (actualPort) {
      ...
      if (!result.ok) {
        sendTo(ws, { type: "workspace_dev_status", running: false, error: result.error });
        return;
      }
      devServers[runAbs] = result.record;
  ...
  function handleWorkspaceMessage(ws, msg) {
    if (msg.type === "workspace_get") {
      ...
      buildState(session, msg.requestId == null ? null : msg.requestId, function (state) { sendTo(ws, state); });
      return true;
    }
    ...
    if (msg.type === "workspace_tasks_get") {
      var taskSession = getSessionForWs(ws);
      sendTo(ws, { type: "workspace_tasks_state", sessionId: taskSession && taskSession.localId,
        requestId: msg.requestId == null ? null : msg.requestId, tasks: taskRows() });
      return true;
    }
    ...
    if (msg.type === "workspace_dev_stop") {
      ...
      if (stopServer) {
        var stopPort = stopServer.port;
        if (stopServer.owner === "managed") {
          devServerSupervisor.stop(stopAbs, function () {
            waitPortFree(stopPort, 12, function () { sendDevStatusTo(ws); });
          });
        } else {
  ...
  devServerSupervisor.restore(function (record) {
    devServers[record.cwd] = record;
    broadcastDevStatus();
  });
  ...
  return {
    ...
    inspectLiveUiTarget: inspectLiveUiTarget,
    bindLiveUiTarget: bindLiveUiTarget,
    attachLiveUiTarget: attachLiveUiTarget,
  };
}

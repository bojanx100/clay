function attachProjectSessionsView(ctx) {
  ...
  // import path lands here. Conversation replay includes owner image attachments.
  function prepareCodexSessionForView(session) {
    ...
    var fresh = hasHistory && session._historyMtime === mtime &&
      session._historyFormatVersion === cliSess.HISTORY_FORMAT_VERSION;
    if (fresh) return;
    if (hasHistory && !session._historyMtime) return;
    var history = null;

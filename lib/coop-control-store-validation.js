...
var OUTCOME_STATUS_LIMIT = 40;
var SUPERSEDED_BY_LIMIT = 40;
...
function normalizeResponse(value) {
  ...
  // The adjacent owner ledger supersedes a request with a bounded control code
  // (`owner_interrupt`, or a caller reason capped at 40 characters), never with
  // an ingress id. A single unspaced code is prose-resistant and is the only
  // evidence that distinguishes one superseded request from another.
  var supersededBy = source.supersededBy;
  if (typeof supersededBy !== "string" || supersededBy.length > SUPERSEDED_BY_LIMIT ||
      (supersededBy && !CODE_RE.test(supersededBy))) {
...
function normalizeOutcome(value) {
  ...
  if (typeof source.status !== "string" || source.status.length > OUTCOME_STATUS_LIMIT) {
    throw taggedError("COOP_CONTROL_STORE_INVALID_RECORD", "outcome.status exceeds its ledger bound.");
  }
...
function normalizeOwnerRequest(value, recordKey) {
  ...
  // Sequence 0 is the adjacent ledger's canonical "unknown sequence" sentinel
  // for a record whose stored sequence was missing or unparsable. Rejecting it
  // would abort the whole projection over one degraded row.
...
module.exports = {
  ...
  OUTCOME_STATUS_LIMIT: OUTCOME_STATUS_LIMIT,
  SUPERSEDED_BY_LIMIT: SUPERSEDED_BY_LIMIT,

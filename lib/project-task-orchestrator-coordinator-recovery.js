...
function createTaskCoordinatorRecovery(options) {
  ...
  function canResumeCurrent(session) {
    if (!opts.executionControl.enabled) return false;
    try {
      opts.executionControl.assert(session, "tool");
      return true;
    } catch (error) {
      return false;
    }
  ...
  function prepareNeedsInputResume(session, envelope) {
    ...
    return { ok: true, request: request, reuseCurrent: canResumeCurrent(session), source: authority };
  }

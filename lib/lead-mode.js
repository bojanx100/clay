...
function isAuthority(user, multiUser) {
...
function migrationOptions(options) {
  var usersModule = options.usersModule;
  var owner = usersModule && typeof usersModule.findAdmin === "function" ? usersModule.findAdmin() : null;
  var ownerId = options.ownerId || (owner && owner.id) || (options.user && options.user.id) || null;
...
function setLeadMode(options) {
  ...
  if (!isAuthority(options.user || null, options.multiUser === true)) {
    return {
      ok: false,
      error: "forbidden",

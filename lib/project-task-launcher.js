...
var attachExternalTaskRouting =
  require("./project-task-launcher-external").attachExternalTaskRouting;
var ownerAcceptanceModule = require("./project-owner-acceptance");
var attachProjectLocalInstructions =
  require("./project-local-instructions").attachProjectLocalInstructions;
var { findCompletionMarker, shouldCloseCompletedSession } = require("./project-task-launcher-completion");
var {
  automationForClaudePermission,
  automationForCodexConfig,
} = require("./automation-modes");

function taskItemKey(recipe, item) {
  ...
  if (source.provider === "sentry") {
    ...
    var sentryId = item && (item.sentry_id || item.number);
    return "sentry:" + organization + "/" + project + "#" + sentryId;
  }
  if (source.repo && item && item.number != null) {
    return source.repo + "#" + item.number;
  }
  return "";
}
...
function attachTaskLauncher(ctx) {
  ...
  // Coop's enforcement point, resolved lazily: auto-launch is attached after
  // the task launcher, so this is a getter rather than a value.
  var getAutomationGate = ctx.getAutomationGate || null;
  ...
  var projectLocalInstructions = attachProjectLocalInstructions({ cwd: cwd });
  ...
  function renderPrompt(recipe, item, loadedWorkflow) {
    var promptCfg = recipe.prompt || {};
    var workflow = loadedWorkflow || projectLocalInstructions.loadForStaffing();
    if (!workflow.ok) {
      throw new Error("Required project instruction is missing or empty: " +
    ...
    var requiredIncludeFiles = promptCfg.requiredIncludeFiles || [];
    var required = {};
    for (var ri = 0; ri < requiredIncludeFiles.length; ri++) {
      required[String(requiredIncludeFiles[ri])] = true;
    }
    var included = [];
    var includedPaths = {};
    if (workflow.configured) {
      for (var wi = 0; wi < workflow.files.length; wi++) {
        included.push(workflow.files[wi].body);
        includedPaths[workflow.files[wi].path] = true;
      }
    }
    for (var i = 0; i < includeFiles.length; i++) {
      var relative = String(includeFiles[i]);
      if (includedPaths[relative]) continue;
      var includePath = path.resolve(cwd, relative);
      if (includePath.indexOf(cwd + path.sep) !== 0 && includePath !== cwd) {
        if (required[relative]) {
          throw new Error("Required project instruction is outside the project: " + relative);
        }
        continue;
      }
      var includeText = readTextIfExists(includePath);
      if (includeText.trim()) included.push(includeText);
      else if (required[relative]) {
        throw new Error("Required project instruction is missing or empty: " + relative);
      }
    }
    for (var requiredPath in required) {
      if (includeFiles.indexOf(requiredPath) === -1) {
        throw new Error("Required project instruction is not configured: " + requiredPath);
      }
    }
  ...
  var externalTaskRouting = attachExternalTaskRouting({
    coordinateExternalTask: ctx.coordinateExternalTask || null,
  ...
  function startSessionForItem(ws, recipe, item, args, user, launchOpts) {
    ...
    var workflow = projectLocalInstructions.loadForStaffing();
    if (!workflow.ok) {
      throw new Error("Required project instruction is missing or empty: " +
        workflow.missing.join(", "));
    }
    var prompt = renderPrompt(recipe, item, workflow);
    ...
    session.taskLauncher = {
      ...
      itemKey: taskItemKey(recipe, item),
      completion: workflow.ownerAcceptanceRequired === true ?
        Object.assign({}, recipe.completion || {}, { requireUserTrigger: true }) :
        recipe.completion || null,
      autoLaunch: !!launchOpts.auto,
      // Used by the sidebar badge + completion notification to distinguish flavors.
      autoKind: autoKind,
      prKey: isPrReview ? (taskItemKey(recipe, item) || null) : null,
    };
  ...
  function handleTaskTurnDone(session, preview, fullText) {
    ...
    // sign-off after a draft PR). An adopted Lead-ON primitive still reports
    // internal implementation completion to Coop here, but only a user-
    // triggered close (the manual "mark as done" path sets closeAfterNextTurn)
    // finishes the local task and runs the external Done workflow.
    if (completion.requireUserTrigger && !closeAfterNextTurn) {
      if (isAdoptedAutoLaunchPrimitive(session)) {
        notifyAutoLaunchCompletion(session, text, matchedCompletion);
      }
      return;
    }
    ...
    notifyAutoLaunchCompletion(session, text, matchedCompletion);
    if (shouldCloseCompletedSession(session, completion)) {
  ...
  // The Done workflow's approval must come from the OWNER, not from whoever
  // happens to be typing in the session. Fabricating `by: session.ownerId`
  // from an arbitrary sender's message would let any member with access to a
  // shared auto-launched session grant PR and board authority.
  //
  // Single-user installs have exactly one principal, so the sender IS the
  // owner. Multi-user installs must match the session's recorded owner, and an
  // unidentified sender never qualifies.
  function handleTaskUserMessageDispatched(session, text, actorUserId) {
    if (!session || !session.taskLauncher || session.taskLauncher.workflowCompleted) return "";
    ...
    if (!ownerAcceptanceModule.matchesCompletionTrigger(completion, text || "")) return "";
    var approval = ownerAcceptance.approvalFor(session, actorUserId, text);
    if (!approval) {
      return "[Clay] Only the project owner may authorize this task's Done workflow. " +
        "Do not change issue, PR, board, assignment, or cleanup state.";
    }
    // This directive is where external authority is actually granted: it is
    // what tells the agent to comment on the issue, take the PR out of draft
    // and move the board. Under Coop that grant is gated on this automation
    // still holding the item's claim; the owner's explicit "mark as done"
    // supplies the approval. With Lead mode off the gate is a pass-through, so
    // the legacy flow is byte-for-byte unchanged.
    var gate = typeof getAutomationGate === "function" ? getAutomationGate() : null;
    var execution = session.orchestrationPolicy && session.orchestrationPolicy.portfolioExecution;
    // An authenticated owner retains the original Done workflow for a session
    // Lead has never adopted. Managed sessions still require their exact claim;
    // a missing binding on a controlled session is not an owner-direct session.
    if (gate && session.taskLauncher.autoLaunch && (execution || session.coopControlledBy)) {
      var verdict = gate.evaluateExternal({
        itemKey: session.taskLauncher.itemKey || session.taskLauncher.prKey || null,
        externalKind: "done_workflow",
        // Only Coop can attest a canonical execution binding for this work.
        // A project controller has no way to synthesize one, which is exactly
        // why external authority cannot originate here.
        coopAuthorization: execution || null,
        // Only an owner-authenticated request is owner-triggered. Anyone else
        // falls through to the full completion-evidence requirement.
        ownerTriggered: !!approval,
        approval: approval,
        completion: session.orchestrationProjectCompletion || null,
      });
      if (verdict.decision !== "execute") {
        return "[Clay] The Done workflow for this task is routed through Coop and was not " +
          "authorized (" + verdict.reason + "). Do not comment on, merge, close, or otherwise " +
          "change anything outside this workspace. Summarize what remains and stop.";
      }
    }
    session.taskLauncher.closeAfterNextTurn = true;
    session.taskLauncher.closeTriggerText = text || "";
    session.taskLauncher.ownerCompletionApproval = approval;
    sm.saveSessionFile(session);
    return completionDirective(completion);
  }

  function isAdoptedAutoLaunchPrimitive(session) {
    var execution = session && session.orchestrationPolicy &&
      session.orchestrationPolicy.portfolioExecution;
    var authorization = execution && execution.automationAuthorization;
    return !!(session && session.taskLauncher && session.taskLauncher.autoLaunch &&
      authorization && authorization.kind === automationAuthorization.PRIMITIVE_KIND);
  }

  function notifyAutoLaunchCompletion(session, text, matchedCompletion) {
    if (!session || !session.taskLauncher || !session.taskLauncher.autoLaunch ||
        !onComplete || session.taskLauncher.completionCallbackInvoked) return;
    var index = matchedCompletion ? matchedCompletion.index : -1;
    var marker = matchedCompletion ? matchedCompletion.marker : "";
    var summary = index !== -1 ?
      text.substring(index + marker.length).split("\n")[0].replace(/^[\s:>-]+/, "").trim() : "";
    var result;
    try { result = onComplete(session, summary); } catch (e) {
      console.log("[task-launcher] onComplete failed:", e && e.message);
      return;
    }
    // Adopted primitives may retry a transient coordinator-report failure on a
    // later completion marker. Latch only after the callback proves delivery;
    // legacy callbacks have no result contract and keep their original path.
    if (isAdoptedAutoLaunchPrimitive(session) && (!result || result.ok !== true)) return;
    if (isAdoptedAutoLaunchPrimitive(session) && session.taskLauncher.completion &&
        session.taskLauncher.completion.requireUserTrigger &&
        !session.taskLauncher.workflowCompleted) return;
    session.taskLauncher.completionCallbackInvoked = true;
  ...
  async function launchExternal(body, user) {
    try {
      ...
      var coordinatedTask = externalTaskRouting.routeExternalTask(body, recipe, items[0], user);
      if (coordinatedTask) return coordinatedTask;
...
module.exports = { attachTaskLauncher: attachTaskLauncher, taskItemKey: taskItemKey };

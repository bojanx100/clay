...
function attachTaskLauncher(ctx) {
  ...
  function startSessionForItem(ws, recipe, item, args, user, launchOpts) {
    ...
    session.taskLauncher = {
      ...
      itemKey: item.key || "",
      // The canonical automation claim key and its fencing token, stamped
      // BEFORE the session is persisted or any query starts. Persisting them
      // afterwards left a window where a crash produced a running session that
      // reconciliation could not tie back to its claim, so another daemon
      // could launch the same item once the lease lapsed.
      automationClaimKey: launchOpts.automationClaimKey || "",
      automationClaimToken: launchOpts.automationClaimToken || "",
  ...
  function ownerApprovalFor(session, actorUserId) {
    var multiUser = usersModule && typeof usersModule.isMultiUser === "function" &&
      usersModule.isMultiUser();
    if (!multiUser) {
      return { granted: true, by: session.ownerId || "local-owner", at: Date.now() };
    }
    if (!actorUserId || !session.ownerId || actorUserId !== session.ownerId) return null;
    return { granted: true, by: actorUserId, at: Date.now() };
  }
  ...
  function handleTaskUserMessageDispatched(session, text, actorUserId) {
    ...
    if (gate && session.taskLauncher.autoLaunch) {
      var approval = ownerApprovalFor(session, actorUserId);
      var verdict = gate.evaluateExternal({
        itemKey: session.taskLauncher.automationClaimKey ||
          session.taskLauncher.itemKey || session.taskLauncher.prKey || null,

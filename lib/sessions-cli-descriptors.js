...
function isBackstagePrompt(text) {
  ...
  if (/^\s*You are a canonical direct-leaf worker for a Coop-owned portfolio task\./.test(text)) return true;
  ...
  function readCliSessionDescriptor(cliSid) {
    ...
    function readClaudeDescriptorLine(line) {
      ...
      if (ev.type === "user" || ev.type === "assistant") {
        if (ev.entrypoint === "sdk-ts" || ev.promptSource === "sdk") sdkSession = true;
        var messageAt = typeof ev.timestamp === "string" ? Date.parse(ev.timestamp) : NaN;
    ...
    // Provider metadata rewrites are not conversation activity. SDK transcripts
    // and legacy greeting probes require explicit import, not startup adoption.
    return {
      cliSid: cliSid, title: title, preview: firstUserText || "",
      createdAt: createdAt, lastActivity: lastMessageAt || createdAt,
      autoAdoptable: !sdkSession && !(userCount === 1 && /^hi[?!,.]*$/i.test((firstUserText || "").trim())),
    };
  }

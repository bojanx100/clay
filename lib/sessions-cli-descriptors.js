...
function readCodexUserMessagePrefix(line) {
  var completed = require("./codex-rollout-message").completedUserPrefix(line, decodeJsonStringPrefix, CODEX_PREVIEW_CHARS);
...
function attachSessionCliDescriptors(ctx) {
  ...
  function readCliSessionDescriptor(cliSid) {
    ...
    var descriptor = require("./claude-session-descriptor").read(path.join(cliSessionsDir(), cliSid + ".jsonl"));
    if (!descriptor || isBackstagePrompt(descriptor.preview)) return null;
    if (descriptor.userCount === 1 && descriptor.assistantCount === 0 && descriptor.preview === "hi") return null;
    return descriptor;
  }

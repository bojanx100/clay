...
var { attachVendorReadiness } = require("./sdk-bridge-vendor-readiness");
...
function createSDKBridge(opts) {
  ...
  // getter form lets callers gate individual servers at call time. Browser
  // tools stay advertised so provider tool snapshots remain stable.
  var _mcpServersSrc = opts.mcpServers || null;
  function getMcpServers(session) {
    if (typeof _mcpServersSrc === "function") return _mcpServersSrc(session) || null;
  ...
  var getRuntimeEnv = opts.getRuntimeEnv || function () { return process.env; };
  var environmentRefreshPending = false;
  var environmentRefreshPromise = null;
  ...
  var vendorReadiness = attachVendorReadiness({
    adapters: adapters,
    sm: sm,
    cwd: cwd,
  ...
  function hasActiveQueries() {
    ...
    sm.sessions.forEach(function (session) {
      if (session && session.queryInstance) active = true;
    });
    return active;
  }

  function applyEnvironmentRefresh() {
    if (!environmentRefreshPending) return environmentRefreshPromise || Promise.resolve(true);
    if (hasActiveQueries()) return Promise.resolve(false);
    if (environmentRefreshPromise) return environmentRefreshPromise;
    var vendorNames = Object.keys(adapters);
    environmentRefreshPromise = Promise.all(vendorNames.map(function (vendor) {
      vendorReadiness.invalidate(vendor);
      var runtime = adapters[vendor];
      if (!runtime || typeof runtime.shutdownIfIdle !== "function") return false;
      return Promise.resolve(runtime.shutdownIfIdle(0)).catch(function () { return false; });
    })).then(function () {
      environmentRefreshPending = false;
      return true;
    }).finally(function () {
  ...
  function refreshEnvironmentRuntime() {
    environmentRefreshPending = true;
    applyEnvironmentRefresh().catch(function () {});
    return true;
  }
  ...
  var bridgeQueryStart = attachBridgeQueryStart({
    getControlSessionContext: opts.getControlSessionContext,
    adapters: adapters,
    adapter: adapter,
    cwd: cwd,
    dangerouslySkipPermissions: dangerouslySkipPermissions,
    ...
    vendorReadiness: vendorReadiness,
    getRuntimeEnv: getRuntimeEnv,
    applyEnvironmentRefresh: applyEnvironmentRefresh,
  });
  var startQuery = bridgeQueryStart.startQuery;
  var pushMessage = bridgeQueryStart.pushMessage;
  var ensureVendorReady = bridgeQueryStart.ensureVendorReady;

  function prepareVendor(vendor, linuxUser) {
    return vendorReadiness.ensure(vendor, linuxUser).then(function (details) {
      return details.adapter;
    });
  }

  function refreshVendor(vendor, linuxUser) {
    return vendorReadiness.refresh(vendor, linuxUser).then(function (details) {
      return details.adapter;
    });
  }

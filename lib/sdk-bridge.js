...
function createSDKBridge(opts) {
  ...
  function handleCanUseTool(session, toolName, input, opts) {
    ...
    // AskUserQuestion: post the card, end this turn, and let the user's
    // answer arrive as an explicit user_message on the next turn. Returning
    // answers via permission metadata proved unreliable across vendors and
    // reloads; a normal user turn is the contract the model understands.
    if (toolName === "AskUserQuestion") {

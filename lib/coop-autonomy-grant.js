...
// Retracted: the owner-review admission used to check only the string prefix.
// It now checks every segment too. This standing-grant form also tolerates
// empty separators, but never a writable nonempty segment.
...
function readOnlyDiagnosis(input) {
  ...
  if (readOnlyReview.hasMutatingInstruction(input)) return false;

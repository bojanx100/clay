...
function createIssueLaunchState(cwd) {
  ...
  // Owner-requested bounce repair for a stale launched/unarmed entry whose
  // session never became durable launch evidence. The caller must pass the
  // exact value it observed so a concurrent launch-state change fails closed.
  function clearStaleLaunch(key, evidence, options) {
    if (!key) return { ok: false, reason: "invalid_key" };
    var all = read();
    if (!Object.prototype.hasOwnProperty.call(all, key)) {
      return { ok: false, reason: "not_found" };
    }
    var prepared = bookkeepingRepair.prepareIssueLaunchStateClear(all[key], evidence,
      defaultEntry(), Object.assign({}, options, { itemKey: key }));
    ...
    return {
      ...
      reconsideration: prepared.reconsideration,
    };
  }

  return {
    clearStaleLaunch: clearStaleLaunch,
    read: read,
    get: get,
    restore: restore,
    snapshot: snapshot,

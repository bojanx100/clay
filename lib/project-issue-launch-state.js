...
function createIssueLaunchState(cwd) {
  ...
  // Undo a recordLaunch whose session never started. `existedBefore` says
  // whether this issue already had an entry: if it did not, the entry is
  // removed entirely, otherwise it is left alone rather than inventing a
  // state we did not observe. Without this a failed start marks the issue
  // launched forever and it is never retried.
  function forget(key, existedBefore) {
    if (!key || existedBefore) return;
    var all = read();

...
// The dev watcher's counterpart to the daemon banner above. Returns the text
// that follows the coloured "[dev]" tag, so bin/cli.js keeps its own prefix.
//
// The asymmetry this line exists to state: the daemon is supervised and the
// watcher is not. bin/cli.js respawns the daemon 500ms after ANY unexpected
// exit, so killing the daemon alone is a blip it recovers from by itself. But
// shutdownWatcher sets intentionalKill, which suppresses that respawn, and
// nothing supervises the watcher — so once the watcher goes, Clay stays down
// until a human runs `clay` again.
//
// The old line was a bare "Shutting down...", identical whether the owner
// pressed Ctrl+C or a script sent `kill -TERM`, and it never said the outage
// was terminal. On 2026-09-04 a repair script killed the watcher five times and
// every outage ended only when the owner happened to notice Clay was gone.
// Naming the signal is what separates "I meant this" from "something did this
// to me": a Ctrl+C reaches this process as SIGINT, so any other signal came
// from a script, a takeover or a supervisor.
function watcherShutdownMessage(signal) {
  var why = normalizeReason(signal);
  var unexpected = why !== "SIGINT";
  return "Shutting down (" + why + ")"
    + (unexpected ? " — not a Ctrl+C, something sent " + why : "")
    + ". Clay stays down: the daemon is supervised, this watcher is not."
    + " Run `clay` to bring it back.";
}

module.exports = {
  createShutdownGate: createShutdownGate,
  watcherShutdownMessage: watcherShutdownMessage,

  ...
  function handleFilesystemMessage(ws, msg) {
    ...
    if (msg.type === "set_project_env") {
      if (typeof opts.onSetProjectEnv === "function") {
        ...
        sendTo(ws, { type: "set_project_env_result", ok: setResult.ok, slug: msg.slug,
          error: setResult.error, timing: setResult.ok ?
            "Saved. Newly created coding-agent processes use these values; active processes keep their current environment." : undefined });
      } else {

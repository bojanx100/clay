...
var OUT_OF_SCOPE_PAYLOAD_KEYS = Object.freeze({
  chainofthought: true,
  history: true,
  messages: true,
  projection: true,
  ...
  topicbody: true,
  topicindex: true,
  topicmembership: true,
  topicmemberships: true,
  topicrecord: true,
  topictitle: true,
  topics: true,
  transcript: true,
...
function assertControlPayload(value, ancestors) {
  ...
  stack.push(value);
  if (Array.isArray(value)) {
    for (var i = 0; i < value.length; i++) assertControlPayload(value[i], stack);
    stack.pop();
    return;
  }
  var keys = Object.keys(value);
  for (var j = 0; j < keys.length; j++) {
    var normalized = keys[j].replace(/[^A-Za-z0-9]/g, "").toLowerCase();
    if (Object.prototype.hasOwnProperty.call(OUT_OF_SCOPE_PAYLOAD_KEYS, normalized)) {
      throw taggedError("COOP_CONTROL_STORE_OUT_OF_SCOPE",
        "ControlStore payloads cannot contain " + keys[j] + ".");
    }
    assertControlPayload(value[keys[j]], stack);
  }
  stack.pop();
}
...
function validateRecordIdentity(recordType, recordKey) {
  ...
  if (typeof recordKey !== "string" || !IDENTIFIER_RE.test(recordKey)) {
    throw taggedError("COOP_CONTROL_STORE_INVALID_RECORD", "Control record keys must be bounded identifiers.");
  }
}
...
function buildHandle(db, dbPath, options, migration, journalMode) {
  ...
  function putControlRecordDirect(recordType, recordKey, value, revision) {
    ...
    var nextRevision = Number.isInteger(revision) && revision > 0
    ...
    db.prepare("INSERT INTO coop_control_records " +
      ...
      .run(recordType, recordKey, nextRevision, canonicalStringify(value), createdAt, timestamp);
  ...
  function transaction(work) {
    ...
    var tx = { putControlRecord: putControlRecordDirect };
    try {
  ...
  function replaceShadowProjection(sourceId, input) {
    ...
    validateSourceId(sourceId);
    var source = input || {};
    var records = Array.isArray(source.records) ? source.records.slice() : [];

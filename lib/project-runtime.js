...
var { createProjectExecutionProviderSwitch } = require("./project-execution-provider-switch");
...
var createProjectRuntimeEnvResolver = require("./project-runtime-env").createProjectRuntimeEnvResolver;
...
function attachProjectRuntime(ctx) {
  ...
  var getTaskOrchestrator = ctx.getTaskOrchestrator || function () { return null; };
  ...
  var workspace = null;
  var getRuntimeEnv = createProjectRuntimeEnvResolver({
    slug: slug,
    getLinuxUserForSession: getLinuxUserForSession,
    getOsUserInfoForLinuxUser: getOsUserInfoForLinuxUser,
    onGetSharedEnv: opts.onGetSharedEnv,
    onGetProjectEnv: opts.onGetProjectEnv,
  });
  ...
  function autoResumeRestartSession(session, options) {
    if (!require("./scheduled-execution-policy").restoresWork() && !(options && options.userInitiated)) return false;
    reconcileQueuedUserMessages(session);
    if (!scheduledMessages) return false;
    return scheduledMessages.autoResumeRestartSession(session, options);
  }

  // Records that the owner's Coop request was actually answered by this turn.
  // A no-op for every non-Coop session, and for a Coop turn that errored out or
  // was cut short by the owner's next message.
  function coopTurnDone(session) {
    var userMessage = getUserMessage();
    if (!userMessage || typeof userMessage.handleCoopTurnDone !== "function") return false;
    try { return userMessage.handleCoopTurnDone(session); }
  ...
  function reconcileQueuedUserMessages(session) {
    ...
    if (typeof userMessage.reconcileQueuedUserMessages === "function") {
      return userMessage.reconcileQueuedUserMessages(session);
    }
    if (typeof userMessage.scheduleQueuedUserMessageFlush === "function") {
      return userMessage.scheduleQueuedUserMessageFlush(session);
    }
    return false;
  }
  ...
  function resumeCoopIngress(session, ingressId) {
    var userMessage = getUserMessage();
    if (!userMessage || typeof userMessage.resumeCoopIngress !== "function") return false;
    return userMessage.resumeCoopIngress(session, ingressId);
  }
  ...
  var sdk = createSDKBridge({
    ...
    slug: slug,
    sessionManager: sm,
    getControlSessionContext: opts.crossProject && opts.crossProject.getControlSessionContext,
    send: send,
    ...
    onTurnDone: function (session, preview, fullText) {
      ...
      // Strictly before the reconcile below: draining the Coop ingress lane
      // clears the active ingress id this answer has to be attributed to.
      coopTurnDone(session);
      reconcileQueuedUserMessages(session);
      if (taskOrchestrator && typeof taskOrchestrator.handleCoordinatorTurnDone === "function") {
        taskOrchestrator.handleCoordinatorTurnDone(session);
      } else if (taskOrchestrator && typeof taskOrchestrator.flushCoordinatorUpdates === "function") {
        taskOrchestrator.flushCoordinatorUpdates(session);
      }
    },
  ...
  function prepareFallbackProviders(session) {
    if (!sdk || typeof sdk.ensureVendorReady !== "function") return Promise.resolve();
    var installed = sm.installedVendors || [];
    var currentVendor = (session && session.vendor) || "claude";
    var linuxUser = typeof getLinuxUserForSession === "function" ? getLinuxUserForSession(session) : null;
    var vendors = Object.keys(adapters);
    var pending = [];
    for (var i = 0; i < vendors.length; i++) {
      var vendor = vendors[i];
      if (vendor === currentVendor || installed.indexOf(vendor) === -1) continue;
      pending.push(Promise.resolve(sdk.ensureVendorReady(vendor, linuxUser)).catch(function (err) {
        console.warn("[project] Fallback provider preparation failed: " + (err && err.message ? err.message : err));
        return null;
      }));
    }
  ...
  providerFailover = attachProjectProviderFailover({
    ...
    prepareFallbackProviders: prepareFallbackProviders,
  });
  ...
  var switchProjectExecutionProvider = createProjectExecutionProviderSwitch({
    sm: sm,
  ...
  workspace = attachWorkspace({
    ...
    sm: sm,
    getProjectId: function () { return typeof sm.getProjectId === "function" ? sm.getProjectId() : null; },
    getExecutionBindings: function () {
      return opts.crossProject && typeof opts.crossProject.getExecutionBindings === "function" ?
        opts.crossProject.getExecutionBindings() : [];
    },
  });
  ...
  function warmup(linuxUser) {
    sdk.warmup(linuxUser || undefined);
    sdk.startIdleReaper();

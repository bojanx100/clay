    ...
    if (msg.vendor) {
      try {
        ...
        if (vendorAdapter && vendorAdapter._clayReadyResult) {
          var lazyReady = vendorAdapter._clayReadyResult;
          sm.capabilitiesByVendor = sm.capabilitiesByVendor || {};
          sm.capabilitiesByVendor[msg.vendor] = lazyReady.capabilities || {};
          if (Array.isArray(lazyReady.models)) {
            sm.modelsByVendor = sm.modelsByVendor || {};
            var lazyModels = modelCatalogCache.applyDiscovery(msg.vendor, lazyReady.models);
            sm.modelsByVendor[msg.vendor] = msg.vendor === "claude" ? withClaudeFallbackModels(lazyModels) : lazyModels;
          }
        }
        ...
        if (vendorAdapter && needsReadyMetadata && !vendorAdapter._clayReadyResult &&
          ...
          try {
            ...
            sm.modelsByVendor = sm.modelsByVendor || {};
            if (Array.isArray(readyResult.models)) {
              var readyModels = modelCatalogCache.applyDiscovery(msg.vendor, readyResult.models);
              sm.modelsByVendor[msg.vendor] = msg.vendor === "claude" ? withClaudeFallbackModels(readyModels) : readyModels;
            }
          } catch (e) {
        ...
        if (vendorAdapter) {
          ...
          if (!sm.modelsByVendor[msg.vendor] && typeof vendorAdapter.supportedModels === "function") {
            ...
            var resolvedModels = modelCatalogCache.applyDiscovery(msg.vendor, discoveredModels);
            sm.modelsByVendor[msg.vendor] = msg.vendor === "claude" ? withClaudeFallbackModels(resolvedModels) : resolvedModels;
          }
        }
      } catch (e) {

...
function attachProjectVendorModels(ctx) {
  ...
  async function sendVendorModels(ws, msg) {
    ...
    vendorModels = require("./coop-model-routing").modelsForSession(
      activeForModels, msg.vendor, msg.providerRouteId, vendorModels);
    ...
    if (require("./coop-model-policy").appliesToSession(activeForModels) &&
        activeForModels.vendor === msg.vendor &&
        (!msg.providerRouteId || activeForModels.providerRouteId === msg.providerRouteId)) {
      modelToSend = activeForModels.verifiedModel || activeForModels.requestedModel || activeForModels.model || defaultModel;
    }
    sendTo(ws, {
      type: "model_info",
      model: modelToSend,

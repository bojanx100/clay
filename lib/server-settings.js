...
function attachSettings(ctx) {
  ...
  function handleRequest(req, res, fullUrl) {
    ...
    // GET /api/user/workspace-group-states
    if (req.method === "GET" && fullUrl === "/api/user/workspace-group-states") {
      var isMultiUserWGS = users.isMultiUser();
      var muWGS = getMultiUserFromReq(req);
      var groupsWGS = {};
      if (!isMultiUserWGS) {
        if (typeof opts.onGetWorkspaceGroupStates === "function") groupsWGS = opts.onGetWorkspaceGroupStates() || {};
      } else {
        if (!muWGS) {
          res.writeHead(401, { "Content-Type": "application/json" });
          res.end('{"error":"unauthorized"}');
        ...
        groupsWGS = users.getWorkspaceGroupStates(muWGS.id) || {};
      }
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ groups: groupsWGS }));
      return true;
    }

    // PUT /api/user/workspace-group-states
    if (req.method === "PUT" && fullUrl === "/api/user/workspace-group-states") {
      var isMultiUserWGSWrite = users.isMultiUser();
      var muWGSWrite = getMultiUserFromReq(req);
      var bodyWGS = "";
      req.on("data", function (chunk) { bodyWGS += chunk; });
      req.on("end", function () {
        try {
          var dataWGS = JSON.parse(bodyWGS || "{}");
          var resultWGS;
          if (!isMultiUserWGSWrite) {
            if (typeof opts.onSetWorkspaceGroupStates !== "function") {
              res.writeHead(500, { "Content-Type": "application/json" });
              res.end('{"error":"Not supported"}');
              return;
            }
            resultWGS = opts.onSetWorkspaceGroupStates(dataWGS.groups);
          } else {
            if (!muWGSWrite) {
              res.writeHead(401, { "Content-Type": "application/json" });
              res.end('{"error":"unauthorized"}');
            ...
            resultWGS = users.setWorkspaceGroupStates(muWGSWrite.id, dataWGS.groups);
          }
          if (resultWGS && resultWGS.error) {
            res.writeHead(400, { "Content-Type": "application/json" });
            res.end(JSON.stringify({ error: resultWGS.error }));
            return;
          }
          res.writeHead(200, { "Content-Type": "application/json" });
          res.end(JSON.stringify(resultWGS));
        } catch (e) {
          res.writeHead(400, { "Content-Type": "application/json" });
          res.end('{"error":"Invalid request"}');
        }

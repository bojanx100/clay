...
function attachSessionAdoption(ctx) {
  ...
  function listCoordinators(source) {
    ...
    sm.sessions.forEach(function (session) {
      if (!session || session.hidden || session === source || session.orchestrationParent ||
          directLeafBinding(session) || readOnlyExecution.isReadOnly(session, sm)) return;
      candidates.push({
        id: session.localId,
  ...
  function propose(source, coordinator, options) {
    if (readOnlyExecution.isReadOnly(coordinator, sm)) return false;
  ...
  function adoptFromTool(input) {
    ...
    if (readOnlyExecution.isReadOnly(coordinator, sm)) {
      return ctx.error("Read-only evidence executions cannot take over existing sessions. " +
        "Request a separately admitted task from the owning coordinator.");
    }

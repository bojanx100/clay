...
function backgroundSummary(session) {
  var tasks = session && Array.isArray(session.orchestrationTasks) ? session.orchestrationTasks : [];
  var count = 0;
  var activity = "";
  for (var i = 0; i < tasks.length; i++) {
    var task = tasks[i] || {};
    if (!ACTIVE_TASK_STATUSES[task.status]) continue;
    count++;
    if (!activity) activity = task.currentActivity || task.title || task.objective || "";
  }
  return { count: count, activity: String(activity || "").trim() };
}
...
function clientState(session) {
  ...
  var background = backgroundSummary(session);
  return {
    ...
    backgroundTaskCount: background.count,

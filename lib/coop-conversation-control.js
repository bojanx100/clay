...
var ownerResponseResolution = require("./coop-owner-response-resolution");
...
function foregroundText(reservation, text) {
  ...
  return [
    ...
    "This is an owner-facing Coop foreground turn. Own the discussion: reason about priorities, plans, tradeoffs, and business questions, and answer the owner's actual request.",
    "Delegate substantial project execution and independent evidence gathering to project-bound coordinators/helpers. Keep the owner informed while they work, and synthesize their results here. A brief acknowledgement alone is not an answer.",
...
function markIngressAnswered(session, ledger) {
  ...
  try {
    ...
    // The index we just computed is correct right now and wrong after the next
    // reload, because transcript coalescing rebases session.history under it.
    // Stamp the answering event's durable anchor alongside it while the history
    // that produced the index is still in hand. No anchor is not a failure --
    // the record is written exactly as before, just without one.
    return !!ledger.markAnswered(ingressId, {
      eventIndex: event.eventIndex,
      messageUuid: ownerResponseResolution.anchorForDone(session.history, event.eventIndex),
    });
...
function attachCoopConversationControl(ctx) {
  ...
  function markIdle(session) {
    if (!isCoopConversation(session)) return;
    var state = stateFor(session);
    var drainedIngressId = state.activeIngressId;
    state.activeIngressId = null;
    state.activeResponseStartIndex = null;
    if (sm && typeof sm.saveSessionFile === "function") sm.saveSessionFile(session);
    publish(session);
    if (onIngressDrained && drainedIngressId && !session.isProcessing &&
        (!session.pendingCoopIngress || session.pendingCoopIngress.length === 0)) {
      onIngressDrained(session, drainedIngressId);
    }
  }

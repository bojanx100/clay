...
// answering. A turn that errored out (`done` with a non-zero code) did not
// answer the owner, and neither did one still mid-flight.
function answeringEvent(session) {
  var history = session && Array.isArray(session.history) ? session.history : [];
  for (var i = history.length - 1; i >= 0; i--) {
    var item = history[i];
    if (!item || item.type !== "done") continue;
    return { eventIndex: i, answered: !item.code };
  }
  return null;
}
...
function markIngressAnswered(session, ledger) {
  ...
  if (session.coopPriorityInterruptRequested) return false;
  ...
  var store = ledger || ownerRequests.getDefaultOwnerRequests();
  ...
  try {
    ...
    return !!store.markAnswered(ingressId, { eventIndex: event.eventIndex });

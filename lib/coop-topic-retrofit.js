...
var TITLE_RETROFIT_SCHEMA_VERSION = 3;
...
function turnText(history, ref) {
  ...
  var sessionId = ref && ref.sessionStorageId || "";
  var segment = Array.isArray(history) ? { length: history.length } : lineage.segmentFor(history, sessionId);
  var index = segment ? topicAnchors.candidateIndex(ref, verdict.offset, segment.length) : null;
  var record = index === null ? null : (lineage.recordAt(history, sessionId, index) || {}).record;
  return record ? String(record.text || "") : "";
}

function provenTexts(topic, history) {
...
// Moves a fragment's own membership into `target` and marks the fragment
// merged, mirroring the existing manual merge() convention (status: "merged",
// mergedInto) so downstream projection code has exactly one meaning for those
// fields regardless of which path produced them.
//
// Always was target-agnostic; exported under this name so the retro
// consolidation pass (coop-topic-consolidation.js) folds a fragment into a
// conversation through exactly this code path, rather than growing a second
// merge implementation that could drift on what "merged" means.
function mergeMembershipInto(topic, target, now) {
...
function retrofitTitles(index, options) {
  ...
  for (var i = 0; i < ids.length; i++) {
    ...
    // A topic keeps its own row only if some proven owner turn names a
    // concrete subject. Scanning every proven turn in order lets a vague
    // opener ("You know what, scratch 1") borrow its subject from the rest of
    // the same turn or a later anchored turn; when NO defensible subject
    // exists anywhere, a vague owner utterance is not a topic name and the
    // fragment folds into the catch-all instead.
    var texts = provenTexts(topic, history);
    var combined = texts.join(" ");
    var subject = "";
    for (var tx = 0; tx < texts.length && !subject; tx++) {
      subject = classification.diagnosticTitle(texts[tx]);
    }
    var canRouteToUncategorised = uncategorised && uncategorised.status === "open" && uncategorised.topicRef.topicId !== id;
    if ((classification.lowInformation(combined) || !subject) && canRouteToUncategorised) {
      var beforeTitle = topic.title;
      var moved = mergeMembershipInto(topic, uncategorised, now);
    ...
    var metadata = classification.derivedMetadata(combined);
    var newTitle = subject || metadata.title;
    newTitle = newTitle ? uniqueTitle(newTitle, id) : "";
...
module.exports = {
  ...
  mergeMembershipInto: mergeMembershipInto,
};

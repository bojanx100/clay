function attachPreferences(deps) {
  ...
  // --- Lead mode (CTO orchestrator, roadmap §1.1) ---
  //
  // Per-user opt-in flag for the Lead: with it OFF (the default), Clay
  // behaves exactly as today — the additive-only rule means no core path
  // may ever depend on this being on. Flipping it off mid-flight is the
  // kill switch: delegated worker sessions are ordinary sessions and keep
  // working; only the Lead's orchestration duties stop.
  function getLeadMode(userId) {
    var data = loadUsers();
    for (var i = 0; i < data.users.length; i++) {
  ...
  function setLeadMode(userId, enabled) {
    var data = loadUsers();
    for (var i = 0; i < data.users.length; i++) {
      if (data.users[i].id === userId) {
        data.users[i].leadMode = enabled === true;
        data.users[i].leadModeSetAt = Date.now();
        saveUsers(data);
        return { ok: true, leadMode: data.users[i].leadMode };
      }
    }
    return { error: "User not found" };
  }

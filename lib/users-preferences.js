function attachPreferences(deps) {
  ...
  // --- Per-user, per-project last vendor ---

  function getProjectLastVendor(userId, slug) {
    if (typeof slug !== "string" || !slug) return null;
    var data = loadUsers();
    for (var i = 0; i < data.users.length; i++) {
      if (data.users[i].id === userId) {
        var choices = data.users[i].projectLastVendors || {};
        return typeof choices[slug] === "string" ? choices[slug] : null;
      }
    }
    return null;
  }

  function setProjectLastVendor(userId, slug, vendor) {
    if (typeof slug !== "string" || !slug) return { error: "Project slug is required" };
    if (vendor && typeof vendor !== "string") return { error: "Vendor must be a string" };
    var data = loadUsers();
    for (var i = 0; i < data.users.length; i++) {
      if (data.users[i].id === userId) {
        var choices = Object.assign({}, data.users[i].projectLastVendors || {});
        if (vendor) choices[slug] = vendor;
        else delete choices[slug];
        data.users[i].projectLastVendors = choices;
        saveUsers(data);
        return { ok: true, vendor: vendor || null };
      }
    }
    return { error: "User not found" };
  }
  ...
  // --- Legacy Lead mode read ---
  //
  // Lead mode moved to daemon config. Keep this narrow read available only
  // while the config migration needs to discover an owner's former opt-in.
  function getLegacyLeadMode(userId) {
    var data = loadUsers();
    for (var i = 0; i < data.users.length; i++) {
      if (data.users[i].id === userId) return data.users[i].leadMode === true;
    }
    return false;
  }
  ...
  // --- Workspace group disclosure preferences ---
  // Only collapsed keys are stored. Missing keys therefore stay expanded,
  // including groups introduced by a later version of the Workspace panel.

  function normalizeWorkspaceGroupStates(states) {
    var safe = {};
    if (!states || typeof states !== "object" || Array.isArray(states)) return safe;
    var keys = Object.keys(states);
    for (var i = 0; i < keys.length; i++) {
      if (typeof keys[i] === "string" && keys[i].length > 0 && keys[i].length <= 160 && states[keys[i]] === true) {
        safe[keys[i]] = true;
      }
    }
    return safe;
  ...
  function getWorkspaceGroupStates(userId) {
    var data = loadUsers();
    for (var i = 0; i < data.users.length; i++) {
      if (data.users[i].id === userId) return normalizeWorkspaceGroupStates(data.users[i].workspaceGroupStates);
    }
    return {};
  }

  function setWorkspaceGroupStates(userId, states) {
    var safe = normalizeWorkspaceGroupStates(states);
    var data = loadUsers();
    for (var i = 0; i < data.users.length; i++) {
      if (data.users[i].id === userId) {
        data.users[i].workspaceGroupStates = safe;
        saveUsers(data);
  ...
  return {
    ...
    getProjectLastVendor: getProjectLastVendor,
    setProjectLastVendor: setProjectLastVendor,
    defaultClaudeOpenMode: defaultClaudeOpenMode,
    getClaudeOpenMode: getClaudeOpenMode,
    setClaudeOpenMode: setClaudeOpenMode,
    getLegacyLeadMode: getLegacyLeadMode,
    ...
    getWorkspaceGroupStates: getWorkspaceGroupStates,
    setWorkspaceGroupStates: setWorkspaceGroupStates,

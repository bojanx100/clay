...
function createDurableDelivery(opts) {
  ...
  function reclaimCursor(envelope, destinationOnly) {
    if (!destinationOnly) {
      var sequenceKeys = Object.keys(state.sequences);
      for (var i = 0; i < sequenceKeys.length; i++) {
        var sequenceKey = sequenceKeys[i];
        if (hasPendingSource(sequenceKey)) continue;
        var hasStream = false;
        var inboxKeys = Object.keys(state.inbox);
        for (var j = 0; j < inboxKeys.length; j++) {
          var inbox = state.inbox[inboxKeys[j]];
          if (inbox && inbox.streams && inbox.streams[sequenceKey]) {
            hasStream = true;
            break;
          }
        }
        if (!hasStream) {
          delete state.sequences[sequenceKey];
          return true;
        }
      }
    }
  ...
  function deliverEnvelope(input) {
    ...
    return attempt(input.eventId, false, !newlyQueued);
  }

...
function findTurnBoundary(history, targetIndex) {
  ...
  for (var i = targetIndex; i >= floorIndex; i--) {
    if (isUserTurnBoundary(history[i])) return i;
  }
  return targetIndex;
}

function isUserTurnBoundary(item) {
  return item && item.type === "user_message" &&
    !item.queuedDuringProcessing && !item.queuedPending;
}
...
function indexedTurnBoundary(history, indexes, targetOffset) {
  ...
  for (var i = targetOffset; i >= floorOffset; i--) {
    ...
    if (isUserTurnBoundary(item)) return i;
  }
  return targetOffset;
}

function replayTimestamp(item) {
  return item && typeof item._ts === "number" && Number.isFinite(item._ts) ? item._ts : null;
}

function orderQueuedHistoryItems(items, itemOf) {
  var source = Array.isArray(items) ? items : [];
  var readItem = typeof itemOf === "function" ? itemOf : function(item) { return item; };
  var queued = [];
  var ordered = [];
  for (var i = 0; i < source.length; i++) {
    var item = readItem(source[i]);
    if (item && item.type === "user_message" && item.queuedDuringProcessing) {
      queued.push({ value: source[i], item: item, sourceIndex: i });
    } else {
      ordered.push(source[i]);
    }
  }
  queued.sort(function(a, b) {
    var aTimestamp = replayTimestamp(a.item);
    var bTimestamp = replayTimestamp(b.item);
  ...
  for (var qi = 0; qi < queued.length; qi++) {
    var queuedTimestamp = replayTimestamp(queued[qi].item);
    var insertAt = ordered.length;
    if (queuedTimestamp != null) {
      for (var oi = 0; oi < ordered.length; oi++) {
        var orderedTimestamp = replayTimestamp(readItem(ordered[oi]));
        if (orderedTimestamp != null && orderedTimestamp > queuedTimestamp) {
          insertAt = oi;
          break;
        }
      }
    }
    ordered.splice(insertAt, 0, queued[qi].value);
  }
  return ordered;
}
...
function attachSessionHistory(ctx) {
  ...
  function replayHistory(session, fromIndex, targetWs, transform, replayOptions) {
    ...
    replayEntries = orderQueuedHistoryItems(replayEntries, function(entry) { return entry.item; });

    for (var ei = 0; ei < replayEntries.length; ei++) {
      var replayEntry = replayEntries[ei];
      var replayItem = replayEntry.item;
      if (replayItem && (replayItem.type === "mention_user" || replayItem.type === "mention_response")) {
        console.log("[DEBUG replayHistory] sending mention at index=" + replayEntry.canonicalIndex + " from=" + replayFrom + " total=" + total + " type=" + replayItem.type + " mate=" + (replayItem.mateName || ""));
      }
      sendReplayEntry(replayEntry);
    }
    ...
    _send({
      type: "history_done",
      // Human lenses already have the full canonical view. All needs no
      // conversation filter; keep its bounded tail replay lazy.
      coopConversationState: session.coopHome && indexed
        ? require("./coop-main-replay").liveState(session, historyView) : null,
      lastUsage: lastUsage,
...
module.exports = {
  ...
  isReplayableHistoryItem: isReplayableHistoryItem,
  orderQueuedHistoryItems: orderQueuedHistoryItems,

...
function attachSessionHistory(ctx) {
  ...
  function replayHistory(session, fromIndex, targetWs, transform, replayOptions) {
    ...
    // actually needed -- compacted sessions, and topic/indexed replay, which
    // options.eventIndexes already excludes above.
    var simplePagedReplay = !options.historyView && !Array.isArray(options.eventIndexes) &&
      !Number.isInteger(options.focusEventIndex) && !Number.isInteger(fromIndex) &&
      !session.compactedFromStorageId &&
      historyStore.isLazy(session) && !historyStore.isResident(session) &&
      Number.isInteger(session._persistedHistoryLength) &&
      typeof session._readPersistedHistoryRange === "function";
    var persistedTotal = simplePagedReplay ? session._persistedHistoryLength : 0;
    var historyOffset = simplePagedReplay
      ? Math.max(0, persistedTotal - (HISTORY_PAGE_SIZE * 2)) : 0;
    var pagedHistory = simplePagedReplay
      ? session._readPersistedHistoryRange(historyOffset, persistedTotal) : null;
    if (!Array.isArray(pagedHistory)) {
      simplePagedReplay = false;
      historyOffset = 0;
    }

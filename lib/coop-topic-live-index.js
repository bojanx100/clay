...
function refreshGlobalCoopProjection(options) {
  var opts = options || {};
  var session = opts.session;
  if (!session || !session.coopHome) {
  ...
  if (typeof opts.forEachClient !== "function" || typeof opts.projectionFor !== "function" ||
      typeof opts.sendTo !== "function") return { ok: true, sent: sent };
...
function refreshCanonicalCoopTopics(options) {
  ...
  var refreshed = refreshGlobalCoopProjection(opts);
  return { ok: true, changed: true, sent: refreshed.sent };
}

module.exports = {
  refreshGlobalCoopProjection: refreshGlobalCoopProjection,

...
function reconcileTopicDisposition(seam, ref, decision) {
  ...
  var targetCloseOutcome = null;
  if (targetStatus === "closed") {
    targetCloseOutcome = decision && decision.closeOutcome || result.topic.closeOutcome ||
      threadLifecycle.CLOSE_OUTCOMES.IMPLEMENTED_RESOLVED;
    if (!threadLifecycle.validCloseOutcome(targetCloseOutcome)) {
      return { ok: false, code: "invalid_close_outcome" };
    }
  }
  var legacyEffect = {
    topicId: topicId,
  ...
  var legacyEffectFingerprint = crypto.createHash("sha256")
    .update(JSON.stringify(legacyEffect)).digest("hex");
  var effect = Object.assign({}, legacyEffect, { closeOutcome: targetCloseOutcome });
  var effectFingerprint = crypto.createHash("sha256")
    .update(JSON.stringify(effect)).digest("hex");
  var seen = recordedRequest(index, requestId, topicId);
  if (seen && seen.conflict) return { ok: false, code: "request_conflict" };
  if (seen) {
    var legacyReplay = decision && decision.closeOutcome == null &&
      seen.entry.reconciliationFingerprint === legacyEffectFingerprint;
    if (seen.entry.reconciliationFingerprint &&
        seen.entry.reconciliationFingerprint !== effectFingerprint && !legacyReplay) {
  ...
  var applied = topicDisposition.applyOwnerDecision(result.topic, decision, { now: seam.now });
  if (!applied.ok) return applied;
  // Reconciling status to "closed" has to retire the thread, not just stamp the
  // status field. Assigning result.topic.status directly left threadState at
  // its live value, and the owner's Threads rail filters on threadState alone,
  // so every topic closed through this path kept rendering as a live Thread --
  // and became ineligible for the bulk-closure sweep, which only selects
  // status === "open". The lifecycle layer owns the whole tuple.
  //
  // closeOutcome is not inferable from a status field, so the caller may name
  // it. An already-closed record keeps the outcome it has: a closed -> closed
  // reconciliation that only revises a note must not silently re-label the
  // close, and re-labelling not_pursuing as implemented_resolved would also
  // clear hidden and resurrect a row the owner had suppressed. Only a genuinely
  // unclassified close falls back to the default the single-topic owner close
  // uses (implemented_resolved), which keeps the row visible under Done.
  var lifecycle = threadLifecycle.applyRecordStatus(result.topic, targetStatus, {
    closeOutcome: targetCloseOutcome,
    now: seam.now,
  });

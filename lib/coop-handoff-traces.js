...
function boundedRecords(records, maxCases) {
  ...
  var retained = records.slice().sort(function (left, right) {
    return caseAge(right) - caseAge(left);
  }).slice(0, maxCases);
  var retainedKeys = {};
  for (var i = 0; i < retained.length; i++) retainedKeys[liveObserverKey(retained[i].ownerId, retained[i].id)] = true;
  for (var j = 0; j < records.length; j++) {
    var recordKey = liveObserverKey(records[j].ownerId, records[j].id);
    if (!retainedKeys[recordKey]) liveIntentObservers.delete(recordKey);
  }
  return retained;
}
...
function createStore(options) {
  ...
  function recordResolution(input, status, action) {
    ...
    var result = mutate(function (records, at) {
      ...
      if (action) {
        var observedTurns = assistantTurns(record);
        var middlemanTurns = action === "clickable_session_ref" && observedTurns !== null
          ? Math.max(0, observedTurns - 1) : observedTurns;
        if (middlemanTurns === null || !record.expectedTarget) status = "unmeasurable";
        record.trace.events = assistantEvents(middlemanTurns || 0).concat([navigationEvent(action, target)]);
      }

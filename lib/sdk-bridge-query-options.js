...
function applyThinking(sm, loopSettings, claudeOpts) {
  ...
  if (mode === "disabled") claudeOpts.thinking = { type: "disabled" };
...
function sendCopilotModelInfo(ctx, session, model, vendor) {
  ...
  ctx.sendToSession(session, { type: "model_info", model: model,
    ...
    providerRoutes: ctx.sm.providerRoutes || listProviderRoutes(available, installed) });
}
...
function buildQueryOptions(ctx, session, text, linuxUser, fence, prepared) {
  ...
  var merged = ctx.mergeMcpServers(ctx.getMcpServers(), ctx.getRemoteMcpServers) || undefined;
  ...
  var options = Object.assign({ cwd: ctx.cwd, linuxUser: linuxUser || undefined,
    model: prepared.model, effort: prepared.loopSettings.effort || ctx.sm.currentEffort || undefined,

...
var readOnlyQuery = require("./yoke/read-only-query");
...
function resetTurn(session, linuxUser) {
  ...
  Object.defineProperty(session, "_sessionControlToolQuery", { configurable: true, writable: true,
    enumerable: false, value: { signal: !linuxUser && session.abortController.signal || null } });
}
...
function callbackOptions(ctx, session, fence, mergedMcpServers) {
  return {
    canUseTool: function (toolName, input, opts) {
      ...
      var restricted = readOnlyExecution.toolDecision(session, ctx.sm, toolName, input);
      if (restricted) return Promise.resolve(restricted);
    ...
    onElicitation: function (request, opts) {
      ...
      if (readOnlyExecution.isReadOnly(session, ctx.sm)) return Promise.resolve({ action: "decline" });
    ...
    callMcpTool: function (server, tool, args) {
      executionFence.assertAction(session, "tool", fence);
      if (readOnlyExecution.isReadOnly(session, ctx.sm)) {
        throw new Error(readOnlyExecution.denial().message);
      }
...
function buildQueryOptions(ctx, session, text, linuxUser, fence, prepared) {
  ...
  var controlTurn = prepareControlTurn(ctx, session, text);
  if (controlTurn.systemPrompt) {
    options.systemPrompt = [controlTurn.systemPrompt, options.systemPrompt].filter(Boolean).join("\n\n");
  }
  if (readOnlyExecution.isReadOnly(session, ctx.sm)) {
    readOnlyQuery.apply(options, session.vendor || ctx.adapter && ctx.adapter.vendor);
  }
  require("./turn-performance").configure(session, options, ctx.adapter && ctx.adapter.vendor);
  return { options: options, initialTitle: title, text: controlTurn.text, controlInput: controlTurn.context ? text : undefined };
}

async function prepareQuery(ctx, session, text, linuxUser, fence) {
  readOnlyExecution.retain(session, ctx.sm);

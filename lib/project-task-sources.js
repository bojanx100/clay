...
// means everyone matches" convenience: automatic pickup requires assignment to
// be PROVEN, so an unconfigured filter, an unresolvable login, or an empty
// assignee list all answer false rather than waving the item through.
function issueAssignedToOwner(recipe, args, issue, currentLogin) {
  ...
  // ownership, so it cannot make an item automatically eligible.
  if (!assigned || assigned === "any") return false;
  var login = assigned === "me" ? (currentLogin || "") : assigned;

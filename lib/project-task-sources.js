...
function collectFeedback(cwd, repo, env, currentLogin, number) {
  ...
  // AI PR QA verdict — a top-level conversation comment (not a review), flagged
  // by a stable HTML marker. Surface it separately from reviewer feedback so the
  // fix session gets dedicated PASS/FAIL handling. Its timestamp counts toward
  // latestTs so a fresh verdict triggers a pass (bounded by watermark + budget).
  var qa = null;
  try {
    var qowner = repo.split("/")[0];
    var qname = repo.split("/")[1];
    var rawQa = execGhRaw(cwd, [
      "api", "repos/" + qowner + "/" + qname + "/issues/" + number + "/comments",
      "--paginate", "-q", ".",
    ], env);
    qa = prQaVerdict.extractQaFromComments(JSON.parse(rawQa), toMs);
  } catch (e) { /* QA verdict is best-effort */ }

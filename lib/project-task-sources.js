...
// Bound the extra read work per scan. Once the bound is reached the source
// fails closed with a typed reason rather than returning a mix of qualified and
// status-only issue records that the gate cannot safely distinguish.
...
function reportBoardEvidenceFailure(repo, number, reason) {
  console.error("[task-source] excluded " + repo + "#" + number +
    " (" + (reason || "qualification_board_evidence_unavailable") + ")");
}

function exactBoardEvidence(cwd, env, repo, number, configuredBoard) {
  return boardEvidence.collectBoardItemEvidenceSync(function (command, commandArgs) {
    if (command !== "gh") throw new Error("unsupported board evidence command");
    return execGhRaw(cwd, commandArgs, env);
  }, repo, number, configuredBoard);
}

function qualificationBoardFromArgs(args) {
  return args && args.qualificationBoard || null;
}
...
function githubIssues(cwd, recipe, args) {
  ...
  var needsExactBoardEvidence = issueNeedsProjectItems(recipe, args);
  var configuredBoard = qualificationBoardFromArgs(args);
  if (needsExactBoardEvidence) fields += ",projectItems";
  if (args.issue) {
  ...
  for (var i = 0; i < issues.length; i++) {
    ...
    if (needsExactBoardEvidence) {
      if (evidenceLookups >= MAX_QUALIFICATION_EVIDENCE_ISSUES) {
        reportBoardEvidenceFailure(repo, issues[i].number, "qualification_board_evidence_rate_limited");
        continue;
      }
      evidenceLookups++;
      var evidence = exactBoardEvidence(cwd, env, repo, issues[i].number, configuredBoard);
      if (!evidence.ok) {
        reportBoardEvidenceFailure(repo, issues[i].number, evidence.reason);
        continue;
      }
      // Reapply all source filters to the current, authoritative board state.
      // The CLI list can be stale and its projectItems lack the node identity
      // that qualification receipts require, so it is never safe to carry it
      // beyond this point once exact evidence was requested.
      issues[i] = Object.assign({}, issues[i], { projectItems: evidence.projectItems });
      if (!issueMatches(recipe, args, issues[i], currentLogin)) continue;
    }

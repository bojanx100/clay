...
// Bumped whenever history is REASSIGNED, which is the only way the head of a
// transcript can change (compaction and recovery rewrites replace the array).
// release() clears the backing field directly and deliberately does not bump:
// dropping a resident copy does not change what the next reload produces.
// Callers memoizing a value derived from the transcript head key it on this.
var GENERATION = Symbol("clayHistoryGeneration");
...
function defineLazyHistory(session, history, load) {
  ...
  Object.defineProperty(session, "history", {
    ...
    set: function (value) {
      session[UNAVAILABLE] = false;
      session[GENERATION] = generation(session) + 1;
...
// Identity of the current transcript contents. See GENERATION above.

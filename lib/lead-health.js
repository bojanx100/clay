...
// Session metadata is the first JSONL record and can outgrow the normal head.
var SESSION_FIRST_LINE_MAX_BYTES = 8 * 1024 * 1024;
...
function readBoundedSession(file, size) {
  ...
  try {
    ...
    var head = readSessionHeadThroughFirstLine(fd, size);
...
function readSessionHeadThroughFirstLine(fd, size) {
  var limit = Math.min(size, SESSION_FIRST_LINE_MAX_BYTES);
  var minBytes = Math.min(size, SESSION_HEAD_BYTES);
  var chunks = [];
  var offset = 0;
  while (offset < limit) {
    var chunkSize = Math.min(SESSION_HEAD_BYTES, limit - offset);
    var chunk = Buffer.alloc(chunkSize);
    var bytesRead = fs.readSync(fd, chunk, 0, chunk.length, offset);
    if (bytesRead <= 0) break;
    var slice = bytesRead === chunk.length ? chunk : chunk.subarray(0, bytesRead);
    chunks.push(slice);
    offset += bytesRead;
    if (offset >= minBytes && slice.indexOf(10) !== -1) break;
  }

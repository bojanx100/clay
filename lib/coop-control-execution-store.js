// Restricted SQLite operations for Slice 2. The generic ControlStore owns the
// connection and injects this API; callers never receive the raw database.

var crypto = require("crypto");
var rehydration = require("./coop-control-rehydration");
var staleR6Reconciliation = require("./coop-control-stale-r6-reconciliation-store");
var validation = require("./coop-control-store-validation");

function error(code, message) {
  return validation.taggedError(code, message);
}

function executionMatches(execution, authority, ref) {
  return !!execution && !!authority && execution.current_epoch === ref.epoch &&
    execution.authority_id === ref.authorityId && authority.revoked_at === null;
}

function incarnationMatches(incarnation, ref) {
  return !!incarnation && incarnation.incarnation_id === ref.incarnationId &&
    incarnation.capability_digest === ref.capabilityDigest;
}

function leaseMatches(lease, ref) {
  return !!lease && lease.incarnation_id === ref.incarnationId && lease.epoch === ref.epoch &&
    lease.authority_id === ref.authorityId;
}

function digest(value) {
  return crypto.createHash("sha256").update(String(value), "utf8").digest("hex");
}

function createExecutionStoreApi(db, options) {
  var opts = options || {};
  var now = typeof opts.now === "function" ? opts.now : Date.now;
  var faults = opts.faults || {};
  var assertOpen = opts.assertOpen || function () {};

  function timestamp() {
    var value = now();
    if (!Number.isSafeInteger(value) || value < 0) {
      throw error("COOP_CONTROL_EXECUTION_INVALID", "Execution timestamps must be non-negative safe integers.");
    }
    return value;
  }

  function transaction(operation, work) {
    assertOpen();
    try {
      db.exec("BEGIN IMMEDIATE");
      var result = work();
      if (typeof faults.beforeExecutionCommit === "function") {
        faults.beforeExecutionCommit({ operation: operation });
      }
      db.exec("COMMIT");
      return result;
    } catch (cause) {
      try { db.exec("ROLLBACK"); } catch (rollbackError) {}
      throw cause;
    }
  }

  function authorityRow(authorityId) {
    return db.prepare("SELECT * FROM coop_control_authorities WHERE authority_id = ?").get(authorityId);
  }

  function executionRow(executionId) {
    return db.prepare("SELECT * FROM coop_control_executions WHERE execution_id = ?").get(executionId);
  }

  function bindingExecution(spec) {
    return db.prepare("SELECT * FROM coop_control_executions WHERE portfolio_task_id = ? " +
      "AND binding_revision = ? AND target_project_id = ?")
      .get(spec.portfolioTaskId, spec.bindingRevision, spec.targetProjectId);
  }

  function currentIncarnation(executionId, epoch) {
    return db.prepare("SELECT * FROM coop_control_incarnations WHERE execution_id = ? AND epoch = ?")
      .get(executionId, epoch);
  }

  function currentLease(executionId, role) {
    return db.prepare("SELECT * FROM coop_control_role_leases WHERE execution_id = ? AND role = ?")
      .get(executionId, role);
  }

  function activeExecutionForTarget(sessionRef) {
    var rows = db.prepare("SELECT e.execution_id, e.current_epoch, e.authority_id, i.incarnation_id, " +
      "i.capability_digest, i.start_state FROM coop_control_executions e " +
      "JOIN coop_control_incarnations i ON i.execution_id = e.execution_id AND i.epoch = e.current_epoch " +
      "WHERE e.status IN ('pending', 'running') AND i.session_project_id = ? AND i.session_storage_id = ? " +
      "ORDER BY e.execution_id").all(sessionRef.projectId, sessionRef.sessionStorageId);
    if (rows.length > 1) {
      throw error("COOP_CONTROL_STORE_LOGICAL_CORRUPTION",
        "A recovery SessionRef resolves to more than one active execution.");
    }
    if (!rows.length) return null;
    var lease = db.prepare("SELECT * FROM coop_control_role_leases WHERE execution_id = ?")
      .get(rows[0].execution_id);
    if (!lease || lease.incarnation_id !== rows[0].incarnation_id ||
        Number(lease.epoch) !== Number(rows[0].current_epoch) ||
        lease.authority_id !== rows[0].authority_id) {
      throw error("COOP_CONTROL_STORE_LOGICAL_CORRUPTION",
        "A recovery target has no matching current execution lease.");
    }
    return Object.assign({}, rows[0], { role: lease.role });
  }

  function findRecoveryTargets(sessionRefs) {
    assertOpen();
    var refs = Array.isArray(sessionRefs) ? sessionRefs : [];
    var found = [];
    var seen = Object.create(null);
    for (var i = 0; i < refs.length; i++) {
      var current = activeExecutionForTarget(refs[i]);
      if (!current || seen[current.execution_id]) continue;
      seen[current.execution_id] = true;
      found.push({ execution_id: current.execution_id, session_project_id: refs[i].projectId,
        session_storage_id: refs[i].sessionStorageId });
    }
    return found;
  }

  function listIncompleteExecutions() {
    assertOpen();
    return db.prepare("SELECT e.execution_id, e.portfolio_task_id, e.binding_revision, " +
      "e.target_project_id, e.mode, e.authority_id, e.current_epoch, e.status, " +
      "i.incarnation_id, i.session_project_id, i.session_storage_id, i.start_state, " +
      "l.role, l.authority_id AS lease_authority_id, l.incarnation_id AS lease_incarnation_id, " +
      "l.epoch AS lease_epoch FROM coop_control_executions e " +
      "JOIN coop_control_incarnations i ON i.execution_id = e.execution_id " +
      "AND i.epoch = e.current_epoch " +
      "JOIN coop_control_role_leases l ON l.execution_id = e.execution_id " +
      "WHERE e.status IN ('pending', 'running') ORDER BY e.execution_id").all();
  }

  function packetRecord(packet, field, value) {
    var rows = packet && Array.isArray(packet[field]) ? packet[field] : [];
    var key = field === "authorities" ? "authorityId" : "executionId";
    for (var i = 0; i < rows.length; i++) {
      if (rows[i][key] === value) return rows[i];
    }
    return null;
  }

  function assertRestartPacket(spec, execution, authority, incarnation, lease) {
    if (!spec || !spec.control || !spec.sessionRef ||
        !validation.DIGEST_RE.test(spec.packetDigest || "") ||
        typeof spec.packetJson !== "string") {
      throw error("COOP_CONTROL_RESTART_CHECKPOINT_INVALID",
        "Graceful restart requires one canonical continuity checkpoint.");
    }
    var ref = spec.control;
    if (!executionMatches(execution, authority, ref) || !incarnation ||
        incarnation.incarnation_id !== ref.incarnationId ||
        !leaseMatches(lease, ref) || incarnation.session_project_id !== spec.sessionRef.projectId ||
        incarnation.session_storage_id !== spec.sessionRef.sessionStorageId ||
        (execution.status !== "pending" && execution.status !== "running")) {
      throw error("COOP_CONTROL_RESTART_CHECKPOINT_INVALID",
        "Graceful restart checkpoint identity is not the current active execution lease.");
    }
    var examined = rehydration.examineStoredCheckpoint(spec.packetJson, spec.packetDigest);
    var packetExecution = packetRecord(examined.packet, "executions", execution.execution_id);
    var packetAuthority = packetRecord(examined.packet, "authorities", authority.authority_id);
    if (!packetExecution || !packetAuthority ||
        packetExecution.authorityId !== authority.authority_id ||
        packetExecution.portfolioTaskId !== execution.portfolio_task_id ||
        Number(packetExecution.bindingRevision) !== Number(execution.binding_revision) ||
        packetExecution.targetProject.projectId !== execution.target_project_id ||
        packetExecution.mode !== execution.mode || packetExecution.role !== authority.role ||
        packetAuthority.portfolioTaskId !== authority.portfolio_task_id ||
        Number(packetAuthority.bindingRevision) !== Number(authority.binding_revision) ||
        packetAuthority.targetProject.projectId !== authority.target_project_id ||
        packetAuthority.role !== authority.role || Number(packetAuthority.actionMask) !== 31) {
      throw error("COOP_CONTROL_RESTART_CHECKPOINT_INVALID",
        "Graceful restart continuity does not match durable execution authority.");
    }
    return examined;
  }

  function prepareRestartHandoff(spec) {
    return transaction("prepare_restart_handoff", function () {
      var ref = spec && spec.control || {};
      var execution = executionRow(ref.executionId);
      var authority = execution && authorityRow(execution.authority_id);
      var incarnation = execution && currentIncarnation(execution.execution_id,
        Number(execution.current_epoch));
      var lease = execution && currentLease(execution.execution_id, ref.role);
      assertRestartPacket(spec, execution, authority, incarnation, lease);
      var identitySeed = [execution.execution_id, execution.current_epoch,
        spec.sessionRef.projectId, spec.sessionRef.sessionStorageId, spec.packetDigest].join("\u0000");
      var handoffId = "handoff:restart:" + digest(identitySeed).slice(0, 48);
      var checkpointId = "checkpoint:restart:" + digest(handoffId + "\u0000" + spec.packetDigest).slice(0, 48);
      var existing = db.prepare("SELECT * FROM coop_control_handoffs WHERE execution_id = ? " +
        "AND state IN ('prepared', 'cutover', 'replaying') ORDER BY created_at, handoff_id")
        .all(execution.execution_id);
      if (existing.length) {
        var checkpoint = db.prepare("SELECT * FROM coop_control_checkpoints WHERE handoff_id = ?")
          .get(existing[0].handoff_id);
        if (existing.length !== 1 || existing[0].handoff_id !== handoffId ||
            existing[0].state !== "prepared" || !checkpoint ||
            checkpoint.checkpoint_id !== checkpointId || checkpoint.packet_json !== spec.packetJson ||
            checkpoint.packet_digest !== spec.packetDigest) {
          throw error("COOP_CONTROL_HANDOFF_ACTIVE",
            "Graceful restart found a different active handoff for this execution.");
        }
        return { created: false, checkpoint: checkpoint, handoff: existing[0] };
      }
      var nextEpoch = Number(execution.current_epoch) + 1;
      if (!Number.isSafeInteger(nextEpoch)) {
        throw error("COOP_CONTROL_EXECUTION_INVALID", "Execution epoch overflowed.");
      }
      var at = timestamp();
      var successorId = "inc:restart:" + digest(handoffId + "\u0000incarnation").slice(0, 48);
      var successorDigest = digest(handoffId + "\u0000capability");
      db.prepare("INSERT INTO coop_control_handoffs (handoff_id, execution_id, handoff_class, reason, " +
        "from_project_id, from_session_id, to_project_id, to_session_id, from_incarnation_id, " +
        "from_epoch, to_incarnation_id, to_epoch, successor_receipt_id, successor_capability_digest, " +
        "state, successor_state, packet_digest, created_at, updated_at, cutover_at, completed_at, " +
        "failure_code) VALUES (?, ?, 'A', 'provider_unhealthy', ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?, " +
        "'prepared', 'retained', ?, ?, ?, NULL, NULL, NULL)")
        .run(handoffId, execution.execution_id, spec.sessionRef.projectId,
          spec.sessionRef.sessionStorageId, spec.sessionRef.projectId,
          spec.sessionRef.sessionStorageId, incarnation.incarnation_id, execution.current_epoch,
          successorId, nextEpoch, successorDigest, spec.packetDigest, at, at);
      db.prepare("INSERT INTO coop_control_checkpoints " +
        "(checkpoint_id, handoff_id, packet_json, packet_digest, created_at) VALUES (?, ?, ?, ?, ?)")
        .run(checkpointId, handoffId, spec.packetJson, spec.packetDigest, at);
      return { created: true,
        checkpoint: db.prepare("SELECT * FROM coop_control_checkpoints WHERE handoff_id = ?").get(handoffId),
        handoff: db.prepare("SELECT * FROM coop_control_handoffs WHERE handoff_id = ?").get(handoffId) };
    });
  }

  function sameAuthority(row, spec) {
    return row && row.source_project_id === spec.sourceProjectId &&
      row.source_session_id === spec.sourceSessionId &&
      row.portfolio_task_id === spec.portfolioTaskId &&
      Number(row.binding_revision) === spec.bindingRevision &&
      row.target_project_id === spec.targetProjectId && row.role === spec.role &&
      Number(row.action_mask) === spec.actionMask && row.revoked_at === null;
  }

  function ensureAuthority(spec, at) {
    var existing = authorityRow(spec.authorityId);
    if (existing) {
      if (!sameAuthority(existing, spec)) {
        throw error("COOP_CONTROL_AUTHORITY_CONFLICT", "Authority identity resolves to different structured fields.");
      }
      return;
    }
    db.prepare("INSERT INTO coop_control_authorities " +
      "(authority_id, source_project_id, source_session_id, portfolio_task_id, binding_revision, " +
      "target_project_id, role, action_mask, issued_at, revoked_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)")
      .run(spec.authorityId, spec.sourceProjectId, spec.sourceSessionId, spec.portfolioTaskId,
        spec.bindingRevision, spec.targetProjectId, spec.role, spec.actionMask, at);
  }

  function sameExecution(row, spec) {
    return row && row.execution_id === spec.executionId &&
      row.portfolio_task_id === spec.portfolioTaskId &&
      Number(row.binding_revision) === spec.bindingRevision &&
      row.idempotency_key === spec.idempotencyKey && row.target_project_id === spec.targetProjectId &&
      row.mode === spec.mode && row.authority_id === spec.authorityId;
  }

  function insertIncarnation(spec, epoch, at) {
    db.prepare("INSERT INTO coop_control_incarnations " +
      "(incarnation_id, execution_id, epoch, session_project_id, session_storage_id, capability_digest, " +
      "start_state, failure_code, created_at, updated_at, started_at) " +
      "VALUES (?, ?, ?, NULL, NULL, ?, 'reserved', NULL, ?, ?, NULL)")
      .run(spec.incarnationId, spec.executionId, epoch, spec.capabilityDigest, at, at);
    db.prepare("INSERT INTO coop_control_role_leases " +
      "(execution_id, role, incarnation_id, epoch, authority_id, acquired_at, updated_at) " +
      "VALUES (?, ?, ?, ?, ?, ?, ?)")
      .run(spec.executionId, spec.role, spec.incarnationId, epoch, spec.authorityId, at, at);
  }

  function reserveExecution(spec) {
    return transaction("reserve_execution", function () {
      var at = timestamp();
      var existing = bindingExecution(spec);
      if (existing && !sameExecution(existing, spec)) {
        throw error("COOP_CONTROL_EXECUTION_CONFLICT", "The binding revision already names a different logical execution.");
      }
      if (existing && (existing.status === "pending" || existing.status === "running")) {
        var lease = currentLease(existing.execution_id, spec.role);
        var incarnation = currentIncarnation(existing.execution_id, Number(existing.current_epoch));
        if (!lease || !incarnation) {
          throw error("COOP_CONTROL_STORE_LOGICAL_CORRUPTION", "An active execution has no current lease or incarnation.");
        }
        return { active: true, execution: existing, incarnation: incarnation, lease: lease };
      }
      if (existing && existing.status === "completed") {
        throw error("COOP_CONTROL_EXECUTION_COMPLETE", "A completed logical execution cannot be restarted.");
      }
      ensureAuthority(spec, at);
      if (!existing) {
        db.prepare("INSERT INTO coop_control_executions " +
          "(execution_id, portfolio_task_id, binding_revision, idempotency_key, target_project_id, mode, " +
          "authority_id, current_epoch, status, created_at, updated_at, finished_at) " +
          "VALUES (?, ?, ?, ?, ?, ?, ?, 1, 'pending', ?, ?, NULL)")
          .run(spec.executionId, spec.portfolioTaskId, spec.bindingRevision, spec.idempotencyKey,
            spec.targetProjectId, spec.mode, spec.authorityId, at, at);
        insertIncarnation(spec, 1, at);
        return { active: false, epoch: 1 };
      }
      var nextEpoch = Number(existing.current_epoch) + 1;
      if (!Number.isSafeInteger(nextEpoch)) {
        throw error("COOP_CONTROL_EXECUTION_INVALID", "Execution epoch overflowed.");
      }
      db.prepare("UPDATE coop_control_executions SET current_epoch = ?, status = 'pending', " +
        "updated_at = ?, finished_at = NULL WHERE execution_id = ?")
        .run(nextEpoch, at, spec.executionId);
      insertIncarnation(spec, nextEpoch, at);
      return { active: false, epoch: nextEpoch };
    });
  }

  function assertCurrent(ref) {
    assertOpen();
    var execution = executionRow(ref.executionId);
    var incarnation = execution && currentIncarnation(ref.executionId, ref.epoch);
    var lease = execution && currentLease(ref.executionId, ref.role);
    var authority = execution && authorityRow(execution.authority_id);
    if (!executionMatches(execution, authority, ref) || !incarnationMatches(incarnation, ref) ||
        !leaseMatches(lease, ref)) {
      throw error("COOP_CONTROL_FENCE_REJECTED", "The execution capability is stale or no longer holds its role lease.");
    }
    return { execution: execution, incarnation: incarnation, lease: lease, authority: authority };
  }

  function requireState(ref, states) {
    var current = assertCurrent(ref);
    if (!states[current.incarnation.start_state]) {
      throw error("COOP_CONTROL_START_STATE_INVALID", "Execution start transition is out of order.");
    }
    return current;
  }

  function bindStart(ref, sessionRef) {
    return transaction("bind_execution_start", function () {
      var current = requireState(ref, { reserved: true });
      if (current.execution.target_project_id !== sessionRef.projectId) {
        throw error("COOP_CONTROL_EXECUTION_INVALID",
          "The execution SessionRef must belong to its authorized target project.");
      }
      var at = timestamp();
      db.prepare("UPDATE coop_control_incarnations SET session_project_id = ?, session_storage_id = ?, " +
        "start_state = 'bound', updated_at = ? WHERE incarnation_id = ?")
        .run(sessionRef.projectId, sessionRef.sessionStorageId, at, ref.incarnationId);
      return true;
    });
  }

  function openBarrier(ref) {
    return transaction("open_execution_barrier", function () {
      requireState(ref, { bound: true });
      var at = timestamp();
      db.prepare("UPDATE coop_control_incarnations SET start_state = 'ready', updated_at = ? " +
        "WHERE incarnation_id = ?").run(at, ref.incarnationId);
      return true;
    });
  }

  function markStarted(ref) {
    return transaction("mark_execution_started", function () {
      requireState(ref, { ready: true });
      var at = timestamp();
      db.prepare("UPDATE coop_control_incarnations SET start_state = 'started', started_at = ?, updated_at = ? " +
        "WHERE incarnation_id = ?").run(at, at, ref.incarnationId);
      db.prepare("UPDATE coop_control_executions SET status = 'running', updated_at = ? WHERE execution_id = ?")
        .run(at, ref.executionId);
      return true;
    });
  }

  var staleR6 = staleR6Reconciliation.createStaleR6ReconciliationStore({
    authorityRow: authorityRow, assertCurrent: assertCurrent, currentIncarnation: currentIncarnation,
    currentLease: currentLease, db: db, digest: digest, error: error, executionRow: executionRow,
    timestamp: timestamp, transaction: transaction, validation: validation,
  });

  function terminalize(ref, status, startState, reason) {
    return transaction("terminalize_execution", function () {
      var current = assertCurrent(ref);
      if (status === "completed" && current.incarnation.start_state !== "started") {
        throw error("COOP_CONTROL_START_STATE_INVALID", "Only a started execution may complete.");
      }
      var at = timestamp();
      db.prepare("UPDATE coop_control_incarnations SET start_state = ?, failure_code = ?, updated_at = ? " +
        "WHERE incarnation_id = ?")
        .run(startState, reason || null, at, ref.incarnationId);
      db.prepare("UPDATE coop_control_executions SET status = ?, updated_at = ?, finished_at = ? " +
        "WHERE execution_id = ?").run(status, at, at, ref.executionId);
      db.prepare("DELETE FROM coop_control_role_leases WHERE execution_id = ? AND role = ?")
        .run(ref.executionId, ref.role);
      return true;
    });
  }

  function recoverIncomplete(excludedExecutionIds) {
    return transaction("recover_incomplete_executions", function () {
      var rows = db.prepare("SELECT execution_id, current_epoch FROM coop_control_executions " +
        "WHERE status IN ('pending', 'running') ORDER BY execution_id").all();
      var excluded = Object.create(null);
      var ids = Array.isArray(excludedExecutionIds) ? excludedExecutionIds : [];
      for (var excludedIndex = 0; excludedIndex < ids.length; excludedIndex++) {
        excluded[ids[excludedIndex]] = true;
      }
      var at = timestamp();
      var recovered = 0;
      for (var i = 0; i < rows.length; i++) {
        if (excluded[rows[i].execution_id]) continue;
        db.prepare("UPDATE coop_control_incarnations SET start_state = 'failed', failure_code = 'restart_recovery', " +
          "updated_at = ? WHERE execution_id = ? AND epoch = ?")
          .run(at, rows[i].execution_id, rows[i].current_epoch);
        db.prepare("UPDATE coop_control_executions SET status = 'failed', updated_at = ?, finished_at = ? " +
          "WHERE execution_id = ?").run(at, at, rows[i].execution_id);
        db.prepare("DELETE FROM coop_control_role_leases WHERE execution_id = ?").run(rows[i].execution_id);
        recovered += 1;
      }
      return recovered;
    });
  }

  function rotateRecoveryTarget(sessionRef, next) {
    return transaction("rotate_replay_execution", function () {
      if (!next || !validation.IDENTIFIER_RE.test(next.incarnationId || "") ||
          !validation.DIGEST_RE.test(next.capabilityDigest || "")) {
        throw error("COOP_CONTROL_EXECUTION_INVALID", "Recovery rotation requires a new capability identity.");
      }
      var current = activeExecutionForTarget(sessionRef);
      if (!current) return null;
      var nextEpoch = Number(current.current_epoch) + 1;
      if (!Number.isSafeInteger(nextEpoch)) {
        throw error("COOP_CONTROL_EXECUTION_INVALID", "Execution epoch overflowed.");
      }
      var at = timestamp();
      db.prepare("UPDATE coop_control_incarnations SET start_state = 'failed', " +
        "failure_code = 'restart_replay', updated_at = ? WHERE incarnation_id = ?")
        .run(at, current.incarnation_id);
      db.prepare("INSERT INTO coop_control_incarnations " +
        "(incarnation_id, execution_id, epoch, session_project_id, session_storage_id, capability_digest, " +
        "start_state, failure_code, created_at, updated_at, started_at) " +
        "VALUES (?, ?, ?, ?, ?, ?, 'ready', NULL, ?, ?, NULL)")
        .run(next.incarnationId, current.execution_id, nextEpoch, sessionRef.projectId,
          sessionRef.sessionStorageId, next.capabilityDigest, at, at);
      db.prepare("DELETE FROM coop_control_role_leases WHERE execution_id = ?").run(current.execution_id);
      db.prepare("INSERT INTO coop_control_role_leases " +
        "(execution_id, role, incarnation_id, epoch, authority_id, acquired_at, updated_at) " +
        "VALUES (?, ?, ?, ?, ?, ?, ?)")
        .run(current.execution_id, current.role, next.incarnationId, nextEpoch,
          current.authority_id, at, at);
      db.prepare("UPDATE coop_control_executions SET current_epoch = ?, status = 'pending', " +
        "updated_at = ?, finished_at = NULL WHERE execution_id = ?")
        .run(nextEpoch, at, current.execution_id);
      return { authority_id: current.authority_id, epoch: nextEpoch,
        execution_id: current.execution_id, incarnation_id: next.incarnationId, role: current.role };
    });
  }

  function inspectExecution(executionId) {
    assertOpen();
    var execution = executionRow(executionId);
    if (!execution) return null;
    return {
      authority: authorityRow(execution.authority_id),
      execution: execution,
      incarnations: db.prepare("SELECT * FROM coop_control_incarnations WHERE execution_id = ? ORDER BY epoch")
        .all(executionId),
      leases: db.prepare("SELECT * FROM coop_control_role_leases WHERE execution_id = ? ORDER BY role")
        .all(executionId),
    };
  }

  return {
    abandonExecution: function (ref, reason) { return terminalize(ref, "failed", "failed", reason); },
    assertCurrentExecution: assertCurrent,
    bindExecutionStart: bindStart,
    completeExecution: function (ref) { return terminalize(ref, "completed", "completed", null); },
    inspectExecution: inspectExecution,
    getStaleR6ReconciliationReceipt: function (receiptId) { assertOpen(); return staleR6.getReceipt(receiptId); },
    findExecutionRecoveryTargets: findRecoveryTargets,
    listIncompleteExecutions: listIncompleteExecutions,
    markExecutionStarted: markStarted,
    openExecutionBarrier: openBarrier,
    prepareRestartHandoff: prepareRestartHandoff,
    recoverIncompleteExecutions: recoverIncomplete,
    rotateExecutionRecoveryTarget: rotateRecoveryTarget,
    reserveExecution: reserveExecution,
    reconcileStaleR6Execution: function (ref, receipt) {
      return staleR6.reconcile(ref, receipt);
    },
  };
}

module.exports = { createExecutionStoreApi: createExecutionStoreApi };

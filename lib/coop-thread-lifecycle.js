...
function snapshot(thread) {
  return {
    ...
    ownerDisposition: clone(thread.ownerDisposition),
  };
}

function restore(thread, before) {
  var fields = ["topicRef", "threadRef", "status", "threadState", "closeOutcome", "mergedInto",
    "mergedIntoThreadRef", "eventRefs", "turnRefs", "relatedExecutions", "hidden", "ownerDisposition",
    "updatedAt", "threadStateUpdatedAt"];
...
function reassignTurnInternal(seam, sourceRef, targetRef, turnRefValue, options, operation, allowHandedOffSource) {
  var state = seam.load();
  migrateState(state, seam.now);
  ...
  if ((source.thread.threadState === THREAD_STATES.HANDED_OFF && !allowHandedOffSource) ||

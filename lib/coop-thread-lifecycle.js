...
function setThreadState(seam, ref, stateName, options) {
  ...
  if (stateName === THREAD_STATES.CLOSED && closeOutcome === CLOSE_OUTCOMES.NOT_PURSUING) {
    var removed = clone(resolved.thread);
    removed.threadState = THREAD_STATES.CLOSED;
    removed.closeOutcome = CLOSE_OUTCOMES.NOT_PURSUING;
    removed.status = "closed";
    removed.updatedAt = valueNow(seam.now);
    removed.threadStateUpdatedAt = removed.updatedAt;
    delete state.topics[resolved.threadRef.threadId];
    seam.save();
    return { ok: true, deleted: true, thread: removed };
  }

var restoreSnapshots = require("./coop-thread-undo").restoreSnapshots;
...
// The canonical place a Thread's lifecycle tuple is written.
//
// threadState is the primary axis: the owner's Threads rail filters on it alone
// (sidebar-coop-topic-model.coopTopicSections), and status is derived FROM it,
// never the reverse. Every path that retires a record must land the whole tuple
// -- threadState, closeOutcome, status, hidden -- or it produces a half-close:
// the record passes the Done section's status test while threadState stays live,
// so the row renders in Threads forever and re-closing cannot fix it (the sweep
// only considers status === "open").
//
// Free of the seam, authority and history concerns on purpose, so the
// non-interactive closure paths can reuse the field writes without inheriting
// setThreadState's owner-socket contract. Reports whether anything moved.
function applyThreadTransition(thread, stateName, closeOutcome, now) {
  // A thread with live linked work cannot sit in "exploring": the projection
  // recomputes it as handed_off anyway, so record what will be read back.
  if (stateName === THREAD_STATES.EXPLORING &&
      Array.isArray(thread.relatedExecutions) && thread.relatedExecutions.length) {
    stateName = THREAD_STATES.HANDED_OFF;
  }
  var outcome = stateName === THREAD_STATES.CLOSED ? closeOutcome : null;
  var hidden = stateName === THREAD_STATES.CLOSED && outcome === CLOSE_OUTCOMES.NOT_PURSUING;
  var status = stateName === THREAD_STATES.CLOSED ? "closed" : "open";
  if (thread.threadState === stateName && (thread.closeOutcome || null) === outcome &&
      !!thread.hidden === hidden && thread.status === status) {
    return { changed: false, threadState: stateName, closeOutcome: outcome, hidden: hidden };
  }
  var updatedAt = valueNow(now);
  thread.threadState = stateName;
  thread.closeOutcome = outcome;
  thread.status = status;
  thread.hidden = hidden;
  thread.updatedAt = updatedAt;
  thread.threadStateUpdatedAt = updatedAt;
  return { changed: true, threadState: stateName, closeOutcome: outcome, hidden: hidden };
}
...
function setThreadState(seam, ref, stateName, options) {
  ...
  var applied = applyThreadTransition(resolved.thread, stateName, closeOutcome, seam.now);
  if (!applied.changed) return { ok: true, unchanged: true, thread: clone(resolved.thread) };
...
// A conventional open/closed status change, applied so the record stays
// self-consistent. For the paths that legitimately own their own authority
// check and their own persistence -- the owner-confirmed bulk sweep and the
// daemon's ledger reconciliation -- but which used to assign topic.status
// directly and so never retired the thread they claimed to close.
//
// NOT a way around setThreadState's owner gate: it takes an already resolved
// record from a caller that has done its own gating, and records no lifecycle
// history because the caller owns its own audit trail (closureAudit for the
// sweep, the disposition request log for reconciliation).
function applyRecordStatus(thread, targetStatus, options) {
  if (!thread || typeof thread !== "object") return { ok: false, code: "thread_not_found" };
  if (thread.status === "merged") return { ok: false, code: "topic_merged" };
  var opts = options || {};
  if (targetStatus === "closed") {
    if (!validCloseOutcome(opts.closeOutcome)) return { ok: false, code: "close_outcome_required" };
    return Object.assign({ ok: true },
      applyThreadTransition(thread, THREAD_STATES.CLOSED, opts.closeOutcome, opts.now));
  }
  if (targetStatus === "open") {
    // Reopening never demotes a handed-off thread to exploring -- the same
    // downgrade setThreadState refuses outright. Silent no-op rather than an
    // error: a refusal would fail an otherwise valid status reconciliation.
    var next = thread.threadState === THREAD_STATES.HANDED_OFF
      ? THREAD_STATES.HANDED_OFF : THREAD_STATES.EXPLORING;
    return Object.assign({ ok: true }, applyThreadTransition(thread, next, null, opts.now));
  }
  return { ok: false, code: "invalid_thread_status" };
}
...
function mergeThreads(seam, targetRefValue, sourceRefValues, options) {
  ...
  for (var si = 0; si < sources.length; si++) {
    ...
    if (!Array.isArray(target.thread.relatedPlanning)) target.thread.relatedPlanning = [];
    appendUnique(target.thread.relatedPlanning, current.thread.relatedPlanning || [], JSON.stringify);
...
function undoLastCorrection(seam) {
  ...
  var restored = restoreSnapshots(state.topics, record.before, record.after);
...
function redoCorrection(seam, correctionId) {
  ...
  var restored = restoreSnapshots(state.topics, record.after, record.before);
  if (!restored.ok) return restored;
...
module.exports = {
  ...
  applyThreadTransition: applyThreadTransition,
  applyRecordStatus: applyRecordStatus,
  validCloseOutcome: validCloseOutcome,

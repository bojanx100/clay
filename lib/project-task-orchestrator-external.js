...
var createTaskCoordinatorRecovery = require("./project-task-orchestrator-coordinator-recovery").createTaskCoordinatorRecovery;
...
var createPortfolioRestaffRuntime = require("./portfolio-restaff-runtime").createPortfolioRestaffRuntime;
...
function projectIdForManager(sm) { return sm && typeof sm.getProjectId === "function" ? sm.getProjectId() : null; }
...
function attachPortfolioExecutionTarget(ctx) {
  ...
  var routerControlled = crossProject && typeof crossProject.controlledExecutionEnabled === "function" ?
  ...
  var runtimeDisabled = routerControlled === false || (!crossProject && Object.prototype.hasOwnProperty.call(ctx, "crossProject"));
  ...
  var runtimeExecutionControl = ctx.coopExecutionControl || (!runtimeDisabled ? controlRuntime.getExecutionControl() : executionControlModule.createExecutionControl({ enabled: false }));
  ...
  var recoveryDelivery = ctx.coopDeliveryControl || (!accessoryRuntimeDisabled ? controlRuntime.getDeliveryControl() : deliveryControlModule.createDeliveryControl({ enabled: false }));
  var runtimeHandoffControl = ctx.coopHandoffControl || (!accessoryRuntimeDisabled ? controlRuntime.getHandoffControl() : handoffControlModule.createHandoffControl({ enabled: false }));
  ...
  var restaffRuntime = createPortfolioRestaffRuntime(Object.assign({}, ctx, { crossProject: crossProject,
    discardSession: function (session) { targetControlModule.discardUnstartedSession(sm, session); },
    projectIdForManager: projectIdForManager, setExecutionStatus: setExecutionStatus, sm: sm }));
  var infrastructureRecovery = restaffRuntime.infrastructureRecovery;
  var executionStarter = createExecutionStarter({
    ensureProjectAccessForSession: ensureProjectAccessForSession,
    executionControl: executionControl,
    onProcessingChanged: onProcessingChanged, onStartFailure: infrastructureRecovery.recoverAfterFailure,
    sdk: sdk,
    sm: sm,
  ...
  function createSession(payload, request, source) {
    var launchCheck = restaffRuntime.launchRevalidation(payload, request);
    if (!launchCheck || launchCheck.ok !== true) return launchCheck;
    ...
    var session = createExecutionSession({ executionControl: executionControl, sm: sm,
      ...
      captureRecovery: function (created) { infrastructureRecovery.capture(created, payload, request, { save: false }); },
  ...
  function messageExecution(envelope) {
    ...
    if (!recoveryDelivery.enabled) {
      return resume && !resume.reuseCurrent ? coordinatorRecovery.restart({
        session: session,
    ...
    recoveryDelivery.reconcileOne(accepted.effectId, function (effect) {
      result = resume && !resume.reuseCurrent ? coordinatorRecovery.restart({
        session: session,
        request: resume.request,
        source: resume.source,
        text: text,
  ...
  function reconcilePersistedSessions() {
    ...
    sm.sessions.forEach(function (session) {
      ...
      if (executionControl.reconcileSession(session, metadata)) {
        sm.saveSessionFile(session);
        sm.broadcastSessionList();
        if (metadata && metadata.settledReadOnlyReviewReconciliation && typeof ctx.reconcileSettledReview === "function") ctx.reconcileSettledReview(session);
      }
      if (metadata && metadata.mode === "direct_leaf" &&
  ...
  return { handleEnvelope: handleEnvelope, handoffExecution: handoffExecution,
    recoverInfrastructureExecution: infrastructureRecovery.recoverReaped,
    revalidatePortfolioLaunch: restaffRuntime.launchRevalidation,

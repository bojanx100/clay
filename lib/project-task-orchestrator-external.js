...
function attachPortfolioExecutionTarget(ctx) {
  ...
  function watchDirectLeaf(session) {
    ...
    var unsubscribe = sm.subscribeSession(session.localId, function (event) {
      ...
      if (status === "completed" || status === "failed") directLeafCompletion.deliver(session, result, true);
      else deliverDirectLeafUpdate(session, "direct-result-" + crypto.randomUUID(), [
  ...
  function continueExecution(session, prompt) {
    if (session.isProcessing && typeof sdk.pushMessage === "function") {
      sdk.pushMessage(session, prompt, null);
      return;
    }

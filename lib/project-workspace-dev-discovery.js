...
function listenerWorkingDirs(port, cb) {
  if (!port) return cb([]);
  execFile("lsof", ["-tiTCP:" + parseInt(port, 10), "-sTCP:LISTEN"], {
    ...
    var pids = String(stdout || "").split(/\s+/).filter(function (pid) {
      if (!/^\d+$/.test(pid) || seenPids[pid]) return false;
      seenPids[pid] = true;
      return true;
    });
    if (err || !pids.length) return cb([]);
    var dirs = [];
    var seenDirs = {};
      ...
      execFile("lsof", ["-a", "-p", pid, "-d", "cwd", "-Fn"], {
        ...
        if (!cwdErr) {
          ...
          for (var i = 0; i < lines.length; i++) {
            if (lines[i].charAt(0) !== "n") continue;
            var dir = canonicalDir(lines[i].slice(1));
            if (!seenDirs[dir]) {
              seenDirs[dir] = true;
              dirs.push(dir);
            }
...
module.exports = {
  listenerWorkingDirs: listenerWorkingDirs,

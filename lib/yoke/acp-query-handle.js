...
function createAcpQueryHandle(acp, queryOpts) {
  ...
  function sessionParamsWithMcp(base) {
    var params = Object.assign({}, base || {});
    if (Array.isArray(queryOpts.mcpServers) && queryOpts.mcpServers.length > 0) {
      params.mcpServers = queryOpts.mcpServers;
    }
    return params;
  }
  ...
  async function openDefaultSession(handlerEntry) {
    if (state.sessionId && queryOpts.canResumeSession) {
      ...
      var resumed = await acp.send("session/resume", buildSessionParams("resume", sessionParamsWithMcp({
        sessionId: state.sessionId,
        cwd: queryOpts.cwd,
      })), 60000);
      return resumed;
    ...
    if (state.sessionId && queryOpts.canLoadSession) {
      ...
      try {
        var loaded = await acp.send("session/load", buildSessionParams("load", sessionParamsWithMcp({
          sessionId: state.sessionId,
          cwd: queryOpts.cwd,
        })), 60000);
        return loaded;
    ...
    var result = await acp.send("session/new", buildSessionParams("new", sessionParamsWithMcp({
      cwd: queryOpts.cwd,
    })), 60000);
    return result;
  }

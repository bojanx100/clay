...
var MODEL_GUIDANCE_MARKER = "--- Model selection guidance ---";

function looksLikeInjectedPrefix(text) {
  return text.indexOf("--- Instructions from ") === 0
    || text.indexOf(MODEL_GUIDANCE_MARKER) === 0;
}

function containsClayInjectedBlock(text) {
  return text.indexOf("--- Instructions from ") !== -1
    || text.indexOf(MODEL_GUIDANCE_MARKER) !== -1;
}
...
function stripInjectedInstructions(text, projectDir, vendor) {
  ...
  if (mi !== -1) {
    var injectedPrefix = text.slice(0, mi);
    if (looksLikeInjectedPrefix(text) || containsClayInjectedBlock(injectedPrefix)) {
      return text.slice(mi + 1 + INSTRUCTIONS_END_MARKER.length).replace(/^\n+/, "");
    }
  }
  if (!looksLikeInjectedPrefix(text)) return text;
...
module.exports = {
  ...
  MODEL_GUIDANCE_MARKER: MODEL_GUIDANCE_MARKER,

...
function createCodexQueryHandle(appServer, queryOpts) {
  ...
  function startInterruptDrain() {
    ...
    var timeoutMs = queryOpts.interruptDrainTimeoutMs || 4000;
...
function createCodexCoreAdapter(opts) {
  ...
  // overwrite a real catalog. Always "fallback-seed" until live discovery
  // actually succeeds.
  var _modelsProvenance = "fallback-seed";
  var _appServer = null;
  ...
  function clearRuntimeState() {
    ...
    _modelsProvenance = "fallback-seed";
    _refCount = 0;
    _activeQueries = [];
    ...
    init: function(initOpts) {
      ...
      _initPromise = (async function() {
        ...
        if (liveModels.length > 0) {
          ...
          _defaultModel = codexModels.preferredCodexDefault(_cachedModels);
          _modelsProvenance = "live-discovery";
        } else {
          _cachedModels = codexModels.fallbackCodexModels();
          _defaultModel = codexModels.preferredCodexDefault(_cachedModels);
          _modelsProvenance = "fallback-seed";

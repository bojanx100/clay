...
var codexDynamicTools = require("./codex-dynamic-tools");
...
function createCodexQueryHandle(appServer, queryOpts) {
  ...
  state.turnTerminalSeen = false;
  state.awaitingTurnTerminal = false;
  var dynamicTools = codexDynamicTools.createDynamicToolLifecycle(appServer, {
    onCancel: function(count, reason) {
      console.info("[yoke/codex] Cancelled " + count + " pending dynamic tool call(s): " + reason);
    },
  });
  ...
  function endIterator() {
    ...
    if (!state.awaitingTurnTerminal || state.turnTerminalSeen) unsubscribeEvents();
    notifyFinished();
  }
  ...
  function handleServerEvent(msg) {
    ...
    var isWorkspaceToolCall = method === "item/tool/call" && params.tool === codexWorkspaceDependencies.TOOL_NAME;
    if (method === "turn/completed" || method === "turn/failed") {
      state.turnTerminalSeen = true;
      state.awaitingTurnTerminal = false;
    }

    // An interrupted turn still needs an explicit response to a workspace
    // dynamic-tool request that was already in flight. All other non-terminal
    // events remain ignored after abort so permissions cannot be revived.
    if (isCancelled() && method !== "turn/completed" && method !== "turn/failed" &&
        method !== "serverRequest/resolved" && method !== "thread/status/changed" && !isWorkspaceToolCall) return;
    ...
    if (isWorkspaceToolCall) {
      dynamicTools.handleWorkspaceToolCall(msg, params, workspaceDependencies, isCancelled);
      return;
    }
    ...
    if (method === "turn/completed" || method === "turn/failed") {
      ...
      // A normal handle remains subscribed for its next queued user message.
      // Abort has already ended the iterator, so this is the point where its
      // short drain window can safely release the subscription.
      if (state.aborted || iteratorDone) unsubscribeEvents();
    }
  }
  ...
  async function runQueryLoop(initialMessage) {
    ...
    try {
      ...
      if (queryOpts.resumeSessionId) {
        ...
        threadResult = await appServer.send("thread/resume", resumeParams, 60000);
      } else {
  ...
  var handle = {
    ...
    abort: function() {
      ...
      state.aborted = true;
      state.awaitingTurnTerminal = !!state.threadId;
      dynamicTools.cancelPending("the Codex turn was interrupted");

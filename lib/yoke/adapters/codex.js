...
var createSessionTools = require("./codex-session-tools").createSessionTools;
var readOnlyQuery = require("../read-only-query");
...
function createCodexQueryHandle(appServer, queryOpts) {
  if (queryOpts.readOnlyExecution) {
    queryOpts = Object.assign({}, queryOpts, { workspaceDependencies: null,
      toolServerDescriptors: [], callMcpTool: null,
      canUseTool: async function () { return { behavior: "deny" }; },
      onElicitation: async function () { return { action: "decline" }; } });
  }
  var abortController = queryOpts.abortController;
  var workspaceDependencies = queryOpts.workspaceDependencies || null;
  var sessionTools = createSessionTools(queryOpts.toolServerDescriptors, queryOpts.callMcpTool);
  if (sessionTools.dynamicTools.length && workspaceDependencies &&
      workspaceDependencies.canConfigureDynamicTools === false) {
    throw new Error("This Codex server cannot provide session-scoped control tools.");
  }
  ...
  var userInputRequests = require("./codex-user-input").createUserInputRequests(onElicitation,
    function (id, response) { appServer.respond(id, response); }, abortController && abortController.signal);
  ...
  function handleServerEvent(msg) {
    ...
    if (method === "serverRequest/resolved") userInputRequests.resolved(params.requestId);
    if (method === "turn/completed" || method === "turn/failed") userInputRequests.cancelAll();
    if (method === "item/tool/requestUserInput") { userInputRequests.handle(msg); return; }
    ...
    // Privileged in-app tools must stay on the exact query callback; the shared
    // native MCP bridge knows only the project, not the originating session.
    if (method === "item/tool/call" && sessionTools.isSessionTool(params.tool)) {
      if (!turnResolve || !state.threadId || params.threadId !== state.threadId ||
          !state.turnId || params.turnId !== state.turnId) {
        appServer.respond(msg.id, sessionTools.failure("Session-scoped tool requires the current thread and turn."));
        return;
      }
      sessionTools.call(params.tool, params.arguments).then(function (result) {
        appServer.respond(msg.id, result);
      }).catch(function (error) {
        console.error("[yoke/codex] Session-scoped tool response failed:", error.message || error);
      });
      return;
    }
  ...
  async function runQueryLoop(initialMessage) {
    ...
    try {
      ...
      if (queryOpts.readOnlyExecution) {
        ...
        threadParams.config = readOnlyQuery.codexConfig();
      }
      ...
      if (sessionTools.dynamicTools.length) {
        threadParams.dynamicTools = (threadParams.dynamicTools || []).concat(sessionTools.dynamicTools);
      }
      ...
      if (queryOpts.resumeSessionId) {
        if (queryOpts.readOnlyExecution) await readOnlyQuery.prepareCodexResume(appServer, queryOpts.resumeSessionId);
        var resumeParams = {
        ...
        if (threadParams.config) resumeParams.config = threadParams.config;
      ...
      if (queryOpts.readOnlyExecution) readOnlyQuery.assertCodexThread(threadResult);
      ...
      while (!isCancelled()) {
        ...
        var availableSkills = queryOpts.readOnlyExecution ? [] : discoverClaudeSkills(queryOpts.cwd);
  ...
  var handle = {
    ...
    pushMessage: function(text, images) {
      if (iteratorDone || state.done || state.aborted || messageQueueEnded) return false;
  ...
  var adapter = {
    ...
    createQuery: async function(queryOpts) {
      ...
      var handleOpts = {
        ...
        toolServerDescriptors: queryOpts.toolServerDescriptors,
        callMcpTool: queryOpts.callMcpTool,
        readOnlyExecution: queryOpts.readOnlyExecution === true,
      };

...
function flattenEvent(notification, state) {
  ...
  if (method === "turn/completed") {
    ...
    for (var cti = 0; cti < completedItems.length; cti++) {
      ...
      if (!completedItem ||
          (completedItem.type !== "dynamicToolCall" && completedItem.type !== "dynamic_tool_call")) continue;
  ...
  if (method === "item/started" || method === "item/updated" || method === "item/completed") {
    ...
    if (item.type === "dynamicToolCall" || item.type === "dynamic_tool_call") {
      ...
      if (evtPhase === "completed" && !state.dynamicToolResults[item.id]) {
        state.dynamicToolResults[item.id] = true;
        var dynamicText = [];
        var dynamicImages = [];
        var contentItems = Array.isArray(item.contentItems) ? item.contentItems : [];
        for (var dci = 0; dci < contentItems.length; dci++) {
          var contentItem = contentItems[dci];

...
var flattenReasoningNotification = codexRichEvents.flattenReasoningNotification;
var commandOutput = codexRoutingUtils.attachCodexCommandOutput();
...
function flattenEvent(notification, state) {
  ...
  var reasoningEvents = flattenReasoningNotification(method, params, state);
  if (reasoningEvents) return reasoningEvents;
    ...
    if (item.type === "commandExecution" || item.type === "command_execution") {
      ...
      if (evtPhase === "completed") {
        ...
        events.push({
          ...
          content: item.aggregated_output || item.output || commandOutput.fallback(cmdBuf),
  ...
  if (method === "thread/tokenUsage/updated") {
    ...
    if (tu) {
      // `total` is cumulative for the whole thread. Result usage is persisted
      // and summed per turn, so only the app-server's `last` payload is safe
      // to carry across this adapter boundary.
      var lastInputTokens = positiveNumberValue(tu.last, [
        "inputTokens",
        "input_tokens",
        "promptTokens",
        "prompt_tokens",
      ]);
      state.lastInputTokens = lastInputTokens || null;
  ...
  if (method === "item/commandExecution/outputDelta") {
    ...
    var coBuf = commandOutput.bufferFor(state, coItemId);
    commandOutput.append(coBuf, params.delta);
    if (coBuf.pendingLength >= commandOutput.coalesceChars) {
      var tail = commandOutput.takePending(coBuf);
      events.push({

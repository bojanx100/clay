...
function retainClayConnectionOptions(defaultOpts, initOpts) {
  ...
  for (var i = 0; i < fields.length; i++) {
    if (Object.prototype.hasOwnProperty.call(incoming, fields[i])) retained[fields[i]] = incoming[fields[i]];
  }
  return retained;
}

function buildClayMcpBridgeServers(initOpts, fallbackSlug) {
  initOpts = initOpts || {};
  var bridgePath = path.join(__dirname, "..", "mcp-bridge-server.js");
  try { if (!fs.existsSync(bridgePath)) return []; } catch (e) { return []; }
  var clayPort = initOpts.clayPort || process.env.CLAY_PORT || 2633;
  var claySlug = initOpts.slug || fallbackSlug || "";
  var args = [bridgePath, "--port", String(clayPort), "--slug", claySlug];
  if (initOpts.clayTls) args.push("--tls");
  var server = { name: "clay-tools", command: process.execPath, args: args };
  if (initOpts.clayAuthToken) server.env = { CLAY_AUTH_TOKEN: initOpts.clayAuthToken };
  return [server];
}

function acpMcpServersWithClayBridge(mcpServers, initOpts, fallbackSlug) {
  var result = Array.isArray(mcpServers) ? mcpServers.slice() : [];
  var bridge = buildClayMcpBridgeServers(initOpts, fallbackSlug);
  for (var i = 0; i < bridge.length; i++) {
    var replaced = false;
    for (var j = 0; j < result.length; j++) {
      if (result[j] && result[j].name === bridge[i].name) { result[j] = bridge[i]; replaced = true; break; }
    }
    if (!replaced) result.push(bridge[i]);
  }
  return result;
}
function sessionParamsWithMcp(base, mcpServers) {
  var params = Object.assign({}, base || {});
  if (Array.isArray(mcpServers) && mcpServers.length > 0) params.mcpServers = mcpServers;
  return params;
}
async function startCopilotSession(connection, capabilities, opts) {
  opts = opts || {};
  var base = sessionParamsWithMcp({ cwd: opts.cwd }, opts.mcpServers);
...
module.exports = {
  ...
  retainClayConnectionOptions: retainClayConnectionOptions, buildClayMcpBridgeServers: buildClayMcpBridgeServers,
  acpMcpServersWithClayBridge: acpMcpServersWithClayBridge,
};

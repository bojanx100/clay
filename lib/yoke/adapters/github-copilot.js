...
var { createAsyncQueue, delay, findCopilotPath, redactSecrets, safeJson, extractModelEvidence,
  canonicalModelId, resolveKnownCopilotModel, textFromContentList, imagesFromContentList,
  textFromRawToolOutput, imagesFromRawToolOutput, runtimeMetadataPrompt, isCopilotAuthError,
  isCopilotQuotaError, describeAcpError, planFromEntries, permissionResponse, modelConfigOption,
  resolveModelOptionValue, copilotTurnUsage, copilotTruncation, startCopilotSession,
  copilotPromptBlocks, copilotSupportsPromptImages, retainClayConnectionOptions,
  acpMcpServersWithClayBridge } = helpers;
...
function createCopilotQueryHandle(opts) {
  ...
  function pushEvent(ev) { queue.push(ev); }
  ...
  function deathPromise() {
    ...
    return new Promise(function(_resolve, reject) { deathWaiters.push(reject); });
  }
  ...
  function isInternalCancellationNotice(text) {
    return /^Info:\s*Operation cancell?ed by user\.?$/i.test(String(text || "").trim());
  }
  ...
  function emitToolCall(update) {
    ...
    if (update.status === "completed" || update.status === "failed") {
      var resultImages = imagesFromContentList(update.content);
      if (resultImages.length === 0) resultImages = imagesFromRawToolOutput(update.rawOutput);
      var toolResultEvent = {
        yokeType: "tool_result",
        toolId: id,
        content: textFromContentList(update.content) || textFromRawToolOutput(update.rawOutput),
        isError: update.status === "failed",
      };
      if (resultImages.length > 0) toolResultEvent.images = resultImages;
  ...
  function handleSessionUpdate(params) {
    ...
    if (update.sessionUpdate === "agent_message_chunk" && update.content && update.content.type === "text") {
      ...
      // Copilot emits this ACP control notice when a new prompt supersedes
      // leftover background work. It is not evidence that the user pressed
      // Clay's Stop button and should not become assistant transcript text.
      if (isInternalCancellationNotice(messageText)) return Promise.resolve();
      if (isCopilotAuthError(messageText)) {
        promptErrorText = messageText;
        return Promise.resolve();
      }
      if (isCopilotQuotaError(messageText)) {
        promptErrorText = messageText.trim();
        return Promise.resolve();
      }
  ...
  async function ensureStarted() {
    ...
    var client = {
      requestPermission: function(params) { return Promise.resolve(permissionResponse(params, toolPolicy)); },
  ...
  return {
    ...
    pushMessage: function(text, images) {
      if (closed || procExited) return false;
    ...
    setToolPolicy: function(policy) { toolPolicy = policy || toolPolicy; return Promise.resolve(); },
    stopTask: function() {
...
function createGitHubCopilotAdapter(opts) {
  ...
  var _copilotPath = opts._copilotPath || findCopilotPath();
  var _createCopilotQueryHandle = opts._createCopilotQueryHandle || createCopilotQueryHandle;
  var _catalog = opts._catalog || entitlements.createCopilotModelCatalog({ cwd: process.env.HOME || _cwd, fallbackModels: knownModelsForProvider("github-copilot"), resolveExecutable: function() { return _copilotPath; } });
  _catalog.start();
  ...
  return {
    ...
    init: function(initOpts) {
      ...
      _copilotPath = opts._copilotPath || findCopilotPath();
    ...
    supportedModels: function() { return _catalog.supportedModels(); },
    refreshModels: function() { return _catalog.refreshModels(); },
    createToolServer: function() { return null; },
    createQuery: function(queryOpts) {
      queryOpts = Object.assign({}, _defaultInitOpts, queryOpts || {});
      if (queryOpts.readOnlyExecution) require("../read-only-query").apply({}, "github-copilot");
      if (!_copilotPath) _copilotPath = opts._copilotPath || findCopilotPath();
      if (!_copilotPath) throw new Error("GitHub Copilot CLI is not installed. Run: npm install -g @github/copilot");
      return _createCopilotQueryHandle({
        ...
        mcpServers: acpMcpServersWithClayBridge(queryOpts.mcpServers, queryOpts, _slug),
      });
    },

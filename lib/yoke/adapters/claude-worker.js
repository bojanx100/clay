...
async function sendCurrentRateLimitUsage() {
  if (!queryInstance || typeof queryInstance.usage_EXPERIMENTAL_MAY_CHANGE_DO_NOT_RELY_ON_THIS_API_YET !== "function") return;
  try {
    var usage = await queryInstance.usage_EXPERIMENTAL_MAY_CHANGE_DO_NOT_RELY_ON_THIS_API_YET();
    ...
    console.error("[sdk-worker] current usage fetch failed (non-fatal):", e.message || e);
  }
}
...
async function handleWarmup(msg) {
  ...
  // Warmup must never leave a resumable transcript, including writes that land
  // after abort or errors before a session id is returned.
  var warmupOptions = Object.assign({}, msg.options, { persistSession: false });
  warmupOptions.abortController = ac;
  applyWorkerClaudeBinary(warmupOptions);

  // Retain session-id cleanup as a fallback for older runtimes. Disabling
  // persistence above also covers writes after cleanup and failures before init.

...
function flattenReasoningNotification(method, params, state) {
  var events = [];
  if (method === "item/reasoning/summaryTextDelta" ||
      method === "item/reasoning/textDelta") {
    var itemId = params.itemId || params.id;
    if (itemId && !state.thinkingBlocks[itemId]) {
      state.blockCounter++;
      state.thinkingBlocks[itemId] = "blk_" + state.blockCounter;
      events.push({ yokeType: "thinking_start", blockId: state.thinkingBlocks[itemId] });
    }
    if (params.delta) {
      events.push({
        yokeType: "thinking_delta",
        blockId: itemId ? state.thinkingBlocks[itemId] : ("blk_" + state.blockCounter),
        text: params.delta,
      });
      if (itemId) {
        state.thinkingLengths[itemId] =
          (state.thinkingLengths[itemId] || 0) + params.delta.length;
      }
    }
    return events;
  }
  if (method !== "item/reasoning/summaryPartAdded") return null;
  var partItemId = params.itemId || params.id;
  if (partItemId && state.thinkingBlocks[partItemId]) {
    events.push({
      yokeType: "thinking_delta",
      blockId: state.thinkingBlocks[partItemId],
      text: "\n\n",
    });
    state.thinkingLengths[partItemId] =
      (state.thinkingLengths[partItemId] || 0) + 2;
  }
  return events;
}
...
module.exports = {
  ...
  flattenReasoningNotification: flattenReasoningNotification,

...
var { flattenEvent, rateLimitEventsFromUsage } = require("./claude-events");
var claudeImageInput = require("./claude-image-input");
...
// Newer Claude runtimes otherwise interpret the global Stop control as
// permission to kill every background task attached to the session.
var PER_TASK_STOP_AFFORDANCE = true;

// --- Effort mapping ---
// "ultracode" is a Clay-level pseudo effort: xhigh reasoning plus the
// session-scoped `ultracode` settings key (standing dynamic-workflow
// orchestration). The SDK only accepts low|medium|high|xhigh|max for
// `effort`, so map ultracode to xhigh and merge the settings flag.
function applyEffortOption(options, effort) {
  if (!effort) return;
  if (effort === "ultracode") {
    options.effort = "xhigh";
    var base = (options.settings && typeof options.settings === "object") ? options.settings : null;
    options.settings = Object.assign({}, base, { ultracode: true });
    return;
  }
  options.effort = effort;
}

// Build the flag-settings payload for switching effort on a live query via
// query.applyFlagSettings(). Keep in sync with handleSetEffort() in
// sdk-worker.js (worker sessions apply the same mapping in-process).
// The settings schema has no "max" level, so "max" pins xhigh (the closest
// runtime level) and clears ultracode; the real max takes effect on the
// next query via the effort option.
function effortFlagSettings(effort) {
  if (!effort) return null;
  if (effort === "ultracode") return { effortLevel: "xhigh", ultracode: true };
  if (effort === "max") return { effortLevel: "xhigh", ultracode: null };
  return { effortLevel: effort, ultracode: null };
}
...
function createQueryHandle(rawQuery, messageQueue, abortController) {
  var handle = {
      ...
      return {
        next: function() {
          return rawIter.next().then(function(result) {
            if (result.done) { messageQueue.end(); return result; }
    ...
    pushMessage: function(text, images) {
      var content = claudeImageInput.buildContent(text, images);
    ...
    setEffort: function(effort) {
      // Claude SDK has no setEffort on active query; effort is switched at
      // runtime via the flag-settings layer (see effortFlagSettings).
      var flags = effortFlagSettings(effort);
      if (flags && rawQuery && typeof rawQuery.applyFlagSettings === "function") {
        return rawQuery.applyFlagSettings(flags);
      }
      return Promise.resolve();
    },
    ...
    getRateLimitUsageEvents: function() {
      var method = "usage_EXPERIMENTAL_MAY_CHANGE_DO_NOT_RELY_ON_THIS_API_YET";
      if (rawQuery && typeof rawQuery[method] === "function") {
        return rawQuery[method]().then(rateLimitEventsFromUsage);
      }
      return Promise.resolve([]);
    },
...
function spawnWorker(linuxUser, workerScriptPath, cwd) {
  ...
  worker.send = function(msg) {
    if (!worker.connection || worker.connection.destroyed) return false;
    try {
      ...
      if (msg.type === "push_message") throw e;
      return false;
    }
  };
...
function createWorkerQueryHandle(worker, canUseTool, onElicitation, callMcpTool, onUserDialog) {
  ...
  var handle = {
    ...
    pushMessage: function(text, images) {
      ...
      if (iterEnded || inputEnded || worker._abortSent) return false;
      return worker.send({ type: "push_message", content: userMsg });
    },
...
function createClaudeAdapter(opts) {
  ...
  var adapter = {
    ...
    init: async function(initOpts) {
      ...
      if (initOpts && initOpts.env) warmupOptions.env = initOpts.env;
    ...
    createQuery: async function(queryOpts) {
      ...
      if (queryOpts.readOnlyExecution === true) {
        co = require("../read-only-query").claudeOptions(co);
        queryOpts = Object.assign({}, queryOpts, { toolServers: {}, toolServerDescriptors: [] });
      }
      ...
      if (queryOpts.env) sdkOptions.env = queryOpts.env;
      ...
      ["tools", "allowedTools", "disallowedTools", "strictMcpConfig", "plugins"].forEach(function (key) {
        if (co[key] !== undefined) sdkOptions[key] = co[key];
      });
      applyEffortOption(sdkOptions, queryOpts.effort);
  ...
  async function createWorkerQuery(queryOpts, claudeOpts, linuxUser) {
    ...
    if (queryOpts.env) queryOptions.env = queryOpts.env;
    ...
    ["tools", "allowedTools", "disallowedTools", "strictMcpConfig", "plugins"].forEach(function (key) {
      if (claudeOpts[key] !== undefined) queryOptions[key] = claudeOpts[key];
    });
    ...
    applyEffortOption(queryOptions, queryOpts.effort);
  ...
  async function initViaWorker(linuxUser, initOpts) {
    ...
    // Wait for ready, then send warmup
  ...
  adapter.init = async function(initOpts) {
    ...
    if (!linuxUser) {
      ...
      // Capture the session_id so we can delete the SDK-created jsonl after
      // we abort. The SDK starts a real CLI session as soon as it receives
      // our dummy "hi" prompt; without cleanup these one-line ("hi") sessions
      // pile up and get adopted into the project sidebar on restart.
      var warmupSessionId = null;
      try {
        var stream = sdk.query({ prompt: mq, options: warmupOptions });
...
module.exports = {
  ...
  resolveClaudeBinaryPath: resolveClaudeBinaryPath,
  _test: {
    applyEffortOption: applyEffortOption,
    createQueryHandle: createQueryHandle,
    effortFlagSettings: effortFlagSettings,
  },
  contractTestKit: {
    createMessageQueue: createMessageQueue,

...
function flattenEvent(raw) {
  ...
  if (raw.type === "stream_event" && raw.event) {
    ...
    if (evt.type === "message_delta" && evt.delta) {
      // Carries the turn's stop_reason (e.g. "max_tokens" when the response was
      // cut off by the output token limit). Surface it so the result handler can
      // flag a truncated turn instead of silently reporting a clean completion.
  ...
  if (raw.type === "result") {
    ...
    // Normalized truncation note (consistent across adapters). The SDK reports a
    // turn cut short by the turn cap as subtype "error_max_turns".

...
function codexHasCredits(bucket, params) {
  ...
  var resetCredits = params && params.rateLimitResetCredits;
  var available = resetCredits && resetCredits.availableCount;
  if (typeof available === "number" && available > 0) return true;
  if (typeof available === "string" && parseFloat(available.replace(/,/g, "")) > 0) return true;
  return false;
}
...
function flattenCodexRateLimitEvents(params) {
  ...
  for (var wi = 0; wi < windows.length; wi++) {
    ...
    var status = "allowed";
    if (usedPercent >= 100 || (!hasUsedPercent && bucket.rateLimitReachedType)) status = "rejected";
    ...
    events.push({
      ...
      rateLimitInfo: {
        ...
        isUsingOverage: status === "rejected" && hasCredits,
      },
    });
  }
  return events;

...
function isRichToolResultBlock(block) {
  if (!block || !block.type) return false;
  return block.type === "mcp_tool_result" ||
    block.type.indexOf("_tool_result") !== -1 ||
    block.type.indexOf("_execution_result") !== -1;
}

function appendRichResult(value, text, images) {
  ...
  if (Array.isArray(value)) {
    for (var i = 0; i < value.length; i++) appendRichResult(value[i], text, images);
    return;
  }
  if (typeof value !== "object") return;
  if (value.type === "image" && value.source && value.source.data) {
    images.push({
      mediaType: value.source.media_type || "image/png",
  ...
  if (value.url || value.title) {
    var label = value.title || value.url;
    text.push(value.url && label !== value.url ? label + " — " + value.url : label);
    if (value.snippet) text.push(value.snippet);
    return;
  }
  var fields = ["content", "output", "stdout", "stderr", "text", "message", "error"];
  var handled = false;
  for (var j = 0; j < fields.length; j++) {
    if (value[fields[j]] == null) continue;
    appendRichResult(value[fields[j]], text, images);
    handled = true;
  }
  if (!handled && value.name) text.push(String(value.name));
}

function flattenRichToolResult(block, base) {
  var text = [];
  var images = [];
  appendRichResult(block.content != null ? block.content : block, text, images);
  base.yokeType = "tool_result";
  base.toolId = block.tool_use_id || block.toolUseId || block.id;
  base.content = text.join("\n");
  base.images = images;
  base.isError = !!block.is_error || /_error$/.test(block.type);
  return base;
}
...
function flattenEvent(raw) {
  ...
  if (raw.type === "stream_event" && raw.event) {
    ...
    if (evt.type === "content_block_start" && evt.content_block) {
      ...
      } else if (isRichToolResultBlock(block)) {
        return flattenRichToolResult(block, base);
...
// Normalize the structured data behind Claude Code's `/usage` command into
// the same YOKE events as live SDK rate-limit warnings. The SDK exposes this
// only for eligible claude.ai plan sessions; callers get an empty list for
// API-key/provider auth or tokens without the required profile scope.
function rateLimitEventsFromUsage(usage) {
  if (!usage || !usage.rate_limits_available || !usage.rate_limits) return [];
  var windows = [
  ...
  for (var i = 0; i < windows.length; i++) {
    var windowInfo = usage.rate_limits[windows[i].key];
    if (!windowInfo || !windowInfo.resets_at) continue;
    var resetsAtMs = Date.parse(windowInfo.resets_at);
    if (!isFinite(resetsAtMs)) continue;
    events.push(flattenEvent({
      type: "rate_limit_event",
      rate_limit_info: {
        status: "allowed",
        resetsAt: Math.floor(resetsAtMs / 1000),
        rateLimitType: windows[i].type,
        utilization: typeof windowInfo.utilization === "number" ? windowInfo.utilization / 100 : null,
...
module.exports = {
  ...
  rateLimitEventsFromUsage: rateLimitEventsFromUsage,
};

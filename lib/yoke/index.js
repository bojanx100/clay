...
var additionalVendors = require("./additional-vendors");
...
function createAdapter(opts) {
  ...
  if (vendor === "claude") {
    ...
    adapter = additionalVendors.createAdditionalAdapter(vendor, opts);
    if (!adapter) throw new Error("[YOKE] Unknown adapter vendor: " + vendor);
  }
...
function logAuthCheck(auth) {
  ...
  _lastAuthLogKey = key;
  _lastAuthLogAt = now;
  var parts = Object.keys(auth || {}).sort().map(function(vendor) {
    return vendor + "=" + auth[vendor];
  });
  console.log("[yoke] Auth check: " + parts.join(" "));
}
...
function checkAuth(opts) {
  ...
  if (opts.linuxUser) {
    try {
      ...
      return additionalVendors.appendAdditionalAuth(
        { claude: false, codex: false, "github-copilot": false }, {}, true);
    }
  }
  ...
  additionalVendors.appendAdditionalAuth(auth, checkInstalled(), !!osUserInfo);
...
function checkInstalled() {
  ...
  var result = Object.assign(
    { claude: false, codex: false, "github-copilot": false },
    additionalVendors.emptyVendorMap());
  try {
  ...
  Object.assign(result, additionalVendors.checkAdditionalInstalled());
...
function createAdapters(opts) {
  ...
  additionalVendors.createInstalledAdditionalAdapters(
    adapters, auth, installed, opts, supportsConfiguredIsolation);
 ...
 * Re-checks installation state and creates the adapter. Initialization and
 * discovery are owned by sdk-bridge-vendor-readiness so every caller shares
 * one lifecycle and one in-flight promise.

...
function candidateReconsiderationProof(existing, evidence, options) {
  ...
  if (!existing.ownerDecision && (existing.status === "pending" ||
      existing.admission === "owner_approval" && !existing.qualificationReceipt)) {
...
function prepareCandidateReconsideration(existing, evidence, options, timestamp) {
  ...
  if (!existing || (existing.status !== "admitted" && existing.status !== "pending")) {

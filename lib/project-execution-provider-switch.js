...
function createProjectExecutionProviderSwitch(deps) {
  return function switchProjectExecutionProvider(input) {
    ...
    // Routed executions belong to the resident controller; source remains the authorized caller.
    var projectCoordinator = projectIdentity.normalizeSessionRef(binding.projectCoordinator);
    var residentControlled = binding.mode === "project_coordinator" &&
      projectCoordinator && projectCoordinator.projectId === projectIdentity.LEAD_PROJECT_ID;
    var controller = residentControlled ? projectCoordinator : source;
    if (!controlledBy || controlledBy.coopSessionStorageId !== controller.sessionStorageId ||
        ...
        (residentControlled &&
          (!portfolioBindings.sameSessionRef(session.projectCoordinatorRef, controller) ||
           !portfolioBindings.sameSessionRef(execution.source, controller)))) {

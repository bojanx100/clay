var ROUTES = [
  ...
  },
  {
    ...
    vendor: "antigravity",
    provider: "google",
    label: "Antigravity via Google",
    description: "Use the locally authenticated Antigravity CLI.",
    executable: true,
    defaultModel: "auto",
    allowMetaModel: true,
    defaultCapabilityTier: 2,
    freeAllowancePotential: true,
  },
  {
    id: "opencode-local",
    vendor: "opencode",
    provider: "opencode",
    label: "OpenCode",
    description: "Use OpenCode with its configured model providers.",
    executable: true,
    defaultModel: "auto",
    allowMetaModel: true,
    defaultCapabilityTier: 1,
    freeAllowancePotential: true,
  },
  {
    id: "kimi-moonshot",
    ...
    provider: "moonshot",
    label: "Kimi Code",
    description: "Use the locally authenticated Kimi Code CLI.",
    executable: true,
    defaultModel: "auto",
    allowMetaModel: true,
    defaultCapabilityTier: 2,
  ...
  },
  {
    ...
    provider: "xai",
    label: "Grok Build",
    description: "Use the locally authenticated Grok Build CLI.",
    executable: true,
    defaultModel: "auto",
    allowMetaModel: true,
    defaultCapabilityTier: 2,
  ...
  },
  {
    ...
    provider: "alibaba",
    label: "Qwen Code",
    description: "Use the locally authenticated Qwen Code CLI.",
    executable: true,
    defaultModel: "auto",
    allowMetaModel: true,
    defaultCapabilityTier: 2,
  ...
  },
  {
    ...
    provider: "jetbrains",
    label: "Junie CLI",
    description: "Use the locally authenticated Junie CLI.",
    executable: true,
    defaultModel: "auto",
    allowMetaModel: true,
    defaultCapabilityTier: 2,
  ...
  },
  {
    ...
    provider: "aws",
    label: "Kiro CLI",
    description: "Use the locally authenticated Kiro CLI.",
    executable: true,
    defaultModel: "auto",
    allowMetaModel: true,
    defaultCapabilityTier: 2,
...
function listProviderRoutes(availableVendors, installedVendors, state) {
  ...
  for (var i = 0; i < ROUTES.length; i++) {
    ...
    var catalog = verifiedCatalogForRoute(route, state);
...
function concreteModelCatalog(models, family, allowMetaModel) {
  var values = [];
  var entries = [];
  var seen = {};
  ...
  for (var i = 0; i < (models || []).length; i++) {
    ...
    if (!value || !allowMetaModel &&
        (value === "default" || value === "best" || value === "auto")) continue;
    value = String(value);
...
function concreteModels(models, family, allowMetaModel) {
  return concreteModelCatalog(models, family, allowMetaModel).models;
}
...
function catalogFromRuntimeRoutes(route, state) {
  ...
  for (var ri = 0; ri < routes.length; ri++) {
    ...
    if (matching && (matching.catalogVerified === true || legacyInjected)) {
      ...
      // A route decorated from the durable last-known-good catalog does not
      // turn its runtime model copy into new route evidence. Fall through to
      // that durable catalog so its live-discovery provenance remains attached;
      // otherwise a rich Codex list identical to the cold-start seed is rejected
      // below even though the persisted catalog was verified.
      if (matching.catalogSource === "last-known-good") return null;
      ...
      if (concreteModels(liveModels, route.modelFamily, route.allowMetaModel).length === 0) return null;
...
function verifiedNativeCatalog(route, models, source, provenance) {
  ...
  var catalog = concreteModelCatalog(models || [], route.modelFamily, route.allowMetaModel);
...
function verifiedCatalogForRoute(route, state) {
  ...
  if (route.provider === "github-copilot") {
    ...
    var snapshot = copilotEntitlements.currentCopilotEntitlements();
    var copilotCatalog = concreteModelCatalog(snapshot && snapshot.models || [], route.modelFamily, route.allowMetaModel);

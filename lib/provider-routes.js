...
function concreteModels(models, family) {
  ...
  for (var i = 0; i < (models || []).length; i++) {
    ...
    if (!modelMatchesFamily(String(value).toLowerCase(), family)) continue;
...
function sameModelSet(left, right, family) {
  var leftModels = concreteModels(left, family).slice().sort();
  var rightModels = concreteModels(right, family).slice().sort();
...
function looksLikeStaticCodexSeed(models) {
  ...
  for (var i = 0; i < models.length; i++) {
    var entry = models[i];
    if (!entry || typeof entry !== "object" || entry.id || entry.model ||
...
function verifiedNativeCatalog(route, models, source) {
  ...
  if (route.vendor === "codex" && source === "last-known-good" &&
      (sameModelSet(models, fallbackCodexModels(), route.modelFamily) || looksLikeStaticCodexSeed(models))) {
    return { models: [], source: null };
  }
  return {
    models: concreteModels(models || [], route.modelFamily),
    source: models && models.length ? source : null,
  };
}
...
function withProbedClaudeCapability(route, catalog, state) {
  ...
  return {
    models: catalog.models.concat(concreteModels(extras, route.modelFamily)),
    source: catalog.source ? catalog.source + "+exact-probe" : "exact-probe",
  };
}
...
function verifiedCatalogForRoute(route, state) {
  ...
  if (route.provider === "github-copilot") {
    ...
    var snapshot = copilotEntitlements.currentCopilotEntitlements();
    return {
      models: concreteModels(snapshot && snapshot.models || [], route.modelFamily),
      source: snapshot && snapshot.source || "live",
    };
  }
  var cached = modelCatalogCache.cachedModels(route.vendor);
  return withProbedClaudeCapability(route,
    verifiedNativeCatalog(route, cached || [], "last-known-good"), state);
}

...
function isImplementationIntent(text) {
  return !!lifecycle.explicitImplementationDecision(text);
}
...
function latestOwnerMessageIndex(history) {
  ...
  for (var i = items.length - 1; i >= 0; i--) {
    ...
    // Scheduled ticks and other machine-injected user_message records share
    // the carrier type but are not owner context. Skip them while walking back
    // to the latest real owner turn; treating one as a hard boundary makes a
    // clear Main follow-up lose the current Thread after background activity.
    if (relevance.isInternalHistoryItem(item) || !relevance.hasOwnerProvenance(item)) continue;
    return i;
  }
  return -1;
}

function anchorDecision(index, state, session, anchor, historyView) {
  var normalized = replyAnchor.normalizeReplyAnchor(anchor);
  var storageId = projectIdentity.sessionStorageId(session);
  var record = null;
  if (historyView && Array.isArray(historyView.entries) && Array.isArray(historyView.history)) {
    for (var hi = 0; hi < historyView.entries.length; hi++) {
      var entry = historyView.entries[hi];
      if (entry.sessionStorageId === (normalized && normalized.sessionStorageId) &&
          entry.eventIndex === (normalized && normalized.eventIndex)) {
        record = historyView.history[entry.historyIndex];
        break;
      }
    }
  } else if (normalized && normalized.sessionStorageId === storageId) {
    record = session.history && session.history[normalized.eventIndex];
  }
  var fingerprint = record && replyAnchor.fingerprintOf(record);
  var matches = normalized && fingerprint && fingerprint.type === normalized.type &&
    (normalized.ts === null || fingerprint.ts === normalized.ts) &&
    (!normalized.clientMessageId || fingerprint.clientMessageId === normalized.clientMessageId);
  if (!matches) {
  ...
  if (!eventInTopic(topic, normalized.sessionStorageId, normalized.eventIndex)) {
...
function resolveDominantTarget(index, session, options) {
  ...
  var historyView = opts.historyView && Array.isArray(opts.historyView.history)
    ? opts.historyView : null;
  var history = historyView ? historyView.history : (Array.isArray(session && session.history) ? session.history : []);
  var entries = historyView && Array.isArray(historyView.entries) ? historyView.entries.slice() : [];
  if (!historyView) {
    for (var hi = 0; hi < history.length; hi++) {
      entries.push({ historyIndex: hi, sessionStorageId: storageId, eventIndex: hi });
    }
  }
  var canonicalEvidence = !!(state && state.canonicalSessionStorageId === storageId);
  if (!canonicalEvidence && state) {
    for (var ei = 0; ei < entries.length; ei++) {
      if (entries[ei].sessionStorageId === state.canonicalSessionStorageId) {
        canonicalEvidence = true;
        break;
      }
    }
  }
  if (!session || !session.coopHome || !storageId || !state || !canonicalEvidence) {
    return { ok: false, code: "canonical_coop_required" };
  }

  if (hasOwn.call(opts, "replyAnchor") && opts.replyAnchor != null) {
    return anchorDecision(index, state, session, opts.replyAnchor, historyView);
  }
  ...
  for (var i = history.length - 1; i > ownerIndex; i--) {
    var historyEntry = entries[i] || { sessionStorageId: storageId, eventIndex: i };
    var assistantCandidates = candidatesAtEvent(state, historyEntry.sessionStorageId, historyEntry.eventIndex);
...
module.exports = {
  ...
  isImplementationIntent: isImplementationIntent,
  parse: parse,

...
// A canonical turn anchor is the durable identity of an automatic Thread. The
// readable title is deliberately absent from this key: title refinement,
// alternate summaries, and owner renames must never create a second identity.
// The no-evidence branch is retained only for legacy callers that manufacture
// fixture topics before a canonical turn exists; production ingress and retro
// indexing always provide the anchor.
function identityKey(identity) {
  if (!identity || typeof identity !== "object") return "";
  var projectId = identity.projectId || projectIdentity.LEAD_PROJECT_ID;
  if (projectId !== projectIdentity.LEAD_PROJECT_ID ||
      !projectIdentity.isSessionStorageId(identity.sessionStorageId) ||
      !Number.isInteger(identity.eventIndex) || identity.eventIndex < 0) return "";
  return "canonical-turn\n" + projectId + "\n" + identity.sessionStorageId + "\n" + identity.eventIndex;
}

function topicIdentityKey(topic) {
  if (!topic || typeof topic !== "object") return "";
  var identity = topic.threadIdentity && topic.threadIdentity.eventRef;
  var direct = identityKey(identity);
  if (direct) return direct;
  var refs = Array.isArray(topic.turnRefs) ? topic.turnRefs.slice() : [];
  refs = refs.filter(function (ref) {
    return !!identityKey({ projectId: ref && ref.projectId,
      sessionStorageId: ref && ref.sessionStorageId, eventIndex: ref && ref.startEventIndex });
  }).sort(function (left, right) {
    return left.startEventIndex - right.startEventIndex ||
      String(left.sessionStorageId).localeCompare(String(right.sessionStorageId));
  });
  if (!refs.length) return "";
  return identityKey({ projectId: refs[0].projectId,
    sessionStorageId: refs[0].sessionStorageId, eventIndex: refs[0].startEventIndex });
}

function automaticTopicId(text, group, identity) {
  var stableKey = identityKey(identity);
  var source = stableKey || (groupKey(group) + "\n" + normalizeText(derivedMetadata(text).title));
  var digest = crypto.createHash("sha256").update(source).digest("hex");
...
function sealedExisting(index, text, preferredGroup, options) {
  var input = matchableKeywords(keywords(text));
  if (input.length < STRONG_OVERLAP) return null;
  var ids = Object.keys(index.topics || {}).sort();
  for (var i = 0; i < ids.length; i++) {
    var topic = index.topics[ids[i]];
    if (!reusableTopic(topic) || openTopic(topic) || topic.status === "merged") continue;
    if (!groupMatches(topic, preferredGroup) || !projectAllowed(topic, options.canAccessProject)) continue;
    if (overlap(input, matchableKeywords(topicKeywords(topic))) >= STRONG_OVERLAP) return topic;
  }
  return null;
}
...
function ensureAutomatic(index, text, group, options) {
  ...
  var identity = options.identityEvidence || null;
  var stableKey = identityKey(identity);
  var ids = Object.keys(index.topics || {}).sort();
  if (stableKey) {
    for (var i = 0; i < ids.length; i++) {
      var candidate = index.topics[ids[i]];
      if (!reusableTopic(candidate) || !openTopic(candidate) || topicIdentityKey(candidate) !== stableKey) continue;
      if (!projectAllowed(candidate, options.canAccessProject)) continue;
      return { topic: candidate, created: false, identityMatched: true };
    }
  }
  var id = automaticTopicId(text, group, identity);
  ...
  if (stableKey) {
    topic.threadIdentity = {
      schemaVersion: 1, kind: "canonical_turn",
      eventRef: { projectId: projectIdentity.LEAD_PROJECT_ID,
        sessionStorageId: identity.sessionStorageId, eventIndex: identity.eventIndex },
    };
    topic.titleRefinement = { version: 1, managed: true, manual: false };
  }
...
function classifyIngress(index, text, preferredGroup, options) {
  ...
  var sealed = sealedExisting(index, text, preferredGroup, options);
  if (sealed) {
    fallback = openCatchAll(index, preferredGroup, options);
    if (fallback) return { ok: true, topic: fallback, created: false };
  ...
  if (!usable(automatic.topic, group, options.canAccessProject) && !automatic.identityMatched) {
...
function topicsForTurn(index, turn, options) {
  ...
  if (!explicit && !seedMatched && !sealed && ownerRelevant) {
    var identityEvidence = {
      ...
      sessionStorageId: turn.sessionStorageId || options.sessionStorageId,
      eventIndex: turn.startEventIndex,
    };
    var classified = classifyIngress(index, turn.userText || turn.text, preferredGroup,
      Object.assign({}, options, { identityEvidence: identityEvidence }));
    if (classified.ok) {
...
// Whether this automatic topic's title is still machine-managed. New rows use
// canonical turn identity, so an owner rename is tracked explicitly rather
// than inferred from the title. Legacy rows retain the old fingerprint check
// until their next proven lineage replay.
function isUnmodifiedAutomaticTitle(topic) {
  ...
  if (topic.threadIdentity && topic.threadIdentity.eventRef) {
    return topic.titleManuallySet !== true && !(topic.titleRefinement && topic.titleRefinement.manual === true);
  }
...
module.exports = {
  ...
  topicIdentityKey: topicIdentityKey,

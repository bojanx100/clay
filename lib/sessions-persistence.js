...
function metadataFingerprint(metaObj, viewStateDurable) {
  var comparable = Object.assign({}, metaObj);
  delete comparable.lastActivity;
  if (viewStateDurable) delete comparable.lastViewedAt;
...
function writeSessionJsonlSync(tmpPath, meta, history) {
  ...
  var chunkChars = 0;
  var bytesWritten = 0;
  // Count HISTORY records actually written, excluding the meta line. Deltas are
  // coalesced on the way to disk, so this is smaller than history.length -- and
  // it, not history.length, is what a later range read has to index against.
  ...
  return { bytes: bytesWritten, records: recordsWritten };
}
...
function attachSessionPersistence(ctx) {
  ...
  // Every appended history item has already reached the JSONL file. A later
  // save therefore needs a full rewrite only when metadata other than
  // append-driven activity or separately durable view recency changed, or when
  // a caller changed the transcript in place. Keeping this as the serialized
  // comparison preserves JSON's omission semantics for undefined metadata.
  function historyAlreadyPersisted(session, history) {
    return session._persistedHistoryLength === history.length &&
      !session._historyNeedsRewrite;
  }
  ...
  function saveSessionFile(session, options) {
    ...
    if (!usableSession(session, durable)) return false;
    var storageId = storageIdForSave(session, durable);
    if (!storageId) return false;
    if (options && options.viewOnly === true) {
      return !!(viewState && viewState.persist(storageId, session.lastViewedAt));
    }
  ...
  function writeSessionFileNow(session, throwOnFailure) {
    ...
    var viewStateDurable = !session.lastViewedAt ||
      !!(viewState && viewState.persist(storageId, session.lastViewedAt));
    try {
      var metaObj = {
        type: "meta",
      ...
      if (typeof session._historyFormatVersion === "number") metaObj.historyFormatVersion = session._historyFormatVersion;
      ...
      if (session.contextRecovery) metaObj.contextRecovery = session.contextRecovery;
      ...
      if (session.coordinatorContextReceipt) metaObj.coordinatorContextReceipt = session.coordinatorContextReceipt;
      ...
      var comparableMeta = metadataFingerprint(metaObj, viewStateDurable);
      ...
      if (session._persistedMetaFingerprint === comparableMeta &&
          typeof session._persistedHistoryLength === "number" &&
          !session._historyNeedsRewrite && historyStore.isLazy(session) && !historyStore.isResident(session)) {
        return true;
      }
      ...
      if (session._persistedMetaFingerprint === comparableMeta &&
          historyAlreadyPersisted(session, history)) {
        return true;
      }
      var sfPath = sessionFilePath(storageId);
      var tmpPath = sfPath + ".tmp." + process.pid;
      var _saveT0 = Date.now();
      var _saveResult = writeSessionJsonlSync(tmpPath, meta, history);
      var _saveBytes = _saveResult.bytes;
      ...
      // _persistedHistoryLength keeps its meaning: the IN-MEMORY length at the
      // last persist, which is what historyAlreadyPersisted compares against to
      // decide whether a full rewrite is needed. Changing it to the disk count
      // would make that comparison permanently unequal for any session with
      // coalesced deltas, forcing a full rewrite on every single save.
      session._persistedHistoryLength = history.length;
      // The disk truth is tracked separately, because deltas are coalesced on
      // write and readHistoryRangeFromDisk indexes backwards from EOF. When the
      // range reader used the in-memory count it computed a negative start and
      // refused, silently falling back to a full transcript read for every
      // session saved this run -- exactly the sessions the owner switches
      // between, which is why the paged replay never took effect for them.
      session._persistedDiskRecords = _saveResult.records;
      session._persistedMetaFingerprint = comparableMeta;
  ...
  function appendToSessionFile(session, obj) {
    ...
    try {
      ...
      if (session._persistedHistoryLength === session.history.length - 1 &&
          session.history[session.history.length - 1] === obj) {

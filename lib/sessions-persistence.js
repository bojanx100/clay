...
function attachSessionPersistence(ctx) {
  ...
  // save therefore needs a full rewrite only when metadata other than the
  // append-driven activity timestamp changed, or when a caller changed the
  // transcript in place. Keeping this as the serialized comparison preserves
  // JSON's omission semantics for undefined optional metadata.
  function metaWithoutActivity(metaObj) {
  ...
  function writeSessionFileNow(session, throwOnFailure) {
    ...
    try {
      ...
      var comparableMeta = metaWithoutActivity(metaObj);
      ...
      if (session._persistedMetaWithoutActivity === comparableMeta &&
      ...
      session._persistedMetaWithoutActivity = comparableMeta;

...
function openLeadItems() {
  ...
  var items;
  try { items = JSON.parse(fs.readFileSync(file, "utf8")); } catch (e) { return []; }
  return Array.isArray(items) ? items : [];
}

function defaultHasPendingWork() {
  var items = openLeadItems();
  for (var i = 0; i < items.length; i++) {
    var state = items[i] && (items[i].state || items[i].status);
    if (state === "open" || state === "pending" || state === "ready") return true;
  }
  try { return require("./lead-ledger").inFlight().length > 0; } catch (e) { return false; }
}
...
function homeIsIdle(home) {
  return !!home && !home.destroying && !home.isProcessing && !home.queryInstance && !home.scheduledMessage;
}
...
function createLeadWakeHandler(ctx) {
  ...
  return function (tick) {
    ...
    if (typeof scheduleMessage !== "function" || !hasPendingWork()) return false;
    var home = coopHomeSession(ctx.sm);
    if (!homeIsIdle(home)) return false;
    scheduleMessage(home, "lead tick", now(), LEAD_WAKE_PROMPT, "↻ Lead tick", { autoAction: true });
    return true;
  };
}
...
function attachCoopSelfCleanupRuntime(ctx) {
  ...
  function basicSessionGuard(session) {
    ...
    if (isCurrentActiveSession(session)) return "active_session";

...
function taskState(root, task) {
  ...
  return {
    ...
    resultSummary: task.resultSummary || task.resolutionSummary || "",
    verification: task.verification || "", resolvedAt: task.resolvedAt || null,
    resolvedByCoordinator: task.resolvedByCoordinator === true,
...
function workerState(ref, projectContexts, rootRef) {
  ...
  var execution = worker.orchestrationPolicy && worker.orchestrationPolicy.portfolioExecution || {};
  return { sessionRef: ref, available: true, processing: !!worker.isProcessing,
    lastActivity: worker.lastActivity || null, executionStatus: execution.status || null,
    executionReason: execution.reason || execution.statusReason || execution.completionRefusalReason || "",
    contextRecovery: copy(worker.contextRecovery),

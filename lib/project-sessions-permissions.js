...
function attachProjectSessionsPermissions(ctx) {
  ...
  var handleVoiceQuestions = require("./voice-questions").attachVoiceQuestions(ctx, handlePermissionsMessage);

  function handlePermissionsMessage(ws, msg) {
    if (handleVoiceQuestions(ws, msg)) return true;
    if (msg.type === "ask_user_response") {
      ...
      if (!pending) {
        ...
        if (msg.ingressType === "voice" && ctx.getUserMessage) {
          ctx.getUserMessage().handleUserMessage(ws, {
            type: "message", text: answerText, sessionId: session.localId, intent: "chat",
            ingressType: "voice", clientMessageId: msg.clientMessageId,
            coopComposerScope: msg.coopComposerScope,
            coopTopicRef: msg.coopTopicRef, coopProjectRef: msg.coopProjectRef,
          });
        } else sendUserMessageToAgent(session, answerText);
      } else {
    ...
    if (msg.type === "permission_response") {
      ...
      var session = msg.ingressType === "voice" ? getSessionForWs(ws) :
        sessionId ? sm.sessions.get(sessionId) : getSessionForWs(ws);
      if (!session) return true;
      ...
      if (decision === "deny_with_feedback") {
        ...
        if (feedback && msg.ingressType !== "voice") {
          setTimeout(function () {

...
var emergencyRepairBridge = require("./coop-emergency-repair-bridge");
...
function isRejected(task) {
  var acceptance = task && task.ownerAcceptance;
  return !!acceptance && acceptance.status === "rejected";
}

// request_changes has two distinct meanings depending on what it is aimed at.
// Against blocked work it is an answer to an open question. Against finished
// work awaiting acceptance it is the owner REJECTING that work, which is the
// other half of the acceptance decision and has to be recorded as such.
function isAcceptanceRejection(task) {
  return isCompleted(task) && !isAccepted(task);
}
...
function updatesFor(decision, note, now, existingOwnerDecision, task) {
  if (decision === "keep_waiting") return null;
  // Every verdict transition is also appended as a typed event. `ownerAcceptance`
  // is last-write-wins, so without these an accept following a rejection would
  // erase the fact that the owner ever rejected the work.
  var priorEvents = task && task.ownerAcceptanceEvents;
  function withAcceptance(acceptance, activity, eventNote) {
    return {
      ownerAcceptance: acceptance,
      ownerAcceptanceEvents: acceptanceEvents.appendForAcceptance(priorEvents, acceptance, {
        at: now, source: "owner_decision", note: eventNote,
      }),
  ...
  if (decision === "accept") {
    return withAcceptance({ status: "accepted", at: now, withdrawnAt: null },
      "Accepted by the owner", "");
  }
  if (decision === "revoke_acceptance") {
    return withAcceptance({ status: "accepted", at: now, withdrawnAt: now },
      "Owner reopened this after accepting it", "");
  }
  // Rejecting finished work. Symmetric with accept: it records an owner verdict
  // and deliberately leaves `status` alone, because the owner rejecting the
  // result is not a claim that the implementation was un-done. Moving it to
  // "reviewing" here would also destroy the completion evidence that makes the
  // work re-acceptable once the coordinator responds.
  if (decision === "request_changes" && isAcceptanceRejection(task)) {
    var rejection = withAcceptance({ status: "rejected", at: now },
      "Owner rejected this; coordinator is reworking", cleanText(note));
    rejection.ownerDecision = { kind: decision, note: cleanText(note), at: now };
    rejection.userAnsweredAt = now;
    return rejection;
  }
...
function applyDecision(deps) {
  ...
  } else if (verb === "request_changes" && isAcceptanceRejection(hit.task)) {
    // Rejecting verified work. Previously this fell through to the
    // ATTENTION_STATUSES check below, which does not list "completed", so the
    // owner's rejection was refused as already_decided and the item went on
    // reporting that it awaited their acceptance.
    if (isRejected(hit.task)) return result(false, "already_decided", { status: hit.task.status });
  ...
  if (!orchestrator || typeof orchestrator.recordOwnerDecision !== "function") {
    // This remains a strict opt-in. The bound policy is created from one exact
    // owner-approved ProjectRef/task/revision and rejects every other action.
    // No missing orchestrator may turn into ordinary task creation.
    var repair = emergencyRepairBridge.startRepair({
      actorId: options.actorId,
      decision: { decision: request.decision, note: request.note,
        directive: directiveFor(request.decision, hit.task, request.note) },
      observedAt: typeof options.now === "function" ? options.now() : Date.now(),
      policy: options.emergencyRepairPolicy,
      projectRef: request.projectRef,
      session: hit.session,
      task: hit.task,
    });
    if (repair && repair.ok) {
      return result(false, "emergency_repair_started", {
  ...
  var applied = orchestrator.recordOwnerDecision(hit.session, request.taskId, {
    updates: updatesFor(request.decision, request.note, now, hit.task.ownerDecision, hit.task),
    directive: directiveFor(request.decision, hit.task, request.note),
  });
  ...
  return result(true, "", {
    ...
    status: verb === "accept" || verb === "revoke_acceptance" ||
      (verb === "request_changes" && isAcceptanceRejection(hit.task)) ?
      hit.task.status : "reviewing",
    changed: true,
  });
}

...
function controlMetadata(session) {
  var incarnation = session && session.coopHome && session.coopIncarnation;
  if (incarnation) {
    return {
...
function attachCoopFence(session, fence) {
  var metadata = metadataForFence(fence);
  if (!session || !session.coopHome || !session.coopIncarnation ||
      !sameRefs(controlMetadata(session), metadata)) {
    throw error("COOP_CONTROL_FENCE_REJECTED", "Canonical Coop fence is invalid.");
  }
  return attachFence(session, fence);
}
...
function fenceFor(session, suppliedFence) {
  ...
  if (metadata && !fence && !session.coopIncarnation &&
      !executions.isExecutionControlEnabled()) return null;
...
module.exports = {
  ...
  attachCoopFence: attachCoopFence,

...
function attachTaskOrchestrator(ctx) {
  ...
  function finishWorkerTurn(parentSession, task, worker) {
    ...
    var taskUpdates = {
      ...
      resolutionSummary: status === "completed" ? structuredField(resultText, "SUMMARY") : "",
  ...
  function dismissTask(parentSession, task, reason) {
    return closeOrchestrationTask(ctx, parentSession, task.taskId, null, {
      reason: reason,
    });
  }
  ...
  function requestTaskInput(parentSession, tasks, question, reason) {
    for (var i = 0; i < tasks.length; i++) {
      taskGraph.transition(parentSession, tasks[i], "waiting_user", {

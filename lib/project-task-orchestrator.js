...
function attachTaskOrchestrator(ctx) {
  ...
  var toolHandlers = createToolHandlers({
    afterResolve: function (parentSession, resolvedTask) { orchestratorHelpers.archiveTaskWorker(sm, parentSession, resolvedTask, workerForTask(resolvedTask), "Resolved by coordinator"); coordinatorDemotion.completePending(parentSession); },
    ...
    isProjectExecutionInput: externalOrchestration.hasProjectExecutionInput,
  });
  ...
  return {
    ...
    deliverCrossProjectEnvelope: function (envelope) {
      if (envelope && envelope.schema === externalOrchestration.COMMAND_SCHEMA) {

...
function attachTaskOrchestrator(ctx) {
  ...
  function coordinateQueuedMessage(parentSession, queueItem) {
    if (!parentSession || !queueItem || !String(queueItem.text || "").trim()) return null;
    parentSession.coordinationMode = true;
    var storageId = storageIdForSession(parentSession) || parentSession.localId;
    var activation = coordinatorActivationPrompt(storageId, String(queueItem.text).trim());
    queueCoordinatorUpdate(parentSession, activation);
    sm.saveSessionFile(parentSession);
    return {
      taskId: null,
      status: "reviewing",
      title: taskTitleFromText(queueItem.displayText || queueItem.text),
    };
  }

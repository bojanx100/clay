...
function attachTaskOrchestrator(ctx) {
  ...
  return {
    ...
    // One task, by id. The pre-existing owner-answer path
    // (resumeWaitingFromUser) transitions every waiting_user task in the
    // session at once, so routing an Action-required decision through it would
    // silently answer the owner's other open questions too.
    recordOwnerDecision: function (parentSession, taskId, decision) {
      ...
      // Queued after the state change so the coordinator reads the task in its
      // decided state rather than racing the update.
    ...
    listCoopSessionsFromTool: coopSessionQuery.listFromTool,

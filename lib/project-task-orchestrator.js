...
var createExternalTaskCoordinator =
  require("./project-task-orchestrator-external").createExternalTaskCoordinator;
...
function attachTaskOrchestrator(ctx) {
  ...
  // Cross-project delivery surface consumed by server-cross-project's
  // router: resolve a session in THIS project scope by storage id and
  // queue a coordinator update on it. Returns true when delivered.
  function deliverCoordinatorUpdate(sessionStorageId, text) {
    var session = sessionByStorageId(String(sessionStorageId || ""));
    if (!session) return false;
    queueCoordinatorUpdate(session, text);
    return true;
  }
  ...
  function retryExistingWorker(parentSession, task) {
    ...
    taskGraph.transition(parentSession, task, "running", {
      ...
      verification: "",
      progress: 0,
      currentActivity: "Worker session " + worker.localId + " is retrying",
    });
  ...
  function messageFromTool(input) {
    ...
    if (task.status === "completed") return toolError("task is already completed; use retry_task for another pass");
    var worker = workerForTask(task);

...
var { orchestrationTasksForClient, workerPrompt, workerResultUpdateText, workerResultText,
  workerStatusFromResult, isVerifiedCompletion } = require("./orchestration-task-state");
function attachTaskOrchestrator(ctx) {
  ...
  function hasActiveTasks(parentSession) {
    ...
    return tasks.some(function (task) {
      ...
      return status === "queued" || status === "ready" || status === "running" || status === "reviewing";
    });
  }
  function completePendingDemotion(parentSession) {
    if (!parentSession || !parentSession.demoteCoordinatorWhenIdle || hasActiveTasks(parentSession)) return;
    parentSession.coordinationMode = false;
    parentSession.demoteCoordinatorWhenIdle = false;
    sm.saveSessionFile(parentSession);
    sm.broadcastSessionList();
    sendToSession(parentSession.localId, { type: "coordinator_status", coordinationMode: false, demotionPending: false });
  }
  ...
  function finishWorkerTurn(parentSession, task, worker) {
    ...
    scheduleReadyTasks(parentSession);
    completePendingDemotion(parentSession);
  }
  ...
  return {
    ...
    closeTask: function (parentSession, taskId, targetWs) {
      return closeOrchestrationTask(ctx, parentSession, taskId, targetWs);
    },
    ...
    resolveTask: function (parentSession, taskId, summary, verification) {
      return toolHandlers.resolve({
        coordinatorSessionId: storageIdForSession(parentSession),
        taskId: taskId,
        summary: summary,

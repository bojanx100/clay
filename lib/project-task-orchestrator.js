...
function attachTaskOrchestrator(ctx) {
  ...
  var toolHandlers = createToolHandlers({
    afterResolve: function (parentSession, task) {
      coordinatorDemotion.completePending(parentSession);
      if (task && task.externalTaskCoordinator && task.resolvedByCoordinator &&
          crossProject && crossProject.resolveProjectCoordinatorTask) {
        return crossProject.resolveProjectCoordinatorTask({ taskId: task.taskId,
          source: { projectId: sm.getProjectId(), sessionStorageId: parentSession.storageId } });
      }
    },

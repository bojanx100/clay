...
// Only these keys may appear inside the explicit `automation` block. Provider
// rules and the recipe inventory are always evidence from the project's own
// files; candidateEligibility is the bounded machine-readable migration path
// for a workflow rule that has not yet been represented by a launcher recipe.
var AUTOMATION_KEYS = ["autonomy", "externalActions", "candidateEligibility", "qualification"];
var CANDIDATE_ELIGIBILITY_KEYS = ["boardExclusions"];
...
function digestSubject(policy) {
  return {
    ...
    qualification: (policy && policy.qualification) || null,
    providerRules: (policy && policy.providerRules) || {},
    recipes: (policy && policy.recipes) || [],
  };
}

// The exact launcher evidence Lead must match before it can reuse a recipe.
// This intentionally covers only the fields that change which GitHub issues a
// launcher is allowed to discover. Session/prompt presentation fields are not
// eligibility authority and must not make a policy digest churn.
function recipeDigest(recipe) {
  var value = recipe && typeof recipe === "object" ? recipe : {};
  var subject = {
    id: typeof value.id === "string" ? value.id : "",
    source: isPlainObject(value.source) ? value.source : {},
    filter: isPlainObject(value.filter) ? value.filter : {},
  };
...
function buildPolicy(ref, derived, autonomy, externalActions, boardExclusions, qualificationPolicy, vendors, recipes, sources) {
  var policy = {
...
function restrictiveDefaultPolicy(ref) {
  return buildPolicy(ref, true, restrictiveAutonomy(), restrictiveExternalActions(), [], null, {}, [], []);
}
...
function parseExplicit(automation) {
  ...
  var boardExclusions = [];
  if (Object.prototype.hasOwnProperty.call(automation, "candidateEligibility")) {
    var candidateEligibility = automation.candidateEligibility;
    if (!isPlainObject(candidateEligibility)) return null;
    var eligibilityKeys = Object.keys(candidateEligibility);
    for (i = 0; i < eligibilityKeys.length; i++) {
      if (!contains(CANDIDATE_ELIGIBILITY_KEYS, eligibilityKeys[i])) return null;
    }
    if (Object.prototype.hasOwnProperty.call(candidateEligibility, "boardExclusions")) {
      if (!Array.isArray(candidateEligibility.boardExclusions)) return null;
      var seenStatuses = {};
      for (i = 0; i < candidateEligibility.boardExclusions.length; i++) {
        var status = candidateEligibility.boardExclusions[i];
        if (typeof status !== "string" || !status.trim()) return null;
        var normalized = status.trim().toLowerCase();
        if (seenStatuses[normalized]) continue;
        seenStatuses[normalized] = true;
        boardExclusions.push(normalized);
      }
      boardExclusions.sort();
    }
  }

  // New issue implementation authority requires this typed profile. Keeping a
  // missing profile as `null` preserves the ability to read older policies for
  // non-issue work while making issue qualification fail closed at the gate.
  var qualificationPolicy = null;
  if (Object.prototype.hasOwnProperty.call(automation, "qualification")) {
    qualificationPolicy = qualification.normalizePolicy(automation.qualification);
    if (!qualificationPolicy) return null;
  }
...
function mergeBoardExclusions() {
  var seen = {};
  var out = [];
  for (var i = 0; i < arguments.length; i++) {
    var list = Array.isArray(arguments[i]) ? arguments[i] : [];
    for (var j = 0; j < list.length; j++) {
      var status = String(list[j] || "").trim().toLowerCase();
      if (!status || seen[status]) continue;
      seen[status] = true;
      out.push(status);
    }
...
function recipeInventory(recipes) {
  ...
  for (var i = 0; i < recipes.length; i++) {
    ...
    out.push({
      ...
      // Binds a Lead source to this exact launcher selection rule. A matching
      // id/repo alone is not enough: a stale copy can widen assignment, labels
      // or board exclusions while preserving both fields.
      digest: recipeDigest(recipes[i]),
    });
  }
...
function loadProjectAutomationPolicy(options) {
  ...
  var recipeBoardExclusions = deriveBoardExclusions(parsedRecipes);
  ...
  if (config && Object.prototype.hasOwnProperty.call(config, "automation")) {
    ...
    return {
      ...
      policy: buildPolicy(ref, false, explicit.autonomy, explicit.externalActions,
        mergeBoardExclusions(recipeBoardExclusions, explicit.boardExclusions), explicit.qualification,
        vendors, inventory, sources),
    };
  }
  ...
  return {
    ...
    policy: buildPolicy(ref, true, deriveAutonomy(parsedRecipes), restrictiveExternalActions(),
      recipeBoardExclusions, null, vendors, inventory, sources),
  };
}

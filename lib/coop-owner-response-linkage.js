...
// One shared bound, imported rather than re-typed. This module, the MCP gate's
// zod schema and the Lead's batching all used to carry their own literal 16;
// nothing tied them together, so the producer could build a payload two
// independent validators were guaranteed to refuse. See
// coop-owner-request-batching for why the fix is batching, not a bigger number.
var MAX_LINKED_REQUESTS = require("./coop-owner-request-batching").MAX_OWNER_REQUEST_BATCH;
...
function normalizeRequests(values, aggregate) {
  var list = Array.isArray(values) ? values : [];
  // The tool call is bounded; its durable union may contain several batches.
  // Each staged entry is validated against the ledger and deduplicated, so the
  // union cannot grow beyond the actual requests being answered.
  if (!list.length || !aggregate && list.length > MAX_LINKED_REQUESTS) return null;
  var seen = {};
  var normalized = [];
...
function normalizeLink(value) {
  ...
  var requests = normalizeRequests(source.requests, source.version === 2);
  if ((source.version !== 1 && source.version !== 2) || !turnRef || !Number.isInteger(start) || start < 0 ||
...
function saveLink(session, state, link, saveSession, topicRefs) {
  var previous = state.pendingOwnerResponse;
  state.pendingOwnerResponse = link;
  // A field-bearing record is never coalesced by the JSONL writer. Mark the
  // last pre-response event so the boundary survives index drift on reload.
  var boundary = session.history[link.responseStartEventIndex - 1];
  var previousMarker = boundary.coopOwnerResponseStartsAfter;
  var previousTopics = boundary.coopOwnerResponseTopicRefs;
  var topics = (previousTopics || []).slice();
  (topicRefs || []).forEach(function (ref) {
    if (!topics.some(function (old) { return old.topicId === ref.topicId; })) topics.push(ref);
  });
  boundary.coopOwnerResponseTopicRefs = topics;
  boundary.coopOwnerResponseStartsAfter = true;
  session._historyNeedsRewrite = true;
  try {
    if (saveSession(session) === false) throw new Error("session_persistence_failed");
    return true;
  } catch (error) {
    if (previousTopics === undefined) delete boundary.coopOwnerResponseTopicRefs;
    else boundary.coopOwnerResponseTopicRefs = previousTopics;
    if (previousMarker === undefined) delete boundary.coopOwnerResponseStartsAfter;
    else boundary.coopOwnerResponseStartsAfter = previousMarker;
...
function stageOwnerResponse(input) {
  ...
  for (var i = 0; i < requests.length; i++) {
    ...
    var topicRef = require("./coop-topic-ref").normalizeTopicRefInput(exact.record.topicRef);
    if (topicRef) topicRefs.push(topicRef);
  }
  ...
  if (state.pendingOwnerResponse && !existing) {
    return { ok: false, code: "invalid_pending_response" };
  }
  if (existing && sameEventRef(existing.turnRef, turnRef)) {
    var merged = existing.requests.slice();
    for (var r = 0; r < requests.length; r++) {
      var prior = merged.find(function (entry) { return entry.ingressId === requests[r].ingressId; });
      if (prior && !sameEventRef(prior.requestRef, requests[r].requestRef)) {
        return { ok: false, code: "response_link_conflict" };
      }
      if (!prior) merged.push(requests[r]);
    }
    if (merged.length === existing.requests.length) {
      return { ok: true, duplicate: true, link: clone(existing) };
    }
    var extended = Object.assign({}, existing, { version: 2, requests: merged });
    if (!saveLink(session, state, extended, options.saveSession, topicRefs)) {
      return { ok: false, code: "session_persistence_failed" };
    }
    return { ok: true, duplicate: false, link: clone(extended) };
  }
  var link = {
    version: 2,
    turnRef: turnRef,
  ...
  if (!saveLink(session, state, link, options.saveSession, topicRefs)) {
    return { ok: false, code: "session_persistence_failed" };
  }

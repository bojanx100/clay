...
function createControlPlaneBindingMigration(deps) {
  ...
  function sessionsForMissingBinding(targetManager, portfolioTaskId, bindingRevision) {
    var matches = [];
    if (!targetManager || !targetManager.sessions ||
        typeof targetManager.sessions.forEach !== "function") return matches;
    targetManager.sessions.forEach(function (session) {
      var metadata = portfolioBindings.sessionExecutionBinding(session);
      if (metadata && metadata.portfolioTaskId === portfolioTaskId &&
          metadata.bindingRevision === bindingRevision) matches.push(session);
    });
    return matches;
  }

  function validMissingMetadata(metadata, request, targetProject) {
    return !!metadata && metadata.mode === "project_coordinator" &&
      !!metadata.targetProject && metadata.targetProject.projectId === targetProject.projectId &&
      metadata.idempotencyKey === request.idempotencyKey && metadata.status === "needs_input" &&
      metadata.reviewOnly === true && typeof metadata.taskPayloadDigest === "string" &&
  ...
  function validMissingSessionAuthority(session, metadata, coordinator, rootRef) {
    var source = projectIdentity.normalizeSessionRef(metadata && metadata.source);
    var controlledBy = session && session.coopControlledBy;
    return !!coordinator && !!rootRef && !!source &&
      rootRef.projectId === projectIdentity.LEAD_PROJECT_ID && sameRef(source, rootRef) &&
      isCoopControlled(session) && session.coordinationMode === true &&
      session.coordinationRole === "task_coordinator" && !!controlledBy &&
      controlledBy.coopSessionStorageId === rootRef.sessionStorageId;
  }

  function validMissingProvenance(metadata) {
    var provenance = metadata && metadata.controlPlaneProvenance;
    return !!provenance &&
      provenance.schema === portfolioBindings.CONTROL_PLANE_PROVENANCE_SCHEMA &&
      provenance.version === portfolioBindings.CONTROL_PLANE_PROVENANCE_VERSION;
  }

  function validMissingRoot(root, rootRef, actualRootRef, targetProject) {
    var policy = coopControlPlane.projectCoordinatorPolicy(root);
    return !!root && !!actualRootRef && sameRef(actualRootRef, rootRef) &&
      isCoopControlled(root) && !!policy && !!policy.projectRef &&
      policy.projectRef.projectId === targetProject.projectId;
  }

  function projectedTaskRef(task, targetProject) {
    return projectIdentity.normalizeSessionRef(task && (task.workerSessionRef || {
      projectId: targetProject.projectId,
      sessionStorageId: task.workerStorageId,
    }));
  }

  function validMissingTask(task, taskRef, coordinator, metadata, targetProject) {
    return !!task && task.externalTaskCoordinator === true && task.reviewOnly === true &&
      !!taskRef && sameRef(taskRef, coordinator) && !!task.coopProjectRef &&
      task.coopProjectRef.projectId === targetProject.projectId && !!task.coopTopicRef &&
      !!metadata.coopTopicRef && task.coopTopicRef.topicId === metadata.coopTopicRef.topicId;
  }

  function matchingMissingClaims(metadata, targetProject, rootRef) {
    if (!ownerRequests || typeof ownerRequests.canonicalProjectCoordinator !== "function" ||
        typeof ownerRequests.canonicalCoordinator !== "function") {
      return { ok: false, reason: "coordinator_claim_unavailable" };
    }
    var projectClaim;
    var topicClaim;
    try {
      projectClaim = ownerRequests.canonicalProjectCoordinator(targetProject);
      topicClaim = ownerRequests.canonicalCoordinator(metadata.coopTopicRef, targetProject);
    } catch (e) {
      return { ok: false, reason: "coordinator_claim_unavailable" };
    }
    if (!projectClaim || !sameRef(projectClaim, rootRef) ||
        (topicClaim && !sameRef(topicClaim, rootRef))) {
      return { ok: false, reason: "missing_binding_coordinator_claim_mismatch" };
    }
    return { ok: true };
  }

  function restoredBinding(metadata, targetProject, coordinator, rootRef) {
    return {
      ...
      reviewOnly: true,
      coopTopicRef: metadata.coopTopicRef,
      controlPlaneProvenance: metadata.controlPlaneProvenance,
      taskPayloadDigest: metadata.taskPayloadDigest,
  ...
  // A missing store row is reconstructible only when three durable authorities
  // agree without inference: the target session's typed execution metadata, the
  // resident Lead root's exact task projection, and the project coordinator
  // claim. The topic claim may have been compacted; if present it must agree.
  function inspectMissingBinding(request, targetProject, targetManager) {
    var matches = sessionsForMissingBinding(targetManager, request.portfolioTaskId,
      request.bindingRevision);
    if (matches.length === 0) return { ok: false, reason: "binding_not_found" };
    if (matches.length !== 1) {
      return { ok: false, reason: "ambiguous_missing_binding_evidence" };
    }
    var session = matches[0];
    var metadata = portfolioBindings.sessionExecutionBinding(session);
    var coordinator = projectIdentity.sessionRef(targetProject, session);
    var rootRef = projectIdentity.normalizeSessionRef(session.projectCoordinatorRef);
    if (!validMissingMetadata(metadata, request, targetProject) ||
        !validMissingSessionAuthority(session, metadata, coordinator, rootRef) ||
        !validMissingProvenance(metadata)) {
      return { ok: false, reason: "missing_binding_session_evidence_invalid" };
    }
    ...
    var root = leadManager ? coopControlPlane.projectCoordinatorFor(leadManager, targetProject) : null;
    var actualRootRef = projectIdentity.sessionRef(
      { projectId: projectIdentity.LEAD_PROJECT_ID }, root);
    if (!leadManager || !validMissingRoot(root, rootRef, actualRootRef, targetProject)) {
      return { ok: false, reason: "missing_binding_root_evidence_invalid" };
    }
    var task = coopControlPlane.taskForRequest(root, request);
    var taskRef = projectedTaskRef(task, targetProject);
    if (!validMissingTask(task, taskRef, coordinator, metadata, targetProject)) {
      return { ok: false, reason: "missing_binding_task_projection_mismatch" };
    }
    var claims = matchingMissingClaims(metadata, targetProject, rootRef);
    if (!claims.ok) return claims;
    return {
      ok: true,
      binding: restoredBinding(metadata, targetProject, coordinator, rootRef),
      evidence: {
  ...
  function lookupMigrationBinding(request, targetProject) {
    var binding = bindingStore.get(request.portfolioTaskId, request.bindingRevision);
    if (binding && binding.mode !== "project_coordinator") {
      return { ok: false, reason: "invalid_migration" };
    }
    if (binding && binding.targetProject.projectId !== targetProject.projectId) {
    ...
    var missing = null;
    if (!binding) {
      missing = inspectMissingBinding(request, targetProject, targetManager);
      if (!missing.ok) return missing;
      binding = missing.binding;
    }
    return { ok: true, binding: binding, missing: missing, targetContext: targetContext,
      targetManager: targetManager };
  }

  function persistInspectedBinding(binding, missing) {
    if (!missing) return { ok: true, binding: binding };
    if (typeof bindingStore.restoreMissingProjectCoordinator !== "function") {
      return { ok: false, reason: "missing_binding_recovery_unavailable" };
    }
    return bindingStore.restoreMissingProjectCoordinator(binding, missing.evidence);
  }
  ...
  return function migrateControlPlaneBinding(input) {
    ...
    var lookup = lookupMigrationBinding(request, targetProject);
    if (!lookup.ok) return lookup;
    var binding = lookup.binding;
    var targetContext = lookup.targetContext;
    var targetManager = lookup.targetManager;
    ...
    var restored = persistInspectedBinding(binding, lookup.missing);
    ...
    if (binding.projectCoordinator &&
      ...
      if (!evidence) {
        return Object.assign(successResult(binding,
          ...
          recoveredMissingBinding: !!binding.missingBindingRecovery,
        });
      }

function attachBridgeMentions(ctx) {
  ...
  async function createMentionSession(opts) {
    ...
    function current() { return !opts.isCurrent || opts.isCurrent(); }

    var handle;
    try {
      var queryOptions = {
        cwd: cwd,
        env: getRuntimeEnv(opts.session || { linuxUser: opts.linuxUser || null }),
      ...
      if (opts.readOnlyExecution === true) {
        require("./yoke/read-only-query").apply(queryOptions, mentionAdapter.vendor);
        queryOptions.canUseTool = function (name, input) {
          return Promise.resolve(require("./read-only-execution").toolDecision(
            { orchestrationPolicy: { readOnlyExecution: true } }, null, name, input));
        };
      }
      handle = await mentionAdapter.createQuery(queryOptions);
    } catch (e) {
      if (current()) opts.onError("Failed to create mention query: " + (e.message || e));
      return null;
    }
    var query = handle;

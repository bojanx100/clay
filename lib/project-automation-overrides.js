...
function createOverrideStore(options) {
  ...
  //
  // eligibility(itemKey, assignedToOwner, recipeAllowsUnassigned) ->
  //   { ok, eligible, reason, decision }
  //
  // `assignedToOwner` must be a proven boolean from the fetch layer. The third
  // argument is a separate strict boolean stamped from an explicit project
  // `filter.assigned: "any"` recipe. It is never inferred from an empty
  // assignee list, so other recipes retain their strict ownership requirement.
  function eligibility(itemKey, assignedToOwner, recipeAllowsUnassigned) {
    ...
    if (recipeAllowsUnassigned === true) {
      return { ok: true, eligible: true, reason: "recipe_allows_unassigned", decision: found.decision || null };
    }

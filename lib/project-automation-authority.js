...
// Board work normally must be ASSIGNED TO THE OWNER to be automatically
// eligible. An explicit `filter.assigned: "any"` project recipe is the narrow
// alternative: it permits that project's unassigned work to reach its own
// policy decision, without changing any other project's recipe.
//
// This is an eligibility precondition, not a policy stance, so it is checked
// before any stance is consulted. An unassigned board item with neither proof
// is not work the owner has taken on, and automatic pickup of one is
// indistinguishable from the automation choosing its own work: trialview/v2#2539
// was unassigned, was proposed bug/auto, and produced PR #2591 that nobody had
// asked for.
//
// Only an explicit `true` passes. An absent ownership stamp means the fetch
// layer could not prove ownership; an absent recipe stamp means this recipe
// did not explicitly opt into unassigned work. Either keeps ordinary board work
// refused.
//
...
function assignmentEligible(input, itemClass) {
  ...
  return input.assignedToOwner === true || input.recipeAllowsUnassigned === true;
}
...
function decideAutomation(input) {
  ...
  var normalized = {
    ...
    // Separate evidence from a project's explicit `filter.assigned: "any"`
    // recipe. This is never an inferred substitute for ownership.
    recipeAllowsUnassigned: request.recipeAllowsUnassigned === true,

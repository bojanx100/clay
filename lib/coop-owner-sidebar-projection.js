...
function sidebarSections(entries) {
  ...
  var attention = openWork.filter(function (entry) { return entry.status !== "working"; });
...
function buildOwnerSidebar(input) {
  ...
  var entries = sortBySequence(workIdentity.group(candidates).map(workRows.mergeEntries));

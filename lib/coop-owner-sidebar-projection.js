...
function actionSessions(source, status) {
  var destination = source && source.destination && source.destination.ref;
...
function actionProjects(source) {
  ...
  return [{ projectRef: copy(source.projectRef), title: "Project" }];
}
...
function actionEntry(action, topics) {
  ...
  return {
    ...
    bindings: actionBindings(source), ingressIds: [], requestRefs: [], sourceSessionRefs: [],

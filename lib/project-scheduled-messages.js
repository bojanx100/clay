...
function attachProjectScheduledMessages(ctx) {
  ...
  function restoreScheduledMessageTimer(session) {
    if (!require("./scheduled-execution-policy").restoresWork()) return false;
  ...
  function autoResumeRestartSession(session, options) {
    if (!require("./scheduled-execution-policy").restoresWork() && !(options && options.userInitiated)) return;

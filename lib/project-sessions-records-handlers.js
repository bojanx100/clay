...
// Close a hidden TUI session's live PTY(s) immediately instead of waiting for
// the idle reaper. Mirrors the suspend_tui_session teardown: markReclaimed so
// the PTY exit is treated as an intentional reclaim (not a user /exit), close
// both the embedded and runtime terminals, then hydrate the transcript so the
// session reopens as a read-only Resume view.
function stopTuiRuntimeOnHide(ctx, session) {
  if (!session || session.mode !== "tui") return;
  var tm = ctx.tm;
  if (tm) {
    var tid = typeof session.terminalId === "number" ? session.terminalId : null;
    var rid = typeof session.runtimeTerminalId === "number" ? session.runtimeTerminalId : null;
    if (tid != null && tm.has(tid)) { tm.markReclaimed(tid); tm.close(tid); }
    if (rid != null && tm.has(rid)) { tm.markReclaimed(rid); tm.close(rid); }
  }
  session.terminalId = null;
  session.runtimeTerminalId = null;
  if (typeof ctx.prepareTuiSessionForGuiView === "function") {
    try { ctx.prepareTuiSessionForGuiView(session); } catch (e) {}
  }
...
function handleHide(ctx, ws, msg) {
  ...
  stopTuiRuntimeOnHide(ctx, target);

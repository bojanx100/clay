...
function save(sm, session) {
  try { return !!sm && typeof sm.saveSessionFile === "function" &&
    sm.saveSessionFile(session, { durable: true }) !== false; }
...
function mark(ctx, session) {
  ...
  if (!save(ctx.sm, session)) session.contextRecovery.persistenceFailed = true;
}
...
function blockedReason(input) {
  ...
  if (session.contextRecovery.persistenceFailed) return "recovery_not_durable";
...
function finish(input, priorBlocker) {
  ...
  if (!reason) {
    ...
    state.attempts++;
    if (!save(input.sm, session)) reason = "recovery_not_durable";
    try {
    ...
    } catch (error) { reason = reason || "renewal_failed"; }
  }

...
var { execFile, execFileSync, spawn } = require("child_process");
...
var serveDashboardPage = require("./project-task-dashboard-page").serveDashboardPage;
...
function attachHTTP(ctx) {
  ...
  var taskLauncher = ctx.taskLauncher || null;

  function taskLaunchHeaders(req) {
    var headers = { "Content-Type": "application/json" };
    var origin = req.headers.origin || "";
    if (isDashboardOriginAllowed(req, origin)) {
  ...
  function taskLaunchToken() {
    var cfgPath = path.join(cwd, ".clay", "tasks", "config.json");
    try {
      var cfg = JSON.parse(fs.readFileSync(cfgPath, "utf8"));
      return cfg && cfg.launchApi && cfg.launchApi.token ? String(cfg.launchApi.token) : "";
    } catch (e) {
      return "";
    }
  }
  ...
  function dashboardStatePath() {
    return path.join(cwd, ".clay", "tasks", "dashboard-state.json");
  }

  function readDashboardState() {
    try {
      var raw = fs.readFileSync(dashboardStatePath(), "utf8");
      var data = JSON.parse(raw);
      if (!data || !Array.isArray(data.issues)) return { ok: true, issues: [], savedAt: null };
      return { ok: true, issues: data.issues, savedAt: data.savedAt || null };
    } catch (e) {
      return { ok: true, issues: [], savedAt: null };
    }
  }

  // Constant-time token comparison to avoid leaking the launch token via
  ...
  // --- GitHub media proxy --------------------------------------------------
  // Stream private GitHub assets (issue/PR images & videos) to the browser via
  // this project's authenticated gh token, so the session-context panel can
  // show them in-app instead of bouncing the user out to github.com.
  var MEDIA_FETCH_TIMEOUT = 20000;
  var MEDIA_TOKEN_TTL = 5 * 60 * 1000;
  var _mediaTokenCache = { token: "", at: 0 };
  var _mediaTokenPending = null;

  // Resolve the project's gh token, cached briefly so a panel full of
  // thumbnails doesn't shell out to `gh` once per image.
  function finishGhMediaToken(token) {
    if (token) { _mediaTokenCache.token = token; _mediaTokenCache.at = Date.now(); }
    var pending = _mediaTokenPending || [];
    _mediaTokenPending = null;
    for (var i = 0; i < pending.length; i++) {
      try { pending[i](token || ""); } catch (e) {}
    }
  }

  function runGhMediaToken(args, cb) {
    execFile("gh", args, {
      cwd: cwd,
      encoding: "utf8",
      timeout: 5000,
  ...
  function ghMediaToken(cb) {
    var nowMs = Date.now();
    if (_mediaTokenCache.token && (nowMs - _mediaTokenCache.at) < MEDIA_TOKEN_TTL) {
      cb(_mediaTokenCache.token);
      return;
    }
    if (_mediaTokenPending) {
      _mediaTokenPending.push(cb);
      return;
    }
    _mediaTokenPending = [cb];

    var pinnedAccount = "";
    try {
      pinnedAccount = gitAccounts.getProjectGitAccount(cwd) || "";
    } catch (e) {}
    var envToken = process.env.GH_TOKEN || "";
    if (pinnedAccount) {
      runGhMediaToken(["auth", "token", "--user", pinnedAccount], function (token) {
        if (token) { finishGhMediaToken(token); return; }
        if (envToken) { finishGhMediaToken(envToken); return; }
        runGhMediaToken(["auth", "token"], finishGhMediaToken);
      });
      return;
    }
    if (envToken) { finishGhMediaToken(envToken); return; }
  ...
  // Only GitHub-controlled hosts may be proxied — prevents this from becoming an
  // open proxy / SSRF vector.
  function isProxyableMediaHost(hostname) {
    return hostname === "github.com"
      || hostname === "githubusercontent.com"
      || /\.githubusercontent\.com$/i.test(hostname);
  }

  function proxyGithubMedia(req, res, targetUrl, token, depth) {
    if (depth > 5) { res.writeHead(508); res.end("Too many redirects"); return; }
    var parsed;
    try { parsed = new URL(targetUrl); } catch (e) { res.writeHead(400); res.end("Bad URL"); return; }
    if (parsed.protocol !== "https:" || !isProxyableMediaHost(parsed.hostname)) {
      res.writeHead(400); res.end("Host not allowed"); return;
    }
    var headers = { "User-Agent": "Clay/1.0", "Accept": "*/*" };
    // Send the gh token only to github.com itself. Signed CDN redirect targets
    // (e.g. private-user-images.githubusercontent.com) carry their own JWT and
    // reject a second credential.
    if (token && parsed.hostname === "github.com") headers["Authorization"] = "Bearer " + token;
    // Forward Range so the browser can seek within videos without downloading
    ...
    var upstream = https.request({
      ...
      var sc = up.statusCode || 502;
      if (sc >= 300 && sc < 400 && up.headers.location) {
        up.resume();
        var next;
        try { next = new URL(up.headers.location, targetUrl).toString(); }
        catch (e) { if (!res.headersSent) res.writeHead(502); res.end(); return; }
        proxyGithubMedia(req, res, next, token, depth + 1);
        return;
      }
      if (sc !== 200 && sc !== 206) {
        up.resume();
        res.writeHead(sc === 404 ? 404 : 502);
        res.end();
        return;
      }
      var out = {
    ...
    upstream.on("error", function () { if (!res.headersSent) res.writeHead(502); res.end(); });
    upstream.setTimeout(MEDIA_FETCH_TIMEOUT, function () { upstream.destroy(); });
    upstream.end();
  }

  function handleHTTP(req, res, urlPath) {
    if (serveDashboardPage(cwd, ctx.slug, req, res, urlPath)) return true;

    // Proxy GitHub issue/PR media so the session-context panel renders it
    // in-app. Auth is already enforced upstream (server.js) before we get here.
    if (req.method === "GET" && urlPath.indexOf("/api/media") === 0) {
      var mq = urlPath.indexOf("?");
      var mediaUrl = null;
      if (mq >= 0) {
        try { mediaUrl = new URLSearchParams(urlPath.substring(mq + 1)).get("url"); } catch (e) {}
      }
      if (!mediaUrl) {
      ...
      ghMediaToken(function (token) {
        if (res.destroyed) return;
        proxyGithubMedia(req, res, mediaUrl, token, 0);
      });
      return true;
    }

    if (req.method === "POST" && urlPath === "/api/task-dashboard-state") {
      parseJsonBody(req).then(function (body) {
        var token = taskLaunchToken();
        if (!token || !body || !tokensMatch(body.token, token)) {
          res.writeHead(403, taskLaunchHeaders(req));
          res.end(JSON.stringify({ ok: false, error: "Not allowed" }));
          return;
        }
        if (body.action === "save") {
          var issues = Array.isArray(body.issues) ? body.issues : [];
          var state = { savedAt: Date.now(), issues: issues };
          try {
            fs.mkdirSync(path.dirname(dashboardStatePath()), { recursive: true });
            fs.writeFileSync(dashboardStatePath(), JSON.stringify(state, null, 2), "utf8");
            res.writeHead(200, taskLaunchHeaders(req));
            res.end(JSON.stringify({ ok: true, count: issues.length }));
          } catch (e) {
            res.writeHead(500, taskLaunchHeaders(req));
            res.end(JSON.stringify({ ok: false, error: e.message || "Save failed" }));
          }
          return;
        }
        if (body.action === "load") {
        ...
        res.writeHead(400, taskLaunchHeaders(req));
    ...
    if (req.method === "POST" && urlPath === "/api/task-launch") {
      parseJsonBody(req).then(function (body) {
        var token = taskLaunchToken();
        if (!taskLauncher || !token || !body || !tokensMatch(body.token, token)) {
          res.writeHead(403, taskLaunchHeaders(req));
          res.end(JSON.stringify({ ok: false, error: "Not allowed" }));
          return;
        }
        // launchExternal is async (GitHub fetch runs in a worker process).
        // Handle its failure here with a real error message — the outer catch
        // is only for unparseable request bodies.
        return Promise.resolve(taskLauncher.launchExternal(body, req._clayUser || null)).then(function (result) {
          res.writeHead(result.ok ? 200 : 400, taskLaunchHeaders(req));
          res.end(JSON.stringify(result));
        }, function (e) {
          res.writeHead(500, taskLaunchHeaders(req));
          res.end(JSON.stringify({ ok: false, error: (e && e.message) || "Launch failed" }));
        });
      }).catch(function () {
        res.writeHead(400, taskLaunchHeaders(req));
        res.end(JSON.stringify({ ok: false, error: "Invalid JSON body" }));
    ...
    if (req.method === "POST" && urlPath === "/ext-command") {
      parseJsonBody(req).then(function (body) {
        // Validate auth token (constant-time to avoid timing leaks)
        if (!tokensMatch(body.token, _extToken)) {

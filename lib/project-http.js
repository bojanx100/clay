...
function attachHTTP(ctx) {
  ...
  function ghMediaToken() {
    var nowMs = Date.now();
    if (_mediaTokenCache.token && (nowMs - _mediaTokenCache.at) < MEDIA_TOKEN_TTL) {
      return _mediaTokenCache.token;
    }
    var token = "";
    try {
      var env = gitw.ghEnvFor(cwd);
      if (env && env.GH_TOKEN) token = env.GH_TOKEN;
    } catch (e) {}
    if (!token) {
      try { token = execFileSync("gh", ["auth", "token"], { cwd: cwd, encoding: "utf8", timeout: 5000 }).trim(); }
      catch (e) {}
    }
    if (token) { _mediaTokenCache.token = token; _mediaTokenCache.at = nowMs; }
    return token;
  }
  ...
  function handleHTTP(req, res, urlPath) {
    ...
    if (req.method === "GET" && urlPath.indexOf("/api/media") === 0) {
      ...
      proxyGithubMedia(req, res, mediaUrl, ghMediaToken(), 0);
      return true;
    }

...
// alone: the retry has to be the SAME work at a LATER revision. Same target
// project, same task, and `bindingRevision` strictly increasing -- so an approval
// can never be replayed sideways onto another task or project, and never walked
// backwards onto a revision the owner has already seen the outcome of.
//
// The other half -- that the approved revision ended terminal-unsuccessful and
// that no revision of this task has ever completed -- lives in the binding store,
// which this module cannot read. `server-cross-project.approvalCarriesForward`
// checks it there and both halves must hold; neither is sufficient. This function
...
function carryForwardEligible(current, next) {
  if (!current || !next || !current.projectRef || !next.projectRef) return false;

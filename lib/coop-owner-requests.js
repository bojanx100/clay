...
function attachCoopOwnerRequests(options) {
  ...
  // implementation decision behind. First scope wins; only a byte-equivalent
  // retry may reuse it -- or an explicit carry-forward onto a later revision of
  // the same task, which replaces the scope and records that it did so.
  function scopeImplementation(id, options) {
    ...
    var namedApproval = !!(found && !found.implementationDecision && suppliedDecision);
    ...
    var snapshot = clone(found);
    if (namedApproval) found.implementationDecision = suppliedDecision;

...
var normalizeResponseRef = records.normalizeResponseRef;
...
function attachCoopOwnerRequests(options) {
  ...
  function fileVersion() {
    if (typeof fsImpl.statSync !== "function") return ledgerFile.readIdentity(fsImpl, file);
    try {
  ...
  function refresh(force) {
    ...
    if (!force && sameVersion(loadedVersion, current)) return state;
  ...
  function mutate(fallback, operation) {
    try {
      ...
      try { refresh(true); } catch (refreshError) {}
      return typeof fallback === "function" ? fallback(error) : fallback;
    }
  }

  function read(operation, fallback) {
    try { refresh(false); return operation(); }
  ...
  function markAnswered(id, options) {
    ...
    // messageUuid is the caller's durable anchor for the answering `done` (see
    // coop-owner-response-resolution.js). The index is derived positional data
    // and drifts on every reload; the anchor is what survives. It is optional
    // by construction -- callers that cannot compute one, and every record
    // written before the field existed, keep the index-only behaviour.
    var responseRef = Number.isInteger(eventIndex) && eventIndex >= 0
      ? normalizeResponseRef({
  ...
  // The read-only counterpart to scopeImplementation, and deliberately NOT a
  // variant of it.
  //
  // Why it exists: review admission returned ok without recording anything, so
  // the router had no durable evidence that a review turn covered a given task.
  // The coverage check therefore could not be applied to review ingresses at all,
  // which left that branch of the history scan unbounded -- the defect that let
  // owner turn :595 ("Do both") claim the route for unrelated work on 2026-08-23
  // and hide the route that could actually authorize it.
  //
  // Three things it must never do, because each would turn read-only
  // acknowledgement into implementation authority:
  //
  //   It never requires, sets or reads implementationDecision. scopeImplementation
  //   refuses without one (:417-420), which is exactly why a review turn such as
  //   "do them" -- decision null -- could never write through it. That refusal is
  //   correct there and must not be relaxed.
  //
  //   It never touches implementationScope, implementationScopes or
  //   expectsExecution. Review coverage lives only in reviewScopes, and only the
  //   router's coverage check for an already-provably-read-only dispatch reads it.
  //
  //   It never widens projectRefs or mints a topicRef on the record. Those feed
  //   implementation admission's project matching; a review must not enlarge the
  //   set of projects an owner turn is taken to have named.
  //
  // Idempotent per exact scope, so re-dispatching the same review reuses its
  // record instead of accumulating duplicates.
  function scopeReview(id, options) {
    var found = find(id);
    if (!found) return { ok: false, reason: "owner_request_unavailable" };
    var scope = normalizeImplementationScope(options);
    if (!scope) return { ok: false, reason: "invalid_owner_review_scope" };
    var scopes = records.reviewScopesFor(found);
    for (var i = 0; i < scopes.length; i++) {
      if (sameImplementationScope(scopes[i], scope)) {
        return { ok: true, reused: true, request: clone(found) };
      }
    ...
    var snapshot = clone(found);
    scopes.push(scope);
    found.reviewScopes = scopes.map(clone);
    var persisted = touch(found, snapshot);
    return persisted ? { ok: true, reused: false, request: persisted } :
  ...
  // Re-points stored requestRef offsets onto the events they already denote.
  //
  // requestRef.eventIndex is an absolute offset into a transcript the
  // persistence layer re-indexes on every write (delta coalescing, cf7f197ee1),
  // so it rots and never recovers on its own. coopIngressId is the immutable
  // identity the ingress was stamped with, so the caller resolves each record
  // by that identity and hands the true offset back here.
  //
  // This is a COORDINATE repair and nothing else. Only eventIndex moves; the
  // projectId and sessionStorageId the ref was recorded against are left
  // exactly as they are, because a ref that names a different session is a
  // different claim, not a drifted one. Nothing else on the record is touched,
  // and a record with no requestRef is skipped rather than given one -- minting
  // a missing ref is backfill's job and it needs evidence this path does not
  // have.
  //
  // Applied as ONE transaction: a single persist() for the whole batch, so a
  // failed write leaves every record at its previous offset rather than
  // repairing a prefix and abandoning the rest.
  function repointRequestRefs(corrections) {
    var list = Array.isArray(corrections) ? corrections : [];
    var undo = [];
    var applied = [];
    for (var i = 0; i < list.length; i++) {
      var correction = list[i] || {};
      var found = find(correction.ingressId);
      var eventIndex = correction.eventIndex;
      if (!found || !found.requestRef) continue;
      if (!Number.isInteger(eventIndex) || eventIndex < 0) continue;
      // The caller resolved against one session's history; refuse to move a ref
      // that was recorded against a different one.
      if (correction.sessionStorageId &&
          found.requestRef.sessionStorageId !== correction.sessionStorageId) continue;
      if (found.requestRef.eventIndex === eventIndex) continue;
      undo.push({ record: found, snapshot: clone(found) });
      applied.push({
        ingressId: found.ingressId,
        from: found.requestRef.eventIndex,
        to: eventIndex,
        sessionStorageId: found.requestRef.sessionStorageId,
      });
      found.requestRef.eventIndex = eventIndex;
      found.updatedAt = now();
    }
    if (!applied.length) return { ok: true, repointed: 0, corrections: [] };
    if (!persist()) {
      for (var u = 0; u < undo.length; u++) restore(undo[u].record, undo[u].snapshot);
      return { ok: false, reason: "persistence_failed", repointed: 0, corrections: [] };
    }
    return { ok: true, repointed: applied.length, corrections: applied };
  }
  ...
  return {
    ...
    repointRequestRefs: function (corrections) { return mutate({ ok: false, reason: "persistence_failed", repointed: 0, corrections: [] }, function () { return repointRequestRefs(corrections); }); },
    ...
    scopeReview: function (id, input) { return mutate({ ok: false, reason: "persistence_failed" }, function () { return scopeReview(id, input); }); },
...
module.exports = {
  ...
  reviewScopesFor: records.reviewScopesFor,
  STATES: STATES,

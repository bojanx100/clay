...
function attachCoopOwnerRequests(options) {
  ...
  // scope wins; only a byte-equivalent retry may reuse it.
  function scopeImplementation(id, options) {
    ...
    if (found.implementationScope) {
      return sameImplementationScope(found.implementationScope, scope)
        ? { ok: true, reused: true, request: clone(found) }
    ...
    found.classification = {
      kind: "existing_topic",
      source: "owner_directed_execution",
      at: now(),
    };

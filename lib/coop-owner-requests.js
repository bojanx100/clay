...
function attachCoopOwnerRequests(options) {
  ...
  // The half of the approval carry-forward rule that this module can prove on
  // its own: the retry has to be the SAME work at a LATER revision. Same target
  // project, same task, and `bindingRevision` strictly increasing -- so an
  // approval can never be replayed sideways onto another task or project, and
  // never walked backwards onto a revision the owner has already seen the
  // outcome of.
  //
  // The other half -- that the approved revision ended terminal-unsuccessful and
  // that no revision of this task has ever completed -- lives in the binding
  // store, which this module cannot read. The caller checks it there and both
  // halves must hold; neither is sufficient. This function therefore stays
  // fail-closed on its own terms rather than trusting the caller's intent: a
  // caller that asks for a carry-forward it has not earned still gets refused
  // here if the identity or the ordering is wrong.
  function carryForwardEligible(current, next) {
    if (!current || !next) return false;
    return current.projectRef.projectId === next.projectRef.projectId &&
      current.portfolioTaskId === next.portfolioTaskId &&
      Number(next.bindingRevision) > Number(current.bindingRevision);
  }

  // A route-free Main command gains execution authority only when the typed
  // dispatch atomically names its exact destination and task identity. First
  // scope wins; only a byte-equivalent retry may reuse it -- or an explicit
  // carry-forward onto a later revision of the same task, which replaces the
  // scope and records that it did so.
  function scopeImplementation(id, options) {
    var found = find(id);
    if (!found || !found.implementationDecision || found.expectsExecution !== true) {
    ...
    if (found.implementationScope) {
      if (sameImplementationScope(found.implementationScope, scope)) {
        return { ok: true, reused: true, request: clone(found) };
      }
      if (input.carryForward !== true ||
          !carryForwardEligible(found.implementationScope, scope)) {
        return { ok: false, reason: "owner_implementation_scope_mismatch" };
      }
      carriedForward = true;
    }
    var snapshot = clone(found);
    found.implementationScope = scope;
    found.topicRef = clone(scope.topicRef);
    found.projectRefs = [clone(scope.projectRef)];
  ...
  return {
    ...
    retopicTurn: function (from, to, turn) { return mutate({ ok: false, reason: "persistence_failed", requests: 0 }, function () { return threadCorrections.retopicTurn(threadCorrectionSeam(), from, to, turn); }); },

...
function attachCoopOwnerRequests(options) {
  ...
  function record(input) {
    ...
    if (existing) {
      ...
      if (!existing.requestRef && late) {
        ...
        return touch(existing, snapshot) || clone(existing);
      }
  ...
  function repointCoordinatorLinks(losing, winning) {
  ...
  function retopic(fromTopicRef, toTopicRef) {
    ...
    for (var j = 0; j < state.coordinators.length; j++) {
      ...
      if (existing) {
        ...
        repointCoordinatorLinks(claim.coordinator, existing.coordinator);
        movedClaims++;
        continue;
      }

...
function attachProjectUserMessageHandlers(ctx) {
  ...
  handlers.resolve_coordinator_updates = function (ws, msg) {
    var session = ctx.getSessionForMessage(ws, msg);
    if (!session || !ctx.resolveCoordinatorUpdates || !ctx.resolveCoordinatorUpdates(session, msg)) {
      ctx.sendTo(ws, { type: "error", text: "Reports could not be updated. Check the session state and try again." });
    }
    return true;
  };

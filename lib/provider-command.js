...
function attachProviderCommand(ctx) {
  ...
  function suggestionForRoute(route, session) {
    ...
    if (coopModelPolicy.appliesToSession(session)) {
      ...
      if (decision.ok) {
        return { model: decision.model, match: "top-tier-policy",
          sourceModel: sourceModel };
      }
      return { model: null, match: "coop-policy", sourceModel: sourceModel,
  ...
  function recordNoComparable(session, route, suggestion) {
    if (suggestion && suggestion.policyDecision) {
      sm.sendAndRecord(session, { type: "info", variant: "warning",
        text: suggestion.policyDecision.message });
      return;
    }

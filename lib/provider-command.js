...
function attachProviderCommand(ctx) {
  ...
  function resolveTargetRoute(token, session) {
    ...
    if (target === "claude" || target === "anthropic") return routeForId("claude-anthropic");
    if (target === "codex" || target === "openai") return routeForId("codex-openai");
    ...
    for (var i = 0; i < routes.length; i++) {
      ...
      if (label && label.indexOf(target) === 0) return routeForId(routes[i].id);
    }
    return null;
  }
  ...
  function suggestionForRoute(route, session) {
    ...
    if (!route || !sourceModel) return { model: null, match: "unknown", sourceModel: sourceModel };

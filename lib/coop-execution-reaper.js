...
//     session_interrupted_before_runtime
//                             a non-terminal log tail predates the exact runtime
//                             that observed the project idle. The process that
//                             owned the turn is therefore gone.
...
function runtimeActivityReason(session) {
  ...
  if (session.restartResumeEligible || session.restartAutoContinueQueued) return "restart_resume_pending";
  if (session.rateLimitAutoContinuePending || session.rateLimitUseCreditsPending) {
    return "rate_limit_resume_pending";
  }
  ...
  if (Array.isArray(session.pendingUserMessageQueue) && session.pendingUserMessageQueue.length) {
    return "queued_user_input";
  }
  if (Array.isArray(session.pendingCoopIngress) && session.pendingCoopIngress.length) {
    return "queued_coop_ingress";
  }
  if (Array.isArray(session.pendingCoordinatorUpdates) && session.pendingCoordinatorUpdates.length) {
    return "queued_coordinator_update";
  }
  if (Array.isArray(session.pendingCoordinatorMessages) && session.pendingCoordinatorMessages.length) {
    return "queued_coordinator_message";
  }
...
function createExecutionReaper(options) {
  ...
  function observe(binding) {
    ...
    return {
      ...
      runtimeStartedAt: typeof view.runtimeStartedAt === "number" &&
        Number.isFinite(view.runtimeStartedAt) ? view.runtimeStartedAt : null,
  ...
  function deathEvidence(binding, view, timestamp) {
    ...
    if (!TERMINAL_LOG_EVENTS[view.tail.type]) {
      if (view.tail.at !== null && view.runtimeStartedAt !== null &&
          view.tail.at < view.runtimeStartedAt) {
        return Object.assign({}, base, {
          kind: "session_interrupted_before_runtime",
          lastEventType: view.tail.type,
          lastEventAt: view.tail.at,
          runtimeStartedAt: view.runtimeStartedAt,
          detail: "non-terminal session log predates the runtime that observed it idle",
        });
      }

...
function attachAuth(ctx) {
  ...
  var TOKENS_FILE = ctx.tokensFile ||
    path.join(CONFIG_DIR, _isDevMode ? "auth-tokens-dev.json" : "auth-tokens.json");
  var MULTI_USER_COOKIE = _isDevMode ? "relay_auth_user_dev" : "relay_auth_user";
  var multiUserTokens = {};
  // Set when an existing token file could not be read or parsed. Sessions then
  // live in memory only (users re-login), but writes are refused so the intact
  // on-disk file survives for the next boot instead of being replaced by an
  // empty map — that replacement is a silent, unrecoverable mass logout.
  var tokensUnreadable = null;

  function recordTokenCanary(operation, code, message) {
    var record = typeof ctx.recordRecoveryEvent === "function"
      ? ctx.recordRecoveryEvent : recoveryLog.recordRecoveryEvent;
    try {
      record({ kind: "coop_persistence", store: "auth_tokens", op: operation,
        code: code, message: message, file: TOKENS_FILE });
    } catch (e) {}
  }

  function loadTokens() {
    var raw;
    try { raw = fsImpl.readFileSync(TOKENS_FILE, "utf8"); }
    catch (e) {
      multiUserTokens = {};
      // ENOENT is the only genuinely fresh install.
      if (e && e.code === "ENOENT") { tokensUnreadable = null; return; }
      tokensUnreadable = { code: e && e.code || "read_failed" };
      recordTokenCanary("read", tokensUnreadable.code,
        e && e.message || "Auth token file read failed.");
      return;
    }
    var data;
    try { data = JSON.parse(raw); }
      ...
      multiUserTokens = {};
      tokensUnreadable = { code: "invalid_json" };
      recordTokenCanary("parse", "invalid_json",
        e && e.message || "Auth token file is not valid JSON.");
      return;
    }
    if (!data || typeof data !== "object" || Array.isArray(data)) {
      multiUserTokens = {};
      tokensUnreadable = { code: "invalid_shape" };
      recordTokenCanary("validate", "invalid_shape",
        "Auth token file is not a token map; refusing to overwrite it.");
      return;
    }
    tokensUnreadable = null;
    multiUserTokens = data;
  }

  function saveTokens() {
    if (tokensUnreadable) {
      recordTokenCanary("write_refused", tokensUnreadable.code,
        "Refusing to overwrite an unreadable auth token file.");
      return false;
    }
    try {
      fsImpl.mkdirSync(path.dirname(TOKENS_FILE), { recursive: true });
      var tmpPath = TOKENS_FILE + ".tmp";
      fsImpl.writeFileSync(tmpPath, JSON.stringify(multiUserTokens));
      fsImpl.renameSync(tmpPath, TOKENS_FILE);
      return true;
    } catch (e) {
      // A failed write means the session only exists in memory: surface it
      // instead of letting logins silently evaporate on the next restart.
      recordTokenCanary("write", e && e.code || "write_failed",
        e && e.message || "Auth token file write failed.");
      return false;
    }
  }
  ...
  return {
    ...
    tokenPersistenceState: function () {
      return tokensUnreadable ? { code: tokensUnreadable.code } : null;
    },
  };
}

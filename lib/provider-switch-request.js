...
var { candidateHealth, listProviderRoutes } = require("./provider-routes");
...
function attachProviderSwitchRequest(ctx) {
  ...
  function switchControlledSession(input) {
    ...
    if (session.isProcessing) {
      return { ok: false, reason: "processing", message: "The target session is still processing" };
    }
    var route = switcher.resolveSwitchTargetRoute(target, session);
    if (!route) return { ok: false, reason: "route-unavailable", message: "Unknown provider route" };
    if (route.enabled !== true) {
      var liveRoutes = sm.providerRoutes || listProviderRoutes(
        sm.availableVendors || [], sm.installedVendors || []
      );
      for (var ri = 0; ri < liveRoutes.length; ri++) {
        if (liveRoutes[ri].id === route.id) {
          route = liveRoutes[ri];
          break;
        }
      }
    ...
    if (!route.enabled) {
      return { ok: false, reason: "route-unavailable", message: (route.label || route.vendor) + " is not available" };
    }

  ...
  function postConfirmationCard(session, route, suggestion, reason) {
    ...
    session.pendingUserDialogs[requestId] = {
      ...
      resolve: function (response) {
        ...
        whenSessionIdle(session, 20000, function (idle) {
          if (!idle) {
            sm.sendAndRecord(session, {
              type: "info",
              variant: "warning",
              text: "Approved the provider switch, but the current turn did not finish in time. Use /provider " + route.id + " to switch manually.",
            });
            return;
          }

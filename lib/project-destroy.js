...
function createProjectDestroy(ctx) {
  ...
  var updateChecker = ctx.updateChecker;

  function destroy(liveUi) {
    controlRuntime.unregisterRecoveryTarget(sm);
    stopCoopSelfCleanup(timers);
    stopLiveUi(liveUi);
    sdk.stopIdleReaper();
    stopTaskOrchestrator(getTaskOrchestrator);
    if (updateChecker && typeof updateChecker.stop === "function") updateChecker.stop();
    ...
    if (timers.externalCodexSyncStop) {
      timers.externalCodexSyncStop();
      timers.externalCodexSyncStop = null;
    }
    loop.stopTimer();
    email.destroy();
    ...
    if (adapters) {
      Object.keys(adapters).forEach(function (vendor) {
        ...
        if (!adapter || adapter.shared || typeof adapter.shutdown !== "function") return;
        try {

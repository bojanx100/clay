...
var handoffSweep = require("./coop-control-handoff-sweep");
...
function localDelegationProblem(session) {
  var policy = projectCoordinatorPolicy(session);
  if (!policy) return null;
  var ref = projectIdentity.normalizeProjectRef(policy.projectRef);
  return "project_coordinator_scope_required: this resident coordinator organizes work for " +
    (ref ? ref.projectId : "its bound project") + ". Request typed project dispatch from Coop. " +
    "Local delegation would create a worker in " +
    "the shared Lead workspace and cannot execute this project's work.";
}
...
function persistSession(sm, session) {
  try {
    if (sm.saveSessionFile(session, { durable: true }) === false) throw new Error("Control-plane session was not saved");
    delete session._controlPlaneSavePending;
  } catch (error) { session._controlPlaneSavePending = true; throw error; }
  return session;
}
...
function reactivate(sm, session, title, role, coopRef, projectRef) {
  ...
  if (before !== after || session._controlPlaneSavePending) persistSession(sm, session);
  return session;
}
...
function ensureControlPlane(sm, projects) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    if (coordinator) {
      ...
      // The Class B recoverable handoff trigger's production call site. Removing
      // this line is what test/coop-control-handoff-trigger-wiring.test.js
      // exists to catch: the trigger would keep passing every one of its own
      // unit tests while never firing again, which is the exact failure mode
      // that left the handoff controller dark for eight days.
      handoffs.push(handoffSweep.sweepHandoffTriggers(sm, item, coordinator, {
        sendToSession: item.sendToSession, trigger: item.handoffTrigger,
      }));
    }
  }
...
function bindTask(sm, root, task, sessionRef) {
  ...
  if (task.status === "running" && task.workerSessionRef && task.workerSessionRef.projectId === ref.projectId &&
      task.workerSessionRef.sessionStorageId === ref.sessionStorageId) return true;
  var previous = Object.assign({}, task);
  var events = (root.orchestrationEvents || []).slice();
  ...
  try { persistSession(sm, root); }
  catch (error) {
    Object.keys(task).forEach(function (key) { delete task[key]; });
    Object.assign(task, previous);
    root.orchestrationEvents = events;
    return false;
  }
  return true;
}
...
function removePreparedTask(sm, root, task) {
  if (task && task.projectAssignment) return false;
...
function completeTask(sm, root, request, status, summary) {
  ...
  // The binding revision identifies an immutable attempt. Transport closes it
  // before inbox delivery, so the destination normally sees the same completion
  // again. A later attention report cannot reopen that terminal attempt either.
  if (task.status === "completed" || task.status === "failed" ||
      task.status === "cancelled" || task.status === "superseded") return true;
  var resultSummary = String(summary || "").trim().slice(0, 4000);
  if (task.status === next && task.resultSummary === resultSummary) return true;
  task.status = next;
...
function reconcileSettledTask(sm, root, request, evidence) {
  return require("./coop-control-settled-task-reconciliation").reconcileTask(
    root, taskForRequest(root, request), evidence,
    function () { persistSession(sm, root); });
}
...
module.exports = {
  ...
  controlPlaneDependencies: controlPlaneDependencies,
  completeTask: completeTask,
  ensureControlPlane: ensureControlPlane,
  ensureProjectCoordinator: ensureProjectCoordinator,
  handoffObservations: handoffSweep.handoffObservations,
  localDelegationProblem: localDelegationProblem,
  prepareTask: prepareTask,
  sweepHandoffTriggers: handoffSweep.sweepHandoffTriggers,
  projectCoordinatorFor: projectCoordinatorFor,
  reconcileSettledTask: reconcileSettledTask,
  projectCoordinatorPolicy: projectCoordinatorPolicy,
  removePreparedTask: removePreparedTask,
  storageId: storageId,
  taskForRequest: taskForRequest,
  taskClientRef: taskClientRef,
};

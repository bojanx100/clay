...
- Always commit and push completed work. Commit on a dedicated worktree branch, then run `node scripts/push-bojan.js` from that worktree; the versioned pre-push guard rejects direct pushes to `bojan`, and the wrapper rebases, pushes only to `bojan`, then fast-forwards the clean main checkout. Never commit directly on local `bojan`, and never commit or push to `master`/`main`.
- Every new branch/worktree must have an intended landing target (normally `bojan`) and an owning task. Completion includes deleting its local branch and worktree after the push is verified. `push-bojan.js` attempts this automatically; if active processes prevent removal, exit that worktree and run `node scripts/cleanup-worktree.js <path>` from the primary checkout. Keep `main`, `bojan`, and UI-overhaul worktrees. Reconcile unfinished work toward landing; delete stale/superseded work only after recording the replacement commit or evidence that its changes were adopted. Age alone is not proof. Never delete another agent's dirty or active worktree; record its owner and remaining work instead.

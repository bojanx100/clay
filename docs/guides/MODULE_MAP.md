...
| `cross-project-delivery.js` | Atomic bounded outbox/inbox/cursor persistence, immediate reclamation of acknowledged idle cursors, acknowledgement, ordered replay/retry, and dead-letter state |
| `cross-project-delivery-envelope.js` | Typed delivery envelope validation, bounded payloads, and stable replay identity |
...
| `scripts/lead-tick-state.js` | One-shot gatherer for every Lead/Coop tick state source (leadMode, owner requests, current bindings, loose items, ledger, provider health, budget) in a single process and a single model round trip, projected to the fields the tick acts on; `--refresh` warms the usage cache off the foreground turn |

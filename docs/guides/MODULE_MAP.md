...
| `coop-thread-lifecycle.js` | Thread migration, state transitions, and correction history | Exploring/Parked/Handed off/Closed states, reversible hidden records, bounded lifecycle undo snapshots, ThreadRef aliases, and strict implementation-intent detection |
| `coop-main-ingress-recovery.js` + `coop-threads-implementation-recovery.js` + `coop-recovered-thread-admission.js` | Finite recovered-Thread implementation admission | Digest-bound Voice ingress 360 and Threads ingress 371 repairs, exact canonical ThreadRef aliases, Clay-only implementation decisions, idempotent startup activation, and fail-closed replay routing |
...
| `project-automation-execution-authorization.js` | Typed authorization receipt for policy-autonomous candidate admission | Strictly binds candidate, policy, eligibility, source, scope, and exact ProjectRef evidence; re-reads current project state and reruns pure authority before allowing execution |

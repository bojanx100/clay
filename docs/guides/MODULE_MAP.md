...
| `coop-owner-requests.js` | Durable owner-request ledger keyed by Coop ingress id | Reference-only request/response records, the single markAnswered transition (starting work is never answering), one canonical coordinator per (TopicRef, ProjectRef), fan-in, and idempotent topic-closure reconciliation. Persisted under `~/.clay/lead/`; write paths are injection-only |
...
| `coop-owner-request-query.js` | Owner-facing projection of the execution flow | Pure join of requests, coordinator claims and the session ledger into unanswered requests plus topic → projects → coordinators → workers; hidden, missing and terminal sessions never count as working |

...
| `voice-questions.js` (server) | `voice_question_state_request`, `voice_question_answer` | Discovers live Claude/Codex questions and permission/plan decisions and validates the exact session, instance revision and answer before calling the existing response path |
| `voice-decisions.js` | (called by Voice question handling) | Reads current permission details and plan contents, prepares spoken choices and translates one-request decisions to the existing provider callback |
| `voice-turn-replay.js` | `voice_turn_state_request` | Persists actual dispatch markers and reads only the exact client turn for reconnect recovery, separating completion, failure, interruption and unknown state |
...
| `coop-sidebar-execution-state.js` | Read-only sidebar execution state | Requires a current provider turn for running labels; joins exact durable binding and worker failure evidence, keeps waiting/review work visible, and preserves terminal/superseded history |
...
| `coop-control-role-context.js` + `coop-control-role-prompt.js` | Resident control role and project context | Resolves exact registered role and canonical ProjectRef; joins refreshed project instructions, durable assignments, worker state and pending reports for provider input without changing conversation cwd or granting authority |
| `coop-project-instructions.js` + `coop-project-instruction-refs.js` | Coordinator instruction closure | Discovers root/local rules, follows explicit required Markdown references and imports, handles cycles, rejects missing/ambiguous/outside/oversized required context, and indexes supporting documents separately |
| `coop-coordinator-work-context.js` | Reconstruct coordinator obligations | Reads durable admitted assignment scope, TaskRefs, dependencies, exact bound target workers, pending reports and bounded recent outcomes/events on every turn |
| `coop-coordinator-context-delivery.js` | Provider input receipt and freshness | Resolves context at actual initial/warm/buffered dispatch, saves an owner-visible receipt, and guards new assignment acceptance using the exact live handle and current rule digest; persisted receipts grant no authority |
| `coop-coordinator-compaction.js` | Resident provider renewal | Keeps the coordinator's Clay identity and assignment graph while replacing its provider conversation after the prior stream drains |
| `coop-coordinator-transparency.js` + client `sidebar-coordinator-details.js` | Owner-visible coordinator status | Projects current activity, pending work/reports, recent recorded events and instruction delivery details through existing session ACLs; renders shared desktop/mobile details with ephemeral expansion state |
...
| `project-primitive-launch-evidence.js` | Immutable per-session original launch receipts; verifies historical eligibility against the actual session and mints process-local proofs while current receipts retain normal freshness checks |
...
| `project-automation-reconsideration.js` | Normalizes explicit owner reconsideration evidence and matches only the exact verified completed binding; unrelated completed or active work remains protected from duplicate execution |
...
| `yoke/adapters/codex-user-input.js` | Converts Codex structured questions into Clay forms and translates answers back to the distinct Codex protocol; cancels live questions on provider resolution or turn termination |
...
| `server-tui-hooks.js` | Claude TUI notification hook and auto-approve allow-list installation for single-user and OS-user modes; comparison instances can disable native settings management |
...
| `voice-conversation.js` + `voice-conversation-controller.js` | Dedicated browser Voice conversation UI and testable turn controller. ~~It is available only from the canonical Voice Thread.~~ Retracted 2026-09-06: Lead on routes Voice to canonical Coop; Lead off routes to the selected ordinary session. Recording captures the destination, session/Lead changes stop audio, and final speech sends automatically after a pause. Actual user-turn dispatch identifies the reply; complete sanitized speech plays in chunks and listening resumes. |
| `voice-conversation-routing.js` + `voice-sanitization.js` | ~~Coop-only route staging~~ (retracted 2026-09-06). Immutable recording destinations, current Lead/session guards, and the outbound speech redaction boundary. Voice never uses a later composer lens, and browser TTS never receives likely credentials or code blocks. |
| `voice-interruption.js` | Playback-only speech recognizer for “Coop pause”; ignores echoed command text and never forwards ordinary speech or work decisions |
| `voice-questions.js` (client) | Spoken question options, free-text answers, readback, explicit submission and reconciliation against live request revisions. Publishes question UI state through the store. |
| `voice-conversation-send.js` | Sends voice through the authenticated message/queue protocol with the captured destination and selected provider; checks the actual socket result without modifying typed drafts or staged attachments. |

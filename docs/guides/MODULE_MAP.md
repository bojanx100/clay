...
| `project-automation-authority.js` | PURE decision function for what project automation may do: `decideAutomation({ leadMode, policy, action, itemClass, claim, completion, approval, now })` → `execute` / `propose` / `deny`. Lead mode OFF is an audited pass-through (roadmap §1.1 additive-only). Externally visible actions need claim + completion evidence + approval, with one narrow owner-triggered `comment` carve-out that never extends to merge/close |
| `automation-claim-leases.js` | Restart-safe expiring claim leases, unique per `(projectId, key)`, in `~/.clay/lead/automation-claims.json`. Typed ProjectRef only; temp+fsync+rename persistence with rollback; malformed state fails closed. This is what stops duplicate launches |
| `project-automation-gate.js` | Composes lead-mode + policy + leases + authority + audit into the single call the runtime makes (`evaluateLaunch`, `evaluateExternal`, `evaluateDiscovery`, `reconcileClaims`). Only a FRESHLY created lease authorizes a launch, which closes the overlapping-tick hole. **Scope:** the gate governs AUTO-LAUNCHED sessions only (`taskLauncher.autoLaunch`). Manual `/launch` and the token-gated `POST /api/task-launch` are owner/operator-initiated and deliberately keep legacy behavior, so a project's `externalActions` policy constrains automation, not the owner |

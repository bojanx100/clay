...
| `coop-topic-connection.js` | `coop_topic_projection_request`, `coop_topic_select`, and canonical event resolution | ACL-safe topic projection, per-WebSocket lens selection, filtered canonical replay (closed Done topics replay their own indexed history), and exact original-event drill-through; re-exports the mutation handlers from `coop-topic-management.js` |
| `coop-topic-management.js` | `coop_topic_rename/move/merge/split/close/reopen/link_execution`, `coop_topic_disposition`, `coop_action_decision` | Owner-gated durable mutations (canonical-owner ACL, fail-closed multi-user), stale-state and disposition-revision preconditions, durable request dedup, and all-viewer projection fan-out |
...
| `global-coop-coordinator-tree.js` | Global Coop sidebar execution hierarchy | Exact bounded projection of persistent project coordinators → current task coordinators → current bound worker sessions, with canonical TaskRef/SessionRef ownership and active/attention visibility gates |
...
| `project-coordinator-hierarchy.js` | One durable reusable project coordinator per ProjectRef, archived-root reactivation, bounded child task-coordinator linking/unlinking, and terminal task→project rollup |
...
| `project-task-orchestrator-external.js` | (called by task orchestrator) | Local external-task routing plus canonical durable project-root reuse, child task-coordinator/direct-leaf creation, messaging, retry, stop, and restart recovery |
...
| `sidebar-coop-topic-close.js` | Conventional per-topic Close action behind an explicit confirmation, plus Reopen for closed topics. Cancel is a no-op; confirm sends exactly one `coop_topic_close`; a closed row offers `coop_topic_reopen` instead of a dead second Close. Transport arrives via `options.send`, so the module stays off the app connection graph |

...
| `project-automation-gate.js` | Composes lead-mode + policy + leases + authority + audit into the single call the runtime makes (`evaluateLaunch`, `evaluateExternal`, `evaluateDiscovery`, `reconcileClaims`). Only a FRESHLY created lease authorizes a launch, which closes the overlapping-tick hole |

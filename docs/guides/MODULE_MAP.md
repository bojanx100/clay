...
| `project-sessions.js` | `new_session`, `switch_session`, `delete_session`, `rename_session`, `resume_session`, `fork_session`, `rewind_*`, `permission_response`, `elicitation_response`, `set_model`, `set_effort`, `set_thinking`, `set_betas`, `set_*_mode`, `stop`, `stop_task`, `kill_process`, `ask_user_response`, `input_sync`, `cursor_*`, `text_select`, `push_subscribe`, `load_more_history`, `set_session_visibility`, `set_mate_dm`, `list_git_accounts`, `get_project_git_account`, `set_project_git_account` | Session lifecycle, permissions, live session controls, project git account settings |
